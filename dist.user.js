...
`;class v{bot;image;width;exactColor;static async fromJSON(o,s){let a=new Image;return a.src=s.url.startsWith("http")?await fetch(s.url,{cache:"no-store"}).then((i)=>i.blob()).then((i)=>URL.createObjectURL(i)):s.url,await T(a,["load"],["error"]),new v(o,a,s.width,s.exactColor)}canvas=document.createElement("canvas");context=this.canvas.getContext("2d");pixels;colors=new Map;resolution;get height(){return this.width/this.resolution|0}set height(o){this.width=o*this.resolution|0}constructor(o,s,a=s.naturalWidth,i=!1){this.bot=o;this.image=s;this.width=a;this.exactColor=i;if(i)this.resolution=1,this.width=1000;else this.resolution=this.image.naturalWidth/this.image.naturalHeight;this.update()}update(){this.canvas.width=this.width,this.canvas.height=this.height,this.colors.clear();let o=new Map;for(let a=1;a<64;a++)o.set(W[a],[a,a]);this.context.imageSmoothingEnabled=!1,this.context.imageSmoothingQuality="low",this.context.drawImage(this.image,0,0,this.canvas.width,this.canvas.height),this.pixels=Array.from({length:this.canvas.height},()=>Array(this.canvas.width));let s=this.context.getImageData(0,0,this.canvas.width,this.canvas.height).data;for(let a=0;a<this.canvas.height;a++)for(let i=0;i<this.canvas.width;i++){let r=(a*this.canvas.width+i)*4,l=s[r],e=s[r+1],t=s[r+2],n=s[r+3],c=l,g=e,f=t,d=`${c},${g},${f}`;if(this.exactColor){this.pixels[a][i]=n<100?0:W.indexOf(d);continue}let w,m;if(n<100)w=m=0;else if(o.has(d))[w,m]=o.get(d);else{let u=1/0,b=1/0;for(let k=0;k<B.length;k++){let z=B[k],D=Go(Qo(c,g,f),z,0);if(D<u)u=D,w=k;if(D<b)b=D,m=k}o.set(d,[w,m])}if(w!==0)this.context.fillStyle=`oklab(${B[w][0]*100}% ${B[w][1]} ${B[w][2]})`,this.context.fillRect(i,a,1,1);this.pixels[a][i]=w;let h=this.colors.get(m);if(h)h.amount++;else this.colors.set(m,{color:m,amount:1,realColor:m})}}toJSON(){let o=document.createElement("canvas");return o.width=this.image.naturalWidth,o.height=this.image.naturalHeight,o.getContext("2d").drawImage(this.image,0,0),{url:o.toDataURL("image/webp",1),width:this.width,exactColor:this.exactColor}}}function Da(){let o=[K,...xo];for(let s=0;s<o.length;s++){let a=o[s],i=localStorage.getItem(a);if(!i)continue;return{json:i,key:a}}return}function Zo(){let o=Da();if(!o)return;let s;try{if(s=JSON.parse(o.json),typeof s!=="object")throw Error("NOT VALID SAVE");if(s.version===1){let a=s.widget;if(a)s.images=a.images,s.strategy=a.strategy,delete s.widget}if(o.key!==K)localStorage.setItem(K,o.json)}catch{localStorage.removeItem(o.key),s=void 0}return s}var Bo;function M(o,s=!1){if(clearTimeout(Bo),s)localStorage.setItem(K,JSON.stringify(o));else Bo=setTimeout(()=>{localStorage.setItem(K,JSON.stringify(o))},600)}var P=1000,Ha=2048,R=P*Ha,L=[],O=[],Fa=Date.now();function y(o){L.push(o),O.push({id:Fa++,latitude:(2*Math.atan(Math.exp(-(o.y/R*(2*Math.PI)-Math.PI)))-Math.PI/2)*180/Math.PI,longitude:(o.x/R*(2*Math.PI)-Math.PI)*180/Math.PI,name:"KGLACER_MACRO_FAVORITE"})}y({x:R/3|0,y:R/3|0});y({x:R/3*2|0,y:R/3*2|0});function Z(o){let[s,a]=o.style.transform.slice(32,-31).split(", ").map((i)=>Number.parseFloat(i));return{x:s,y:a}}class H{bot;static fromJSON(o,s){return new H(o,...s)}static fromScreenPosition(o,s){let{anchorScreenPosition:a,pixelSize:i,anchorWorldPosition:r}=o.findAnchorsForScreen(s);return new H(o,r.x+(s.x-a.x)/i|0,r.y+(s.y-a.y)/i|0)}globalX=0;globalY=0;get tileX(){return this.globalX/P|0}set tileX(o){this.globalX=o*P+this.x}get tileY(){return this.globalY/P|0}set tileY(o){this.globalY=o*P+this.y}get x(){return this.globalX%P}set x(o){this.globalX=this.tileX*P+o}get y(){return this.globalY%P}set y(o){this.globalY=this.tileY*P+o}anchor1Index;anchor2Index;get pixelSize(){return(Z(this.bot.$stars[this.anchor2Index]).x-Z(this.bot.$stars[this.anchor1Index]).x)/(L[this.anchor2Index].x-L[this.anchor1Index].x)}constructor(o,s,a,i,r){this.bot=o;if(i===void 0||r===void 0)this.globalX=s,this.globalY=a;else this.globalX=s*P+i,this.globalY=a*P+r;this.updateAnchor()}updateAnchor(){this.anchor1Index=0,this.anchor2Index=1;let o=1/0,s=1/0;for(let a=0;a<L.length;a++){let{x:i,y:r}=L[a];if(i<this.globalX&&r<this.globalY){let l=this.globalX-i+(this.globalY-r);if(l<o)o=l,this.anchor1Index=a}else if(i>this.globalX&&r>this.globalY){let l=i-this.globalX+(r-this.globalY);if(l<s)s=l,this.anchor2Index=a}}}toScreenPosition(){let o=L[this.anchor1Index],s=Z(this.bot.$stars[this.anchor1Index]);return{x:(this.globalX-o.x)*this.pixelSize+s.x,y:(this.globalY-o.y)*this.pixelSize+s.y}}getMapColor(){return this.bot.mapsCache.get(this.tileX+"/"+this.tileY).pixels[this.y][this.x]}setMapColor(o){let s=this.bot.mapsCache.get(this.tileX+"/"+this.tileY);if(!s)return;let a=s.pixels[this.y];if(!a)return;a[this.x]=o}scrollScreenTo(){let{x:o,y:s}=this.toScreenPosition();this.bot.moveMap({x:o-window.innerWidth/3,y:s-window.innerHeight/3})}clone(){return new H(this.bot,this.tileX,this.tileY,this.x,this.y)}toJSON(){return[this.globalX,this.globalY]}}function Na(o){let s=[];for(let{x:a,y:i}of o.iterate){let r=o.pixels[i]?.[a]??0;if(o.disabledColors.has(r))continue;let l=o.readMapColor(a,i);if(r!==l&&(o.drawTransparentPixels||r!==0))s.push({x:a,y:i,color:r})}return s}class U extends ${bot;position;pixels;strategy;opacity;drawTransparentPixels;drawColorsInOrder;skipUnavailableColors;colors;lock;static PREVIEW_MASK_BASE_WIDTH=96;static PREVIEW_MASK_BASE_HEIGHT=96;static async fromJSON(o,s){return new U(o,H.fromJSON(o,s.position),await v.fromJSON(o,s.pixels),s.strategy,s.opacity,s.drawTransparentPixels,s.drawColorsInOrder,s.skipUnavailableColors,s.colors,s.lock)}element=document.createElement("div");tasks=[];moveInfo;$canvas;$colorsDialog;$colorsDialogList;$colorSearch;$openColors;$openPreview;$toggleAllColors;$closeColors;$closePreview;$delete;$drawColorsInOrder;$drawTransparent;$skipUnavailable;$export;$lock;$opacity;$progressLine;$progressText;$previewDialog;$previewDialogList;$previewStrategySelect;$resetSize;$resetSizeSpan;$settings;$strategy;$topbar;$wrapper;colorDialogDragState;suppressNextColorDialogBackdropClick=!1;previewCacheSignature;previewSequenceCache=new Map;previewAnimations=new WeakMap;previewAnimationHandles=new Set;constructor(o,s,a,i="SPIRAL_FROM_CENTER",r=50,l=!1,e=!1,t=!0,n=[],c=!1){super();this.bot=o;this.position=s;this.pixels=a;this.strategy=i;this.opacity=r;this.drawTransparentPixels=l;this.drawColorsInOrder=e;this.skipUnavailableColors=t;this.colors=n;this.lock=c;this.element.innerHTML=Wo,this.element.classList.add("wimage"),x(this.element),document.body.append(this.element),this.populateElementsWithSelector(this.element,{$colorsDialog:".colors-dialog",$colorsDialogList:".colors-dialog-list",$colorSearch:".color-search",$openColors:".open-colors",$openPreview:".open-preview",$toggleAllColors:".toggle-all-colors",$closeColors:".close-colors",$closePreview:".close-preview",$delete:".delete",$drawColorsInOrder:".draw-colors-in-order",$drawTransparent:".draw-transparent",$skipUnavailable:".skip-unavailable",$export:".export",$lock:".lock",$opacity:".opacity",$progressLine:".wprogress div",$progressText:".wprogress span",$previewDialog:".preview-dialog",$previewDialogList:".preview-dialog-list",$previewStrategySelect:".preview-strategy-select",$resetSize:".reset-size",$settings:".wform",$strategy:".strategy",$topbar:".wtopbar",$wrapper:".wrapper"}),this.$resetSizeSpan=this.$resetSize.querySelector("span"),this.$canvas=this.pixels.canvas,this.$wrapper.prepend(this.pixels.canvas),document.body.append(this.$colorsDialog,this.$previewDialog),this.registerEvent(this.$strategy,"change",()=>{this.strategy=this.$strategy.value,this.$previewStrategySelect.value=this.strategy,M(this.bot)}),this.registerEvent(this.$previewStrategySelect,"change",()=>{this.$strategy.value=this.$previewStrategySelect.value,this.$strategy.dispatchEvent(new Event("change")),this.renderStrategyPreviewSamples()}),this.registerEvent(this.$opacity,"input",()=>{this.opacity=this.$opacity.valueAsNumber,this.$opacity.style.setProperty("--val",this.opacity+"%"),this.update(),M(this.bot)}),this.$opacity.style.setProperty("--val",this.opacity+"%"),this.registerEvent(this.$resetSize,"click",()=>{this.pixels.width=this.pixels.image.naturalWidth,this.pixels.update(),this.updateColors(),this.update(),M(this.bot)}),this.registerEvent(this.$drawTransparent,"click",()=>{this.drawTransparentPixels=this.$drawTransparent.checked,M(this.bot)}),this.registerEvent(this.$skipUnavailable,"click",()=>{this.skipUnavailableColors=this.$skipUnavailable.checked,this.updateTasks(),M(this.bot)}),this.registerEvent(this.$drawColorsInOrder,"click",()=>{this.drawColorsInOrder=this.$drawColorsInOrder.checked,M(this.bot)}),this.registerEvent(this.$lock,"click",()=>{this.lock=!this.lock,this.update(),M(this.bot)}),this.registerEvent(this.$delete,"click",this.destroy.bind(this)),this.registerEvent(this.$openColors,"click",()=>{this.openColorPanel()}),this.registerEvent(this.$openPreview,"click",()=>{this.openPreviewPanel()}),this.registerEvent(this.$closeColors,"click",()=>{this.closeDialog(this.$colorsDialog)}),this.registerEvent(this.$closePreview,"click",()=>{this.closeDialog(this.$previewDialog)}),this.registerEvent(this.$colorsDialog.querySelector(".colors-dialog-head"),"pointerdown",this.startColorDialogDrag.bind(this)),this.registerEvent(document,"pointermove",this.moveColorDialog.bind(this),{passive:!1}),this.registerEvent(document,"pointerup",this.stopColorDialogDrag.bind(this)),this.registerEvent(document,"pointercancel",this.stopColorDialogDrag.bind(this)),this.registerEvent(this.$colorsDialog,"click",(g)=>{if(this.suppressNextColorDialogBackdropClick){this.suppressNextColorDialogBackdropClick=!1;return}if(g.target===this.$colorsDialog)this.closeDialog(this.$colorsDialog)}),this.registerEvent(this.$previewDialog,"click",(g)=>{if(g.target===this.$previewDialog)this.closeDialog(this.$previewDialog)}),this.registerEvent(this.$colorSearch,"input",()=>{this.updateColors()}),this.registerEvent(this.$toggleAllColors,"change",()=>{let g=!this.$toggleAllColors.checked;for(let f of this.colors)f.disabled=g||void 0;this.syncColorBulkToggle(),this.updateTasks(),this.updateColors(),M(this.bot)}),this.registerEvent(this.$export,"click",this.export.bind(this)),this.registerEvent(this.$topbar,"mousedown",this.moveStart.bind(this)),this.registerEvent(this.$canvas,"mousedown",this.moveStart.bind(this)),this.registerEvent(document,"mouseup",this.moveStop.bind(this)),this.registerEvent(document,"mousemove",this.move.bind(this));for(let g of this.element.querySelectorAll(".resize"))this.registerEvent(g,"mousedown",this.resizeStart.bind(this));this.update(),this.updateColors()}toJSON(){return{pixels:this.pixels.toJSON(),position:this.position.toJSON(),strategy:this.strategy,opacity:this.opacity,drawTransparentPixels:this.drawTransparentPixels,drawColorsInOrder:this.drawColorsInOrder,skipUnavailableColors:this.skipUnavailableColors,colors:this.colors,lock:this.lock}}updateTasks(){this.tasks.length=0;let o=this.position.clone(),s=new Set,a=new Map;for(let r=0;r<this.colors.length;r++){let l=this.colors[r];if(l.disabled||this.skipUnavailableColors&&this.bot.unavailableColors.has(l.realColor))s.add(l.realColor);a.set(l.realColor,r)}let i=Na({pixels:this.pixels.pixels,drawTransparentPixels:this.drawTransparentPixels,disabledColors:s,iterate:this.strategyPositionIterator(),readMapColor:(r,l)=>{return o.globalX=this.position.globalX+r,o.globalY=this.position.globalY+l,o.getMapColor()}});for(let r=0;r<i.length;r++){let l=i[r];o.globalX=this.position.globalX+l.x,o.globalY=this.position.globalY+l.y,this.tasks.push({position:o.clone(),color:l.color})}if(this.drawColorsInOrder)this.tasks.sort((r,l)=>(a.get(r.color)??0)-(a.get(l.color)??0));this.update(),this.bot.widget.update()}update(){let{x:o,y:s}=this.position.toScreenPosition(),a=this.position.pixelSize*this.pixels.width,i=this.position.pixelSize*this.pixels.height;this.element.style.transform=`translate3d(${o.toFixed(3)}px, ${s.toFixed(3)}px, 0)`,this.element.style.width=`${a}px`,this.element.style.height=`${i}px`,this.$canvas.style.opacity=`${this.opacity}%`,this.element.classList.remove("hidden"),this.$resetSizeSpan.textContent=this.pixels.width.toString(),this.$strategy.value=this.strategy,this.$opacity.valueAsNumber=this.opacity,this.$drawTransparent.checked=this.drawTransparentPixels,this.$drawColorsInOrder.checked=this.drawColorsInOrder,this.$skipUnavailable.checked=this.skipUnavailableColors;let r=this.pixels.pixels.length*this.pixels.pixels[0].length,l=Math.max(0,r-this.tasks.length),e=r>0?l/r*100|0:0;this.$progressText.textContent=`${l}/${r} ${e}% ETA: ${this.tasks.length/120|0}h`,this.$progressLine.style.transform=`scaleX(${e/100})`,this.$canvas.classList[this.lock?"add":"remove"]("no-pointer-events");for(let t of this.element.querySelectorAll(".resize"))t.classList[this.lock?"add":"remove"]("no-pointer-events");this.$lock.classList[this.lock?"add":"remove"]("locked")}exportImage(){this.export()}destroy(){super.destroy(),this.element.remove(),this.$colorsDialog.remove(),this.$previewDialog.remove(),fo(this.bot.images,this),this.bot.widget.update(),M(this.bot)}openColorPanel(){if(this.$colorsDialog.open){this.$colorSearch.focus();return}this.$colorsDialog.style.position="fixed",this.$colorsDialog.style.left="",this.$colorsDialog.style.top="",this.$colorsDialog.style.margin="auto",this.$colorsDialog.showModal(),this.$colorSearch.focus()}openPreviewPanel(){if(this.syncPreviewStrategySelect(),this.$previewDialog.open){this.renderStrategyPreviewSamples();return}this.$previewDialog.style.position="fixed",this.$previewDialog.style.left="",this.$previewDialog.style.top="",this.$previewDialog.style.margin="auto",this.$previewDialog.showModal(),this.renderStrategyPreviewSamples()}syncPreviewStrategySelect(){if(!this.$previewStrategySelect.childElementCount){let o=document.createDocumentFragment();for(let s of this.$strategy.options){let a=document.createElement("option");a.value=s.value,a.textContent=s.textContent,o.append(a)}this.$previewStrategySelect.append(o)}this.$previewStrategySelect.value=this.strategy}closeDialog(o){if(!o.open)return;if(o===this.$previewDialog)this.stopPreviewAnimations();if(typeof o.requestClose==="function")o.requestClose();else o.close()}stopPreviewAnimations(){for(let o of this.previewAnimationHandles)cancelAnimationFrame(o);this.previewAnimationHandles.clear()}startColorDialogDrag(o){if(o.button!==0)return;if(o.target?.closest("button,input,select,textarea,a,label"))return;let a=this.$colorsDialog.getBoundingClientRect();this.colorDialogDragState={pointerId:o.pointerId,offsetX:o.clientX-a.left,offsetY:o.clientY-a.top,moved:!1},o.preventDefault()}moveColorDialog(o){if(!this.colorDialogDragState)return;if(o.pointerId!==this.colorDialogDragState.pointerId)return;let s=this.$colorsDialog.getBoundingClientRect(),a=Math.max(8,window.innerWidth-s.width-8),i=Math.max(8,window.innerHeight-s.height-8),r=Math.min(a,Math.max(8,o.clientX-this.colorDialogDragState.offsetX)),l=Math.min(i,Math.max(8,o.clientY-this.colorDialogDragState.offsetY));if(!this.colorDialogDragState.moved&&(Math.abs(o.movementX)>0||Math.abs(o.movementY)>0))this.colorDialogDragState.moved=!0;this.$colorsDialog.style.left=`${Math.round(r)}px`,this.$colorsDialog.style.top=`${Math.round(l)}px`,o.preventDefault()}stopColorDialogDrag(o){if(!this.colorDialogDragState)return;if(o.pointerId!==this.colorDialogDragState.pointerId)return;if(this.colorDialogDragState.moved)this.suppressNextColorDialogBackdropClick=!0;this.colorDialogDragState=void 0}renderStrategyPreviewSamples(){this.stopPreviewAnimations(),this.invalidatePreviewCacheIfNeeded();let o=this.$strategy.value;this.$previewDialogList.innerHTML="";let s=document.createDocumentFragment(),a=document.createElement("article");a.className="preview-card";let i=document.createElement("strong");i.textContent=this.getStrategyLabel(o);let r=document.createElement("canvas");r.className="preview-canvas",r.width=156,r.height=156,this.paintStrategyPreview(r,o),a.append(i,r),s.append(a),this.$previewDialogList.append(s)}invalidatePreviewCacheIfNeeded(){let o=this.colors.map((a,i)=>`${i}:${a.realColor}:${a.disabled?1:0}`).join("|"),s=`${this.pixels.width}x${this.pixels.height}:${this.pixels.image.src.length}:${this.drawColorsInOrder?1:0}:${o}`;if(this.previewCacheSignature===s)return;this.previewCacheSignature=s,this.previewSequenceCache.clear()}getStrategyLabel(o){switch(o){case"RANDOM":return p("random");case"HUMANIZED":return p("humanized");case"HUMAN_SOFT_DITHER":return p("humanSoftDither");case"HUMAN_PATCHY":return p("humanPatchy");case"HUMAN_SWEEP_ARCS":return p("humanSweepArcs");case"HUMAN_MICRO_CORRECTIONS":return p("humanMicroCorrections");case"HUMAN_JITTER_FILL":return p("humanJitterFill");case"HUMAN_CORNER_BIAS":return p("humanCornerBias");case"HUMAN_LONG_STROKES":return p("humanLongStrokes");case"HUMAN_TAP_CLUSTERS":return p("humanTapClusters");case"HUMAN_MESSY_SPIRAL":return p("humanMessySpiral");case"HUMAN_DRUNK_WALK":return p("humanDrunkWalk");case"HUMAN_NOISE_CLOUD":return p("humanNoiseCloud");case"HUMAN_PATCH_JUMP":return p("humanPatchJump");case"HUMAN_HESITANT_LINES":return p("humanHesitantLines");case"HUMAN_OVERLAP_SWEEPS":return p("humanOverlapSweeps");case"HUMAN_WOBBLE_DRIFT":return p("humanWobbleDrift");case"HUMAN_GAP_RECOVERY":return p("humanGapRecovery");case"HUMAN_STAIRCASE":return p("humanStaircase");case"HUMAN_EDGE_HUGGER":return p("humanEdgeHugger");case"HUMAN_BLOBS":return p("humanBlobs");case"HUMAN_BACKTRACK":return p("humanBacktrack");case"HUMAN_SHAKY_DIAGONAL":return p("humanShakyDiagonal");case"HUMAN_LATE_FIXES":return p("humanLateFixes");case"ZIGZAG":return p("zigzag");case"BRUSH_STROKES":return p("brushStrokes");case"DIAGONAL_BRUSH":return p("diagonalBrush");case"DOWN":return p("down");case"UP":return p("up");case"LEFT":return p("left");case"RIGHT":return p("right");case"SPIRAL_FROM_CENTER":return p("spiralOut");case"SPIRAL_TO_CENTER":return p("spiralIn");case"SCRIBBLE":return p("scribble");case"CROSSHATCH":return p("crosshatch");case"WAVE_SWEEP":return p("waveSweep");case"SCATTERED_LINES":return p("scatteredLines");case"CONTOUR_JITTER":return p("contourJitter");case"SPIRAL_WOBBLE":return p("spiralWobble");case"CLUSTER_BURSTS":return p("clusterBursts");case"ORBITAL":return p("orbital");case"FLOW_FIELD":return p("flowField");case"EDGE_IN":return p("edgeIn");default:return o}}paintStrategyPreview(o,s){let a=o.getContext("2d");if(!a)return;a.fillStyle="#0f1526",a.fillRect(0,0,o.width,o.height);let i=this.getSampledImagePreviewData(),r=this.getCachedPreviewSequence(s,i.mask,i.width,i.height),l=Math.min(o.width/i.width,o.height/i.height),e=(o.width-i.width*l)/2,t=(o.height-i.height*l)/2,n=this.previewAnimations.get(o);if(n)cancelAnimationFrame(n),this.previewAnimationHandles.delete(n);let c=(u)=>{let b=requestAnimationFrame((k)=>{this.previewAnimationHandles.delete(b),u(k)});return this.previewAnimationHandles.add(b),b},g=(u)=>{a.fillStyle="#0f1526",a.fillRect(0,0,o.width,o.height);for(let b=0;b<Math.min(u,r.length);b++){let k=r[b],z=i.colors.get(`${k.x}:${k.y}`)??0;if(!z)continue;a.fillStyle=So(z),a.fillRect(e+k.x*l,t+k.y*l,Math.max(1,l),Math.max(1,l))}},f=Math.min(3400,Math.max(900,r.length*8)),w=f+220,m=(u,b)=>{if(!this.$previewDialog.open)return;let k=(b-u)%w,z=Math.min(1,k/f),D=z*z*(3-2*z);g(Math.floor(r.length*D));let C=c((N)=>{m(u,N)});this.previewAnimations.set(o,C)},h=performance.now();m(h,h)}getCachedPreviewSequence(o,s,a=this.pixels.width,i=this.pixels.height){let r=this.colors.map((n,c)=>`${c}:${n.realColor}:${n.disabled?1:0}`).join("|"),l=`${o}:${a}x${i}:${s.length}:${this.drawColorsInOrder?1:0}:${r}`,e=this.previewSequenceCache.get(l);if(e)return e;let t=a===this.pixels.width&&i===this.pixels.height?this.getExactPreviewSequence(o,s):this.getApproxPreviewSequence(o,s,a);if(this.drawColorsInOrder){let n=new Map;for(let c=0;c<this.colors.length;c++)n.set(this.colors[c].realColor,c);t.sort((c,g)=>(n.get(this.pixels.pixels[c.y]?.[c.x]??0)??0)-(n.get(this.pixels.pixels[g.y]?.[g.x]??0)??0))}return this.previewSequenceCache.set(l,t),t}getExactPreviewSequence(o,s){let a=this.strategy;this.strategy=o;let i=[...this.strategyPositionIterator()];this.strategy=a;let r=new Set(s.map(({x:l,y:e})=>`${l}:${e}`));return i.filter(({x:l,y:e})=>r.has(`${l}:${e}`))}getApproxPreviewSequence(o,s,a){let i=[...s],r=(t,n,c)=>{return(t*73856093+n*19349663+c*83492791>>>0)/4294967296},l=(t,n)=>i.sort((c,g)=>c.x*t+c.y*n-(g.x*t+g.y*n)||c.y-g.y||c.x-g.x),e=i.sort((t,n)=>{if(t.y!==n.y)return t.y-n.y;let c=t.y%2===0?t.x:a-t.x,g=n.y%2===0?n.x:a-n.x;return c-g});switch(o){case"UP":return l(0,-1);case"LEFT":return l(-1,0);case"RIGHT":return l(1,0);case"SPIRAL_FROM_CENTER":case"SPIRAL_TO_CENTER":{let t=a/2,n=Math.max(1,Math.round(i.reduce((c,g)=>c+g.y,0)/Math.max(1,i.length)));return i.sort((c,g)=>{let f=(c.x-t)**2+(c.y-n)**2,d=(g.x-t)**2+(g.y-n)**2;return o==="SPIRAL_FROM_CENTER"?f-d:d-f}),i}case"RANDOM":case"HUMANIZED":case"HUMAN_SOFT_DITHER":case"HUMAN_PATCHY":case"HUMAN_SWEEP_ARCS":case"HUMAN_MICRO_CORRECTIONS":case"HUMAN_JITTER_FILL":case"HUMAN_CORNER_BIAS":case"HUMAN_LONG_STROKES":case"HUMAN_TAP_CLUSTERS":case"HUMAN_MESSY_SPIRAL":case"HUMAN_DRUNK_WALK":case"HUMAN_NOISE_CLOUD":case"HUMAN_PATCH_JUMP":case"HUMAN_HESITANT_LINES":case"HUMAN_OVERLAP_SWEEPS":case"HUMAN_WOBBLE_DRIFT":case"HUMAN_GAP_RECOVERY":case"HUMAN_STAIRCASE":case"HUMAN_EDGE_HUGGER":case"HUMAN_BLOBS":case"HUMAN_BACKTRACK":case"HUMAN_SHAKY_DIAGONAL":case"HUMAN_LATE_FIXES":return i.sort((t,n)=>r(t.x,t.y,o.length)-r(n.x,n.y,o.length));default:return e}}getSampledImagePreviewData(){let o=this.pixels.width,s=this.pixels.height,a=U.PREVIEW_MASK_BASE_WIDTH,i=U.PREVIEW_MASK_BASE_HEIGHT,r=Math.min(1,a/Math.max(1,o),i/Math.max(1,s)),l=Math.max(1,Math.round(o*r)),e=Math.max(1,Math.round(s*r)),t=new Set;for(let f=0;f<this.colors.length;f++){let d=this.colors[f];if(d.disabled)t.add(d.realColor)}let n=new Map,c=new Map;for(let f=0;f<s;f++)for(let d=0;d<o;d++){let w=this.pixels.pixels[f]?.[d]??0;if(!w||t.has(w))continue;let m=Math.min(l-1,Math.floor(d/o*l)),h=Math.min(e-1,Math.floor(f/s*e)),u=`${m}:${h}`;if(!n.has(u))n.set(u,{x:m,y:h});if(!c.has(u))c.set(u,w)}let g=[...n.values()];if(!g.length){let f=this.fallbackPreviewMask();return{width:o,height:s,mask:f,colors:new Map(f.map((d)=>[`${d.x}:${d.y}`,this.pixels.pixels[d.y]?.[d.x]??0]))}}return{width:l,height:e,mask:g,colors:c}}getImagePreviewMask(){let o=this.pixels.width,s=this.pixels.height,a=new Set;for(let r=0;r<this.colors.length;r++){let l=this.colors[r];if(l.disabled)a.add(l.realColor)}let i=[];for(let r=0;r<s;r++)for(let l=0;l<o;l++){let e=this.pixels.pixels[r]?.[l]??0;if(e!==0&&!a.has(e))i.push({x:l,y:r})}return i.length?i:this.fallbackPreviewMask()}fallbackPreviewMask(){let o=[],s=this.pixels.width/2,a=this.pixels.height/2,i=Math.max(4,Math.min(this.pixels.width,this.pixels.height)/2.5);for(let r=0;r<this.pixels.height;r++)for(let l=0;l<this.pixels.width;l++)if((l-s)**2+(r-a)**2<=i**2)o.push({x:l,y:r});return o}applyLocale(){if(x(this.element),this.updateColors(),this.$previewDialog.open)this.renderStrategyPreviewSamples()}colorHex(o){let s=W[o]??"0,0,0",[a=0,i=0,r=0]=s.split(",").map((l)=>Number.parseInt(l,10));return`#${[a,i,r].map((l)=>l.toString(16).padStart(2,"0")).join("")}`}colorKeywords(o){let s=W[o]??"0,0,0",[a=0,i=0,r=0]=s.split(",").map((n)=>Number.parseInt(n,10)),l=Math.max(a,i,r),e=Math.min(a,i,r);if(l-e<15)return["gray","grey","gris","neutral","neutro"];if(a>i+30&&a>r+30)return["red","rojo"];if(i>a+30&&i>r+30)return["green","verde"];if(r>a+30&&r>i+30)return["blue","azul"];if(a>170&&i>120&&r<130)return["orange","naranja"];if(a>170&&i>110&&r>140)return["pink","rosa"];if(a>120&&i<100&&r>120)return["purple","violet","morado"];if(a>130&&i>130&&r<90)return["yellow","amarillo"];return["brown","cafe","marron"]}updateColors(){this.$colorsDialogList.innerHTML="";let o=this.pixels.pixels.length*this.pixels.pixels[0].length;this.$colorsDialogList.setAttribute("aria-label",p("colorPanelResults"));let s=this.$colorSearch.value.trim().toLowerCase();if(this.colors.length!==this.pixels.colors.size||this.colors.some((a)=>!this.pixels.colors.has(a.realColor))){let a=new Map(this.colors.map((i)=>[i.realColor,i]));this.colors=this.pixels.colors.values().toArray().sort((i,r)=>r.amount-i.amount).map((i)=>({realColor:i.realColor,disabled:a.get(i.realColor)?.disabled})),M(this.bot)}this.syncColorBulkToggle();for(let a=0;a<this.colors.length;a++){let i=this.colors[a],r=this.pixels.colors.get(i.realColor),l=!1,e=r.amount/o*100,t=this.colorHex(r.realColor),n=this.colorKeywords(r.realColor),c=this.bot.unavailableColors.has(i.realColor),g=Boolean(i.disabled)||this.skipUnavailableColors&&c,f=()=>{if(this.skipUnavailableColors&&c)return;i.disabled=i.disabled?void 0:!0,d.classList.toggle("disabled",Boolean(i.disabled));let h=d.querySelector(".state");if(h)h.textContent=i.disabled||this.skipUnavailableColors&&c?p("disabled"):p("enabled");this.syncColorBulkToggle(),M(this.bot)},d=document.createElement("button");d.className=`color-chip ${g?"disabled":""}`,d.draggable=!0,d.setAttribute("aria-label",`${p("overlayColors")} #${a+1}: ${t.toUpperCase()}`),d.innerHTML=`<span class="order-index">#${a+1}</span>
<span class="drag" title="${p("up")} / ${p("down")}">⋮⋮</span>
<span class="swatch"></span>
<span class="meta">
  <span class="coverage">${e.toFixed(1)}%</span>
  <span class="hex">${t.toUpperCase()}</span>
  <span class="state">${g?p("disabled"):p("enabled")}</span>
</span>
<span class="premium"></span>`,d.querySelector(".swatch").style.setProperty("--swatch-color",So(r.realColor)),d.addEventListener("click",()=>{if(l){l=!1;return}f(),this.updateTasks()}),d.addEventListener("dragstart",(h)=>{l=!0,d.classList.add("dragging"),h.dataTransfer?.setData("text/plain",String(a)),h.dataTransfer.effectAllowed="move"}),d.addEventListener("dragend",()=>{d.classList.remove("dragging")}),d.addEventListener("dragover",(h)=>{h.preventDefault(),d.classList.add("drag-target")}),d.addEventListener("dragleave",()=>{d.classList.remove("drag-target")}),d.addEventListener("drop",(h)=>{h.preventDefault(),d.classList.remove("drag-target");let u=Number.parseInt(h.dataTransfer?.getData("text/plain")??"-1",10);if(u<0||u===a||u>=this.colors.length)return;this.colors.splice(a,0,...this.colors.splice(u,1)),M(this.bot),this.updateColors()});let w=document.createElement("button");w.textContent=p("buy"),w.className="buy-chip",w.addEventListener("click",(h)=>{h.stopPropagation(),document.getElementById("color-"+r.realColor)?.click()}),d.append(w);let m=`${t} ${n.join(" ")} ${r.realColor} ${W[r.realColor]}`;if(!s||m.toLowerCase().includes(s))this.$colorsDialogList.append(d)}}syncColorBulkToggle(){let o=this.colors.filter((a)=>!a.disabled).length,s=o===this.colors.length;this.$toggleAllColors.checked=s,this.$toggleAllColors.indeterminate=o>0&&!s}*strategyPositionIterator(){let o=this.pixels.pixels[0].length,s=this.pixels.pixels.length;switch(this.strategy){case"DOWN":{for(let a=0;a<s;a++)for(let i=0;i<o;i++)yield{x:i,y:a};break}case"UP":{for(let a=s-1;a>=0;a--)for(let i=0;i<o;i++)yield{x:i,y:a};break}case"LEFT":{for(let a=0;a<o;a++)for(let i=0;i<s;i++)yield{x:a,y:i};break}case"RIGHT":{for(let a=o-1;a>=0;a--)for(let i=0;i<s;i++)yield{x:a,y:i};break}case"RANDOM":{let a=[];for(let i=0;i<s;i++)for(let r=0;r<o;r++)a.push({x:r,y:i});for(let i=a.length-1;i>=0;i--){let r=Math.floor(Math.random()*(i+1)),l=a[i];a[i]=a[r],a[r]=l}yield*a;break}case"ZIGZAG":{for(let a=0;a<s;a++)if(a%2===0)for(let i=0;i<o;i++)yield{x:i,y:a};else for(let i=o-1;i>=0;i--)yield{x:i,y:a};break}case"HUMANIZED":{let a=Math.max(4,Math.floor(Math.min(o,s)/10)),i=[];for(let r=0;r<s;r+=a)for(let l=0;l<o;l+=a)i.push({x:l,y:r});for(let r=i.length-1;r>=0;r--){let l=Math.floor(Math.random()*(r+1)),e=i[r];i[r]=i[l],i[l]=e}for(let r=0;r<i.length;r++){let l=i[r],e=Math.min(s,l.y+a),t=Math.min(o,l.x+a);for(let n=l.y;n<e;n++)if(Math.random()>0.35)for(let g=l.x;g<t;g++)yield{x:g,y:n};else for(let g=t-1;g>=l.x;g--)yield{x:g,y:n}}break}case"HUMAN_SOFT_DITHER":{let a=new Set;for(let i=0;i<s;i++){let r=Math.floor(Math.random()*3)-1;if((i+r)%2===0)for(let e=0;e<o;e+=2)a.add(`${e},${i}`),yield{x:e,y:i};else for(let e=1;e<o;e+=2)a.add(`${e},${i}`),yield{x:e,y:i}}for(let i=0;i<s;i++)for(let r=0;r<o;r++){let l=`${r},${i}`;if(a.has(l))continue;yield{x:r,y:i}}break}case"HUMAN_PATCHY":{let a=new Set,i=o*s;while(a.size<i){let r=Math.floor(Math.random()*o),l=Math.floor(Math.random()*s),e=1+Math.floor(Math.random()*5);for(let t=l-e;t<=l+e;t++)for(let n=r-e;n<=r+e;n++){if(n<0||n>=o||t<0||t>=s)continue;if(Math.hypot(n-r,t-l)>e+Math.random()*1.2)continue;let c=`${n},${t}`;if(a.has(c))continue;a.add(c),yield{x:n,y:t}}}break}case"HUMAN_SWEEP_ARCS":{let a=new Set,i=(o-1)/2,r=(s-1)/2,l=Math.hypot(i,r);for(let e=0;e<4;e++){let t=Math.random()*Math.PI*2;for(let n=0;n<=l;n+=0.35){let c=Math.PI/2+Math.random()*(Math.PI/1.5),g=Math.max(10,Math.floor(n*8));for(let f=0;f<g;f++){let d=t+c*f/g+Math.sin(n)*0.08,w=Math.round(i+Math.cos(d)*n),m=Math.round(r+Math.sin(d)*n);if(w<0||w>=o||m<0||m>=s)continue;let h=`${w},${m}`;if(a.has(h))continue;a.add(h),yield{x:w,y:m}}}}for(let e=0;e<s;e++)for(let t=0;t<o;t++){let n=`${t},${e}`;if(a.has(n))continue;yield{x:t,y:e}}break}case"HUMAN_MICRO_CORRECTIONS":{let a=new Set;for(let i=0;i<s;i++){let r=i%2===0?1:-1,l=r>0?0:o-1;for(let e=0;e<o;e++){let t=l+(Math.random()>0.82?r:0),n=i+(Math.random()>0.9?1:0);for(let c of[{x:l,y:i},{x:t,y:i},{x:l,y:n}]){if(c.x<0||c.x>=o||c.y<0||c.y>=s)continue;let g=`${c.x},${c.y}`;if(a.has(g))continue;a.add(g),yield c}l+=r}}for(let i=0;i<s;i++)for(let r=0;r<o;r++){let l=`${r},${i}`;if(a.has(l))continue;yield{x:r,y:i}}break}case"HUMAN_JITTER_FILL":{let a=[];for(let i=0;i<s;i++)for(let r=0;r<o;r++)a.push({x:r,y:i});a.sort((i,r)=>{let l=i.y+(Math.random()-0.5)*1.8,e=r.y+(Math.random()-0.5)*1.8;if(l!==e)return l-e;return i.x+(Math.random()-0.5)*2-(r.x+(Math.random()-0.5)*2)}),yield*a;break}case"HUMAN_CORNER_BIAS":{let a=[{x:0,y:0},{x:o-1,y:0},{x:0,y:s-1},{x:o-1,y:s-1}],i=a[Math.floor(Math.random()*a.length)],r=[];for(let l=0;l<s;l++)for(let e=0;e<o;e++){let n=Math.hypot(e-i.x,l-i.y)+Math.random()*3.5;r.push({point:{x:e,y:l},score:n})}r.sort((l,e)=>l.score-e.score);for(let l of r)yield l.point;break}case"HUMAN_LONG_STROKES":{let a=new Set,i=o*s;while(a.size<i){let r=Math.floor(Math.random()*o),l=Math.floor(Math.random()*s),e=Math.random()*Math.PI*2,t=Math.sign(Math.cos(e)),n=Math.sign(Math.sin(e)),c=10+Math.floor(Math.random()*40);for(let g=0;g<c;g++){if(r<0||r>=o||l<0||l>=s)break;let f=`${r},${l}`;if(!a.has(f))a.add(f),yield{x:r,y:l};if(Math.random()>0.78)r+=n,l+=t;else r+=t,l+=n}}break}case"HUMAN_TAP_CLUSTERS":{let a=new Set,i=o*s;while(a.size<i){let r=Math.floor(Math.random()*o),l=Math.floor(Math.random()*s),e=3+Math.floor(Math.random()*10);for(let t=0;t<e;t++){let n=Math.round(r+(Math.random()-0.5)*6),c=Math.round(l+(Math.random()-0.5)*6);if(n<0||n>=o||c<0||c>=s)continue;let g=`${n},${c}`;if(a.has(g))continue;a.add(g),yield{x:n,y:c}}}break}case"HUMAN_MESSY_SPIRAL":{let a=new Set,i=(o-1)/2,r=(s-1)/2,l=Math.hypot(i,r)+2;for(let e=0;a.size<o*s;e++){let t=e/3,n=Math.min(l,t*0.18),c=t*0.29+Math.sin(t*0.13)*0.8,g=Math.round(i+Math.cos(c)*n+Math.sin(t)*0.7),f=Math.round(r+Math.sin(c)*n+Math.cos(t)*0.7);if(g<0||g>=o||f<0||f>=s){if(e>o*s*18)break;continue}let d=`${g},${f}`;if(a.has(d)){if(Math.random()>0.9)continue}else a.add(d),yield{x:g,y:f};if(e>o*s*18)break}for(let e=0;e<s;e++)for(let t=0;t<o;t++){let n=`${t},${e}`;if(a.has(n))continue;yield{x:t,y:e}}break}case"HUMAN_DRUNK_WALK":{let a=new Set,i=Math.floor(Math.random()*o),r=Math.floor(Math.random()*s),l=[{x:-1,y:0},{x:1,y:0},{x:0,y:-1},{x:0,y:1},{x:-1,y:-1},{x:1,y:-1},{x:-1,y:1},{x:1,y:1}];while(a.size<o*s){let e=`${i},${r}`;if(!a.has(e))a.add(e),yield{x:i,y:r};let t=[];for(let g of l){let f=i+g.x,d=r+g.y;if(f<0||f>=o||d<0||d>=s)continue;t.push({x:f,y:d})}if(!t.length)break;let n=t.filter((g)=>{return!a.has(`${g.x},${g.y}`)});if(n.length&&Math.random()>0.2){let g=n[Math.floor(Math.random()*n.length)];i=g.x,r=g.y;continue}let c=t[Math.floor(Math.random()*t.length)];i=c.x,r=c.y}for(let e=0;e<s;e++)for(let t=0;t<o;t++){let n=`${t},${e}`;if(a.has(n))continue;yield{x:t,y:e}}break}case"HUMAN_NOISE_CLOUD":{let a=[];for(let i=0;i<s;i++)for(let r=0;r<o;r++){let l=Math.sin((r+1)*0.93+Math.random()*0.8)+Math.cos((i+1)*1.17+Math.random()*0.8),e=(Math.random()-0.5)*2.6,t=Math.hypot(r-o/2,i-s/2)*0.08;a.push({point:{x:r,y:i},score:l+e+t})}a.sort((i,r)=>i.score-r.score);for(let i of a)yield i.point;break}case"HUMAN_PATCH_JUMP":{let a=new Set,i=[];for(let r=0;r<Math.max(6,o*s/18);r++)i.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*s)});while(a.size<o*s){let r=i[Math.floor(Math.random()*i.length)],l=1+Math.floor(Math.random()*3),e=1+Math.floor(Math.random()*3);for(let t=r.y-e;t<=r.y+e;t++)for(let n=r.x-l;n<=r.x+l;n++){if(n<0||n>=o||t<0||t>=s)continue;if(Math.random()>0.86)continue;let c=`${n},${t}`;if(a.has(c))continue;a.add(c),yield{x:n,y:t}}if(Math.random()>0.72&&i.length<o*s/2)i.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*s)});if(a.size>o*s*0.92)break}for(let r=0;r<s;r++)for(let l=0;l<o;l++){let e=`${l},${r}`;if(a.has(e))continue;yield{x:l,y:r}}break}case"HUMAN_HESITANT_LINES":{let a=new Set;for(let i=0;i<s;i++){let r=i%2===0;for(let l=0;l<o;l++){let e=r?l:o-1-l,t=`${e},${i}`;if(!a.has(t))a.add(t),yield{x:e,y:i};if(Math.random()>0.7){let n=Math.max(0,Math.min(o-1,e+(Math.random()>0.5?1:-1))),c=Math.max(0,Math.min(s-1,i+(Math.random()>0.65?1:0))),g=`${n},${c}`;if(!a.has(g))a.add(g),yield{x:n,y:c}}}}for(let i=0;i<s;i++)for(let r=0;r<o;r++){let l=`${r},${i}`;if(a.has(l))continue;yield{x:r,y:i}}break}case"HUMAN_OVERLAP_SWEEPS":{let a=[],i=Math.random()*Math.PI*2;for(let r=0;r<s;r++)for(let l=0;l<o;l++){let e=Math.sin((l+r)*0.42+i)*2.2,t=Math.cos((l-r)*0.3+i)*1.4;a.push({point:{x:l,y:r},score:r+e+t+(Math.random()-0.5)*3.4})}a.sort((r,l)=>r.score-l.score);for(let r of a)yield r.point;break}case"HUMAN_WOBBLE_DRIFT":{let a=[],i=o/2,r=s/2;for(let l=0;l<s;l++)for(let e=0;e<o;e++){let t=Math.hypot(e-i,l-r)*0.25,n=Math.sin((e+1)*0.9)*1.8+Math.cos((l+1)*1.1)*1.8+Math.sin((e+l)*0.35)*1.4;a.push({point:{x:e,y:l},score:t+n+(Math.random()-0.5)*2.8})}a.sort((l,e)=>l.score-e.score);for(let l of a)yield l.point;break}case"HUMAN_GAP_RECOVERY":{let a=new Set,i=[];for(let r=0;r<s;r++)for(let l=0;l<o;l++){if(Math.random()>0.87){i.push({x:l,y:r});continue}a.add(`${l},${r}`),yield{x:l,y:r}}i.sort((r,l)=>Math.hypot(r.x-o/2,r.y-s/2)-Math.hypot(l.x-o/2,l.y-s/2));for(let r of i){let l=`${r.x},${r.y}`;if(a.has(l))continue;a.add(l),yield r}break}case"HUMAN_STAIRCASE":{let a=new Set,i=o+s-1;for(let r=0;r<i;r++){let l=Math.max(0,r-o+1),e=Math.min(s-1,r);for(let t=l;t<=e;t++){let n=r-t,c=[{x:n,y:t},{x:n+(Math.random()>0.5?1:-1),y:t},{x:n,y:t+(Math.random()>0.5?1:-1)}];for(let g of c){if(g.x<0||g.x>=o||g.y<0||g.y>=s)continue;let f=`${g.x},${g.y}`;if(a.has(f))continue;a.add(f),yield g}}}for(let r=0;r<s;r++)for(let l=0;l<o;l++){let e=`${l},${r}`;if(a.has(e))continue;yield{x:l,y:r}}break}case"HUMAN_EDGE_HUGGER":{let a=[];for(let i=0;i<s;i++)for(let r=0;r<o;r++){let l=Math.min(r,i,o-1-r,s-1-i);a.push({point:{x:r,y:i},score:l*3.5+(Math.random()-0.5)*5.5})}a.sort((i,r)=>i.score-r.score);for(let i of a)yield i.point;break}case"HUMAN_BLOBS":{let a=new Set,i=o*s;while(a.size<i){let r=Math.floor(Math.random()*o),l=Math.floor(Math.random()*s),e=1+Math.floor(Math.random()*4);for(let t=l-e;t<=l+e;t++)for(let n=r-e;n<=r+e;n++){if(n<0||n>=o||t<0||t>=s)continue;let c=Math.atan2(t-l,n-r),g=e+Math.sin(c*3+Math.random())*0.8;if(Math.hypot(n-r,t-l)>g)continue;let f=`${n},${t}`;if(a.has(f))continue;a.add(f),yield{x:n,y:t}}}break}case"HUMAN_BACKTRACK":{let a=new Set,i=[];for(let r=0;r<s;r++)for(let l=0;l<o;l++)i.push({x:l,y:r});i.sort((r,l)=>r.y-l.y+(Math.random()-0.5)*2.2+(r.x-l.x)*0.04);for(let r=0;r<i.length;r++){let l=i[r],e=`${l.x},${l.y}`;if(a.has(e))continue;if(a.add(e),yield l,r>1&&Math.random()>0.74){let t=i[r-1],n=`${t.x},${t.y}`;if(!a.has(n))a.add(n),yield t}}for(let r=0;r<s;r++)for(let l=0;l<o;l++){let e=`${l},${r}`;if(a.has(e))continue;yield{x:l,y:r}}break}case"HUMAN_SHAKY_DIAGONAL":{let a=[];for(let i=0;i<s;i++)for(let r=0;r<o;r++){let l=Math.abs(r-i)*0.6,e=Math.sin(r*1.4+i*0.8)*1.8+Math.cos(i*1.1+r*0.5)*1.5;a.push({point:{x:r,y:i},score:l+e+(Math.random()-0.5)*3.2})}a.sort((i,r)=>i.score-r.score);for(let i of a)yield i.point;break}case"HUMAN_LATE_FIXES":{let a=[],i=[];for(let r=0;r<s;r++)for(let l=0;l<o;l++)if(Math.random()>0.9)i.push({x:l,y:r});else a.push({x:l,y:r});a.sort((r,l)=>r.y-l.y+(Math.random()-0.5)*1.5+(Math.random()>0.85?r.x-l.x:0)),i.sort((r,l)=>Math.hypot(l.x-o/2,l.y-s/2)-Math.hypot(r.x-o/2,r.y-s/2)),yield*a,yield*i;break}case"DIAGONAL_BRUSH":{for(let a=0;a<o+s-1;a++){let i=a%2===0,r=[],l=Math.max(0,a-o+1),e=Math.min(s-1,a);for(let t=l;t<=e;t++){let n=a-t;if(n>=0&&n<o)r.push({x:n,y:t})}if(Math.random()>0.55)r.reverse();if(i)for(let t=r.length-1;t>=0;t--)yield r[t];else yield*r}break}case"BRUSH_STROKES":{let a=Array.from({length:s},()=>Array(o).fill(!1)),i=[{x:1,y:0},{x:-1,y:0},{x:0,y:1},{x:0,y:-1},{x:1,y:1},{x:1,y:-1},{x:-1,y:1},{x:-1,y:-1}],r=(t,n)=>t>=0&&t<o&&n>=0&&n<s,l=0,e=o*s;for(let t=0;t<e*6&&l<e;t++){let n=Math.floor(Math.random()*o),c=Math.floor(Math.random()*s),g=i[Math.floor(Math.random()*i.length)],f=3+Math.floor(Math.random()*16);for(let d=0;d<f;d++){if(!r(n,c))break;if(!a[c][n])a[c][n]=!0,l++,yield{x:n,y:c};if(Math.random()>0.72)g=i[Math.floor(Math.random()*i.length)];n+=g.x,c+=g.y}}for(let t=0;t<s;t++)for(let n=0;n<o;n++)if(!a[t][n])yield{x:n,y:t};break}case"SPIRAL_FROM_CENTER":case"SPIRAL_TO_CENTER":{let a=new Set,i=o*s,r=Math.floor(o/2),l=Math.floor(s/2),e=[[1,0],[0,1],[-1,0],[0,-1]],t=0,n=1,c=(f,d)=>f>=0&&f<o&&d>=0&&d<s,g=function*(){let f=0;while(f<i){for(let d=0;d<2;d++){for(let w=0;w<n;w++){if(c(r,l)){let m=`${r},${l}`;if(!a.has(m)){if(a.add(m),yield{x:r,y:l},f++,f>=i)return}}r+=e[t][0],l+=e[t][1]}t=(t+1)%4}n++}};if(this.strategy==="SPIRAL_FROM_CENTER")yield*g();else{let f=[...g()];for(let d=f.length-1;d>=0;d--)yield f[d]}break}case"SCRIBBLE":{let a=new Set,i=o*s,r=Math.floor(o/2),l=Math.floor(s/2);for(let e=0;a.size<i&&e<i*24;e++){let t=`${r},${l}`;if(!a.has(t))a.add(t),yield{x:r,y:l};if(r+=Math.floor(Math.random()*3)-1,l+=Math.floor(Math.random()*3)-1,r<0||r>=o||l<0||l>=s)r=Math.floor(Math.random()*o),l=Math.floor(Math.random()*s)}for(let e=0;e<s;e++)for(let t=0;t<o;t++){let n=`${t},${e}`;if(a.has(n))continue;a.add(n),yield{x:t,y:e}}break}case"CROSSHATCH":{let a=[];for(let l=0;l<o+s-1;l++)for(let e=Math.max(0,l-o+1);e<=Math.min(s-1,l);e++){let t=l-e;a.push({x:t,y:e})}let i=[];for(let l=-s+1;l<o;l++)for(let e=0;e<s;e++){let t=e+l;if(t>=0&&t<o)i.push({x:t,y:e})}let r=new Set;for(let l of[...a,...i]){let e=`${l.x},${l.y}`;if(r.has(e))continue;r.add(e),yield l}break}case"WAVE_SWEEP":{let a=new Set;for(let i=0;i<o;i++){let l=(Math.sin(i/Math.max(1,o-1)*Math.PI*4)+1)*0.5*(s-1)|0;for(let e=0;e<s;e++){let t=l+e,n=l-e;for(let c of[t,n]){if(c<0||c>=s)continue;let g=`${i},${c}`;if(a.has(g))continue;a.add(g),yield{x:i,y:c}}}}break}case"SCATTERED_LINES":{let a=new Set,i=o*s;for(let r=0;a.size<i&&r<i*14;r++){let l=Math.floor(Math.random()*o),e=Math.floor(Math.random()*s),t=Math.random()*Math.PI*2,n=Math.round(Math.cos(t)),c=Math.round(Math.sin(t)),g=6+Math.floor(Math.random()*28);for(let f=0;f<g;f++){if(l<0||l>=o||e<0||e>=s)break;let d=`${l},${e}`;if(!a.has(d))a.add(d),yield{x:l,y:e};l+=n,e+=c}}for(let r=0;r<s;r++)for(let l=0;l<o;l++){let e=`${l},${r}`;if(a.has(e))continue;a.add(e),yield{x:l,y:r}}break}case"CONTOUR_JITTER":{let a=new Set;for(let i=0;i<Math.ceil(Math.min(o,s)/2);i++){let r=[],l=i,e=i,t=o-i-1,n=s-i-1;for(let c=l;c<=t;c++)r.push({x:c,y:e});for(let c=e+1;c<=n;c++)r.push({x:t,y:c});for(let c=t-1;c>=l;c--)r.push({x:c,y:n});for(let c=n-1;c>e;c--)r.push({x:l,y:c});for(let c=r.length-1;c>0;c--){let g=Math.floor(Math.random()*(c+1)),f=r[c];r[c]=r[g],r[g]=f}for(let c of r){let g=`${c.x},${c.y}`;if(a.has(g))continue;a.add(g),yield c}}break}case"SPIRAL_WOBBLE":{let a=new Set,i=o/2,r=s/2,l=Math.hypot(i,r);for(let e=0;a.size<o*s&&e<o*s*9;e++){let t=e/(o*s*9)*l,n=e*0.31+Math.sin(e*0.07)*0.7,c=Math.round(i+Math.cos(n)*t),g=Math.round(r+Math.sin(n)*t);if(c<0||c>=o||g<0||g>=s)continue;let f=`${c},${g}`;if(a.has(f))continue;a.add(f),yield{x:c,y:g}}for(let e=0;e<s;e++)for(let t=0;t<o;t++){let n=`${t},${e}`;if(a.has(n))continue;yield{x:t,y:e}}break}case"CLUSTER_BURSTS":{let a=new Set,i=o*s;for(let r=0;a.size<i&&r<i*12;r++){let l=Math.floor(Math.random()*o),e=Math.floor(Math.random()*s),t=2+Math.floor(Math.random()*10);for(let n=e-t;n<=e+t;n++)for(let c=l-t;c<=l+t;c++){if(c<0||c>=o||n<0||n>=s)continue;if(Math.hypot(c-l,n-e)>t)continue;let g=`${c},${n}`;if(a.has(g))continue;a.add(g),yield{x:c,y:n}}}for(let r=0;r<s;r++)for(let l=0;l<o;l++){let e=`${l},${r}`;if(a.has(e))continue;a.add(e),yield{x:l,y:r}}break}case"ORBITAL":{let a=new Set,i=(o-1)/2,r=(s-1)/2,l=Math.ceil(Math.max(i,r));for(let e=0;e<=l;e++){let t=Math.max(16,Math.ceil(2*Math.PI*Math.max(1,e)*2));for(let n=0;n<t;n++){let c=n/t*Math.PI*2+(e%2?0.3:-0.3),g=Math.round(i+Math.cos(c)*e),f=Math.round(r+Math.sin(c)*e);if(g<0||g>=o||f<0||f>=s)continue;let d=`${g},${f}`;if(a.has(d))continue;a.add(d),yield{x:g,y:f}}}for(let e=0;e<s;e++)for(let t=0;t<o;t++){let n=`${t},${e}`;if(a.has(n))continue;yield{x:t,y:e}}break}case"FLOW_FIELD":{let a=new Set,i=o*s;for(let r=0;a.size<i&&r<i*18;r++){let l=Math.floor(Math.random()*o),e=Math.floor(Math.random()*s);for(let t=0;t<120;t++){if(l<0||l>=o||e<0||e>=s)break;let n=`${l},${e}`;if(!a.has(n))a.add(n),yield{x:l,y:e};let c=Math.sin(l*0.09)*1.8+Math.cos(e*0.08)*1.6+Math.sin((l+e)*0.05);l+=Math.round(Math.cos(c)),e+=Math.round(Math.sin(c))}}for(let r=0;r<s;r++)for(let l=0;l<o;l++){let e=`${l},${r}`;if(a.has(e))continue;a.add(e),yield{x:l,y:r}}break}case"EDGE_IN":{let a=new Set,i=Math.ceil(Math.min(o,s)/2);for(let r=0;r<i;r++){let l=r,e=o-1-r,t=r,n=s-1-r;for(let c=l;c<=e;c++)for(let g of[t,n]){let f=`${c},${g}`;if(a.has(f))continue;a.add(f),yield{x:c,y:g}}for(let c=t+1;c<=n-1;c++)for(let g of[l,e]){let f=`${g},${c}`;if(a.has(f))continue;a.add(f),yield{x:g,y:c}}}break}}}moveStart(o){if(o.button!==0)return;if(o.preventDefault(),o.stopPropagation(),!this.lock)this.moveInfo={globalX:this.position.globalX,globalY:this.position.globalY,clientX:o.clientX,clientY:o.clientY}}moveStop(){if(this.moveInfo)this.moveInfo=void 0,this.position.updateAnchor(),this.pixels.update(),this.updateColors(),M(this.bot)}move(o){if(!this.moveInfo)return;let s=Math.round((o.clientX-this.moveInfo.clientX)/this.position.pixelSize),a=Math.round((o.clientY-this.moveInfo.clientY)/this.position.pixelSize);if(this.moveInfo.globalX!==void 0){if(this.position.globalX=s+this.moveInfo.globalX,this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,this.moveInfo.width-s)}else if(this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,s+this.moveInfo.width);if(this.moveInfo.globalY!==void 0){if(this.position.globalY=a+this.moveInfo.globalY,this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,this.moveInfo.height-a)}else if(this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,a+this.moveInfo.height);this.update(),M(this.bot)}resizeStart(o){if(this.lock||o.button!==0)return;o.preventDefault(),o.stopPropagation(),this.moveInfo={clientX:o.clientX,clientY:o.clientY};let s=o.target;if(s.classList.contains("n"))this.moveInfo.height=this.pixels.height,this.moveInfo.globalY=this.position.globalY;if(s.classList.contains("e"))this.moveInfo.width=this.pixels.width;if(s.classList.contains("s"))this.moveInfo.height=this.pixels.height;if(s.classList.contains("w"))this.moveInfo.width=this.pixels.width,this.moveInfo.globalX=this.position.globalX}export(){let o=document.createElement("a");document.body.append(o),o.href=URL.createObjectURL(new Blob([JSON.stringify(this.toJSON())],{type:"application/json"})),o.download=`${this.pixels.width}x${this.pixels.height}.${Y}`,o.click(),URL.revokeObjectURL(o.href),o.href=this.pixels.canvas.toDataURL("image/webp",1),o.download=`${this.pixels.width}x${this.pixels.height}.webp`,o.click(),URL.revokeObjectURL(o.href),o.remove()}}function Mo(){let o=localStorage.getItem("kglacer-macro:shield-config");if(!o)return!1;try{return JSON.parse(o).enabled!==!1}catch{return!1}}function Eo(o){localStorage.setItem("kglacer-macro:shield-config",JSON.stringify({enabled:o}))}function xa(o){let s=`${o?.host??""} ${o?.username??""}`.toLowerCase(),a=/(mx|mex|mexico)/.test(s)?"MX":"AUTO";localStorage.setItem("__afm_proxy_hint",a)}function qo(o){if(!Mo())return;if(document.getElementById("kgm-shield-full"))return;xa(o);let s=document.createElement("script");s.id="kgm-shield-full",s.textContent=`// ==UserScript==
...
(function injectAntiFingerprintMergedShield() {
    ...
    pageScript.remove();
})();`,document.documentElement.append(s),s.remove()}var Ro=`/* stylelint-disable declaration-no-important */
...
`;var Yo="kglacer-macro:overlay-hidden",Io="kglacer-macro:images-collapsed",Xo="kglacer-macro:auto-farm-config",_o="kglacer-macro:auto-overlay-config",$o="kglacer-macro:proxy-config",yo="__afm_proxy_hint",ja=["https://api.ipify.org?format=json","https://icanhazip.com"],Ta="https://raw.githubusercontent.com/robgallardof/kglacer-macro/refs/heads/main/src/img/logo.svg",Ka="https://pepoafonso.github.io/color_converter_wplace/es/index.html",oa="https://wplace.samuelscheit.com/",aa="https://wplace.eralyon.net/",sa="v69.051",va="https://receive-smss.com/",La="https://esimplus.me/temporary-numbers",Qa="https://receive-sms-free.cc/",Ga="https://quackr.io/?srsltid=AfmBOoqu2h3Pt6-h3HtJ_tixaj5WGtA7ZaI9sLQiQnPTnisDxe0MXbje",Wa="https://www.textverified.com/free";class Co extends ${bot;element=document.createElement("div");get status(){return this.$status.innerHTML}set status(o){this.$status.innerHTML=o}get open(){return this.element.classList.contains("wopen")}set open(o){if(o)this.element.classList.add("wopen");else this.element.classList.remove("wopen");let s=this.element.querySelector(".wopen-button");if(!s)return;s.setAttribute("aria-expanded",String(o)),s.setAttribute("aria-label",o?p("mobileMinimize"):p("mobileShowPanel")),s.title=o?p("mobileMinimize"):p("mobileShowPanel")}$settings;$status;$openConfig;$mobileMinimize;$mobileSettings;$mobileScrollImages;$topbar;$draw;$drawAndPaint;$addImage;$captureTemplate;$toolColorConverter;$toolSamuelArchive;$toolEralyonArchive;$toolReceiveSmss;$toolEsimplus;$toolReceiveSmsFree;$toolQuackr;$toolTextverified;$toggleOverlay;$autofarmConfig;$autofarmStart;$autofarmStop;$autofarmStatus;$autoOverlayConfig;$autoOverlayStart;$autoOverlayStop;$autoOverlayStatus;$strategy;$progressLine;$progressText;$images;$imagesSection;$imagesCollapseState;$wopenButton;$widgetLogo;activeImageIndex=-1;autoFarmIntervalId;autoFarmConfig;autoFarmTickRunning=!1;autoFarmNextTickAt;autoOverlayIntervalId;autoOverlayConfig;autoOverlayTickRunning=!1;autoOverlayNextTickAt;statusRefreshIntervalId;challengeWatcherObserver;challengeWatcherRunning=!1;imagesListDirty=!0;constructor(o){super();this.bot=o;this.element.classList.add("wwidget"),this.element.innerHTML=Vo,x(this.element),document.body.append(this.element),this.populateElementsWithSelector(this.element,{$wopenButton:".wopen-button",$widgetLogo:".widget-logo",$settings:".wform",$status:".wstatus",$openConfig:".open-config",$mobileMinimize:".mobile-minimize",$mobileSettings:".mobile-settings",$mobileScrollImages:".mobile-scroll-images",$topbar:".wtopbar",$draw:".draw",$drawAndPaint:".draw-and-paint",$addImage:".add-image",$captureTemplate:".capture-template",$toolColorConverter:".tool-color-converter",$toolSamuelArchive:".tool-samuel-archive",$toolEralyonArchive:".tool-eralyon-archive",$toolReceiveSmss:".tool-receive-smss",$toolEsimplus:".tool-esimplus",$toolReceiveSmsFree:".tool-receive-sms-free",$toolQuackr:".tool-quackr",$toolTextverified:".tool-textverified",$toggleOverlay:".toggle-overlay",$autofarmConfig:".autofarm-config",$autofarmStart:".autofarm-start",$autofarmStop:".autofarm-stop",$autofarmStatus:".autofarm-status",$autoOverlayConfig:".autooverlay-config",$autoOverlayStart:".autooverlay-start",$autoOverlayStop:".autooverlay-stop",$autoOverlayStatus:".autooverlay-status",$strategy:".strategy",$progressLine:".wprogress div",$progressText:".wprogress span",$images:".images",$imagesSection:".widget-section-images",$imagesCollapseState:".images-collapse-state"}),this.$widgetLogo.src=Ta,this.$wopenButton.addEventListener("click",()=>this.open=!this.open),this.$draw.addEventListener("click",()=>this.bot.draw()),this.$drawAndPaint.addEventListener("click",()=>{this.drawAndClickPaintWhenReady()}),this.$addImage.addEventListener("click",()=>this.addImage()),this.$openConfig.addEventListener("click",()=>{this.openSettingsModal()}),this.$mobileMinimize.addEventListener("click",()=>{this.open=!1}),this.$mobileSettings.addEventListener("click",()=>{this.openSettingsModal()}),this.$mobileScrollImages.addEventListener("click",()=>{this.open=!0,this.$imagesSection.open=!0,this.$imagesSection.scrollIntoView({behavior:"smooth",block:"start"})}),this.$captureTemplate.addEventListener("click",()=>{this.captureTemplate()}),this.$toolColorConverter.addEventListener("click",()=>{this.openExternalTool("colorConverter")}),this.$toolSamuelArchive.addEventListener("click",()=>{this.openExternalTool("samuelArchive")}),this.$toolEralyonArchive.addEventListener("click",()=>{this.openExternalTool("eralyonArchive")}),this.$toolReceiveSmss.addEventListener("click",()=>{this.openExternalTool("receiveSmss")}),this.$toolEsimplus.addEventListener("click",()=>{this.openExternalTool("esimplus")}),this.$toolReceiveSmsFree.addEventListener("click",()=>{this.openExternalTool("receiveSmsFree")}),this.$toolQuackr.addEventListener("click",()=>{this.openExternalTool("quackr")}),this.$toolTextverified.addEventListener("click",()=>{this.openExternalTool("textverified")}),this.$toggleOverlay.addEventListener("click",()=>{this.toggleOverlay()}),this.$autofarmConfig.addEventListener("click",()=>{this.openAutoFarmModal()}),this.$autofarmStart.addEventListener("click",()=>{this.startAutoFarm()}),this.$autofarmStop.addEventListener("click",()=>{this.stopAutoFarm()}),this.$autoOverlayConfig.addEventListener("click",()=>{this.openAutoOverlayModal()}),this.$autoOverlayStart.addEventListener("click",()=>{this.startAutoOverlay()}),this.$autoOverlayStop.addEventListener("click",()=>{this.stopAutoOverlay()}),this.$strategy.addEventListener("change",()=>{this.bot.strategy=this.$strategy.value}),this.applyImagesCollapsedPreference(),this.$imagesSection.addEventListener("toggle",()=>{if(this.persistImagesCollapsedPreference(!this.$imagesSection.open),this.refreshImagesCollapseText(),!this.$imagesSection.open||!this.imagesListDirty)return;this.renderImagesList(),this.imagesListDirty=!1}),this.registerEvent(document,"keydown",this.handleKeyboard.bind(this),{passive:!1}),this.update(),this.syncOverlayVisibilityFromStorage(),this.loadAutoFarmConfigFromStorage(),this.loadAutoOverlayConfigFromStorage(),this.refreshAutoFarmStatusText(),this.refreshAutoOverlayStatusText(),this.statusRefreshIntervalId=window.setInterval(()=>{this.refreshAutoFarmStatusText(),this.refreshAutoOverlayStatusText(),this.refreshProgress()},1000),this.open=!0,window.setTimeout(()=>{this.recommendUpdateIfOutdated()},2500),console.log("[KGM][Widget] Widget mounted and opened")}startChallengeWatcher(){let o=()=>{if(!this.isChallengeBlockingPaint())return;if(this.challengeWatcherRunning)return;this.challengeWatcherRunning=!0,this.status=`⌛ ${p("taskWaitingChallengeResolve")}`,this.waitForChallengeToResolve().finally(()=>{this.challengeWatcherRunning=!1})};this.challengeWatcherObserver=new MutationObserver(()=>{o()}),this.challengeWatcherObserver.observe(document.documentElement,{childList:!0,subtree:!0,attributes:!0,attributeFilter:["open","style","class","value","aria-hidden"]});let s=window.setInterval(o,750);this.runOnDestroy.push(()=>{this.challengeWatcherObserver?.disconnect(),clearInterval(s)}),o()}addImage(){return console.log("[KGM][Widget] Add image flow started"),this.setDisabled("add-image",!0),this.run(p("taskAddingImage"),async()=>{await this.bot.updateColors();let o=document.createElement("input");o.type="file",o.accept=`image/*,.${Y},.wplace`,o.click(),await T(o,["change"],["cancel","error"]);let s=o.files?.[0];if(!s)throw new Po(this.bot);console.log("[KGM][Widget] File selected",{name:s.name,size:s.size,type:s.type});let a;if(s.name.endsWith(`.${Y}`))a=await U.fromJSON(this.bot,JSON.parse(await s.text()));else if(s.name.endsWith(".wplace")){let i=JSON.parse(await s.text());if(!i.image?.dataUrl)throw Error("Invalid .wplace file: image.dataUrl missing");let r=new Image;if(r.src=i.image.dataUrl,await T(r,["load"],["error"]),await this.waitForStableViewportProjection(),a=new U(this.bot,H.fromScreenPosition(this.bot,this.defaultImageScreenPosition()),new v(this.bot,r)),typeof i.opacity==="number")a.opacity=Math.max(0,Math.min(1,i.opacity))}else{let i=new FileReader;i.readAsDataURL(s),await T(i,["load"],["error"]);let r=await this.compressImageBeforeLoad(i.result),l=new Image;l.src=r,await T(l,["load"],["error"]),await this.waitForStableViewportProjection(),a=new U(this.bot,H.fromScreenPosition(this.bot,this.defaultImageScreenPosition()),new v(this.bot,l))}this.bot.images.push(a),console.log("[KGM][Widget] Image instance added",{images:this.bot.images.length}),await this.bot.readMap(),a.updateTasks(),M(this.bot,!0),this.bot.updateTasks(),this.update(),window.setTimeout(()=>{globalThis.location.reload()},120)},()=>{this.setDisabled("add-image",!1)})}captureTemplate(){return this.setDisabled("capture-template",!0),this.run(p("taskCapturingMapImage"),async()=>{let o=await this.resolveCaptureBounds(),{minGlobalX:s,minGlobalY:a,maxGlobalX:i,maxGlobalY:r}=o,l=document.createElement("canvas");l.width=Math.max(1,i-s+1),l.height=Math.max(1,r-a+1);let e=l.getContext("2d");if(!e)throw Error("Capture context unavailable");e.imageSmoothingEnabled=!1;let t=Math.floor(s/P),n=Math.floor(a/P),c=Math.floor(i/P),g=Math.floor(r/P),f=(c-t+1)*(g-n+1),d=0;for(let m=t;m<=c;m++)for(let h=n;h<=g;h++){this.status=`⌛ ${p("taskReadingTiles")} [${++d}/${f}]`;let u=await this.loadTileImage(m,h),b=m*P,k=h*P,z=Math.max(s,b),D=Math.min(i,b+P-1),C=Math.max(a,k),N=Math.min(r,k+P-1),F=z-b,oo=C-k,V=D-z+1,j=N-C+1,ao=z-s,so=C-a;e.drawImage(u,F,oo,V,j,ao,so,V,j)}let w=Date.now();await this.downloadCapture(l,"png",w)},()=>{this.setDisabled("capture-template",!1)})}async downloadCapture(o,s,a){let i=s==="webp"?"image/webp":"image/png",r=await new Promise((t,n)=>{o.toBlob((c)=>{if(!c){n(Error(`Failed to create ${s.toUpperCase()} capture file`));return}t(c)},i)}),l=URL.createObjectURL(r),e=document.createElement("a");e.href=l,e.download=`wplace-capture-${a}.${s}`,e.click(),URL.revokeObjectURL(l)}async loadTileImage(o,s){let a;for(let i=1;i<=3;i++)try{let r=new Image;return r.crossOrigin="anonymous",r.referrerPolicy="no-referrer",r.src=`https://backend.wplace.live/files/s0/tiles/${o}/${s}.png?ts=${Date.now()}-${i}`,await T(r,["load"],["error"]),r}catch(r){if(a=r,i<3)await new Promise((l)=>setTimeout(l,i*200))}throw a instanceof Error?a:Error(`Tile fetch failed (${o}/${s})`)}async resolveCaptureBounds(){return this.selectCaptureBounds()}selectCaptureBounds(){return new Promise((o,s)=>{let a=document.createElement("div");a.className="kgm-capture-overlay",a.innerHTML=`<div class="kgm-capture-hint">${p("captureHintSelectArea")}: A → B</div><div class="kgm-capture-box"></div>`;let i=a.querySelector(".kgm-capture-box");document.body.append(a);let r,l,e=()=>{window.removeEventListener("keydown",f,!0),a.removeEventListener("pointermove",c),a.removeEventListener("pointerdown",g),a.remove()},t=(d)=>{let w=Math.min(r.x,d.x),m=Math.min(r.y,d.y),h=Math.abs(r.x-d.x)+1,u=Math.abs(r.y-d.y)+1;return{left:w,top:m,width:h,height:u}},n=(d)=>{let{left:w,top:m,width:h,height:u}=t(d);i.style.left=`${w}px`,i.style.top=`${m}px`,i.style.width=`${h}px`,i.style.height=`${u}px`},c=(d)=>{if(!r)return;n({x:d.clientX,y:d.clientY})},g=(d)=>{if(d.preventDefault(),!r){r={x:d.clientX,y:d.clientY};let z=H.fromScreenPosition(this.bot,r);l={x:z.globalX,y:z.globalY},n(r);return}let w={x:d.clientX,y:d.clientY},m=H.fromScreenPosition(this.bot,w);if(e(),!l){s(Error("Capture anchor point unavailable"));return}let h=Math.min(l.x,m.globalX),u=Math.min(l.y,m.globalY),b=Math.max(l.x,m.globalX),k=Math.max(l.y,m.globalY);if(b-h<1||k-u<1){s(Error("Capture area too small"));return}o({minGlobalX:h,minGlobalY:u,maxGlobalX:b,maxGlobalY:k})},f=(d)=>{if(d.key!=="Escape")return;e(),s(Error("Capture cancelled"))};window.addEventListener("keydown",f,!0),a.addEventListener("pointermove",c),a.addEventListener("pointerdown",g)})}defaultImageScreenPosition(){let o=Math.round(this.element.getBoundingClientRect().width);return{x:Math.max(256,o),y:32}}async compressImageBeforeLoad(o){let s=new Image;if(s.src=o,await T(s,["load"],["error"]),!(s.naturalWidth*s.naturalHeight>3000000||o.length>3000000))return o;let i=document.createElement("canvas");i.width=s.naturalWidth,i.height=s.naturalHeight;let r=i.getContext("2d");if(!r)return o;return r.drawImage(s,0,0),i.toDataURL("image/png")}async waitForStableViewportProjection(){let o=this.defaultImageScreenPosition(),s=0,a;for(let i=0;i<45;i++){await new Promise((c)=>requestAnimationFrame(()=>{c()}));let{anchorScreenPosition:{x:r,y:l},pixelSize:e}=this.bot.findAnchorsForScreen(o);if(!Number.isFinite(e)||e<=0){s=0;continue}let t={anchorX:r,anchorY:l,pixelSize:e};if(!a){a=t,s=1;continue}if(Math.abs(t.anchorX-a.anchorX)+Math.abs(t.anchorY-a.anchorY)+Math.abs(t.pixelSize-a.pixelSize)<0.0012)s++;else s=0;if(a=t,s>=3)return}}update(){if(this.$strategy.value=this.bot.strategy,this.refreshProgress(),this.imagesListDirty=!0,!this.$imagesSection.open)return;this.renderImagesList(),this.imagesListDirty=!1}renderImagesList(){this.$images.innerHTML="";let o=document.createDocumentFragment();for(let s=0;s<this.bot.images.length;s++){let a=this.bot.images[s],i=document.createElement("div");o.append(i),i.className="image",i.innerHTML=`<button class="preview" title="View preview">
  ...
  </div>`,i.querySelector(".preview").addEventListener("click",()=>{this.activeImageIndex=s,a.openPreviewPanel()}),i.querySelector(".focus-map").addEventListener("click",()=>{this.activeImageIndex=s,a.position.scrollScreenTo()}),i.querySelector(".colors").addEventListener("click",()=>{this.activeImageIndex=s,a.openColorPanel()}),i.querySelector(".strategy-modal").addEventListener("click",()=>{this.activeImageIndex=s,a.openPreviewPanel()}),i.querySelector(".preview-strategy").addEventListener("click",()=>{this.activeImageIndex=s,a.openPreviewPanel()}),i.querySelector(".download").addEventListener("click",()=>{a.exportImage()}),i.querySelector(".delete").addEventListener("click",()=>{a.destroy()}),i.querySelector(".up").addEventListener("click",()=>{go(this.bot.images,s,s-1),this.update(),M(this.bot)}),i.querySelector(".down").addEventListener("click",()=>{go(this.bot.images,s,s+1),this.update(),M(this.bot)})}this.$images.append(o)}refreshProgress(){let o=0,s=0;for(let r=0;r<this.bot.images.length;r++){let l=this.bot.images[r];o+=l.pixels.pixels.length*l.pixels.pixels[0].length,s+=l.tasks.length}let a=Math.max(0,o-s),i=o>0?a/o*100|0:0;this.$progressText.textContent=`${a}/${o} ${i}% ETA: ${s/120|0}h`,this.$progressLine.style.transform=`scaleX(${i/100})`}syncOverlayVisibilityFromStorage(){let o=localStorage.getItem(Yo)==="true";document.body.classList.toggle("overlay-hidden",o),this.refreshOverlayToggleText()}toggleOverlay(o){let s=o??!document.body.classList.contains("overlay-hidden");document.body.classList.toggle("overlay-hidden",s),localStorage.setItem(Yo,String(s)),this.refreshOverlayToggleText()}refreshOverlayToggleText(){let o=document.body.classList.contains("overlay-hidden"),s=o?p("disabled"):p("enabled"),a=o?'<i class="fa-solid fa-circle-xmark" aria-hidden="true"></i>':'<i class="fa-solid fa-circle-check" aria-hidden="true"></i>';this.$toggleOverlay.innerHTML=`<i class="fa-solid fa-layer-group"></i><span>${p("toggleOverlay")} (${s})</span>${a}`}applyLocaleToUI(o){no(o),x(this.element);for(let s=0;s<this.bot.images.length;s++)this.bot.images[s].applyLocale();this.refreshOverlayToggleText(),this.refreshImagesCollapseText(),this.refreshAutoFarmStatusText(),this.refreshAutoOverlayStatusText()}applyImagesCollapsedPreference(){let o=this.readImagesCollapsedPreference();if(this.$imagesSection.open=!o,this.refreshImagesCollapseText(),this.$imagesSection.open&&this.imagesListDirty)this.renderImagesList(),this.imagesListDirty=!1}readImagesCollapsedPreference(){let o=localStorage.getItem(Io);if(o==="true")return!0;if(o==="false")return!1;return lo().imagesCollapsed??!1}persistImagesCollapsedPreference(o){localStorage.setItem(Io,String(o)),X({imagesCollapsed:o})}refreshImagesCollapseText(){this.$imagesCollapseState.textContent=this.$imagesSection.open?p("widgetImagesCollapse"):p("widgetImagesExpand")}openSettingsModal(){let o=document.createElement("dialog");o.className="kgm-modal autofarm-dialog",o.innerHTML=`<form method="dialog" class="autofarm-form">
  <div class="kgm-modal-head">
    <strong data-i18n="settingsModalTitle">Settings</strong>
    <button type="button" class="modal-close" aria-label="${p("close")}"><span class="icon">×</span></button>
  </div>
...
</form>`,document.body.append(o),x(o);let s=o.querySelector(".settings-locale");s.value=_(),o.querySelector(".script-update").addEventListener("click",()=>{globalThis.open("https://github.com/robgallardof/kglacer-macro/raw/refs/heads/main/dist.user.js","_blank","noopener,noreferrer")}),s.addEventListener("change",()=>{this.applyLocaleToUI(s.value),x(o)});let a=JSON.parse(localStorage.getItem($o)??"{}"),i=o.querySelector(".proxy-enabled"),r=o.querySelector(".proxy-host"),l=o.querySelector(".proxy-port"),e=o.querySelector(".proxy-user"),t=o.querySelector(".proxy-pass"),n=o.querySelector(".shield-enabled"),c=o.querySelector(".proxy-settings"),g=o.querySelector(".shield-settings"),f=o.querySelector(".shield-controls"),d=o.querySelector(".proxy-test"),w=o.querySelector(".proxy-test-output"),m=o.querySelector(".public-ip-value"),h=o.querySelector(".public-ip-route"),u=o.querySelector(".account-info-refresh"),b=o.querySelector(".account-info-output"),k=async()=>{u.disabled=!0,await this.renderAccountInfoOutput(b),u.disabled=!1};u.addEventListener("click",async()=>{await this.bot.refreshControlAccess("settings").catch(()=>null),await k()}),k(),i.checked=Boolean(a.enabled),n.checked=Mo(),c.open=i.checked,g.open=n.checked,this.renderShieldControls(f),r.value=a.host??"",l.value=a.port??"",e.value=a.username??"",t.value=a.password??"";let z=()=>{let C=i.checked,N=r.value.trim(),F=l.value.trim();localStorage.setItem($o,JSON.stringify({enabled:C,host:N,port:F,username:e.value.trim(),password:t.value})),localStorage.setItem(yo,C&&N&&F?`${N}:${F}`:"DIRECT/SHIELD")},D=async()=>{if(m)m.textContent=p("publicIpChecking");if(h)h.textContent=this.getPublicIpRouteLabel({enabled:i.checked,host:r.value.trim(),port:l.value.trim()});let C=await this.fetchPublicIp();if(m)m.textContent=C??p("publicIpUnavailable")};for(let C of[i,r,l,e,t])C.addEventListener("change",()=>{z(),D()});i.addEventListener("change",()=>{c.open=i.checked}),z(),D(),d.addEventListener("click",async()=>{z();let C=r.value.trim(),N=l.value.trim();if(d.disabled=!0,w)w.innerHTML=`<div class="pending">⏳ ${p("proxyTesting")}</div>`;let F=await this.testProxyConnection(C,N);if(await D(),w)w.innerHTML=`<div class="${F?"ok":"fail"}">${F?"✅":"❌"} ${F?p("proxyOk"):p("proxyFail")}</div>`;else alert(F?p("proxyOk"):p("proxyFail"));d.disabled=!1}),n.addEventListener("change",()=>{g.open=n.checked,this.renderShieldControls(f),Eo(n.checked),window.setTimeout(()=>{location.reload()},120)}),o.querySelector(".modal-close").onclick=()=>{o.close(),o.remove()},o.addEventListener("close",()=>{o.remove()}),o.showModal()}async renderAccountInfoOutput(o){o.innerHTML=`<div class="pending">⌛ ${p("accountInfoLoading")}</div>`;let s=this.bot.getControlSession(),[a,i,r]=await Promise.all([this.bot.fetchAccountInfo(!0).catch(()=>null),this.bot.getAccountCookieStatus({force:!0}).catch(()=>({hasToken:!1,source:"none"})),eo().catch(()=>null)]),l=s?.access,e=s?.serial,t=[[p("settingsAccessStatus"),l?.allowed===!1?p("disabled"):p("enabled")],[p("settingsApiMode"),l?.mode??"—"],[p("settingsControlUser"),s?p("enabled"):p("disabled")],[p("settingsLicenseUser"),e?.username??l?.username??"—"],[p("settingsSerialStatus"),e?.status??(e?.valid?"active":"—")],[p("settingsSerialValidatedAt"),e?.validatedAt??"—"],[p("settingsLicenseOwner"),e?.ownerName??"—"],[p("settingsDeviceLimit"),this.formatDeviceLimit(l,e)],[p("settingsCookieJ"),i.hasToken?p("settingsCookieJDetected"):p("settingsCookieJNotDetected")],[p("settingsCookieSource"),i.source],[p("settingsWplaceId"),a?.id??"—"],[p("settingsWplaceName"),a?.name??"—"],[p("settingsDiscord"),a?.discord??"—"],[p("settingsDiscordId"),a?.discordId??"—"],[p("settingsCountry"),a?.country??"—"],[p("settingsAlliance"),a?.allianceName??"—"],[p("settingsAllianceRole"),a?.allianceRole??"—"],[p("settingsLevel"),a?.level??"—"],[p("settingsPixelsPainted"),a?.pixelsPainted??"—"],[p("settingsDroplets"),a?.droplets??"—"],[p("settingsCharges"),this.formatCharges(a?.charges)],[p("settingsCustomer"),a?.isCustomer===void 0?"—":a.isCustomer?p("enabled"):p("disabled")],[p("settingsSuspension"),a?.suspensionReason??"—"],[p("settingsTimeout"),a?.timeoutUntil??"—"],[p("settingsLocalDeviceId"),r?.localDeviceId??"—"],[p("settingsFingerprint"),r?.deviceFingerprintHash??"—"],[p("settingsUserAgent"),r?.userAgent??navigator.userAgent],[p("settingsPlatform"),r?.platform??navigator.platform],[p("settingsLanguage"),r?.language??navigator.language],[p("settingsTimezone"),r?.timezone??"—"],[p("settingsScreen"),r?`${r.screenWidth}×${r.screenHeight} @${r.devicePixelRatio}`:"—"],[p("settingsTouchSupport"),r?.touchSupport?p("enabled"):p("disabled")],[p("settingsHardwareConcurrency"),r?.hardwareConcurrency??"—"],[p("settingsDeviceMemory"),r?.deviceMemory??"—"],[p("settingsMacAddress"),p("settingsMacUnavailable")]];o.innerHTML=`<div class="account-info-grid">${t.map(([n,c])=>`<div class="account-info-card"><span>${this.escapeHtml(n)}</span><strong>${this.escapeHtml(this.stringifyShieldValue(c))}</strong></div>`).join("")}</div>`}formatDeviceLimit(o,s){let a=o?.registeredDevices,i=o?.maxDevices??s?.maxDevices;if(a===void 0&&i===void 0)return"—";return`${a??"—"} / ${i??"—"}`}formatCharges(o){if(!o||typeof o!=="object")return"—";let s=o,a=typeof s.count==="number"?Math.floor(s.count):s.count;return`${this.formatUnknownValue(a)} / ${this.formatUnknownValue(s.max)} (${this.formatUnknownValue(s.cooldownMs)} ms)`}formatUnknownValue(o){if(typeof o==="string"||typeof o==="number"||typeof o==="boolean")return String(o);return"—"}renderShieldControls(o){let r={navigator:p("shieldFeatureNavigator"),userAgentData:p("shieldFeatureUaData"),screen:p("shieldFeatureScreen"),timezone:p("shieldFeatureTimezone"),canvas:p("shieldFeatureCanvas"),webgl:p("shieldFeatureWebgl"),audio:p("shieldFeatureAudio"),plugins:p("shieldFeaturePlugins"),mediaDevices:p("shieldFeatureMediaDevices"),storageEstimate:p("shieldFeatureStorage"),battery:p("shieldFeatureBattery"),speechSynthesis:p("shieldFeatureSpeech"),fonts:p("shieldFeatureFonts"),matchMedia:p("shieldFeatureMatchMedia"),sharedArrayBuffer:p("shieldFeatureSharedArrayBuffer")},l=this.readStorageJson("__afm_profile",null),e="__afm_profile_choices",t=Number(localStorage.getItem("__afm_profile_expiry")??"0"),n=this.readStorageJson("__afm_settings",{}),c=this.readStorageJson("__afm_profile_choices",[]),f={...Object.fromEntries(Object.keys(r).map((u)=>[u,!0])),...n},d=t>0?new Date(t).toLocaleString():"—",w=l?.id??"Auto",m=c.map((u)=>`<option value="${u.id}" ${u.id===w?"selected":""}>${u.id}</option>`).join(""),h=Object.entries(r).map(([u,b])=>`<label class="kgm-switch-row"><span>${b}</span><span class="kgm-switch"><input type="checkbox" data-shield-key="${u}" ${f[u]?"checked":""}/><span class="kgm-switch-slider" aria-hidden="true"></span></span></label>`).join("");o.innerHTML=`<div class="shield-profile-row"><label>${p("shieldProfile")}</label><select class="shield-profile-select"><option value="">${p("shieldProfileAuto")}</option>${m}</select></div><div class="wp shield-expiry-line">${p("shieldExpires")}: <strong>${d}</strong></div><div class="widget-actions kgm-button-grid"><button type="button" class="challenge-button shield-refresh-profile"><i class="fa-solid fa-rotate"></i><span>${p("shieldRefreshProfile")}</span></button><button type="button" class="challenge-button shield-checker"><i class="fa-solid fa-shield-check"></i><span>${p("shieldChecker")}</span></button><button type="button" class="challenge-button shield-info"><i class="fa-solid fa-circle-info"></i><span>${p("shieldInfo")}</span></button></div><div class="shield-checker-output" aria-live="polite"></div><div class="shield-control-grid">${h}</div>`,o.querySelectorAll("input[data-shield-key]").forEach((u)=>{u.addEventListener("change",()=>{let b=u.dataset.shieldKey;f[b]=u.checked,localStorage.setItem("__afm_settings",JSON.stringify(f)),window.setTimeout(()=>{location.reload()},120)})}),o.querySelector(".shield-profile-select")?.addEventListener("change",(u)=>{let b=u.currentTarget.value;if(!b)localStorage.removeItem("__afm_profile");else{let k=c.find((z)=>z.id===b);localStorage.setItem("__afm_profile",JSON.stringify(k??{id:b}))}location.reload()}),o.querySelector(".shield-checker")?.addEventListener("click",()=>{let u=o.querySelector(".shield-checker-output");if(!u)return;let b=this.runShieldChecker();u.innerHTML=b.map((k)=>`<div class="${k.ok?"ok":"fail"}">${k.ok?"✅":"❌"} ${k.label}</div>`).join("")}),o.querySelector(".shield-info")?.addEventListener("click",()=>{this.openShieldInfoModal()}),o.querySelector(".shield-refresh-profile")?.addEventListener("click",()=>{localStorage.removeItem("__afm_profile"),localStorage.removeItem("__afm_profile_expiry"),location.reload()})}getShieldInfo(){let o=this.readStorageJson("__afm_profile",null),s=this.readStorageJson("__afm_settings",{}),a=this.readStorageJson("__afm_profile_choices",[]);return{injectedInfo:globalThis.__kgmShieldInfo,profile:o,settings:s,choices:a,expiry:Number(localStorage.getItem("__afm_profile_expiry")??"0"),enabled:localStorage.getItem("__afm_enabled")!=="false",proxyHint:localStorage.getItem(yo)??"AUTO"}}readStorageJson(o,s){try{let a=localStorage.getItem(o);if(!a)return s;return JSON.parse(a)}catch{return s}}getPublicIpRouteLabel(o){if(o.enabled&&o.host&&o.port)return`${p("publicIpProxyRoute")} (${o.host}:${o.port})`;return p("publicIpShieldRoute")}async fetchPublicIp(){for(let o of ja)try{let s=await fetch(o,{cache:"no-store"});if(!s.ok)continue;if((s.headers.get("content-type")??"").includes("application/json")){let i=await s.json();if(typeof i.ip==="string"&&i.ip.trim())return i.ip.trim()}else{let i=(await s.text()).trim();if(i)return i}}catch{}return}openShieldInfoModal(){let o=this.getShieldInfo(),s=o.injectedInfo?.profile,a=typeof s==="object"&&s!==null?s:o.profile,i=o.injectedInfo?.settings??o.settings,r=Number(o.injectedInfo?.expiresAt??o.expiry),l=(f,d="—")=>this.stringifyShieldValue(a?.[f],d),e=a?`${l("screenWidth")}×${l("screenHeight")} @${l("devicePixelRatio")}`:"—",t=a?`${l("webglVendor")} / ${l("webglRenderer")}`:"—",n=[[p("shieldInfoInjected"),o.injectedInfo?p("enabled"):p("disabled")],[p("shieldInfoEnabled"),o.enabled?p("enabled"):p("disabled")],[p("shieldProfile"),l("id")],[p("shieldExpires"),r>0?new Date(r).toLocaleString():"—"],[p("shieldInfoBrowser"),this.stringifyShieldValue(o.injectedInfo?.detectedBrowser)],[p("shieldInfoProxyHint"),this.stringifyShieldValue(o.injectedInfo?.proxyHint,o.proxyHint)],[p("publicIpTitle"),p("publicIpChecking")],[p("shieldInfoProfiles"),o.choices.length>0?String(o.choices.length):"—"],["User-Agent",l("userAgent",navigator.userAgent)],["Platform",l("platform",navigator.platform)],["Language",l("language",navigator.language)],["Screen",e],["WebGL",t]],c=Object.entries(i).filter(([,f])=>f).map(([f])=>f).join(", "),g=document.createElement("dialog");g.className="kgm-modal shield-info-dialog",g.innerHTML=`<div class="kgm-modal-head"><strong>${p("shieldInfoTitle")}</strong><button type="button" class="modal-close" aria-label="${p("close")}"><span class="icon">×</span></button></div><div class="shield-info-grid">${n.map(([f,d])=>`<div class="shield-info-card"><span>${this.escapeHtml(f)}</span><strong${f===p("publicIpTitle")?' class="shield-info-public-ip"':""}>${this.escapeHtml(d)}</strong></div>`).join("")}</div><div class="shield-info-modules"><span>${p("shieldInfoModules")}</span><p>${this.escapeHtml(c.length>0?c:"—")}</p></div>`,document.body.append(g),this.fetchPublicIp().then((f)=>{let d=g.querySelector(".shield-info-public-ip");if(d)d.textContent=f??p("publicIpUnavailable")}),g.querySelector(".modal-close").onclick=()=>{g.close(),g.remove()},g.addEventListener("close",()=>{g.remove()}),g.showModal()}stringifyShieldValue(o,s="—"){if(o===void 0||o===null||o==="")return s;if(typeof o==="string"||typeof o==="number"||typeof o==="boolean")return String(o);return JSON.stringify(o)}escapeHtml(o){return String(o).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}async testProxyConnection(o,s){if(!o||!s)return!1;try{return await fetch(`http://${o}:${s}`,{method:"HEAD",mode:"no-cors"}),!0}catch{return!1}}runShieldChecker(){let o=this.getShieldInfo(),s=o.profile,a=o.injectedInfo?.settings,i=typeof a==="object"&&a!==null?a:o.settings,r=Boolean(o.injectedInfo??s);return[{label:p("shieldCheckInjected"),ok:r},{label:p("shieldCheckSettings"),ok:Object.keys(i).length>0},{label:p("shieldCheckProfile"),ok:Boolean(s?.id??o.injectedInfo?.profileId)},{label:p("shieldCheckChoices"),ok:o.choices.length>0},{label:p("shieldCheckNavigator"),ok:navigator.hardwareConcurrency!==0&&typeof navigator.platform==="string"}]}refreshAutoFarmStatusText(){if(!this.autoFarmConfig){this.$autofarmStatus.textContent=p("autoFarmNeedsConfig");return}this.$autofarmStatus.textContent=this.autoFarmIntervalId?`${p("autoFarmRunning")} (${this.formatAutoFarmDelay(this.autoFarmConfig.timerMs)}) · ${this.formatCountdown(this.autoFarmNextTickAt)}`:p("autoFarmStopped")}refreshAutoOverlayStatusText(){if(!this.autoOverlayConfig){this.$autoOverlayStatus.textContent=p("autoOverlayNeedsConfig");return}this.$autoOverlayStatus.textContent=this.autoOverlayIntervalId?`${p("autoOverlayRunning")} (${this.formatAutoFarmDelay(this.autoOverlayConfig.timerMs)}) · ${this.formatCountdown(this.autoOverlayNextTickAt)}`:p("autoOverlayStopped")}formatCountdown(o){if(!o)return"00:00";let s=Math.max(0,o-Date.now()),a=Math.ceil(s/1000),i=Math.floor(a/60),r=a%60;return`${p("nextRunIn")} ${String(i).padStart(2,"0")}:${String(r).padStart(2,"0")}`}formatAutoFarmDelay(o){if(o%3600000===0)return`${o/3600000}h`;if(o%60000===0)return`${o/60000}m`;return`${o/1000}s`}stopAutoFarm(){if(!this.autoFarmIntervalId)return;clearInterval(this.autoFarmIntervalId),this.autoFarmIntervalId=void 0,this.autoFarmNextTickAt=void 0,this.refreshAutoFarmStatusText()}stopAutoOverlay(){if(!this.autoOverlayIntervalId)return;clearInterval(this.autoOverlayIntervalId),this.autoOverlayIntervalId=void 0,this.autoOverlayNextTickAt=void 0,this.refreshAutoOverlayStatusText()}startAutoFarm(){if(!this.autoFarmConfig){this.status=`⚠️ ${p("autoFarmNeedsConfig")}`,this.refreshAutoFarmStatusText();return}this.stopAutoFarm(),this.autoFarmNextTickAt=Date.now()+this.autoFarmConfig.timerMs,this.autoFarmIntervalId=window.setInterval(()=>{this.autoFarmNextTickAt=Date.now()+this.autoFarmConfig.timerMs,this.runAutoFarmCycle()},this.autoFarmConfig.timerMs),this.runAutoFarmCycle(),this.refreshAutoFarmStatusText()}startAutoOverlay(){if(!this.autoOverlayConfig){this.status=`⚠️ ${p("autoOverlayNeedsConfig")}`,this.refreshAutoOverlayStatusText();return}this.stopAutoOverlay(),this.autoOverlayNextTickAt=Date.now()+this.autoOverlayConfig.timerMs,this.autoOverlayIntervalId=window.setInterval(()=>{this.autoOverlayNextTickAt=Date.now()+this.autoOverlayConfig.timerMs,this.runAutoOverlayCycle()},this.autoOverlayConfig.timerMs),this.runAutoOverlayCycle(),this.refreshAutoOverlayStatusText()}async runAutoFarmCycle(){if(!this.autoFarmConfig||this.autoFarmTickRunning)return;this.autoFarmTickRunning=!0;try{if(!await this.bot.drawRandomPixelsBatch(this.resolveCyclePixelCount(this.autoFarmConfig),0)){this.status=`⚠️ ${p("autoFarmStopped")}: ${p("autoFarmTransparentUnavailable")}`,this.stopAutoFarm();return}await this.waitAndClickPaintButton()}finally{this.autoFarmTickRunning=!1}}async runAutoOverlayCycle(){if(!this.autoOverlayConfig||this.autoOverlayTickRunning)return;this.autoOverlayTickRunning=!0;try{if(!await this.bot.drawOverlayPixelsBatch(this.resolveCyclePixelCount(this.autoOverlayConfig))){this.status=`⚠️ ${p("autoOverlayStopped")}: ${p("autoOverlayNoTasks")}`,this.stopAutoOverlay();return}await this.waitAndClickPaintButton()}finally{this.autoOverlayTickRunning=!1}}saveAutoFarmConfig(o){this.autoFarmConfig=o,localStorage.setItem(Xo,JSON.stringify(o)),X({farm:this.toControlPixelSettings(o)})}saveAutoOverlayConfig(o){this.autoOverlayConfig=o,localStorage.setItem(_o,JSON.stringify(o)),X({autoDraw:this.toControlPixelSettings(o)})}resolveCyclePixelCount(o){if(!o.usePixelRange)return Math.max(1,Math.floor(o.pixels));let s=Math.max(1,Math.floor(o.pixelRange.min)),a=Math.max(s,Math.floor(o.pixelRange.max));return s+Math.floor(Math.random()*(a-s+1))}toControlPixelSettings(o){return{usePixelRange:o.usePixelRange,pixel:Math.max(1,Math.floor(o.pixels)),pixelRange:{min:Math.max(1,Math.floor(o.pixelRange.min)),max:Math.max(1,Math.floor(o.pixelRange.max))}}}getRemotePixelSettings(o){return lo()[o]}loadAutoFarmConfigFromStorage(){let o=this.getRemotePixelSettings("farm"),s=localStorage.getItem(Xo);if(!s&&o){this.autoFarmConfig=this.createDefaultAutoConfig(o);return}if(!s)return;try{let a=JSON.parse(s);if(typeof a.value!=="number"||!Number.isFinite(a.value)||a.value<1)return;let i=typeof a.pixels==="number"&&Number.isFinite(a.pixels)&&a.pixels>=1?Math.floor(a.pixels):Math.max(1,Math.floor(o?.pixel??60)),r=this.normalizePixelRange(a.pixelRange,o),l=a.unit==="hours"||a.unit==="minutes"||a.unit==="seconds"?a.unit:"minutes",e=typeof a.timerMs==="number"&&a.timerMs>0?a.timerMs:l==="hours"?a.value*3600000:l==="minutes"?a.value*60000:a.value*1000;this.autoFarmConfig={value:Math.max(1,Math.floor(a.value)),pixels:i,usePixelRange:a.usePixelRange??o?.usePixelRange??!1,pixelRange:r,unit:l,timerMs:e}}catch{return}}loadAutoOverlayConfigFromStorage(){let o=this.getRemotePixelSettings("autoDraw"),s=localStorage.getItem(_o);if(!s&&o){this.autoOverlayConfig=this.createDefaultAutoConfig(o);return}if(!s)return;try{let a=JSON.parse(s);if(typeof a.value!=="number"||!Number.isFinite(a.value)||a.value<1)return;let i=typeof a.pixels==="number"&&Number.isFinite(a.pixels)&&a.pixels>=1?Math.floor(a.pixels):Math.max(1,Math.floor(o?.pixel??60)),r=this.normalizePixelRange(a.pixelRange,o),l=a.unit==="hours"||a.unit==="minutes"||a.unit==="seconds"?a.unit:"minutes",e=typeof a.timerMs==="number"&&a.timerMs>0?a.timerMs:l==="hours"?a.value*3600000:l==="minutes"?a.value*60000:a.value*1000;this.autoOverlayConfig={value:Math.max(1,Math.floor(a.value)),pixels:i,usePixelRange:a.usePixelRange??o?.usePixelRange??!1,pixelRange:r,unit:l,timerMs:e}}catch{return}}createDefaultAutoConfig(o){return{value:1,unit:"minutes",pixels:Math.max(1,Math.floor(o.pixel??60)),usePixelRange:o.usePixelRange??!1,pixelRange:this.normalizePixelRange(o.pixelRange,o),timerMs:60000}}normalizePixelRange(o,s){let a=o&&typeof o==="object"?o:s?.pixelRange,i=typeof a?.min==="number"&&Number.isFinite(a.min)?Math.max(1,Math.floor(a.min)):1,r=typeof a?.max==="number"&&Number.isFinite(a.max)?Math.max(i,Math.floor(a.max)):Math.max(i,5);return{min:i,max:r}}openAutoFarmModal(){let o=document.createElement("dialog");o.className="kgm-modal autofarm-dialog";let s=this.autoFarmConfig?.unit??"minutes",a=this.autoFarmConfig?.value??1,i=this.autoFarmConfig?.pixels??60,r=this.autoFarmConfig?.usePixelRange??!1,l=this.autoFarmConfig?.pixelRange??{min:1,max:5};o.innerHTML=`<form method="dialog" class="autofarm-form">
  <div class="kgm-modal-head">
...
</form>`,document.body.append(o),x(o);let e=o.querySelector(".autofarm-unit");e.value=s;let t=o.querySelector(".autofarm-value"),n=o.querySelector(".autofarm-pixels"),c=o.querySelector(".autofarm-use-range"),g=o.querySelector(".autofarm-range-row"),f=o.querySelector(".autofarm-range-min"),d=o.querySelector(".autofarm-range-max"),w=o.querySelector(".pixel-range-error"),m=()=>{g.hidden=!c.checked,n.disabled=c.checked};c.addEventListener("change",m),m();let h=()=>{let b=Math.max(1,Number.parseInt(t.value||"1",10));if(e.value==="hours")return b*3600000;if(e.value==="minutes")return b*60000;return b*1000},u=()=>{let b=Math.max(1,Number.parseInt(f.value||"1",10)),k=Math.max(1,Number.parseInt(d.value||"1",10));if(b>k)return w.textContent=p("pixelRangeInvalid"),null;return w.textContent="",{min:b,max:k}};o.querySelector(".autofarm-start").onclick=()=>{let b=u();if(!b)return;this.saveAutoFarmConfig({value:Math.max(1,Number.parseInt(t.value||"1",10)),pixels:Math.max(1,Number.parseInt(n.value||"60",10)),usePixelRange:c.checked,pixelRange:b,unit:e.value,timerMs:h()}),this.startAutoFarm(),o.close(),o.remove()},o.querySelector(".autofarm-stop").onclick=()=>{this.stopAutoFarm(),o.close(),o.remove()},o.querySelector(".modal-close").onclick=()=>{o.close(),o.remove()},o.addEventListener("close",()=>{o.remove()}),o.showModal()}openAutoOverlayModal(){let o=document.createElement("dialog");o.className="kgm-modal autofarm-dialog";let s=this.autoOverlayConfig?.unit??"minutes",a=this.autoOverlayConfig?.value??1,i=this.autoOverlayConfig?.pixels??60,r=this.autoOverlayConfig?.usePixelRange??!1,l=this.autoOverlayConfig?.pixelRange??{min:1,max:5};o.innerHTML=`<form method="dialog" class="autofarm-form">
  <div class="kgm-modal-head">
...
</form>`,document.body.append(o),x(o);let e=o.querySelector(".autofarm-unit");e.value=s;let t=o.querySelector(".autofarm-value"),n=o.querySelector(".autofarm-pixels"),c=o.querySelector(".autofarm-use-range"),g=o.querySelector(".autofarm-range-row"),f=o.querySelector(".autofarm-range-min"),d=o.querySelector(".autofarm-range-max"),w=o.querySelector(".pixel-range-error"),m=()=>{g.hidden=!c.checked,n.disabled=c.checked};c.addEventListener("change",m),m();let h=()=>{let b=Math.max(1,Number.parseInt(t.value||"1",10));if(e.value==="hours")return b*3600000;if(e.value==="minutes")return b*60000;return b*1000},u=()=>{let b=Math.max(1,Number.parseInt(f.value||"1",10)),k=Math.max(1,Number.parseInt(d.value||"1",10));if(b>k)return w.textContent=p("pixelRangeInvalid"),null;return w.textContent="",{min:b,max:k}};o.querySelector(".autooverlay-start").onclick=()=>{let b=u();if(!b)return;this.saveAutoOverlayConfig({value:Math.max(1,Number.parseInt(t.value||"1",10)),pixels:Math.max(1,Number.parseInt(n.value||"60",10)),usePixelRange:c.checked,pixelRange:b,unit:e.value,timerMs:h()}),this.startAutoOverlay(),o.close(),o.remove()},o.querySelector(".autooverlay-stop").onclick=()=>{this.stopAutoOverlay(),o.close(),o.remove()},o.querySelector(".modal-close").onclick=()=>{o.close(),o.remove()},o.addEventListener("close",()=>{o.remove()}),o.showModal()}getCurrentWplaceLocation(){let o=(f)=>{let d=new URLSearchParams(f.replace(/^#/,"").replace(/^\?/,"")),w=Number.parseFloat(d.get("lat")??""),m=Number.parseFloat(d.get("lng")??""),h=Number.parseFloat(d.get("zoom")??"");if(Number.isFinite(w)&&Number.isFinite(m)&&Number.isFinite(h))return{lat:w,lng:m,zoom:h}},s=globalThis.location.hash,a=s.includes("?")?s.slice(s.indexOf("?")+1):"",i=[globalThis.location.search,s,a].filter(Boolean);for(let f of i){let d=o(f);if(d)return d}let r=/#?\/?(?<zoom>-?\d+(?:\.\d+)?)\/(?<lat>-?\d+(?:\.\d+)?)\/(?<lng>-?\d+(?:\.\d+)?)/.exec(s);if(!r?.groups)return;let{lat:l,lng:e,zoom:t}=r.groups;if(!l||!e||!t)return;let n=Number.parseFloat(l),c=Number.parseFloat(e),g=Number.parseFloat(t);if(!Number.isFinite(n)||!Number.isFinite(c)||!Number.isFinite(g))return;return{lat:n,lng:c,zoom:g}}buildExternalToolUrl(o){let s=this.getCurrentWplaceLocation();if(o==="colorConverter")return Ka;if(o==="receiveSmss")return va;if(o==="esimplus")return La;if(o==="receiveSmsFree")return Qa;if(o==="quackr")return Ga;if(o==="textverified")return Wa;if(!s){if(o==="samuelArchive")return oa;let i=new URL(aa);return i.searchParams.set("lat","0.000000"),i.searchParams.set("lng","0.000000"),i.searchParams.set("zoom","2.00"),i.searchParams.set("version",sa),i.toString()}if(o==="samuelArchive"){let i=new URL(oa);return i.hash=`${s.zoom.toFixed(2)}/${s.lat.toFixed(6)}/${s.lng.toFixed(6)}`,i.toString()}let a=new URL(aa);return a.searchParams.set("lat",s.lat.toFixed(6)),a.searchParams.set("lng",s.lng.toFixed(6)),a.searchParams.set("zoom",s.zoom.toFixed(2)),a.searchParams.set("version",sa),a.toString()}openExternalTool(o){this.openUrlInNewTab(this.buildExternalToolUrl(o))}openUrlInNewTab(o){let s=globalThis.open(o,"_blank","noopener");if(s){s.opener=null;return}let a=document.createElement("a");a.href=o,a.target="_blank",a.rel="noopener noreferrer",a.style.display="none",document.body.append(a),a.click(),a.remove()}setDisabled(o,s){this.element.querySelector("."+o).disabled=s}async run(o,s,a,i="..."){console.log("[KGM][Widget] Task started",{status:o});let r=this.status;this.status=`${i} ${o}`;try{let l=await s();return this.status=r,console.log("[KGM][Widget] Task completed",{status:o}),l}catch(l){if(!(l instanceof co))console.error(l),this.status=`${p("taskErrorPrefix")}: ${o}`;throw console.error("[KGM][Widget] Task failed",{status:o,error:l}),l}finally{await a?.()}}handleKeyboard(o){if(Oo(o.target))return;if(S(o,A.toggleWidget)){o.preventDefault(),this.open=!this.open;return}if(S(o,A.minimizeWidget)){o.preventDefault(),this.open=!1;return}if(S(o,A.showWidgetPanel)){o.preventDefault(),this.open=!0;return}if(S(o,A.hideWidgetPanel)){o.preventDefault(),this.open=!1;return}if(S(o,A.showShortcuts)){o.preventDefault(),this.open=!0,this.openSettingsModal();return}if(S(o,A.toggleOverlay)){o.preventDefault(),this.toggleOverlay();return}if(S(o,A.focusNextImage)){o.preventDefault(),this.focusImageByStep(1);return}if(S(o,A.focusPreviousImage)){o.preventDefault(),this.focusImageByStep(-1);return}if(S(o,A.openColorPanel)){o.preventDefault(),this.openColorPanelForActiveImage();return}if(S(o,A.toggleImageLock)){o.preventDefault(),this.toggleLockForActiveImage();return}if(S(o,A.clickPaintWhenReady)){o.preventDefault(),this.drawAndClickPaintWhenReady();return}if(S(o,A.startAutoFarm)){o.preventDefault(),this.startAutoFarm();return}if(S(o,A.stopAutoFarm)){o.preventDefault(),this.stopAutoFarm();return}if(S(o,A.openColorConverterTool)){o.preventDefault(),this.openExternalTool("colorConverter");return}if(S(o,A.openSamuelArchiveTool)){o.preventDefault(),this.openExternalTool("samuelArchive");return}if(S(o,A.openEralyonArchiveTool)){o.preventDefault(),this.openExternalTool("eralyonArchive");return}if(S(o,A.openReceiveSmssTool)){o.preventDefault(),this.openExternalTool("receiveSmss");return}if(S(o,A.openEsimplusTool)){o.preventDefault(),this.openExternalTool("esimplus");return}if(S(o,A.openReceiveSmsFreeTool)){o.preventDefault(),this.openExternalTool("receiveSmsFree");return}if(S(o,A.openQuackrTool)){o.preventDefault(),this.openExternalTool("quackr");return}if(S(o,A.openTextverifiedTool)){o.preventDefault(),this.openExternalTool("textverified");return}if(S(o,A.addImage)&&!this.$addImage.disabled){o.preventDefault(),this.addImage();return}if(S(o,A.draw)&&!this.$draw.disabled)o.preventDefault(),this.bot.draw()}focusImageByStep(o){if(!this.bot.images.length)return;if(this.activeImageIndex<0||this.activeImageIndex>=this.bot.images.length)this.activeImageIndex=o>0?0:this.bot.images.length-1;else this.activeImageIndex=(this.activeImageIndex+o+this.bot.images.length)%this.bot.images.length;this.bot.images[this.activeImageIndex].position.scrollScreenTo()}async recommendUpdateIfOutdated(){let o=new AbortController,s=window.setTimeout(()=>{o.abort()},1800);try{let a=await fetch("https://raw.githubusercontent.com/robgallardof/kglacer-macro/main/src/version.ts",{signal:o.signal});if(!a.ok)return;let i=await a.text(),l=/APP_VERSION = '([^']+)'/.exec(i)?.[1];if(!l)return;if(this.compareSemver(l,Q)<=0)return;let e=`kglacer-macro:update-notice:${l}`;if(localStorage.getItem(e)==="dismissed")return;if(confirm(`Hay una versión nueva (${l}) disponible. Tu versión actual es ${Q}. ¿Quieres actualizar ahora?`))this.openUrlInNewTab("https://github.com/robgallardof/kglacer-macro/raw/refs/heads/main/dist.user.js");else localStorage.setItem(e,"dismissed")}catch{}finally{clearTimeout(s)}}compareSemver(o,s){let a=o.split(".").map((r)=>Number(r)||0),i=s.split(".").map((r)=>Number(r)||0);for(let r=0;r<3;r++){if((a[r]??0)>(i[r]??0))return 1;if((a[r]??0)<(i[r]??0))return-1}return 0}getActiveImage(){if(!this.bot.images.length)return;if(this.activeImageIndex<0||this.activeImageIndex>=this.bot.images.length)this.activeImageIndex=0;return this.bot.images[this.activeImageIndex]}openColorPanelForActiveImage(){let o=this.getActiveImage();if(!o)return;o.openColorPanel()}toggleLockForActiveImage(){let o=this.getActiveImage();if(!o)return;o.lock=!o.lock,o.update(),M(this.bot)}async waitAndClickPaintButton(){await this.run(p("taskWaitingPaintButton"),async()=>{for(;;){if(this.isChallengeBlockingPaint()){await this.waitForChallengeToResolve(),await new Promise((s)=>setTimeout(s,250));continue}let o=this.findNativePaintButton();if(o&&!o.disabled&&o.ariaDisabled!=="true"){await this.triggerNativePaintClickWithChallengeRecovery(o);return}await new Promise((s)=>setTimeout(s,500))}})}async drawAndClickPaintWhenReady(){if(!this.$draw.disabled)await this.bot.draw();await this.waitAndClickPaintButton()}findNativePaintButton(){return["button.btn.btn-primary.btn-lg.sm\\:btn-xl.relative","button.btn.btn-primary.btn-lg.relative","button.btn.btn-primary.btn-lg.relative.z-30","button.btn.btn-primary.btn-lg.sm\\:btn-xl.relative.z-30","div.absolute.bottom-0.left-1\\/2.-translate-x-1\\/2 button.btn.btn-primary"].flatMap((a)=>Array.from(document.querySelectorAll(a))).find((a)=>/pintar|paint/i.test(a.textContent))}triggerNativePaintClick(o){o.dispatchEvent(new PointerEvent("pointerdown",{bubbles:!0,cancelable:!0,pointerType:"mouse",button:0})),o.dispatchEvent(new MouseEvent("mousedown",{bubbles:!0,cancelable:!0,button:0})),o.dispatchEvent(new PointerEvent("pointerup",{bubbles:!0,cancelable:!0,pointerType:"mouse",button:0})),o.dispatchEvent(new MouseEvent("mouseup",{bubbles:!0,cancelable:!0,button:0})),o.click()}async triggerNativePaintClickWithChallengeRecovery(o){for(let a=0;a<3;a++){let i=a===0?o:this.findNativePaintButton();if(!i)return;if(i.disabled||i.ariaDisabled==="true")return;this.triggerNativePaintClick(i);let r=await this.waitForPaintAttemptOutcome(6000);if(r==="painted")return;if(r==="challenge"){await this.waitForChallengeToResolve(),await new Promise((l)=>setTimeout(l,350));continue}await new Promise((l)=>setTimeout(l,350))}console.log("[KGM][Widget] Paint click finished without a clear success signal after retries")}async waitForPaintAttemptOutcome(o){let s=Date.now();while(Date.now()-s<=o){if(this.isChallengeBlockingPaint())return"challenge";let a=this.findNativePaintButton();if(a&&(a.disabled||a.ariaDisabled==="true"))return await this.waitForDelayedChallenge(1200)?"challenge":"painted";await new Promise((i)=>setTimeout(i,200))}return"unknown"}async waitForDelayedChallenge(o){let s=Date.now();while(Date.now()-s<=o){if(this.isChallengeBlockingPaint())return!0;await new Promise((a)=>setTimeout(a,150))}return!1}async waitForChallengeToResolve(){await this.run(p("taskWaitingChallengeResolve"),async()=>{let o=Date.now(),s=90000;while(this.isChallengeBlockingPaint()&&Date.now()-o<=90000)await new Promise((a)=>setTimeout(a,500))})}isChallengeBlockingPaint(){let a=Array.from(document.querySelectorAll('h-captcha, .h-captcha, iframe[src*="hcaptcha.com"], iframe[src*="newassets.hcaptcha.com"], iframe[src*="captcha"], [data-hcaptcha-widget-id]')).filter((l)=>{if(l.closest("dialog")?.matches("dialog:not([open])"))return!1;let e=globalThis.getComputedStyle(l);if(e.display==="none"||e.visibility==="hidden")return!1;let t=l.getBoundingClientRect();return t.width>0&&t.height>0});if(!a.length)return!1;let i=document.querySelector("dialog.modal[open], dialog[open]");if(i?.querySelector('h-captcha, .h-captcha, iframe[src*="hcaptcha.com"], iframe[src*="newassets.hcaptcha.com"], iframe[src*="captcha"], [data-hcaptcha-widget-id]')){if(!i)return!1;if(!Array.from(i.querySelectorAll('textarea[name="h-captcha-response"], textarea[name^="h-captcha-response-"]')).some((e)=>e.value.trim().length>0))return!0}return a.some((l)=>{let e=l.closest("h-captcha")??l.parentElement??document.documentElement,t=Array.from(e.querySelectorAll('textarea[name="h-captcha-response"], textarea[name^="h-captcha-response-"]'));if(!t.length)return!0;return t.every((n)=>n.value.trim().length===0)})}}var Ba=2;function Za(){let o=globalThis;if(typeof o.fp_assemble_injection!=="function")o.fp_assemble_injection=()=>({});if(!o.__kgmUnhandledRejectionPatched)o.__kgmUnhandledRejectionPatched=!0,o.addEventListener("unhandledrejection",(s)=>{let a=s.reason,i=typeof a==="object"&&a!==null&&"name"in a&&typeof a.name==="string"?a.name:"",r=a instanceof Error?a.message:a;if(i==="NotAllowedError"&&r.includes("play() failed"))s.preventDefault()});if(!o.__kgmMediaPlayPatched&&"HTMLMediaElement"in o){o.__kgmMediaPlayPatched=!0;let s=Reflect.get(o.HTMLMediaElement.prototype,"play");o.HTMLMediaElement.prototype.play=function(){return Reflect.apply(s,this,[]).catch((r)=>{let l=r instanceof Error?r.message:r;if((typeof r==="object"&&r!==null&&"name"in r&&typeof r.name==="string"?r.name:"")==="NotAllowedError"&&l.includes("play() failed"))return;throw r})}}}var ia="[KGM]",ra="kgm-access-locked";class la{unavailableColors=new Set;mapsCache=new Map;me;$stars=[];strategy="SEQUENTIAL";images=[];_widget;get widget(){if(!this._widget)throw Error("Widget is not initialized yet");return this._widget}markerPixelPositionResolvers=[];lastColor;accountCookieTokenCache;accountCookieTokenSource="none";controlSession=bo();controlAccessAllowed=!1;log(o,s){if(s===void 0)console.log(`${ia} ${o}`);else console.log(`${ia} ${o}`,s)}constructor(){this.log("Boot sequence started"),document.body.classList.add(ra);let o=Zo();if(this.log("Save loaded",{hasSave:Boolean(o),imageCount:o?.images.length??0,strategy:o?.strategy}),o){for(let i=0;i<o.images.length;i++){let r=o.images[i];y({x:r.position[0]-1000,y:r.position[1]-1000}),y({x:r.position[0]+1000,y:r.position[1]+1000})}this.strategy=o.strategy}let s=JSON.parse(localStorage.getItem("kglacer-macro:proxy-config")??"{}");qo(s),this.registerFetchInterceptor(),this.log("Fetch interceptor registered");let a=document.createElement("style");a.textContent=Ro.replace("FAKE_FAVORITE_LOCATIONS",O.length.toString()),document.head.append(a),this.log("Styles injected",{fakeFavoriteLocations:O.length}),(async()=>{this.log("Widget initialization flow started"),await this.ensureControlAccess(),document.body.classList.remove(ra),this._widget=new Co(this),await this.widget.run(p("taskInitializing"),async()=>{await this.waitForElement("login",".avatar.center-absolute.absolute"),await this.waitForElement("pixel count",".btn.btn-primary.btn-lg.relative.z-30 canvas");let i=await this.waitForElement("canvas",".maplibregl-canvas-container");if(new MutationObserver((r)=>{for(let l=0;l<r.length;l++)if(r[l].removedNodes.length!==0){this.updateStars();break}this.updateImages()}).observe(i,{attributes:!0,childList:!0,subtree:!0}),this.updateStars(),this.log("Stars updated after boot",{stars:this.$stars.length}),await J(500),await this.updateColors(),o)for(let r=0;r<o.images.length;r++){let l=await U.fromJSON(this,o.images[r]);this.images.push(l),l.update()}this.log("Saved images restored",{images:this.images.length}),await this.readMap(),this.updateTasks(),this.widget.setDisabled("draw",!1),this.widget.setDisabled("draw-and-paint",!1),this.widget.setDisabled("add-image",!1),this.widget.setDisabled("capture-template",!1),this.log("Initialization completed; controls enabled")})})()}async ensureControlAccess(){let o=bo();if(ko(o)){this.controlSession=o,this.controlAccessAllowed=!0;try{await this.refreshControlAccess("startup");return}catch(s){if(this.log("Cached Control API session rejected",{reason:s instanceof Error?s.message:"unknown"}),this.shouldClearControlSession(s))ho(),this.controlSession=null,this.controlAccessAllowed=!1;else{this.log("Keeping cached Control API session after transient check failure");return}}}await new Promise((s)=>{let a=document.createElement("dialog");a.className="kgm-modal access-dialog",a.innerHTML=`<form method="dialog" class="access-form">
  <div class="kgm-modal-head">
...
</form>`,document.body.append(a),x(a);let i=a.querySelector(".access-serial"),r=a.querySelector(".access-submit"),l=a.querySelector(".access-error"),e=a.querySelector(".access-locale");e.innerHTML=Lo().map((t)=>`<option value="${t}" ${t===_()?"selected":""}>${t.toUpperCase()}</option>`).join(""),e.addEventListener("change",()=>{no(e.value),x(a)}),a.addEventListener("cancel",(t)=>{t.preventDefault()}),a.querySelector("form").addEventListener("submit",(t)=>{t.preventDefault(),l.textContent="",r.disabled=!0,r.textContent=p("loginChecking"),(async()=>{try{let n=await this.fetchAccountInfo(!0).catch(()=>null),c=await this.readAccountCookieToken({force:!0});this.controlSession=await Ko({serialKey:i.value.trim(),wplaceMe:n,wplaceCookieJToken:c}),this.controlAccessAllowed=!0,a.close(),a.remove(),s()}catch(n){let c=n instanceof Error?n.message:p("loginErrorUnknown");l.textContent=this.mapControlLoginError(c),r.disabled=!1,r.textContent=p("loginSubmit")}})()}),a.showModal(),i.focus()})}mapControlLoginError(o){if(/invalid_serial|invalid_token|blocked_token|expired_license|inactive_license/i.test(o))return p("invalidAccessKey");if(/device_limit/i.test(o))return p("accessDeviceLimit");return p("loginErrorUnknown")}shouldClearControlSession(o){if(!(o instanceof I))return!1;if(o.status===401||o.status===403)return!0;return/invalid|expired|inactive|blocked|device_limit/i.test(o.reason??o.message)}getControlSession(){return this.controlSession}isControlAccessAllowed(){return this.controlAccessAllowed&&ko(this.controlSession)}async refreshControlAccess(o="manual"){if(!this.controlSession)throw Error(p("accessLoginRequired"));let s=this.me??await this.fetchAccountInfo().catch(()=>null),a=await this.readAccountCookieToken({force:!0}),i={hasToken:Boolean(a),source:this.accountCookieTokenSource};return this.controlSession=await zo({session:this.controlSession,eventType:"check",wplaceMe:s,wplaceCookieJToken:a,cookieStatus:i,metadata:{reason:o}}),this.controlAccessAllowed=!0,{session:this.controlSession,cookieStatus:i}}ensureFeatureAccess(o){if(this.isControlAccessAllowed())return!0;this.log("Feature blocked by Control API access state",{feature:o});try{this.widget.status=`⚠️ ${p("accessDenied")}`}catch{}return!1}getPageWindow(){return globalThis.unsafeWindow??globalThis}async fetchAccountInfo(o=!1){if(!o&&this.me)return this.me;let s=await fetch("https://backend.wplace.live/me",{credentials:"include",cache:"no-store"});if(!s.ok)throw Error(`/me failed (${s.status})`);let a=await s.json();return this.me=a,a}async getAccountCookieStatus(o={}){let s=await this.readAccountCookieToken(o);return{hasToken:Boolean(s),source:this.accountCookieTokenSource}}async readAccountCookieToken(o={}){if(!o.force&&this.accountCookieTokenCache)return this.accountCookieTokenCache;let s=this.getCookieFromDocument("j");if(s)return this.accountCookieTokenCache=s,this.accountCookieTokenSource="document",s;let a=await this.readCookieWithCookieStore("j");if(a)return this.accountCookieTokenCache=a,this.accountCookieTokenSource="cookie_store",a;let i=await this.readCookieWithUserscriptApi("j");if(i){if(this.accountCookieTokenCache=i,!this.accountCookieTokenSource.startsWith("gm_cookie"))this.accountCookieTokenSource="gm_cookie";return i}return this.accountCookieTokenSource="none",null}getCookieFromDocument(o){let s=this.getPageWindow(),a=[document.cookie,s.document.cookie].filter((i)=>typeof i==="string");for(let i of a){let r=this.parseCookieString(i,o);if(r)return r}return null}parseCookieString(o,s){let a=`${s}=`;for(let i of o.split(";")){let r=i.trim();if(!r.startsWith(a))continue;return decodeURIComponent(r.slice(a.length))}return null}async readCookieWithCookieStore(o){let s=this.getPageWindow(),a=[Reflect.get(globalThis,"cookieStore"),Reflect.get(s,"cookieStore")];for(let i of a){if(!i||typeof i!=="object")continue;let r=i.get;if(typeof r!=="function")continue;try{let l=await r.call(i,o);if(l?.value)return l.value}catch(l){this.log("cookieStore read failed",l)}}return null}async readCookieWithUserscriptApi(o){let s=this.getPageWindow(),a=globalThis,i=s,r=[a.GM?.cookie,i.GM?.cookie,a.GM_cookie,i.GM_cookie].filter((e)=>e!==void 0&&e!==null);this.log("Reading WPlace j cookie through userscript APIs",{apiCount:r.length,cookieDomain:".wplace.live",cookieName:o});let l=[{domain:".wplace.live",name:o,path:"/"},{domain:".wplace.live",name:o},{domain:".wplace.live"},{domain:"wplace.live",name:o,path:"/"},{domain:"wplace.live",name:o},{domain:"wplace.live"},{url:"https://wplace.live/",name:o,path:"/"},{url:"https://wplace.live/",name:o},{url:"https://wplace.live/"},{url:"https://backend.wplace.live/",name:o,path:"/"},{url:"https://backend.wplace.live/",name:o},{url:"https://backend.wplace.live/"},{url:"https://www.wplace.live/",name:o,path:"/"},{url:"https://www.wplace.live/",name:o},{url:"https://www.wplace.live/"},{firstPartyDomain:"https://wplace.live",domain:".wplace.live",name:o},{firstPartyDomain:"wplace.live",domain:".wplace.live",name:o},{name:o},{}];for(let e of r)for(let t of l){let n=await this.callUserscriptCookieApi(e,"list",t),c=this.findCookieValue(n,o);if(c)return this.accountCookieTokenSource=`gm_cookie:list:${this.describeCookieQuery(t)}`,c;let g=await this.callUserscriptCookieApi(e,"get",t),f=this.extractCookieValue(g,o);if(f)return this.accountCookieTokenSource=`gm_cookie:get:${this.describeCookieQuery(t)}`,f}return null}describeCookieQuery(o){if(o.domain)return o.domain;if(o.url)return o.url;if(o.firstPartyDomain)return o.firstPartyDomain;if(o.name)return o.name;return"all"}async callUserscriptCookieApi(o,s,a){return new Promise((i)=>{let r=!1,l=(t)=>{if(r)return;r=!0,i(t)},e=(...t)=>{l(this.normalizeUserscriptCookieCallbackArgs(t))};try{if(typeof o==="function"){let t=o(s,a,e);this.resolveCookieApiResult(t,l)}else if(o&&typeof o==="object"){let t=o[s];if(typeof t==="function"){let n=t.call(o,a,e);this.resolveCookieApiResult(n,l)}else l(void 0)}else l(void 0)}catch(t){this.log(`GM.cookie ${s} failed`,t),l(void 0)}window.setTimeout(()=>{l(void 0)},1200)})}normalizeUserscriptCookieCallbackArgs(o){if(o.length<=1)return o[0];return o.find((a)=>{if(Array.isArray(a))return!0;if(!a||typeof a!=="object")return!1;let i=a;return Array.isArray(i.cookies)||typeof i.name==="string"||typeof i.value==="string"})??o}resolveCookieApiResult(o,s){if(o&&typeof o.then==="function"){o.then(s,()=>{s(void 0)});return}if(o!==void 0)s(o)}findCookieValue(o,s){let a=this.normalizeCookieList(o);for(let i of a)if(i.name===s&&i.value)return i.value;return null}extractCookieValue(o,s){let a=o;if(a?.name===s&&a.value)return a.value;if(a&&!a.name&&a.value)return a.value;return this.findCookieValue(o,s)}normalizeCookieList(o){if(Array.isArray(o))return o.flatMap((s)=>this.normalizeCookieList(s));if(o&&typeof o==="object"){let s=o;if(Array.isArray(s.cookies))return this.normalizeCookieList(s.cookies);if(s.cookie)return this.normalizeCookieList(s.cookie);if(s.result)return this.normalizeCookieList(s.result);if(s.response)return this.normalizeCookieList(s.response);if(s.name||s.value)return[s]}return[]}async syncAccountInfoWithControl(o="account_info"){if(!this.controlSession)return{ok:!1,cookieStatus:{hasToken:!1,source:this.accountCookieTokenSource}};let s=this.me??await this.fetchAccountInfo().catch(()=>null),a=await this.readAccountCookieToken({force:!0}),i={hasToken:Boolean(a),source:this.accountCookieTokenSource};try{return this.controlSession=await zo({session:this.controlSession,eventType:"heartbeat",wplaceMe:s,wplaceCookieJToken:a,cookieStatus:i,metadata:{app:No,version:Q,reason:o,sentAt:new Date().toISOString(),cookieName:"j",accountTokenAvailable:Boolean(a),jTokenAvailable:Boolean(a),page:{href:location.href,host:location.host}}}),this.controlAccessAllowed=!0,{ok:!0,cookieStatus:i}}catch(r){return this.controlAccessAllowed=!1,this.log("Control API sync failed",{reason:r instanceof Error?r.message:"unknown"}),{ok:!1,cookieStatus:i}}}draw(){if(!this.ensureFeatureAccess("draw"))return Promise.resolve();this.log("Draw requested",{strategy:this.strategy,images:this.images.length}),this.widget.setDisabled("draw",!0),this.widget.setDisabled("draw-and-paint",!0),this.widget.status="",this.mapsCache.clear();let o=document.querySelector(".maplibregl-canvas"),s=(a)=>{if(!a.shiftKey)a.stopPropagation()};return this.widget.run(p("taskDrawing"),async()=>{await this.widget.run(p("taskInitializingDraw"),()=>Promise.all([this.updateColors(),this.readMap()])),globalThis.addEventListener("mousemove",s,!0),o.addEventListener("wheel",s,!0),this.updateTasks();let a=await fetch("https://backend.wplace.live/me",{credentials:"include"}).then((l)=>l.json()),i=Math.floor(a.charges.count);this.log("Charges fetched",{charges:i});let r=0;for(let l=0;l<this.images.length;l++)r+=this.images[l].tasks.length;switch(this.log("Tasks prepared",{tasks:r}),this.strategy){case"ALL":{while(i>0){let l=!0;for(let e=0;e<this.images.length;e++){let t=this.images[e].tasks.shift();if(!t)continue;this.drawTask(t),i--,await J(1),l=!1}if(l)break}break}case"PERCENTAGE":{for(let l=0;l<r&&i>0;l++){let e=1,t;for(let n=0;n<this.images.length;n++){let c=this.images[n],g=1-c.tasks.length/(c.pixels.pixels.length*c.pixels.pixels[0].length);if(g<e)e=g,t=c}this.drawTask(t.tasks.shift()),i--,await J(1)}break}case"SEQUENTIAL":for(let l=0;l<this.images.length;l++){let e=this.images[l];for(let t=e.tasks.shift();t&&i>0;t=e.tasks.shift())this.drawTask(t),i--,await J(1)}}this.widget.update(),await this.readMap(),this.updateTasks(),this.log("Draw flow finished",{remainingCharges:i,remainingTasks:this.images.reduce((l,e)=>l+e.tasks.length,0)})},()=>{globalThis.removeEventListener("mousemove",s,!0),o.removeEventListener("wheel",s,!0),this.widget.setDisabled("draw",!1),this.widget.setDisabled("draw-and-paint",!1)})}toJSON(){return{version:Ba,images:this.images.map((o)=>o.toJSON()),strategy:this.strategy}}async updateColors(){this.log("Updating colors palette"),await this.openColors(),this.unavailableColors.clear();for(let o of document.querySelectorAll("button.btn.relative.w-full"))if(o.children.length!==0)this.unavailableColors.add(Math.abs(Number.parseInt(o.id.slice(6))));this.updateImageColors(),this.log("Colors updated",{unavailableColors:this.unavailableColors.size})}moveMap(o){let s=document.querySelector(".maplibregl-canvas"),a=window.innerWidth/2,i=window.innerHeight/2,r=a-o.x,l=i-o.y;function e(t,n,c){s.dispatchEvent(new MouseEvent(t,{bubbles:!0,cancelable:!0,clientX:n,clientY:c,buttons:1}))}e("mousedown",a,i),e("mousemove",r,l),e("mouseup",r,l)}readMap(){this.mapsCache.clear();let o=new Set;for(let a=0;a<this.images.length;a++){let i=this.images[a],{tileX:r,tileY:l}=new H(this,i.position.globalX+i.pixels.pixels[0].length,i.position.globalY+i.pixels.pixels.length);for(let e=i.position.tileX;e<=r;e++)for(let t=i.position.tileY;t<=l;t++)o.add(`${e}/${t}`)}let s=0;return this.log("Reading map tiles",{tileCount:o.size}),this.widget.run(`${p("taskReadingMap")} [0/${o.size}]`,()=>Promise.all([...o].map(async(a)=>{this.mapsCache.set(a,await v.fromJSON(this,{url:`https://backend.wplace.live/files/s0/tiles/${a}.png`,exactColor:!0})),this.widget.status=`⌛ ${p("taskReadingMap")} [${++s}/${o.size}]`})))}waitForUnfocus(){return this.widget.run("UNFOCUS WINDOW",()=>new Promise((o)=>{if(!document.hasFocus())o();window.addEventListener("blur",()=>{setTimeout(o,1)},{once:!0})}),void 0,"\uD83D\uDDB1️")}findAnchorsForScreen(o){let s=0,a=1,i=1/0,r=1/0;for(let t=0;t<this.$stars.length;t++){let{x:n,y:c}=Z(this.$stars[t]);if(n<o.x&&c<o.y){let g=o.x-n+(o.y-c);if(g<i)i=g,s=t}else if(n>o.x&&c>o.y){let g=n-o.x+(c-o.y);if(g<r)r=g,a=t}}let l=Z(this.$stars[s]),e=L[s];return{anchorScreenPosition:l,anchorWorldPosition:e,pixelSize:(Z(this.$stars[a]).x-l.x)/(L[a].x-e.x)}}async openColors(){this.lastColor=void 0,document.querySelector(".flex.gap-2.px-3 > .btn-circle")?.click(),await J(1),document.querySelector(".btn.btn-primary.btn-lg.relative.z-30")?.click(),await J(1);let o=document.querySelector("button.bottom-0");if(o?.innerHTML==='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor" class="size-5"><path d="M480-120 300-300l58-58 122 122 122-122 58 58-180 180ZM358-598l-58-58 180-180 180 180-58 58-122-122-122 122Z"></path></svg><!---->')o.click(),await J(1)}drawTask(o){if(this.lastColor!==o.color){let i=document.getElementById("color-"+o.color);if(!i){this.log("Skipped draw task: color button not found",{color:o.color,tileX:o.position.tileX,tileY:o.position.tileY,x:o.position.x,y:o.position.y});return}i.click(),this.lastColor=o.color,this.log("Color switched for draw task",{color:o.color})}let s=o.position.pixelSize/2,a=o.position.toScreenPosition();if(!Number.isFinite(a.x)||!Number.isFinite(a.y)){this.log("Skipped draw task: invalid screen position",{color:o.color});return}document.documentElement.dispatchEvent(new MouseEvent("mousemove",{bubbles:!0,clientX:a.x+s,clientY:a.y+s,shiftKey:!0})),document.documentElement.dispatchEvent(new KeyboardEvent("keydown",{key:" ",code:"Space",keyCode:32,which:32,bubbles:!0,cancelable:!0})),document.documentElement.dispatchEvent(new KeyboardEvent("keyup",{key:" ",code:"Space",keyCode:32,which:32,bubbles:!0,cancelable:!0})),o.position.setMapColor(o.color)}async paintRandomPixelInViewport(){if(!this.ensureFeatureAccess("autoFarm"))return;try{await this.updateColors();let o=Array.from(document.querySelectorAll('button[id^="color-"]')).filter((d)=>!d.disabled&&d.getAttribute("aria-disabled")!=="true"&&d.offsetParent!==null);if(!o.length)return;let s=o[Math.floor(Math.random()*o.length)],a=Number.parseInt(s.id.slice(6),10);if(!Number.isFinite(a))return;let i=document.querySelector(".maplibregl-canvas");if(!i)return;let r=i.getBoundingClientRect(),l=24,e=r.left+l,t=r.right-l,n=r.top+l,c=r.bottom-l;if(t<=e||c<=n)return;let g=e+Math.random()*(t-e),f=n+Math.random()*(c-n);this.drawTask({color:a,position:H.fromScreenPosition(this,{x:g,y:f})})}catch(o){this.log("Auto farm tick failed",o)}}async drawRandomPixelsBatch(o,s){if(!this.ensureFeatureAccess("autoFarm"))return 0;let a=Math.max(1,Math.floor(o)),i=0;return await this.widget.run(p("taskDrawingRandomPixels"),async()=>{await this.widget.run(p("taskInitializingDraw"),()=>this.updateColors());let r=Array.from(document.querySelectorAll('button[id^="color-"]')).filter((w)=>!w.disabled&&w.getAttribute("aria-disabled")!=="true"&&w.offsetParent!==null),l=document.querySelector(".maplibregl-canvas");if(!r.length||!l)return;let e=s===void 0?void 0:r.find((w)=>Number.parseInt(w.id.slice(6),10)===s);if(s!==void 0&&!e)return;let t=l.getBoundingClientRect(),n=24,c=t.left+n,g=t.right-n,f=t.top+n,d=t.bottom-n;if(g<=c||d<=f)return;for(let w=0;w<a;w++){let m=e??r[Math.floor(Math.random()*r.length)],h=Number.parseInt(m.id.slice(6),10);if(!Number.isFinite(h))continue;let u=c+Math.random()*(g-c),b=f+Math.random()*(d-f);this.drawTask({color:h,position:H.fromScreenPosition(this,{x:u,y:b})}),i++,await J(1)}}),i}async drawOverlayPixelsBatch(o){if(!this.ensureFeatureAccess("autoDraw"))return 0;let s=Math.max(1,Math.floor(o)),a=0;return await this.widget.run(p("taskDrawingOverlayPixels"),async()=>{await this.widget.run(p("taskInitializingDraw"),()=>Promise.all([this.updateColors(),this.readMap()])),this.updateTasks();for(let i=0;i<s;i++){let r=this.takeNextTaskFromStrategy();if(!r)break;this.drawTask(r),a++,await J(1)}this.widget.update()}),a}takeNextTaskFromStrategy(){switch(this.strategy){case"ALL":case"SEQUENTIAL":{for(let o=0;o<this.images.length;o++){let s=this.images[o].tasks.shift();if(s)return s}return}case"PERCENTAGE":{let o,s=Number.POSITIVE_INFINITY;for(let a=0;a<this.images.length;a++){let i=this.images[a];if(!i.tasks.length)continue;let r=i.pixels.pixels.length*i.pixels.pixels[0].length,l=1-i.tasks.length/r;if(l<s)s=l,o=i}return o?.tasks.shift()}}}registerFetchInterceptor(){let o=this.getPageWindow(),s=o.fetch.bind(o),a=/https:\/\/backend.wplace.live\/s\d+\/pixel\/(-?\d+)\/(-?\d+)\?x=(-?\d+)&y=(-?\d+)/,i=async(r,l)=>{let e=await s(r,l),t=e.clone(),n=this.resolveFetchUrl(r);if(e.url==="https://backend.wplace.live/me")this.me=await t.json(),this.me.favoriteLocations.unshift(...O),this.me.maxFavoriteLocations=1/0,e.json=()=>Promise.resolve(this.me),this.log("Patched /me response with favorite locations",{totalFavorites:this.me.favoriteLocations.length}),this.syncAccountInfoWithControl("wplace_me").catch((g)=>{this.log("Control API /me sync failed",g)});let c=a.exec(n);if(c){for(let g=0;g<this.markerPixelPositionResolvers.length;g++)this.markerPixelPositionResolvers[g](new H(this,+c[1],+c[2],+c[3],+c[4]));this.markerPixelPositionResolvers.length=0,this.log("Resolved marker pixel position from network event")}return e};o.fetch=i,globalThis.fetch=i}resolveFetchUrl(o){if(typeof o==="string")return o;if(o instanceof URL)return o.href;if(o&&typeof o==="object"&&"url"in o){let s=o.url;if(typeof s==="string")return s}return""}async closeAll(){for(let o of document.querySelectorAll("button"))if(o.innerHTML==="✕"||o.innerHTML==='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor" class="size-4"><path d="m256-200-56-56 224-224-224-224 56-56 224 224 224-224 56 56-224 224 224 224-56 56-224-224-224 224Z"></path></svg><!---->')o.click(),await J(1)}waitForElement(o,s){return this.log("Waiting for element",{name:o,selector:s}),this.widget.run(`${p("taskWaitingFor")} ${o}`,()=>{return new Promise((a)=>{let i=document.querySelector(s);if(i){a(i);return}let r=new MutationObserver(()=>{let l=document.querySelector(s);if(l)r.disconnect(),a(l)});r.observe(document.documentElement,{childList:!0,subtree:!0})})})}updateStars(){this.$stars=[...document.querySelectorAll(".text-yellow-400.cursor-pointer.z-10.maplibregl-marker.maplibregl-marker-anchor-center")].slice(0,O.length),this.log("Star cache updated",{stars:this.$stars.length})}updateImages(){for(let o=0;o<this.images.length;o++)this.images[o].position.updateAnchor(),this.images[o].update()}updateTasks(){for(let o=0;o<this.images.length;o++)this.images[o].updateTasks()}updateImageColors(){for(let o=0;o<this.images.length;o++)this.images[o].updateColors()}}Za();if(location.hostname.includes("hcaptcha.com"))Fo();else globalThis.kglacerMacro=new la,globalThis.kgm=globalThis.kglacerMacro,globalThis.wbot=globalThis.kglacerMacro;
