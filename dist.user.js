...
<span class="premium ${g?"on":""}">${g?b("premium"):""}</span>`,d.querySelector(".swatch").style.setProperty("--swatch-color",go(a.realColor)),d.addEventListener("click",()=>{if(c){c=!1;return}u()}),d.addEventListener("dragstart",(n)=>{c=!0,d.classList.add("dragging"),n.dataTransfer?.setData("text/plain",String(l)),n.dataTransfer.effectAllowed="move"}),d.addEventListener("dragend",()=>{d.classList.remove("dragging")}),d.addEventListener("dragover",(n)=>{n.preventDefault(),d.classList.add("drag-target")}),d.addEventListener("dragleave",()=>{d.classList.remove("drag-target")}),d.addEventListener("drop",(n)=>{n.preventDefault(),d.classList.remove("drag-target");let m=Number.parseInt(n.dataTransfer?.getData("text/plain")??"-1",10);if(m<0||m===l||m>=this.colors.length)return;this.colors.splice(l,0,...this.colors.splice(m,1)),P(this.bot),this.updateColors()}),g){let n=document.createElement("button");n.textContent=b("buy"),n.className="buy-chip",n.addEventListener("click",(m)=>{m.stopPropagation(),document.getElementById("color-"+a.realColor)?.click()}),d.append(n)}let i=`${f} ${w.join(" ")} ${a.realColor} ${j[a.realColor]}`;if(!p||i.toLowerCase().includes(p))this.$colorsDialogList.append(d)}}*strategyPositionIterator(){let o=this.pixels.pixels[0].length,p=this.pixels.pixels.length;switch(this.strategy){case"DOWN":{for(let l=0;l<p;l++)for(let s=0;s<o;s++)yield{x:s,y:l};break}case"UP":{for(let l=p-1;l>=0;l--)for(let s=0;s<o;s++)yield{x:s,y:l};break}case"LEFT":{for(let l=0;l<o;l++)for(let s=0;s<p;s++)yield{x:l,y:s};break}case"RIGHT":{for(let l=o-1;l>=0;l--)for(let s=0;s<p;s++)yield{x:l,y:s};break}case"RANDOM":{let l=[];for(let s=0;s<p;s++)for(let a=0;a<o;a++)l.push({x:a,y:s});for(let s=l.length-1;s>=0;s--){let a=Math.floor(Math.random()*(s+1)),c=l[s];l[s]=l[a],l[a]=c}yield*l;break}case"ZIGZAG":{for(let l=0;l<p;l++)if(l%2===0)for(let s=0;s<o;s++)yield{x:s,y:l};else for(let s=o-1;s>=0;s--)yield{x:s,y:l};break}case"HUMANIZED":{let l=Math.max(4,Math.floor(Math.min(o,p)/10)),s=[];for(let a=0;a<p;a+=l)for(let c=0;c<o;c+=l)s.push({x:c,y:a});for(let a=s.length-1;a>=0;a--){let c=Math.floor(Math.random()*(a+1)),g=s[a];s[a]=s[c],s[c]=g}for(let a=0;a<s.length;a++){let c=s[a],g=Math.min(p,c.y+l),r=Math.min(o,c.x+l);for(let f=c.y;f<g;f++)if(Math.random()>0.35)for(let u=c.x;u<r;u++)yield{x:u,y:f};else for(let u=r-1;u>=c.x;u--)yield{x:u,y:f}}break}case"HUMAN_SOFT_DITHER":{let l=new Set;for(let s=0;s<p;s++){let a=Math.floor(Math.random()*3)-1;if((s+a)%2===0)for(let g=0;g<o;g+=2)l.add(`${g},${s}`),yield{x:g,y:s};else for(let g=1;g<o;g+=2)l.add(`${g},${s}`),yield{x:g,y:s}}for(let s=0;s<p;s++)for(let a=0;a<o;a++){let c=`${a},${s}`;if(l.has(c))continue;yield{x:a,y:s}}break}case"HUMAN_PATCHY":{let l=new Set,s=o*p;while(l.size<s){let a=Math.floor(Math.random()*o),c=Math.floor(Math.random()*p),g=1+Math.floor(Math.random()*5);for(let r=c-g;r<=c+g;r++)for(let f=a-g;f<=a+g;f++){if(f<0||f>=o||r<0||r>=p)continue;if(Math.hypot(f-a,r-c)>g+Math.random()*1.2)continue;let w=`${f},${r}`;if(l.has(w))continue;l.add(w),yield{x:f,y:r}}}break}case"HUMAN_SWEEP_ARCS":{let l=new Set,s=(o-1)/2,a=(p-1)/2,c=Math.hypot(s,a);for(let g=0;g<4;g++){let r=Math.random()*Math.PI*2;for(let f=0;f<=c;f+=0.35){let w=Math.PI/2+Math.random()*(Math.PI/1.5),u=Math.max(10,Math.floor(f*8));for(let d=0;d<u;d++){let i=r+w*d/u+Math.sin(f)*0.08,n=Math.round(s+Math.cos(i)*f),m=Math.round(a+Math.sin(i)*f);if(n<0||n>=o||m<0||m>=p)continue;let H=`${n},${m}`;if(l.has(H))continue;l.add(H),yield{x:n,y:m}}}}for(let g=0;g<p;g++)for(let r=0;r<o;r++){let f=`${r},${g}`;if(l.has(f))continue;yield{x:r,y:g}}break}case"HUMAN_MICRO_CORRECTIONS":{let l=new Set;for(let s=0;s<p;s++){let a=s%2===0?1:-1,c=a>0?0:o-1;for(let g=0;g<o;g++){let r=c+(Math.random()>0.82?a:0),f=s+(Math.random()>0.9?1:0);for(let w of[{x:c,y:s},{x:r,y:s},{x:c,y:f}]){if(w.x<0||w.x>=o||w.y<0||w.y>=p)continue;let u=`${w.x},${w.y}`;if(l.has(u))continue;l.add(u),yield w}c+=a}}for(let s=0;s<p;s++)for(let a=0;a<o;a++){let c=`${a},${s}`;if(l.has(c))continue;yield{x:a,y:s}}break}case"HUMAN_JITTER_FILL":{let l=[];for(let s=0;s<p;s++)for(let a=0;a<o;a++)l.push({x:a,y:s});l.sort((s,a)=>{let c=s.y+(Math.random()-0.5)*1.8,g=a.y+(Math.random()-0.5)*1.8;if(c!==g)return c-g;return s.x+(Math.random()-0.5)*2-(a.x+(Math.random()-0.5)*2)}),yield*l;break}case"HUMAN_CORNER_BIAS":{let l=[{x:0,y:0},{x:o-1,y:0},{x:0,y:p-1},{x:o-1,y:p-1}],s=l[Math.floor(Math.random()*l.length)],a=[];for(let c=0;c<p;c++)for(let g=0;g<o;g++){let f=Math.hypot(g-s.x,c-s.y)+Math.random()*3.5;a.push({point:{x:g,y:c},score:f})}a.sort((c,g)=>c.score-g.score);for(let c of a)yield c.point;break}case"HUMAN_LONG_STROKES":{let l=new Set,s=o*p;while(l.size<s){let a=Math.floor(Math.random()*o),c=Math.floor(Math.random()*p),g=Math.random()*Math.PI*2,r=Math.sign(Math.cos(g)),f=Math.sign(Math.sin(g)),w=10+Math.floor(Math.random()*40);for(let u=0;u<w;u++){if(a<0||a>=o||c<0||c>=p)break;let d=`${a},${c}`;if(!l.has(d))l.add(d),yield{x:a,y:c};if(Math.random()>0.78)a+=f,c+=r;else a+=r,c+=f}}break}case"HUMAN_TAP_CLUSTERS":{let l=new Set,s=o*p;while(l.size<s){let a=Math.floor(Math.random()*o),c=Math.floor(Math.random()*p),g=3+Math.floor(Math.random()*10);for(let r=0;r<g;r++){let f=Math.round(a+(Math.random()-0.5)*6),w=Math.round(c+(Math.random()-0.5)*6);if(f<0||f>=o||w<0||w>=p)continue;let u=`${f},${w}`;if(l.has(u))continue;l.add(u),yield{x:f,y:w}}}break}case"HUMAN_MESSY_SPIRAL":{let l=new Set,s=(o-1)/2,a=(p-1)/2,c=Math.hypot(s,a)+2;for(let g=0;l.size<o*p;g++){let r=g/3,f=Math.min(c,r*0.18),w=r*0.29+Math.sin(r*0.13)*0.8,u=Math.round(s+Math.cos(w)*f+Math.sin(r)*0.7),d=Math.round(a+Math.sin(w)*f+Math.cos(r)*0.7);if(u<0||u>=o||d<0||d>=p){if(g>o*p*18)break;continue}let i=`${u},${d}`;if(l.has(i)){if(Math.random()>0.9)continue}else l.add(i),yield{x:u,y:d};if(g>o*p*18)break}for(let g=0;g<p;g++)for(let r=0;r<o;r++){let f=`${r},${g}`;if(l.has(f))continue;yield{x:r,y:g}}break}case"HUMAN_DRUNK_WALK":{let l=new Set,s=Math.floor(Math.random()*o),a=Math.floor(Math.random()*p),c=[{x:-1,y:0},{x:1,y:0},{x:0,y:-1},{x:0,y:1},{x:-1,y:-1},{x:1,y:-1},{x:-1,y:1},{x:1,y:1}];while(l.size<o*p){let g=`${s},${a}`;if(!l.has(g))l.add(g),yield{x:s,y:a};let r=[];for(let u of c){let d=s+u.x,i=a+u.y;if(d<0||d>=o||i<0||i>=p)continue;r.push({x:d,y:i})}if(!r.length)break;let f=r.filter((u)=>{return!l.has(`${u.x},${u.y}`)});if(f.length&&Math.random()>0.2){let u=f[Math.floor(Math.random()*f.length)];s=u.x,a=u.y;continue}let w=r[Math.floor(Math.random()*r.length)];s=w.x,a=w.y}for(let g=0;g<p;g++)for(let r=0;r<o;r++){let f=`${r},${g}`;if(l.has(f))continue;yield{x:r,y:g}}break}case"HUMAN_NOISE_CLOUD":{let l=[];for(let s=0;s<p;s++)for(let a=0;a<o;a++){let c=Math.sin((a+1)*0.93+Math.random()*0.8)+Math.cos((s+1)*1.17+Math.random()*0.8),g=(Math.random()-0.5)*2.6,r=Math.hypot(a-o/2,s-p/2)*0.08;l.push({point:{x:a,y:s},score:c+g+r})}l.sort((s,a)=>s.score-a.score);for(let s of l)yield s.point;break}case"HUMAN_PATCH_JUMP":{let l=new Set,s=[];for(let a=0;a<Math.max(6,o*p/18);a++)s.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*p)});while(l.size<o*p){let a=s[Math.floor(Math.random()*s.length)],c=1+Math.floor(Math.random()*3),g=1+Math.floor(Math.random()*3);for(let r=a.y-g;r<=a.y+g;r++)for(let f=a.x-c;f<=a.x+c;f++){if(f<0||f>=o||r<0||r>=p)continue;if(Math.random()>0.86)continue;let w=`${f},${r}`;if(l.has(w))continue;l.add(w),yield{x:f,y:r}}if(Math.random()>0.72&&s.length<o*p/2)s.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*p)});if(l.size>o*p*0.92)break}for(let a=0;a<p;a++)for(let c=0;c<o;c++){let g=`${c},${a}`;if(l.has(g))continue;yield{x:c,y:a}}break}case"HUMAN_HESITANT_LINES":{let l=new Set;for(let s=0;s<p;s++){let a=s%2===0;for(let c=0;c<o;c++){let g=a?c:o-1-c,r=`${g},${s}`;if(!l.has(r))l.add(r),yield{x:g,y:s};if(Math.random()>0.7){let f=Math.max(0,Math.min(o-1,g+(Math.random()>0.5?1:-1))),w=Math.max(0,Math.min(p-1,s+(Math.random()>0.65?1:0))),u=`${f},${w}`;if(!l.has(u))l.add(u),yield{x:f,y:w}}}}for(let s=0;s<p;s++)for(let a=0;a<o;a++){let c=`${a},${s}`;if(l.has(c))continue;yield{x:a,y:s}}break}case"HUMAN_OVERLAP_SWEEPS":{let l=[],s=Math.random()*Math.PI*2;for(let a=0;a<p;a++)for(let c=0;c<o;c++){let g=Math.sin((c+a)*0.42+s)*2.2,r=Math.cos((c-a)*0.3+s)*1.4;l.push({point:{x:c,y:a},score:a+g+r+(Math.random()-0.5)*3.4})}l.sort((a,c)=>a.score-c.score);for(let a of l)yield a.point;break}case"HUMAN_WOBBLE_DRIFT":{let l=[],s=o/2,a=p/2;for(let c=0;c<p;c++)for(let g=0;g<o;g++){let r=Math.hypot(g-s,c-a)*0.25,f=Math.sin((g+1)*0.9)*1.8+Math.cos((c+1)*1.1)*1.8+Math.sin((g+c)*0.35)*1.4;l.push({point:{x:g,y:c},score:r+f+(Math.random()-0.5)*2.8})}l.sort((c,g)=>c.score-g.score);for(let c of l)yield c.point;break}case"HUMAN_GAP_RECOVERY":{let l=new Set,s=[];for(let a=0;a<p;a++)for(let c=0;c<o;c++){if(Math.random()>0.87){s.push({x:c,y:a});continue}l.add(`${c},${a}`),yield{x:c,y:a}}s.sort((a,c)=>Math.hypot(a.x-o/2,a.y-p/2)-Math.hypot(c.x-o/2,c.y-p/2));for(let a of s){let c=`${a.x},${a.y}`;if(l.has(c))continue;l.add(c),yield a}break}case"HUMAN_STAIRCASE":{let l=new Set,s=o+p-1;for(let a=0;a<s;a++){let c=Math.max(0,a-o+1),g=Math.min(p-1,a);for(let r=c;r<=g;r++){let f=a-r,w=[{x:f,y:r},{x:f+(Math.random()>0.5?1:-1),y:r},{x:f,y:r+(Math.random()>0.5?1:-1)}];for(let u of w){if(u.x<0||u.x>=o||u.y<0||u.y>=p)continue;let d=`${u.x},${u.y}`;if(l.has(d))continue;l.add(d),yield u}}}for(let a=0;a<p;a++)for(let c=0;c<o;c++){let g=`${c},${a}`;if(l.has(g))continue;yield{x:c,y:a}}break}case"HUMAN_EDGE_HUGGER":{let l=[];for(let s=0;s<p;s++)for(let a=0;a<o;a++){let c=Math.min(a,s,o-1-a,p-1-s);l.push({point:{x:a,y:s},score:c*3.5+(Math.random()-0.5)*5.5})}l.sort((s,a)=>s.score-a.score);for(let s of l)yield s.point;break}case"HUMAN_BLOBS":{let l=new Set,s=o*p;while(l.size<s){let a=Math.floor(Math.random()*o),c=Math.floor(Math.random()*p),g=1+Math.floor(Math.random()*4);for(let r=c-g;r<=c+g;r++)for(let f=a-g;f<=a+g;f++){if(f<0||f>=o||r<0||r>=p)continue;let w=Math.atan2(r-c,f-a),u=g+Math.sin(w*3+Math.random())*0.8;if(Math.hypot(f-a,r-c)>u)continue;let d=`${f},${r}`;if(l.has(d))continue;l.add(d),yield{x:f,y:r}}}break}case"HUMAN_BACKTRACK":{let l=new Set,s=[];for(let a=0;a<p;a++)for(let c=0;c<o;c++)s.push({x:c,y:a});s.sort((a,c)=>a.y-c.y+(Math.random()-0.5)*2.2+(a.x-c.x)*0.04);for(let a=0;a<s.length;a++){let c=s[a],g=`${c.x},${c.y}`;if(l.has(g))continue;if(l.add(g),yield c,a>1&&Math.random()>0.74){let r=s[a-1],f=`${r.x},${r.y}`;if(!l.has(f))l.add(f),yield r}}for(let a=0;a<p;a++)for(let c=0;c<o;c++){let g=`${c},${a}`;if(l.has(g))continue;yield{x:c,y:a}}break}case"HUMAN_SHAKY_DIAGONAL":{let l=[];for(let s=0;s<p;s++)for(let a=0;a<o;a++){let c=Math.abs(a-s)*0.6,g=Math.sin(a*1.4+s*0.8)*1.8+Math.cos(s*1.1+a*0.5)*1.5;l.push({point:{x:a,y:s},score:c+g+(Math.random()-0.5)*3.2})}l.sort((s,a)=>s.score-a.score);for(let s of l)yield s.point;break}case"HUMAN_LATE_FIXES":{let l=[],s=[];for(let a=0;a<p;a++)for(let c=0;c<o;c++)if(Math.random()>0.9)s.push({x:c,y:a});else l.push({x:c,y:a});l.sort((a,c)=>a.y-c.y+(Math.random()-0.5)*1.5+(Math.random()>0.85?a.x-c.x:0)),s.sort((a,c)=>Math.hypot(c.x-o/2,c.y-p/2)-Math.hypot(a.x-o/2,a.y-p/2)),yield*l,yield*s;break}case"DIAGONAL_BRUSH":{for(let l=0;l<o+p-1;l++){let s=l%2===0,a=[],c=Math.max(0,l-o+1),g=Math.min(p-1,l);for(let r=c;r<=g;r++){let f=l-r;if(f>=0&&f<o)a.push({x:f,y:r})}if(Math.random()>0.55)a.reverse();if(s)for(let r=a.length-1;r>=0;r--)yield a[r];else yield*a}break}case"BRUSH_STROKES":{let l=Array.from({length:p},()=>Array(o).fill(!1)),s=[{x:1,y:0},{x:-1,y:0},{x:0,y:1},{x:0,y:-1},{x:1,y:1},{x:1,y:-1},{x:-1,y:1},{x:-1,y:-1}],a=(r,f)=>r>=0&&r<o&&f>=0&&f<p,c=0,g=o*p;for(let r=0;r<g*6&&c<g;r++){let f=Math.floor(Math.random()*o),w=Math.floor(Math.random()*p),u=s[Math.floor(Math.random()*s.length)],d=3+Math.floor(Math.random()*16);for(let i=0;i<d;i++){if(!a(f,w))break;if(!l[w][f])l[w][f]=!0,c++,yield{x:f,y:w};if(Math.random()>0.72)u=s[Math.floor(Math.random()*s.length)];f+=u.x,w+=u.y}}for(let r=0;r<p;r++)for(let f=0;f<o;f++)if(!l[r][f])yield{x:f,y:r};break}case"SPIRAL_FROM_CENTER":case"SPIRAL_TO_CENTER":{let l=new Set,s=o*p,a=Math.floor(o/2),c=Math.floor(p/2),g=[[1,0],[0,1],[-1,0],[0,-1]],r=0,f=1,w=(d,i)=>d>=0&&d<o&&i>=0&&i<p,u=function*(){let d=0;while(d<s){for(let i=0;i<2;i++){for(let n=0;n<f;n++){if(w(a,c)){let m=`${a},${c}`;if(!l.has(m)){if(l.add(m),yield{x:a,y:c},d++,d>=s)return}}a+=g[r][0],c+=g[r][1]}r=(r+1)%4}f++}};if(this.strategy==="SPIRAL_FROM_CENTER")yield*u();else{let d=[...u()];for(let i=d.length-1;i>=0;i--)yield d[i]}break}case"SCRIBBLE":{let l=new Set,s=o*p,a=Math.floor(o/2),c=Math.floor(p/2);for(let g=0;l.size<s&&g<s*24;g++){let r=`${a},${c}`;if(!l.has(r))l.add(r),yield{x:a,y:c};if(a+=Math.floor(Math.random()*3)-1,c+=Math.floor(Math.random()*3)-1,a<0||a>=o||c<0||c>=p)a=Math.floor(Math.random()*o),c=Math.floor(Math.random()*p)}for(let g=0;g<p;g++)for(let r=0;r<o;r++){let f=`${r},${g}`;if(l.has(f))continue;l.add(f),yield{x:r,y:g}}break}case"CROSSHATCH":{let l=[];for(let c=0;c<o+p-1;c++)for(let g=Math.max(0,c-o+1);g<=Math.min(p-1,c);g++){let r=c-g;l.push({x:r,y:g})}let s=[];for(let c=-p+1;c<o;c++)for(let g=0;g<p;g++){let r=g+c;if(r>=0&&r<o)s.push({x:r,y:g})}let a=new Set;for(let c of[...l,...s]){let g=`${c.x},${c.y}`;if(a.has(g))continue;a.add(g),yield c}break}case"WAVE_SWEEP":{let l=new Set;for(let s=0;s<o;s++){let c=(Math.sin(s/Math.max(1,o-1)*Math.PI*4)+1)*0.5*(p-1)|0;for(let g=0;g<p;g++){let r=c+g,f=c-g;for(let w of[r,f]){if(w<0||w>=p)continue;let u=`${s},${w}`;if(l.has(u))continue;l.add(u),yield{x:s,y:w}}}}break}case"SCATTERED_LINES":{let l=new Set,s=o*p;for(let a=0;l.size<s&&a<s*14;a++){let c=Math.floor(Math.random()*o),g=Math.floor(Math.random()*p),r=Math.random()*Math.PI*2,f=Math.round(Math.cos(r)),w=Math.round(Math.sin(r)),u=6+Math.floor(Math.random()*28);for(let d=0;d<u;d++){if(c<0||c>=o||g<0||g>=p)break;let i=`${c},${g}`;if(!l.has(i))l.add(i),yield{x:c,y:g};c+=f,g+=w}}for(let a=0;a<p;a++)for(let c=0;c<o;c++){let g=`${c},${a}`;if(l.has(g))continue;l.add(g),yield{x:c,y:a}}break}case"CONTOUR_JITTER":{let l=new Set;for(let s=0;s<Math.ceil(Math.min(o,p)/2);s++){let a=[],c=s,g=s,r=o-s-1,f=p-s-1;for(let w=c;w<=r;w++)a.push({x:w,y:g});for(let w=g+1;w<=f;w++)a.push({x:r,y:w});for(let w=r-1;w>=c;w--)a.push({x:w,y:f});for(let w=f-1;w>g;w--)a.push({x:c,y:w});for(let w=a.length-1;w>0;w--){let u=Math.floor(Math.random()*(w+1)),d=a[w];a[w]=a[u],a[u]=d}for(let w of a){let u=`${w.x},${w.y}`;if(l.has(u))continue;l.add(u),yield w}}break}case"SPIRAL_WOBBLE":{let l=new Set,s=o/2,a=p/2,c=Math.hypot(s,a);for(let g=0;l.size<o*p&&g<o*p*9;g++){let r=g/(o*p*9)*c,f=g*0.31+Math.sin(g*0.07)*0.7,w=Math.round(s+Math.cos(f)*r),u=Math.round(a+Math.sin(f)*r);if(w<0||w>=o||u<0||u>=p)continue;let d=`${w},${u}`;if(l.has(d))continue;l.add(d),yield{x:w,y:u}}for(let g=0;g<p;g++)for(let r=0;r<o;r++){let f=`${r},${g}`;if(l.has(f))continue;yield{x:r,y:g}}break}case"CLUSTER_BURSTS":{let l=new Set,s=o*p;for(let a=0;l.size<s&&a<s*12;a++){let c=Math.floor(Math.random()*o),g=Math.floor(Math.random()*p),r=2+Math.floor(Math.random()*10);for(let f=g-r;f<=g+r;f++)for(let w=c-r;w<=c+r;w++){if(w<0||w>=o||f<0||f>=p)continue;if(Math.hypot(w-c,f-g)>r)continue;let u=`${w},${f}`;if(l.has(u))continue;l.add(u),yield{x:w,y:f}}}for(let a=0;a<p;a++)for(let c=0;c<o;c++){let g=`${c},${a}`;if(l.has(g))continue;l.add(g),yield{x:c,y:a}}break}case"ORBITAL":{let l=new Set,s=(o-1)/2,a=(p-1)/2,c=Math.ceil(Math.max(s,a));for(let g=0;g<=c;g++){let r=Math.max(16,Math.ceil(2*Math.PI*Math.max(1,g)*2));for(let f=0;f<r;f++){let w=f/r*Math.PI*2+(g%2?0.3:-0.3),u=Math.round(s+Math.cos(w)*g),d=Math.round(a+Math.sin(w)*g);if(u<0||u>=o||d<0||d>=p)continue;let i=`${u},${d}`;if(l.has(i))continue;l.add(i),yield{x:u,y:d}}}for(let g=0;g<p;g++)for(let r=0;r<o;r++){let f=`${r},${g}`;if(l.has(f))continue;yield{x:r,y:g}}break}case"FLOW_FIELD":{let l=new Set,s=o*p;for(let a=0;l.size<s&&a<s*18;a++){let c=Math.floor(Math.random()*o),g=Math.floor(Math.random()*p);for(let r=0;r<120;r++){if(c<0||c>=o||g<0||g>=p)break;let f=`${c},${g}`;if(!l.has(f))l.add(f),yield{x:c,y:g};let w=Math.sin(c*0.09)*1.8+Math.cos(g*0.08)*1.6+Math.sin((c+g)*0.05);c+=Math.round(Math.cos(w)),g+=Math.round(Math.sin(w))}}for(let a=0;a<p;a++)for(let c=0;c<o;c++){let g=`${c},${a}`;if(l.has(g))continue;l.add(g),yield{x:c,y:a}}break}case"EDGE_IN":{let l=new Set,s=Math.ceil(Math.min(o,p)/2);for(let a=0;a<s;a++){let c=a,g=o-1-a,r=a,f=p-1-a;for(let w=c;w<=g;w++)for(let u of[r,f]){let d=`${w},${u}`;if(l.has(d))continue;l.add(d),yield{x:w,y:u}}for(let w=r+1;w<=f-1;w++)for(let u of[c,g]){let d=`${u},${w}`;if(l.has(d))continue;l.add(d),yield{x:u,y:w}}}break}}}moveStart(o){if(o.button!==0)return;if(o.preventDefault(),o.stopPropagation(),!this.lock)this.moveInfo={globalX:this.position.globalX,globalY:this.position.globalY,clientX:o.clientX,clientY:o.clientY}}moveStop(){if(this.moveInfo)this.moveInfo=void 0,this.position.updateAnchor(),this.pixels.update(),this.updateColors(),P(this.bot)}move(o){if(!this.moveInfo)return;let p=Math.round((o.clientX-this.moveInfo.clientX)/this.position.pixelSize),l=Math.round((o.clientY-this.moveInfo.clientY)/this.position.pixelSize);if(this.moveInfo.globalX!==void 0){if(this.position.globalX=p+this.moveInfo.globalX,this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,this.moveInfo.width-p)}else if(this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,p+this.moveInfo.width);if(this.moveInfo.globalY!==void 0){if(this.position.globalY=l+this.moveInfo.globalY,this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,this.moveInfo.height-l)}else if(this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,l+this.moveInfo.height);this.update(),P(this.bot)}resizeStart(o){if(this.lock||o.button!==0)return;o.preventDefault(),o.stopPropagation(),this.moveInfo={clientX:o.clientX,clientY:o.clientY};let p=o.target;if(p.classList.contains("n"))this.moveInfo.height=this.pixels.height,this.moveInfo.globalY=this.position.globalY;if(p.classList.contains("e"))this.moveInfo.width=this.pixels.width;if(p.classList.contains("s"))this.moveInfo.height=this.pixels.height;if(p.classList.contains("w"))this.moveInfo.width=this.pixels.width,this.moveInfo.globalX=this.position.globalX}export(){let o=document.createElement("a");document.body.append(o),o.href=URL.createObjectURL(new Blob([JSON.stringify(this.toJSON())],{type:"application/json"})),o.download=`${this.pixels.width}x${this.pixels.height}.${O}`,o.click(),URL.revokeObjectURL(o.href),o.href=this.pixels.canvas.toDataURL("image/webp",1),o.download=`${this.pixels.width}x${this.pixels.height}.webp`,o.click(),URL.revokeObjectURL(o.href),o.remove()}}var ko=`/* stylelint-disable declaration-no-important */
...
`;var No="kglacer-macro:overlay-hidden",Yo="https://raw.githubusercontent.com/robgallardof/kglacer-macro/refs/heads/main/src/img/logo.svg";class fo extends X{bot;element=document.createElement("div");get status(){return this.$status.innerHTML}set status(o){this.$status.innerHTML=o}get open(){return this.element.classList.contains("wopen")}set open(o){if(o)this.element.classList.add("wopen");else this.element.classList.remove("wopen")}$settings;$status;$shortcuts;$locale;$topbar;$draw;$drawAndPaint;$addImage;$captureTemplate;$toggleOverlay;$strategy;$progressLine;$progressText;$images;$wopenButton;$widgetLogo;activeImageIndex=-1;constructor(o){super();this.bot=o;this.element.classList.add("wwidget"),this.element.innerHTML=Jo,W(this.element),document.body.append(this.element),this.populateElementsWithSelector(this.element,{$wopenButton:".wopen-button",$widgetLogo:".widget-logo",$settings:".wform",$status:".wstatus",$shortcuts:".shortcuts",$locale:".locale",$topbar:".wtopbar",$draw:".draw",$drawAndPaint:".draw-and-paint",$addImage:".add-image",$captureTemplate:".capture-template",$toggleOverlay:".toggle-overlay",$strategy:".strategy",$progressLine:".wprogress div",$progressText:".wprogress span",$images:".images"}),this.$widgetLogo.src=Yo,this.$wopenButton.addEventListener("click",()=>this.open=!this.open),this.$draw.addEventListener("click",()=>this.bot.draw()),this.$drawAndPaint.addEventListener("click",()=>{this.drawAndClickPaintWhenReady()}),this.$addImage.addEventListener("click",()=>this.addImage()),this.$captureTemplate.addEventListener("click",()=>{this.captureTemplate()}),this.$toggleOverlay.addEventListener("click",()=>{this.toggleOverlay()}),this.$strategy.addEventListener("change",()=>{this.bot.strategy=this.$strategy.value}),this.$locale.value=_(),this.$locale.addEventListener("change",()=>{oo(this.$locale.value),W(this.element);for(let p=0;p<this.bot.images.length;p++)this.bot.images[p].applyLocale();this.refreshOverlayToggleText()}),this.registerEvent(document,"keydown",this.handleKeyboard.bind(this),{passive:!1}),this.update(),this.syncOverlayVisibilityFromStorage(),this.open=!0,console.log("[KGM][Widget] Widget mounted and opened")}addImage(){return console.log("[KGM][Widget] Add image flow started"),this.setDisabled("add-image",!0),this.run(b("taskAddingImage"),async()=>{await this.bot.updateColors();let o=document.createElement("input");o.type="file",o.accept=`image/*,.${O}`,o.click(),await F(o,["change"],["cancel","error"]);let p=o.files?.[0];if(!p)throw new ro(this.bot);console.log("[KGM][Widget] File selected",{name:p.name,size:p.size,type:p.type});let l;if(p.name.endsWith(`.${O}`))l=await V.fromJSON(this.bot,JSON.parse(await p.text()));else{let s=new FileReader;s.readAsDataURL(p),await F(s,["load"],["error"]);let a=await this.compressImageBeforeLoad(s.result),c=new Image;c.src=a,await F(c,["load"],["error"]),await this.waitForStableViewportProjection(),l=new V(this.bot,N.fromScreenPosition(this.bot,this.defaultImageScreenPosition()),new B(this.bot,c))}this.bot.images.push(l),console.log("[KGM][Widget] Image instance added",{images:this.bot.images.length}),await this.bot.readMap(),l.updateTasks(),P(this.bot,!0),this.bot.updateTasks(),this.update()},()=>{this.setDisabled("add-image",!1)})}captureTemplate(){return this.setDisabled("capture-template",!0),this.run(b("taskCapturingMapImage"),async()=>{let o=await this.resolveCaptureBounds(),{minGlobalX:p,minGlobalY:l,maxGlobalX:s,maxGlobalY:a}=o,c=document.createElement("canvas");c.width=Math.max(1,s-p+1),c.height=Math.max(1,a-l+1);let g=c.getContext("2d");if(!g)throw new Error("Capture context unavailable");g.imageSmoothingEnabled=!1;let r=Math.floor(p/h),f=Math.floor(l/h),w=Math.floor(s/h),u=Math.floor(a/h),d=(w-r+1)*(u-f+1),i=0;for(let m=r;m<=w;m++)for(let H=f;H<=u;H++){this.status=`⌛ ${b("taskReadingTiles")} [${++i}/${d}]`;let M=await this.loadTileImage(m,H),A=m*h,z=H*h,k=Math.max(p,A),J=Math.min(s,A+h-1),K=Math.max(l,z),Z=Math.min(a,z+h-1),E=k-A,I=K-z,T=J-k+1,Q=Z-K+1,v=k-p,e=K-l;g.drawImage(M,E,I,T,Q,v,e,T,Q)}let n=Date.now();await this.downloadCapture(c,"png",n)},()=>{this.setDisabled("capture-template",!1)})}async downloadCapture(o,p,l){let s=p==="webp"?"image/webp":"image/png",a=await new Promise((r,f)=>{o.toBlob((w)=>{if(!w){f(new Error(`Failed to create ${p.toUpperCase()} capture file`));return}r(w)},s)}),c=URL.createObjectURL(a),g=document.createElement("a");g.href=c,g.download=`wplace-capture-${l}.${p}`,g.click(),URL.revokeObjectURL(c)}async loadTileImage(o,p){let l;for(let s=1;s<=3;s++)try{let a=new Image;return a.crossOrigin="anonymous",a.referrerPolicy="no-referrer",a.src=`https://backend.wplace.live/files/s0/tiles/${o}/${p}.png?ts=${Date.now()}-${s}`,await F(a,["load"],["error"]),a}catch(a){if(l=a,s<3)await new Promise((c)=>setTimeout(c,s*200))}throw l instanceof Error?l:new Error(`Tile fetch failed (${o}/${p})`)}async resolveCaptureBounds(){return this.selectCaptureBounds()}selectCaptureBounds(){return new Promise((o,p)=>{let l=document.createElement("div");l.className="kgm-capture-overlay",l.innerHTML=`<div class="kgm-capture-hint">${b("captureHintSelectArea")}: A → B</div><div class="kgm-capture-box"></div>`;let s=l.querySelector(".kgm-capture-box");document.body.append(l);let a,c,g=()=>{window.removeEventListener("keydown",d,!0),l.removeEventListener("pointermove",w),l.removeEventListener("pointerdown",u),l.remove()},r=(i)=>{let n=Math.min(a.x,i.x),m=Math.min(a.y,i.y),H=Math.abs(a.x-i.x)+1,M=Math.abs(a.y-i.y)+1;return{left:n,top:m,width:H,height:M}},f=(i)=>{let{left:n,top:m,width:H,height:M}=r(i);s.style.left=`${n}px`,s.style.top=`${m}px`,s.style.width=`${H}px`,s.style.height=`${M}px`},w=(i)=>{if(!a)return;f({x:i.clientX,y:i.clientY})},u=(i)=>{if(i.preventDefault(),!a){a={x:i.clientX,y:i.clientY};let k=N.fromScreenPosition(this.bot,a);c={x:k.globalX,y:k.globalY},f(a);return}let n={x:i.clientX,y:i.clientY},m=N.fromScreenPosition(this.bot,n);if(g(),!c){p(new Error("Capture anchor point unavailable"));return}let H=Math.min(c.x,m.globalX),M=Math.min(c.y,m.globalY),A=Math.max(c.x,m.globalX),z=Math.max(c.y,m.globalY);if(A-H<1||z-M<1){p(new Error("Capture area too small"));return}o({minGlobalX:H,minGlobalY:M,maxGlobalX:A,maxGlobalY:z})},d=(i)=>{if(i.key!=="Escape")return;g(),p(new Error("Capture cancelled"))};window.addEventListener("keydown",d,!0),l.addEventListener("pointermove",w),l.addEventListener("pointerdown",u)})}defaultImageScreenPosition(){let o=Math.round(this.element.getBoundingClientRect().width);return{x:Math.max(256,o),y:32}}async compressImageBeforeLoad(o){let p=new Image;if(p.src=o,await F(p,["load"],["error"]),!(p.naturalWidth*p.naturalHeight>3000000||o.length>3000000))return o;let s=document.createElement("canvas");s.width=p.naturalWidth,s.height=p.naturalHeight;let a=s.getContext("2d");if(!a)return o;return a.drawImage(p,0,0),s.toDataURL("image/png")}async waitForStableViewportProjection(){let o=this.defaultImageScreenPosition(),p=0,l;for(let s=0;s<45;s++){await new Promise((w)=>requestAnimationFrame(()=>{w()}));let{anchorScreenPosition:{x:a,y:c},pixelSize:g}=this.bot.findAnchorsForScreen(o);if(!Number.isFinite(g)||g<=0){p=0;continue}let r={anchorX:a,anchorY:c,pixelSize:g};if(!l){l=r,p=1;continue}if(Math.abs(r.anchorX-l.anchorX)+Math.abs(r.anchorY-l.anchorY)+Math.abs(r.pixelSize-l.pixelSize)<0.0012)p++;else p=0;if(l=r,p>=3)return}}update(){this.$strategy.value=this.bot.strategy;let o=0,p=0;for(let c=0;c<this.bot.images.length;c++){let g=this.bot.images[c];o+=g.pixels.pixels.length*g.pixels.pixels[0].length,p+=g.tasks.length}let l=Math.max(0,o-p),s=o>0?l/o*100|0:0;this.$progressText.textContent=`${l}/${o} ${s}% ETA: ${p/120|0}h`,this.$progressLine.style.transform=`scaleX(${s/100})`,this.$images.innerHTML="";let a=document.createDocumentFragment();for(let c=0;c<this.bot.images.length;c++){let g=this.bot.images[c],r=document.createElement("div");a.append(r),r.className="image",r.innerHTML=`<button class="preview" title="View preview">
  ...
  </div>`,r.querySelector(".preview").addEventListener("click",()=>{this.activeImageIndex=c,g.openPreviewPanel()}),r.querySelector(".colors").addEventListener("click",()=>{this.activeImageIndex=c,g.openColorPanel()}),r.querySelector(".preview-strategy").addEventListener("click",()=>{this.activeImageIndex=c,g.openPreviewPanel()}),r.querySelector(".download").addEventListener("click",()=>{g.exportImage()}),r.querySelector(".delete").addEventListener("click",()=>{g.destroy()}),r.querySelector(".up").addEventListener("click",()=>{so(this.bot.images,c,c-1),this.update(),P(this.bot)}),r.querySelector(".down").addEventListener("click",()=>{so(this.bot.images,c,c+1),this.update(),P(this.bot)})}this.$images.append(a)}syncOverlayVisibilityFromStorage(){let o=localStorage.getItem(No)==="true";document.body.classList.toggle("overlay-hidden",o),this.refreshOverlayToggleText()}toggleOverlay(o){let p=o??!document.body.classList.contains("overlay-hidden");document.body.classList.toggle("overlay-hidden",p),localStorage.setItem(No,String(p)),this.refreshOverlayToggleText()}refreshOverlayToggleText(){this.$toggleOverlay.textContent=document.body.classList.contains("overlay-hidden")?`${b("toggleOverlay")} (${b("disabled")})`:`${b("toggleOverlay")} (${b("enabled")})`}setDisabled(o,p){this.element.querySelector("."+o).disabled=p}async run(o,p,l,s="..."){console.log("[KGM][Widget] Task started",{status:o});let a=this.status;this.status=`${s} ${o}`;try{let c=await p();return this.status=a,console.log("[KGM][Widget] Task completed",{status:o}),c}catch(c){if(!(c instanceof ao))console.error(c),this.status=`${b("taskErrorPrefix")}: ${o}`;throw console.error("[KGM][Widget] Task failed",{status:o,error:c}),c}finally{await l?.()}}handleKeyboard(o){if(Po(o.target))return;if(D(o,U.toggleWidget)){o.preventDefault(),this.open=!this.open;return}if(D(o,U.showShortcuts)){o.preventDefault(),this.open=!0,this.$shortcuts.open=!0,this.$shortcuts.scrollIntoView({behavior:"smooth",block:"center"}),this.$shortcuts.classList.remove("shortcut-pulse"),requestAnimationFrame(()=>{this.$shortcuts.classList.add("shortcut-pulse")});return}if(D(o,U.toggleOverlay)){o.preventDefault(),this.toggleOverlay();return}if(D(o,U.focusNextImage)){o.preventDefault(),this.focusImageByStep(1);return}if(D(o,U.focusPreviousImage)){o.preventDefault(),this.focusImageByStep(-1);return}if(D(o,U.openColorPanel)){o.preventDefault(),this.openColorPanelForActiveImage();return}if(D(o,U.toggleImageLock)){o.preventDefault(),this.toggleLockForActiveImage();return}if(D(o,U.clickPaintWhenReady)){o.preventDefault(),this.drawAndClickPaintWhenReady();return}if(D(o,U.addImage)&&!this.$addImage.disabled){o.preventDefault(),this.addImage();return}if(D(o,U.draw)&&!this.$draw.disabled)o.preventDefault(),this.bot.draw()}focusImageByStep(o){if(!this.bot.images.length)return;if(this.activeImageIndex<0||this.activeImageIndex>=this.bot.images.length)this.activeImageIndex=o>0?0:this.bot.images.length-1;else this.activeImageIndex=(this.activeImageIndex+o+this.bot.images.length)%this.bot.images.length;this.bot.images[this.activeImageIndex].position.scrollScreenTo()}getActiveImage(){if(!this.bot.images.length)return;if(this.activeImageIndex<0||this.activeImageIndex>=this.bot.images.length)this.activeImageIndex=0;return this.bot.images[this.activeImageIndex]}openColorPanelForActiveImage(){let o=this.getActiveImage();if(!o)return;o.openColorPanel()}toggleLockForActiveImage(){let o=this.getActiveImage();if(!o)return;o.lock=!o.lock,o.update(),P(this.bot)}async waitAndClickPaintButton(){await this.run(b("taskWaitingPaintButton"),async()=>{for(;;){let o=this.findNativePaintButton();if(o&&!o.disabled&&o.ariaDisabled!=="true"){this.triggerNativePaintClick(o);return}await new Promise((p)=>setTimeout(p,500))}})}async drawAndClickPaintWhenReady(){if(!this.$draw.disabled)await this.bot.draw();await this.waitAndClickPaintButton()}findNativePaintButton(){return["button.btn.btn-primary.btn-lg.sm\\:btn-xl.relative","button.btn.btn-primary.btn-lg.relative","button.btn.btn-primary.btn-lg.relative.z-30","button.btn.btn-primary.btn-lg.sm\\:btn-xl.relative.z-30","div.absolute.bottom-0.left-1\\/2.-translate-x-1\\/2 button.btn.btn-primary"].flatMap((l)=>Array.from(document.querySelectorAll(l))).find((l)=>/pintar|paint/i.test(l.textContent??""))}triggerNativePaintClick(o){o.dispatchEvent(new PointerEvent("pointerdown",{bubbles:!0,cancelable:!0,pointerType:"mouse",button:0})),o.dispatchEvent(new MouseEvent("mousedown",{bubbles:!0,cancelable:!0,button:0})),o.dispatchEvent(new PointerEvent("pointerup",{bubbles:!0,cancelable:!0,pointerType:"mouse",button:0})),o.dispatchEvent(new MouseEvent("mouseup",{bubbles:!0,cancelable:!0,button:0})),o.click()}}var Eo=2,Uo="[KGM]",Do="kglacer-macro:access-ok",wo="S0dNLXlZUjhTMW81bEhVemVjS1RFMEhxRVB4OVFkcjgxaEVz",Ko="kgm-access-locked";class Qo{unavailableColors=new Set;mapsCache=new Map;me;$stars=[];strategy="SEQUENTIAL";images=[];_widget;get widget(){if(!this._widget)throw new Error("Widget is not initialized yet");return this._widget}markerPixelPositionResolvers=[];lastColor;log(o,p){if(p===void 0)console.log(`${Uo} ${o}`);else console.log(`${Uo} ${o}`,p)}constructor(){this.log("Boot sequence started"),document.body.classList.add(Ko);let o=ho();if(this.log("Save loaded",{hasSave:Boolean(o),imageCount:o?.images.length??0,strategy:o?.strategy}),o){for(let l=0;l<o.images.length;l++){let s=o.images[l];$({x:s.position[0]-1000,y:s.position[1]-1000}),$({x:s.position[0]+1000,y:s.position[1]+1000})}this.strategy=o.strategy}this.registerFetchInterceptor(),this.log("Fetch interceptor registered");let p=document.createElement("style");p.textContent=ko.replace("FAKE_FAVORITE_LOCATIONS",Y.length.toString()),document.head.append(p),this.log("Styles injected",{fakeFavoriteLocations:Y.length}),(async()=>{this.log("Widget initialization flow started"),await this.ensureAccessKey(),document.body.classList.remove(Ko),this._widget=new fo(this),await this.widget.run(b("taskInitializing"),async()=>{await this.waitForElement("login",".avatar.center-absolute.absolute"),await this.waitForElement("pixel count",".btn.btn-primary.btn-lg.relative.z-30 canvas");let l=await this.waitForElement("canvas",".maplibregl-canvas-container");if(new MutationObserver((s)=>{for(let a=0;a<s.length;a++)if(s[a].removedNodes.length!==0){this.updateStars();break}this.updateImages()}).observe(l,{attributes:!0,childList:!0,subtree:!0}),this.updateStars(),this.log("Stars updated after boot",{stars:this.$stars.length}),await G(500),await this.updateColors(),o)for(let s=0;s<o.images.length;s++){let a=await V.fromJSON(this,o.images[s]);this.images.push(a),a.update()}this.log("Saved images restored",{images:this.images.length}),await this.readMap(),this.updateTasks(),this.widget.setDisabled("draw",!1),this.widget.setDisabled("draw-and-paint",!1),this.widget.setDisabled("add-image",!1),this.widget.setDisabled("capture-template",!1),this.log("Initialization completed; controls enabled")})})()}async ensureAccessKey(){if(localStorage.getItem(Do)===wo)return;await new Promise((o)=>{let p=document.createElement("dialog");p.className="kgm-modal access-dialog",p.innerHTML=`<form method="dialog" class="access-form">
...
</form>`,document.body.append(p),W(p);let l=p.querySelector(".access-input"),s=p.querySelector(".access-error"),a=p.querySelector(".access-locale");a.innerHTML=no().map((c)=>`<option value="${c}" ${c===_()?"selected":""}>${c.toUpperCase()}</option>`).join(""),a.addEventListener("change",()=>{oo(a.value),W(p)}),p.addEventListener("cancel",(c)=>{c.preventDefault()}),p.querySelector("form").addEventListener("submit",(c)=>{c.preventDefault();let g=atob(wo);if(l.value.trim()!==g){s.textContent=b("invalidAccessKey");return}localStorage.setItem(Do,wo),p.close(),p.remove(),o()}),p.showModal(),l.focus()})}draw(){this.log("Draw requested",{strategy:this.strategy,images:this.images.length}),this.widget.setDisabled("draw",!0),this.widget.setDisabled("draw-and-paint",!0),this.widget.status="",this.mapsCache.clear();let o=document.querySelector(".maplibregl-canvas"),p=(l)=>{if(!l.shiftKey)l.stopPropagation()};return this.widget.run(b("taskDrawing"),async()=>{await this.widget.run(b("taskInitializingDraw"),()=>Promise.all([this.updateColors(),this.readMap()])),globalThis.addEventListener("mousemove",p,!0),o.addEventListener("wheel",p,!0),this.updateTasks();let l=await fetch("https://backend.wplace.live/me",{credentials:"include"}).then((c)=>c.json()),s=Math.floor(l.charges.count);this.log("Charges fetched",{charges:s});let a=0;for(let c=0;c<this.images.length;c++)a+=this.images[c].tasks.length;switch(this.log("Tasks prepared",{tasks:a}),this.strategy){case"ALL":{while(s>0){let c=!0;for(let g=0;g<this.images.length;g++){let r=this.images[g].tasks.shift();if(!r)continue;this.drawTask(r),s--,await G(1),c=!1}if(c)break}break}case"PERCENTAGE":{for(let c=0;c<a&&s>0;c++){let g=1,r;for(let f=0;f<this.images.length;f++){let w=this.images[f],u=1-w.tasks.length/(w.pixels.pixels.length*w.pixels.pixels[0].length);if(u<g)g=u,r=w}this.drawTask(r.tasks.shift()),s--,await G(1)}break}case"SEQUENTIAL":for(let c=0;c<this.images.length;c++){let g=this.images[c];for(let r=g.tasks.shift();r&&s>0;r=g.tasks.shift())this.drawTask(r),s--,await G(1)}}this.widget.update(),this.log("Draw flow finished",{remainingCharges:s})},()=>{globalThis.removeEventListener("mousemove",p,!0),o.removeEventListener("wheel",p,!0),this.widget.setDisabled("draw",!1),this.widget.setDisabled("draw-and-paint",!1)})}toJSON(){return{version:Eo,images:this.images.map((o)=>o.toJSON()),strategy:this.strategy}}async updateColors(){this.log("Updating colors palette"),await this.openColors(),this.unavailableColors.clear();for(let o of document.querySelectorAll("button.btn.relative.w-full"))if(o.children.length!==0)this.unavailableColors.add(Math.abs(Number.parseInt(o.id.slice(6))));this.updateImageColors(),this.log("Colors updated",{unavailableColors:this.unavailableColors.size})}moveMap(o){let p=document.querySelector(".maplibregl-canvas"),l=window.innerWidth/2,s=window.innerHeight/2,a=l-o.x,c=s-o.y;function g(r,f,w){p.dispatchEvent(new MouseEvent(r,{bubbles:!0,cancelable:!0,clientX:f,clientY:w,buttons:1}))}g("mousedown",l,s),g("mousemove",a,c),g("mouseup",a,c)}readMap(){this.mapsCache.clear();let o=new Set;for(let l=0;l<this.images.length;l++){let s=this.images[l],{tileX:a,tileY:c}=new N(this,s.position.globalX+s.pixels.pixels[0].length,s.position.globalY+s.pixels.pixels.length);for(let g=s.position.tileX;g<=a;g++)for(let r=s.position.tileY;r<=c;r++)o.add(`${g}/${r}`)}let p=0;return this.log("Reading map tiles",{tileCount:o.size}),this.widget.run(`${b("taskReadingMap")} [0/${o.size}]`,()=>Promise.all([...o].map(async(l)=>{this.mapsCache.set(l,await B.fromJSON(this,{url:`https://backend.wplace.live/files/s0/tiles/${l}.png`,exactColor:!0})),this.widget.status=`⌛ ${b("taskReadingMap")} [${++p}/${o.size}]`})))}waitForUnfocus(){return this.widget.run("UNFOCUS WINDOW",()=>new Promise((o)=>{if(!document.hasFocus())o();window.addEventListener("blur",()=>{setTimeout(o,1)},{once:!0})}),void 0,"\uD83D\uDDB1️")}findAnchorsForScreen(o){let p=0,l=1,s=1/0,a=1/0;for(let r=0;r<this.$stars.length;r++){let{x:f,y:w}=L(this.$stars[r]);if(f<o.x&&w<o.y){let u=o.x-f+(o.y-w);if(u<s)s=u,p=r}else if(f>o.x&&w>o.y){let u=f-o.x+(w-o.y);if(u<a)a=u,l=r}}let c=L(this.$stars[p]),g=q[p];return{anchorScreenPosition:c,anchorWorldPosition:g,pixelSize:(L(this.$stars[l]).x-c.x)/(q[l].x-g.x)}}async openColors(){this.lastColor=void 0,document.querySelector(".flex.gap-2.px-3 > .btn-circle")?.click(),await G(1),document.querySelector(".btn.btn-primary.btn-lg.relative.z-30")?.click(),await G(1);let o=document.querySelector("button.bottom-0");if(o?.innerHTML==='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor" class="size-5"><path d="M480-120 300-300l58-58 122 122 122-122 58 58-180 180ZM358-598l-58-58 180-180 180 180-58 58-122-122-122 122Z"></path></svg><!---->')o.click(),await G(1)}drawTask(o){if(this.lastColor!==o.color){let s=document.getElementById("color-"+o.color);if(!s){this.log("Skipped draw task: color button not found",{color:o.color,tileX:o.position.tileX,tileY:o.position.tileY,x:o.position.x,y:o.position.y});return}s.click(),this.lastColor=o.color,this.log("Color switched for draw task",{color:o.color})}let p=o.position.pixelSize/2,l=o.position.toScreenPosition();if(!Number.isFinite(l.x)||!Number.isFinite(l.y)){this.log("Skipped draw task: invalid screen position",{color:o.color});return}document.documentElement.dispatchEvent(new MouseEvent("mousemove",{bubbles:!0,clientX:l.x+p,clientY:l.y+p,shiftKey:!0})),document.documentElement.dispatchEvent(new KeyboardEvent("keydown",{key:" ",code:"Space",keyCode:32,which:32,bubbles:!0,cancelable:!0})),document.documentElement.dispatchEvent(new KeyboardEvent("keyup",{key:" ",code:"Space",keyCode:32,which:32,bubbles:!0,cancelable:!0}))}registerFetchInterceptor(){let o=globalThis.fetch,p=/https:\/\/backend.wplace.live\/s\d+\/pixel\/(-?\d+)\/(-?\d+)\?x=(-?\d+)&y=(-?\d+)/;globalThis.fetch=async(l,s)=>{let a=await o(l,s),c=a.clone(),g="";if(typeof l=="string")g=l;else if(l instanceof Request)g=l.url;else if(l instanceof URL)g=l.href;if(a.url==="https://backend.wplace.live/me")this.me=await c.json(),this.me.favoriteLocations.unshift(...Y),this.me.maxFavoriteLocations=1/0,a.json=()=>Promise.resolve(this.me),this.log("Patched /me response with favorite locations",{totalFavorites:this.me.favoriteLocations.length});let r=p.exec(g);if(r){for(let f=0;f<this.markerPixelPositionResolvers.length;f++)this.markerPixelPositionResolvers[f](new N(this,+r[1],+r[2],+r[3],+r[4]));this.markerPixelPositionResolvers.length=0,this.log("Resolved marker pixel position from network event")}return a}}async closeAll(){for(let o of document.querySelectorAll("button"))if(o.innerHTML==="✕"||o.innerHTML==='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor" class="size-4"><path d="m256-200-56-56 224-224-224-224 56-56 224 224 224-224 56 56-224 224 224 224-56 56-224-224-224 224Z"></path></svg><!---->')o.click(),await G(1)}waitForElement(o,p){return this.log("Waiting for element",{name:o,selector:p}),this.widget.run(`${b("taskWaitingFor")} ${o}`,()=>{return new Promise((l)=>{let s=document.querySelector(p);if(s){l(s);return}let a=new MutationObserver(()=>{let c=document.querySelector(p);if(c)a.disconnect(),l(c)});a.observe(document.documentElement,{childList:!0,subtree:!0})})})}updateStars(){this.$stars=[...document.querySelectorAll(".text-yellow-400.cursor-pointer.z-10.maplibregl-marker.maplibregl-marker-anchor-center")].slice(0,Y.length),this.log("Star cache updated",{stars:this.$stars.length})}updateImages(){for(let o=0;o<this.images.length;o++)this.images[o].position.updateAnchor(),this.images[o].update()}updateTasks(){for(let o=0;o<this.images.length;o++)this.images[o].updateTasks()}updateImageColors(){for(let o=0;o<this.images.length;o++)this.images[o].updateColors()}}globalThis.kglacerMacro=new Qo;globalThis.kgm=globalThis.kglacerMacro;globalThis.wbot=globalThis.kglacerMacro;
