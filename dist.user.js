...
`;class K{bot;image;width;exactColor;static async fromJSON(o,a){let r=new Image;return r.src=a.url.startsWith("http")?await fetch(a.url,{cache:"no-store"}).then((s)=>s.blob()).then((s)=>URL.createObjectURL(s)):a.url,await Q(r,["load"],["error"]),new K(o,r,a.width,a.exactColor)}canvas=document.createElement("canvas");context=this.canvas.getContext("2d");pixels;colors=new Map;resolution;get height(){return this.width/this.resolution|0}set height(o){this.width=o*this.resolution|0}constructor(o,a,r=a.naturalWidth,s=!1){this.bot=o;this.image=a;this.width=r;this.exactColor=s;if(s)this.resolution=1,this.width=1000;else this.resolution=this.image.naturalWidth/this.image.naturalHeight;this.update()}update(){this.canvas.width=this.width,this.canvas.height=this.height,this.colors.clear();let o=new Map;for(let r=1;r<64;r++)o.set(q[r],[r,r]);this.context.imageSmoothingEnabled=!1,this.context.imageSmoothingQuality="low",this.context.drawImage(this.image,0,0,this.canvas.width,this.canvas.height),this.pixels=Array.from({length:this.canvas.height},()=>new Array(this.canvas.width));let a=this.context.getImageData(0,0,this.canvas.width,this.canvas.height).data;for(let r=0;r<this.canvas.height;r++)for(let s=0;s<this.canvas.width;s++){let l=(r*this.canvas.width+s)*4,i=a[l],c=a[l+1],p=a[l+2],n=a[l+3],g=i,t=c,d=p,u=`${g},${t},${d}`;if(this.exactColor){this.pixels[r][s]=n<100?0:q.indexOf(u);continue}let e,w;if(n<100)e=w=0;else if(o.has(u))[e,w]=o.get(u);else{let m=1/0,h=1/0;for(let b=0;b<C.length;b++){let A=C[b],M=ko(Mo(g,t,d),A,0);if(M<m)m=M,e=b;if(M<h)h=M,w=b}o.set(u,[e,w])}if(e!==0)this.context.fillStyle=`oklab(${C[e][0]*100}% ${C[e][1]} ${C[e][2]})`,this.context.fillRect(s,r,1,1);this.pixels[r][s]=e;let z=this.colors.get(w);if(z)z.amount++;else this.colors.set(w,{color:w,amount:1,realColor:w})}}toJSON(){let o=document.createElement("canvas");return o.width=this.image.naturalWidth,o.height=this.image.naturalHeight,o.getContext("2d").drawImage(this.image,0,0),{url:o.toDataURL("image/webp",1),width:this.width,exactColor:this.exactColor}}}var L="kglacer-macro-settings",Ho=["kglacermacro","wbot"],Y="kgm";function ca(){let o=[L,...Ho];for(let a=0;a<o.length;a++){let r=o[a],s=localStorage.getItem(r);if(!s)continue;return{json:s,key:r}}return}function So(){let o=ca();if(!o)return;let a;try{if(a=JSON.parse(o.json),typeof a!=="object")throw new Error("NOT VALID SAVE");if(a.version===1){let r=a.widget;if(r)a.images=r.images,a.strategy=r.strategy,delete a.widget}if(o.key!==L)localStorage.setItem(L,o.json)}catch{localStorage.removeItem(o.key),a=void 0}return a}var Fo;function k(o,a=!1){if(clearTimeout(Fo),a)localStorage.setItem(L,JSON.stringify(o));else Fo=setTimeout(()=>{localStorage.setItem(L,JSON.stringify(o))},600)}var P=1000,pa=2048,R=P*pa,G=[],E=[],na=Date.now();function _(o){G.push(o),E.push({id:na++,latitude:(2*Math.atan(Math.exp(-(o.y/R*(2*Math.PI)-Math.PI)))-Math.PI/2)*180/Math.PI,longitude:(o.x/R*(2*Math.PI)-Math.PI)*180/Math.PI,name:"KGLACER_MACRO_FAVORITE"})}_({x:R/3|0,y:R/3|0});_({x:R/3*2|0,y:R/3*2|0});function Z(o){let[a,r]=o.style.transform.slice(32,-31).split(", ").map((s)=>Number.parseFloat(s));return{x:a,y:r}}class S{bot;static fromJSON(o,a){return new S(o,...a)}static fromScreenPosition(o,a){let{anchorScreenPosition:r,pixelSize:s,anchorWorldPosition:l}=o.findAnchorsForScreen(a);return new S(o,l.x+(a.x-r.x)/s|0,l.y+(a.y-r.y)/s|0)}globalX=0;globalY=0;get tileX(){return this.globalX/P|0}set tileX(o){this.globalX=o*P+this.x}get tileY(){return this.globalY/P|0}set tileY(o){this.globalY=o*P+this.y}get x(){return this.globalX%P}set x(o){this.globalX=this.tileX*P+o}get y(){return this.globalY%P}set y(o){this.globalY=this.tileY*P+o}anchor1Index;anchor2Index;get pixelSize(){return(Z(this.bot.$stars[this.anchor2Index]).x-Z(this.bot.$stars[this.anchor1Index]).x)/(G[this.anchor2Index].x-G[this.anchor1Index].x)}constructor(o,a,r,s,l){this.bot=o;if(s===void 0||l===void 0)this.globalX=a,this.globalY=r;else this.globalX=a*P+s,this.globalY=r*P+l;this.updateAnchor()}updateAnchor(){this.anchor1Index=0,this.anchor2Index=1;let o=1/0,a=1/0;for(let r=0;r<G.length;r++){let{x:s,y:l}=G[r];if(s<this.globalX&&l<this.globalY){let i=this.globalX-s+(this.globalY-l);if(i<o)o=i,this.anchor1Index=r}else if(s>this.globalX&&l>this.globalY){let i=s-this.globalX+(l-this.globalY);if(i<a)a=i,this.anchor2Index=r}}}toScreenPosition(){let o=G[this.anchor1Index],a=Z(this.bot.$stars[this.anchor1Index]);return{x:(this.globalX-o.x)*this.pixelSize+a.x,y:(this.globalY-o.y)*this.pixelSize+a.y}}getMapColor(){return this.bot.mapsCache.get(this.tileX+"/"+this.tileY).pixels[this.y][this.x]}setMapColor(o){let a=this.bot.mapsCache.get(this.tileX+"/"+this.tileY);if(!a)return;let r=a.pixels[this.y];if(!r)return;r[this.x]=o}scrollScreenTo(){let{x:o,y:a}=this.toScreenPosition();this.bot.moveMap({x:o-window.innerWidth/3,y:a-window.innerHeight/3})}clone(){return new S(this.bot,this.tileX,this.tileY,this.x,this.y)}toJSON(){return[this.globalX,this.globalY]}}function ga(o){let a=[];for(let{x:r,y:s}of o.iterate){let l=o.pixels[s]?.[r]??0;if(o.disabledColors.has(l))continue;let i=o.readMapColor(r,s);if(l!==i&&(o.drawTransparentPixels||l!==0))a.push({x:r,y:s,color:l})}return a}class j extends X{bot;position;pixels;strategy;opacity;drawTransparentPixels;drawColorsInOrder;skipUnavailableColors;colors;lock;static PREVIEW_MASK_BASE_WIDTH=96;static PREVIEW_MASK_BASE_HEIGHT=96;static async fromJSON(o,a){return new j(o,S.fromJSON(o,a.position),await K.fromJSON(o,a.pixels),a.strategy,a.opacity,a.drawTransparentPixels,a.drawColorsInOrder,a.skipUnavailableColors,a.colors,a.lock)}element=document.createElement("div");tasks=[];moveInfo;$canvas;$colorsDialog;$colorsDialogList;$colorSearch;$openColors;$openPreview;$enableAllColors;$disableAllColors;$closeColors;$closePreview;$delete;$drawColorsInOrder;$drawTransparent;$skipUnavailable;$export;$lock;$opacity;$progressLine;$progressText;$previewDialog;$previewDialogList;$resetSize;$resetSizeSpan;$settings;$strategy;$topbar;$wrapper;colorDialogDragState;suppressNextColorDialogBackdropClick=!1;previewCacheSignature;previewSequenceCache=new Map;previewAnimations=new WeakMap;previewAnimationHandles=new Set;constructor(o,a,r,s="SPIRAL_FROM_CENTER",l=50,i=!1,c=!1,p=!0,n=[],g=!1){super();this.bot=o;this.position=a;this.pixels=r;this.strategy=s;this.opacity=l;this.drawTransparentPixels=i;this.drawColorsInOrder=c;this.skipUnavailableColors=p;this.colors=n;this.lock=g;this.element.innerHTML=Po,this.element.classList.add("wimage"),N(this.element),document.body.append(this.element),this.populateElementsWithSelector(this.element,{$colorsDialog:".colors-dialog",$colorsDialogList:".colors-dialog-list",$colorSearch:".color-search",$openColors:".open-colors",$openPreview:".open-preview",$enableAllColors:".enable-all-colors",$disableAllColors:".disable-all-colors",$closeColors:".close-colors",$closePreview:".close-preview",$delete:".delete",$drawColorsInOrder:".draw-colors-in-order",$drawTransparent:".draw-transparent",$skipUnavailable:".skip-unavailable",$export:".export",$lock:".lock",$opacity:".opacity",$progressLine:".wprogress div",$progressText:".wprogress span",$previewDialog:".preview-dialog",$previewDialogList:".preview-dialog-list",$resetSize:".reset-size",$settings:".wform",$strategy:".strategy",$topbar:".wtopbar",$wrapper:".wrapper"}),this.$resetSizeSpan=this.$resetSize.querySelector("span"),this.$canvas=this.pixels.canvas,this.$wrapper.prepend(this.pixels.canvas),document.body.append(this.$colorsDialog,this.$previewDialog),this.registerEvent(this.$strategy,"change",()=>{if(this.strategy=this.$strategy.value,this.$previewDialog.open)this.renderStrategyPreviewSamples();k(this.bot)}),this.registerEvent(this.$opacity,"input",()=>{this.opacity=this.$opacity.valueAsNumber,this.$opacity.style.setProperty("--val",this.opacity+"%"),this.update(),k(this.bot)}),this.$opacity.style.setProperty("--val",this.opacity+"%"),this.registerEvent(this.$resetSize,"click",()=>{this.pixels.width=this.pixels.image.naturalWidth,this.pixels.update(),this.updateColors(),this.update(),k(this.bot)}),this.registerEvent(this.$drawTransparent,"click",()=>{this.drawTransparentPixels=this.$drawTransparent.checked,k(this.bot)}),this.registerEvent(this.$skipUnavailable,"click",()=>{this.skipUnavailableColors=this.$skipUnavailable.checked,this.updateTasks(),k(this.bot)}),this.registerEvent(this.$drawColorsInOrder,"click",()=>{this.drawColorsInOrder=this.$drawColorsInOrder.checked,k(this.bot)}),this.registerEvent(this.$lock,"click",()=>{this.lock=!this.lock,this.update(),k(this.bot)}),this.registerEvent(this.$delete,"click",this.destroy.bind(this)),this.registerEvent(this.$openColors,"click",()=>{this.openColorPanel()}),this.registerEvent(this.$openPreview,"click",()=>{this.openPreviewPanel()}),this.registerEvent(this.$closeColors,"click",()=>{this.closeDialog(this.$colorsDialog)}),this.registerEvent(this.$closePreview,"click",()=>{this.closeDialog(this.$previewDialog)}),this.registerEvent(this.$colorsDialog.querySelector(".colors-dialog-head"),"pointerdown",this.startColorDialogDrag.bind(this)),this.registerEvent(document,"pointermove",this.moveColorDialog.bind(this),{passive:!1}),this.registerEvent(document,"pointerup",this.stopColorDialogDrag.bind(this)),this.registerEvent(document,"pointercancel",this.stopColorDialogDrag.bind(this)),this.registerEvent(this.$colorsDialog,"click",(t)=>{if(this.suppressNextColorDialogBackdropClick){this.suppressNextColorDialogBackdropClick=!1;return}if(t.target===this.$colorsDialog)this.closeDialog(this.$colorsDialog)}),this.registerEvent(this.$previewDialog,"click",(t)=>{if(t.target===this.$previewDialog)this.closeDialog(this.$previewDialog)}),this.registerEvent(this.$colorSearch,"input",()=>{this.updateColors()}),this.registerEvent(this.$enableAllColors,"click",()=>{for(let t of this.colors)t.disabled=void 0;this.updateTasks(),this.updateColors(),k(this.bot)}),this.registerEvent(this.$disableAllColors,"click",()=>{for(let t of this.colors)t.disabled=!0;this.updateTasks(),this.updateColors(),k(this.bot)}),this.registerEvent(this.$export,"click",this.export.bind(this)),this.registerEvent(this.$topbar,"mousedown",this.moveStart.bind(this)),this.registerEvent(this.$canvas,"mousedown",this.moveStart.bind(this)),this.registerEvent(document,"mouseup",this.moveStop.bind(this)),this.registerEvent(document,"mousemove",this.move.bind(this));for(let t of this.element.querySelectorAll(".resize"))this.registerEvent(t,"mousedown",this.resizeStart.bind(this));this.update(),this.updateColors()}toJSON(){return{pixels:this.pixels.toJSON(),position:this.position.toJSON(),strategy:this.strategy,opacity:this.opacity,drawTransparentPixels:this.drawTransparentPixels,drawColorsInOrder:this.drawColorsInOrder,skipUnavailableColors:this.skipUnavailableColors,colors:this.colors,lock:this.lock}}updateTasks(){this.tasks.length=0;let o=this.position.clone(),a=new Set,r=new Map;for(let l=0;l<this.colors.length;l++){let i=this.colors[l];if(i.disabled)a.add(i.realColor);r.set(i.realColor,l)}let s=ga({pixels:this.pixels.pixels,drawTransparentPixels:this.drawTransparentPixels,disabledColors:a,iterate:this.strategyPositionIterator(),readMapColor:(l,i)=>{return o.globalX=this.position.globalX+l,o.globalY=this.position.globalY+i,o.getMapColor()}});for(let l=0;l<s.length;l++){let i=s[l];o.globalX=this.position.globalX+i.x,o.globalY=this.position.globalY+i.y,this.tasks.push({position:o.clone(),color:i.color})}if(this.drawColorsInOrder)this.tasks.sort((l,i)=>(r.get(l.color)??0)-(r.get(i.color)??0));this.update(),this.bot.widget.update()}update(){let{x:o,y:a}=this.position.toScreenPosition(),r=this.position.pixelSize*this.pixels.width,s=this.position.pixelSize*this.pixels.height;this.element.style.transform=`translate3d(${o.toFixed(3)}px, ${a.toFixed(3)}px, 0)`,this.element.style.width=`${r}px`,this.element.style.height=`${s}px`,this.$canvas.style.opacity=`${this.opacity}%`,this.element.classList.remove("hidden"),this.$resetSizeSpan.textContent=this.pixels.width.toString(),this.$strategy.value=this.strategy,this.$opacity.valueAsNumber=this.opacity,this.$drawTransparent.checked=this.drawTransparentPixels,this.$drawColorsInOrder.checked=this.drawColorsInOrder,this.$skipUnavailable.checked=this.skipUnavailableColors;let l=this.pixels.pixels.length*this.pixels.pixels[0].length,i=Math.max(0,l-this.tasks.length),c=l>0?i/l*100|0:0;this.$progressText.textContent=`${i}/${l} ${c}% ETA: ${this.tasks.length/120|0}h`,this.$progressLine.style.transform=`scaleX(${c/100})`,this.$canvas.classList[this.lock?"add":"remove"]("no-pointer-events");for(let p of this.element.querySelectorAll(".resize"))p.classList[this.lock?"add":"remove"]("no-pointer-events");this.$lock.classList[this.lock?"add":"remove"]("locked")}exportImage(){this.export()}destroy(){super.destroy(),this.element.remove(),this.$colorsDialog.remove(),this.$previewDialog.remove(),co(this.bot.images,this),this.bot.widget.update(),k(this.bot)}openColorPanel(){if(this.$colorsDialog.open){this.$colorSearch.focus();return}this.$colorsDialog.style.position="fixed",this.$colorsDialog.style.left="",this.$colorsDialog.style.top="",this.$colorsDialog.style.margin="auto",this.$colorsDialog.showModal(),this.$colorSearch.focus()}openPreviewPanel(){if(this.$previewDialog.open){this.renderStrategyPreviewSamples();return}this.$previewDialog.style.position="fixed",this.$previewDialog.style.left="",this.$previewDialog.style.top="",this.$previewDialog.style.margin="auto",this.$previewDialog.showModal(),this.renderStrategyPreviewSamples()}closeDialog(o){if(!o.open)return;if(o===this.$previewDialog)this.stopPreviewAnimations();if(typeof o.requestClose==="function")o.requestClose();else o.close()}stopPreviewAnimations(){for(let o of this.previewAnimationHandles)cancelAnimationFrame(o);this.previewAnimationHandles.clear()}startColorDialogDrag(o){if(o.button!==0)return;if(o.target?.closest("button,input,select,textarea,a,label"))return;let r=this.$colorsDialog.getBoundingClientRect();this.colorDialogDragState={pointerId:o.pointerId,offsetX:o.clientX-r.left,offsetY:o.clientY-r.top,moved:!1},o.preventDefault()}moveColorDialog(o){if(!this.colorDialogDragState)return;if(o.pointerId!==this.colorDialogDragState.pointerId)return;let a=this.$colorsDialog.getBoundingClientRect(),r=Math.max(8,window.innerWidth-a.width-8),s=Math.max(8,window.innerHeight-a.height-8),l=Math.min(r,Math.max(8,o.clientX-this.colorDialogDragState.offsetX)),i=Math.min(s,Math.max(8,o.clientY-this.colorDialogDragState.offsetY));if(!this.colorDialogDragState.moved&&(Math.abs(o.movementX)>0||Math.abs(o.movementY)>0))this.colorDialogDragState.moved=!0;this.$colorsDialog.style.left=`${Math.round(l)}px`,this.$colorsDialog.style.top=`${Math.round(i)}px`,o.preventDefault()}stopColorDialogDrag(o){if(!this.colorDialogDragState)return;if(o.pointerId!==this.colorDialogDragState.pointerId)return;if(this.colorDialogDragState.moved)this.suppressNextColorDialogBackdropClick=!0;this.colorDialogDragState=void 0}renderStrategyPreviewSamples(){this.stopPreviewAnimations(),this.invalidatePreviewCacheIfNeeded();let o=this.$strategy.value;this.$previewDialogList.innerHTML="";let a=document.createDocumentFragment(),r=document.createElement("article");r.className="preview-card";let s=document.createElement("strong");s.textContent=this.getStrategyLabel(o);let l=document.createElement("canvas");l.className="preview-canvas",l.width=156,l.height=156,this.paintStrategyPreview(l,o),r.append(s,l),a.append(r),this.$previewDialogList.append(a)}invalidatePreviewCacheIfNeeded(){let o=this.colors.map((r,s)=>`${s}:${r.realColor}:${r.disabled?1:0}`).join("|"),a=`${this.pixels.width}x${this.pixels.height}:${this.pixels.image.src.length}:${this.drawColorsInOrder?1:0}:${o}`;if(this.previewCacheSignature===a)return;this.previewCacheSignature=a,this.previewSequenceCache.clear()}getStrategyLabel(o){switch(o){case"RANDOM":return f("random");case"HUMANIZED":return f("humanized");case"HUMAN_SOFT_DITHER":return f("humanSoftDither");case"HUMAN_PATCHY":return f("humanPatchy");case"HUMAN_SWEEP_ARCS":return f("humanSweepArcs");case"HUMAN_MICRO_CORRECTIONS":return f("humanMicroCorrections");case"HUMAN_JITTER_FILL":return f("humanJitterFill");case"HUMAN_CORNER_BIAS":return f("humanCornerBias");case"HUMAN_LONG_STROKES":return f("humanLongStrokes");case"HUMAN_TAP_CLUSTERS":return f("humanTapClusters");case"HUMAN_MESSY_SPIRAL":return f("humanMessySpiral");case"HUMAN_DRUNK_WALK":return f("humanDrunkWalk");case"HUMAN_NOISE_CLOUD":return f("humanNoiseCloud");case"HUMAN_PATCH_JUMP":return f("humanPatchJump");case"HUMAN_HESITANT_LINES":return f("humanHesitantLines");case"HUMAN_OVERLAP_SWEEPS":return f("humanOverlapSweeps");case"HUMAN_WOBBLE_DRIFT":return f("humanWobbleDrift");case"HUMAN_GAP_RECOVERY":return f("humanGapRecovery");case"HUMAN_STAIRCASE":return f("humanStaircase");case"HUMAN_EDGE_HUGGER":return f("humanEdgeHugger");case"HUMAN_BLOBS":return f("humanBlobs");case"HUMAN_BACKTRACK":return f("humanBacktrack");case"HUMAN_SHAKY_DIAGONAL":return f("humanShakyDiagonal");case"HUMAN_LATE_FIXES":return f("humanLateFixes");case"ZIGZAG":return f("zigzag");case"BRUSH_STROKES":return f("brushStrokes");case"DIAGONAL_BRUSH":return f("diagonalBrush");case"DOWN":return f("down");case"UP":return f("up");case"LEFT":return f("left");case"RIGHT":return f("right");case"SPIRAL_FROM_CENTER":return f("spiralOut");case"SPIRAL_TO_CENTER":return f("spiralIn");case"SCRIBBLE":return f("scribble");case"CROSSHATCH":return f("crosshatch");case"WAVE_SWEEP":return f("waveSweep");case"SCATTERED_LINES":return f("scatteredLines");case"CONTOUR_JITTER":return f("contourJitter");case"SPIRAL_WOBBLE":return f("spiralWobble");case"CLUSTER_BURSTS":return f("clusterBursts");case"ORBITAL":return f("orbital");case"FLOW_FIELD":return f("flowField");case"EDGE_IN":return f("edgeIn");default:return o}}paintStrategyPreview(o,a){let r=o.getContext("2d");if(!r)return;r.fillStyle="#0f1526",r.fillRect(0,0,o.width,o.height);let s=this.getSampledImagePreviewData(),l=this.getCachedPreviewSequence(a,s.mask,s.width,s.height),i=Math.min(o.width/s.width,o.height/s.height),c=(o.width-s.width*i)/2,p=(o.height-s.height*i)/2,n=this.previewAnimations.get(o);if(n)cancelAnimationFrame(n),this.previewAnimationHandles.delete(n);let g=(m)=>{let h=requestAnimationFrame((b)=>{this.previewAnimationHandles.delete(h),m(b)});return this.previewAnimationHandles.add(h),h},t=(m)=>{r.fillStyle="#0f1526",r.fillRect(0,0,o.width,o.height);for(let h=0;h<Math.min(m,l.length);h++){let b=l[h],A=s.colors.get(`${b.x}:${b.y}`)??0;if(!A)continue;r.fillStyle=to(A),r.fillRect(c+b.x*i,p+b.y*i,Math.max(1,i),Math.max(1,i))}},d=Math.min(3400,Math.max(900,l.length*8)),e=d+220,w=(m,h)=>{if(!this.$previewDialog.open)return;let b=(h-m)%e,A=Math.min(1,b/d),M=A*A*(3-2*A);t(Math.floor(l.length*M));let J=g((W)=>{w(m,W)});this.previewAnimations.set(o,J)},z=performance.now();w(z,z)}getCachedPreviewSequence(o,a,r=this.pixels.width,s=this.pixels.height){let l=this.colors.map((n,g)=>`${g}:${n.realColor}:${n.disabled?1:0}`).join("|"),i=`${o}:${r}x${s}:${a.length}:${this.drawColorsInOrder?1:0}:${l}`,c=this.previewSequenceCache.get(i);if(c)return c;let p=r===this.pixels.width&&s===this.pixels.height?this.getExactPreviewSequence(o,a):this.getApproxPreviewSequence(o,a,r);if(this.drawColorsInOrder){let n=new Map;for(let g=0;g<this.colors.length;g++)n.set(this.colors[g].realColor,g);p.sort((g,t)=>(n.get(this.pixels.pixels[g.y]?.[g.x]??0)??0)-(n.get(this.pixels.pixels[t.y]?.[t.x]??0)??0))}return this.previewSequenceCache.set(i,p),p}getExactPreviewSequence(o,a){let r=this.strategy;this.strategy=o;let s=[...this.strategyPositionIterator()];this.strategy=r;let l=new Set(a.map(({x:i,y:c})=>`${i}:${c}`));return s.filter(({x:i,y:c})=>l.has(`${i}:${c}`))}getApproxPreviewSequence(o,a,r){let s=[...a],l=(p,n,g)=>{return(p*73856093+n*19349663+g*83492791>>>0)/4294967296},i=(p,n)=>s.sort((g,t)=>g.x*p+g.y*n-(t.x*p+t.y*n)||g.y-t.y||g.x-t.x),c=s.sort((p,n)=>{if(p.y!==n.y)return p.y-n.y;let g=p.y%2===0?p.x:r-p.x,t=n.y%2===0?n.x:r-n.x;return g-t});switch(o){case"UP":return i(0,-1);case"LEFT":return i(-1,0);case"RIGHT":return i(1,0);case"SPIRAL_FROM_CENTER":case"SPIRAL_TO_CENTER":{let p=r/2,n=Math.max(1,Math.round(s.reduce((g,t)=>g+t.y,0)/Math.max(1,s.length)));return s.sort((g,t)=>{let d=(g.x-p)**2+(g.y-n)**2,u=(t.x-p)**2+(t.y-n)**2;return o==="SPIRAL_FROM_CENTER"?d-u:u-d}),s}case"RANDOM":case"HUMANIZED":case"HUMAN_SOFT_DITHER":case"HUMAN_PATCHY":case"HUMAN_SWEEP_ARCS":case"HUMAN_MICRO_CORRECTIONS":case"HUMAN_JITTER_FILL":case"HUMAN_CORNER_BIAS":case"HUMAN_LONG_STROKES":case"HUMAN_TAP_CLUSTERS":case"HUMAN_MESSY_SPIRAL":case"HUMAN_DRUNK_WALK":case"HUMAN_NOISE_CLOUD":case"HUMAN_PATCH_JUMP":case"HUMAN_HESITANT_LINES":case"HUMAN_OVERLAP_SWEEPS":case"HUMAN_WOBBLE_DRIFT":case"HUMAN_GAP_RECOVERY":case"HUMAN_STAIRCASE":case"HUMAN_EDGE_HUGGER":case"HUMAN_BLOBS":case"HUMAN_BACKTRACK":case"HUMAN_SHAKY_DIAGONAL":case"HUMAN_LATE_FIXES":return s.sort((p,n)=>l(p.x,p.y,o.length)-l(n.x,n.y,o.length));default:return c}}getSampledImagePreviewData(){let o=this.pixels.width,a=this.pixels.height,r=j.PREVIEW_MASK_BASE_WIDTH,s=j.PREVIEW_MASK_BASE_HEIGHT,l=Math.min(1,r/Math.max(1,o),s/Math.max(1,a)),i=Math.max(1,Math.round(o*l)),c=Math.max(1,Math.round(a*l)),p=new Set;for(let d=0;d<this.colors.length;d++){let u=this.colors[d];if(u.disabled)p.add(u.realColor)}let n=new Map,g=new Map;for(let d=0;d<a;d++)for(let u=0;u<o;u++){let e=this.pixels.pixels[d]?.[u]??0;if(!e||p.has(e))continue;let w=Math.min(i-1,Math.floor(u/o*i)),z=Math.min(c-1,Math.floor(d/a*c)),m=`${w}:${z}`;if(!n.has(m))n.set(m,{x:w,y:z});if(!g.has(m))g.set(m,e)}let t=[...n.values()];if(!t.length){let d=this.fallbackPreviewMask();return{width:o,height:a,mask:d,colors:new Map(d.map((u)=>[`${u.x}:${u.y}`,this.pixels.pixels[u.y]?.[u.x]??0]))}}return{width:i,height:c,mask:t,colors:g}}getImagePreviewMask(){let o=this.pixels.width,a=this.pixels.height,r=new Set;for(let l=0;l<this.colors.length;l++){let i=this.colors[l];if(i.disabled)r.add(i.realColor)}let s=[];for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=this.pixels.pixels[l]?.[i]??0;if(c!==0&&!r.has(c))s.push({x:i,y:l})}return s.length?s:this.fallbackPreviewMask()}fallbackPreviewMask(){let o=[],a=this.pixels.width/2,r=this.pixels.height/2,s=Math.max(4,Math.min(this.pixels.width,this.pixels.height)/2.5);for(let l=0;l<this.pixels.height;l++)for(let i=0;i<this.pixels.width;i++)if((i-a)**2+(l-r)**2<=s**2)o.push({x:i,y:l});return o}applyLocale(){if(N(this.element),this.updateColors(),this.$previewDialog.open)this.renderStrategyPreviewSamples()}colorHex(o){let a=q[o]??"0,0,0",[r=0,s=0,l=0]=a.split(",").map((i)=>Number.parseInt(i,10));return`#${[r,s,l].map((i)=>i.toString(16).padStart(2,"0")).join("")}`}colorKeywords(o){let a=q[o]??"0,0,0",[r=0,s=0,l=0]=a.split(",").map((n)=>Number.parseInt(n,10)),i=Math.max(r,s,l),c=Math.min(r,s,l);if(i-c<15)return["gray","grey","gris","neutral","neutro"];if(r>s+30&&r>l+30)return["red","rojo"];if(s>r+30&&s>l+30)return["green","verde"];if(l>r+30&&l>s+30)return["blue","azul"];if(r>170&&s>120&&l<130)return["orange","naranja"];if(r>170&&s>110&&l>140)return["pink","rosa"];if(r>120&&s<100&&l>120)return["purple","violet","morado"];if(r>130&&s>130&&l<90)return["yellow","amarillo"];return["brown","cafe","marron"]}updateColors(){this.$colorsDialogList.innerHTML="";let o=this.pixels.pixels.length*this.pixels.pixels[0].length;this.$colorsDialogList.setAttribute("aria-label",f("colorPanelResults"));let a=this.$colorSearch.value.trim().toLowerCase();if(this.colors.length!==this.pixels.colors.size||this.colors.some((r)=>!this.pixels.colors.has(r.realColor))){let r=new Map(this.colors.map((s)=>[s.realColor,s]));this.colors=this.pixels.colors.values().toArray().sort((s,l)=>l.amount-s.amount).map((s)=>({realColor:s.realColor,disabled:r.get(s.realColor)?.disabled})),k(this.bot)}for(let r=0;r<this.colors.length;r++){let s=this.colors[r],l=this.pixels.colors.get(s.realColor),i=!1,c=l.amount/o*100,p=this.colorHex(l.realColor),n=this.colorKeywords(l.realColor),g=()=>{s.disabled=s.disabled?void 0:!0,t.classList.toggle("disabled",Boolean(s.disabled));let e=t.querySelector(".state");if(e)e.textContent=s.disabled?f("disabled"):f("enabled");k(this.bot)},t=document.createElement("button");t.className=`color-chip ${s.disabled?"disabled":""}`,t.draggable=!0,t.setAttribute("aria-label",`${f("overlayColors")} #${r+1}: ${p.toUpperCase()}`),t.innerHTML=`<span class="order-index">#${r+1}</span>
<span class="drag" title="${f("up")} / ${f("down")}">⋮⋮</span>
<span class="swatch"></span>
<span class="meta">
  <span class="coverage">${c.toFixed(1)}%</span>
  <span class="hex">${p.toUpperCase()}</span>
  <span class="state">${s.disabled?f("disabled"):f("enabled")}</span>
</span>
<span class="premium"></span>`,t.querySelector(".swatch").style.setProperty("--swatch-color",to(l.realColor)),t.addEventListener("click",()=>{if(i){i=!1;return}g(),this.updateTasks()}),t.addEventListener("dragstart",(e)=>{i=!0,t.classList.add("dragging"),e.dataTransfer?.setData("text/plain",String(r)),e.dataTransfer.effectAllowed="move"}),t.addEventListener("dragend",()=>{t.classList.remove("dragging")}),t.addEventListener("dragover",(e)=>{e.preventDefault(),t.classList.add("drag-target")}),t.addEventListener("dragleave",()=>{t.classList.remove("drag-target")}),t.addEventListener("drop",(e)=>{e.preventDefault(),t.classList.remove("drag-target");let w=Number.parseInt(e.dataTransfer?.getData("text/plain")??"-1",10);if(w<0||w===r||w>=this.colors.length)return;this.colors.splice(r,0,...this.colors.splice(w,1)),k(this.bot),this.updateColors()});let d=document.createElement("button");d.textContent=f("buy"),d.className="buy-chip",d.addEventListener("click",(e)=>{e.stopPropagation(),document.getElementById("color-"+l.realColor)?.click()}),t.append(d);let u=`${p} ${n.join(" ")} ${l.realColor} ${q[l.realColor]}`;if(!a||u.toLowerCase().includes(a))this.$colorsDialogList.append(t)}}*strategyPositionIterator(){let o=this.pixels.pixels[0].length,a=this.pixels.pixels.length;switch(this.strategy){case"DOWN":{for(let r=0;r<a;r++)for(let s=0;s<o;s++)yield{x:s,y:r};break}case"UP":{for(let r=a-1;r>=0;r--)for(let s=0;s<o;s++)yield{x:s,y:r};break}case"LEFT":{for(let r=0;r<o;r++)for(let s=0;s<a;s++)yield{x:r,y:s};break}case"RIGHT":{for(let r=o-1;r>=0;r--)for(let s=0;s<a;s++)yield{x:r,y:s};break}case"RANDOM":{let r=[];for(let s=0;s<a;s++)for(let l=0;l<o;l++)r.push({x:l,y:s});for(let s=r.length-1;s>=0;s--){let l=Math.floor(Math.random()*(s+1)),i=r[s];r[s]=r[l],r[l]=i}yield*r;break}case"ZIGZAG":{for(let r=0;r<a;r++)if(r%2===0)for(let s=0;s<o;s++)yield{x:s,y:r};else for(let s=o-1;s>=0;s--)yield{x:s,y:r};break}case"HUMANIZED":{let r=Math.max(4,Math.floor(Math.min(o,a)/10)),s=[];for(let l=0;l<a;l+=r)for(let i=0;i<o;i+=r)s.push({x:i,y:l});for(let l=s.length-1;l>=0;l--){let i=Math.floor(Math.random()*(l+1)),c=s[l];s[l]=s[i],s[i]=c}for(let l=0;l<s.length;l++){let i=s[l],c=Math.min(a,i.y+r),p=Math.min(o,i.x+r);for(let n=i.y;n<c;n++)if(Math.random()>0.35)for(let t=i.x;t<p;t++)yield{x:t,y:n};else for(let t=p-1;t>=i.x;t--)yield{x:t,y:n}}break}case"HUMAN_SOFT_DITHER":{let r=new Set;for(let s=0;s<a;s++){let l=Math.floor(Math.random()*3)-1;if((s+l)%2===0)for(let c=0;c<o;c+=2)r.add(`${c},${s}`),yield{x:c,y:s};else for(let c=1;c<o;c+=2)r.add(`${c},${s}`),yield{x:c,y:s}}for(let s=0;s<a;s++)for(let l=0;l<o;l++){let i=`${l},${s}`;if(r.has(i))continue;yield{x:l,y:s}}break}case"HUMAN_PATCHY":{let r=new Set,s=o*a;while(r.size<s){let l=Math.floor(Math.random()*o),i=Math.floor(Math.random()*a),c=1+Math.floor(Math.random()*5);for(let p=i-c;p<=i+c;p++)for(let n=l-c;n<=l+c;n++){if(n<0||n>=o||p<0||p>=a)continue;if(Math.hypot(n-l,p-i)>c+Math.random()*1.2)continue;let g=`${n},${p}`;if(r.has(g))continue;r.add(g),yield{x:n,y:p}}}break}case"HUMAN_SWEEP_ARCS":{let r=new Set,s=(o-1)/2,l=(a-1)/2,i=Math.hypot(s,l);for(let c=0;c<4;c++){let p=Math.random()*Math.PI*2;for(let n=0;n<=i;n+=0.35){let g=Math.PI/2+Math.random()*(Math.PI/1.5),t=Math.max(10,Math.floor(n*8));for(let d=0;d<t;d++){let u=p+g*d/t+Math.sin(n)*0.08,e=Math.round(s+Math.cos(u)*n),w=Math.round(l+Math.sin(u)*n);if(e<0||e>=o||w<0||w>=a)continue;let z=`${e},${w}`;if(r.has(z))continue;r.add(z),yield{x:e,y:w}}}}for(let c=0;c<a;c++)for(let p=0;p<o;p++){let n=`${p},${c}`;if(r.has(n))continue;yield{x:p,y:c}}break}case"HUMAN_MICRO_CORRECTIONS":{let r=new Set;for(let s=0;s<a;s++){let l=s%2===0?1:-1,i=l>0?0:o-1;for(let c=0;c<o;c++){let p=i+(Math.random()>0.82?l:0),n=s+(Math.random()>0.9?1:0);for(let g of[{x:i,y:s},{x:p,y:s},{x:i,y:n}]){if(g.x<0||g.x>=o||g.y<0||g.y>=a)continue;let t=`${g.x},${g.y}`;if(r.has(t))continue;r.add(t),yield g}i+=l}}for(let s=0;s<a;s++)for(let l=0;l<o;l++){let i=`${l},${s}`;if(r.has(i))continue;yield{x:l,y:s}}break}case"HUMAN_JITTER_FILL":{let r=[];for(let s=0;s<a;s++)for(let l=0;l<o;l++)r.push({x:l,y:s});r.sort((s,l)=>{let i=s.y+(Math.random()-0.5)*1.8,c=l.y+(Math.random()-0.5)*1.8;if(i!==c)return i-c;return s.x+(Math.random()-0.5)*2-(l.x+(Math.random()-0.5)*2)}),yield*r;break}case"HUMAN_CORNER_BIAS":{let r=[{x:0,y:0},{x:o-1,y:0},{x:0,y:a-1},{x:o-1,y:a-1}],s=r[Math.floor(Math.random()*r.length)],l=[];for(let i=0;i<a;i++)for(let c=0;c<o;c++){let n=Math.hypot(c-s.x,i-s.y)+Math.random()*3.5;l.push({point:{x:c,y:i},score:n})}l.sort((i,c)=>i.score-c.score);for(let i of l)yield i.point;break}case"HUMAN_LONG_STROKES":{let r=new Set,s=o*a;while(r.size<s){let l=Math.floor(Math.random()*o),i=Math.floor(Math.random()*a),c=Math.random()*Math.PI*2,p=Math.sign(Math.cos(c)),n=Math.sign(Math.sin(c)),g=10+Math.floor(Math.random()*40);for(let t=0;t<g;t++){if(l<0||l>=o||i<0||i>=a)break;let d=`${l},${i}`;if(!r.has(d))r.add(d),yield{x:l,y:i};if(Math.random()>0.78)l+=n,i+=p;else l+=p,i+=n}}break}case"HUMAN_TAP_CLUSTERS":{let r=new Set,s=o*a;while(r.size<s){let l=Math.floor(Math.random()*o),i=Math.floor(Math.random()*a),c=3+Math.floor(Math.random()*10);for(let p=0;p<c;p++){let n=Math.round(l+(Math.random()-0.5)*6),g=Math.round(i+(Math.random()-0.5)*6);if(n<0||n>=o||g<0||g>=a)continue;let t=`${n},${g}`;if(r.has(t))continue;r.add(t),yield{x:n,y:g}}}break}case"HUMAN_MESSY_SPIRAL":{let r=new Set,s=(o-1)/2,l=(a-1)/2,i=Math.hypot(s,l)+2;for(let c=0;r.size<o*a;c++){let p=c/3,n=Math.min(i,p*0.18),g=p*0.29+Math.sin(p*0.13)*0.8,t=Math.round(s+Math.cos(g)*n+Math.sin(p)*0.7),d=Math.round(l+Math.sin(g)*n+Math.cos(p)*0.7);if(t<0||t>=o||d<0||d>=a){if(c>o*a*18)break;continue}let u=`${t},${d}`;if(r.has(u)){if(Math.random()>0.9)continue}else r.add(u),yield{x:t,y:d};if(c>o*a*18)break}for(let c=0;c<a;c++)for(let p=0;p<o;p++){let n=`${p},${c}`;if(r.has(n))continue;yield{x:p,y:c}}break}case"HUMAN_DRUNK_WALK":{let r=new Set,s=Math.floor(Math.random()*o),l=Math.floor(Math.random()*a),i=[{x:-1,y:0},{x:1,y:0},{x:0,y:-1},{x:0,y:1},{x:-1,y:-1},{x:1,y:-1},{x:-1,y:1},{x:1,y:1}];while(r.size<o*a){let c=`${s},${l}`;if(!r.has(c))r.add(c),yield{x:s,y:l};let p=[];for(let t of i){let d=s+t.x,u=l+t.y;if(d<0||d>=o||u<0||u>=a)continue;p.push({x:d,y:u})}if(!p.length)break;let n=p.filter((t)=>{return!r.has(`${t.x},${t.y}`)});if(n.length&&Math.random()>0.2){let t=n[Math.floor(Math.random()*n.length)];s=t.x,l=t.y;continue}let g=p[Math.floor(Math.random()*p.length)];s=g.x,l=g.y}for(let c=0;c<a;c++)for(let p=0;p<o;p++){let n=`${p},${c}`;if(r.has(n))continue;yield{x:p,y:c}}break}case"HUMAN_NOISE_CLOUD":{let r=[];for(let s=0;s<a;s++)for(let l=0;l<o;l++){let i=Math.sin((l+1)*0.93+Math.random()*0.8)+Math.cos((s+1)*1.17+Math.random()*0.8),c=(Math.random()-0.5)*2.6,p=Math.hypot(l-o/2,s-a/2)*0.08;r.push({point:{x:l,y:s},score:i+c+p})}r.sort((s,l)=>s.score-l.score);for(let s of r)yield s.point;break}case"HUMAN_PATCH_JUMP":{let r=new Set,s=[];for(let l=0;l<Math.max(6,o*a/18);l++)s.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*a)});while(r.size<o*a){let l=s[Math.floor(Math.random()*s.length)],i=1+Math.floor(Math.random()*3),c=1+Math.floor(Math.random()*3);for(let p=l.y-c;p<=l.y+c;p++)for(let n=l.x-i;n<=l.x+i;n++){if(n<0||n>=o||p<0||p>=a)continue;if(Math.random()>0.86)continue;let g=`${n},${p}`;if(r.has(g))continue;r.add(g),yield{x:n,y:p}}if(Math.random()>0.72&&s.length<o*a/2)s.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*a)});if(r.size>o*a*0.92)break}for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=`${i},${l}`;if(r.has(c))continue;yield{x:i,y:l}}break}case"HUMAN_HESITANT_LINES":{let r=new Set;for(let s=0;s<a;s++){let l=s%2===0;for(let i=0;i<o;i++){let c=l?i:o-1-i,p=`${c},${s}`;if(!r.has(p))r.add(p),yield{x:c,y:s};if(Math.random()>0.7){let n=Math.max(0,Math.min(o-1,c+(Math.random()>0.5?1:-1))),g=Math.max(0,Math.min(a-1,s+(Math.random()>0.65?1:0))),t=`${n},${g}`;if(!r.has(t))r.add(t),yield{x:n,y:g}}}}for(let s=0;s<a;s++)for(let l=0;l<o;l++){let i=`${l},${s}`;if(r.has(i))continue;yield{x:l,y:s}}break}case"HUMAN_OVERLAP_SWEEPS":{let r=[],s=Math.random()*Math.PI*2;for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=Math.sin((i+l)*0.42+s)*2.2,p=Math.cos((i-l)*0.3+s)*1.4;r.push({point:{x:i,y:l},score:l+c+p+(Math.random()-0.5)*3.4})}r.sort((l,i)=>l.score-i.score);for(let l of r)yield l.point;break}case"HUMAN_WOBBLE_DRIFT":{let r=[],s=o/2,l=a/2;for(let i=0;i<a;i++)for(let c=0;c<o;c++){let p=Math.hypot(c-s,i-l)*0.25,n=Math.sin((c+1)*0.9)*1.8+Math.cos((i+1)*1.1)*1.8+Math.sin((c+i)*0.35)*1.4;r.push({point:{x:c,y:i},score:p+n+(Math.random()-0.5)*2.8})}r.sort((i,c)=>i.score-c.score);for(let i of r)yield i.point;break}case"HUMAN_GAP_RECOVERY":{let r=new Set,s=[];for(let l=0;l<a;l++)for(let i=0;i<o;i++){if(Math.random()>0.87){s.push({x:i,y:l});continue}r.add(`${i},${l}`),yield{x:i,y:l}}s.sort((l,i)=>Math.hypot(l.x-o/2,l.y-a/2)-Math.hypot(i.x-o/2,i.y-a/2));for(let l of s){let i=`${l.x},${l.y}`;if(r.has(i))continue;r.add(i),yield l}break}case"HUMAN_STAIRCASE":{let r=new Set,s=o+a-1;for(let l=0;l<s;l++){let i=Math.max(0,l-o+1),c=Math.min(a-1,l);for(let p=i;p<=c;p++){let n=l-p,g=[{x:n,y:p},{x:n+(Math.random()>0.5?1:-1),y:p},{x:n,y:p+(Math.random()>0.5?1:-1)}];for(let t of g){if(t.x<0||t.x>=o||t.y<0||t.y>=a)continue;let d=`${t.x},${t.y}`;if(r.has(d))continue;r.add(d),yield t}}}for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=`${i},${l}`;if(r.has(c))continue;yield{x:i,y:l}}break}case"HUMAN_EDGE_HUGGER":{let r=[];for(let s=0;s<a;s++)for(let l=0;l<o;l++){let i=Math.min(l,s,o-1-l,a-1-s);r.push({point:{x:l,y:s},score:i*3.5+(Math.random()-0.5)*5.5})}r.sort((s,l)=>s.score-l.score);for(let s of r)yield s.point;break}case"HUMAN_BLOBS":{let r=new Set,s=o*a;while(r.size<s){let l=Math.floor(Math.random()*o),i=Math.floor(Math.random()*a),c=1+Math.floor(Math.random()*4);for(let p=i-c;p<=i+c;p++)for(let n=l-c;n<=l+c;n++){if(n<0||n>=o||p<0||p>=a)continue;let g=Math.atan2(p-i,n-l),t=c+Math.sin(g*3+Math.random())*0.8;if(Math.hypot(n-l,p-i)>t)continue;let d=`${n},${p}`;if(r.has(d))continue;r.add(d),yield{x:n,y:p}}}break}case"HUMAN_BACKTRACK":{let r=new Set,s=[];for(let l=0;l<a;l++)for(let i=0;i<o;i++)s.push({x:i,y:l});s.sort((l,i)=>l.y-i.y+(Math.random()-0.5)*2.2+(l.x-i.x)*0.04);for(let l=0;l<s.length;l++){let i=s[l],c=`${i.x},${i.y}`;if(r.has(c))continue;if(r.add(c),yield i,l>1&&Math.random()>0.74){let p=s[l-1],n=`${p.x},${p.y}`;if(!r.has(n))r.add(n),yield p}}for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=`${i},${l}`;if(r.has(c))continue;yield{x:i,y:l}}break}case"HUMAN_SHAKY_DIAGONAL":{let r=[];for(let s=0;s<a;s++)for(let l=0;l<o;l++){let i=Math.abs(l-s)*0.6,c=Math.sin(l*1.4+s*0.8)*1.8+Math.cos(s*1.1+l*0.5)*1.5;r.push({point:{x:l,y:s},score:i+c+(Math.random()-0.5)*3.2})}r.sort((s,l)=>s.score-l.score);for(let s of r)yield s.point;break}case"HUMAN_LATE_FIXES":{let r=[],s=[];for(let l=0;l<a;l++)for(let i=0;i<o;i++)if(Math.random()>0.9)s.push({x:i,y:l});else r.push({x:i,y:l});r.sort((l,i)=>l.y-i.y+(Math.random()-0.5)*1.5+(Math.random()>0.85?l.x-i.x:0)),s.sort((l,i)=>Math.hypot(i.x-o/2,i.y-a/2)-Math.hypot(l.x-o/2,l.y-a/2)),yield*r,yield*s;break}case"DIAGONAL_BRUSH":{for(let r=0;r<o+a-1;r++){let s=r%2===0,l=[],i=Math.max(0,r-o+1),c=Math.min(a-1,r);for(let p=i;p<=c;p++){let n=r-p;if(n>=0&&n<o)l.push({x:n,y:p})}if(Math.random()>0.55)l.reverse();if(s)for(let p=l.length-1;p>=0;p--)yield l[p];else yield*l}break}case"BRUSH_STROKES":{let r=Array.from({length:a},()=>Array(o).fill(!1)),s=[{x:1,y:0},{x:-1,y:0},{x:0,y:1},{x:0,y:-1},{x:1,y:1},{x:1,y:-1},{x:-1,y:1},{x:-1,y:-1}],l=(p,n)=>p>=0&&p<o&&n>=0&&n<a,i=0,c=o*a;for(let p=0;p<c*6&&i<c;p++){let n=Math.floor(Math.random()*o),g=Math.floor(Math.random()*a),t=s[Math.floor(Math.random()*s.length)],d=3+Math.floor(Math.random()*16);for(let u=0;u<d;u++){if(!l(n,g))break;if(!r[g][n])r[g][n]=!0,i++,yield{x:n,y:g};if(Math.random()>0.72)t=s[Math.floor(Math.random()*s.length)];n+=t.x,g+=t.y}}for(let p=0;p<a;p++)for(let n=0;n<o;n++)if(!r[p][n])yield{x:n,y:p};break}case"SPIRAL_FROM_CENTER":case"SPIRAL_TO_CENTER":{let r=new Set,s=o*a,l=Math.floor(o/2),i=Math.floor(a/2),c=[[1,0],[0,1],[-1,0],[0,-1]],p=0,n=1,g=(d,u)=>d>=0&&d<o&&u>=0&&u<a,t=function*(){let d=0;while(d<s){for(let u=0;u<2;u++){for(let e=0;e<n;e++){if(g(l,i)){let w=`${l},${i}`;if(!r.has(w)){if(r.add(w),yield{x:l,y:i},d++,d>=s)return}}l+=c[p][0],i+=c[p][1]}p=(p+1)%4}n++}};if(this.strategy==="SPIRAL_FROM_CENTER")yield*t();else{let d=[...t()];for(let u=d.length-1;u>=0;u--)yield d[u]}break}case"SCRIBBLE":{let r=new Set,s=o*a,l=Math.floor(o/2),i=Math.floor(a/2);for(let c=0;r.size<s&&c<s*24;c++){let p=`${l},${i}`;if(!r.has(p))r.add(p),yield{x:l,y:i};if(l+=Math.floor(Math.random()*3)-1,i+=Math.floor(Math.random()*3)-1,l<0||l>=o||i<0||i>=a)l=Math.floor(Math.random()*o),i=Math.floor(Math.random()*a)}for(let c=0;c<a;c++)for(let p=0;p<o;p++){let n=`${p},${c}`;if(r.has(n))continue;r.add(n),yield{x:p,y:c}}break}case"CROSSHATCH":{let r=[];for(let i=0;i<o+a-1;i++)for(let c=Math.max(0,i-o+1);c<=Math.min(a-1,i);c++){let p=i-c;r.push({x:p,y:c})}let s=[];for(let i=-a+1;i<o;i++)for(let c=0;c<a;c++){let p=c+i;if(p>=0&&p<o)s.push({x:p,y:c})}let l=new Set;for(let i of[...r,...s]){let c=`${i.x},${i.y}`;if(l.has(c))continue;l.add(c),yield i}break}case"WAVE_SWEEP":{let r=new Set;for(let s=0;s<o;s++){let i=(Math.sin(s/Math.max(1,o-1)*Math.PI*4)+1)*0.5*(a-1)|0;for(let c=0;c<a;c++){let p=i+c,n=i-c;for(let g of[p,n]){if(g<0||g>=a)continue;let t=`${s},${g}`;if(r.has(t))continue;r.add(t),yield{x:s,y:g}}}}break}case"SCATTERED_LINES":{let r=new Set,s=o*a;for(let l=0;r.size<s&&l<s*14;l++){let i=Math.floor(Math.random()*o),c=Math.floor(Math.random()*a),p=Math.random()*Math.PI*2,n=Math.round(Math.cos(p)),g=Math.round(Math.sin(p)),t=6+Math.floor(Math.random()*28);for(let d=0;d<t;d++){if(i<0||i>=o||c<0||c>=a)break;let u=`${i},${c}`;if(!r.has(u))r.add(u),yield{x:i,y:c};i+=n,c+=g}}for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=`${i},${l}`;if(r.has(c))continue;r.add(c),yield{x:i,y:l}}break}case"CONTOUR_JITTER":{let r=new Set;for(let s=0;s<Math.ceil(Math.min(o,a)/2);s++){let l=[],i=s,c=s,p=o-s-1,n=a-s-1;for(let g=i;g<=p;g++)l.push({x:g,y:c});for(let g=c+1;g<=n;g++)l.push({x:p,y:g});for(let g=p-1;g>=i;g--)l.push({x:g,y:n});for(let g=n-1;g>c;g--)l.push({x:i,y:g});for(let g=l.length-1;g>0;g--){let t=Math.floor(Math.random()*(g+1)),d=l[g];l[g]=l[t],l[t]=d}for(let g of l){let t=`${g.x},${g.y}`;if(r.has(t))continue;r.add(t),yield g}}break}case"SPIRAL_WOBBLE":{let r=new Set,s=o/2,l=a/2,i=Math.hypot(s,l);for(let c=0;r.size<o*a&&c<o*a*9;c++){let p=c/(o*a*9)*i,n=c*0.31+Math.sin(c*0.07)*0.7,g=Math.round(s+Math.cos(n)*p),t=Math.round(l+Math.sin(n)*p);if(g<0||g>=o||t<0||t>=a)continue;let d=`${g},${t}`;if(r.has(d))continue;r.add(d),yield{x:g,y:t}}for(let c=0;c<a;c++)for(let p=0;p<o;p++){let n=`${p},${c}`;if(r.has(n))continue;yield{x:p,y:c}}break}case"CLUSTER_BURSTS":{let r=new Set,s=o*a;for(let l=0;r.size<s&&l<s*12;l++){let i=Math.floor(Math.random()*o),c=Math.floor(Math.random()*a),p=2+Math.floor(Math.random()*10);for(let n=c-p;n<=c+p;n++)for(let g=i-p;g<=i+p;g++){if(g<0||g>=o||n<0||n>=a)continue;if(Math.hypot(g-i,n-c)>p)continue;let t=`${g},${n}`;if(r.has(t))continue;r.add(t),yield{x:g,y:n}}}for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=`${i},${l}`;if(r.has(c))continue;r.add(c),yield{x:i,y:l}}break}case"ORBITAL":{let r=new Set,s=(o-1)/2,l=(a-1)/2,i=Math.ceil(Math.max(s,l));for(let c=0;c<=i;c++){let p=Math.max(16,Math.ceil(2*Math.PI*Math.max(1,c)*2));for(let n=0;n<p;n++){let g=n/p*Math.PI*2+(c%2?0.3:-0.3),t=Math.round(s+Math.cos(g)*c),d=Math.round(l+Math.sin(g)*c);if(t<0||t>=o||d<0||d>=a)continue;let u=`${t},${d}`;if(r.has(u))continue;r.add(u),yield{x:t,y:d}}}for(let c=0;c<a;c++)for(let p=0;p<o;p++){let n=`${p},${c}`;if(r.has(n))continue;yield{x:p,y:c}}break}case"FLOW_FIELD":{let r=new Set,s=o*a;for(let l=0;r.size<s&&l<s*18;l++){let i=Math.floor(Math.random()*o),c=Math.floor(Math.random()*a);for(let p=0;p<120;p++){if(i<0||i>=o||c<0||c>=a)break;let n=`${i},${c}`;if(!r.has(n))r.add(n),yield{x:i,y:c};let g=Math.sin(i*0.09)*1.8+Math.cos(c*0.08)*1.6+Math.sin((i+c)*0.05);i+=Math.round(Math.cos(g)),c+=Math.round(Math.sin(g))}}for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=`${i},${l}`;if(r.has(c))continue;r.add(c),yield{x:i,y:l}}break}case"EDGE_IN":{let r=new Set,s=Math.ceil(Math.min(o,a)/2);for(let l=0;l<s;l++){let i=l,c=o-1-l,p=l,n=a-1-l;for(let g=i;g<=c;g++)for(let t of[p,n]){let d=`${g},${t}`;if(r.has(d))continue;r.add(d),yield{x:g,y:t}}for(let g=p+1;g<=n-1;g++)for(let t of[i,c]){let d=`${t},${g}`;if(r.has(d))continue;r.add(d),yield{x:t,y:g}}}break}}}moveStart(o){if(o.button!==0)return;if(o.preventDefault(),o.stopPropagation(),!this.lock)this.moveInfo={globalX:this.position.globalX,globalY:this.position.globalY,clientX:o.clientX,clientY:o.clientY}}moveStop(){if(this.moveInfo)this.moveInfo=void 0,this.position.updateAnchor(),this.pixels.update(),this.updateColors(),k(this.bot)}move(o){if(!this.moveInfo)return;let a=Math.round((o.clientX-this.moveInfo.clientX)/this.position.pixelSize),r=Math.round((o.clientY-this.moveInfo.clientY)/this.position.pixelSize);if(this.moveInfo.globalX!==void 0){if(this.position.globalX=a+this.moveInfo.globalX,this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,this.moveInfo.width-a)}else if(this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,a+this.moveInfo.width);if(this.moveInfo.globalY!==void 0){if(this.position.globalY=r+this.moveInfo.globalY,this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,this.moveInfo.height-r)}else if(this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,r+this.moveInfo.height);this.update(),k(this.bot)}resizeStart(o){if(this.lock||o.button!==0)return;o.preventDefault(),o.stopPropagation(),this.moveInfo={clientX:o.clientX,clientY:o.clientY};let a=o.target;if(a.classList.contains("n"))this.moveInfo.height=this.pixels.height,this.moveInfo.globalY=this.position.globalY;if(a.classList.contains("e"))this.moveInfo.width=this.pixels.width;if(a.classList.contains("s"))this.moveInfo.height=this.pixels.height;if(a.classList.contains("w"))this.moveInfo.width=this.pixels.width,this.moveInfo.globalX=this.position.globalX}export(){let o=document.createElement("a");document.body.append(o),o.href=URL.createObjectURL(new Blob([JSON.stringify(this.toJSON())],{type:"application/json"})),o.download=`${this.pixels.width}x${this.pixels.height}.${Y}`,o.click(),URL.revokeObjectURL(o.href),o.href=this.pixels.canvas.toDataURL("image/webp",1),o.download=`${this.pixels.width}x${this.pixels.height}.webp`,o.click(),URL.revokeObjectURL(o.href),o.remove()}}function fo(){let o=localStorage.getItem("kglacer-macro:shield-config");if(!o)return!1;try{return JSON.parse(o).enabled!==!1}catch{return!1}}function No(o){localStorage.setItem("kglacer-macro:shield-config",JSON.stringify({enabled:o}))}function ta(o){let a=`${o?.host??""} ${o?.username??""}`.toLowerCase(),r=/(mx|mex|mexico)/.test(a)?"MX":"AUTO";localStorage.setItem("__afm_proxy_hint",r)}function Do(o){if(!fo())return;if(document.getElementById("kgm-shield-full"))return;ta(o);let a=document.createElement("script");a.id="kgm-shield-full",a.textContent=`// ==UserScript==
...
`;var Qo="kglacer-macro:overlay-hidden",Ko="kglacer-macro:auto-farm-config",Go="kglacer-macro:auto-overlay-config",qo="kglacer-macro:proxy-config",Wo="__afm_proxy_hint",ua=["https://api.ipify.org?format=json","https://icanhazip.com"],ea="https://raw.githubusercontent.com/robgallardof/kglacer-macro/refs/heads/main/src/img/logo.svg",wa="https://pepoafonso.github.io/color_converter_wplace/es/index.html",Co="https://wplace.samuelscheit.com/",Zo="https://wplace.eralyon.net/",Bo="v69.051";class eo extends X{bot;element=document.createElement("div");get status(){return this.$status.innerHTML}set status(o){this.$status.innerHTML=o}get open(){return this.element.classList.contains("wopen")}set open(o){if(o)this.element.classList.add("wopen");else this.element.classList.remove("wopen")}$settings;$status;$openConfig;$topbar;$draw;$drawAndPaint;$addImage;$captureTemplate;$toolColorConverter;$toolSamuelArchive;$toolEralyonArchive;$toggleOverlay;$autofarmConfig;$autofarmStart;$autofarmStop;$autofarmStatus;$autoOverlayConfig;$autoOverlayStart;$autoOverlayStop;$autoOverlayStatus;$strategy;$progressLine;$progressText;$images;$wopenButton;$widgetLogo;activeImageIndex=-1;autoFarmIntervalId;autoFarmConfig;autoFarmTickRunning=!1;autoFarmNextTickAt;autoOverlayIntervalId;autoOverlayConfig;autoOverlayTickRunning=!1;autoOverlayNextTickAt;statusRefreshIntervalId;challengeWatcherObserver;challengeWatcherRunning=!1;constructor(o){super();this.bot=o;this.element.classList.add("wwidget"),this.element.innerHTML=Uo,N(this.element),document.body.append(this.element),this.populateElementsWithSelector(this.element,{$wopenButton:".wopen-button",$widgetLogo:".widget-logo",$settings:".wform",$status:".wstatus",$openConfig:".open-config",$topbar:".wtopbar",$draw:".draw",$drawAndPaint:".draw-and-paint",$addImage:".add-image",$captureTemplate:".capture-template",$toolColorConverter:".tool-color-converter",$toolSamuelArchive:".tool-samuel-archive",$toolEralyonArchive:".tool-eralyon-archive",$toggleOverlay:".toggle-overlay",$autofarmConfig:".autofarm-config",$autofarmStart:".autofarm-start",$autofarmStop:".autofarm-stop",$autofarmStatus:".autofarm-status",$autoOverlayConfig:".autooverlay-config",$autoOverlayStart:".autooverlay-start",$autoOverlayStop:".autooverlay-stop",$autoOverlayStatus:".autooverlay-status",$strategy:".strategy",$progressLine:".wprogress div",$progressText:".wprogress span",$images:".images"}),this.$widgetLogo.src=ea,this.$wopenButton.addEventListener("click",()=>this.open=!this.open),this.$draw.addEventListener("click",()=>this.bot.draw()),this.$drawAndPaint.addEventListener("click",()=>{this.drawAndClickPaintWhenReady()}),this.$addImage.addEventListener("click",()=>this.addImage()),this.$openConfig.addEventListener("click",()=>{this.openSettingsModal()}),this.$captureTemplate.addEventListener("click",()=>{this.captureTemplate()}),this.$toolColorConverter.addEventListener("click",()=>{this.openExternalTool("colorConverter")}),this.$toolSamuelArchive.addEventListener("click",()=>{this.openExternalTool("samuelArchive")}),this.$toolEralyonArchive.addEventListener("click",()=>{this.openExternalTool("eralyonArchive")}),this.$toggleOverlay.addEventListener("click",()=>{this.toggleOverlay()}),this.$autofarmConfig.addEventListener("click",()=>{this.openAutoFarmModal()}),this.$autofarmStart.addEventListener("click",()=>{this.startAutoFarm()}),this.$autofarmStop.addEventListener("click",()=>{this.stopAutoFarm()}),this.$autoOverlayConfig.addEventListener("click",()=>{this.openAutoOverlayModal()}),this.$autoOverlayStart.addEventListener("click",()=>{this.startAutoOverlay()}),this.$autoOverlayStop.addEventListener("click",()=>{this.stopAutoOverlay()}),this.$strategy.addEventListener("change",()=>{this.bot.strategy=this.$strategy.value}),this.registerEvent(document,"keydown",this.handleKeyboard.bind(this),{passive:!1}),this.update(),this.syncOverlayVisibilityFromStorage(),this.loadAutoFarmConfigFromStorage(),this.loadAutoOverlayConfigFromStorage(),this.refreshAutoFarmStatusText(),this.refreshAutoOverlayStatusText(),this.statusRefreshIntervalId=window.setInterval(()=>{this.refreshAutoFarmStatusText(),this.refreshAutoOverlayStatusText(),this.refreshProgress()},1000),this.open=!0,console.log("[KGM][Widget] Widget mounted and opened")}startChallengeWatcher(){let o=()=>{if(!this.isChallengeBlockingPaint())return;if(this.challengeWatcherRunning)return;this.challengeWatcherRunning=!0,this.status=`⌛ ${f("taskWaitingChallengeResolve")}`,this.waitForChallengeToResolve().finally(()=>{this.challengeWatcherRunning=!1})};this.challengeWatcherObserver=new MutationObserver(()=>{o()}),this.challengeWatcherObserver.observe(document.documentElement,{childList:!0,subtree:!0,attributes:!0,attributeFilter:["open","style","class","value","aria-hidden"]});let a=window.setInterval(o,750);this.runOnDestroy.push(()=>{this.challengeWatcherObserver?.disconnect(),clearInterval(a)}),o()}addImage(){return console.log("[KGM][Widget] Add image flow started"),this.setDisabled("add-image",!0),this.run(f("taskAddingImage"),async()=>{await this.bot.updateColors();let o=document.createElement("input");o.type="file",o.accept=`image/*,.${Y},.wplace`,o.click(),await Q(o,["change"],["cancel","error"]);let a=o.files?.[0];if(!a)throw new uo(this.bot);console.log("[KGM][Widget] File selected",{name:a.name,size:a.size,type:a.type});let r;if(a.name.endsWith(`.${Y}`))r=await j.fromJSON(this.bot,JSON.parse(await a.text()));else if(a.name.endsWith(".wplace")){let s=JSON.parse(await a.text());if(!s.image?.dataUrl)throw new Error("Invalid .wplace file: image.dataUrl missing");let l=new Image;if(l.src=s.image.dataUrl,await Q(l,["load"],["error"]),await this.waitForStableViewportProjection(),r=new j(this.bot,S.fromScreenPosition(this.bot,this.defaultImageScreenPosition()),new K(this.bot,l)),typeof s.opacity==="number")r.opacity=Math.max(0,Math.min(1,s.opacity))}else{let s=new FileReader;s.readAsDataURL(a),await Q(s,["load"],["error"]);let l=await this.compressImageBeforeLoad(s.result),i=new Image;i.src=l,await Q(i,["load"],["error"]),await this.waitForStableViewportProjection(),r=new j(this.bot,S.fromScreenPosition(this.bot,this.defaultImageScreenPosition()),new K(this.bot,i))}this.bot.images.push(r),console.log("[KGM][Widget] Image instance added",{images:this.bot.images.length}),await this.bot.readMap(),r.updateTasks(),k(this.bot,!0),this.bot.updateTasks(),this.update(),window.setTimeout(()=>{globalThis.location.reload()},120)},()=>{this.setDisabled("add-image",!1)})}captureTemplate(){return this.setDisabled("capture-template",!0),this.run(f("taskCapturingMapImage"),async()=>{let o=await this.resolveCaptureBounds(),{minGlobalX:a,minGlobalY:r,maxGlobalX:s,maxGlobalY:l}=o,i=document.createElement("canvas");i.width=Math.max(1,s-a+1),i.height=Math.max(1,l-r+1);let c=i.getContext("2d");if(!c)throw new Error("Capture context unavailable");c.imageSmoothingEnabled=!1;let p=Math.floor(a/P),n=Math.floor(r/P),g=Math.floor(s/P),t=Math.floor(l/P),d=(g-p+1)*(t-n+1),u=0;for(let w=p;w<=g;w++)for(let z=n;z<=t;z++){this.status=`⌛ ${f("taskReadingTiles")} [${++u}/${d}]`;let m=await this.loadTileImage(w,z),h=w*P,b=z*P,A=Math.max(a,h),M=Math.min(s,h+P-1),J=Math.max(r,b),W=Math.min(l,b+P-1),v=A-h,x=J-b,V=M-A+1,U=W-J+1,I=A-a,$=J-r;c.drawImage(m,v,x,V,U,I,$,V,U)}let e=Date.now();await this.downloadCapture(i,"png",e)},()=>{this.setDisabled("capture-template",!1)})}async downloadCapture(o,a,r){let s=a==="webp"?"image/webp":"image/png",l=await new Promise((p,n)=>{o.toBlob((g)=>{if(!g){n(new Error(`Failed to create ${a.toUpperCase()} capture file`));return}p(g)},s)}),i=URL.createObjectURL(l),c=document.createElement("a");c.href=i,c.download=`wplace-capture-${r}.${a}`,c.click(),URL.revokeObjectURL(i)}async loadTileImage(o,a){let r;for(let s=1;s<=3;s++)try{let l=new Image;return l.crossOrigin="anonymous",l.referrerPolicy="no-referrer",l.src=`https://backend.wplace.live/files/s0/tiles/${o}/${a}.png?ts=${Date.now()}-${s}`,await Q(l,["load"],["error"]),l}catch(l){if(r=l,s<3)await new Promise((i)=>setTimeout(i,s*200))}throw r instanceof Error?r:new Error(`Tile fetch failed (${o}/${a})`)}async resolveCaptureBounds(){return this.selectCaptureBounds()}selectCaptureBounds(){return new Promise((o,a)=>{let r=document.createElement("div");r.className="kgm-capture-overlay",r.innerHTML=`<div class="kgm-capture-hint">${f("captureHintSelectArea")}: A → B</div><div class="kgm-capture-box"></div>`;let s=r.querySelector(".kgm-capture-box");document.body.append(r);let l,i,c=()=>{window.removeEventListener("keydown",d,!0),r.removeEventListener("pointermove",g),r.removeEventListener("pointerdown",t),r.remove()},p=(u)=>{let e=Math.min(l.x,u.x),w=Math.min(l.y,u.y),z=Math.abs(l.x-u.x)+1,m=Math.abs(l.y-u.y)+1;return{left:e,top:w,width:z,height:m}},n=(u)=>{let{left:e,top:w,width:z,height:m}=p(u);s.style.left=`${e}px`,s.style.top=`${w}px`,s.style.width=`${z}px`,s.style.height=`${m}px`},g=(u)=>{if(!l)return;n({x:u.clientX,y:u.clientY})},t=(u)=>{if(u.preventDefault(),!l){l={x:u.clientX,y:u.clientY};let A=S.fromScreenPosition(this.bot,l);i={x:A.globalX,y:A.globalY},n(l);return}let e={x:u.clientX,y:u.clientY},w=S.fromScreenPosition(this.bot,e);if(c(),!i){a(new Error("Capture anchor point unavailable"));return}let z=Math.min(i.x,w.globalX),m=Math.min(i.y,w.globalY),h=Math.max(i.x,w.globalX),b=Math.max(i.y,w.globalY);if(h-z<1||b-m<1){a(new Error("Capture area too small"));return}o({minGlobalX:z,minGlobalY:m,maxGlobalX:h,maxGlobalY:b})},d=(u)=>{if(u.key!=="Escape")return;c(),a(new Error("Capture cancelled"))};window.addEventListener("keydown",d,!0),r.addEventListener("pointermove",g),r.addEventListener("pointerdown",t)})}defaultImageScreenPosition(){let o=Math.round(this.element.getBoundingClientRect().width);return{x:Math.max(256,o),y:32}}async compressImageBeforeLoad(o){let a=new Image;if(a.src=o,await Q(a,["load"],["error"]),!(a.naturalWidth*a.naturalHeight>3000000||o.length>3000000))return o;let s=document.createElement("canvas");s.width=a.naturalWidth,s.height=a.naturalHeight;let l=s.getContext("2d");if(!l)return o;return l.drawImage(a,0,0),s.toDataURL("image/png")}async waitForStableViewportProjection(){let o=this.defaultImageScreenPosition(),a=0,r;for(let s=0;s<45;s++){await new Promise((g)=>requestAnimationFrame(()=>{g()}));let{anchorScreenPosition:{x:l,y:i},pixelSize:c}=this.bot.findAnchorsForScreen(o);if(!Number.isFinite(c)||c<=0){a=0;continue}let p={anchorX:l,anchorY:i,pixelSize:c};if(!r){r=p,a=1;continue}if(Math.abs(p.anchorX-r.anchorX)+Math.abs(p.anchorY-r.anchorY)+Math.abs(p.pixelSize-r.pixelSize)<0.0012)a++;else a=0;if(r=p,a>=3)return}}update(){this.$strategy.value=this.bot.strategy,this.refreshProgress(),this.$images.innerHTML="";let o=document.createDocumentFragment();for(let a=0;a<this.bot.images.length;a++){let r=this.bot.images[a],s=document.createElement("div");o.append(s),s.className="image",s.innerHTML=`<button class="preview" title="View preview">
...
</form>`,document.body.append(o),N(o);let a=o.querySelector(".settings-locale");a.value=O(),o.querySelector(".script-update").addEventListener("click",()=>{globalThis.open("https://github.com/robgallardof/kglacer-macro/raw/refs/heads/main/dist.user.js","_blank","noopener,noreferrer")}),a.addEventListener("change",()=>{this.applyLocaleToUI(a.value),N(o)});let r=JSON.parse(localStorage.getItem(qo)??"{}"),s=o.querySelector(".proxy-enabled"),l=o.querySelector(".proxy-host"),i=o.querySelector(".proxy-port"),c=o.querySelector(".proxy-user"),p=o.querySelector(".proxy-pass"),n=o.querySelector(".shield-enabled"),g=o.querySelector(".proxy-settings"),t=o.querySelector(".shield-settings"),d=o.querySelector(".shield-controls"),u=o.querySelector(".proxy-test"),e=o.querySelector(".proxy-test-output"),w=o.querySelector(".public-ip-value"),z=o.querySelector(".public-ip-route");s.checked=Boolean(r.enabled),n.checked=fo(),g.open=s.checked,t.open=n.checked,this.renderShieldControls(d),l.value=r.host??"",i.value=r.port??"",c.value=r.username??"",p.value=r.password??"";let m=()=>{let b=s.checked,A=l.value.trim(),M=i.value.trim();localStorage.setItem(qo,JSON.stringify({enabled:b,host:A,port:M,username:c.value.trim(),password:p.value})),localStorage.setItem(Wo,b&&A&&M?`${A}:${M}`:"DIRECT/SHIELD")},h=async()=>{if(w)w.textContent=f("publicIpChecking");if(z)z.textContent=this.getPublicIpRouteLabel({enabled:s.checked,host:l.value.trim(),port:i.value.trim()});let b=await this.fetchPublicIp();if(w)w.textContent=b??f("publicIpUnavailable")};for(let b of[s,l,i,c,p])b.addEventListener("change",()=>{m(),h()});s.addEventListener("change",()=>{g.open=s.checked}),m(),h(),u.addEventListener("click",async()=>{m();let b=l.value.trim(),A=i.value.trim();if(u.disabled=!0,e)e.innerHTML=`<div class="pending">⏳ ${f("proxyTesting")}</div>`;let M=await this.testProxyConnection(b,A);if(await h(),e)e.innerHTML=`<div class="${M?"ok":"fail"}">${M?"✅":"❌"} ${M?f("proxyOk"):f("proxyFail")}</div>`;else alert(M?f("proxyOk"):f("proxyFail"));u.disabled=!1}),n.addEventListener("change",()=>{t.open=n.checked,this.renderShieldControls(d),No(n.checked),window.setTimeout(()=>{location.reload()},120)}),o.querySelector(".modal-close").onclick=()=>{o.close(),o.remove()},o.addEventListener("close",()=>{o.remove()}),o.showModal()}renderShieldControls(o){let l={navigator:f("shieldFeatureNavigator"),userAgentData:f("shieldFeatureUaData"),screen:f("shieldFeatureScreen"),timezone:f("shieldFeatureTimezone"),canvas:f("shieldFeatureCanvas"),webgl:f("shieldFeatureWebgl"),audio:f("shieldFeatureAudio"),plugins:f("shieldFeaturePlugins"),mediaDevices:f("shieldFeatureMediaDevices"),storageEstimate:f("shieldFeatureStorage"),battery:f("shieldFeatureBattery"),speechSynthesis:f("shieldFeatureSpeech"),fonts:f("shieldFeatureFonts"),matchMedia:f("shieldFeatureMatchMedia"),sharedArrayBuffer:f("shieldFeatureSharedArrayBuffer")},i=this.readStorageJson("__afm_profile",null),c="__afm_profile_choices",p=Number(localStorage.getItem("__afm_profile_expiry")??"0"),n=this.readStorageJson("__afm_settings",{}),g=this.readStorageJson("__afm_profile_choices",[]),d={...Object.fromEntries(Object.keys(l).map((m)=>[m,!0])),...n},u=p>0?new Date(p).toLocaleString():"—",e=i?.id??"Auto",w=g.map((m)=>`<option value="${m.id}" ${m.id===e?"selected":""}>${m.id}</option>`).join(""),z=Object.entries(l).map(([m,h])=>`<label class="kgm-switch-row"><span>${h}</span><span class="kgm-switch"><input type="checkbox" data-shield-key="${m}" ${d[m]?"checked":""}/><span class="kgm-switch-slider" aria-hidden="true"></span></span></label>`).join("");o.innerHTML=`<div class="shield-profile-row"><label>${f("shieldProfile")}</label><select class="shield-profile-select"><option value="">${f("shieldProfileAuto")}</option>${w}</select></div><div class="wp shield-expiry-line">${f("shieldExpires")}: <strong>${u}</strong></div><div class="widget-actions kgm-button-grid"><button type="button" class="challenge-button shield-refresh-profile"><i class="fa-solid fa-rotate"></i><span>${f("shieldRefreshProfile")}</span></button><button type="button" class="challenge-button shield-checker"><i class="fa-solid fa-shield-check"></i><span>${f("shieldChecker")}</span></button><button type="button" class="challenge-button shield-info"><i class="fa-solid fa-circle-info"></i><span>${f("shieldInfo")}</span></button></div><div class="shield-checker-output" aria-live="polite"></div><div class="shield-control-grid">${z}</div>`,o.querySelectorAll("input[data-shield-key]").forEach((m)=>{m.addEventListener("change",()=>{let h=m.dataset.shieldKey;d[h]=m.checked,localStorage.setItem("__afm_settings",JSON.stringify(d)),window.setTimeout(()=>{location.reload()},120)})}),o.querySelector(".shield-profile-select")?.addEventListener("change",(m)=>{let h=m.currentTarget.value;if(!h)localStorage.removeItem("__afm_profile");else{let b=g.find((A)=>A.id===h);localStorage.setItem("__afm_profile",JSON.stringify(b??{id:h}))}location.reload()}),o.querySelector(".shield-checker")?.addEventListener("click",()=>{let m=o.querySelector(".shield-checker-output");if(!m)return;let h=this.runShieldChecker();m.innerHTML=h.map((b)=>`<div class="${b.ok?"ok":"fail"}">${b.ok?"✅":"❌"} ${b.label}</div>`).join("")}),o.querySelector(".shield-info")?.addEventListener("click",()=>{this.openShieldInfoModal()}),o.querySelector(".shield-refresh-profile")?.addEventListener("click",()=>{localStorage.removeItem("__afm_profile"),localStorage.removeItem("__afm_profile_expiry"),location.reload()})}getShieldInfo(){let o=this.readStorageJson("__afm_profile",null),a=this.readStorageJson("__afm_settings",{}),r=this.readStorageJson("__afm_profile_choices",[]);return{injectedInfo:globalThis.__kgmShieldInfo,profile:o,settings:a,choices:r,expiry:Number(localStorage.getItem("__afm_profile_expiry")??"0"),enabled:localStorage.getItem("__afm_enabled")!=="false",proxyHint:localStorage.getItem(Wo)??"AUTO"}}readStorageJson(o,a){try{let r=localStorage.getItem(o);if(!r)return a;return JSON.parse(r)}catch{return a}}getPublicIpRouteLabel(o){if(o.enabled&&o.host&&o.port)return`${f("publicIpProxyRoute")} (${o.host}:${o.port})`;return f("publicIpShieldRoute")}async fetchPublicIp(){for(let o of ua)try{let a=await fetch(o,{cache:"no-store"});if(!a.ok)continue;if((a.headers.get("content-type")??"").includes("application/json")){let s=await a.json();if(typeof s.ip==="string"&&s.ip.trim())return s.ip.trim()}else{let s=(await a.text()).trim();if(s)return s}}catch{}return}openShieldInfoModal(){let o=this.getShieldInfo(),a=o.injectedInfo?.profile,r=typeof a==="object"&&a!==null?a:o.profile,s=o.injectedInfo?.settings??o.settings,l=Number(o.injectedInfo?.expiresAt??o.expiry),i=(d,u="—")=>this.stringifyShieldValue(r?.[d],u),c=r?`${i("screenWidth")}×${i("screenHeight")} @${i("devicePixelRatio")}`:"—",p=r?`${i("webglVendor")} / ${i("webglRenderer")}`:"—",n=[[f("shieldInfoInjected"),o.injectedInfo?f("enabled"):f("disabled")],[f("shieldInfoEnabled"),o.enabled?f("enabled"):f("disabled")],[f("shieldProfile"),i("id")],[f("shieldExpires"),l>0?new Date(l).toLocaleString():"—"],[f("shieldInfoBrowser"),this.stringifyShieldValue(o.injectedInfo?.detectedBrowser)],[f("shieldInfoProxyHint"),this.stringifyShieldValue(o.injectedInfo?.proxyHint,o.proxyHint)],[f("publicIpTitle"),f("publicIpChecking")],[f("shieldInfoProfiles"),o.choices.length>0?String(o.choices.length):"—"],["User-Agent",i("userAgent",navigator.userAgent)],["Platform",i("platform",navigator.platform)],["Language",i("language",navigator.language)],["Screen",c],["WebGL",p]],g=Object.entries(s).filter(([,d])=>d).map(([d])=>d).join(", "),t=document.createElement("dialog");t.className="kgm-modal shield-info-dialog",t.innerHTML=`<div class="kgm-modal-head"><strong>${f("shieldInfoTitle")}</strong><button type="button" class="modal-close" aria-label="${f("close")}"><span class="icon">×</span></button></div><div class="shield-info-grid">${n.map(([d,u])=>`<div class="shield-info-card"><span>${this.escapeHtml(d)}</span><strong${d===f("publicIpTitle")?' class="shield-info-public-ip"':""}>${this.escapeHtml(u)}</strong></div>`).join("")}</div><div class="shield-info-modules"><span>${f("shieldInfoModules")}</span><p>${this.escapeHtml(g.length>0?g:"—")}</p></div>`,document.body.append(t),this.fetchPublicIp().then((d)=>{let u=t.querySelector(".shield-info-public-ip");if(u)u.textContent=d??f("publicIpUnavailable")}),t.querySelector(".modal-close").onclick=()=>{t.close(),t.remove()},t.addEventListener("close",()=>{t.remove()}),t.showModal()}stringifyShieldValue(o,a="—"){if(o===void 0||o===null||o==="")return a;if(typeof o==="string"||typeof o==="number"||typeof o==="boolean")return String(o);return JSON.stringify(o)}escapeHtml(o){return String(o).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}async testProxyConnection(o,a){if(!o||!a)return!1;try{return await fetch(`http://${o}:${a}`,{method:"HEAD",mode:"no-cors"}),!0}catch{return!1}}runShieldChecker(){let o=this.getShieldInfo(),a=o.profile,r=o.injectedInfo?.settings,s=typeof r==="object"&&r!==null?r:o.settings,l=Boolean(o.injectedInfo??a);return[{label:f("shieldCheckInjected"),ok:l},{label:f("shieldCheckSettings"),ok:Object.keys(s).length>0},{label:f("shieldCheckProfile"),ok:Boolean(a?.id??o.injectedInfo?.profileId)},{label:f("shieldCheckChoices"),ok:o.choices.length>0},{label:f("shieldCheckNavigator"),ok:navigator.hardwareConcurrency!==0&&typeof navigator.platform==="string"}]}refreshAutoFarmStatusText(){if(!this.autoFarmConfig){this.$autofarmStatus.textContent=f("autoFarmNeedsConfig");return}this.$autofarmStatus.textContent=this.autoFarmIntervalId?`${f("autoFarmRunning")} (${this.formatAutoFarmDelay(this.autoFarmConfig.timerMs)}) · ${this.formatCountdown(this.autoFarmNextTickAt)}`:f("autoFarmStopped")}refreshAutoOverlayStatusText(){if(!this.autoOverlayConfig){this.$autoOverlayStatus.textContent=f("autoOverlayNeedsConfig");return}this.$autoOverlayStatus.textContent=this.autoOverlayIntervalId?`${f("autoOverlayRunning")} (${this.formatAutoFarmDelay(this.autoOverlayConfig.timerMs)}) · ${this.formatCountdown(this.autoOverlayNextTickAt)}`:f("autoOverlayStopped")}formatCountdown(o){if(!o)return"00:00";let a=Math.max(0,o-Date.now()),r=Math.ceil(a/1000),s=Math.floor(r/60),l=r%60;return`next in ${String(s).padStart(2,"0")}:${String(l).padStart(2,"0")}`}formatAutoFarmDelay(o){if(o%3600000===0)return`${o/3600000}h`;if(o%60000===0)return`${o/60000}m`;return`${o/1000}s`}stopAutoFarm(){if(!this.autoFarmIntervalId)return;clearInterval(this.autoFarmIntervalId),this.autoFarmIntervalId=void 0,this.autoFarmNextTickAt=void 0,this.refreshAutoFarmStatusText()}stopAutoOverlay(){if(!this.autoOverlayIntervalId)return;clearInterval(this.autoOverlayIntervalId),this.autoOverlayIntervalId=void 0,this.autoOverlayNextTickAt=void 0,this.refreshAutoOverlayStatusText()}startAutoFarm(){if(!this.autoFarmConfig){this.status=`⚠️ ${f("autoFarmNeedsConfig")}`,this.refreshAutoFarmStatusText();return}this.stopAutoFarm(),this.autoFarmNextTickAt=Date.now()+this.autoFarmConfig.timerMs,this.autoFarmIntervalId=window.setInterval(()=>{this.autoFarmNextTickAt=Date.now()+this.autoFarmConfig.timerMs,this.runAutoFarmCycle()},this.autoFarmConfig.timerMs),this.runAutoFarmCycle(),this.refreshAutoFarmStatusText()}startAutoOverlay(){if(!this.autoOverlayConfig){this.status=`⚠️ ${f("autoOverlayNeedsConfig")}`,this.refreshAutoOverlayStatusText();return}this.stopAutoOverlay(),this.autoOverlayNextTickAt=Date.now()+this.autoOverlayConfig.timerMs,this.autoOverlayIntervalId=window.setInterval(()=>{this.autoOverlayNextTickAt=Date.now()+this.autoOverlayConfig.timerMs,this.runAutoOverlayCycle()},this.autoOverlayConfig.timerMs),this.runAutoOverlayCycle(),this.refreshAutoOverlayStatusText()}async runAutoFarmCycle(){if(!this.autoFarmConfig||this.autoFarmTickRunning)return;this.autoFarmTickRunning=!0;try{if(!await this.bot.drawRandomPixelsBatch(this.autoFarmConfig.pixels,0)){this.status=`⚠️ ${f("autoFarmStopped")}: ${f("autoFarmTransparentUnavailable")}`,this.stopAutoFarm();return}await this.waitAndClickPaintButton()}finally{this.autoFarmTickRunning=!1}}async runAutoOverlayCycle(){if(!this.autoOverlayConfig||this.autoOverlayTickRunning)return;this.autoOverlayTickRunning=!0;try{if(!await this.bot.drawOverlayPixelsBatch(this.autoOverlayConfig.pixels)){this.status=`⚠️ ${f("autoOverlayStopped")}: ${f("autoOverlayNoTasks")}`,this.stopAutoOverlay();return}await this.waitAndClickPaintButton()}finally{this.autoOverlayTickRunning=!1}}saveAutoFarmConfig(o){this.autoFarmConfig=o,localStorage.setItem(Ko,JSON.stringify(o))}saveAutoOverlayConfig(o){this.autoOverlayConfig=o,localStorage.setItem(Go,JSON.stringify(o))}loadAutoFarmConfigFromStorage(){let o=localStorage.getItem(Ko);if(!o)return;try{let a=JSON.parse(o);if(typeof a.value!=="number"||!Number.isFinite(a.value)||a.value<1)return;let r=typeof a.pixels==="number"&&Number.isFinite(a.pixels)&&a.pixels>=1?Math.floor(a.pixels):60,s=a.unit==="hours"||a.unit==="minutes"||a.unit==="seconds"?a.unit:"minutes",l=typeof a.timerMs==="number"&&a.timerMs>0?a.timerMs:s==="hours"?a.value*3600000:s==="minutes"?a.value*60000:a.value*1000;this.autoFarmConfig={value:Math.max(1,Math.floor(a.value)),pixels:r,unit:s,timerMs:l}}catch{return}}loadAutoOverlayConfigFromStorage(){let o=localStorage.getItem(Go);if(!o)return;try{let a=JSON.parse(o);if(typeof a.value!=="number"||!Number.isFinite(a.value)||a.value<1)return;let r=typeof a.pixels==="number"&&Number.isFinite(a.pixels)&&a.pixels>=1?Math.floor(a.pixels):60,s=a.unit==="hours"||a.unit==="minutes"||a.unit==="seconds"?a.unit:"minutes",l=typeof a.timerMs==="number"&&a.timerMs>0?a.timerMs:s==="hours"?a.value*3600000:s==="minutes"?a.value*60000:a.value*1000;this.autoOverlayConfig={value:Math.max(1,Math.floor(a.value)),pixels:r,unit:s,timerMs:l}}catch{return}}openAutoFarmModal(){let o=document.createElement("dialog");o.className="kgm-modal autofarm-dialog";let a=this.autoFarmConfig?.unit??"minutes",r=this.autoFarmConfig?.value??1,s=this.autoFarmConfig?.pixels??60;o.innerHTML=`<form method="dialog" class="autofarm-form">
  <div class="kgm-modal-head">
...
</form>`,document.body.append(o),N(o);let l=o.querySelector(".autofarm-unit");l.value=a;let i=o.querySelector(".autofarm-value"),c=o.querySelector(".autofarm-pixels"),p=()=>{let n=Math.max(1,Number.parseInt(i.value||"1",10));if(l.value==="hours")return n*3600000;if(l.value==="minutes")return n*60000;return n*1000};o.querySelector(".autooverlay-start").onclick=()=>{this.saveAutoOverlayConfig({value:Math.max(1,Number.parseInt(i.value||"1",10)),pixels:Math.max(1,Number.parseInt(c.value||"60",10)),unit:l.value,timerMs:p()}),this.startAutoOverlay(),o.close(),o.remove()},o.querySelector(".autooverlay-stop").onclick=()=>{this.stopAutoOverlay(),o.close(),o.remove()},o.querySelector(".modal-close").onclick=()=>{o.close(),o.remove()},o.addEventListener("close",()=>{o.remove()}),o.showModal()}getCurrentWplaceLocation(){let o=[globalThis.location.search,globalThis.location.hash];for(let n of o){let g=new URLSearchParams(n.replace(/^#/,"").replace(/^\?/,"")),t=Number.parseFloat(g.get("lat")??""),d=Number.parseFloat(g.get("lng")??""),u=Number.parseFloat(g.get("zoom")??"");if(Number.isFinite(t)&&Number.isFinite(d)&&Number.isFinite(u))return{lat:t,lng:d,zoom:u}}let a=/#?\/?(?<zoom>-?\d+(?:\.\d+)?)\/(?<lat>-?\d+(?:\.\d+)?)\/(?<lng>-?\d+(?:\.\d+)?)/.exec(globalThis.location.hash);if(!a?.groups)return;let{lat:r,lng:s,zoom:l}=a.groups;if(!r||!s||!l)return;let i=Number.parseFloat(r),c=Number.parseFloat(s),p=Number.parseFloat(l);if(!Number.isFinite(i)||!Number.isFinite(c)||!Number.isFinite(p))return;return{lat:i,lng:c,zoom:p}}buildExternalToolUrl(o){let a=this.getCurrentWplaceLocation();if(o==="colorConverter")return wa;if(!a){if(o==="samuelArchive")return Co;let s=new URL(Zo);return s.searchParams.set("lat","0.000000"),s.searchParams.set("lng","0.000000"),s.searchParams.set("zoom","2.00"),s.searchParams.set("version",Bo),s.toString()}if(o==="samuelArchive"){let s=new URL(Co);return s.hash=`${a.zoom.toFixed(2)}/${a.lat.toFixed(6)}/${a.lng.toFixed(6)}`,s.toString()}let r=new URL(Zo);return r.searchParams.set("lat",a.lat.toFixed(6)),r.searchParams.set("lng",a.lng.toFixed(6)),r.searchParams.set("zoom",a.zoom.toFixed(2)),r.searchParams.set("version",Bo),r.toString()}openExternalTool(o){globalThis.open(this.buildExternalToolUrl(o),"_blank","noopener,noreferrer")}setDisabled(o,a){this.element.querySelector("."+o).disabled=a}async run(o,a,r,s="..."){console.log("[KGM][Widget] Task started",{status:o});let l=this.status;this.status=`${s} ${o}`;try{let i=await a();return this.status=l,console.log("[KGM][Widget] Task completed",{status:o}),i}catch(i){if(!(i instanceof lo))console.error(i),this.status=`${f("taskErrorPrefix")}: ${o}`;throw console.error("[KGM][Widget] Task failed",{status:o,error:i}),i}finally{await r?.()}}handleKeyboard(o){if(jo(o.target))return;if(F(o,H.toggleWidget)){o.preventDefault(),this.open=!this.open;return}if(F(o,H.minimizeWidget)){o.preventDefault(),this.open=!1;return}if(F(o,H.showWidgetPanel)){o.preventDefault(),this.open=!0;return}if(F(o,H.hideWidgetPanel)){o.preventDefault(),this.open=!1;return}if(F(o,H.showShortcuts)){o.preventDefault(),this.open=!0,this.openSettingsModal();return}if(F(o,H.toggleOverlay)){o.preventDefault(),this.toggleOverlay();return}if(F(o,H.focusNextImage)){o.preventDefault(),this.focusImageByStep(1);return}if(F(o,H.focusPreviousImage)){o.preventDefault(),this.focusImageByStep(-1);return}if(F(o,H.openColorPanel)){o.preventDefault(),this.openColorPanelForActiveImage();return}if(F(o,H.toggleImageLock)){o.preventDefault(),this.toggleLockForActiveImage();return}if(F(o,H.clickPaintWhenReady)){o.preventDefault(),this.drawAndClickPaintWhenReady();return}if(F(o,H.startAutoFarm)){o.preventDefault(),this.startAutoFarm();return}if(F(o,H.stopAutoFarm)){o.preventDefault(),this.stopAutoFarm();return}if(F(o,H.openColorConverterTool)){o.preventDefault(),this.openExternalTool("colorConverter");return}if(F(o,H.openSamuelArchiveTool)){o.preventDefault(),this.openExternalTool("samuelArchive");return}if(F(o,H.openEralyonArchiveTool)){o.preventDefault(),this.openExternalTool("eralyonArchive");return}if(F(o,H.addImage)&&!this.$addImage.disabled){o.preventDefault(),this.addImage();return}if(F(o,H.draw)&&!this.$draw.disabled)o.preventDefault(),this.bot.draw()}focusImageByStep(o){if(!this.bot.images.length)return;if(this.activeImageIndex<0||this.activeImageIndex>=this.bot.images.length)this.activeImageIndex=o>0?0:this.bot.images.length-1;else this.activeImageIndex=(this.activeImageIndex+o+this.bot.images.length)%this.bot.images.length;this.bot.images[this.activeImageIndex].position.scrollScreenTo()}getActiveImage(){if(!this.bot.images.length)return;if(this.activeImageIndex<0||this.activeImageIndex>=this.bot.images.length)this.activeImageIndex=0;return this.bot.images[this.activeImageIndex]}openColorPanelForActiveImage(){let o=this.getActiveImage();if(!o)return;o.openColorPanel()}toggleLockForActiveImage(){let o=this.getActiveImage();if(!o)return;o.lock=!o.lock,o.update(),k(this.bot)}async waitAndClickPaintButton(){await this.run(f("taskWaitingPaintButton"),async()=>{for(;;){if(this.isChallengeBlockingPaint()){await this.waitForChallengeToResolve(),await new Promise((a)=>setTimeout(a,250));continue}let o=this.findNativePaintButton();if(o&&!o.disabled&&o.ariaDisabled!=="true"){await this.triggerNativePaintClickWithChallengeRecovery(o);return}await new Promise((a)=>setTimeout(a,500))}})}async drawAndClickPaintWhenReady(){if(!this.$draw.disabled)await this.bot.draw();await this.waitAndClickPaintButton()}findNativePaintButton(){return["button.btn.btn-primary.btn-lg.sm\\:btn-xl.relative","button.btn.btn-primary.btn-lg.relative","button.btn.btn-primary.btn-lg.relative.z-30","button.btn.btn-primary.btn-lg.sm\\:btn-xl.relative.z-30","div.absolute.bottom-0.left-1\\/2.-translate-x-1\\/2 button.btn.btn-primary"].flatMap((r)=>Array.from(document.querySelectorAll(r))).find((r)=>/pintar|paint/i.test(r.textContent))}triggerNativePaintClick(o){o.dispatchEvent(new PointerEvent("pointerdown",{bubbles:!0,cancelable:!0,pointerType:"mouse",button:0})),o.dispatchEvent(new MouseEvent("mousedown",{bubbles:!0,cancelable:!0,button:0})),o.dispatchEvent(new PointerEvent("pointerup",{bubbles:!0,cancelable:!0,pointerType:"mouse",button:0})),o.dispatchEvent(new MouseEvent("mouseup",{bubbles:!0,cancelable:!0,button:0})),o.click()}async triggerNativePaintClickWithChallengeRecovery(o){for(let r=0;r<3;r++){let s=r===0?o:this.findNativePaintButton();if(!s)return;if(s.disabled||s.ariaDisabled==="true")return;this.triggerNativePaintClick(s);let l=await this.waitForPaintAttemptOutcome(6000);if(l==="painted")return;if(l==="challenge"){await this.waitForChallengeToResolve(),await new Promise((i)=>setTimeout(i,350));continue}await new Promise((i)=>setTimeout(i,350))}console.log("[KGM][Widget] Paint click finished without a clear success signal after retries")}async waitForPaintAttemptOutcome(o){let a=Date.now();while(Date.now()-a<=o){if(this.isChallengeBlockingPaint())return"challenge";let r=this.findNativePaintButton();if(r&&(r.disabled||r.ariaDisabled==="true"))return await this.waitForDelayedChallenge(1200)?"challenge":"painted";await new Promise((s)=>setTimeout(s,200))}return"unknown"}async waitForDelayedChallenge(o){let a=Date.now();while(Date.now()-a<=o){if(this.isChallengeBlockingPaint())return!0;await new Promise((r)=>setTimeout(r,150))}return!1}async waitForChallengeToResolve(){await this.run(f("taskWaitingChallengeResolve"),async()=>{let o=Date.now(),a=90000;while(this.isChallengeBlockingPaint()&&Date.now()-o<=90000)await new Promise((r)=>setTimeout(r,500))})}isChallengeBlockingPaint(){let r=Array.from(document.querySelectorAll('h-captcha, .h-captcha, iframe[src*="hcaptcha.com"], iframe[src*="newassets.hcaptcha.com"], iframe[src*="captcha"], [data-hcaptcha-widget-id]')).filter((i)=>{if(i.closest("dialog")?.matches("dialog:not([open])"))return!1;let c=globalThis.getComputedStyle(i);if(c.display==="none"||c.visibility==="hidden")return!1;let p=i.getBoundingClientRect();return p.width>0&&p.height>0});if(!r.length)return!1;let s=document.querySelector("dialog.modal[open], dialog[open]");if(s?.querySelector('h-captcha, .h-captcha, iframe[src*="hcaptcha.com"], iframe[src*="newassets.hcaptcha.com"], iframe[src*="captcha"], [data-hcaptcha-widget-id]')){if(!s)return!1;if(!Array.from(s.querySelectorAll('textarea[name="h-captcha-response"], textarea[name^="h-captcha-response-"]')).some((c)=>c.value.trim().length>0))return!0}return r.some((i)=>{let c=i.closest("h-captcha")??i.parentElement??document.documentElement,p=Array.from(c.querySelectorAll('textarea[name="h-captcha-response"], textarea[name^="h-captcha-response-"]'));if(!p.length)return!0;return p.every((n)=>n.value.trim().length===0)})}}var ma=2;function ba(){let o=globalThis;if(typeof o.fp_assemble_injection!=="function")o.fp_assemble_injection=()=>({});if(!o.__kgmUnhandledRejectionPatched)o.__kgmUnhandledRejectionPatched=!0,o.addEventListener("unhandledrejection",(a)=>{let r=a.reason,s=typeof r==="object"&&r!==null&&"name"in r&&typeof r.name==="string"?r.name:"",l=r instanceof Error?r.message:r;if(s==="NotAllowedError"&&l.includes("play() failed"))a.preventDefault()});if(!o.__kgmMediaPlayPatched&&"HTMLMediaElement"in o){o.__kgmMediaPlayPatched=!0;let a=Reflect.get(o.HTMLMediaElement.prototype,"play");o.HTMLMediaElement.prototype.play=function r(){return Reflect.apply(a,this,[]).catch((l)=>{let i=l instanceof Error?l.message:l;if((typeof l==="object"&&l!==null&&"name"in l&&typeof l.name==="string"?l.name:"")==="NotAllowedError"&&i.includes("play() failed"))return;throw l})}}}var To="[KGM]",Lo="kglacer-macro:access-ok",wo="S0dNLXlZUjhTMW81bEhVemVjS1RFMEhxRVB4OVFkcjgxaEVz",Ro="kgm-access-locked";class Eo{unavailableColors=new Set;mapsCache=new Map;me;$stars=[];strategy="SEQUENTIAL";images=[];_widget;get widget(){if(!this._widget)throw new Error("Widget is not initialized yet");return this._widget}markerPixelPositionResolvers=[];lastColor;log(o,a){if(a===void 0)console.log(`${To} ${o}`);else console.log(`${To} ${o}`,a)}constructor(){this.log("Boot sequence started"),document.body.classList.add(Ro);let o=So();if(this.log("Save loaded",{hasSave:Boolean(o),imageCount:o?.images.length??0,strategy:o?.strategy}),o){for(let s=0;s<o.images.length;s++){let l=o.images[s];_({x:l.position[0]-1000,y:l.position[1]-1000}),_({x:l.position[0]+1000,y:l.position[1]+1000})}this.strategy=o.strategy}let a=JSON.parse(localStorage.getItem("kglacer-macro:proxy-config")??"{}");Do(a),this.registerFetchInterceptor(),this.log("Fetch interceptor registered");let r=document.createElement("style");r.textContent=Jo.replace("FAKE_FAVORITE_LOCATIONS",E.length.toString()),document.head.append(r),this.log("Styles injected",{fakeFavoriteLocations:E.length}),(async()=>{this.log("Widget initialization flow started"),await this.ensureAccessKey(),document.body.classList.remove(Ro),this._widget=new eo(this),await this.widget.run(f("taskInitializing"),async()=>{await this.waitForElement("login",".avatar.center-absolute.absolute"),await this.waitForElement("pixel count",".btn.btn-primary.btn-lg.relative.z-30 canvas");let s=await this.waitForElement("canvas",".maplibregl-canvas-container");if(new MutationObserver((l)=>{for(let i=0;i<l.length;i++)if(l[i].removedNodes.length!==0){this.updateStars();break}this.updateImages()}).observe(s,{attributes:!0,childList:!0,subtree:!0}),this.updateStars(),this.log("Stars updated after boot",{stars:this.$stars.length}),await D(500),await this.updateColors(),o)for(let l=0;l<o.images.length;l++){let i=await j.fromJSON(this,o.images[l]);this.images.push(i),i.update()}this.log("Saved images restored",{images:this.images.length}),await this.readMap(),this.updateTasks(),this.widget.setDisabled("draw",!1),this.widget.setDisabled("draw-and-paint",!1),this.widget.setDisabled("add-image",!1),this.widget.setDisabled("capture-template",!1),this.log("Initialization completed; controls enabled")})})()}async ensureAccessKey(){if(localStorage.getItem(Lo)===wo)return;await new Promise((o)=>{let a=document.createElement("dialog");a.className="kgm-modal access-dialog",a.innerHTML=`<form method="dialog" class="access-form">
  <div class="kgm-modal-head">
