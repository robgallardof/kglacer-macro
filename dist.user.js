...
`;class V{bot;image;width;brightness;exactColor;static async fromJSON(o,p){let l=new Image;return l.src=p.url.startsWith("http")?await fetch(p.url,{cache:"no-store"}).then((f)=>f.blob()).then((f)=>URL.createObjectURL(f)):p.url,await L(l,["load"],["error"]),new V(o,l,p.width,p.brightness,p.exactColor)}canvas=document.createElement("canvas");context=this.canvas.getContext("2d");pixels;colors=new Map;resolution;get height(){return this.width/this.resolution|0}set height(o){this.width=o*this.resolution|0}constructor(o,p,l=p.naturalWidth,f=0,w=!1){this.bot=o;this.image=p;this.width=l;this.brightness=f;this.exactColor=w;if(w)this.resolution=1,this.width=1000;else this.resolution=this.image.naturalWidth/this.image.naturalHeight;this.update()}update(){this.canvas.width=this.width,this.canvas.height=this.height,this.colors.clear();let o=new Map;for(let l=1;l<64;l++)if(this.exactColor||!this.bot.unavailableColors.has(l))o.set(E[l],[l,l]);this.context.imageSmoothingEnabled=!1,this.context.imageSmoothingQuality="low",this.context.drawImage(this.image,0,0,this.canvas.width,this.canvas.height),this.pixels=Array.from({length:this.canvas.height},()=>new Array(this.canvas.width));let p=this.context.getImageData(0,0,this.canvas.width,this.canvas.height).data;for(let l=0;l<this.canvas.height;l++)for(let f=0;f<this.canvas.width;f++){let w=(l*this.canvas.width+f)*4,g=p[w],b=p[w+1],z=p[w+2],A=p[w+3],M=`${g},${b},${z}`;if(this.exactColor){this.pixels[l][f]=A<100?0:E.indexOf(M);continue}let H,c;if(A<100)H=c=0;else if(o.has(M))[H,c]=o.get(M);else{let D=1/0,N=1/0;for(let J=0;J<R.length;J++){let Q=R[J],Z=c0(H0(g,b,z),Q,this.brightness);if(!this.bot.unavailableColors.has(J)&&Z<D)D=Z,H=J;if(Z<N)N=Z,c=J}o.set(M,[H,c])}if(H!==0)this.context.fillStyle=`oklab(${R[H][0]*100}% ${R[H][1]} ${R[H][2]})`,this.context.fillRect(f,l,1,1);this.pixels[l][f]=H;let G=this.colors.get(c);if(G)G.amount++;else this.colors.set(c,{color:H,amount:1,realColor:c})}}toJSON(){let o=document.createElement("canvas");return o.width=this.image.naturalWidth,o.height=this.image.naturalHeight,o.getContext("2d").drawImage(this.image,0,0),{url:o.toDataURL("image/webp",1),width:this.width,brightness:this.brightness,exactColor:this.exactColor}}}var s="kglacer-macro-settings",K0=["kglacermacro","wbot"],n="kgm";function E0(){let o=[s,...K0];for(let p=0;p<o.length;p++){let l=o[p],f=localStorage.getItem(l);if(!f)continue;return{json:f,key:l}}return}function U0(){let o=E0();if(!o)return;let p;try{if(p=JSON.parse(o.json),typeof p!=="object")throw new Error("NOT VALID SAVE");if(p.version===1){let l=p;p.images=l.widget.images,p.strategy=l.widget.strategy,delete l.widget}if(o.key!==s)localStorage.setItem(s,o.json)}catch{localStorage.removeItem(o.key),p=void 0}return p}var Q0;function Y(o,p=!1){if(clearTimeout(Q0),p)localStorage.setItem(s,JSON.stringify(o));else Q0=setTimeout(()=>{localStorage.setItem(s,JSON.stringify(o))},600)}var a=1000,_0=2048,$=a*_0,W=[],v=[],C0=Date.now();function i(o){W.push(o),v.push({id:C0++,latitude:(2*Math.atan(Math.exp(-(o.y/$*(2*Math.PI)-Math.PI)))-Math.PI/2)*180/Math.PI,longitude:(o.x/$*(2*Math.PI)-Math.PI)*180/Math.PI,name:"KGLACER_MACRO_FAVORITE"})}i({x:$/3|0,y:$/3|0});i({x:$/3*2|0,y:$/3*2|0});function _(o){let[p,l]=o.style.transform.slice(32,-31).split(", ").map((f)=>Number.parseFloat(f));return{x:p,y:l}}class j{bot;static fromJSON(o,p){return new j(o,...p)}static fromScreenPosition(o,p){let{anchorScreenPosition:l,pixelSize:f,anchorWorldPosition:w}=o.findAnchorsForScreen(p);return new j(o,w.x+(p.x-l.x)/f|0,w.y+(p.y-l.y)/f|0)}globalX=0;globalY=0;get tileX(){return this.globalX/a|0}set tileX(o){this.globalX=o*a+this.x}get tileY(){return this.globalY/a|0}set tileY(o){this.globalY=o*a+this.y}get x(){return this.globalX%a}set x(o){this.globalX=this.tileX*a+o}get y(){return this.globalY%a}set y(o){this.globalY=this.tileY*a+o}anchor1Index;anchor2Index;get pixelSize(){return(_(this.bot.$stars[this.anchor2Index]).x-_(this.bot.$stars[this.anchor1Index]).x)/(W[this.anchor2Index].x-W[this.anchor1Index].x)}constructor(o,p,l,f,w){this.bot=o;if(f===void 0||w===void 0)this.globalX=p,this.globalY=l;else this.globalX=p*a+f,this.globalY=l*a+w;this.updateAnchor()}updateAnchor(){this.anchor1Index=0,this.anchor2Index=1;let o=1/0,p=1/0;for(let l=0;l<W.length;l++){let{x:f,y:w}=W[l];if(f<this.globalX&&w<this.globalY){let g=this.globalX-f+(this.globalY-w);if(g<o)o=g,this.anchor1Index=l}else if(f>this.globalX&&w>this.globalY){let g=f-this.globalX+(w-this.globalY);if(g<p)p=g,this.anchor2Index=l}}}toScreenPosition(){let o=W[this.anchor1Index],p=_(this.bot.$stars[this.anchor1Index]);return{x:(this.globalX-o.x)*this.pixelSize+p.x,y:(this.globalY-o.y)*this.pixelSize+p.y}}getMapColor(){return this.bot.mapsCache.get(this.tileX+"/"+this.tileY).pixels[this.y][this.x]}scrollScreenTo(){let{x:o,y:p}=this.toScreenPosition();this.bot.moveMap({x:o-window.innerWidth/3,y:p-window.innerHeight/3})}clone(){return new j(this.bot,this.tileX,this.tileY,this.x,this.y)}toJSON(){return[this.globalX,this.globalY]}}var S0="https://raw.githubusercontent.com/robgallardof/kglacer-macro/refs/heads/main/src/img/logo.svg",N0;((K)=>{K.RANDOM="RANDOM";K.HUMANIZED="HUMANIZED";K.HUMAN_SOFT_DITHER="HUMAN_SOFT_DITHER";K.HUMAN_PATCHY="HUMAN_PATCHY";K.HUMAN_SWEEP_ARCS="HUMAN_SWEEP_ARCS";K.HUMAN_MICRO_CORRECTIONS="HUMAN_MICRO_CORRECTIONS";K.HUMAN_JITTER_FILL="HUMAN_JITTER_FILL";K.HUMAN_CORNER_BIAS="HUMAN_CORNER_BIAS";K.HUMAN_LONG_STROKES="HUMAN_LONG_STROKES";K.HUMAN_TAP_CLUSTERS="HUMAN_TAP_CLUSTERS";K.HUMAN_MESSY_SPIRAL="HUMAN_MESSY_SPIRAL";K.HUMAN_DRUNK_WALK="HUMAN_DRUNK_WALK";K.HUMAN_NOISE_CLOUD="HUMAN_NOISE_CLOUD";K.HUMAN_PATCH_JUMP="HUMAN_PATCH_JUMP";K.ZIGZAG="ZIGZAG";K.BRUSH_STROKES="BRUSH_STROKES";K.DIAGONAL_BRUSH="DIAGONAL_BRUSH";K.DOWN="DOWN";K.UP="UP";K.LEFT="LEFT";K.RIGHT="RIGHT";K.SPIRAL_FROM_CENTER="SPIRAL_FROM_CENTER";K.SPIRAL_TO_CENTER="SPIRAL_TO_CENTER";K.SCRIBBLE="SCRIBBLE";K.CROSSHATCH="CROSSHATCH";K.WAVE_SWEEP="WAVE_SWEEP";K.SCATTERED_LINES="SCATTERED_LINES";K.CONTOUR_JITTER="CONTOUR_JITTER";K.SPIRAL_WOBBLE="SPIRAL_WOBBLE";K.CLUSTER_BURSTS="CLUSTER_BURSTS";K.ORBITAL="ORBITAL";K.FLOW_FIELD="FLOW_FIELD";K.EDGE_IN="EDGE_IN"})(N0||={});class C extends d{bot;position;pixels;strategy;opacity;drawTransparentPixels;drawColorsInOrder;colors;lock;static async fromJSON(o,p){return new C(o,j.fromJSON(o,p.position),await V.fromJSON(o,p.pixels),p.strategy,p.opacity,p.drawTransparentPixels,p.drawColorsInOrder,p.colors,p.lock)}element=document.createElement("div");tasks=[];moveInfo;$brightness;$canvas;$colors;$colorsDialog;$colorsDialogList;$colorSearch;$openColors;$openPreview;$closeColors;$closePreview;$delete;$drawColorsInOrder;$drawTransparent;$export;$lock;$opacity;$progressLine;$progressText;$previewDialog;$previewDialogList;$resetSize;$resetSizeSpan;$settings;$strategy;$topbar;$wrapper;colorDialogDragState;suppressNextColorDialogBackdropClick=!1;logoPreviewMask;logoPreviewImage;previewAnimations=new WeakMap;constructor(o,p,l,f="SPIRAL_FROM_CENTER",w=50,g=!1,b=!1,z=[],A=!1){super();this.bot=o;this.position=p;this.pixels=l;this.strategy=f;this.opacity=w;this.drawTransparentPixels=g;this.drawColorsInOrder=b;this.colors=z;this.lock=A;this.element.innerHTML=G0,this.element.classList.add("wimage"),k(this.element),document.body.append(this.element),this.populateElementsWithSelector(this.element,{$brightness:".brightness",$colors:".colors",$colorsDialog:".colors-dialog",$colorsDialogList:".colors-dialog-list",$colorSearch:".color-search",$openColors:".open-colors",$openPreview:".open-preview",$closeColors:".close-colors",$closePreview:".close-preview",$delete:".delete",$drawColorsInOrder:".draw-colors-in-order",$drawTransparent:".draw-transparent",$export:".export",$lock:".lock",$opacity:".opacity",$progressLine:".wprogress div",$progressText:".wprogress span",$previewDialog:".preview-dialog",$previewDialogList:".preview-dialog-list",$resetSize:".reset-size",$settings:".wform",$strategy:".strategy",$topbar:".wtopbar",$wrapper:".wrapper"}),this.$resetSizeSpan=this.$resetSize.querySelector("span"),this.$canvas=this.pixels.canvas,this.$wrapper.prepend(this.pixels.canvas),document.body.append(this.$colorsDialog,this.$previewDialog),this.registerEvent(this.$strategy,"change",()=>{this.strategy=this.$strategy.value,Y(this.bot)}),this.registerEvent(this.$opacity,"input",()=>{this.opacity=this.$opacity.valueAsNumber,this.$opacity.style.setProperty("--val",this.opacity+"%"),this.update(),Y(this.bot)}),this.$opacity.style.setProperty("--val",this.opacity+"%");let M;this.registerEvent(this.$brightness,"change",()=>{clearTimeout(M),M=setTimeout(()=>{this.pixels.brightness=this.$brightness.valueAsNumber,this.pixels.update(),this.updateColors(),this.update(),Y(this.bot)},1000)}),this.registerEvent(this.$resetSize,"click",()=>{this.pixels.width=this.pixels.image.naturalWidth,this.pixels.update(),this.updateColors(),this.update(),Y(this.bot)}),this.registerEvent(this.$drawTransparent,"click",()=>{this.drawTransparentPixels=this.$drawTransparent.checked,Y(this.bot)}),this.registerEvent(this.$drawColorsInOrder,"click",()=>{this.drawColorsInOrder=this.$drawColorsInOrder.checked,Y(this.bot)}),this.registerEvent(this.$lock,"click",()=>{this.lock=!this.lock,this.update(),Y(this.bot)}),this.registerEvent(this.$delete,"click",this.destroy.bind(this)),this.registerEvent(this.$openColors,"click",()=>{this.openColorPanel()}),this.registerEvent(this.$openPreview,"click",()=>{this.openPreviewPanel()}),this.registerEvent(this.$closeColors,"click",()=>{this.$colorsDialog.close()}),this.registerEvent(this.$closePreview,"click",()=>{this.$previewDialog.close()}),this.registerEvent(this.$colorsDialog.querySelector(".colors-dialog-head"),"pointerdown",this.startColorDialogDrag.bind(this)),this.registerEvent(document,"pointermove",this.moveColorDialog.bind(this),{passive:!1}),this.registerEvent(document,"pointerup",this.stopColorDialogDrag.bind(this)),this.registerEvent(document,"pointercancel",this.stopColorDialogDrag.bind(this)),this.registerEvent(this.$colorsDialog,"click",(H)=>{if(this.suppressNextColorDialogBackdropClick){this.suppressNextColorDialogBackdropClick=!1;return}if(H.target===this.$colorsDialog)this.$colorsDialog.close()}),this.registerEvent(this.$previewDialog,"click",(H)=>{if(H.target===this.$previewDialog)this.$previewDialog.close()}),this.registerEvent(this.$colorSearch,"input",()=>{this.updateColors()}),this.registerEvent(this.$export,"click",this.export.bind(this)),this.registerEvent(this.$topbar,"mousedown",this.moveStart.bind(this)),this.registerEvent(this.$canvas,"mousedown",this.moveStart.bind(this)),this.registerEvent(document,"mouseup",this.moveStop.bind(this)),this.registerEvent(document,"mousemove",this.move.bind(this));for(let H of this.element.querySelectorAll(".resize"))this.registerEvent(H,"mousedown",this.resizeStart.bind(this));this.update(),this.updateColors()}toJSON(){return{pixels:this.pixels.toJSON(),position:this.position.toJSON(),strategy:this.strategy,opacity:this.opacity,drawTransparentPixels:this.drawTransparentPixels,drawColorsInOrder:this.drawColorsInOrder,colors:this.colors,lock:this.lock}}updateTasks(){this.tasks.length=0;let o=this.position.clone(),p=new Set,l=new Map;for(let f=0;f<this.colors.length;f++){let w=this.colors[f];if(w.disabled)p.add(w.realColor);l.set(w.realColor,f)}for(let{x:f,y:w}of this.strategyPositionIterator()){let g=this.pixels.pixels[w][f];if(p.has(g))continue;o.globalX=this.position.globalX+f,o.globalY=this.position.globalY+w;let b=o.getMapColor();if(g!==b&&(this.drawTransparentPixels||g!==0))this.tasks.push({position:o.clone(),color:g})}if(this.drawColorsInOrder)this.tasks.sort((f,w)=>(l.get(f.color)??0)-(l.get(w.color)??0));this.update(),this.bot.widget.update()}update(){let{x:o,y:p}=this.position.toScreenPosition(),l=Math.round(this.position.pixelSize*this.pixels.width),f=Math.round(this.position.pixelSize*this.pixels.height);this.element.style.transform=`translate3d(${Math.round(o)}px, ${Math.round(p)}px, 0)`,this.element.style.width=`${l}px`,this.element.style.height=`${f}px`,this.$canvas.style.opacity=`${this.opacity}%`,this.element.classList.remove("hidden"),this.$resetSizeSpan.textContent=this.pixels.width.toString(),this.$brightness.valueAsNumber=this.pixels.brightness,this.$strategy.value=this.strategy,this.$opacity.valueAsNumber=this.opacity,this.$drawTransparent.checked=this.drawTransparentPixels,this.$drawColorsInOrder.checked=this.drawColorsInOrder;let w=this.pixels.pixels.length*this.pixels.pixels[0].length,g=w-this.tasks.length,b=g/w*100|0;this.$progressText.textContent=`${g}/${w} ${b}% ETA: ${this.tasks.length/120|0}h`,this.$progressLine.style.transform=`scaleX(${b}%)`,this.$wrapper.classList[this.lock?"add":"remove"]("no-pointer-events"),this.$lock.classList[this.lock?"add":"remove"]("locked")}destroy(){super.destroy(),this.element.remove(),this.$colorsDialog.remove(),this.$previewDialog.remove(),p0(this.bot.images,this),this.bot.widget.update(),Y(this.bot)}openColorPanel(){if(this.$colorsDialog.open){this.$colorSearch.focus();return}this.$colorsDialog.style.position="fixed",this.$colorsDialog.style.left="",this.$colorsDialog.style.top="",this.$colorsDialog.style.margin="auto",this.$colorsDialog.showModal(),this.$colorSearch.focus()}openPreviewPanel(){if(this.$previewDialog.open){this.renderStrategyPreviewSamples();return}this.$previewDialog.style.position="fixed",this.$previewDialog.style.left="",this.$previewDialog.style.top="",this.$previewDialog.style.margin="auto",this.$previewDialog.showModal(),this.renderStrategyPreviewSamples()}startColorDialogDrag(o){if(o.button!==0)return;if(o.target?.closest("button,input,select,textarea,a,label"))return;let l=this.$colorsDialog.getBoundingClientRect();this.colorDialogDragState={pointerId:o.pointerId,offsetX:o.clientX-l.left,offsetY:o.clientY-l.top,moved:!1},o.preventDefault()}moveColorDialog(o){if(!this.colorDialogDragState)return;if(o.pointerId!==this.colorDialogDragState.pointerId)return;let p=this.$colorsDialog.getBoundingClientRect(),l=Math.max(8,window.innerWidth-p.width-8),f=Math.max(8,window.innerHeight-p.height-8),w=Math.min(l,Math.max(8,o.clientX-this.colorDialogDragState.offsetX)),g=Math.min(f,Math.max(8,o.clientY-this.colorDialogDragState.offsetY));if(!this.colorDialogDragState.moved&&(Math.abs(o.movementX)>0||Math.abs(o.movementY)>0))this.colorDialogDragState.moved=!0;this.$colorsDialog.style.left=`${Math.round(w)}px`,this.$colorsDialog.style.top=`${Math.round(g)}px`,o.preventDefault()}stopColorDialogDrag(o){if(!this.colorDialogDragState)return;if(o.pointerId!==this.colorDialogDragState.pointerId)return;if(this.colorDialogDragState.moved)this.suppressNextColorDialogBackdropClick=!0;this.colorDialogDragState=void 0}renderStrategyPreviewSamples(){let o=this.$strategy.value,p=Object.values(N0).sort((f,w)=>{if(f===o)return-1;if(w===o)return 1;return 0});this.$previewDialogList.innerHTML="";let l=document.createDocumentFragment();for(let f=0;f<p.length;f++){let w=p[f],g=document.createElement("article");g.className="preview-card";let b=document.createElement("strong");b.textContent=this.getStrategyLabel(w);let z=document.createElement("canvas");z.className="preview-canvas",z.width=156,z.height=156,this.paintStrategyPreview(z,w),g.append(b,z),l.append(g)}this.$previewDialogList.append(l)}getStrategyLabel(o){switch(o){case"RANDOM":return u("random");case"HUMANIZED":return u("humanized");case"HUMAN_SOFT_DITHER":return u("humanSoftDither");case"HUMAN_PATCHY":return u("humanPatchy");case"HUMAN_SWEEP_ARCS":return u("humanSweepArcs");case"HUMAN_MICRO_CORRECTIONS":return u("humanMicroCorrections");case"HUMAN_JITTER_FILL":return u("humanJitterFill");case"HUMAN_CORNER_BIAS":return u("humanCornerBias");case"HUMAN_LONG_STROKES":return u("humanLongStrokes");case"HUMAN_TAP_CLUSTERS":return u("humanTapClusters");case"HUMAN_MESSY_SPIRAL":return u("humanMessySpiral");case"HUMAN_DRUNK_WALK":return u("humanDrunkWalk");case"HUMAN_NOISE_CLOUD":return u("humanNoiseCloud");case"HUMAN_PATCH_JUMP":return u("humanPatchJump");case"ZIGZAG":return u("zigzag");case"BRUSH_STROKES":return u("brushStrokes");case"DIAGONAL_BRUSH":return u("diagonalBrush");case"DOWN":return u("down");case"UP":return u("up");case"LEFT":return u("left");case"RIGHT":return u("right");case"SPIRAL_FROM_CENTER":return u("spiralOut");case"SPIRAL_TO_CENTER":return u("spiralIn");case"SCRIBBLE":return u("scribble");case"CROSSHATCH":return u("crosshatch");case"WAVE_SWEEP":return u("waveSweep");case"SCATTERED_LINES":return u("scatteredLines");case"CONTOUR_JITTER":return u("contourJitter");case"SPIRAL_WOBBLE":return u("spiralWobble");case"CLUSTER_BURSTS":return u("clusterBursts");case"ORBITAL":return u("orbital");case"FLOW_FIELD":return u("flowField");case"EDGE_IN":return u("edgeIn");default:return o}}paintStrategyPreview(o,p){let l=o.getContext("2d");if(!l)return;l.fillStyle="#0f1526",l.fillRect(0,0,o.width,o.height);let f=this.getLogoPreviewMask(),w=this.strategy;this.strategy=p;let g=[...this.strategyPositionIterator()];this.strategy=w;let b=new Set(f.map(({x:N,y:J})=>`${N}:${J}`)),z=g.filter(({x:N,y:J})=>b.has(`${N}:${J}`)),A=o.width/this.pixels.width,M=this.previewAnimations.get(o);if(M)cancelAnimationFrame(M);let H=(N)=>{l.fillStyle="#0f1526",l.fillRect(0,0,o.width,o.height),this.paintLogoGhost(l,A,f);for(let J=0;J<Math.min(N,z.length);J++){let Q=z[J],Z=J/Math.max(1,z.length-1);l.fillStyle=`hsl(${220-Z*110} 90% ${43+Z*18}%)`,l.fillRect(Q.x*A,Q.y*A,Math.max(1,A),Math.max(1,A))}},c=performance.now(),G=Math.min(3800,Math.max(900,z.length*8)),D=(N)=>{let J=N-c,Q=Math.min(1,J/G);H(Math.floor(z.length*Q));let Z=Q>=1?requestAnimationFrame(()=>{H(z.length)}):requestAnimationFrame(D);this.previewAnimations.set(o,Z)};D(c)}paintLogoGhost(o,p,l){if(this.logoPreviewImage){o.save(),o.globalAlpha=0.22,o.drawImage(this.logoPreviewImage,0,0,this.pixels.width*p,this.pixels.height*p),o.restore();return}o.fillStyle="rgb(115 132 190 / 28%)";for(let f=0;f<l.length;f++){let w=l[f];o.fillRect(w.x*p,w.y*p,Math.max(1,p),Math.max(1,p))}}getLogoPreviewMask(){if(this.logoPreviewMask)return this.logoPreviewMask;this.logoPreviewMask=this.fallbackPreviewMask();let o=new Image;return o.src=S0,o.decode().then(()=>{this.logoPreviewImage=o;let p=document.createElement("canvas");p.width=this.pixels.width,p.height=this.pixels.height;let l=p.getContext("2d");if(!l)return;if(l.clearRect(0,0,p.width,p.height),l.drawImage(o,0,0,p.width,p.height),this.logoPreviewMask=this.alphaMaskFromCanvas(l,p.width,p.height),this.$previewDialog.open)this.renderStrategyPreviewSamples()}).catch(()=>{return}),this.logoPreviewMask}alphaMaskFromCanvas(o,p,l){let f=o.getImageData(0,0,p,l).data,w=[];for(let g=0;g<l;g++)for(let b=0;b<p;b++)if((f[(g*p+b)*4+3]??0)>24)w.push({x:b,y:g});return w.length?w:this.fallbackPreviewMask()}fallbackPreviewMask(){let o=[],p=this.pixels.width/2,l=this.pixels.height/2,f=Math.max(4,Math.min(this.pixels.width,this.pixels.height)/2.5);for(let w=0;w<this.pixels.height;w++)for(let g=0;g<this.pixels.width;g++)if((g-p)**2+(w-l)**2<=f**2)o.push({x:g,y:w});return o}applyLocale(){if(k(this.element),this.updateColors(),this.$previewDialog.open)this.renderStrategyPreviewSamples()}colorHex(o){let p=E[o]??"0,0,0",[l=0,f=0,w=0]=p.split(",").map((g)=>Number.parseInt(g,10));return`#${[l,f,w].map((g)=>g.toString(16).padStart(2,"0")).join("")}`}colorKeywords(o){let p=E[o]??"0,0,0",[l=0,f=0,w=0]=p.split(",").map((A)=>Number.parseInt(A,10)),g=Math.max(l,f,w),b=Math.min(l,f,w);if(g-b<15)return["gray","grey","gris","neutral","neutro"];if(l>f+30&&l>w+30)return["red","rojo"];if(f>l+30&&f>w+30)return["green","verde"];if(w>l+30&&w>f+30)return["blue","azul"];if(l>170&&f>120&&w<130)return["orange","naranja"];if(l>170&&f>110&&w>140)return["pink","rosa"];if(l>120&&f<100&&w>120)return["purple","violet","morado"];if(l>130&&f>130&&w<90)return["yellow","amarillo"];return["brown","cafe","marron"]}updateColors(){this.$colors.innerHTML="",this.$colorsDialogList.innerHTML="";let o=this.pixels.pixels.length*this.pixels.pixels[0].length,p=100/this.pixels.colors.size,l=document.createElement("div");l.className="colors-track",l.setAttribute("aria-label",u("overlayColors")),this.$colorsDialogList.setAttribute("aria-label",u("colorPanelResults"));let f=this.$colorSearch.value.trim().toLowerCase();if(this.colors.length!==this.pixels.colors.size||this.colors.some((g)=>!this.pixels.colors.has(g.realColor)))this.colors=this.pixels.colors.values().toArray().sort((g,b)=>b.amount-g.amount).map((g)=>({realColor:g.realColor,disabled:!1})),Y(this.bot);let w=0;for(let g=0;g<this.colors.length;g++){let b=this.colors[g],z=this.pixels.colors.get(b.realColor),A=!1,M=!1,H=z.realColor!==z.color,c=z.amount/o*100,G=this.colorHex(z.realColor),D=this.colorKeywords(z.realColor),N=()=>{if(A)return;b.disabled=b.disabled?void 0:!0,J.classList.toggle("color-disabled"),Q.classList.toggle("disabled",Boolean(b.disabled));let U=Q.querySelector(".state");if(U)U.textContent=b.disabled?u("disabled"):u("enabled");Y(this.bot)},J=document.createElement("button");if(J.setAttribute("aria-label",`${u("overlayColors")} #${g+1}: ${G.toUpperCase()}`),J.title=`${c.toFixed(1)}%`,J.innerHTML=`<span class="order-index">#${g+1}</span>`,b.disabled)J.classList.add("color-disabled");if(!H)J.style.background=x(z.realColor);else{J.classList.add("substitution"),J.style.setProperty("--wreal-color",x(z.realColor)),J.style.setProperty("--wsubstitution-color",x(z.color));let U=document.createElement("button"),B=document.createElement("button");U.textContent=u("buy"),B.textContent="✓",U.classList.add("buy"),B.classList.add("disable"),U.addEventListener("click",()=>{document.getElementById("color-"+z.realColor)?.click()}),B.addEventListener("click",N),J.append(U),J.append(B)}J.style.left=w+"%",J.style.width=c+"%",w+=c,J.style.setProperty("--wleft",p*g+"%"),J.style.setProperty("--wwidth",p+"%"),l.append(J);let Q=document.createElement("button");if(Q.className=`color-chip ${b.disabled?"disabled":""}`,Q.draggable=!0,Q.innerHTML=`<span class="order-index">#${g+1}</span>
<span class="drag" title="${u("up")} / ${u("down")}">⋮⋮</span>
<span class="swatch"></span>
<span class="meta">
  <span class="coverage">${c.toFixed(1)}% · ${G.toUpperCase()}</span>
  <span class="state">${b.disabled?u("disabled"):u("enabled")}</span>
</span>
<span class="premium ${H?"on":""}">${H?u("premium"):""}</span>`,Q.querySelector(".swatch").style.setProperty("--swatch-color",x(z.realColor)),Q.addEventListener("click",()=>{if(M){M=!1;return}N()}),Q.addEventListener("dragstart",(U)=>{M=!0,Q.classList.add("dragging"),U.dataTransfer?.setData("text/plain",String(g)),U.dataTransfer.effectAllowed="move"}),Q.addEventListener("dragend",()=>{Q.classList.remove("dragging")}),Q.addEventListener("dragover",(U)=>{U.preventDefault(),Q.classList.add("drag-target")}),Q.addEventListener("dragleave",()=>{Q.classList.remove("drag-target")}),Q.addEventListener("drop",(U)=>{U.preventDefault(),Q.classList.remove("drag-target");let B=Number.parseInt(U.dataTransfer?.getData("text/plain")??"-1",10);if(B<0||B===g||B>=this.colors.length)return;this.colors.splice(g,0,...this.colors.splice(B,1)),Y(this.bot),this.updateColors()}),H){let U=document.createElement("button");U.textContent=u("buy"),U.className="buy-chip",U.addEventListener("click",(B)=>{B.stopPropagation(),document.getElementById("color-"+z.realColor)?.click()}),Q.append(U)}let Z=`${G} ${D.join(" ")} ${z.realColor} ${E[z.realColor]}`;if(!f||Z.toLowerCase().includes(f))this.$colorsDialogList.append(Q);let S=(U)=>{let B=g,T=J.getBoundingClientRect().width,O=(I)=>{if(B=Math.min(this.colors.length-1,Math.max(0,Math.round(g+(I.clientX-U.clientX)/T))),B!==g)A=!0;let X=0;for(let h of l.children){if(h===J)continue;if(X===B)X++;h.style.setProperty("--wleft",p*X+"%"),X++}J.style.setProperty("--wleft",p*B+"%")};document.addEventListener("mousemove",O),document.addEventListener("mouseup",()=>{if(document.removeEventListener("mousemove",O),B!==g)this.colors.splice(B,0,...this.colors.splice(g,1));Y(this.bot),J.removeEventListener("mousedown",S),setTimeout(()=>{this.updateColors()},200)},{once:!0})};if(J.addEventListener("mousedown",S),!H)J.addEventListener("click",N)}this.$colors.append(l)}*strategyPositionIterator(){let o=this.pixels.pixels[0].length,p=this.pixels.pixels.length;switch(this.strategy){case"DOWN":{for(let l=0;l<p;l++)for(let f=0;f<o;f++)yield{x:f,y:l};break}case"UP":{for(let l=p-1;l>=0;l--)for(let f=0;f<o;f++)yield{x:f,y:l};break}case"LEFT":{for(let l=0;l<o;l++)for(let f=0;f<p;f++)yield{x:l,y:f};break}case"RIGHT":{for(let l=o-1;l>=0;l--)for(let f=0;f<p;f++)yield{x:l,y:f};break}case"RANDOM":{let l=[];for(let f=0;f<p;f++)for(let w=0;w<o;w++)l.push({x:w,y:f});for(let f=l.length-1;f>=0;f--){let w=Math.floor(Math.random()*(f+1)),g=l[f];l[f]=l[w],l[w]=g}yield*l;break}case"ZIGZAG":{for(let l=0;l<p;l++)if(l%2===0)for(let f=0;f<o;f++)yield{x:f,y:l};else for(let f=o-1;f>=0;f--)yield{x:f,y:l};break}case"HUMANIZED":{let l=Math.max(4,Math.floor(Math.min(o,p)/10)),f=[];for(let w=0;w<p;w+=l)for(let g=0;g<o;g+=l)f.push({x:g,y:w});for(let w=f.length-1;w>=0;w--){let g=Math.floor(Math.random()*(w+1)),b=f[w];f[w]=f[g],f[g]=b}for(let w=0;w<f.length;w++){let g=f[w],b=Math.min(p,g.y+l),z=Math.min(o,g.x+l);for(let A=g.y;A<b;A++)if(Math.random()>0.35)for(let H=g.x;H<z;H++)yield{x:H,y:A};else for(let H=z-1;H>=g.x;H--)yield{x:H,y:A}}break}case"HUMAN_SOFT_DITHER":{let l=new Set;for(let f=0;f<p;f++){let w=Math.floor(Math.random()*3)-1;if((f+w)%2===0)for(let b=0;b<o;b+=2)l.add(`${b},${f}`),yield{x:b,y:f};else for(let b=1;b<o;b+=2)l.add(`${b},${f}`),yield{x:b,y:f}}for(let f=0;f<p;f++)for(let w=0;w<o;w++){let g=`${w},${f}`;if(l.has(g))continue;yield{x:w,y:f}}break}case"HUMAN_PATCHY":{let l=new Set,f=o*p;while(l.size<f){let w=Math.floor(Math.random()*o),g=Math.floor(Math.random()*p),b=1+Math.floor(Math.random()*5);for(let z=g-b;z<=g+b;z++)for(let A=w-b;A<=w+b;A++){if(A<0||A>=o||z<0||z>=p)continue;if(Math.hypot(A-w,z-g)>b+Math.random()*1.2)continue;let M=`${A},${z}`;if(l.has(M))continue;l.add(M),yield{x:A,y:z}}}break}case"HUMAN_SWEEP_ARCS":{let l=new Set,f=(o-1)/2,w=(p-1)/2,g=Math.hypot(f,w);for(let b=0;b<4;b++){let z=Math.random()*Math.PI*2;for(let A=0;A<=g;A+=0.35){let M=Math.PI/2+Math.random()*(Math.PI/1.5),H=Math.max(10,Math.floor(A*8));for(let c=0;c<H;c++){let G=z+M*c/H+Math.sin(A)*0.08,D=Math.round(f+Math.cos(G)*A),N=Math.round(w+Math.sin(G)*A);if(D<0||D>=o||N<0||N>=p)continue;let J=`${D},${N}`;if(l.has(J))continue;l.add(J),yield{x:D,y:N}}}}for(let b=0;b<p;b++)for(let z=0;z<o;z++){let A=`${z},${b}`;if(l.has(A))continue;yield{x:z,y:b}}break}case"HUMAN_MICRO_CORRECTIONS":{let l=new Set;for(let f=0;f<p;f++){let w=f%2===0?1:-1,g=w>0?0:o-1;for(let b=0;b<o;b++){let z=g+(Math.random()>0.82?w:0),A=f+(Math.random()>0.9?1:0);for(let M of[{x:g,y:f},{x:z,y:f},{x:g,y:A}]){if(M.x<0||M.x>=o||M.y<0||M.y>=p)continue;let H=`${M.x},${M.y}`;if(l.has(H))continue;l.add(H),yield M}g+=w}}for(let f=0;f<p;f++)for(let w=0;w<o;w++){let g=`${w},${f}`;if(l.has(g))continue;yield{x:w,y:f}}break}case"HUMAN_JITTER_FILL":{let l=[];for(let f=0;f<p;f++)for(let w=0;w<o;w++)l.push({x:w,y:f});l.sort((f,w)=>{let g=f.y+(Math.random()-0.5)*1.8,b=w.y+(Math.random()-0.5)*1.8;if(g!==b)return g-b;return f.x+(Math.random()-0.5)*2-(w.x+(Math.random()-0.5)*2)}),yield*l;break}case"HUMAN_CORNER_BIAS":{let l=[{x:0,y:0},{x:o-1,y:0},{x:0,y:p-1},{x:o-1,y:p-1}],f=l[Math.floor(Math.random()*l.length)],w=[];for(let g=0;g<p;g++)for(let b=0;b<o;b++){let A=Math.hypot(b-f.x,g-f.y)+Math.random()*3.5;w.push({point:{x:b,y:g},score:A})}w.sort((g,b)=>g.score-b.score);for(let g of w)yield g.point;break}case"HUMAN_LONG_STROKES":{let l=new Set,f=o*p;while(l.size<f){let w=Math.floor(Math.random()*o),g=Math.floor(Math.random()*p),b=Math.random()*Math.PI*2,z=Math.sign(Math.cos(b)),A=Math.sign(Math.sin(b)),M=10+Math.floor(Math.random()*40);for(let H=0;H<M;H++){if(w<0||w>=o||g<0||g>=p)break;let c=`${w},${g}`;if(!l.has(c))l.add(c),yield{x:w,y:g};if(Math.random()>0.78)w+=A,g+=z;else w+=z,g+=A}}break}case"HUMAN_TAP_CLUSTERS":{let l=new Set,f=o*p;while(l.size<f){let w=Math.floor(Math.random()*o),g=Math.floor(Math.random()*p),b=3+Math.floor(Math.random()*10);for(let z=0;z<b;z++){let A=Math.round(w+(Math.random()-0.5)*6),M=Math.round(g+(Math.random()-0.5)*6);if(A<0||A>=o||M<0||M>=p)continue;let H=`${A},${M}`;if(l.has(H))continue;l.add(H),yield{x:A,y:M}}}break}case"HUMAN_MESSY_SPIRAL":{let l=new Set,f=(o-1)/2,w=(p-1)/2,g=Math.hypot(f,w)+2;for(let b=0;l.size<o*p;b++){let z=b/3,A=Math.min(g,z*0.18),M=z*0.29+Math.sin(z*0.13)*0.8,H=Math.round(f+Math.cos(M)*A+Math.sin(z)*0.7),c=Math.round(w+Math.sin(M)*A+Math.cos(z)*0.7);if(H<0||H>=o||c<0||c>=p){if(b>o*p*18)break;continue}let G=`${H},${c}`;if(l.has(G)){if(Math.random()>0.9)continue}else l.add(G),yield{x:H,y:c};if(b>o*p*18)break}for(let b=0;b<p;b++)for(let z=0;z<o;z++){let A=`${z},${b}`;if(l.has(A))continue;yield{x:z,y:b}}break}case"HUMAN_DRUNK_WALK":{let l=new Set,f=Math.floor(Math.random()*o),w=Math.floor(Math.random()*p),g=[{x:-1,y:0},{x:1,y:0},{x:0,y:-1},{x:0,y:1},{x:-1,y:-1},{x:1,y:-1},{x:-1,y:1},{x:1,y:1}];while(l.size<o*p){let b=`${f},${w}`;if(!l.has(b))l.add(b),yield{x:f,y:w};let z=[];for(let H of g){let c=f+H.x,G=w+H.y;if(c<0||c>=o||G<0||G>=p)continue;z.push({x:c,y:G})}if(!z.length)break;let A=z.filter((H)=>{return!l.has(`${H.x},${H.y}`)});if(A.length&&Math.random()>0.2){let H=A[Math.floor(Math.random()*A.length)];f=H.x,w=H.y;continue}let M=z[Math.floor(Math.random()*z.length)];f=M.x,w=M.y}for(let b=0;b<p;b++)for(let z=0;z<o;z++){let A=`${z},${b}`;if(l.has(A))continue;yield{x:z,y:b}}break}case"HUMAN_NOISE_CLOUD":{let l=[];for(let f=0;f<p;f++)for(let w=0;w<o;w++){let g=Math.sin((w+1)*0.93+Math.random()*0.8)+Math.cos((f+1)*1.17+Math.random()*0.8),b=(Math.random()-0.5)*2.6,z=Math.hypot(w-o/2,f-p/2)*0.08;l.push({point:{x:w,y:f},score:g+b+z})}l.sort((f,w)=>f.score-w.score);for(let f of l)yield f.point;break}case"HUMAN_PATCH_JUMP":{let l=new Set,f=[];for(let w=0;w<Math.max(6,o*p/18);w++)f.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*p)});while(l.size<o*p){let w=f[Math.floor(Math.random()*f.length)],g=1+Math.floor(Math.random()*3),b=1+Math.floor(Math.random()*3);for(let z=w.y-b;z<=w.y+b;z++)for(let A=w.x-g;A<=w.x+g;A++){if(A<0||A>=o||z<0||z>=p)continue;if(Math.random()>0.86)continue;let M=`${A},${z}`;if(l.has(M))continue;l.add(M),yield{x:A,y:z}}if(Math.random()>0.72&&f.length<o*p/2)f.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*p)});if(l.size>o*p*0.92)break}for(let w=0;w<p;w++)for(let g=0;g<o;g++){let b=`${g},${w}`;if(l.has(b))continue;yield{x:g,y:w}}break}case"DIAGONAL_BRUSH":{for(let l=0;l<o+p-1;l++){let f=l%2===0,w=[],g=Math.max(0,l-o+1),b=Math.min(p-1,l);for(let z=g;z<=b;z++){let A=l-z;if(A>=0&&A<o)w.push({x:A,y:z})}if(Math.random()>0.55)w.reverse();if(f)for(let z=w.length-1;z>=0;z--)yield w[z];else yield*w}break}case"BRUSH_STROKES":{let l=Array.from({length:p},()=>Array(o).fill(!1)),f=[{x:1,y:0},{x:-1,y:0},{x:0,y:1},{x:0,y:-1},{x:1,y:1},{x:1,y:-1},{x:-1,y:1},{x:-1,y:-1}],w=(z,A)=>z>=0&&z<o&&A>=0&&A<p,g=0,b=o*p;for(let z=0;z<b*6&&g<b;z++){let A=Math.floor(Math.random()*o),M=Math.floor(Math.random()*p),H=f[Math.floor(Math.random()*f.length)],c=3+Math.floor(Math.random()*16);for(let G=0;G<c;G++){if(!w(A,M))break;if(!l[M][A])l[M][A]=!0,g++,yield{x:A,y:M};if(Math.random()>0.72)H=f[Math.floor(Math.random()*f.length)];A+=H.x,M+=H.y}}for(let z=0;z<p;z++)for(let A=0;A<o;A++)if(!l[z][A])yield{x:A,y:z};break}case"SPIRAL_FROM_CENTER":case"SPIRAL_TO_CENTER":{let l=new Set,f=o*p,w=Math.floor(o/2),g=Math.floor(p/2),b=[[1,0],[0,1],[-1,0],[0,-1]],z=0,A=1,M=(c,G)=>c>=0&&c<o&&G>=0&&G<p,H=function*(){let c=0;while(c<f){for(let G=0;G<2;G++){for(let D=0;D<A;D++){if(M(w,g)){let N=`${w},${g}`;if(!l.has(N)){if(l.add(N),yield{x:w,y:g},c++,c>=f)return}}w+=b[z][0],g+=b[z][1]}z=(z+1)%4}A++}};if(this.strategy==="SPIRAL_FROM_CENTER")yield*H();else{let c=[...H()];for(let G=c.length-1;G>=0;G--)yield c[G]}break}case"SCRIBBLE":{let l=new Set,f=o*p,w=Math.floor(o/2),g=Math.floor(p/2);for(let b=0;l.size<f&&b<f*24;b++){let z=`${w},${g}`;if(!l.has(z))l.add(z),yield{x:w,y:g};if(w+=Math.floor(Math.random()*3)-1,g+=Math.floor(Math.random()*3)-1,w<0||w>=o||g<0||g>=p)w=Math.floor(Math.random()*o),g=Math.floor(Math.random()*p)}for(let b=0;b<p;b++)for(let z=0;z<o;z++){let A=`${z},${b}`;if(l.has(A))continue;l.add(A),yield{x:z,y:b}}break}case"CROSSHATCH":{let l=[];for(let g=0;g<o+p-1;g++)for(let b=Math.max(0,g-o+1);b<=Math.min(p-1,g);b++){let z=g-b;l.push({x:z,y:b})}let f=[];for(let g=-p+1;g<o;g++)for(let b=0;b<p;b++){let z=b+g;if(z>=0&&z<o)f.push({x:z,y:b})}let w=new Set;for(let g of[...l,...f]){let b=`${g.x},${g.y}`;if(w.has(b))continue;w.add(b),yield g}break}case"WAVE_SWEEP":{let l=new Set;for(let f=0;f<o;f++){let g=(Math.sin(f/Math.max(1,o-1)*Math.PI*4)+1)*0.5*(p-1)|0;for(let b=0;b<p;b++){let z=g+b,A=g-b;for(let M of[z,A]){if(M<0||M>=p)continue;let H=`${f},${M}`;if(l.has(H))continue;l.add(H),yield{x:f,y:M}}}}break}case"SCATTERED_LINES":{let l=new Set,f=o*p;for(let w=0;l.size<f&&w<f*14;w++){let g=Math.floor(Math.random()*o),b=Math.floor(Math.random()*p),z=Math.random()*Math.PI*2,A=Math.round(Math.cos(z)),M=Math.round(Math.sin(z)),H=6+Math.floor(Math.random()*28);for(let c=0;c<H;c++){if(g<0||g>=o||b<0||b>=p)break;let G=`${g},${b}`;if(!l.has(G))l.add(G),yield{x:g,y:b};g+=A,b+=M}}for(let w=0;w<p;w++)for(let g=0;g<o;g++){let b=`${g},${w}`;if(l.has(b))continue;l.add(b),yield{x:g,y:w}}break}case"CONTOUR_JITTER":{let l=new Set;for(let f=0;f<Math.ceil(Math.min(o,p)/2);f++){let w=[],g=f,b=f,z=o-f-1,A=p-f-1;for(let M=g;M<=z;M++)w.push({x:M,y:b});for(let M=b+1;M<=A;M++)w.push({x:z,y:M});for(let M=z-1;M>=g;M--)w.push({x:M,y:A});for(let M=A-1;M>b;M--)w.push({x:g,y:M});for(let M=w.length-1;M>0;M--){let H=Math.floor(Math.random()*(M+1)),c=w[M];w[M]=w[H],w[H]=c}for(let M of w){let H=`${M.x},${M.y}`;if(l.has(H))continue;l.add(H),yield M}}break}case"SPIRAL_WOBBLE":{let l=new Set,f=o/2,w=p/2,g=Math.hypot(f,w);for(let b=0;l.size<o*p&&b<o*p*9;b++){let z=b/(o*p*9)*g,A=b*0.31+Math.sin(b*0.07)*0.7,M=Math.round(f+Math.cos(A)*z),H=Math.round(w+Math.sin(A)*z);if(M<0||M>=o||H<0||H>=p)continue;let c=`${M},${H}`;if(l.has(c))continue;l.add(c),yield{x:M,y:H}}for(let b=0;b<p;b++)for(let z=0;z<o;z++){let A=`${z},${b}`;if(l.has(A))continue;yield{x:z,y:b}}break}case"CLUSTER_BURSTS":{let l=new Set,f=o*p;for(let w=0;l.size<f&&w<f*12;w++){let g=Math.floor(Math.random()*o),b=Math.floor(Math.random()*p),z=2+Math.floor(Math.random()*10);for(let A=b-z;A<=b+z;A++)for(let M=g-z;M<=g+z;M++){if(M<0||M>=o||A<0||A>=p)continue;if(Math.hypot(M-g,A-b)>z)continue;let H=`${M},${A}`;if(l.has(H))continue;l.add(H),yield{x:M,y:A}}}for(let w=0;w<p;w++)for(let g=0;g<o;g++){let b=`${g},${w}`;if(l.has(b))continue;l.add(b),yield{x:g,y:w}}break}case"ORBITAL":{let l=new Set,f=(o-1)/2,w=(p-1)/2,g=Math.ceil(Math.max(f,w));for(let b=0;b<=g;b++){let z=Math.max(16,Math.ceil(2*Math.PI*Math.max(1,b)*2));for(let A=0;A<z;A++){let M=A/z*Math.PI*2+(b%2?0.3:-0.3),H=Math.round(f+Math.cos(M)*b),c=Math.round(w+Math.sin(M)*b);if(H<0||H>=o||c<0||c>=p)continue;let G=`${H},${c}`;if(l.has(G))continue;l.add(G),yield{x:H,y:c}}}for(let b=0;b<p;b++)for(let z=0;z<o;z++){let A=`${z},${b}`;if(l.has(A))continue;yield{x:z,y:b}}break}case"FLOW_FIELD":{let l=new Set,f=o*p;for(let w=0;l.size<f&&w<f*18;w++){let g=Math.floor(Math.random()*o),b=Math.floor(Math.random()*p);for(let z=0;z<120;z++){if(g<0||g>=o||b<0||b>=p)break;let A=`${g},${b}`;if(!l.has(A))l.add(A),yield{x:g,y:b};let M=Math.sin(g*0.09)*1.8+Math.cos(b*0.08)*1.6+Math.sin((g+b)*0.05);g+=Math.round(Math.cos(M)),b+=Math.round(Math.sin(M))}}for(let w=0;w<p;w++)for(let g=0;g<o;g++){let b=`${g},${w}`;if(l.has(b))continue;l.add(b),yield{x:g,y:w}}break}case"EDGE_IN":{let l=new Set,f=Math.ceil(Math.min(o,p)/2);for(let w=0;w<f;w++){let g=w,b=o-1-w,z=w,A=p-1-w;for(let M=g;M<=b;M++)for(let H of[z,A]){let c=`${M},${H}`;if(l.has(c))continue;l.add(c),yield{x:M,y:H}}for(let M=z+1;M<=A-1;M++)for(let H of[g,b]){let c=`${H},${M}`;if(l.has(c))continue;l.add(c),yield{x:H,y:M}}}break}}}moveStart(o){if(o.button!==0)return;if(o.preventDefault(),o.stopPropagation(),!this.lock)this.moveInfo={globalX:this.position.globalX,globalY:this.position.globalY,clientX:o.clientX,clientY:o.clientY}}moveStop(){if(this.moveInfo)this.moveInfo=void 0,this.position.updateAnchor(),this.pixels.update(),this.updateColors(),Y(this.bot)}move(o){if(!this.moveInfo)return;let p=Math.round((o.clientX-this.moveInfo.clientX)/this.position.pixelSize),l=Math.round((o.clientY-this.moveInfo.clientY)/this.position.pixelSize);if(this.moveInfo.globalX!==void 0){if(this.position.globalX=p+this.moveInfo.globalX,this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,this.moveInfo.width-p)}else if(this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,p+this.moveInfo.width);if(this.moveInfo.globalY!==void 0){if(this.position.globalY=l+this.moveInfo.globalY,this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,this.moveInfo.height-l)}else if(this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,l+this.moveInfo.height);this.update(),Y(this.bot)}resizeStart(o){if(this.lock||o.button!==0)return;o.preventDefault(),o.stopPropagation(),this.moveInfo={clientX:o.clientX,clientY:o.clientY};let p=o.target;if(p.classList.contains("n"))this.moveInfo.height=this.pixels.height,this.moveInfo.globalY=this.position.globalY;if(p.classList.contains("e"))this.moveInfo.width=this.pixels.width;if(p.classList.contains("s"))this.moveInfo.height=this.pixels.height;if(p.classList.contains("w"))this.moveInfo.width=this.pixels.width,this.moveInfo.globalX=this.position.globalX}export(){let o=document.createElement("a");document.body.append(o),o.href=URL.createObjectURL(new Blob([JSON.stringify(this.toJSON())],{type:"application/json"})),o.download=`${this.pixels.width}x${this.pixels.height}.${n}`,o.click(),URL.revokeObjectURL(o.href),o.href=this.pixels.canvas.toDataURL("image/webp",1),o.download=`${this.pixels.width}x${this.pixels.height}.webp`,o.click(),URL.revokeObjectURL(o.href),o.remove()}}var B0=`/* stylelint-disable declaration-no-important */
...
`;var Z0="kglacer-macro:overlay-hidden",m0="https://raw.githubusercontent.com/robgallardof/kglacer-macro/refs/heads/main/src/img/logo.svg";class b0 extends d{bot;element=document.createElement("div");get status(){return this.$status.innerHTML}set status(o){this.$status.innerHTML=o}get open(){return this.element.classList.contains("wopen")}set open(o){if(o)this.element.classList.add("wopen");else this.element.classList.remove("wopen")}$settings;$status;$shortcuts;$locale;$topbar;$draw;$addImage;$toggleOverlay;$strategy;$progressLine;$progressText;$images;$wopenButton;$widgetLogo;activeImageIndex=-1;constructor(o){super();this.bot=o;this.element.classList.add("wwidget"),this.element.innerHTML=D0,k(this.element),document.body.append(this.element),this.populateElementsWithSelector(this.element,{$wopenButton:".wopen-button",$widgetLogo:".widget-logo",$settings:".wform",$status:".wstatus",$shortcuts:".shortcuts",$locale:".locale",$topbar:".wtopbar",$draw:".draw",$addImage:".add-image",$toggleOverlay:".toggle-overlay",$strategy:".strategy",$progressLine:".wprogress div",$progressText:".wprogress span",$images:".images"}),this.$widgetLogo.src=m0,this.$wopenButton.addEventListener("click",()=>this.open=!this.open),this.$draw.addEventListener("click",()=>this.bot.draw()),this.$addImage.addEventListener("click",()=>this.addImage()),this.$toggleOverlay.addEventListener("click",()=>{this.toggleOverlay()}),this.$strategy.addEventListener("change",()=>{this.bot.strategy=this.$strategy.value}),this.$locale.value=w0(),this.$locale.addEventListener("change",()=>{J0(this.$locale.value),k(this.element);for(let p=0;p<this.bot.images.length;p++)this.bot.images[p].applyLocale();this.refreshOverlayToggleText()}),this.registerEvent(document,"keydown",this.handleKeyboard.bind(this),{passive:!1}),this.update(),this.syncOverlayVisibilityFromStorage(),this.open=!0,console.log("[KGM][Widget] Widget mounted and opened")}addImage(){return console.log("[KGM][Widget] Add image flow started"),this.setDisabled("add-image",!0),this.run("Adding image",async()=>{await this.bot.updateColors();let o=document.createElement("input");o.type="file",o.accept=`image/*,.${n}`,o.click(),await L(o,["change"],["cancel","error"]);let p=o.files?.[0];if(!p)throw new g0(this.bot);console.log("[KGM][Widget] File selected",{name:p.name,size:p.size,type:p.type});let l;if(p.name.endsWith(`.${n}`))l=await C.fromJSON(this.bot,JSON.parse(await p.text()));else{let f=new FileReader;f.readAsDataURL(p),await L(f,["load"],["error"]);let w=await this.compressImageBeforeLoad(f.result),g=new Image;g.src=w,await L(g,["load"],["error"]),l=new C(this.bot,j.fromScreenPosition(this.bot,this.defaultImageScreenPosition()),new V(this.bot,g))}this.bot.images.push(l),console.log("[KGM][Widget] Image instance added",{images:this.bot.images.length}),await this.bot.readMap(),l.updateTasks(),Y(this.bot,!0),console.log("[KGM][Widget] Image persisted, reloading page"),document.location.reload()},()=>{this.setDisabled("add-image",!1)})}defaultImageScreenPosition(){let o=Math.round(this.element.getBoundingClientRect().width);return{x:Math.max(256,o),y:32}}async compressImageBeforeLoad(o){let p=new Image;if(p.src=o,await L(p,["load"],["error"]),!(p.naturalWidth*p.naturalHeight>3000000||o.length>3000000))return o;let f=document.createElement("canvas");f.width=p.naturalWidth,f.height=p.naturalHeight;let w=f.getContext("2d");if(!w)return o;return w.drawImage(p,0,0),f.toDataURL("image/png")}update(){this.$strategy.value=this.bot.strategy;let o=0,p=0;for(let g=0;g<this.bot.images.length;g++){let b=this.bot.images[g];o+=b.pixels.pixels.length*b.pixels.pixels[0].length,p+=b.tasks.length}let l=o-p,f=l/o*100|0;this.$progressText.textContent=`${l}/${o} ${f}% ETA: ${p/120|0}h`,this.$progressLine.style.transform=`scaleX(${f}%)`,this.$images.innerHTML="";let w=document.createDocumentFragment();for(let g=0;g<this.bot.images.length;g++){let b=this.bot.images[g],z=document.createElement("div");w.append(z),z.className="image",z.innerHTML=`<button class="preview" title="Focus image">
