...
`;class R{bot;image;width;exactColor;static async fromJSON(o,a){let l=new Image;return l.src=a.url.startsWith("http")?await fetch(a.url,{cache:"no-store"}).then((r)=>r.blob()).then((r)=>URL.createObjectURL(r)):a.url,await Z(l,["load"],["error"]),new R(o,l,a.width,a.exactColor)}canvas=document.createElement("canvas");context=this.canvas.getContext("2d");pixels;colors=new Map;resolution;get height(){return this.width/this.resolution|0}set height(o){this.width=o*this.resolution|0}constructor(o,a,l=a.naturalWidth,r=!1){this.bot=o;this.image=a;this.width=l;this.exactColor=r;if(r)this.resolution=1,this.width=1000;else this.resolution=this.image.naturalWidth/this.image.naturalHeight;this.update()}update(){this.canvas.width=this.width,this.canvas.height=this.height,this.colors.clear();let o=new Map;for(let l=1;l<64;l++)if(this.exactColor||!this.bot.unavailableColors.has(l))o.set(W[l],[l,l]);this.context.imageSmoothingEnabled=!1,this.context.imageSmoothingQuality="low",this.context.drawImage(this.image,0,0,this.canvas.width,this.canvas.height),this.pixels=Array.from({length:this.canvas.height},()=>new Array(this.canvas.width));let a=this.context.getImageData(0,0,this.canvas.width,this.canvas.height).data;for(let l=0;l<this.canvas.height;l++)for(let r=0;r<this.canvas.width;r++){let p=(l*this.canvas.width+r)*4,g=a[p],f=a[p+1],c=a[p+2],s=a[p+3],w=g,u=f,n=c,d=`${w},${u},${n}`;if(this.exactColor){this.pixels[l][r]=s<100?0:W.indexOf(d);continue}let b,m;if(s<100)b=m=0;else if(o.has(d))[b,m]=o.get(d);else{let A=1/0,M=1/0;for(let H=0;H<V.length;H++){let N=V[H],D=zo(mo(w,u,n),N,0);if(!this.bot.unavailableColors.has(H)&&D<A)A=D,b=H;if(D<M)M=D,m=H}o.set(d,[b,m])}if(b!==0)this.context.fillStyle=`oklab(${V[b][0]*100}% ${V[b][1]} ${V[b][2]})`,this.context.fillRect(r,l,1,1);this.pixels[l][r]=b;let z=this.colors.get(m);if(z)z.amount++;else this.colors.set(m,{color:b,amount:1,realColor:m})}}toJSON(){let o=document.createElement("canvas");return o.width=this.image.naturalWidth,o.height=this.image.naturalHeight,o.getContext("2d").drawImage(this.image,0,0),{url:o.toDataURL("image/webp",1),width:this.width,exactColor:this.exactColor}}}var L="kglacer-macro-settings",Ho=["kglacermacro","wbot"],X="kgm";function Bo(){let o=[L,...Ho];for(let a=0;a<o.length;a++){let l=o[a],r=localStorage.getItem(l);if(!r)continue;return{json:r,key:l}}return}function Po(){let o=Bo();if(!o)return;let a;try{if(a=JSON.parse(o.json),typeof a!=="object")throw new Error("NOT VALID SAVE");if(a.version===1){let l=a;a.images=l.widget.images,a.strategy=l.widget.strategy,delete l.widget}if(o.key!==L)localStorage.setItem(L,o.json)}catch{localStorage.removeItem(o.key),a=void 0}return a}var Mo;function J(o,a=!1){if(clearTimeout(Mo),a)localStorage.setItem(L,JSON.stringify(o));else Mo=setTimeout(()=>{localStorage.setItem(L,JSON.stringify(o))},600)}var P=1000,Lo=2048,h=P*Lo,S=[],T=[],ho=Date.now();function I(o){S.push(o),T.push({id:ho++,latitude:(2*Math.atan(Math.exp(-(o.y/h*(2*Math.PI)-Math.PI)))-Math.PI/2)*180/Math.PI,longitude:(o.x/h*(2*Math.PI)-Math.PI)*180/Math.PI,name:"KGLACER_MACRO_FAVORITE"})}I({x:h/3|0,y:h/3|0});I({x:h/3*2|0,y:h/3*2|0});function k(o){let[a,l]=o.style.transform.slice(32,-31).split(", ").map((r)=>Number.parseFloat(r));return{x:a,y:l}}class F{bot;static fromJSON(o,a){return new F(o,...a)}static fromScreenPosition(o,a){let{anchorScreenPosition:l,pixelSize:r,anchorWorldPosition:p}=o.findAnchorsForScreen(a);return new F(o,p.x+(a.x-l.x)/r|0,p.y+(a.y-l.y)/r|0)}globalX=0;globalY=0;get tileX(){return this.globalX/P|0}set tileX(o){this.globalX=o*P+this.x}get tileY(){return this.globalY/P|0}set tileY(o){this.globalY=o*P+this.y}get x(){return this.globalX%P}set x(o){this.globalX=this.tileX*P+o}get y(){return this.globalY%P}set y(o){this.globalY=this.tileY*P+o}anchor1Index;anchor2Index;get pixelSize(){return(k(this.bot.$stars[this.anchor2Index]).x-k(this.bot.$stars[this.anchor1Index]).x)/(S[this.anchor2Index].x-S[this.anchor1Index].x)}constructor(o,a,l,r,p){this.bot=o;if(r===void 0||p===void 0)this.globalX=a,this.globalY=l;else this.globalX=a*P+r,this.globalY=l*P+p;this.updateAnchor()}updateAnchor(){this.anchor1Index=0,this.anchor2Index=1;let o=1/0,a=1/0;for(let l=0;l<S.length;l++){let{x:r,y:p}=S[l];if(r<this.globalX&&p<this.globalY){let g=this.globalX-r+(this.globalY-p);if(g<o)o=g,this.anchor1Index=l}else if(r>this.globalX&&p>this.globalY){let g=r-this.globalX+(p-this.globalY);if(g<a)a=g,this.anchor2Index=l}}}toScreenPosition(){let o=S[this.anchor1Index],a=k(this.bot.$stars[this.anchor1Index]);return{x:(this.globalX-o.x)*this.pixelSize+a.x,y:(this.globalY-o.y)*this.pixelSize+a.y}}getMapColor(){return this.bot.mapsCache.get(this.tileX+"/"+this.tileY).pixels[this.y][this.x]}scrollScreenTo(){let{x:o,y:a}=this.toScreenPosition();this.bot.moveMap({x:o-window.innerWidth/3,y:a-window.innerHeight/3})}clone(){return new F(this.bot,this.tileX,this.tileY,this.x,this.y)}toJSON(){return[this.globalX,this.globalY]}}class q extends _{bot;position;pixels;strategy;opacity;drawTransparentPixels;drawColorsInOrder;colors;lock;static PREVIEW_MASK_BASE_WIDTH=96;static PREVIEW_MASK_BASE_HEIGHT=96;static async fromJSON(o,a){return new q(o,F.fromJSON(o,a.position),await R.fromJSON(o,a.pixels),a.strategy,a.opacity,a.drawTransparentPixels,a.drawColorsInOrder,a.colors,a.lock)}element=document.createElement("div");tasks=[];moveInfo;$canvas;$colorsDialog;$colorsDialogList;$colorSearch;$openColors;$openPreview;$closeColors;$closePreview;$delete;$drawColorsInOrder;$drawTransparent;$export;$lock;$opacity;$progressLine;$progressText;$previewDialog;$previewDialogList;$resetSize;$resetSizeSpan;$settings;$strategy;$topbar;$wrapper;colorDialogDragState;suppressNextColorDialogBackdropClick=!1;previewCacheSignature;previewSequenceCache=new Map;previewAnimations=new WeakMap;previewAnimationHandles=new Set;constructor(o,a,l,r="SPIRAL_FROM_CENTER",p=50,g=!1,f=!1,c=[],s=!1){super();this.bot=o;this.position=a;this.pixels=l;this.strategy=r;this.opacity=p;this.drawTransparentPixels=g;this.drawColorsInOrder=f;this.colors=c;this.lock=s;this.element.innerHTML=Ao,this.element.classList.add("wimage"),t(this.element),document.body.append(this.element),this.populateElementsWithSelector(this.element,{$colorsDialog:".colors-dialog",$colorsDialogList:".colors-dialog-list",$colorSearch:".color-search",$openColors:".open-colors",$openPreview:".open-preview",$closeColors:".close-colors",$closePreview:".close-preview",$delete:".delete",$drawColorsInOrder:".draw-colors-in-order",$drawTransparent:".draw-transparent",$export:".export",$lock:".lock",$opacity:".opacity",$progressLine:".wprogress div",$progressText:".wprogress span",$previewDialog:".preview-dialog",$previewDialogList:".preview-dialog-list",$resetSize:".reset-size",$settings:".wform",$strategy:".strategy",$topbar:".wtopbar",$wrapper:".wrapper"}),this.$resetSizeSpan=this.$resetSize.querySelector("span"),this.$canvas=this.pixels.canvas,this.$wrapper.prepend(this.pixels.canvas),document.body.append(this.$colorsDialog,this.$previewDialog),this.registerEvent(this.$strategy,"change",()=>{if(this.strategy=this.$strategy.value,this.$previewDialog.open)this.renderStrategyPreviewSamples();J(this.bot)}),this.registerEvent(this.$opacity,"input",()=>{this.opacity=this.$opacity.valueAsNumber,this.$opacity.style.setProperty("--val",this.opacity+"%"),this.update(),J(this.bot)}),this.$opacity.style.setProperty("--val",this.opacity+"%"),this.registerEvent(this.$resetSize,"click",()=>{this.pixels.width=this.pixels.image.naturalWidth,this.pixels.update(),this.updateColors(),this.update(),J(this.bot)}),this.registerEvent(this.$drawTransparent,"click",()=>{this.drawTransparentPixels=this.$drawTransparent.checked,J(this.bot)}),this.registerEvent(this.$drawColorsInOrder,"click",()=>{this.drawColorsInOrder=this.$drawColorsInOrder.checked,J(this.bot)}),this.registerEvent(this.$lock,"click",()=>{this.lock=!this.lock,this.update(),J(this.bot)}),this.registerEvent(this.$delete,"click",this.destroy.bind(this)),this.registerEvent(this.$openColors,"click",()=>{this.openColorPanel()}),this.registerEvent(this.$openPreview,"click",()=>{this.openPreviewPanel()}),this.registerEvent(this.$closeColors,"click",()=>{this.closeDialog(this.$colorsDialog)}),this.registerEvent(this.$closePreview,"click",()=>{this.closeDialog(this.$previewDialog)}),this.registerEvent(this.$colorsDialog.querySelector(".colors-dialog-head"),"pointerdown",this.startColorDialogDrag.bind(this)),this.registerEvent(document,"pointermove",this.moveColorDialog.bind(this),{passive:!1}),this.registerEvent(document,"pointerup",this.stopColorDialogDrag.bind(this)),this.registerEvent(document,"pointercancel",this.stopColorDialogDrag.bind(this)),this.registerEvent(this.$colorsDialog,"click",(w)=>{if(this.suppressNextColorDialogBackdropClick){this.suppressNextColorDialogBackdropClick=!1;return}if(w.target===this.$colorsDialog)this.closeDialog(this.$colorsDialog)}),this.registerEvent(this.$previewDialog,"click",(w)=>{if(w.target===this.$previewDialog)this.closeDialog(this.$previewDialog)}),this.registerEvent(this.$colorSearch,"input",()=>{this.updateColors()}),this.registerEvent(this.$export,"click",this.export.bind(this)),this.registerEvent(this.$topbar,"mousedown",this.moveStart.bind(this)),this.registerEvent(this.$canvas,"mousedown",this.moveStart.bind(this)),this.registerEvent(document,"mouseup",this.moveStop.bind(this)),this.registerEvent(document,"mousemove",this.move.bind(this));for(let w of this.element.querySelectorAll(".resize"))this.registerEvent(w,"mousedown",this.resizeStart.bind(this));this.update(),this.updateColors()}toJSON(){return{pixels:this.pixels.toJSON(),position:this.position.toJSON(),strategy:this.strategy,opacity:this.opacity,drawTransparentPixels:this.drawTransparentPixels,drawColorsInOrder:this.drawColorsInOrder,colors:this.colors,lock:this.lock}}updateTasks(){this.tasks.length=0;let o=this.position.clone(),a=new Set,l=new Map;for(let r=0;r<this.colors.length;r++){let p=this.colors[r];if(p.disabled)a.add(p.realColor);l.set(p.realColor,r)}for(let{x:r,y:p}of this.strategyPositionIterator()){let g=this.pixels.pixels[p][r];if(a.has(g))continue;o.globalX=this.position.globalX+r,o.globalY=this.position.globalY+p;let f=o.getMapColor();if(g!==f&&(this.drawTransparentPixels||g!==0))this.tasks.push({position:o.clone(),color:g})}if(this.drawColorsInOrder)this.tasks.sort((r,p)=>(l.get(r.color)??0)-(l.get(p.color)??0));this.update(),this.bot.widget.update()}update(){let{x:o,y:a}=this.position.toScreenPosition(),l=this.position.pixelSize*this.pixels.width,r=this.position.pixelSize*this.pixels.height;this.element.style.transform=`translate3d(${o.toFixed(3)}px, ${a.toFixed(3)}px, 0)`,this.element.style.width=`${l}px`,this.element.style.height=`${r}px`,this.$canvas.style.opacity=`${this.opacity}%`,this.element.classList.remove("hidden"),this.$resetSizeSpan.textContent=this.pixels.width.toString(),this.$strategy.value=this.strategy,this.$opacity.valueAsNumber=this.opacity,this.$drawTransparent.checked=this.drawTransparentPixels,this.$drawColorsInOrder.checked=this.drawColorsInOrder;let p=this.pixels.pixels.length*this.pixels.pixels[0].length,g=Math.max(0,p-this.tasks.length),f=p>0?g/p*100|0:0;this.$progressText.textContent=`${g}/${p} ${f}% ETA: ${this.tasks.length/120|0}h`,this.$progressLine.style.transform=`scaleX(${f/100})`,this.$canvas.classList[this.lock?"add":"remove"]("no-pointer-events");for(let c of this.element.querySelectorAll(".resize"))c.classList[this.lock?"add":"remove"]("no-pointer-events");this.$lock.classList[this.lock?"add":"remove"]("locked")}exportImage(){this.export()}destroy(){super.destroy(),this.element.remove(),this.$colorsDialog.remove(),this.$previewDialog.remove(),ro(this.bot.images,this),this.bot.widget.update(),J(this.bot)}openColorPanel(){if(this.$colorsDialog.open){this.$colorSearch.focus();return}this.$colorsDialog.style.position="fixed",this.$colorsDialog.style.left="",this.$colorsDialog.style.top="",this.$colorsDialog.style.margin="auto",this.$colorsDialog.showModal(),this.$colorSearch.focus()}openPreviewPanel(){if(this.$previewDialog.open){this.renderStrategyPreviewSamples();return}this.$previewDialog.style.position="fixed",this.$previewDialog.style.left="",this.$previewDialog.style.top="",this.$previewDialog.style.margin="auto",this.$previewDialog.showModal(),this.renderStrategyPreviewSamples()}closeDialog(o){if(!o.open)return;if(o===this.$previewDialog)this.stopPreviewAnimations();if(typeof o.requestClose==="function")o.requestClose();else o.close()}stopPreviewAnimations(){for(let o of this.previewAnimationHandles)cancelAnimationFrame(o);this.previewAnimationHandles.clear()}startColorDialogDrag(o){if(o.button!==0)return;if(o.target?.closest("button,input,select,textarea,a,label"))return;let l=this.$colorsDialog.getBoundingClientRect();this.colorDialogDragState={pointerId:o.pointerId,offsetX:o.clientX-l.left,offsetY:o.clientY-l.top,moved:!1},o.preventDefault()}moveColorDialog(o){if(!this.colorDialogDragState)return;if(o.pointerId!==this.colorDialogDragState.pointerId)return;let a=this.$colorsDialog.getBoundingClientRect(),l=Math.max(8,window.innerWidth-a.width-8),r=Math.max(8,window.innerHeight-a.height-8),p=Math.min(l,Math.max(8,o.clientX-this.colorDialogDragState.offsetX)),g=Math.min(r,Math.max(8,o.clientY-this.colorDialogDragState.offsetY));if(!this.colorDialogDragState.moved&&(Math.abs(o.movementX)>0||Math.abs(o.movementY)>0))this.colorDialogDragState.moved=!0;this.$colorsDialog.style.left=`${Math.round(p)}px`,this.$colorsDialog.style.top=`${Math.round(g)}px`,o.preventDefault()}stopColorDialogDrag(o){if(!this.colorDialogDragState)return;if(o.pointerId!==this.colorDialogDragState.pointerId)return;if(this.colorDialogDragState.moved)this.suppressNextColorDialogBackdropClick=!0;this.colorDialogDragState=void 0}renderStrategyPreviewSamples(){this.stopPreviewAnimations(),this.invalidatePreviewCacheIfNeeded();let o=this.$strategy.value;this.$previewDialogList.innerHTML="";let a=document.createDocumentFragment(),l=document.createElement("article");l.className="preview-card";let r=document.createElement("strong");r.textContent=this.getStrategyLabel(o);let p=document.createElement("canvas");p.className="preview-canvas",p.width=156,p.height=156,this.paintStrategyPreview(p,o),l.append(r,p),a.append(l),this.$previewDialogList.append(a)}invalidatePreviewCacheIfNeeded(){let o=this.colors.map((l,r)=>`${r}:${l.realColor}:${l.disabled?1:0}`).join("|"),a=`${this.pixels.width}x${this.pixels.height}:${this.pixels.image.src.length}:${this.drawColorsInOrder?1:0}:${o}`;if(this.previewCacheSignature===a)return;this.previewCacheSignature=a,this.previewSequenceCache.clear()}getStrategyLabel(o){switch(o){case"RANDOM":return i("random");case"HUMANIZED":return i("humanized");case"HUMAN_SOFT_DITHER":return i("humanSoftDither");case"HUMAN_PATCHY":return i("humanPatchy");case"HUMAN_SWEEP_ARCS":return i("humanSweepArcs");case"HUMAN_MICRO_CORRECTIONS":return i("humanMicroCorrections");case"HUMAN_JITTER_FILL":return i("humanJitterFill");case"HUMAN_CORNER_BIAS":return i("humanCornerBias");case"HUMAN_LONG_STROKES":return i("humanLongStrokes");case"HUMAN_TAP_CLUSTERS":return i("humanTapClusters");case"HUMAN_MESSY_SPIRAL":return i("humanMessySpiral");case"HUMAN_DRUNK_WALK":return i("humanDrunkWalk");case"HUMAN_NOISE_CLOUD":return i("humanNoiseCloud");case"HUMAN_PATCH_JUMP":return i("humanPatchJump");case"HUMAN_HESITANT_LINES":return i("humanHesitantLines");case"HUMAN_OVERLAP_SWEEPS":return i("humanOverlapSweeps");case"HUMAN_WOBBLE_DRIFT":return i("humanWobbleDrift");case"HUMAN_GAP_RECOVERY":return i("humanGapRecovery");case"HUMAN_STAIRCASE":return i("humanStaircase");case"HUMAN_EDGE_HUGGER":return i("humanEdgeHugger");case"HUMAN_BLOBS":return i("humanBlobs");case"HUMAN_BACKTRACK":return i("humanBacktrack");case"HUMAN_SHAKY_DIAGONAL":return i("humanShakyDiagonal");case"HUMAN_LATE_FIXES":return i("humanLateFixes");case"ZIGZAG":return i("zigzag");case"BRUSH_STROKES":return i("brushStrokes");case"DIAGONAL_BRUSH":return i("diagonalBrush");case"DOWN":return i("down");case"UP":return i("up");case"LEFT":return i("left");case"RIGHT":return i("right");case"SPIRAL_FROM_CENTER":return i("spiralOut");case"SPIRAL_TO_CENTER":return i("spiralIn");case"SCRIBBLE":return i("scribble");case"CROSSHATCH":return i("crosshatch");case"WAVE_SWEEP":return i("waveSweep");case"SCATTERED_LINES":return i("scatteredLines");case"CONTOUR_JITTER":return i("contourJitter");case"SPIRAL_WOBBLE":return i("spiralWobble");case"CLUSTER_BURSTS":return i("clusterBursts");case"ORBITAL":return i("orbital");case"FLOW_FIELD":return i("flowField");case"EDGE_IN":return i("edgeIn");default:return o}}paintStrategyPreview(o,a){let l=o.getContext("2d");if(!l)return;l.fillStyle="#0f1526",l.fillRect(0,0,o.width,o.height);let r=this.getSampledImagePreviewData(),p=this.getCachedPreviewSequence(a,r.mask,r.width,r.height),g=Math.min(o.width/r.width,o.height/r.height),f=(o.width-r.width*g)/2,c=(o.height-r.height*g)/2,s=this.previewAnimations.get(o);if(s)cancelAnimationFrame(s),this.previewAnimationHandles.delete(s);let w=(A)=>{let M=requestAnimationFrame((H)=>{this.previewAnimationHandles.delete(M),A(H)});return this.previewAnimationHandles.add(M),M},u=(A)=>{l.fillStyle="#0f1526",l.fillRect(0,0,o.width,o.height);for(let M=0;M<Math.min(A,p.length);M++){let H=p[M],N=r.colors.get(`${H.x}:${H.y}`)??0;if(!N)continue;l.fillStyle=fo(N),l.fillRect(f+H.x*g,c+H.y*g,Math.max(1,g),Math.max(1,g))}},n=Math.min(3400,Math.max(900,p.length*8)),b=n+220,m=(A,M)=>{if(!this.$previewDialog.open)return;let H=(M-A)%b,N=Math.min(1,H/n),D=N*N*(3-2*N);u(Math.floor(p.length*D));let j=w((C)=>{m(A,C)});this.previewAnimations.set(o,j)},z=performance.now();m(z,z)}getCachedPreviewSequence(o,a,l=this.pixels.width,r=this.pixels.height){let p=this.colors.map((s,w)=>`${w}:${s.realColor}:${s.disabled?1:0}`).join("|"),g=`${o}:${l}x${r}:${a.length}:${this.drawColorsInOrder?1:0}:${p}`,f=this.previewSequenceCache.get(g);if(f)return f;let c=l===this.pixels.width&&r===this.pixels.height?this.getExactPreviewSequence(o,a):this.getApproxPreviewSequence(o,a,l);if(this.drawColorsInOrder){let s=new Map;for(let w=0;w<this.colors.length;w++)s.set(this.colors[w].realColor,w);c.sort((w,u)=>(s.get(this.pixels.pixels[w.y]?.[w.x]??0)??0)-(s.get(this.pixels.pixels[u.y]?.[u.x]??0)??0))}return this.previewSequenceCache.set(g,c),c}getExactPreviewSequence(o,a){let l=this.strategy;this.strategy=o;let r=[...this.strategyPositionIterator()];this.strategy=l;let p=new Set(a.map(({x:g,y:f})=>`${g}:${f}`));return r.filter(({x:g,y:f})=>p.has(`${g}:${f}`))}getApproxPreviewSequence(o,a,l){let r=[...a],p=(c,s,w)=>{return(c*73856093+s*19349663+w*83492791>>>0)/4294967296},g=(c,s)=>r.sort((w,u)=>w.x*c+w.y*s-(u.x*c+u.y*s)||w.y-u.y||w.x-u.x),f=r.sort((c,s)=>{if(c.y!==s.y)return c.y-s.y;let w=c.y%2===0?c.x:l-c.x,u=s.y%2===0?s.x:l-s.x;return w-u});switch(o){case"UP":return g(0,-1);case"LEFT":return g(-1,0);case"RIGHT":return g(1,0);case"SPIRAL_FROM_CENTER":case"SPIRAL_TO_CENTER":{let c=l/2,s=Math.max(1,Math.round(r.reduce((w,u)=>w+u.y,0)/Math.max(1,r.length)));return r.sort((w,u)=>{let n=(w.x-c)**2+(w.y-s)**2,d=(u.x-c)**2+(u.y-s)**2;return o==="SPIRAL_FROM_CENTER"?n-d:d-n}),r}case"RANDOM":case"HUMANIZED":case"HUMAN_SOFT_DITHER":case"HUMAN_PATCHY":case"HUMAN_SWEEP_ARCS":case"HUMAN_MICRO_CORRECTIONS":case"HUMAN_JITTER_FILL":case"HUMAN_CORNER_BIAS":case"HUMAN_LONG_STROKES":case"HUMAN_TAP_CLUSTERS":case"HUMAN_MESSY_SPIRAL":case"HUMAN_DRUNK_WALK":case"HUMAN_NOISE_CLOUD":case"HUMAN_PATCH_JUMP":case"HUMAN_HESITANT_LINES":case"HUMAN_OVERLAP_SWEEPS":case"HUMAN_WOBBLE_DRIFT":case"HUMAN_GAP_RECOVERY":case"HUMAN_STAIRCASE":case"HUMAN_EDGE_HUGGER":case"HUMAN_BLOBS":case"HUMAN_BACKTRACK":case"HUMAN_SHAKY_DIAGONAL":case"HUMAN_LATE_FIXES":return r.sort((c,s)=>p(c.x,c.y,o.length)-p(s.x,s.y,o.length));default:return f}}getSampledImagePreviewData(){let o=this.pixels.width,a=this.pixels.height,l=q.PREVIEW_MASK_BASE_WIDTH,r=q.PREVIEW_MASK_BASE_HEIGHT,p=Math.min(1,l/Math.max(1,o),r/Math.max(1,a)),g=Math.max(1,Math.round(o*p)),f=Math.max(1,Math.round(a*p)),c=new Set;for(let n=0;n<this.colors.length;n++){let d=this.colors[n];if(d.disabled)c.add(d.realColor)}let s=new Map,w=new Map;for(let n=0;n<a;n++)for(let d=0;d<o;d++){let b=this.pixels.pixels[n]?.[d]??0;if(!b||c.has(b))continue;let m=Math.min(g-1,Math.floor(d/o*g)),z=Math.min(f-1,Math.floor(n/a*f)),A=`${m}:${z}`;if(!s.has(A))s.set(A,{x:m,y:z});if(!w.has(A))w.set(A,b)}let u=[...s.values()];if(!u.length){let n=this.fallbackPreviewMask();return{width:o,height:a,mask:n,colors:new Map(n.map((d)=>[`${d.x}:${d.y}`,this.pixels.pixels[d.y]?.[d.x]??0]))}}return{width:g,height:f,mask:u,colors:w}}getImagePreviewMask(){let o=this.pixels.width,a=this.pixels.height,l=new Set;for(let p=0;p<this.colors.length;p++){let g=this.colors[p];if(g.disabled)l.add(g.realColor)}let r=[];for(let p=0;p<a;p++)for(let g=0;g<o;g++){let f=this.pixels.pixels[p]?.[g]??0;if(f!==0&&!l.has(f))r.push({x:g,y:p})}return r.length?r:this.fallbackPreviewMask()}fallbackPreviewMask(){let o=[],a=this.pixels.width/2,l=this.pixels.height/2,r=Math.max(4,Math.min(this.pixels.width,this.pixels.height)/2.5);for(let p=0;p<this.pixels.height;p++)for(let g=0;g<this.pixels.width;g++)if((g-a)**2+(p-l)**2<=r**2)o.push({x:g,y:p});return o}applyLocale(){if(t(this.element),this.updateColors(),this.$previewDialog.open)this.renderStrategyPreviewSamples()}colorHex(o){let a=W[o]??"0,0,0",[l=0,r=0,p=0]=a.split(",").map((g)=>Number.parseInt(g,10));return`#${[l,r,p].map((g)=>g.toString(16).padStart(2,"0")).join("")}`}colorKeywords(o){let a=W[o]??"0,0,0",[l=0,r=0,p=0]=a.split(",").map((s)=>Number.parseInt(s,10)),g=Math.max(l,r,p),f=Math.min(l,r,p);if(g-f<15)return["gray","grey","gris","neutral","neutro"];if(l>r+30&&l>p+30)return["red","rojo"];if(r>l+30&&r>p+30)return["green","verde"];if(p>l+30&&p>r+30)return["blue","azul"];if(l>170&&r>120&&p<130)return["orange","naranja"];if(l>170&&r>110&&p>140)return["pink","rosa"];if(l>120&&r<100&&p>120)return["purple","violet","morado"];if(l>130&&r>130&&p<90)return["yellow","amarillo"];return["brown","cafe","marron"]}updateColors(){this.$colorsDialogList.innerHTML="";let o=this.pixels.pixels.length*this.pixels.pixels[0].length;this.$colorsDialogList.setAttribute("aria-label",i("colorPanelResults"));let a=this.$colorSearch.value.trim().toLowerCase();if(this.colors.length!==this.pixels.colors.size||this.colors.some((l)=>!this.pixels.colors.has(l.realColor)))this.colors=this.pixels.colors.values().toArray().sort((l,r)=>r.amount-l.amount).map((l)=>({realColor:l.realColor,disabled:!1})),J(this.bot);for(let l=0;l<this.colors.length;l++){let r=this.colors[l],p=this.pixels.colors.get(r.realColor),g=!1,f=p.realColor!==p.color,c=p.amount/o*100,s=this.colorHex(p.realColor),w=this.colorKeywords(p.realColor),u=()=>{r.disabled=r.disabled?void 0:!0,n.classList.toggle("disabled",Boolean(r.disabled));let b=n.querySelector(".state");if(b)b.textContent=r.disabled?i("disabled"):i("enabled");J(this.bot)},n=document.createElement("button");if(n.className=`color-chip ${r.disabled?"disabled":""}`,n.draggable=!0,n.setAttribute("aria-label",`${i("overlayColors")} #${l+1}: ${s.toUpperCase()}`),n.innerHTML=`<span class="order-index">#${l+1}</span>
...
<span class="premium ${f?"on":""}">${f?i("premium"):""}</span>`,n.querySelector(".swatch").style.setProperty("--swatch-color",fo(p.realColor)),n.addEventListener("click",()=>{if(g){g=!1;return}u()}),n.addEventListener("dragstart",(b)=>{g=!0,n.classList.add("dragging"),b.dataTransfer?.setData("text/plain",String(l)),b.dataTransfer.effectAllowed="move"}),n.addEventListener("dragend",()=>{n.classList.remove("dragging")}),n.addEventListener("dragover",(b)=>{b.preventDefault(),n.classList.add("drag-target")}),n.addEventListener("dragleave",()=>{n.classList.remove("drag-target")}),n.addEventListener("drop",(b)=>{b.preventDefault(),n.classList.remove("drag-target");let m=Number.parseInt(b.dataTransfer?.getData("text/plain")??"-1",10);if(m<0||m===l||m>=this.colors.length)return;this.colors.splice(l,0,...this.colors.splice(m,1)),J(this.bot),this.updateColors()}),f){let b=document.createElement("button");b.textContent=i("buy"),b.className="buy-chip",b.addEventListener("click",(m)=>{m.stopPropagation(),document.getElementById("color-"+p.realColor)?.click()}),n.append(b)}let d=`${s} ${w.join(" ")} ${p.realColor} ${W[p.realColor]}`;if(!a||d.toLowerCase().includes(a))this.$colorsDialogList.append(n)}}*strategyPositionIterator(){let o=this.pixels.pixels[0].length,a=this.pixels.pixels.length;switch(this.strategy){case"DOWN":{for(let l=0;l<a;l++)for(let r=0;r<o;r++)yield{x:r,y:l};break}case"UP":{for(let l=a-1;l>=0;l--)for(let r=0;r<o;r++)yield{x:r,y:l};break}case"LEFT":{for(let l=0;l<o;l++)for(let r=0;r<a;r++)yield{x:l,y:r};break}case"RIGHT":{for(let l=o-1;l>=0;l--)for(let r=0;r<a;r++)yield{x:l,y:r};break}case"RANDOM":{let l=[];for(let r=0;r<a;r++)for(let p=0;p<o;p++)l.push({x:p,y:r});for(let r=l.length-1;r>=0;r--){let p=Math.floor(Math.random()*(r+1)),g=l[r];l[r]=l[p],l[p]=g}yield*l;break}case"ZIGZAG":{for(let l=0;l<a;l++)if(l%2===0)for(let r=0;r<o;r++)yield{x:r,y:l};else for(let r=o-1;r>=0;r--)yield{x:r,y:l};break}case"HUMANIZED":{let l=Math.max(4,Math.floor(Math.min(o,a)/10)),r=[];for(let p=0;p<a;p+=l)for(let g=0;g<o;g+=l)r.push({x:g,y:p});for(let p=r.length-1;p>=0;p--){let g=Math.floor(Math.random()*(p+1)),f=r[p];r[p]=r[g],r[g]=f}for(let p=0;p<r.length;p++){let g=r[p],f=Math.min(a,g.y+l),c=Math.min(o,g.x+l);for(let s=g.y;s<f;s++)if(Math.random()>0.35)for(let u=g.x;u<c;u++)yield{x:u,y:s};else for(let u=c-1;u>=g.x;u--)yield{x:u,y:s}}break}case"HUMAN_SOFT_DITHER":{let l=new Set;for(let r=0;r<a;r++){let p=Math.floor(Math.random()*3)-1;if((r+p)%2===0)for(let f=0;f<o;f+=2)l.add(`${f},${r}`),yield{x:f,y:r};else for(let f=1;f<o;f+=2)l.add(`${f},${r}`),yield{x:f,y:r}}for(let r=0;r<a;r++)for(let p=0;p<o;p++){let g=`${p},${r}`;if(l.has(g))continue;yield{x:p,y:r}}break}case"HUMAN_PATCHY":{let l=new Set,r=o*a;while(l.size<r){let p=Math.floor(Math.random()*o),g=Math.floor(Math.random()*a),f=1+Math.floor(Math.random()*5);for(let c=g-f;c<=g+f;c++)for(let s=p-f;s<=p+f;s++){if(s<0||s>=o||c<0||c>=a)continue;if(Math.hypot(s-p,c-g)>f+Math.random()*1.2)continue;let w=`${s},${c}`;if(l.has(w))continue;l.add(w),yield{x:s,y:c}}}break}case"HUMAN_SWEEP_ARCS":{let l=new Set,r=(o-1)/2,p=(a-1)/2,g=Math.hypot(r,p);for(let f=0;f<4;f++){let c=Math.random()*Math.PI*2;for(let s=0;s<=g;s+=0.35){let w=Math.PI/2+Math.random()*(Math.PI/1.5),u=Math.max(10,Math.floor(s*8));for(let n=0;n<u;n++){let d=c+w*n/u+Math.sin(s)*0.08,b=Math.round(r+Math.cos(d)*s),m=Math.round(p+Math.sin(d)*s);if(b<0||b>=o||m<0||m>=a)continue;let z=`${b},${m}`;if(l.has(z))continue;l.add(z),yield{x:b,y:m}}}}for(let f=0;f<a;f++)for(let c=0;c<o;c++){let s=`${c},${f}`;if(l.has(s))continue;yield{x:c,y:f}}break}case"HUMAN_MICRO_CORRECTIONS":{let l=new Set;for(let r=0;r<a;r++){let p=r%2===0?1:-1,g=p>0?0:o-1;for(let f=0;f<o;f++){let c=g+(Math.random()>0.82?p:0),s=r+(Math.random()>0.9?1:0);for(let w of[{x:g,y:r},{x:c,y:r},{x:g,y:s}]){if(w.x<0||w.x>=o||w.y<0||w.y>=a)continue;let u=`${w.x},${w.y}`;if(l.has(u))continue;l.add(u),yield w}g+=p}}for(let r=0;r<a;r++)for(let p=0;p<o;p++){let g=`${p},${r}`;if(l.has(g))continue;yield{x:p,y:r}}break}case"HUMAN_JITTER_FILL":{let l=[];for(let r=0;r<a;r++)for(let p=0;p<o;p++)l.push({x:p,y:r});l.sort((r,p)=>{let g=r.y+(Math.random()-0.5)*1.8,f=p.y+(Math.random()-0.5)*1.8;if(g!==f)return g-f;return r.x+(Math.random()-0.5)*2-(p.x+(Math.random()-0.5)*2)}),yield*l;break}case"HUMAN_CORNER_BIAS":{let l=[{x:0,y:0},{x:o-1,y:0},{x:0,y:a-1},{x:o-1,y:a-1}],r=l[Math.floor(Math.random()*l.length)],p=[];for(let g=0;g<a;g++)for(let f=0;f<o;f++){let s=Math.hypot(f-r.x,g-r.y)+Math.random()*3.5;p.push({point:{x:f,y:g},score:s})}p.sort((g,f)=>g.score-f.score);for(let g of p)yield g.point;break}case"HUMAN_LONG_STROKES":{let l=new Set,r=o*a;while(l.size<r){let p=Math.floor(Math.random()*o),g=Math.floor(Math.random()*a),f=Math.random()*Math.PI*2,c=Math.sign(Math.cos(f)),s=Math.sign(Math.sin(f)),w=10+Math.floor(Math.random()*40);for(let u=0;u<w;u++){if(p<0||p>=o||g<0||g>=a)break;let n=`${p},${g}`;if(!l.has(n))l.add(n),yield{x:p,y:g};if(Math.random()>0.78)p+=s,g+=c;else p+=c,g+=s}}break}case"HUMAN_TAP_CLUSTERS":{let l=new Set,r=o*a;while(l.size<r){let p=Math.floor(Math.random()*o),g=Math.floor(Math.random()*a),f=3+Math.floor(Math.random()*10);for(let c=0;c<f;c++){let s=Math.round(p+(Math.random()-0.5)*6),w=Math.round(g+(Math.random()-0.5)*6);if(s<0||s>=o||w<0||w>=a)continue;let u=`${s},${w}`;if(l.has(u))continue;l.add(u),yield{x:s,y:w}}}break}case"HUMAN_MESSY_SPIRAL":{let l=new Set,r=(o-1)/2,p=(a-1)/2,g=Math.hypot(r,p)+2;for(let f=0;l.size<o*a;f++){let c=f/3,s=Math.min(g,c*0.18),w=c*0.29+Math.sin(c*0.13)*0.8,u=Math.round(r+Math.cos(w)*s+Math.sin(c)*0.7),n=Math.round(p+Math.sin(w)*s+Math.cos(c)*0.7);if(u<0||u>=o||n<0||n>=a){if(f>o*a*18)break;continue}let d=`${u},${n}`;if(l.has(d)){if(Math.random()>0.9)continue}else l.add(d),yield{x:u,y:n};if(f>o*a*18)break}for(let f=0;f<a;f++)for(let c=0;c<o;c++){let s=`${c},${f}`;if(l.has(s))continue;yield{x:c,y:f}}break}case"HUMAN_DRUNK_WALK":{let l=new Set,r=Math.floor(Math.random()*o),p=Math.floor(Math.random()*a),g=[{x:-1,y:0},{x:1,y:0},{x:0,y:-1},{x:0,y:1},{x:-1,y:-1},{x:1,y:-1},{x:-1,y:1},{x:1,y:1}];while(l.size<o*a){let f=`${r},${p}`;if(!l.has(f))l.add(f),yield{x:r,y:p};let c=[];for(let u of g){let n=r+u.x,d=p+u.y;if(n<0||n>=o||d<0||d>=a)continue;c.push({x:n,y:d})}if(!c.length)break;let s=c.filter((u)=>{return!l.has(`${u.x},${u.y}`)});if(s.length&&Math.random()>0.2){let u=s[Math.floor(Math.random()*s.length)];r=u.x,p=u.y;continue}let w=c[Math.floor(Math.random()*c.length)];r=w.x,p=w.y}for(let f=0;f<a;f++)for(let c=0;c<o;c++){let s=`${c},${f}`;if(l.has(s))continue;yield{x:c,y:f}}break}case"HUMAN_NOISE_CLOUD":{let l=[];for(let r=0;r<a;r++)for(let p=0;p<o;p++){let g=Math.sin((p+1)*0.93+Math.random()*0.8)+Math.cos((r+1)*1.17+Math.random()*0.8),f=(Math.random()-0.5)*2.6,c=Math.hypot(p-o/2,r-a/2)*0.08;l.push({point:{x:p,y:r},score:g+f+c})}l.sort((r,p)=>r.score-p.score);for(let r of l)yield r.point;break}case"HUMAN_PATCH_JUMP":{let l=new Set,r=[];for(let p=0;p<Math.max(6,o*a/18);p++)r.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*a)});while(l.size<o*a){let p=r[Math.floor(Math.random()*r.length)],g=1+Math.floor(Math.random()*3),f=1+Math.floor(Math.random()*3);for(let c=p.y-f;c<=p.y+f;c++)for(let s=p.x-g;s<=p.x+g;s++){if(s<0||s>=o||c<0||c>=a)continue;if(Math.random()>0.86)continue;let w=`${s},${c}`;if(l.has(w))continue;l.add(w),yield{x:s,y:c}}if(Math.random()>0.72&&r.length<o*a/2)r.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*a)});if(l.size>o*a*0.92)break}for(let p=0;p<a;p++)for(let g=0;g<o;g++){let f=`${g},${p}`;if(l.has(f))continue;yield{x:g,y:p}}break}case"HUMAN_HESITANT_LINES":{let l=new Set;for(let r=0;r<a;r++){let p=r%2===0;for(let g=0;g<o;g++){let f=p?g:o-1-g,c=`${f},${r}`;if(!l.has(c))l.add(c),yield{x:f,y:r};if(Math.random()>0.7){let s=Math.max(0,Math.min(o-1,f+(Math.random()>0.5?1:-1))),w=Math.max(0,Math.min(a-1,r+(Math.random()>0.65?1:0))),u=`${s},${w}`;if(!l.has(u))l.add(u),yield{x:s,y:w}}}}for(let r=0;r<a;r++)for(let p=0;p<o;p++){let g=`${p},${r}`;if(l.has(g))continue;yield{x:p,y:r}}break}case"HUMAN_OVERLAP_SWEEPS":{let l=[],r=Math.random()*Math.PI*2;for(let p=0;p<a;p++)for(let g=0;g<o;g++){let f=Math.sin((g+p)*0.42+r)*2.2,c=Math.cos((g-p)*0.3+r)*1.4;l.push({point:{x:g,y:p},score:p+f+c+(Math.random()-0.5)*3.4})}l.sort((p,g)=>p.score-g.score);for(let p of l)yield p.point;break}case"HUMAN_WOBBLE_DRIFT":{let l=[],r=o/2,p=a/2;for(let g=0;g<a;g++)for(let f=0;f<o;f++){let c=Math.hypot(f-r,g-p)*0.25,s=Math.sin((f+1)*0.9)*1.8+Math.cos((g+1)*1.1)*1.8+Math.sin((f+g)*0.35)*1.4;l.push({point:{x:f,y:g},score:c+s+(Math.random()-0.5)*2.8})}l.sort((g,f)=>g.score-f.score);for(let g of l)yield g.point;break}case"HUMAN_GAP_RECOVERY":{let l=new Set,r=[];for(let p=0;p<a;p++)for(let g=0;g<o;g++){if(Math.random()>0.87){r.push({x:g,y:p});continue}l.add(`${g},${p}`),yield{x:g,y:p}}r.sort((p,g)=>Math.hypot(p.x-o/2,p.y-a/2)-Math.hypot(g.x-o/2,g.y-a/2));for(let p of r){let g=`${p.x},${p.y}`;if(l.has(g))continue;l.add(g),yield p}break}case"HUMAN_STAIRCASE":{let l=new Set,r=o+a-1;for(let p=0;p<r;p++){let g=Math.max(0,p-o+1),f=Math.min(a-1,p);for(let c=g;c<=f;c++){let s=p-c,w=[{x:s,y:c},{x:s+(Math.random()>0.5?1:-1),y:c},{x:s,y:c+(Math.random()>0.5?1:-1)}];for(let u of w){if(u.x<0||u.x>=o||u.y<0||u.y>=a)continue;let n=`${u.x},${u.y}`;if(l.has(n))continue;l.add(n),yield u}}}for(let p=0;p<a;p++)for(let g=0;g<o;g++){let f=`${g},${p}`;if(l.has(f))continue;yield{x:g,y:p}}break}case"HUMAN_EDGE_HUGGER":{let l=[];for(let r=0;r<a;r++)for(let p=0;p<o;p++){let g=Math.min(p,r,o-1-p,a-1-r);l.push({point:{x:p,y:r},score:g*3.5+(Math.random()-0.5)*5.5})}l.sort((r,p)=>r.score-p.score);for(let r of l)yield r.point;break}case"HUMAN_BLOBS":{let l=new Set,r=o*a;while(l.size<r){let p=Math.floor(Math.random()*o),g=Math.floor(Math.random()*a),f=1+Math.floor(Math.random()*4);for(let c=g-f;c<=g+f;c++)for(let s=p-f;s<=p+f;s++){if(s<0||s>=o||c<0||c>=a)continue;let w=Math.atan2(c-g,s-p),u=f+Math.sin(w*3+Math.random())*0.8;if(Math.hypot(s-p,c-g)>u)continue;let n=`${s},${c}`;if(l.has(n))continue;l.add(n),yield{x:s,y:c}}}break}case"HUMAN_BACKTRACK":{let l=new Set,r=[];for(let p=0;p<a;p++)for(let g=0;g<o;g++)r.push({x:g,y:p});r.sort((p,g)=>p.y-g.y+(Math.random()-0.5)*2.2+(p.x-g.x)*0.04);for(let p=0;p<r.length;p++){let g=r[p],f=`${g.x},${g.y}`;if(l.has(f))continue;if(l.add(f),yield g,p>1&&Math.random()>0.74){let c=r[p-1],s=`${c.x},${c.y}`;if(!l.has(s))l.add(s),yield c}}for(let p=0;p<a;p++)for(let g=0;g<o;g++){let f=`${g},${p}`;if(l.has(f))continue;yield{x:g,y:p}}break}case"HUMAN_SHAKY_DIAGONAL":{let l=[];for(let r=0;r<a;r++)for(let p=0;p<o;p++){let g=Math.abs(p-r)*0.6,f=Math.sin(p*1.4+r*0.8)*1.8+Math.cos(r*1.1+p*0.5)*1.5;l.push({point:{x:p,y:r},score:g+f+(Math.random()-0.5)*3.2})}l.sort((r,p)=>r.score-p.score);for(let r of l)yield r.point;break}case"HUMAN_LATE_FIXES":{let l=[],r=[];for(let p=0;p<a;p++)for(let g=0;g<o;g++)if(Math.random()>0.9)r.push({x:g,y:p});else l.push({x:g,y:p});l.sort((p,g)=>p.y-g.y+(Math.random()-0.5)*1.5+(Math.random()>0.85?p.x-g.x:0)),r.sort((p,g)=>Math.hypot(g.x-o/2,g.y-a/2)-Math.hypot(p.x-o/2,p.y-a/2)),yield*l,yield*r;break}case"DIAGONAL_BRUSH":{for(let l=0;l<o+a-1;l++){let r=l%2===0,p=[],g=Math.max(0,l-o+1),f=Math.min(a-1,l);for(let c=g;c<=f;c++){let s=l-c;if(s>=0&&s<o)p.push({x:s,y:c})}if(Math.random()>0.55)p.reverse();if(r)for(let c=p.length-1;c>=0;c--)yield p[c];else yield*p}break}case"BRUSH_STROKES":{let l=Array.from({length:a},()=>Array(o).fill(!1)),r=[{x:1,y:0},{x:-1,y:0},{x:0,y:1},{x:0,y:-1},{x:1,y:1},{x:1,y:-1},{x:-1,y:1},{x:-1,y:-1}],p=(c,s)=>c>=0&&c<o&&s>=0&&s<a,g=0,f=o*a;for(let c=0;c<f*6&&g<f;c++){let s=Math.floor(Math.random()*o),w=Math.floor(Math.random()*a),u=r[Math.floor(Math.random()*r.length)],n=3+Math.floor(Math.random()*16);for(let d=0;d<n;d++){if(!p(s,w))break;if(!l[w][s])l[w][s]=!0,g++,yield{x:s,y:w};if(Math.random()>0.72)u=r[Math.floor(Math.random()*r.length)];s+=u.x,w+=u.y}}for(let c=0;c<a;c++)for(let s=0;s<o;s++)if(!l[c][s])yield{x:s,y:c};break}case"SPIRAL_FROM_CENTER":case"SPIRAL_TO_CENTER":{let l=new Set,r=o*a,p=Math.floor(o/2),g=Math.floor(a/2),f=[[1,0],[0,1],[-1,0],[0,-1]],c=0,s=1,w=(n,d)=>n>=0&&n<o&&d>=0&&d<a,u=function*(){let n=0;while(n<r){for(let d=0;d<2;d++){for(let b=0;b<s;b++){if(w(p,g)){let m=`${p},${g}`;if(!l.has(m)){if(l.add(m),yield{x:p,y:g},n++,n>=r)return}}p+=f[c][0],g+=f[c][1]}c=(c+1)%4}s++}};if(this.strategy==="SPIRAL_FROM_CENTER")yield*u();else{let n=[...u()];for(let d=n.length-1;d>=0;d--)yield n[d]}break}case"SCRIBBLE":{let l=new Set,r=o*a,p=Math.floor(o/2),g=Math.floor(a/2);for(let f=0;l.size<r&&f<r*24;f++){let c=`${p},${g}`;if(!l.has(c))l.add(c),yield{x:p,y:g};if(p+=Math.floor(Math.random()*3)-1,g+=Math.floor(Math.random()*3)-1,p<0||p>=o||g<0||g>=a)p=Math.floor(Math.random()*o),g=Math.floor(Math.random()*a)}for(let f=0;f<a;f++)for(let c=0;c<o;c++){let s=`${c},${f}`;if(l.has(s))continue;l.add(s),yield{x:c,y:f}}break}case"CROSSHATCH":{let l=[];for(let g=0;g<o+a-1;g++)for(let f=Math.max(0,g-o+1);f<=Math.min(a-1,g);f++){let c=g-f;l.push({x:c,y:f})}let r=[];for(let g=-a+1;g<o;g++)for(let f=0;f<a;f++){let c=f+g;if(c>=0&&c<o)r.push({x:c,y:f})}let p=new Set;for(let g of[...l,...r]){let f=`${g.x},${g.y}`;if(p.has(f))continue;p.add(f),yield g}break}case"WAVE_SWEEP":{let l=new Set;for(let r=0;r<o;r++){let g=(Math.sin(r/Math.max(1,o-1)*Math.PI*4)+1)*0.5*(a-1)|0;for(let f=0;f<a;f++){let c=g+f,s=g-f;for(let w of[c,s]){if(w<0||w>=a)continue;let u=`${r},${w}`;if(l.has(u))continue;l.add(u),yield{x:r,y:w}}}}break}case"SCATTERED_LINES":{let l=new Set,r=o*a;for(let p=0;l.size<r&&p<r*14;p++){let g=Math.floor(Math.random()*o),f=Math.floor(Math.random()*a),c=Math.random()*Math.PI*2,s=Math.round(Math.cos(c)),w=Math.round(Math.sin(c)),u=6+Math.floor(Math.random()*28);for(let n=0;n<u;n++){if(g<0||g>=o||f<0||f>=a)break;let d=`${g},${f}`;if(!l.has(d))l.add(d),yield{x:g,y:f};g+=s,f+=w}}for(let p=0;p<a;p++)for(let g=0;g<o;g++){let f=`${g},${p}`;if(l.has(f))continue;l.add(f),yield{x:g,y:p}}break}case"CONTOUR_JITTER":{let l=new Set;for(let r=0;r<Math.ceil(Math.min(o,a)/2);r++){let p=[],g=r,f=r,c=o-r-1,s=a-r-1;for(let w=g;w<=c;w++)p.push({x:w,y:f});for(let w=f+1;w<=s;w++)p.push({x:c,y:w});for(let w=c-1;w>=g;w--)p.push({x:w,y:s});for(let w=s-1;w>f;w--)p.push({x:g,y:w});for(let w=p.length-1;w>0;w--){let u=Math.floor(Math.random()*(w+1)),n=p[w];p[w]=p[u],p[u]=n}for(let w of p){let u=`${w.x},${w.y}`;if(l.has(u))continue;l.add(u),yield w}}break}case"SPIRAL_WOBBLE":{let l=new Set,r=o/2,p=a/2,g=Math.hypot(r,p);for(let f=0;l.size<o*a&&f<o*a*9;f++){let c=f/(o*a*9)*g,s=f*0.31+Math.sin(f*0.07)*0.7,w=Math.round(r+Math.cos(s)*c),u=Math.round(p+Math.sin(s)*c);if(w<0||w>=o||u<0||u>=a)continue;let n=`${w},${u}`;if(l.has(n))continue;l.add(n),yield{x:w,y:u}}for(let f=0;f<a;f++)for(let c=0;c<o;c++){let s=`${c},${f}`;if(l.has(s))continue;yield{x:c,y:f}}break}case"CLUSTER_BURSTS":{let l=new Set,r=o*a;for(let p=0;l.size<r&&p<r*12;p++){let g=Math.floor(Math.random()*o),f=Math.floor(Math.random()*a),c=2+Math.floor(Math.random()*10);for(let s=f-c;s<=f+c;s++)for(let w=g-c;w<=g+c;w++){if(w<0||w>=o||s<0||s>=a)continue;if(Math.hypot(w-g,s-f)>c)continue;let u=`${w},${s}`;if(l.has(u))continue;l.add(u),yield{x:w,y:s}}}for(let p=0;p<a;p++)for(let g=0;g<o;g++){let f=`${g},${p}`;if(l.has(f))continue;l.add(f),yield{x:g,y:p}}break}case"ORBITAL":{let l=new Set,r=(o-1)/2,p=(a-1)/2,g=Math.ceil(Math.max(r,p));for(let f=0;f<=g;f++){let c=Math.max(16,Math.ceil(2*Math.PI*Math.max(1,f)*2));for(let s=0;s<c;s++){let w=s/c*Math.PI*2+(f%2?0.3:-0.3),u=Math.round(r+Math.cos(w)*f),n=Math.round(p+Math.sin(w)*f);if(u<0||u>=o||n<0||n>=a)continue;let d=`${u},${n}`;if(l.has(d))continue;l.add(d),yield{x:u,y:n}}}for(let f=0;f<a;f++)for(let c=0;c<o;c++){let s=`${c},${f}`;if(l.has(s))continue;yield{x:c,y:f}}break}case"FLOW_FIELD":{let l=new Set,r=o*a;for(let p=0;l.size<r&&p<r*18;p++){let g=Math.floor(Math.random()*o),f=Math.floor(Math.random()*a);for(let c=0;c<120;c++){if(g<0||g>=o||f<0||f>=a)break;let s=`${g},${f}`;if(!l.has(s))l.add(s),yield{x:g,y:f};let w=Math.sin(g*0.09)*1.8+Math.cos(f*0.08)*1.6+Math.sin((g+f)*0.05);g+=Math.round(Math.cos(w)),f+=Math.round(Math.sin(w))}}for(let p=0;p<a;p++)for(let g=0;g<o;g++){let f=`${g},${p}`;if(l.has(f))continue;l.add(f),yield{x:g,y:p}}break}case"EDGE_IN":{let l=new Set,r=Math.ceil(Math.min(o,a)/2);for(let p=0;p<r;p++){let g=p,f=o-1-p,c=p,s=a-1-p;for(let w=g;w<=f;w++)for(let u of[c,s]){let n=`${w},${u}`;if(l.has(n))continue;l.add(n),yield{x:w,y:u}}for(let w=c+1;w<=s-1;w++)for(let u of[g,f]){let n=`${u},${w}`;if(l.has(n))continue;l.add(n),yield{x:u,y:w}}}break}}}moveStart(o){if(o.button!==0)return;if(o.preventDefault(),o.stopPropagation(),!this.lock)this.moveInfo={globalX:this.position.globalX,globalY:this.position.globalY,clientX:o.clientX,clientY:o.clientY}}moveStop(){if(this.moveInfo)this.moveInfo=void 0,this.position.updateAnchor(),this.pixels.update(),this.updateColors(),J(this.bot)}move(o){if(!this.moveInfo)return;let a=Math.round((o.clientX-this.moveInfo.clientX)/this.position.pixelSize),l=Math.round((o.clientY-this.moveInfo.clientY)/this.position.pixelSize);if(this.moveInfo.globalX!==void 0){if(this.position.globalX=a+this.moveInfo.globalX,this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,this.moveInfo.width-a)}else if(this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,a+this.moveInfo.width);if(this.moveInfo.globalY!==void 0){if(this.position.globalY=l+this.moveInfo.globalY,this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,this.moveInfo.height-l)}else if(this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,l+this.moveInfo.height);this.update(),J(this.bot)}resizeStart(o){if(this.lock||o.button!==0)return;o.preventDefault(),o.stopPropagation(),this.moveInfo={clientX:o.clientX,clientY:o.clientY};let a=o.target;if(a.classList.contains("n"))this.moveInfo.height=this.pixels.height,this.moveInfo.globalY=this.position.globalY;if(a.classList.contains("e"))this.moveInfo.width=this.pixels.width;if(a.classList.contains("s"))this.moveInfo.height=this.pixels.height;if(a.classList.contains("w"))this.moveInfo.width=this.pixels.width,this.moveInfo.globalX=this.position.globalX}export(){let o=document.createElement("a");document.body.append(o),o.href=URL.createObjectURL(new Blob([JSON.stringify(this.toJSON())],{type:"application/json"})),o.download=`${this.pixels.width}x${this.pixels.height}.${X}`,o.click(),URL.revokeObjectURL(o.href),o.href=this.pixels.canvas.toDataURL("image/webp",1),o.download=`${this.pixels.width}x${this.pixels.height}.webp`,o.click(),URL.revokeObjectURL(o.href),o.remove()}}var No=`/* stylelint-disable declaration-no-important */
...
<div class="wform">
  ...
  <section class="widget-section widget-section-progress">
    <strong class="widget-section-title" data-i18n="progressSection">Progress</strong>
    <div class="wprogress"><div></div><span></span></div>
  </section>

  <details class="widget-section widget-section-autofarm" open>
...
`;var Do="kglacer-macro:overlay-hidden",to="kglacer-macro:auto-farm-config",Uo="kglacer-macro:auto-overlay-config",Eo="https://raw.githubusercontent.com/robgallardof/kglacer-macro/refs/heads/main/src/img/logo.svg";class so extends _{bot;element=document.createElement("div");get status(){return this.$status.innerHTML}set status(o){this.$status.innerHTML=o}get open(){return this.element.classList.contains("wopen")}set open(o){if(o)this.element.classList.add("wopen");else this.element.classList.remove("wopen")}$settings;$status;$openConfig;$topbar;$draw;$drawAndPaint;$addImage;$captureTemplate;$toggleOverlay;$autofarmConfig;$autofarmStart;$autofarmStop;$autofarmStatus;$autoOverlayConfig;$autoOverlayStart;$autoOverlayStop;$autoOverlayStatus;$strategy;$progressLine;$progressText;$images;$wopenButton;$widgetLogo;activeImageIndex=-1;autoFarmIntervalId;autoFarmConfig;autoFarmTickRunning=!1;autoFarmNextTickAt;autoOverlayIntervalId;autoOverlayConfig;autoOverlayTickRunning=!1;autoOverlayNextTickAt;statusRefreshIntervalId;constructor(o){super();this.bot=o;this.element.classList.add("wwidget"),this.element.innerHTML=Fo,t(this.element),document.body.append(this.element),this.populateElementsWithSelector(this.element,{$wopenButton:".wopen-button",$widgetLogo:".widget-logo",$settings:".wform",$status:".wstatus",$openConfig:".open-config",$topbar:".wtopbar",$draw:".draw",$drawAndPaint:".draw-and-paint",$addImage:".add-image",$captureTemplate:".capture-template",$toggleOverlay:".toggle-overlay",$autofarmConfig:".autofarm-config",$autofarmStart:".autofarm-start",$autofarmStop:".autofarm-stop",$autofarmStatus:".autofarm-status",$autoOverlayConfig:".autooverlay-config",$autoOverlayStart:".autooverlay-start",$autoOverlayStop:".autooverlay-stop",$autoOverlayStatus:".autooverlay-status",$strategy:".strategy",$progressLine:".wprogress div",$progressText:".wprogress span",$images:".images"}),this.$widgetLogo.src=Eo,this.$wopenButton.addEventListener("click",()=>this.open=!this.open),this.$draw.addEventListener("click",()=>this.bot.draw()),this.$drawAndPaint.addEventListener("click",()=>{this.drawAndClickPaintWhenReady()}),this.$addImage.addEventListener("click",()=>this.addImage()),this.$openConfig.addEventListener("click",()=>{this.openSettingsModal()}),this.$captureTemplate.addEventListener("click",()=>{this.captureTemplate()}),this.$toggleOverlay.addEventListener("click",()=>{this.toggleOverlay()}),this.$autofarmConfig.addEventListener("click",()=>{this.openAutoFarmModal()}),this.$autofarmStart.addEventListener("click",()=>{this.startAutoFarm()}),this.$autofarmStop.addEventListener("click",()=>{this.stopAutoFarm()}),this.$autoOverlayConfig.addEventListener("click",()=>{this.openAutoOverlayModal()}),this.$autoOverlayStart.addEventListener("click",()=>{this.startAutoOverlay()}),this.$autoOverlayStop.addEventListener("click",()=>{this.stopAutoOverlay()}),this.$strategy.addEventListener("change",()=>{this.bot.strategy=this.$strategy.value}),this.registerEvent(document,"keydown",this.handleKeyboard.bind(this),{passive:!1}),this.update(),this.syncOverlayVisibilityFromStorage(),this.loadAutoFarmConfigFromStorage(),this.loadAutoOverlayConfigFromStorage(),this.refreshAutoFarmStatusText(),this.refreshAutoOverlayStatusText(),this.statusRefreshIntervalId=window.setInterval(()=>{this.refreshAutoFarmStatusText(),this.refreshAutoOverlayStatusText(),this.refreshProgress()},1000),this.open=!0,console.log("[KGM][Widget] Widget mounted and opened")}addImage(){return console.log("[KGM][Widget] Add image flow started"),this.setDisabled("add-image",!0),this.run(i("taskAddingImage"),async()=>{await this.bot.updateColors();let o=document.createElement("input");o.type="file",o.accept=`image/*,.${X}`,o.click(),await Z(o,["change"],["cancel","error"]);let a=o.files?.[0];if(!a)throw new co(this.bot);console.log("[KGM][Widget] File selected",{name:a.name,size:a.size,type:a.type});let l;if(a.name.endsWith(`.${X}`))l=await q.fromJSON(this.bot,JSON.parse(await a.text()));else{let r=new FileReader;r.readAsDataURL(a),await Z(r,["load"],["error"]);let p=await this.compressImageBeforeLoad(r.result),g=new Image;g.src=p,await Z(g,["load"],["error"]),await this.waitForStableViewportProjection(),l=new q(this.bot,F.fromScreenPosition(this.bot,this.defaultImageScreenPosition()),new R(this.bot,g))}this.bot.images.push(l),console.log("[KGM][Widget] Image instance added",{images:this.bot.images.length}),await this.bot.readMap(),l.updateTasks(),J(this.bot,!0),this.bot.updateTasks(),this.update(),globalThis.location.reload()},()=>{this.setDisabled("add-image",!1)})}captureTemplate(){return this.setDisabled("capture-template",!0),this.run(i("taskCapturingMapImage"),async()=>{let o=await this.resolveCaptureBounds(),{minGlobalX:a,minGlobalY:l,maxGlobalX:r,maxGlobalY:p}=o,g=document.createElement("canvas");g.width=Math.max(1,r-a+1),g.height=Math.max(1,p-l+1);let f=g.getContext("2d");if(!f)throw new Error("Capture context unavailable");f.imageSmoothingEnabled=!1;let c=Math.floor(a/P),s=Math.floor(l/P),w=Math.floor(r/P),u=Math.floor(p/P),n=(w-c+1)*(u-s+1),d=0;for(let m=c;m<=w;m++)for(let z=s;z<=u;z++){this.status=`⌛ ${i("taskReadingTiles")} [${++d}/${n}]`;let A=await this.loadTileImage(m,z),M=m*P,H=z*P,N=Math.max(a,M),D=Math.min(r,M+P-1),j=Math.max(l,H),C=Math.min(p,H+P-1),O=N-M,v=j-H,E=D-N+1,G=C-j+1,$=N-a,x=j-l;f.drawImage(A,O,v,E,G,$,x,E,G)}let b=Date.now();await this.downloadCapture(g,"png",b)},()=>{this.setDisabled("capture-template",!1)})}async downloadCapture(o,a,l){let r=a==="webp"?"image/webp":"image/png",p=await new Promise((c,s)=>{o.toBlob((w)=>{if(!w){s(new Error(`Failed to create ${a.toUpperCase()} capture file`));return}c(w)},r)}),g=URL.createObjectURL(p),f=document.createElement("a");f.href=g,f.download=`wplace-capture-${l}.${a}`,f.click(),URL.revokeObjectURL(g)}async loadTileImage(o,a){let l;for(let r=1;r<=3;r++)try{let p=new Image;return p.crossOrigin="anonymous",p.referrerPolicy="no-referrer",p.src=`https://backend.wplace.live/files/s0/tiles/${o}/${a}.png?ts=${Date.now()}-${r}`,await Z(p,["load"],["error"]),p}catch(p){if(l=p,r<3)await new Promise((g)=>setTimeout(g,r*200))}throw l instanceof Error?l:new Error(`Tile fetch failed (${o}/${a})`)}async resolveCaptureBounds(){return this.selectCaptureBounds()}selectCaptureBounds(){return new Promise((o,a)=>{let l=document.createElement("div");l.className="kgm-capture-overlay",l.innerHTML=`<div class="kgm-capture-hint">${i("captureHintSelectArea")}: A → B</div><div class="kgm-capture-box"></div>`;let r=l.querySelector(".kgm-capture-box");document.body.append(l);let p,g,f=()=>{window.removeEventListener("keydown",n,!0),l.removeEventListener("pointermove",w),l.removeEventListener("pointerdown",u),l.remove()},c=(d)=>{let b=Math.min(p.x,d.x),m=Math.min(p.y,d.y),z=Math.abs(p.x-d.x)+1,A=Math.abs(p.y-d.y)+1;return{left:b,top:m,width:z,height:A}},s=(d)=>{let{left:b,top:m,width:z,height:A}=c(d);r.style.left=`${b}px`,r.style.top=`${m}px`,r.style.width=`${z}px`,r.style.height=`${A}px`},w=(d)=>{if(!p)return;s({x:d.clientX,y:d.clientY})},u=(d)=>{if(d.preventDefault(),!p){p={x:d.clientX,y:d.clientY};let N=F.fromScreenPosition(this.bot,p);g={x:N.globalX,y:N.globalY},s(p);return}let b={x:d.clientX,y:d.clientY},m=F.fromScreenPosition(this.bot,b);if(f(),!g){a(new Error("Capture anchor point unavailable"));return}let z=Math.min(g.x,m.globalX),A=Math.min(g.y,m.globalY),M=Math.max(g.x,m.globalX),H=Math.max(g.y,m.globalY);if(M-z<1||H-A<1){a(new Error("Capture area too small"));return}o({minGlobalX:z,minGlobalY:A,maxGlobalX:M,maxGlobalY:H})},n=(d)=>{if(d.key!=="Escape")return;f(),a(new Error("Capture cancelled"))};window.addEventListener("keydown",n,!0),l.addEventListener("pointermove",w),l.addEventListener("pointerdown",u)})}defaultImageScreenPosition(){let o=Math.round(this.element.getBoundingClientRect().width);return{x:Math.max(256,o),y:32}}async compressImageBeforeLoad(o){let a=new Image;if(a.src=o,await Z(a,["load"],["error"]),!(a.naturalWidth*a.naturalHeight>3000000||o.length>3000000))return o;let r=document.createElement("canvas");r.width=a.naturalWidth,r.height=a.naturalHeight;let p=r.getContext("2d");if(!p)return o;return p.drawImage(a,0,0),r.toDataURL("image/png")}async waitForStableViewportProjection(){let o=this.defaultImageScreenPosition(),a=0,l;for(let r=0;r<45;r++){await new Promise((w)=>requestAnimationFrame(()=>{w()}));let{anchorScreenPosition:{x:p,y:g},pixelSize:f}=this.bot.findAnchorsForScreen(o);if(!Number.isFinite(f)||f<=0){a=0;continue}let c={anchorX:p,anchorY:g,pixelSize:f};if(!l){l=c,a=1;continue}if(Math.abs(c.anchorX-l.anchorX)+Math.abs(c.anchorY-l.anchorY)+Math.abs(c.pixelSize-l.pixelSize)<0.0012)a++;else a=0;if(l=c,a>=3)return}}update(){this.$strategy.value=this.bot.strategy,this.refreshProgress(),this.$images.innerHTML="";let o=document.createDocumentFragment();for(let a=0;a<this.bot.images.length;a++){let l=this.bot.images[a],r=document.createElement("div");o.append(r),r.className="image",r.innerHTML=`<button class="preview" title="View preview">
