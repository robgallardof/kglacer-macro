...
<span class="premium"></span>`,d.querySelector(".swatch").style.setProperty("--swatch-color",So(i.realColor)),d.addEventListener("click",()=>{if(l){l=!1;return}f(),this.updateTasks()}),d.addEventListener("dragstart",(h)=>{l=!0,d.classList.add("dragging"),h.dataTransfer?.setData("text/plain",String(a)),h.dataTransfer.effectAllowed="move"}),d.addEventListener("dragend",()=>{d.classList.remove("dragging")}),d.addEventListener("dragover",(h)=>{h.preventDefault(),d.classList.add("drag-target")}),d.addEventListener("dragleave",()=>{d.classList.remove("drag-target")}),d.addEventListener("drop",(h)=>{h.preventDefault(),d.classList.remove("drag-target");let u=Number.parseInt(h.dataTransfer?.getData("text/plain")??"-1",10);if(u<0||u===a||u>=this.colors.length)return;this.colors.splice(a,0,...this.colors.splice(u,1)),M(this.bot),this.updateColors()});let w=document.createElement("button");w.textContent=g("buy"),w.className="buy-chip",w.addEventListener("click",(h)=>{h.stopPropagation(),document.getElementById("color-"+i.realColor)?.click()}),d.append(w);let m=`${t} ${n.join(" ")} ${i.realColor} ${B[i.realColor]}`;if(!s||m.toLowerCase().includes(s))this.$colorsDialogList.append(d)}}syncColorBulkToggle(){let o=this.colors.filter((a)=>!a.disabled).length,s=o===this.colors.length;this.$toggleAllColors.checked=s,this.$toggleAllColors.indeterminate=o>0&&!s}*strategyPositionIterator(){let o=this.pixels.pixels[0].length,s=this.pixels.pixels.length;switch(this.strategy){case"DOWN":{for(let a=0;a<s;a++)for(let r=0;r<o;r++)yield{x:r,y:a};break}case"UP":{for(let a=s-1;a>=0;a--)for(let r=0;r<o;r++)yield{x:r,y:a};break}case"LEFT":{for(let a=0;a<o;a++)for(let r=0;r<s;r++)yield{x:a,y:r};break}case"RIGHT":{for(let a=o-1;a>=0;a--)for(let r=0;r<s;r++)yield{x:a,y:r};break}case"RANDOM":{let a=[];for(let r=0;r<s;r++)for(let i=0;i<o;i++)a.push({x:i,y:r});for(let r=a.length-1;r>=0;r--){let i=Math.floor(Math.random()*(r+1)),l=a[r];a[r]=a[i],a[i]=l}yield*a;break}case"ZIGZAG":{for(let a=0;a<s;a++)if(a%2===0)for(let r=0;r<o;r++)yield{x:r,y:a};else for(let r=o-1;r>=0;r--)yield{x:r,y:a};break}case"HUMANIZED":{let a=Math.max(4,Math.floor(Math.min(o,s)/10)),r=[];for(let i=0;i<s;i+=a)for(let l=0;l<o;l+=a)r.push({x:l,y:i});for(let i=r.length-1;i>=0;i--){let l=Math.floor(Math.random()*(i+1)),e=r[i];r[i]=r[l],r[l]=e}for(let i=0;i<r.length;i++){let l=r[i],e=Math.min(s,l.y+a),t=Math.min(o,l.x+a);for(let n=l.y;n<e;n++)if(Math.random()>0.35)for(let p=l.x;p<t;p++)yield{x:p,y:n};else for(let p=t-1;p>=l.x;p--)yield{x:p,y:n}}break}case"HUMAN_SOFT_DITHER":{let a=new Set;for(let r=0;r<s;r++){let i=Math.floor(Math.random()*3)-1;if((r+i)%2===0)for(let e=0;e<o;e+=2)a.add(`${e},${r}`),yield{x:e,y:r};else for(let e=1;e<o;e+=2)a.add(`${e},${r}`),yield{x:e,y:r}}for(let r=0;r<s;r++)for(let i=0;i<o;i++){let l=`${i},${r}`;if(a.has(l))continue;yield{x:i,y:r}}break}case"HUMAN_PATCHY":{let a=new Set,r=o*s;while(a.size<r){let i=Math.floor(Math.random()*o),l=Math.floor(Math.random()*s),e=1+Math.floor(Math.random()*5);for(let t=l-e;t<=l+e;t++)for(let n=i-e;n<=i+e;n++){if(n<0||n>=o||t<0||t>=s)continue;if(Math.hypot(n-i,t-l)>e+Math.random()*1.2)continue;let c=`${n},${t}`;if(a.has(c))continue;a.add(c),yield{x:n,y:t}}}break}case"HUMAN_SWEEP_ARCS":{let a=new Set,r=(o-1)/2,i=(s-1)/2,l=Math.hypot(r,i);for(let e=0;e<4;e++){let t=Math.random()*Math.PI*2;for(let n=0;n<=l;n+=0.35){let c=Math.PI/2+Math.random()*(Math.PI/1.5),p=Math.max(10,Math.floor(n*8));for(let f=0;f<p;f++){let d=t+c*f/p+Math.sin(n)*0.08,w=Math.round(r+Math.cos(d)*n),m=Math.round(i+Math.sin(d)*n);if(w<0||w>=o||m<0||m>=s)continue;let h=`${w},${m}`;if(a.has(h))continue;a.add(h),yield{x:w,y:m}}}}for(let e=0;e<s;e++)for(let t=0;t<o;t++){let n=`${t},${e}`;if(a.has(n))continue;yield{x:t,y:e}}break}case"HUMAN_MICRO_CORRECTIONS":{let a=new Set;for(let r=0;r<s;r++){let i=r%2===0?1:-1,l=i>0?0:o-1;for(let e=0;e<o;e++){let t=l+(Math.random()>0.82?i:0),n=r+(Math.random()>0.9?1:0);for(let c of[{x:l,y:r},{x:t,y:r},{x:l,y:n}]){if(c.x<0||c.x>=o||c.y<0||c.y>=s)continue;let p=`${c.x},${c.y}`;if(a.has(p))continue;a.add(p),yield c}l+=i}}for(let r=0;r<s;r++)for(let i=0;i<o;i++){let l=`${i},${r}`;if(a.has(l))continue;yield{x:i,y:r}}break}case"HUMAN_JITTER_FILL":{let a=[];for(let r=0;r<s;r++)for(let i=0;i<o;i++)a.push({x:i,y:r});a.sort((r,i)=>{let l=r.y+(Math.random()-0.5)*1.8,e=i.y+(Math.random()-0.5)*1.8;if(l!==e)return l-e;return r.x+(Math.random()-0.5)*2-(i.x+(Math.random()-0.5)*2)}),yield*a;break}case"HUMAN_CORNER_BIAS":{let a=[{x:0,y:0},{x:o-1,y:0},{x:0,y:s-1},{x:o-1,y:s-1}],r=a[Math.floor(Math.random()*a.length)],i=[];for(let l=0;l<s;l++)for(let e=0;e<o;e++){let n=Math.hypot(e-r.x,l-r.y)+Math.random()*3.5;i.push({point:{x:e,y:l},score:n})}i.sort((l,e)=>l.score-e.score);for(let l of i)yield l.point;break}case"HUMAN_LONG_STROKES":{let a=new Set,r=o*s;while(a.size<r){let i=Math.floor(Math.random()*o),l=Math.floor(Math.random()*s),e=Math.random()*Math.PI*2,t=Math.sign(Math.cos(e)),n=Math.sign(Math.sin(e)),c=10+Math.floor(Math.random()*40);for(let p=0;p<c;p++){if(i<0||i>=o||l<0||l>=s)break;let f=`${i},${l}`;if(!a.has(f))a.add(f),yield{x:i,y:l};if(Math.random()>0.78)i+=n,l+=t;else i+=t,l+=n}}break}case"HUMAN_TAP_CLUSTERS":{let a=new Set,r=o*s;while(a.size<r){let i=Math.floor(Math.random()*o),l=Math.floor(Math.random()*s),e=3+Math.floor(Math.random()*10);for(let t=0;t<e;t++){let n=Math.round(i+(Math.random()-0.5)*6),c=Math.round(l+(Math.random()-0.5)*6);if(n<0||n>=o||c<0||c>=s)continue;let p=`${n},${c}`;if(a.has(p))continue;a.add(p),yield{x:n,y:c}}}break}case"HUMAN_MESSY_SPIRAL":{let a=new Set,r=(o-1)/2,i=(s-1)/2,l=Math.hypot(r,i)+2;for(let e=0;a.size<o*s;e++){let t=e/3,n=Math.min(l,t*0.18),c=t*0.29+Math.sin(t*0.13)*0.8,p=Math.round(r+Math.cos(c)*n+Math.sin(t)*0.7),f=Math.round(i+Math.sin(c)*n+Math.cos(t)*0.7);if(p<0||p>=o||f<0||f>=s){if(e>o*s*18)break;continue}let d=`${p},${f}`;if(a.has(d)){if(Math.random()>0.9)continue}else a.add(d),yield{x:p,y:f};if(e>o*s*18)break}for(let e=0;e<s;e++)for(let t=0;t<o;t++){let n=`${t},${e}`;if(a.has(n))continue;yield{x:t,y:e}}break}case"HUMAN_DRUNK_WALK":{let a=new Set,r=Math.floor(Math.random()*o),i=Math.floor(Math.random()*s),l=[{x:-1,y:0},{x:1,y:0},{x:0,y:-1},{x:0,y:1},{x:-1,y:-1},{x:1,y:-1},{x:-1,y:1},{x:1,y:1}];while(a.size<o*s){let e=`${r},${i}`;if(!a.has(e))a.add(e),yield{x:r,y:i};let t=[];for(let p of l){let f=r+p.x,d=i+p.y;if(f<0||f>=o||d<0||d>=s)continue;t.push({x:f,y:d})}if(!t.length)break;let n=t.filter((p)=>{return!a.has(`${p.x},${p.y}`)});if(n.length&&Math.random()>0.2){let p=n[Math.floor(Math.random()*n.length)];r=p.x,i=p.y;continue}let c=t[Math.floor(Math.random()*t.length)];r=c.x,i=c.y}for(let e=0;e<s;e++)for(let t=0;t<o;t++){let n=`${t},${e}`;if(a.has(n))continue;yield{x:t,y:e}}break}case"HUMAN_NOISE_CLOUD":{let a=[];for(let r=0;r<s;r++)for(let i=0;i<o;i++){let l=Math.sin((i+1)*0.93+Math.random()*0.8)+Math.cos((r+1)*1.17+Math.random()*0.8),e=(Math.random()-0.5)*2.6,t=Math.hypot(i-o/2,r-s/2)*0.08;a.push({point:{x:i,y:r},score:l+e+t})}a.sort((r,i)=>r.score-i.score);for(let r of a)yield r.point;break}case"HUMAN_PATCH_JUMP":{let a=new Set,r=[];for(let i=0;i<Math.max(6,o*s/18);i++)r.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*s)});while(a.size<o*s){let i=r[Math.floor(Math.random()*r.length)],l=1+Math.floor(Math.random()*3),e=1+Math.floor(Math.random()*3);for(let t=i.y-e;t<=i.y+e;t++)for(let n=i.x-l;n<=i.x+l;n++){if(n<0||n>=o||t<0||t>=s)continue;if(Math.random()>0.86)continue;let c=`${n},${t}`;if(a.has(c))continue;a.add(c),yield{x:n,y:t}}if(Math.random()>0.72&&r.length<o*s/2)r.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*s)});if(a.size>o*s*0.92)break}for(let i=0;i<s;i++)for(let l=0;l<o;l++){let e=`${l},${i}`;if(a.has(e))continue;yield{x:l,y:i}}break}case"HUMAN_HESITANT_LINES":{let a=new Set;for(let r=0;r<s;r++){let i=r%2===0;for(let l=0;l<o;l++){let e=i?l:o-1-l,t=`${e},${r}`;if(!a.has(t))a.add(t),yield{x:e,y:r};if(Math.random()>0.7){let n=Math.max(0,Math.min(o-1,e+(Math.random()>0.5?1:-1))),c=Math.max(0,Math.min(s-1,r+(Math.random()>0.65?1:0))),p=`${n},${c}`;if(!a.has(p))a.add(p),yield{x:n,y:c}}}}for(let r=0;r<s;r++)for(let i=0;i<o;i++){let l=`${i},${r}`;if(a.has(l))continue;yield{x:i,y:r}}break}case"HUMAN_OVERLAP_SWEEPS":{let a=[],r=Math.random()*Math.PI*2;for(let i=0;i<s;i++)for(let l=0;l<o;l++){let e=Math.sin((l+i)*0.42+r)*2.2,t=Math.cos((l-i)*0.3+r)*1.4;a.push({point:{x:l,y:i},score:i+e+t+(Math.random()-0.5)*3.4})}a.sort((i,l)=>i.score-l.score);for(let i of a)yield i.point;break}case"HUMAN_WOBBLE_DRIFT":{let a=[],r=o/2,i=s/2;for(let l=0;l<s;l++)for(let e=0;e<o;e++){let t=Math.hypot(e-r,l-i)*0.25,n=Math.sin((e+1)*0.9)*1.8+Math.cos((l+1)*1.1)*1.8+Math.sin((e+l)*0.35)*1.4;a.push({point:{x:e,y:l},score:t+n+(Math.random()-0.5)*2.8})}a.sort((l,e)=>l.score-e.score);for(let l of a)yield l.point;break}case"HUMAN_GAP_RECOVERY":{let a=new Set,r=[];for(let i=0;i<s;i++)for(let l=0;l<o;l++){if(Math.random()>0.87){r.push({x:l,y:i});continue}a.add(`${l},${i}`),yield{x:l,y:i}}r.sort((i,l)=>Math.hypot(i.x-o/2,i.y-s/2)-Math.hypot(l.x-o/2,l.y-s/2));for(let i of r){let l=`${i.x},${i.y}`;if(a.has(l))continue;a.add(l),yield i}break}case"HUMAN_STAIRCASE":{let a=new Set,r=o+s-1;for(let i=0;i<r;i++){let l=Math.max(0,i-o+1),e=Math.min(s-1,i);for(let t=l;t<=e;t++){let n=i-t,c=[{x:n,y:t},{x:n+(Math.random()>0.5?1:-1),y:t},{x:n,y:t+(Math.random()>0.5?1:-1)}];for(let p of c){if(p.x<0||p.x>=o||p.y<0||p.y>=s)continue;let f=`${p.x},${p.y}`;if(a.has(f))continue;a.add(f),yield p}}}for(let i=0;i<s;i++)for(let l=0;l<o;l++){let e=`${l},${i}`;if(a.has(e))continue;yield{x:l,y:i}}break}case"HUMAN_EDGE_HUGGER":{let a=[];for(let r=0;r<s;r++)for(let i=0;i<o;i++){let l=Math.min(i,r,o-1-i,s-1-r);a.push({point:{x:i,y:r},score:l*3.5+(Math.random()-0.5)*5.5})}a.sort((r,i)=>r.score-i.score);for(let r of a)yield r.point;break}case"HUMAN_BLOBS":{let a=new Set,r=o*s;while(a.size<r){let i=Math.floor(Math.random()*o),l=Math.floor(Math.random()*s),e=1+Math.floor(Math.random()*4);for(let t=l-e;t<=l+e;t++)for(let n=i-e;n<=i+e;n++){if(n<0||n>=o||t<0||t>=s)continue;let c=Math.atan2(t-l,n-i),p=e+Math.sin(c*3+Math.random())*0.8;if(Math.hypot(n-i,t-l)>p)continue;let f=`${n},${t}`;if(a.has(f))continue;a.add(f),yield{x:n,y:t}}}break}case"HUMAN_BACKTRACK":{let a=new Set,r=[];for(let i=0;i<s;i++)for(let l=0;l<o;l++)r.push({x:l,y:i});r.sort((i,l)=>i.y-l.y+(Math.random()-0.5)*2.2+(i.x-l.x)*0.04);for(let i=0;i<r.length;i++){let l=r[i],e=`${l.x},${l.y}`;if(a.has(e))continue;if(a.add(e),yield l,i>1&&Math.random()>0.74){let t=r[i-1],n=`${t.x},${t.y}`;if(!a.has(n))a.add(n),yield t}}for(let i=0;i<s;i++)for(let l=0;l<o;l++){let e=`${l},${i}`;if(a.has(e))continue;yield{x:l,y:i}}break}case"HUMAN_SHAKY_DIAGONAL":{let a=[];for(let r=0;r<s;r++)for(let i=0;i<o;i++){let l=Math.abs(i-r)*0.6,e=Math.sin(i*1.4+r*0.8)*1.8+Math.cos(r*1.1+i*0.5)*1.5;a.push({point:{x:i,y:r},score:l+e+(Math.random()-0.5)*3.2})}a.sort((r,i)=>r.score-i.score);for(let r of a)yield r.point;break}case"HUMAN_LATE_FIXES":{let a=[],r=[];for(let i=0;i<s;i++)for(let l=0;l<o;l++)if(Math.random()>0.9)r.push({x:l,y:i});else a.push({x:l,y:i});a.sort((i,l)=>i.y-l.y+(Math.random()-0.5)*1.5+(Math.random()>0.85?i.x-l.x:0)),r.sort((i,l)=>Math.hypot(l.x-o/2,l.y-s/2)-Math.hypot(i.x-o/2,i.y-s/2)),yield*a,yield*r;break}case"DIAGONAL_BRUSH":{for(let a=0;a<o+s-1;a++){let r=a%2===0,i=[],l=Math.max(0,a-o+1),e=Math.min(s-1,a);for(let t=l;t<=e;t++){let n=a-t;if(n>=0&&n<o)i.push({x:n,y:t})}if(Math.random()>0.55)i.reverse();if(r)for(let t=i.length-1;t>=0;t--)yield i[t];else yield*i}break}case"BRUSH_STROKES":{let a=Array.from({length:s},()=>Array(o).fill(!1)),r=[{x:1,y:0},{x:-1,y:0},{x:0,y:1},{x:0,y:-1},{x:1,y:1},{x:1,y:-1},{x:-1,y:1},{x:-1,y:-1}],i=(t,n)=>t>=0&&t<o&&n>=0&&n<s,l=0,e=o*s;for(let t=0;t<e*6&&l<e;t++){let n=Math.floor(Math.random()*o),c=Math.floor(Math.random()*s),p=r[Math.floor(Math.random()*r.length)],f=3+Math.floor(Math.random()*16);for(let d=0;d<f;d++){if(!i(n,c))break;if(!a[c][n])a[c][n]=!0,l++,yield{x:n,y:c};if(Math.random()>0.72)p=r[Math.floor(Math.random()*r.length)];n+=p.x,c+=p.y}}for(let t=0;t<s;t++)for(let n=0;n<o;n++)if(!a[t][n])yield{x:n,y:t};break}case"SPIRAL_FROM_CENTER":case"SPIRAL_TO_CENTER":{let a=new Set,r=o*s,i=Math.floor(o/2),l=Math.floor(s/2),e=[[1,0],[0,1],[-1,0],[0,-1]],t=0,n=1,c=(f,d)=>f>=0&&f<o&&d>=0&&d<s,p=function*(){let f=0;while(f<r){for(let d=0;d<2;d++){for(let w=0;w<n;w++){if(c(i,l)){let m=`${i},${l}`;if(!a.has(m)){if(a.add(m),yield{x:i,y:l},f++,f>=r)return}}i+=e[t][0],l+=e[t][1]}t=(t+1)%4}n++}};if(this.strategy==="SPIRAL_FROM_CENTER")yield*p();else{let f=[...p()];for(let d=f.length-1;d>=0;d--)yield f[d]}break}case"SCRIBBLE":{let a=new Set,r=o*s,i=Math.floor(o/2),l=Math.floor(s/2);for(let e=0;a.size<r&&e<r*24;e++){let t=`${i},${l}`;if(!a.has(t))a.add(t),yield{x:i,y:l};if(i+=Math.floor(Math.random()*3)-1,l+=Math.floor(Math.random()*3)-1,i<0||i>=o||l<0||l>=s)i=Math.floor(Math.random()*o),l=Math.floor(Math.random()*s)}for(let e=0;e<s;e++)for(let t=0;t<o;t++){let n=`${t},${e}`;if(a.has(n))continue;a.add(n),yield{x:t,y:e}}break}case"CROSSHATCH":{let a=[];for(let l=0;l<o+s-1;l++)for(let e=Math.max(0,l-o+1);e<=Math.min(s-1,l);e++){let t=l-e;a.push({x:t,y:e})}let r=[];for(let l=-s+1;l<o;l++)for(let e=0;e<s;e++){let t=e+l;if(t>=0&&t<o)r.push({x:t,y:e})}let i=new Set;for(let l of[...a,...r]){let e=`${l.x},${l.y}`;if(i.has(e))continue;i.add(e),yield l}break}case"WAVE_SWEEP":{let a=new Set;for(let r=0;r<o;r++){let l=(Math.sin(r/Math.max(1,o-1)*Math.PI*4)+1)*0.5*(s-1)|0;for(let e=0;e<s;e++){let t=l+e,n=l-e;for(let c of[t,n]){if(c<0||c>=s)continue;let p=`${r},${c}`;if(a.has(p))continue;a.add(p),yield{x:r,y:c}}}}break}case"SCATTERED_LINES":{let a=new Set,r=o*s;for(let i=0;a.size<r&&i<r*14;i++){let l=Math.floor(Math.random()*o),e=Math.floor(Math.random()*s),t=Math.random()*Math.PI*2,n=Math.round(Math.cos(t)),c=Math.round(Math.sin(t)),p=6+Math.floor(Math.random()*28);for(let f=0;f<p;f++){if(l<0||l>=o||e<0||e>=s)break;let d=`${l},${e}`;if(!a.has(d))a.add(d),yield{x:l,y:e};l+=n,e+=c}}for(let i=0;i<s;i++)for(let l=0;l<o;l++){let e=`${l},${i}`;if(a.has(e))continue;a.add(e),yield{x:l,y:i}}break}case"CONTOUR_JITTER":{let a=new Set;for(let r=0;r<Math.ceil(Math.min(o,s)/2);r++){let i=[],l=r,e=r,t=o-r-1,n=s-r-1;for(let c=l;c<=t;c++)i.push({x:c,y:e});for(let c=e+1;c<=n;c++)i.push({x:t,y:c});for(let c=t-1;c>=l;c--)i.push({x:c,y:n});for(let c=n-1;c>e;c--)i.push({x:l,y:c});for(let c=i.length-1;c>0;c--){let p=Math.floor(Math.random()*(c+1)),f=i[c];i[c]=i[p],i[p]=f}for(let c of i){let p=`${c.x},${c.y}`;if(a.has(p))continue;a.add(p),yield c}}break}case"SPIRAL_WOBBLE":{let a=new Set,r=o/2,i=s/2,l=Math.hypot(r,i);for(let e=0;a.size<o*s&&e<o*s*9;e++){let t=e/(o*s*9)*l,n=e*0.31+Math.sin(e*0.07)*0.7,c=Math.round(r+Math.cos(n)*t),p=Math.round(i+Math.sin(n)*t);if(c<0||c>=o||p<0||p>=s)continue;let f=`${c},${p}`;if(a.has(f))continue;a.add(f),yield{x:c,y:p}}for(let e=0;e<s;e++)for(let t=0;t<o;t++){let n=`${t},${e}`;if(a.has(n))continue;yield{x:t,y:e}}break}case"CLUSTER_BURSTS":{let a=new Set,r=o*s;for(let i=0;a.size<r&&i<r*12;i++){let l=Math.floor(Math.random()*o),e=Math.floor(Math.random()*s),t=2+Math.floor(Math.random()*10);for(let n=e-t;n<=e+t;n++)for(let c=l-t;c<=l+t;c++){if(c<0||c>=o||n<0||n>=s)continue;if(Math.hypot(c-l,n-e)>t)continue;let p=`${c},${n}`;if(a.has(p))continue;a.add(p),yield{x:c,y:n}}}for(let i=0;i<s;i++)for(let l=0;l<o;l++){let e=`${l},${i}`;if(a.has(e))continue;a.add(e),yield{x:l,y:i}}break}case"ORBITAL":{let a=new Set,r=(o-1)/2,i=(s-1)/2,l=Math.ceil(Math.max(r,i));for(let e=0;e<=l;e++){let t=Math.max(16,Math.ceil(2*Math.PI*Math.max(1,e)*2));for(let n=0;n<t;n++){let c=n/t*Math.PI*2+(e%2?0.3:-0.3),p=Math.round(r+Math.cos(c)*e),f=Math.round(i+Math.sin(c)*e);if(p<0||p>=o||f<0||f>=s)continue;let d=`${p},${f}`;if(a.has(d))continue;a.add(d),yield{x:p,y:f}}}for(let e=0;e<s;e++)for(let t=0;t<o;t++){let n=`${t},${e}`;if(a.has(n))continue;yield{x:t,y:e}}break}case"FLOW_FIELD":{let a=new Set,r=o*s;for(let i=0;a.size<r&&i<r*18;i++){let l=Math.floor(Math.random()*o),e=Math.floor(Math.random()*s);for(let t=0;t<120;t++){if(l<0||l>=o||e<0||e>=s)break;let n=`${l},${e}`;if(!a.has(n))a.add(n),yield{x:l,y:e};let c=Math.sin(l*0.09)*1.8+Math.cos(e*0.08)*1.6+Math.sin((l+e)*0.05);l+=Math.round(Math.cos(c)),e+=Math.round(Math.sin(c))}}for(let i=0;i<s;i++)for(let l=0;l<o;l++){let e=`${l},${i}`;if(a.has(e))continue;a.add(e),yield{x:l,y:i}}break}case"EDGE_IN":{let a=new Set,r=Math.ceil(Math.min(o,s)/2);for(let i=0;i<r;i++){let l=i,e=o-1-i,t=i,n=s-1-i;for(let c=l;c<=e;c++)for(let p of[t,n]){let f=`${c},${p}`;if(a.has(f))continue;a.add(f),yield{x:c,y:p}}for(let c=t+1;c<=n-1;c++)for(let p of[l,e]){let f=`${p},${c}`;if(a.has(f))continue;a.add(f),yield{x:p,y:c}}}break}}}moveStart(o){if(o.button!==0)return;if(o.preventDefault(),o.stopPropagation(),!this.lock)this.moveInfo={globalX:this.position.globalX,globalY:this.position.globalY,clientX:o.clientX,clientY:o.clientY}}moveStop(){if(this.moveInfo)this.moveInfo=void 0,this.position.updateAnchor(),this.pixels.update(),this.updateColors(),M(this.bot)}move(o){if(!this.moveInfo)return;let s=Math.round((o.clientX-this.moveInfo.clientX)/this.position.pixelSize),a=Math.round((o.clientY-this.moveInfo.clientY)/this.position.pixelSize);if(this.moveInfo.globalX!==void 0){if(this.position.globalX=s+this.moveInfo.globalX,this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,this.moveInfo.width-s)}else if(this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,s+this.moveInfo.width);if(this.moveInfo.globalY!==void 0){if(this.position.globalY=a+this.moveInfo.globalY,this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,this.moveInfo.height-a)}else if(this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,a+this.moveInfo.height);this.update(),M(this.bot)}resizeStart(o){if(this.lock||o.button!==0)return;o.preventDefault(),o.stopPropagation(),this.moveInfo={clientX:o.clientX,clientY:o.clientY};let s=o.target;if(s.classList.contains("n"))this.moveInfo.height=this.pixels.height,this.moveInfo.globalY=this.position.globalY;if(s.classList.contains("e"))this.moveInfo.width=this.pixels.width;if(s.classList.contains("s"))this.moveInfo.height=this.pixels.height;if(s.classList.contains("w"))this.moveInfo.width=this.pixels.width,this.moveInfo.globalX=this.position.globalX}export(){let o=document.createElement("a");document.body.append(o),o.href=URL.createObjectURL(new Blob([JSON.stringify(this.toJSON())],{type:"application/json"})),o.download=`${this.pixels.width}x${this.pixels.height}.${Y}`,o.click(),URL.revokeObjectURL(o.href),o.href=this.pixels.canvas.toDataURL("image/webp",1),o.download=`${this.pixels.width}x${this.pixels.height}.webp`,o.click(),URL.revokeObjectURL(o.href),o.remove()}}function Mo(){let o=localStorage.getItem("kglacer-macro:shield-config");if(!o)return!1;try{return JSON.parse(o).enabled!==!1}catch{return!1}}function Eo(o){localStorage.setItem("kglacer-macro:shield-config",JSON.stringify({enabled:o}))}function Na(o){let s=`${o?.host??""} ${o?.username??""}`.toLowerCase(),a=/(mx|mex|mexico)/.test(s)?"MX":"AUTO";localStorage.setItem("__afm_proxy_hint",a)}function Ro(o){if(!Mo())return;if(document.getElementById("kgm-shield-full"))return;Na(o);let s=document.createElement("script");s.id="kgm-shield-full",s.textContent=`// ==UserScript==
