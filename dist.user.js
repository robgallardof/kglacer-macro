...
<span class="premium ${H?"on":""}">${H?u("premium"):""}</span>`,U.querySelector(".swatch").style.setProperty("--swatch-color",x(b.realColor)),U.addEventListener("click",()=>{if(M){M=!1;return}B()}),U.addEventListener("dragstart",(N)=>{M=!0,U.classList.add("dragging"),N.dataTransfer?.setData("text/plain",String(l)),N.dataTransfer.effectAllowed="move"}),U.addEventListener("dragend",()=>{U.classList.remove("dragging")}),U.addEventListener("dragover",(N)=>{N.preventDefault(),U.classList.add("drag-target")}),U.addEventListener("dragleave",()=>{U.classList.remove("drag-target")}),U.addEventListener("drop",(N)=>{N.preventDefault(),U.classList.remove("drag-target");let Y=Number.parseInt(N.dataTransfer?.getData("text/plain")??"-1",10);if(Y<0||Y===l||Y>=this.colors.length)return;this.colors.splice(l,0,...this.colors.splice(Y,1)),j(this.bot),this.updateColors()}),H){let N=document.createElement("button");N.textContent=u("buy"),N.className="buy-chip",N.addEventListener("click",(Y)=>{Y.stopPropagation(),document.getElementById("color-"+b.realColor)?.click()}),U.append(N)}let L=`${J} ${G.join(" ")} ${b.realColor} ${T[b.realColor]}`;if(!f||L.toLowerCase().includes(f))this.$colorsDialogList.append(U);let S=(N)=>{let Y=l,O=K.getBoundingClientRect().width,r=(I)=>{if(Y=Math.min(this.colors.length-1,Math.max(0,Math.round(l+(I.clientX-N.clientX)/O))),Y!==l)A=!0;let C=0;for(let d of g.children){if(d===K)continue;if(C===Y)C++;d.style.setProperty("--wleft",p*C+"%"),C++}K.style.setProperty("--wleft",p*Y+"%")};document.addEventListener("mousemove",r),document.addEventListener("mouseup",()=>{if(document.removeEventListener("mousemove",r),Y!==l)this.colors.splice(Y,0,...this.colors.splice(l,1));j(this.bot),K.removeEventListener("mousedown",S),setTimeout(()=>{this.updateColors()},200)},{once:!0})};if(K.addEventListener("mousedown",S),!H)K.addEventListener("click",B)}this.$colors.append(g)}*strategyPositionIterator(o){let p=o??this.pixels.pixels[0].length,g=o??this.pixels.pixels.length;switch(this.strategy){case"DOWN":{for(let f=0;f<g;f++)for(let w=0;w<p;w++)yield{x:w,y:f};break}case"UP":{for(let f=g-1;f>=0;f--)for(let w=0;w<p;w++)yield{x:w,y:f};break}case"LEFT":{for(let f=0;f<p;f++)for(let w=0;w<g;w++)yield{x:f,y:w};break}case"RIGHT":{for(let f=p-1;f>=0;f--)for(let w=0;w<g;w++)yield{x:f,y:w};break}case"RANDOM":{let f=[];for(let w=0;w<g;w++)for(let l=0;l<p;l++)f.push({x:l,y:w});for(let w=f.length-1;w>=0;w--){let l=Math.floor(Math.random()*(w+1)),z=f[w];f[w]=f[l],f[l]=z}yield*f;break}case"ZIGZAG":{for(let f=0;f<g;f++)if(f%2===0)for(let w=0;w<p;w++)yield{x:w,y:f};else for(let w=p-1;w>=0;w--)yield{x:w,y:f};break}case"HUMANIZED":{let f=Math.max(4,Math.floor(Math.min(p,g)/10)),w=[];for(let l=0;l<g;l+=f)for(let z=0;z<p;z+=f)w.push({x:z,y:l});for(let l=w.length-1;l>=0;l--){let z=Math.floor(Math.random()*(l+1)),b=w[l];w[l]=w[z],w[z]=b}for(let l=0;l<w.length;l++){let z=w[l],b=Math.min(g,z.y+f),A=Math.min(p,z.x+f);for(let M=z.y;M<b;M++)if(Math.random()>0.35)for(let c=z.x;c<A;c++)yield{x:c,y:M};else for(let c=A-1;c>=z.x;c--)yield{x:c,y:M}}break}case"HUMAN_SOFT_DITHER":{let f=new Set;for(let w=0;w<g;w++){let l=Math.floor(Math.random()*3)-1;if((w+l)%2===0)for(let b=0;b<p;b+=2)f.add(`${b},${w}`),yield{x:b,y:w};else for(let b=1;b<p;b+=2)f.add(`${b},${w}`),yield{x:b,y:w}}for(let w=0;w<g;w++)for(let l=0;l<p;l++){let z=`${l},${w}`;if(f.has(z))continue;yield{x:l,y:w}}break}case"HUMAN_PATCHY":{let f=new Set,w=p*g;while(f.size<w){let l=Math.floor(Math.random()*p),z=Math.floor(Math.random()*g),b=1+Math.floor(Math.random()*5);for(let A=z-b;A<=z+b;A++)for(let M=l-b;M<=l+b;M++){if(M<0||M>=p||A<0||A>=g)continue;if(Math.hypot(M-l,A-z)>b+Math.random()*1.2)continue;let H=`${M},${A}`;if(f.has(H))continue;f.add(H),yield{x:M,y:A}}}break}case"HUMAN_SWEEP_ARCS":{let f=new Set,w=(p-1)/2,l=(g-1)/2,z=Math.hypot(w,l);for(let b=0;b<4;b++){let A=Math.random()*Math.PI*2;for(let M=0;M<=z;M+=0.35){let H=Math.PI/2+Math.random()*(Math.PI/1.5),c=Math.max(10,Math.floor(M*8));for(let J=0;J<c;J++){let G=A+H*J/c+Math.sin(M)*0.08,B=Math.round(w+Math.cos(G)*M),K=Math.round(l+Math.sin(G)*M);if(B<0||B>=p||K<0||K>=g)continue;let U=`${B},${K}`;if(f.has(U))continue;f.add(U),yield{x:B,y:K}}}}for(let b=0;b<g;b++)for(let A=0;A<p;A++){let M=`${A},${b}`;if(f.has(M))continue;yield{x:A,y:b}}break}case"HUMAN_MICRO_CORRECTIONS":{let f=new Set;for(let w=0;w<g;w++){let l=w%2===0?1:-1,z=l>0?0:p-1;for(let b=0;b<p;b++){let A=z+(Math.random()>0.82?l:0),M=w+(Math.random()>0.9?1:0);for(let H of[{x:z,y:w},{x:A,y:w},{x:z,y:M}]){if(H.x<0||H.x>=p||H.y<0||H.y>=g)continue;let c=`${H.x},${H.y}`;if(f.has(c))continue;f.add(c),yield H}z+=l}}for(let w=0;w<g;w++)for(let l=0;l<p;l++){let z=`${l},${w}`;if(f.has(z))continue;yield{x:l,y:w}}break}case"HUMAN_JITTER_FILL":{let f=[];for(let w=0;w<g;w++)for(let l=0;l<p;l++)f.push({x:l,y:w});f.sort((w,l)=>{let z=w.y+(Math.random()-0.5)*1.8,b=l.y+(Math.random()-0.5)*1.8;if(z!==b)return z-b;return w.x+(Math.random()-0.5)*2-(l.x+(Math.random()-0.5)*2)}),yield*f;break}case"HUMAN_CORNER_BIAS":{let f=[{x:0,y:0},{x:p-1,y:0},{x:0,y:g-1},{x:p-1,y:g-1}],w=f[Math.floor(Math.random()*f.length)],l=[];for(let z=0;z<g;z++)for(let b=0;b<p;b++){let M=Math.hypot(b-w.x,z-w.y)+Math.random()*3.5;l.push({point:{x:b,y:z},score:M})}l.sort((z,b)=>z.score-b.score);for(let z of l)yield z.point;break}case"HUMAN_LONG_STROKES":{let f=new Set,w=p*g;while(f.size<w){let l=Math.floor(Math.random()*p),z=Math.floor(Math.random()*g),b=Math.random()*Math.PI*2,A=Math.sign(Math.cos(b)),M=Math.sign(Math.sin(b)),H=10+Math.floor(Math.random()*40);for(let c=0;c<H;c++){if(l<0||l>=p||z<0||z>=g)break;let J=`${l},${z}`;if(!f.has(J))f.add(J),yield{x:l,y:z};if(Math.random()>0.78)l+=M,z+=A;else l+=A,z+=M}}break}case"HUMAN_TAP_CLUSTERS":{let f=new Set,w=p*g;while(f.size<w){let l=Math.floor(Math.random()*p),z=Math.floor(Math.random()*g),b=3+Math.floor(Math.random()*10);for(let A=0;A<b;A++){let M=Math.round(l+(Math.random()-0.5)*6),H=Math.round(z+(Math.random()-0.5)*6);if(M<0||M>=p||H<0||H>=g)continue;let c=`${M},${H}`;if(f.has(c))continue;f.add(c),yield{x:M,y:H}}}break}case"HUMAN_MESSY_SPIRAL":{let f=new Set,w=(p-1)/2,l=(g-1)/2,z=Math.hypot(w,l)+2;for(let b=0;f.size<p*g;b++){let A=b/3,M=Math.min(z,A*0.18),H=A*0.29+Math.sin(A*0.13)*0.8,c=Math.round(w+Math.cos(H)*M+Math.sin(A)*0.7),J=Math.round(l+Math.sin(H)*M+Math.cos(A)*0.7);if(c<0||c>=p||J<0||J>=g){if(b>p*g*18)break;continue}let G=`${c},${J}`;if(f.has(G)){if(Math.random()>0.9)continue}else f.add(G),yield{x:c,y:J};if(b>p*g*18)break}for(let b=0;b<g;b++)for(let A=0;A<p;A++){let M=`${A},${b}`;if(f.has(M))continue;yield{x:A,y:b}}break}case"HUMAN_DRUNK_WALK":{let f=new Set,w=Math.floor(Math.random()*p),l=Math.floor(Math.random()*g),z=[{x:-1,y:0},{x:1,y:0},{x:0,y:-1},{x:0,y:1},{x:-1,y:-1},{x:1,y:-1},{x:-1,y:1},{x:1,y:1}];while(f.size<p*g){let b=`${w},${l}`;if(!f.has(b))f.add(b),yield{x:w,y:l};let A=[];for(let c of z){let J=w+c.x,G=l+c.y;if(J<0||J>=p||G<0||G>=g)continue;A.push({x:J,y:G})}if(!A.length)break;let M=A.filter((c)=>{return!f.has(`${c.x},${c.y}`)});if(M.length&&Math.random()>0.2){let c=M[Math.floor(Math.random()*M.length)];w=c.x,l=c.y;continue}let H=A[Math.floor(Math.random()*A.length)];w=H.x,l=H.y}for(let b=0;b<g;b++)for(let A=0;A<p;A++){let M=`${A},${b}`;if(f.has(M))continue;yield{x:A,y:b}}break}case"HUMAN_NOISE_CLOUD":{let f=[];for(let w=0;w<g;w++)for(let l=0;l<p;l++){let z=Math.sin((l+1)*0.93+Math.random()*0.8)+Math.cos((w+1)*1.17+Math.random()*0.8),b=(Math.random()-0.5)*2.6,A=Math.hypot(l-p/2,w-g/2)*0.08;f.push({point:{x:l,y:w},score:z+b+A})}f.sort((w,l)=>w.score-l.score);for(let w of f)yield w.point;break}case"HUMAN_PATCH_JUMP":{let f=new Set,w=[];for(let l=0;l<Math.max(6,p*g/18);l++)w.push({x:Math.floor(Math.random()*p),y:Math.floor(Math.random()*g)});while(f.size<p*g){let l=w[Math.floor(Math.random()*w.length)],z=1+Math.floor(Math.random()*3),b=1+Math.floor(Math.random()*3);for(let A=l.y-b;A<=l.y+b;A++)for(let M=l.x-z;M<=l.x+z;M++){if(M<0||M>=p||A<0||A>=g)continue;if(Math.random()>0.86)continue;let H=`${M},${A}`;if(f.has(H))continue;f.add(H),yield{x:M,y:A}}if(Math.random()>0.72&&w.length<p*g/2)w.push({x:Math.floor(Math.random()*p),y:Math.floor(Math.random()*g)});if(f.size>p*g*0.92)break}for(let l=0;l<g;l++)for(let z=0;z<p;z++){let b=`${z},${l}`;if(f.has(b))continue;yield{x:z,y:l}}break}case"DIAGONAL_BRUSH":{for(let f=0;f<p+g-1;f++){let w=f%2===0,l=[],z=Math.max(0,f-p+1),b=Math.min(g-1,f);for(let A=z;A<=b;A++){let M=f-A;if(M>=0&&M<p)l.push({x:M,y:A})}if(Math.random()>0.55)l.reverse();if(w)for(let A=l.length-1;A>=0;A--)yield l[A];else yield*l}break}case"BRUSH_STROKES":{let f=Array.from({length:g},()=>Array(p).fill(!1)),w=[{x:1,y:0},{x:-1,y:0},{x:0,y:1},{x:0,y:-1},{x:1,y:1},{x:1,y:-1},{x:-1,y:1},{x:-1,y:-1}],l=(A,M)=>A>=0&&A<p&&M>=0&&M<g,z=0,b=p*g;for(let A=0;A<b*6&&z<b;A++){let M=Math.floor(Math.random()*p),H=Math.floor(Math.random()*g),c=w[Math.floor(Math.random()*w.length)],J=3+Math.floor(Math.random()*16);for(let G=0;G<J;G++){if(!l(M,H))break;if(!f[H][M])f[H][M]=!0,z++,yield{x:M,y:H};if(Math.random()>0.72)c=w[Math.floor(Math.random()*w.length)];M+=c.x,H+=c.y}}for(let A=0;A<g;A++)for(let M=0;M<p;M++)if(!f[A][M])yield{x:M,y:A};break}case"SPIRAL_FROM_CENTER":case"SPIRAL_TO_CENTER":{let f=new Set,w=p*g,l=Math.floor(p/2),z=Math.floor(g/2),b=[[1,0],[0,1],[-1,0],[0,-1]],A=0,M=1,H=(J,G)=>J>=0&&J<p&&G>=0&&G<g,c=function*(){let J=0;while(J<w){for(let G=0;G<2;G++){for(let B=0;B<M;B++){if(H(l,z)){let K=`${l},${z}`;if(!f.has(K)){if(f.add(K),yield{x:l,y:z},J++,J>=w)return}}l+=b[A][0],z+=b[A][1]}A=(A+1)%4}M++}};if(this.strategy==="SPIRAL_FROM_CENTER")yield*c();else{let J=[...c()];for(let G=J.length-1;G>=0;G--)yield J[G]}break}case"SCRIBBLE":{let f=new Set,w=p*g,l=Math.floor(p/2),z=Math.floor(g/2);for(let b=0;f.size<w&&b<w*24;b++){let A=`${l},${z}`;if(!f.has(A))f.add(A),yield{x:l,y:z};if(l+=Math.floor(Math.random()*3)-1,z+=Math.floor(Math.random()*3)-1,l<0||l>=p||z<0||z>=g)l=Math.floor(Math.random()*p),z=Math.floor(Math.random()*g)}for(let b=0;b<g;b++)for(let A=0;A<p;A++){let M=`${A},${b}`;if(f.has(M))continue;f.add(M),yield{x:A,y:b}}break}case"CROSSHATCH":{let f=[];for(let z=0;z<p+g-1;z++)for(let b=Math.max(0,z-p+1);b<=Math.min(g-1,z);b++){let A=z-b;f.push({x:A,y:b})}let w=[];for(let z=-g+1;z<p;z++)for(let b=0;b<g;b++){let A=b+z;if(A>=0&&A<p)w.push({x:A,y:b})}let l=new Set;for(let z of[...f,...w]){let b=`${z.x},${z.y}`;if(l.has(b))continue;l.add(b),yield z}break}case"WAVE_SWEEP":{let f=new Set;for(let w=0;w<p;w++){let z=(Math.sin(w/Math.max(1,p-1)*Math.PI*4)+1)*0.5*(g-1)|0;for(let b=0;b<g;b++){let A=z+b,M=z-b;for(let H of[A,M]){if(H<0||H>=g)continue;let c=`${w},${H}`;if(f.has(c))continue;f.add(c),yield{x:w,y:H}}}}break}case"SCATTERED_LINES":{let f=new Set,w=p*g;for(let l=0;f.size<w&&l<w*14;l++){let z=Math.floor(Math.random()*p),b=Math.floor(Math.random()*g),A=Math.random()*Math.PI*2,M=Math.round(Math.cos(A)),H=Math.round(Math.sin(A)),c=6+Math.floor(Math.random()*28);for(let J=0;J<c;J++){if(z<0||z>=p||b<0||b>=g)break;let G=`${z},${b}`;if(!f.has(G))f.add(G),yield{x:z,y:b};z+=M,b+=H}}for(let l=0;l<g;l++)for(let z=0;z<p;z++){let b=`${z},${l}`;if(f.has(b))continue;f.add(b),yield{x:z,y:l}}break}case"CONTOUR_JITTER":{let f=new Set;for(let w=0;w<Math.ceil(Math.min(p,g)/2);w++){let l=[],z=w,b=w,A=p-w-1,M=g-w-1;for(let H=z;H<=A;H++)l.push({x:H,y:b});for(let H=b+1;H<=M;H++)l.push({x:A,y:H});for(let H=A-1;H>=z;H--)l.push({x:H,y:M});for(let H=M-1;H>b;H--)l.push({x:z,y:H});for(let H=l.length-1;H>0;H--){let c=Math.floor(Math.random()*(H+1)),J=l[H];l[H]=l[c],l[c]=J}for(let H of l){let c=`${H.x},${H.y}`;if(f.has(c))continue;f.add(c),yield H}}break}case"SPIRAL_WOBBLE":{let f=new Set,w=p/2,l=g/2,z=Math.hypot(w,l);for(let b=0;f.size<p*g&&b<p*g*9;b++){let A=b/(p*g*9)*z,M=b*0.31+Math.sin(b*0.07)*0.7,H=Math.round(w+Math.cos(M)*A),c=Math.round(l+Math.sin(M)*A);if(H<0||H>=p||c<0||c>=g)continue;let J=`${H},${c}`;if(f.has(J))continue;f.add(J),yield{x:H,y:c}}for(let b=0;b<g;b++)for(let A=0;A<p;A++){let M=`${A},${b}`;if(f.has(M))continue;yield{x:A,y:b}}break}case"CLUSTER_BURSTS":{let f=new Set,w=p*g;for(let l=0;f.size<w&&l<w*12;l++){let z=Math.floor(Math.random()*p),b=Math.floor(Math.random()*g),A=2+Math.floor(Math.random()*10);for(let M=b-A;M<=b+A;M++)for(let H=z-A;H<=z+A;H++){if(H<0||H>=p||M<0||M>=g)continue;if(Math.hypot(H-z,M-b)>A)continue;let c=`${H},${M}`;if(f.has(c))continue;f.add(c),yield{x:H,y:M}}}for(let l=0;l<g;l++)for(let z=0;z<p;z++){let b=`${z},${l}`;if(f.has(b))continue;f.add(b),yield{x:z,y:l}}break}case"ORBITAL":{let f=new Set,w=(p-1)/2,l=(g-1)/2,z=Math.ceil(Math.max(w,l));for(let b=0;b<=z;b++){let A=Math.max(16,Math.ceil(2*Math.PI*Math.max(1,b)*2));for(let M=0;M<A;M++){let H=M/A*Math.PI*2+(b%2?0.3:-0.3),c=Math.round(w+Math.cos(H)*b),J=Math.round(l+Math.sin(H)*b);if(c<0||c>=p||J<0||J>=g)continue;let G=`${c},${J}`;if(f.has(G))continue;f.add(G),yield{x:c,y:J}}}for(let b=0;b<g;b++)for(let A=0;A<p;A++){let M=`${A},${b}`;if(f.has(M))continue;yield{x:A,y:b}}break}case"FLOW_FIELD":{let f=new Set,w=p*g;for(let l=0;f.size<w&&l<w*18;l++){let z=Math.floor(Math.random()*p),b=Math.floor(Math.random()*g);for(let A=0;A<120;A++){if(z<0||z>=p||b<0||b>=g)break;let M=`${z},${b}`;if(!f.has(M))f.add(M),yield{x:z,y:b};let H=Math.sin(z*0.09)*1.8+Math.cos(b*0.08)*1.6+Math.sin((z+b)*0.05);z+=Math.round(Math.cos(H)),b+=Math.round(Math.sin(H))}}for(let l=0;l<g;l++)for(let z=0;z<p;z++){let b=`${z},${l}`;if(f.has(b))continue;f.add(b),yield{x:z,y:l}}break}case"EDGE_IN":{let f=new Set,w=Math.ceil(Math.min(p,g)/2);for(let l=0;l<w;l++){let z=l,b=p-1-l,A=l,M=g-1-l;for(let H=z;H<=b;H++)for(let c of[A,M]){let J=`${H},${c}`;if(f.has(J))continue;f.add(J),yield{x:H,y:c}}for(let H=A+1;H<=M-1;H++)for(let c of[z,b]){let J=`${c},${H}`;if(f.has(J))continue;f.add(J),yield{x:c,y:H}}}break}}}moveStart(o){if(o.button!==0)return;if(o.preventDefault(),o.stopPropagation(),!this.lock)this.moveInfo={globalX:this.position.globalX,globalY:this.position.globalY,clientX:o.clientX,clientY:o.clientY}}moveStop(){if(this.moveInfo)this.moveInfo=void 0,this.position.updateAnchor(),this.pixels.update(),this.updateColors(),j(this.bot)}move(o){if(!this.moveInfo)return;let p=Math.round((o.clientX-this.moveInfo.clientX)/this.position.pixelSize),g=Math.round((o.clientY-this.moveInfo.clientY)/this.position.pixelSize);if(this.moveInfo.globalX!==void 0){if(this.position.globalX=p+this.moveInfo.globalX,this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,this.moveInfo.width-p)}else if(this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,p+this.moveInfo.width);if(this.moveInfo.globalY!==void 0){if(this.position.globalY=g+this.moveInfo.globalY,this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,this.moveInfo.height-g)}else if(this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,g+this.moveInfo.height);this.update(),j(this.bot)}resizeStart(o){if(this.lock||o.button!==0)return;o.preventDefault(),o.stopPropagation(),this.moveInfo={clientX:o.clientX,clientY:o.clientY};let p=o.target;if(p.classList.contains("n"))this.moveInfo.height=this.pixels.height,this.moveInfo.globalY=this.position.globalY;if(p.classList.contains("e"))this.moveInfo.width=this.pixels.width;if(p.classList.contains("s"))this.moveInfo.height=this.pixels.height;if(p.classList.contains("w"))this.moveInfo.width=this.pixels.width,this.moveInfo.globalX=this.position.globalX}export(){let o=document.createElement("a");document.body.append(o),o.href=URL.createObjectURL(new Blob([JSON.stringify(this.toJSON())],{type:"application/json"})),o.download=`${this.pixels.width}x${this.pixels.height}.${i}`,o.click(),URL.revokeObjectURL(o.href),o.href=this.pixels.canvas.toDataURL("image/webp",1),o.download=`${this.pixels.width}x${this.pixels.height}.webp`,o.click(),URL.revokeObjectURL(o.href),o.remove()}}var j0=`/* stylelint-disable declaration-no-important */
...
`;var Z0="kglacer-macro:overlay-hidden",k0="https://raw.githubusercontent.com/robgallardof/kglacer-macro/refs/heads/main/src/img/logo.svg";class A0 extends n{bot;element=document.createElement("div");get status(){return this.$status.innerHTML}set status(o){this.$status.innerHTML=o}get open(){return this.element.classList.contains("wopen")}set open(o){if(o)this.element.classList.add("wopen");else this.element.classList.remove("wopen")}$settings;$status;$shortcuts;$locale;$topbar;$draw;$addImage;$toggleOverlay;$strategy;$progressLine;$progressText;$images;$wopenButton;$widgetLogo;activeImageIndex=-1;constructor(o){super();this.bot=o;this.element.classList.add("wwidget"),this.element.style.display="none",this.element.innerHTML=F0,$(this.element),document.body.append(this.element),this.populateElementsWithSelector(this.element,{$wopenButton:".wopen-button",$widgetLogo:".widget-logo",$settings:".wform",$status:".wstatus",$shortcuts:".shortcuts",$locale:".locale",$topbar:".wtopbar",$draw:".draw",$addImage:".add-image",$toggleOverlay:".toggle-overlay",$strategy:".strategy",$progressLine:".wprogress div",$progressText:".wprogress span",$images:".images"}),this.$widgetLogo.src=k0,this.$wopenButton.addEventListener("click",()=>this.open=!this.open),this.$draw.addEventListener("click",()=>this.bot.draw()),this.$addImage.addEventListener("click",()=>this.addImage()),this.$toggleOverlay.addEventListener("click",()=>{this.toggleOverlay()}),this.$strategy.addEventListener("change",()=>{this.bot.strategy=this.$strategy.value}),this.$locale.value=b0(),this.$locale.addEventListener("change",()=>{G0(this.$locale.value),$(this.element);for(let p=0;p<this.bot.images.length;p++)this.bot.images[p].applyLocale();this.refreshOverlayToggleText()}),this.registerEvent(document,"keydown",this.handleKeyboard.bind(this),{passive:!1}),this.update(),this.syncOverlayVisibilityFromStorage(),this.open=!0,console.log("[KGM][Widget] Widget mounted and opened")}unlock(){this.element.style.display=""}addImage(){return console.log("[KGM][Widget] Add image flow started"),this.setDisabled("add-image",!0),this.run("Adding image",async()=>{await this.bot.updateColors();let o=document.createElement("input");o.type="file",o.accept=`image/*,.${i}`,o.click(),await V(o,["change"],["cancel","error"]);let p=o.files?.[0];if(!p)throw new z0(this.bot);console.log("[KGM][Widget] File selected",{name:p.name,size:p.size,type:p.type});let g;if(p.name.endsWith(`.${i}`))g=await m.fromJSON(this.bot,JSON.parse(await p.text()));else{let f=new FileReader;f.readAsDataURL(p),await V(f,["load"],["error"]);let w=await this.compressImageBeforeLoad(f.result),l=new Image;l.src=w,await V(l,["load"],["error"]),g=new m(this.bot,F.fromScreenPosition(this.bot,this.defaultImageScreenPosition()),new W(this.bot,l))}this.bot.images.push(g),console.log("[KGM][Widget] Image instance added",{images:this.bot.images.length}),await this.bot.readMap(),g.updateTasks(),j(this.bot,!0),console.log("[KGM][Widget] Image persisted, reloading page"),document.location.reload()},()=>{this.setDisabled("add-image",!1)})}defaultImageScreenPosition(){let o=Math.round(this.element.getBoundingClientRect().width);return{x:Math.max(256,o),y:32}}async compressImageBeforeLoad(o){let p=new Image;if(p.src=o,await V(p,["load"],["error"]),!(p.naturalWidth*p.naturalHeight>3000000||o.length>3000000))return o;let f=document.createElement("canvas");f.width=p.naturalWidth,f.height=p.naturalHeight;let w=f.getContext("2d");if(!w)return o;return w.drawImage(p,0,0),f.toDataURL("image/png")}update(){this.$strategy.value=this.bot.strategy;let o=0,p=0;for(let l=0;l<this.bot.images.length;l++){let z=this.bot.images[l];o+=z.pixels.pixels.length*z.pixels.pixels[0].length,p+=z.tasks.length}let g=o-p,f=g/o*100|0;this.$progressText.textContent=`${g}/${o} ${f}% ETA: ${p/120|0}h`,this.$progressLine.style.transform=`scaleX(${f}%)`,this.$images.innerHTML="";let w=document.createDocumentFragment();for(let l=0;l<this.bot.images.length;l++){let z=this.bot.images[l],b=document.createElement("div");w.append(b),b.className="image",b.innerHTML=`<button class="preview" title="Focus image">
  ...
  </div>`,b.querySelector(".preview").addEventListener("click",()=>{this.activeImageIndex=l,z.position.scrollScreenTo()}),b.querySelector(".settings").addEventListener("click",()=>{this.activeImageIndex=l,z.openSettingsPanel()}),b.querySelector(".remove").addEventListener("click",()=>{if(this.activeImageIndex===l)this.activeImageIndex=-1;z.destroy()}),b.querySelector(".up").addEventListener("click",()=>{l0(this.bot.images,l,l-1),this.update(),j(this.bot)}),b.querySelector(".down").addEventListener("click",()=>{l0(this.bot.images,l,l+1),this.update(),j(this.bot)})}this.$images.append(w)}syncOverlayVisibilityFromStorage(){let o=localStorage.getItem(Z0)==="true";document.body.classList.toggle("overlay-hidden",o),this.refreshOverlayToggleText()}toggleOverlay(o){let p=o??!document.body.classList.contains("overlay-hidden");document.body.classList.toggle("overlay-hidden",p),localStorage.setItem(Z0,String(p)),this.refreshOverlayToggleText()}refreshOverlayToggleText(){this.$toggleOverlay.textContent=document.body.classList.contains("overlay-hidden")?`${u("toggleOverlay")} (${u("disabled")})`:`${u("toggleOverlay")} (${u("enabled")})`}setDisabled(o,p){this.element.querySelector("."+o).disabled=p}async run(o,p,g,f="..."){console.log("[KGM][Widget] Task started",{status:o});let w=this.status;this.status=`${f} ${o}`;try{let l=await p();return this.status=w,console.log("[KGM][Widget] Task completed",{status:o}),l}catch(l){if(!(l instanceof o0))console.error(l),this.status=`Error: ${o}`;throw console.error("[KGM][Widget] Task failed",{status:o,error:l}),l}finally{await g?.()}}handleKeyboard(o){if(D0(o.target))return;if(X(o,q.toggleWidget)){o.preventDefault(),this.open=!this.open;return}if(X(o,q.showShortcuts)){o.preventDefault(),this.$shortcuts.scrollIntoView({behavior:"smooth",block:"center"}),this.$shortcuts.classList.remove("shortcut-pulse"),requestAnimationFrame(()=>{this.$shortcuts.classList.add("shortcut-pulse")});return}if(X(o,q.toggleOverlay)){o.preventDefault(),this.toggleOverlay();return}if(X(o,q.focusNextImage)){o.preventDefault(),this.focusImageByStep(1);return}if(X(o,q.focusPreviousImage)){o.preventDefault(),this.focusImageByStep(-1);return}if(X(o,q.openColorPanel)){o.preventDefault(),this.openColorPanelForActiveImage();return}if(X(o,q.toggleImageLock)){o.preventDefault(),this.toggleLockForActiveImage();return}if(X(o,q.addImage)&&!this.$addImage.disabled){o.preventDefault(),this.addImage();return}if(X(o,q.draw)&&!this.$draw.disabled)o.preventDefault(),this.bot.draw()}focusImageByStep(o){if(!this.bot.images.length)return;if(this.activeImageIndex<0||this.activeImageIndex>=this.bot.images.length)this.activeImageIndex=o>0?0:this.bot.images.length-1;else this.activeImageIndex=(this.activeImageIndex+o+this.bot.images.length)%this.bot.images.length;this.bot.images[this.activeImageIndex].position.scrollScreenTo()}getActiveImage(){if(!this.bot.images.length)return;if(this.activeImageIndex<0||this.activeImageIndex>=this.bot.images.length)this.activeImageIndex=0;return this.bot.images[this.activeImageIndex]}openColorPanelForActiveImage(){let o=this.getActiveImage();if(!o)return;o.openColorPanel()}toggleLockForActiveImage(){let o=this.getActiveImage();if(!o)return;o.lock=!o.lock,o.update(),j(this.bot)}}var $0=2,q0="[KGM]";class X0{unavailableColors=new Set;mapsCache=new Map;me;$stars=[];strategy="SEQUENTIAL";images=[];widget=new A0(this);markerPixelPositionResolvers=[];lastColor;log(o,p){if(p===void 0)console.log(`${q0} ${o}`);else console.log(`${q0} ${o}`,p)}constructor(){this.log("Boot sequence started"),this.requestAccess().then((o)=>{if(!o){this.log("Access denied; controllers remain hidden"),this.widget.status="Access denied";return}this.widget.unlock(),this.log("Access granted; starting initialization flow"),this.start()})}start(){let o=Y0();if(this.log("Save loaded",{hasSave:Boolean(o),imageCount:o?.images.length??0,strategy:o?.strategy}),o){for(let g=0;g<o.images.length;g++){let f=o.images[g];y({x:f.position[0]-1000,y:f.position[1]-1000}),y({x:f.position[0]+1000,y:f.position[1]+1000})}this.strategy=o.strategy}this.registerFetchInterceptor(),this.log("Fetch interceptor registered");let p=document.createElement("style");p.textContent=j0.replace("FAKE_FAVORITE_LOCATIONS",h.length.toString()),document.head.append(p),this.log("Styles injected",{fakeFavoriteLocations:h.length}),this.widget.run("Initializing",async()=>{this.log("Widget initialization flow started"),await this.waitForElement("login",".avatar.center-absolute.absolute"),await this.waitForElement("pixel count",".btn.btn-primary.btn-lg.relative.z-30 canvas");let g=await this.waitForElement("canvas",".maplibregl-canvas-container");if(new MutationObserver((f)=>{for(let w=0;w<f.length;w++)if(f[w].removedNodes.length!==0){this.updateStars();break}this.updateImages()}).observe(g,{attributes:!0,childList:!0,subtree:!0}),this.updateStars(),this.log("Stars updated after boot",{stars:this.$stars.length}),await P(500),await this.updateColors(),o)for(let f=0;f<o.images.length;f++){let w=await m.fromJSON(this,o.images[f]);this.images.push(w),w.update()}this.log("Saved images restored",{images:this.images.length}),await this.readMap(),this.updateTasks(),this.widget.setDisabled("draw",!1),this.widget.setDisabled("add-image",!1),this.log("Initialization completed; controls enabled")})}async requestAccess(){let o="48583a634c2513c2d9d6ffa6107d70b1a5a46cfb515c910b120b1c5f550bcbc2",p=document.createElement("dialog");return p.innerHTML=`<form method="dialog" style="min-width:320px;padding:18px;border:none;border-radius:12px;background:#0f172a;color:#e2e8f0;font:14px/1.4 system-ui,Segoe UI,sans-serif;">
