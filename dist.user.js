...
<span class="premium"></span>`,d.querySelector(".swatch").style.setProperty("--swatch-color",zo(i.realColor)),d.addEventListener("click",()=>{if(l){l=!1;return}f(),this.updateTasks()}),d.addEventListener("dragstart",(h)=>{l=!0,d.classList.add("dragging"),h.dataTransfer?.setData("text/plain",String(a)),h.dataTransfer.effectAllowed="move"}),d.addEventListener("dragend",()=>{d.classList.remove("dragging")}),d.addEventListener("dragover",(h)=>{h.preventDefault(),d.classList.add("drag-target")}),d.addEventListener("dragleave",()=>{d.classList.remove("drag-target")}),d.addEventListener("drop",(h)=>{h.preventDefault(),d.classList.remove("drag-target");let u=Number.parseInt(h.dataTransfer?.getData("text/plain")??"-1",10);if(u<0||u===a||u>=this.colors.length)return;this.colors.splice(a,0,...this.colors.splice(u,1)),S(this.bot),this.updateColors()});let w=document.createElement("button");w.textContent=p("buy"),w.className="buy-chip",w.addEventListener("click",(h)=>{h.stopPropagation(),document.getElementById("color-"+i.realColor)?.click()}),d.append(w);let m=`${t} ${n.join(" ")} ${i.realColor} ${G[i.realColor]}`;if(!r||m.toLowerCase().includes(r))this.$colorsDialogList.append(d)}}syncColorBulkToggle(){let o=this.colors.filter((a)=>!a.disabled).length,r=o===this.colors.length;this.$toggleAllColors.checked=r,this.$toggleAllColors.indeterminate=o>0&&!r}*strategyPositionIterator(){let o=this.pixels.pixels[0].length,r=this.pixels.pixels.length;switch(this.strategy){case"DOWN":{for(let a=0;a<r;a++)for(let s=0;s<o;s++)yield{x:s,y:a};break}case"UP":{for(let a=r-1;a>=0;a--)for(let s=0;s<o;s++)yield{x:s,y:a};break}case"LEFT":{for(let a=0;a<o;a++)for(let s=0;s<r;s++)yield{x:a,y:s};break}case"RIGHT":{for(let a=o-1;a>=0;a--)for(let s=0;s<r;s++)yield{x:a,y:s};break}case"RANDOM":{let a=[];for(let s=0;s<r;s++)for(let i=0;i<o;i++)a.push({x:i,y:s});for(let s=a.length-1;s>=0;s--){let i=Math.floor(Math.random()*(s+1)),l=a[s];a[s]=a[i],a[i]=l}yield*a;break}case"ZIGZAG":{for(let a=0;a<r;a++)if(a%2===0)for(let s=0;s<o;s++)yield{x:s,y:a};else for(let s=o-1;s>=0;s--)yield{x:s,y:a};break}case"HUMANIZED":{let a=Math.max(4,Math.floor(Math.min(o,r)/10)),s=[];for(let i=0;i<r;i+=a)for(let l=0;l<o;l+=a)s.push({x:l,y:i});for(let i=s.length-1;i>=0;i--){let l=Math.floor(Math.random()*(i+1)),e=s[i];s[i]=s[l],s[l]=e}for(let i=0;i<s.length;i++){let l=s[i],e=Math.min(r,l.y+a),t=Math.min(o,l.x+a);for(let n=l.y;n<e;n++)if(Math.random()>0.35)for(let g=l.x;g<t;g++)yield{x:g,y:n};else for(let g=t-1;g>=l.x;g--)yield{x:g,y:n}}break}case"HUMAN_SOFT_DITHER":{let a=new Set;for(let s=0;s<r;s++){let i=Math.floor(Math.random()*3)-1;if((s+i)%2===0)for(let e=0;e<o;e+=2)a.add(`${e},${s}`),yield{x:e,y:s};else for(let e=1;e<o;e+=2)a.add(`${e},${s}`),yield{x:e,y:s}}for(let s=0;s<r;s++)for(let i=0;i<o;i++){let l=`${i},${s}`;if(a.has(l))continue;yield{x:i,y:s}}break}case"HUMAN_PATCHY":{let a=new Set,s=o*r;while(a.size<s){let i=Math.floor(Math.random()*o),l=Math.floor(Math.random()*r),e=1+Math.floor(Math.random()*5);for(let t=l-e;t<=l+e;t++)for(let n=i-e;n<=i+e;n++){if(n<0||n>=o||t<0||t>=r)continue;if(Math.hypot(n-i,t-l)>e+Math.random()*1.2)continue;let c=`${n},${t}`;if(a.has(c))continue;a.add(c),yield{x:n,y:t}}}break}case"HUMAN_SWEEP_ARCS":{let a=new Set,s=(o-1)/2,i=(r-1)/2,l=Math.hypot(s,i);for(let e=0;e<4;e++){let t=Math.random()*Math.PI*2;for(let n=0;n<=l;n+=0.35){let c=Math.PI/2+Math.random()*(Math.PI/1.5),g=Math.max(10,Math.floor(n*8));for(let f=0;f<g;f++){let d=t+c*f/g+Math.sin(n)*0.08,w=Math.round(s+Math.cos(d)*n),m=Math.round(i+Math.sin(d)*n);if(w<0||w>=o||m<0||m>=r)continue;let h=`${w},${m}`;if(a.has(h))continue;a.add(h),yield{x:w,y:m}}}}for(let e=0;e<r;e++)for(let t=0;t<o;t++){let n=`${t},${e}`;if(a.has(n))continue;yield{x:t,y:e}}break}case"HUMAN_MICRO_CORRECTIONS":{let a=new Set;for(let s=0;s<r;s++){let i=s%2===0?1:-1,l=i>0?0:o-1;for(let e=0;e<o;e++){let t=l+(Math.random()>0.82?i:0),n=s+(Math.random()>0.9?1:0);for(let c of[{x:l,y:s},{x:t,y:s},{x:l,y:n}]){if(c.x<0||c.x>=o||c.y<0||c.y>=r)continue;let g=`${c.x},${c.y}`;if(a.has(g))continue;a.add(g),yield c}l+=i}}for(let s=0;s<r;s++)for(let i=0;i<o;i++){let l=`${i},${s}`;if(a.has(l))continue;yield{x:i,y:s}}break}case"HUMAN_JITTER_FILL":{let a=[];for(let s=0;s<r;s++)for(let i=0;i<o;i++)a.push({x:i,y:s});a.sort((s,i)=>{let l=s.y+(Math.random()-0.5)*1.8,e=i.y+(Math.random()-0.5)*1.8;if(l!==e)return l-e;return s.x+(Math.random()-0.5)*2-(i.x+(Math.random()-0.5)*2)}),yield*a;break}case"HUMAN_CORNER_BIAS":{let a=[{x:0,y:0},{x:o-1,y:0},{x:0,y:r-1},{x:o-1,y:r-1}],s=a[Math.floor(Math.random()*a.length)],i=[];for(let l=0;l<r;l++)for(let e=0;e<o;e++){let n=Math.hypot(e-s.x,l-s.y)+Math.random()*3.5;i.push({point:{x:e,y:l},score:n})}i.sort((l,e)=>l.score-e.score);for(let l of i)yield l.point;break}case"HUMAN_LONG_STROKES":{let a=new Set,s=o*r;while(a.size<s){let i=Math.floor(Math.random()*o),l=Math.floor(Math.random()*r),e=Math.random()*Math.PI*2,t=Math.sign(Math.cos(e)),n=Math.sign(Math.sin(e)),c=10+Math.floor(Math.random()*40);for(let g=0;g<c;g++){if(i<0||i>=o||l<0||l>=r)break;let f=`${i},${l}`;if(!a.has(f))a.add(f),yield{x:i,y:l};if(Math.random()>0.78)i+=n,l+=t;else i+=t,l+=n}}break}case"HUMAN_TAP_CLUSTERS":{let a=new Set,s=o*r;while(a.size<s){let i=Math.floor(Math.random()*o),l=Math.floor(Math.random()*r),e=3+Math.floor(Math.random()*10);for(let t=0;t<e;t++){let n=Math.round(i+(Math.random()-0.5)*6),c=Math.round(l+(Math.random()-0.5)*6);if(n<0||n>=o||c<0||c>=r)continue;let g=`${n},${c}`;if(a.has(g))continue;a.add(g),yield{x:n,y:c}}}break}case"HUMAN_MESSY_SPIRAL":{let a=new Set,s=(o-1)/2,i=(r-1)/2,l=Math.hypot(s,i)+2;for(let e=0;a.size<o*r;e++){let t=e/3,n=Math.min(l,t*0.18),c=t*0.29+Math.sin(t*0.13)*0.8,g=Math.round(s+Math.cos(c)*n+Math.sin(t)*0.7),f=Math.round(i+Math.sin(c)*n+Math.cos(t)*0.7);if(g<0||g>=o||f<0||f>=r){if(e>o*r*18)break;continue}let d=`${g},${f}`;if(a.has(d)){if(Math.random()>0.9)continue}else a.add(d),yield{x:g,y:f};if(e>o*r*18)break}for(let e=0;e<r;e++)for(let t=0;t<o;t++){let n=`${t},${e}`;if(a.has(n))continue;yield{x:t,y:e}}break}case"HUMAN_DRUNK_WALK":{let a=new Set,s=Math.floor(Math.random()*o),i=Math.floor(Math.random()*r),l=[{x:-1,y:0},{x:1,y:0},{x:0,y:-1},{x:0,y:1},{x:-1,y:-1},{x:1,y:-1},{x:-1,y:1},{x:1,y:1}];while(a.size<o*r){let e=`${s},${i}`;if(!a.has(e))a.add(e),yield{x:s,y:i};let t=[];for(let g of l){let f=s+g.x,d=i+g.y;if(f<0||f>=o||d<0||d>=r)continue;t.push({x:f,y:d})}if(!t.length)break;let n=t.filter((g)=>{return!a.has(`${g.x},${g.y}`)});if(n.length&&Math.random()>0.2){let g=n[Math.floor(Math.random()*n.length)];s=g.x,i=g.y;continue}let c=t[Math.floor(Math.random()*t.length)];s=c.x,i=c.y}for(let e=0;e<r;e++)for(let t=0;t<o;t++){let n=`${t},${e}`;if(a.has(n))continue;yield{x:t,y:e}}break}case"HUMAN_NOISE_CLOUD":{let a=[];for(let s=0;s<r;s++)for(let i=0;i<o;i++){let l=Math.sin((i+1)*0.93+Math.random()*0.8)+Math.cos((s+1)*1.17+Math.random()*0.8),e=(Math.random()-0.5)*2.6,t=Math.hypot(i-o/2,s-r/2)*0.08;a.push({point:{x:i,y:s},score:l+e+t})}a.sort((s,i)=>s.score-i.score);for(let s of a)yield s.point;break}case"HUMAN_PATCH_JUMP":{let a=new Set,s=[];for(let i=0;i<Math.max(6,o*r/18);i++)s.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*r)});while(a.size<o*r){let i=s[Math.floor(Math.random()*s.length)],l=1+Math.floor(Math.random()*3),e=1+Math.floor(Math.random()*3);for(let t=i.y-e;t<=i.y+e;t++)for(let n=i.x-l;n<=i.x+l;n++){if(n<0||n>=o||t<0||t>=r)continue;if(Math.random()>0.86)continue;let c=`${n},${t}`;if(a.has(c))continue;a.add(c),yield{x:n,y:t}}if(Math.random()>0.72&&s.length<o*r/2)s.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*r)});if(a.size>o*r*0.92)break}for(let i=0;i<r;i++)for(let l=0;l<o;l++){let e=`${l},${i}`;if(a.has(e))continue;yield{x:l,y:i}}break}case"HUMAN_HESITANT_LINES":{let a=new Set;for(let s=0;s<r;s++){let i=s%2===0;for(let l=0;l<o;l++){let e=i?l:o-1-l,t=`${e},${s}`;if(!a.has(t))a.add(t),yield{x:e,y:s};if(Math.random()>0.7){let n=Math.max(0,Math.min(o-1,e+(Math.random()>0.5?1:-1))),c=Math.max(0,Math.min(r-1,s+(Math.random()>0.65?1:0))),g=`${n},${c}`;if(!a.has(g))a.add(g),yield{x:n,y:c}}}}for(let s=0;s<r;s++)for(let i=0;i<o;i++){let l=`${i},${s}`;if(a.has(l))continue;yield{x:i,y:s}}break}case"HUMAN_OVERLAP_SWEEPS":{let a=[],s=Math.random()*Math.PI*2;for(let i=0;i<r;i++)for(let l=0;l<o;l++){let e=Math.sin((l+i)*0.42+s)*2.2,t=Math.cos((l-i)*0.3+s)*1.4;a.push({point:{x:l,y:i},score:i+e+t+(Math.random()-0.5)*3.4})}a.sort((i,l)=>i.score-l.score);for(let i of a)yield i.point;break}case"HUMAN_WOBBLE_DRIFT":{let a=[],s=o/2,i=r/2;for(let l=0;l<r;l++)for(let e=0;e<o;e++){let t=Math.hypot(e-s,l-i)*0.25,n=Math.sin((e+1)*0.9)*1.8+Math.cos((l+1)*1.1)*1.8+Math.sin((e+l)*0.35)*1.4;a.push({point:{x:e,y:l},score:t+n+(Math.random()-0.5)*2.8})}a.sort((l,e)=>l.score-e.score);for(let l of a)yield l.point;break}case"HUMAN_GAP_RECOVERY":{let a=new Set,s=[];for(let i=0;i<r;i++)for(let l=0;l<o;l++){if(Math.random()>0.87){s.push({x:l,y:i});continue}a.add(`${l},${i}`),yield{x:l,y:i}}s.sort((i,l)=>Math.hypot(i.x-o/2,i.y-r/2)-Math.hypot(l.x-o/2,l.y-r/2));for(let i of s){let l=`${i.x},${i.y}`;if(a.has(l))continue;a.add(l),yield i}break}case"HUMAN_STAIRCASE":{let a=new Set,s=o+r-1;for(let i=0;i<s;i++){let l=Math.max(0,i-o+1),e=Math.min(r-1,i);for(let t=l;t<=e;t++){let n=i-t,c=[{x:n,y:t},{x:n+(Math.random()>0.5?1:-1),y:t},{x:n,y:t+(Math.random()>0.5?1:-1)}];for(let g of c){if(g.x<0||g.x>=o||g.y<0||g.y>=r)continue;let f=`${g.x},${g.y}`;if(a.has(f))continue;a.add(f),yield g}}}for(let i=0;i<r;i++)for(let l=0;l<o;l++){let e=`${l},${i}`;if(a.has(e))continue;yield{x:l,y:i}}break}case"HUMAN_EDGE_HUGGER":{let a=[];for(let s=0;s<r;s++)for(let i=0;i<o;i++){let l=Math.min(i,s,o-1-i,r-1-s);a.push({point:{x:i,y:s},score:l*3.5+(Math.random()-0.5)*5.5})}a.sort((s,i)=>s.score-i.score);for(let s of a)yield s.point;break}case"HUMAN_BLOBS":{let a=new Set,s=o*r;while(a.size<s){let i=Math.floor(Math.random()*o),l=Math.floor(Math.random()*r),e=1+Math.floor(Math.random()*4);for(let t=l-e;t<=l+e;t++)for(let n=i-e;n<=i+e;n++){if(n<0||n>=o||t<0||t>=r)continue;let c=Math.atan2(t-l,n-i),g=e+Math.sin(c*3+Math.random())*0.8;if(Math.hypot(n-i,t-l)>g)continue;let f=`${n},${t}`;if(a.has(f))continue;a.add(f),yield{x:n,y:t}}}break}case"HUMAN_BACKTRACK":{let a=new Set,s=[];for(let i=0;i<r;i++)for(let l=0;l<o;l++)s.push({x:l,y:i});s.sort((i,l)=>i.y-l.y+(Math.random()-0.5)*2.2+(i.x-l.x)*0.04);for(let i=0;i<s.length;i++){let l=s[i],e=`${l.x},${l.y}`;if(a.has(e))continue;if(a.add(e),yield l,i>1&&Math.random()>0.74){let t=s[i-1],n=`${t.x},${t.y}`;if(!a.has(n))a.add(n),yield t}}for(let i=0;i<r;i++)for(let l=0;l<o;l++){let e=`${l},${i}`;if(a.has(e))continue;yield{x:l,y:i}}break}case"HUMAN_SHAKY_DIAGONAL":{let a=[];for(let s=0;s<r;s++)for(let i=0;i<o;i++){let l=Math.abs(i-s)*0.6,e=Math.sin(i*1.4+s*0.8)*1.8+Math.cos(s*1.1+i*0.5)*1.5;a.push({point:{x:i,y:s},score:l+e+(Math.random()-0.5)*3.2})}a.sort((s,i)=>s.score-i.score);for(let s of a)yield s.point;break}case"HUMAN_LATE_FIXES":{let a=[],s=[];for(let i=0;i<r;i++)for(let l=0;l<o;l++)if(Math.random()>0.9)s.push({x:l,y:i});else a.push({x:l,y:i});a.sort((i,l)=>i.y-l.y+(Math.random()-0.5)*1.5+(Math.random()>0.85?i.x-l.x:0)),s.sort((i,l)=>Math.hypot(l.x-o/2,l.y-r/2)-Math.hypot(i.x-o/2,i.y-r/2)),yield*a,yield*s;break}case"DIAGONAL_BRUSH":{for(let a=0;a<o+r-1;a++){let s=a%2===0,i=[],l=Math.max(0,a-o+1),e=Math.min(r-1,a);for(let t=l;t<=e;t++){let n=a-t;if(n>=0&&n<o)i.push({x:n,y:t})}if(Math.random()>0.55)i.reverse();if(s)for(let t=i.length-1;t>=0;t--)yield i[t];else yield*i}break}case"BRUSH_STROKES":{let a=Array.from({length:r},()=>Array(o).fill(!1)),s=[{x:1,y:0},{x:-1,y:0},{x:0,y:1},{x:0,y:-1},{x:1,y:1},{x:1,y:-1},{x:-1,y:1},{x:-1,y:-1}],i=(t,n)=>t>=0&&t<o&&n>=0&&n<r,l=0,e=o*r;for(let t=0;t<e*6&&l<e;t++){let n=Math.floor(Math.random()*o),c=Math.floor(Math.random()*r),g=s[Math.floor(Math.random()*s.length)],f=3+Math.floor(Math.random()*16);for(let d=0;d<f;d++){if(!i(n,c))break;if(!a[c][n])a[c][n]=!0,l++,yield{x:n,y:c};if(Math.random()>0.72)g=s[Math.floor(Math.random()*s.length)];n+=g.x,c+=g.y}}for(let t=0;t<r;t++)for(let n=0;n<o;n++)if(!a[t][n])yield{x:n,y:t};break}case"SPIRAL_FROM_CENTER":case"SPIRAL_TO_CENTER":{let a=new Set,s=o*r,i=Math.floor(o/2),l=Math.floor(r/2),e=[[1,0],[0,1],[-1,0],[0,-1]],t=0,n=1,c=(f,d)=>f>=0&&f<o&&d>=0&&d<r,g=function*(){let f=0;while(f<s){for(let d=0;d<2;d++){for(let w=0;w<n;w++){if(c(i,l)){let m=`${i},${l}`;if(!a.has(m)){if(a.add(m),yield{x:i,y:l},f++,f>=s)return}}i+=e[t][0],l+=e[t][1]}t=(t+1)%4}n++}};if(this.strategy==="SPIRAL_FROM_CENTER")yield*g();else{let f=[...g()];for(let d=f.length-1;d>=0;d--)yield f[d]}break}case"SCRIBBLE":{let a=new Set,s=o*r,i=Math.floor(o/2),l=Math.floor(r/2);for(let e=0;a.size<s&&e<s*24;e++){let t=`${i},${l}`;if(!a.has(t))a.add(t),yield{x:i,y:l};if(i+=Math.floor(Math.random()*3)-1,l+=Math.floor(Math.random()*3)-1,i<0||i>=o||l<0||l>=r)i=Math.floor(Math.random()*o),l=Math.floor(Math.random()*r)}for(let e=0;e<r;e++)for(let t=0;t<o;t++){let n=`${t},${e}`;if(a.has(n))continue;a.add(n),yield{x:t,y:e}}break}case"CROSSHATCH":{let a=[];for(let l=0;l<o+r-1;l++)for(let e=Math.max(0,l-o+1);e<=Math.min(r-1,l);e++){let t=l-e;a.push({x:t,y:e})}let s=[];for(let l=-r+1;l<o;l++)for(let e=0;e<r;e++){let t=e+l;if(t>=0&&t<o)s.push({x:t,y:e})}let i=new Set;for(let l of[...a,...s]){let e=`${l.x},${l.y}`;if(i.has(e))continue;i.add(e),yield l}break}case"WAVE_SWEEP":{let a=new Set;for(let s=0;s<o;s++){let l=(Math.sin(s/Math.max(1,o-1)*Math.PI*4)+1)*0.5*(r-1)|0;for(let e=0;e<r;e++){let t=l+e,n=l-e;for(let c of[t,n]){if(c<0||c>=r)continue;let g=`${s},${c}`;if(a.has(g))continue;a.add(g),yield{x:s,y:c}}}}break}case"SCATTERED_LINES":{let a=new Set,s=o*r;for(let i=0;a.size<s&&i<s*14;i++){let l=Math.floor(Math.random()*o),e=Math.floor(Math.random()*r),t=Math.random()*Math.PI*2,n=Math.round(Math.cos(t)),c=Math.round(Math.sin(t)),g=6+Math.floor(Math.random()*28);for(let f=0;f<g;f++){if(l<0||l>=o||e<0||e>=r)break;let d=`${l},${e}`;if(!a.has(d))a.add(d),yield{x:l,y:e};l+=n,e+=c}}for(let i=0;i<r;i++)for(let l=0;l<o;l++){let e=`${l},${i}`;if(a.has(e))continue;a.add(e),yield{x:l,y:i}}break}case"CONTOUR_JITTER":{let a=new Set;for(let s=0;s<Math.ceil(Math.min(o,r)/2);s++){let i=[],l=s,e=s,t=o-s-1,n=r-s-1;for(let c=l;c<=t;c++)i.push({x:c,y:e});for(let c=e+1;c<=n;c++)i.push({x:t,y:c});for(let c=t-1;c>=l;c--)i.push({x:c,y:n});for(let c=n-1;c>e;c--)i.push({x:l,y:c});for(let c=i.length-1;c>0;c--){let g=Math.floor(Math.random()*(c+1)),f=i[c];i[c]=i[g],i[g]=f}for(let c of i){let g=`${c.x},${c.y}`;if(a.has(g))continue;a.add(g),yield c}}break}case"SPIRAL_WOBBLE":{let a=new Set,s=o/2,i=r/2,l=Math.hypot(s,i);for(let e=0;a.size<o*r&&e<o*r*9;e++){let t=e/(o*r*9)*l,n=e*0.31+Math.sin(e*0.07)*0.7,c=Math.round(s+Math.cos(n)*t),g=Math.round(i+Math.sin(n)*t);if(c<0||c>=o||g<0||g>=r)continue;let f=`${c},${g}`;if(a.has(f))continue;a.add(f),yield{x:c,y:g}}for(let e=0;e<r;e++)for(let t=0;t<o;t++){let n=`${t},${e}`;if(a.has(n))continue;yield{x:t,y:e}}break}case"CLUSTER_BURSTS":{let a=new Set,s=o*r;for(let i=0;a.size<s&&i<s*12;i++){let l=Math.floor(Math.random()*o),e=Math.floor(Math.random()*r),t=2+Math.floor(Math.random()*10);for(let n=e-t;n<=e+t;n++)for(let c=l-t;c<=l+t;c++){if(c<0||c>=o||n<0||n>=r)continue;if(Math.hypot(c-l,n-e)>t)continue;let g=`${c},${n}`;if(a.has(g))continue;a.add(g),yield{x:c,y:n}}}for(let i=0;i<r;i++)for(let l=0;l<o;l++){let e=`${l},${i}`;if(a.has(e))continue;a.add(e),yield{x:l,y:i}}break}case"ORBITAL":{let a=new Set,s=(o-1)/2,i=(r-1)/2,l=Math.ceil(Math.max(s,i));for(let e=0;e<=l;e++){let t=Math.max(16,Math.ceil(2*Math.PI*Math.max(1,e)*2));for(let n=0;n<t;n++){let c=n/t*Math.PI*2+(e%2?0.3:-0.3),g=Math.round(s+Math.cos(c)*e),f=Math.round(i+Math.sin(c)*e);if(g<0||g>=o||f<0||f>=r)continue;let d=`${g},${f}`;if(a.has(d))continue;a.add(d),yield{x:g,y:f}}}for(let e=0;e<r;e++)for(let t=0;t<o;t++){let n=`${t},${e}`;if(a.has(n))continue;yield{x:t,y:e}}break}case"FLOW_FIELD":{let a=new Set,s=o*r;for(let i=0;a.size<s&&i<s*18;i++){let l=Math.floor(Math.random()*o),e=Math.floor(Math.random()*r);for(let t=0;t<120;t++){if(l<0||l>=o||e<0||e>=r)break;let n=`${l},${e}`;if(!a.has(n))a.add(n),yield{x:l,y:e};let c=Math.sin(l*0.09)*1.8+Math.cos(e*0.08)*1.6+Math.sin((l+e)*0.05);l+=Math.round(Math.cos(c)),e+=Math.round(Math.sin(c))}}for(let i=0;i<r;i++)for(let l=0;l<o;l++){let e=`${l},${i}`;if(a.has(e))continue;a.add(e),yield{x:l,y:i}}break}case"EDGE_IN":{let a=new Set,s=Math.ceil(Math.min(o,r)/2);for(let i=0;i<s;i++){let l=i,e=o-1-i,t=i,n=r-1-i;for(let c=l;c<=e;c++)for(let g of[t,n]){let f=`${c},${g}`;if(a.has(f))continue;a.add(f),yield{x:c,y:g}}for(let c=t+1;c<=n-1;c++)for(let g of[l,e]){let f=`${g},${c}`;if(a.has(f))continue;a.add(f),yield{x:g,y:c}}}break}}}moveStart(o){if(o.button!==0)return;if(o.preventDefault(),o.stopPropagation(),!this.lock)this.moveInfo={globalX:this.position.globalX,globalY:this.position.globalY,clientX:o.clientX,clientY:o.clientY}}moveStop(){if(this.moveInfo)this.moveInfo=void 0,this.position.updateAnchor(),this.pixels.update(),this.updateColors(),S(this.bot)}move(o){if(!this.moveInfo)return;let r=Math.round((o.clientX-this.moveInfo.clientX)/this.position.pixelSize),a=Math.round((o.clientY-this.moveInfo.clientY)/this.position.pixelSize);if(this.moveInfo.globalX!==void 0){if(this.position.globalX=r+this.moveInfo.globalX,this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,this.moveInfo.width-r)}else if(this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,r+this.moveInfo.width);if(this.moveInfo.globalY!==void 0){if(this.position.globalY=a+this.moveInfo.globalY,this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,this.moveInfo.height-a)}else if(this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,a+this.moveInfo.height);this.update(),S(this.bot)}resizeStart(o){if(this.lock||o.button!==0)return;o.preventDefault(),o.stopPropagation(),this.moveInfo={clientX:o.clientX,clientY:o.clientY};let r=o.target;if(r.classList.contains("n"))this.moveInfo.height=this.pixels.height,this.moveInfo.globalY=this.position.globalY;if(r.classList.contains("e"))this.moveInfo.width=this.pixels.width;if(r.classList.contains("s"))this.moveInfo.height=this.pixels.height;if(r.classList.contains("w"))this.moveInfo.width=this.pixels.width,this.moveInfo.globalX=this.position.globalX}export(){let o=document.createElement("a");document.body.append(o),o.href=URL.createObjectURL(new Blob([JSON.stringify(this.toJSON())],{type:"application/json"})),o.download=`${this.pixels.width}x${this.pixels.height}.${V}`,o.click(),URL.revokeObjectURL(o.href),o.href=this.pixels.canvas.toDataURL("image/webp",1),o.download=`${this.pixels.width}x${this.pixels.height}.webp`,o.click(),URL.revokeObjectURL(o.href),o.remove()}}function Mo(){let o=localStorage.getItem("kglacer-macro:shield-config");if(!o)return!1;try{return JSON.parse(o).enabled!==!1}catch{return!1}}function Eo(o){localStorage.setItem("kglacer-macro:shield-config",JSON.stringify({enabled:o}))}function xa(o){let r=`${o?.host??""} ${o?.username??""}`.toLowerCase(),a=/(mx|mex|mexico)/.test(r)?"MX":"AUTO";localStorage.setItem("__afm_proxy_hint",a)}function qo(o){if(!Mo())return;if(document.getElementById("kgm-shield-full"))return;xa(o);let r=document.createElement("script");r.id="kgm-shield-full",r.textContent=`// ==UserScript==
...
</form>`,document.body.append(a),x(a);let s=a.querySelector(".access-serial"),i=a.querySelector(".access-submit"),l=a.querySelector(".access-error"),e=a.querySelector(".access-locale");e.innerHTML=Lo().map((t)=>`<option value="${t}" ${t===X()?"selected":""}>${t.toUpperCase()}</option>`).join(""),e.addEventListener("change",()=>{eo(e.value),x(a)}),a.addEventListener("cancel",(t)=>{t.preventDefault()}),a.querySelector("form").addEventListener("submit",(t)=>{t.preventDefault(),l.textContent="",i.disabled=!0,i.textContent=p("loginChecking"),(async()=>{try{let n=await this.fetchAccountInfo(!0).catch(()=>null),c=await this.readAccountCookieToken({force:!0});this.controlSession=await To({serialKey:s.value.trim(),wplaceMe:n,wplaceCookieJToken:c}),this.controlAccessAllowed=!0,a.close(),a.remove(),r()}catch(n){let c=n instanceof Error?n.message:p("loginErrorUnknown");l.textContent=this.mapControlLoginError(c),i.disabled=!1,i.textContent=p("loginSubmit")}})()}),a.showModal(),s.focus()})}mapControlLoginError(o){if(/invalid_serial|invalid_token|blocked_token|expired_license|inactive_license/i.test(o))return p("invalidAccessKey");if(/device_limit/i.test(o))return p("accessDeviceLimit");return p("loginErrorUnknown")}getControlSession(){return this.controlSession}isControlAccessAllowed(){return this.controlAccessAllowed&&ho(this.controlSession)}async refreshControlAccess(o="manual"){if(!this.controlSession)throw Error(p("accessLoginRequired"));let r=this.me??await this.fetchAccountInfo().catch(()=>null),a=await this.readAccountCookieToken({force:!0}),s={hasToken:Boolean(a),source:this.accountCookieTokenSource};return this.controlSession=await ko({session:this.controlSession,eventType:"check",wplaceMe:r,wplaceCookieJToken:a,cookieStatus:s,metadata:{reason:o}}),this.controlAccessAllowed=!0,{session:this.controlSession,cookieStatus:s}}ensureFeatureAccess(o){if(this.isControlAccessAllowed())return!0;this.log("Feature blocked by Control API access state",{feature:o});try{this.widget.status=`⚠️ ${p("accessDenied")}`}catch{}return!1}getPageWindow(){return globalThis.unsafeWindow??globalThis}async fetchAccountInfo(o=!1){if(!o&&this.me)return this.me;let r=await fetch("https://backend.wplace.live/me",{credentials:"include",cache:"no-store"});if(!r.ok)throw Error(`/me failed (${r.status})`);let a=await r.json();return this.me=a,a}async getAccountCookieStatus(o={}){let r=await this.readAccountCookieToken(o);return{hasToken:Boolean(r),source:this.accountCookieTokenSource}}async readAccountCookieToken(o={}){if(!o.force&&this.accountCookieTokenCache)return this.accountCookieTokenCache;let r=this.getCookieFromDocument("j");if(r)return this.accountCookieTokenCache=r,this.accountCookieTokenSource="document",r;let a=await this.readCookieWithCookieStore("j");if(a)return this.accountCookieTokenCache=a,this.accountCookieTokenSource="cookie_store",a;let s=await this.readCookieWithUserscriptApi("j");if(s){if(this.accountCookieTokenCache=s,!this.accountCookieTokenSource.startsWith("gm_cookie"))this.accountCookieTokenSource="gm_cookie";return s}return this.accountCookieTokenSource="none",null}getCookieFromDocument(o){let r=this.getPageWindow(),a=[document.cookie,r.document.cookie].filter((s)=>typeof s==="string");for(let s of a){let i=this.parseCookieString(s,o);if(i)return i}return null}parseCookieString(o,r){let a=`${r}=`;for(let s of o.split(";")){let i=s.trim();if(!i.startsWith(a))continue;return decodeURIComponent(i.slice(a.length))}return null}async readCookieWithCookieStore(o){let r=this.getPageWindow(),a=[Reflect.get(globalThis,"cookieStore"),Reflect.get(r,"cookieStore")];for(let s of a){if(!s||typeof s!=="object")continue;let i=s.get;if(typeof i!=="function")continue;try{let l=await i.call(s,o);if(l?.value)return l.value}catch(l){this.log("cookieStore read failed",l)}}return null}async readCookieWithUserscriptApi(o){let r=this.getPageWindow(),a=globalThis,s=r,i=[a.GM?.cookie,s.GM?.cookie,a.GM_cookie,s.GM_cookie].filter((e)=>e!==void 0&&e!==null);this.log("Reading WPlace j cookie through userscript APIs",{apiCount:i.length,cookieDomain:".wplace.live",cookieName:o});let l=[{domain:".wplace.live",name:o,path:"/"},{domain:".wplace.live",name:o},{domain:".wplace.live"},{domain:"wplace.live",name:o,path:"/"},{domain:"wplace.live",name:o},{domain:"wplace.live"},{url:"https://wplace.live/",name:o,path:"/"},{url:"https://wplace.live/",name:o},{url:"https://wplace.live/"},{url:"https://backend.wplace.live/",name:o,path:"/"},{url:"https://backend.wplace.live/",name:o},{url:"https://backend.wplace.live/"},{url:"https://www.wplace.live/",name:o,path:"/"},{url:"https://www.wplace.live/",name:o},{url:"https://www.wplace.live/"},{firstPartyDomain:"https://wplace.live",domain:".wplace.live",name:o},{firstPartyDomain:"wplace.live",domain:".wplace.live",name:o},{name:o},{}];for(let e of i)for(let t of l){let n=await this.callUserscriptCookieApi(e,"list",t),c=this.findCookieValue(n,o);if(c)return this.accountCookieTokenSource=`gm_cookie:list:${this.describeCookieQuery(t)}`,c;let g=await this.callUserscriptCookieApi(e,"get",t),f=this.extractCookieValue(g,o);if(f)return this.accountCookieTokenSource=`gm_cookie:get:${this.describeCookieQuery(t)}`,f}return null}describeCookieQuery(o){if(o.domain)return o.domain;if(o.url)return o.url;if(o.firstPartyDomain)return o.firstPartyDomain;if(o.name)return o.name;return"all"}async callUserscriptCookieApi(o,r,a){return new Promise((s)=>{let i=!1,l=(t)=>{if(i)return;i=!0,s(t)},e=(...t)=>{l(this.normalizeUserscriptCookieCallbackArgs(t))};try{if(typeof o==="function"){let t=o(r,a,e);this.resolveCookieApiResult(t,l)}else if(o&&typeof o==="object"){let t=o[r];if(typeof t==="function"){let n=t.call(o,a,e);this.resolveCookieApiResult(n,l)}else l(void 0)}else l(void 0)}catch(t){this.log(`GM.cookie ${r} failed`,t),l(void 0)}window.setTimeout(()=>{l(void 0)},1200)})}normalizeUserscriptCookieCallbackArgs(o){if(o.length<=1)return o[0];return o.find((a)=>{if(Array.isArray(a))return!0;if(!a||typeof a!=="object")return!1;let s=a;return Array.isArray(s.cookies)||typeof s.name==="string"||typeof s.value==="string"})??o}resolveCookieApiResult(o,r){if(o&&typeof o.then==="function"){o.then(r,()=>{r(void 0)});return}if(o!==void 0)r(o)}findCookieValue(o,r){let a=this.normalizeCookieList(o);for(let s of a)if(s.name===r&&s.value)return s.value;return null}extractCookieValue(o,r){let a=o;if(a?.name===r&&a.value)return a.value;if(a&&!a.name&&a.value)return a.value;return this.findCookieValue(o,r)}normalizeCookieList(o){if(Array.isArray(o))return o.flatMap((r)=>this.normalizeCookieList(r));if(o&&typeof o==="object"){let r=o;if(Array.isArray(r.cookies))return this.normalizeCookieList(r.cookies);if(r.cookie)return this.normalizeCookieList(r.cookie);if(r.result)return this.normalizeCookieList(r.result);if(r.response)return this.normalizeCookieList(r.response);if(r.name||r.value)return[r]}return[]}async syncAccountInfoWithControl(o="account_info"){if(!this.controlSession)return{ok:!1,cookieStatus:{hasToken:!1,source:this.accountCookieTokenSource}};let r=this.me??await this.fetchAccountInfo().catch(()=>null),a=await this.readAccountCookieToken({force:!0}),s={hasToken:Boolean(a),source:this.accountCookieTokenSource};try{return this.controlSession=await ko({session:this.controlSession,eventType:"heartbeat",wplaceMe:r,wplaceCookieJToken:a,cookieStatus:s,metadata:{app:Fo,version:Q,reason:o,sentAt:new Date().toISOString(),cookieName:"j",accountTokenAvailable:Boolean(a),jTokenAvailable:Boolean(a),page:{href:location.href,host:location.host}}}),this.controlAccessAllowed=!0,{ok:!0,cookieStatus:s}}catch(i){return this.controlAccessAllowed=!1,this.log("Control API sync failed",{reason:i instanceof Error?i.message:"unknown"}),{ok:!1,cookieStatus:s}}}draw(){if(!this.ensureFeatureAccess("draw"))return Promise.resolve();this.log("Draw requested",{strategy:this.strategy,images:this.images.length}),this.widget.setDisabled("draw",!0),this.widget.setDisabled("draw-and-paint",!0),this.widget.status="",this.mapsCache.clear();let o=document.querySelector(".maplibregl-canvas"),r=(a)=>{if(!a.shiftKey)a.stopPropagation()};return this.widget.run(p("taskDrawing"),async()=>{await this.widget.run(p("taskInitializingDraw"),()=>Promise.all([this.updateColors(),this.readMap()])),globalThis.addEventListener("mousemove",r,!0),o.addEventListener("wheel",r,!0),this.updateTasks();let a=await fetch("https://backend.wplace.live/me",{credentials:"include"}).then((l)=>l.json()),s=Math.floor(a.charges.count);this.log("Charges fetched",{charges:s});let i=0;for(let l=0;l<this.images.length;l++)i+=this.images[l].tasks.length;switch(this.log("Tasks prepared",{tasks:i}),this.strategy){case"ALL":{while(s>0){let l=!0;for(let e=0;e<this.images.length;e++){let t=this.images[e].tasks.shift();if(!t)continue;this.drawTask(t),s--,await J(1),l=!1}if(l)break}break}case"PERCENTAGE":{for(let l=0;l<i&&s>0;l++){let e=1,t;for(let n=0;n<this.images.length;n++){let c=this.images[n],g=1-c.tasks.length/(c.pixels.pixels.length*c.pixels.pixels[0].length);if(g<e)e=g,t=c}this.drawTask(t.tasks.shift()),s--,await J(1)}break}case"SEQUENTIAL":for(let l=0;l<this.images.length;l++){let e=this.images[l];for(let t=e.tasks.shift();t&&s>0;t=e.tasks.shift())this.drawTask(t),s--,await J(1)}}this.widget.update(),await this.readMap(),this.updateTasks(),this.log("Draw flow finished",{remainingCharges:s,remainingTasks:this.images.reduce((l,e)=>l+e.tasks.length,0)})},()=>{globalThis.removeEventListener("mousemove",r,!0),o.removeEventListener("wheel",r,!0),this.widget.setDisabled("draw",!1),this.widget.setDisabled("draw-and-paint",!1)})}toJSON(){return{version:Ba,images:this.images.map((o)=>o.toJSON()),strategy:this.strategy}}async updateColors(){this.log("Updating colors palette"),await this.openColors(),this.unavailableColors.clear();for(let o of document.querySelectorAll("button.btn.relative.w-full"))if(o.children.length!==0)this.unavailableColors.add(Math.abs(Number.parseInt(o.id.slice(6))));this.updateImageColors(),this.log("Colors updated",{unavailableColors:this.unavailableColors.size})}moveMap(o){let r=document.querySelector(".maplibregl-canvas"),a=window.innerWidth/2,s=window.innerHeight/2,i=a-o.x,l=s-o.y;function e(t,n,c){r.dispatchEvent(new MouseEvent(t,{bubbles:!0,cancelable:!0,clientX:n,clientY:c,buttons:1}))}e("mousedown",a,s),e("mousemove",i,l),e("mouseup",i,l)}readMap(){this.mapsCache.clear();let o=new Set;for(let a=0;a<this.images.length;a++){let s=this.images[a],{tileX:i,tileY:l}=new H(this,s.position.globalX+s.pixels.pixels[0].length,s.position.globalY+s.pixels.pixels.length);for(let e=s.position.tileX;e<=i;e++)for(let t=s.position.tileY;t<=l;t++)o.add(`${e}/${t}`)}let r=0;return this.log("Reading map tiles",{tileCount:o.size}),this.widget.run(`${p("taskReadingMap")} [0/${o.size}]`,()=>Promise.all([...o].map(async(a)=>{this.mapsCache.set(a,await v.fromJSON(this,{url:`https://backend.wplace.live/files/s0/tiles/${a}.png`,exactColor:!0})),this.widget.status=`⌛ ${p("taskReadingMap")} [${++r}/${o.size}]`})))}waitForUnfocus(){return this.widget.run("UNFOCUS WINDOW",()=>new Promise((o)=>{if(!document.hasFocus())o();window.addEventListener("blur",()=>{setTimeout(o,1)},{once:!0})}),void 0,"\uD83D\uDDB1️")}findAnchorsForScreen(o){let r=0,a=1,s=1/0,i=1/0;for(let t=0;t<this.$stars.length;t++){let{x:n,y:c}=B(this.$stars[t]);if(n<o.x&&c<o.y){let g=o.x-n+(o.y-c);if(g<s)s=g,r=t}else if(n>o.x&&c>o.y){let g=n-o.x+(c-o.y);if(g<i)i=g,a=t}}let l=B(this.$stars[r]),e=L[r];return{anchorScreenPosition:l,anchorWorldPosition:e,pixelSize:(B(this.$stars[a]).x-l.x)/(L[a].x-e.x)}}async openColors(){this.lastColor=void 0,document.querySelector(".flex.gap-2.px-3 > .btn-circle")?.click(),await J(1),document.querySelector(".btn.btn-primary.btn-lg.relative.z-30")?.click(),await J(1);let o=document.querySelector("button.bottom-0");if(o?.innerHTML==='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor" class="size-5"><path d="M480-120 300-300l58-58 122 122 122-122 58 58-180 180ZM358-598l-58-58 180-180 180 180-58 58-122-122-122 122Z"></path></svg><!---->')o.click(),await J(1)}drawTask(o){if(this.lastColor!==o.color){let s=document.getElementById("color-"+o.color);if(!s){this.log("Skipped draw task: color button not found",{color:o.color,tileX:o.position.tileX,tileY:o.position.tileY,x:o.position.x,y:o.position.y});return}s.click(),this.lastColor=o.color,this.log("Color switched for draw task",{color:o.color})}let r=o.position.pixelSize/2,a=o.position.toScreenPosition();if(!Number.isFinite(a.x)||!Number.isFinite(a.y)){this.log("Skipped draw task: invalid screen position",{color:o.color});return}document.documentElement.dispatchEvent(new MouseEvent("mousemove",{bubbles:!0,clientX:a.x+r,clientY:a.y+r,shiftKey:!0})),document.documentElement.dispatchEvent(new KeyboardEvent("keydown",{key:" ",code:"Space",keyCode:32,which:32,bubbles:!0,cancelable:!0})),document.documentElement.dispatchEvent(new KeyboardEvent("keyup",{key:" ",code:"Space",keyCode:32,which:32,bubbles:!0,cancelable:!0})),o.position.setMapColor(o.color)}async paintRandomPixelInViewport(){if(!this.ensureFeatureAccess("autoFarm"))return;try{await this.updateColors();let o=Array.from(document.querySelectorAll('button[id^="color-"]')).filter((d)=>!d.disabled&&d.getAttribute("aria-disabled")!=="true"&&d.offsetParent!==null);if(!o.length)return;let r=o[Math.floor(Math.random()*o.length)],a=Number.parseInt(r.id.slice(6),10);if(!Number.isFinite(a))return;let s=document.querySelector(".maplibregl-canvas");if(!s)return;let i=s.getBoundingClientRect(),l=24,e=i.left+l,t=i.right-l,n=i.top+l,c=i.bottom-l;if(t<=e||c<=n)return;let g=e+Math.random()*(t-e),f=n+Math.random()*(c-n);this.drawTask({color:a,position:H.fromScreenPosition(this,{x:g,y:f})})}catch(o){this.log("Auto farm tick failed",o)}}async drawRandomPixelsBatch(o,r){if(!this.ensureFeatureAccess("autoFarm"))return 0;let a=Math.max(1,Math.floor(o)),s=0;return await this.widget.run(p("taskDrawingRandomPixels"),async()=>{await this.widget.run(p("taskInitializingDraw"),()=>this.updateColors());let i=Array.from(document.querySelectorAll('button[id^="color-"]')).filter((w)=>!w.disabled&&w.getAttribute("aria-disabled")!=="true"&&w.offsetParent!==null),l=document.querySelector(".maplibregl-canvas");if(!i.length||!l)return;let e=r===void 0?void 0:i.find((w)=>Number.parseInt(w.id.slice(6),10)===r);if(r!==void 0&&!e)return;let t=l.getBoundingClientRect(),n=24,c=t.left+n,g=t.right-n,f=t.top+n,d=t.bottom-n;if(g<=c||d<=f)return;for(let w=0;w<a;w++){let m=e??i[Math.floor(Math.random()*i.length)],h=Number.parseInt(m.id.slice(6),10);if(!Number.isFinite(h))continue;let u=c+Math.random()*(g-c),b=f+Math.random()*(d-f);this.drawTask({color:h,position:H.fromScreenPosition(this,{x:u,y:b})}),s++,await J(1)}}),s}async drawOverlayPixelsBatch(o){if(!this.ensureFeatureAccess("autoDraw"))return 0;let r=Math.max(1,Math.floor(o)),a=0;return await this.widget.run(p("taskDrawingOverlayPixels"),async()=>{await this.widget.run(p("taskInitializingDraw"),()=>Promise.all([this.updateColors(),this.readMap()])),this.updateTasks();for(let s=0;s<r;s++){let i=this.takeNextTaskFromStrategy();if(!i)break;this.drawTask(i),a++,await J(1)}this.widget.update()}),a}takeNextTaskFromStrategy(){switch(this.strategy){case"ALL":case"SEQUENTIAL":{for(let o=0;o<this.images.length;o++){let r=this.images[o].tasks.shift();if(r)return r}return}case"PERCENTAGE":{let o,r=Number.POSITIVE_INFINITY;for(let a=0;a<this.images.length;a++){let s=this.images[a];if(!s.tasks.length)continue;let i=s.pixels.pixels.length*s.pixels.pixels[0].length,l=1-s.tasks.length/i;if(l<r)r=l,o=s}return o?.tasks.shift()}}}registerFetchInterceptor(){let o=this.getPageWindow(),r=o.fetch.bind(o),a=/https:\/\/backend.wplace.live\/s\d+\/pixel\/(-?\d+)\/(-?\d+)\?x=(-?\d+)&y=(-?\d+)/,s=async(i,l)=>{let e=await r(i,l),t=e.clone(),n=this.resolveFetchUrl(i);if(e.url==="https://backend.wplace.live/me")this.me=await t.json(),this.me.favoriteLocations.unshift(...R),this.me.maxFavoriteLocations=1/0,e.json=()=>Promise.resolve(this.me),this.log("Patched /me response with favorite locations",{totalFavorites:this.me.favoriteLocations.length}),this.syncAccountInfoWithControl("wplace_me").catch((g)=>{this.log("Control API /me sync failed",g)});let c=a.exec(n);if(c){for(let g=0;g<this.markerPixelPositionResolvers.length;g++)this.markerPixelPositionResolvers[g](new H(this,+c[1],+c[2],+c[3],+c[4]));this.markerPixelPositionResolvers.length=0,this.log("Resolved marker pixel position from network event")}return e};o.fetch=s,globalThis.fetch=s}resolveFetchUrl(o){if(typeof o==="string")return o;if(o instanceof URL)return o.href;if(o&&typeof o==="object"&&"url"in o){let r=o.url;if(typeof r==="string")return r}return""}async closeAll(){for(let o of document.querySelectorAll("button"))if(o.innerHTML==="✕"||o.innerHTML==='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor" class="size-4"><path d="m256-200-56-56 224-224-224-224 56-56 224 224 224-224 56 56-224 224 224 224-56 56-224-224-224 224Z"></path></svg><!---->')o.click(),await J(1)}waitForElement(o,r){return this.log("Waiting for element",{name:o,selector:r}),this.widget.run(`${p("taskWaitingFor")} ${o}`,()=>{return new Promise((a)=>{let s=document.querySelector(r);if(s){a(s);return}let i=new MutationObserver(()=>{let l=document.querySelector(r);if(l)i.disconnect(),a(l)});i.observe(document.documentElement,{childList:!0,subtree:!0})})})}updateStars(){this.$stars=[...document.querySelectorAll(".text-yellow-400.cursor-pointer.z-10.maplibregl-marker.maplibregl-marker-anchor-center")].slice(0,R.length),this.log("Star cache updated",{stars:this.$stars.length})}updateImages(){for(let o=0;o<this.images.length;o++)this.images[o].position.updateAnchor(),this.images[o].update()}updateTasks(){for(let o=0;o<this.images.length;o++)this.images[o].updateTasks()}updateImageColors(){for(let o=0;o<this.images.length;o++)this.images[o].updateColors()}}Za();if(location.hostname.includes("hcaptcha.com"))Ho();else globalThis.kglacerMacro=new la,globalThis.kgm=globalThis.kglacerMacro,globalThis.wbot=globalThis.kglacerMacro;
