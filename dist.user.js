...
<span class="premium"></span>`,g.querySelector(".swatch").style.setProperty("--swatch-color",go(l.realColor)),g.addEventListener("click",()=>{if(i){i=!1;return}t(),this.updateTasks()}),g.addEventListener("dragstart",(u)=>{i=!0,g.classList.add("dragging"),u.dataTransfer?.setData("text/plain",String(r)),u.dataTransfer.effectAllowed="move"}),g.addEventListener("dragend",()=>{g.classList.remove("dragging")}),g.addEventListener("dragover",(u)=>{u.preventDefault(),g.classList.add("drag-target")}),g.addEventListener("dragleave",()=>{g.classList.remove("drag-target")}),g.addEventListener("drop",(u)=>{u.preventDefault(),g.classList.remove("drag-target");let w=Number.parseInt(u.dataTransfer?.getData("text/plain")??"-1",10);if(w<0||w===r||w>=this.colors.length)return;this.colors.splice(r,0,...this.colors.splice(w,1)),k(this.bot),this.updateColors()});let d=document.createElement("button");d.textContent=f("buy"),d.className="buy-chip",d.addEventListener("click",(u)=>{u.stopPropagation(),document.getElementById("color-"+l.realColor)?.click()}),g.append(d);let e=`${p} ${n.join(" ")} ${l.realColor} ${q[l.realColor]}`;if(!a||e.toLowerCase().includes(a))this.$colorsDialogList.append(g)}}*strategyPositionIterator(){let o=this.pixels.pixels[0].length,a=this.pixels.pixels.length;switch(this.strategy){case"DOWN":{for(let r=0;r<a;r++)for(let s=0;s<o;s++)yield{x:s,y:r};break}case"UP":{for(let r=a-1;r>=0;r--)for(let s=0;s<o;s++)yield{x:s,y:r};break}case"LEFT":{for(let r=0;r<o;r++)for(let s=0;s<a;s++)yield{x:r,y:s};break}case"RIGHT":{for(let r=o-1;r>=0;r--)for(let s=0;s<a;s++)yield{x:r,y:s};break}case"RANDOM":{let r=[];for(let s=0;s<a;s++)for(let l=0;l<o;l++)r.push({x:l,y:s});for(let s=r.length-1;s>=0;s--){let l=Math.floor(Math.random()*(s+1)),i=r[s];r[s]=r[l],r[l]=i}yield*r;break}case"ZIGZAG":{for(let r=0;r<a;r++)if(r%2===0)for(let s=0;s<o;s++)yield{x:s,y:r};else for(let s=o-1;s>=0;s--)yield{x:s,y:r};break}case"HUMANIZED":{let r=Math.max(4,Math.floor(Math.min(o,a)/10)),s=[];for(let l=0;l<a;l+=r)for(let i=0;i<o;i+=r)s.push({x:i,y:l});for(let l=s.length-1;l>=0;l--){let i=Math.floor(Math.random()*(l+1)),c=s[l];s[l]=s[i],s[i]=c}for(let l=0;l<s.length;l++){let i=s[l],c=Math.min(a,i.y+r),p=Math.min(o,i.x+r);for(let n=i.y;n<c;n++)if(Math.random()>0.35)for(let g=i.x;g<p;g++)yield{x:g,y:n};else for(let g=p-1;g>=i.x;g--)yield{x:g,y:n}}break}case"HUMAN_SOFT_DITHER":{let r=new Set;for(let s=0;s<a;s++){let l=Math.floor(Math.random()*3)-1;if((s+l)%2===0)for(let c=0;c<o;c+=2)r.add(`${c},${s}`),yield{x:c,y:s};else for(let c=1;c<o;c+=2)r.add(`${c},${s}`),yield{x:c,y:s}}for(let s=0;s<a;s++)for(let l=0;l<o;l++){let i=`${l},${s}`;if(r.has(i))continue;yield{x:l,y:s}}break}case"HUMAN_PATCHY":{let r=new Set,s=o*a;while(r.size<s){let l=Math.floor(Math.random()*o),i=Math.floor(Math.random()*a),c=1+Math.floor(Math.random()*5);for(let p=i-c;p<=i+c;p++)for(let n=l-c;n<=l+c;n++){if(n<0||n>=o||p<0||p>=a)continue;if(Math.hypot(n-l,p-i)>c+Math.random()*1.2)continue;let t=`${n},${p}`;if(r.has(t))continue;r.add(t),yield{x:n,y:p}}}break}case"HUMAN_SWEEP_ARCS":{let r=new Set,s=(o-1)/2,l=(a-1)/2,i=Math.hypot(s,l);for(let c=0;c<4;c++){let p=Math.random()*Math.PI*2;for(let n=0;n<=i;n+=0.35){let t=Math.PI/2+Math.random()*(Math.PI/1.5),g=Math.max(10,Math.floor(n*8));for(let d=0;d<g;d++){let e=p+t*d/g+Math.sin(n)*0.08,u=Math.round(s+Math.cos(e)*n),w=Math.round(l+Math.sin(e)*n);if(u<0||u>=o||w<0||w>=a)continue;let z=`${u},${w}`;if(r.has(z))continue;r.add(z),yield{x:u,y:w}}}}for(let c=0;c<a;c++)for(let p=0;p<o;p++){let n=`${p},${c}`;if(r.has(n))continue;yield{x:p,y:c}}break}case"HUMAN_MICRO_CORRECTIONS":{let r=new Set;for(let s=0;s<a;s++){let l=s%2===0?1:-1,i=l>0?0:o-1;for(let c=0;c<o;c++){let p=i+(Math.random()>0.82?l:0),n=s+(Math.random()>0.9?1:0);for(let t of[{x:i,y:s},{x:p,y:s},{x:i,y:n}]){if(t.x<0||t.x>=o||t.y<0||t.y>=a)continue;let g=`${t.x},${t.y}`;if(r.has(g))continue;r.add(g),yield t}i+=l}}for(let s=0;s<a;s++)for(let l=0;l<o;l++){let i=`${l},${s}`;if(r.has(i))continue;yield{x:l,y:s}}break}case"HUMAN_JITTER_FILL":{let r=[];for(let s=0;s<a;s++)for(let l=0;l<o;l++)r.push({x:l,y:s});r.sort((s,l)=>{let i=s.y+(Math.random()-0.5)*1.8,c=l.y+(Math.random()-0.5)*1.8;if(i!==c)return i-c;return s.x+(Math.random()-0.5)*2-(l.x+(Math.random()-0.5)*2)}),yield*r;break}case"HUMAN_CORNER_BIAS":{let r=[{x:0,y:0},{x:o-1,y:0},{x:0,y:a-1},{x:o-1,y:a-1}],s=r[Math.floor(Math.random()*r.length)],l=[];for(let i=0;i<a;i++)for(let c=0;c<o;c++){let n=Math.hypot(c-s.x,i-s.y)+Math.random()*3.5;l.push({point:{x:c,y:i},score:n})}l.sort((i,c)=>i.score-c.score);for(let i of l)yield i.point;break}case"HUMAN_LONG_STROKES":{let r=new Set,s=o*a;while(r.size<s){let l=Math.floor(Math.random()*o),i=Math.floor(Math.random()*a),c=Math.random()*Math.PI*2,p=Math.sign(Math.cos(c)),n=Math.sign(Math.sin(c)),t=10+Math.floor(Math.random()*40);for(let g=0;g<t;g++){if(l<0||l>=o||i<0||i>=a)break;let d=`${l},${i}`;if(!r.has(d))r.add(d),yield{x:l,y:i};if(Math.random()>0.78)l+=n,i+=p;else l+=p,i+=n}}break}case"HUMAN_TAP_CLUSTERS":{let r=new Set,s=o*a;while(r.size<s){let l=Math.floor(Math.random()*o),i=Math.floor(Math.random()*a),c=3+Math.floor(Math.random()*10);for(let p=0;p<c;p++){let n=Math.round(l+(Math.random()-0.5)*6),t=Math.round(i+(Math.random()-0.5)*6);if(n<0||n>=o||t<0||t>=a)continue;let g=`${n},${t}`;if(r.has(g))continue;r.add(g),yield{x:n,y:t}}}break}case"HUMAN_MESSY_SPIRAL":{let r=new Set,s=(o-1)/2,l=(a-1)/2,i=Math.hypot(s,l)+2;for(let c=0;r.size<o*a;c++){let p=c/3,n=Math.min(i,p*0.18),t=p*0.29+Math.sin(p*0.13)*0.8,g=Math.round(s+Math.cos(t)*n+Math.sin(p)*0.7),d=Math.round(l+Math.sin(t)*n+Math.cos(p)*0.7);if(g<0||g>=o||d<0||d>=a){if(c>o*a*18)break;continue}let e=`${g},${d}`;if(r.has(e)){if(Math.random()>0.9)continue}else r.add(e),yield{x:g,y:d};if(c>o*a*18)break}for(let c=0;c<a;c++)for(let p=0;p<o;p++){let n=`${p},${c}`;if(r.has(n))continue;yield{x:p,y:c}}break}case"HUMAN_DRUNK_WALK":{let r=new Set,s=Math.floor(Math.random()*o),l=Math.floor(Math.random()*a),i=[{x:-1,y:0},{x:1,y:0},{x:0,y:-1},{x:0,y:1},{x:-1,y:-1},{x:1,y:-1},{x:-1,y:1},{x:1,y:1}];while(r.size<o*a){let c=`${s},${l}`;if(!r.has(c))r.add(c),yield{x:s,y:l};let p=[];for(let g of i){let d=s+g.x,e=l+g.y;if(d<0||d>=o||e<0||e>=a)continue;p.push({x:d,y:e})}if(!p.length)break;let n=p.filter((g)=>{return!r.has(`${g.x},${g.y}`)});if(n.length&&Math.random()>0.2){let g=n[Math.floor(Math.random()*n.length)];s=g.x,l=g.y;continue}let t=p[Math.floor(Math.random()*p.length)];s=t.x,l=t.y}for(let c=0;c<a;c++)for(let p=0;p<o;p++){let n=`${p},${c}`;if(r.has(n))continue;yield{x:p,y:c}}break}case"HUMAN_NOISE_CLOUD":{let r=[];for(let s=0;s<a;s++)for(let l=0;l<o;l++){let i=Math.sin((l+1)*0.93+Math.random()*0.8)+Math.cos((s+1)*1.17+Math.random()*0.8),c=(Math.random()-0.5)*2.6,p=Math.hypot(l-o/2,s-a/2)*0.08;r.push({point:{x:l,y:s},score:i+c+p})}r.sort((s,l)=>s.score-l.score);for(let s of r)yield s.point;break}case"HUMAN_PATCH_JUMP":{let r=new Set,s=[];for(let l=0;l<Math.max(6,o*a/18);l++)s.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*a)});while(r.size<o*a){let l=s[Math.floor(Math.random()*s.length)],i=1+Math.floor(Math.random()*3),c=1+Math.floor(Math.random()*3);for(let p=l.y-c;p<=l.y+c;p++)for(let n=l.x-i;n<=l.x+i;n++){if(n<0||n>=o||p<0||p>=a)continue;if(Math.random()>0.86)continue;let t=`${n},${p}`;if(r.has(t))continue;r.add(t),yield{x:n,y:p}}if(Math.random()>0.72&&s.length<o*a/2)s.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*a)});if(r.size>o*a*0.92)break}for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=`${i},${l}`;if(r.has(c))continue;yield{x:i,y:l}}break}case"HUMAN_HESITANT_LINES":{let r=new Set;for(let s=0;s<a;s++){let l=s%2===0;for(let i=0;i<o;i++){let c=l?i:o-1-i,p=`${c},${s}`;if(!r.has(p))r.add(p),yield{x:c,y:s};if(Math.random()>0.7){let n=Math.max(0,Math.min(o-1,c+(Math.random()>0.5?1:-1))),t=Math.max(0,Math.min(a-1,s+(Math.random()>0.65?1:0))),g=`${n},${t}`;if(!r.has(g))r.add(g),yield{x:n,y:t}}}}for(let s=0;s<a;s++)for(let l=0;l<o;l++){let i=`${l},${s}`;if(r.has(i))continue;yield{x:l,y:s}}break}case"HUMAN_OVERLAP_SWEEPS":{let r=[],s=Math.random()*Math.PI*2;for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=Math.sin((i+l)*0.42+s)*2.2,p=Math.cos((i-l)*0.3+s)*1.4;r.push({point:{x:i,y:l},score:l+c+p+(Math.random()-0.5)*3.4})}r.sort((l,i)=>l.score-i.score);for(let l of r)yield l.point;break}case"HUMAN_WOBBLE_DRIFT":{let r=[],s=o/2,l=a/2;for(let i=0;i<a;i++)for(let c=0;c<o;c++){let p=Math.hypot(c-s,i-l)*0.25,n=Math.sin((c+1)*0.9)*1.8+Math.cos((i+1)*1.1)*1.8+Math.sin((c+i)*0.35)*1.4;r.push({point:{x:c,y:i},score:p+n+(Math.random()-0.5)*2.8})}r.sort((i,c)=>i.score-c.score);for(let i of r)yield i.point;break}case"HUMAN_GAP_RECOVERY":{let r=new Set,s=[];for(let l=0;l<a;l++)for(let i=0;i<o;i++){if(Math.random()>0.87){s.push({x:i,y:l});continue}r.add(`${i},${l}`),yield{x:i,y:l}}s.sort((l,i)=>Math.hypot(l.x-o/2,l.y-a/2)-Math.hypot(i.x-o/2,i.y-a/2));for(let l of s){let i=`${l.x},${l.y}`;if(r.has(i))continue;r.add(i),yield l}break}case"HUMAN_STAIRCASE":{let r=new Set,s=o+a-1;for(let l=0;l<s;l++){let i=Math.max(0,l-o+1),c=Math.min(a-1,l);for(let p=i;p<=c;p++){let n=l-p,t=[{x:n,y:p},{x:n+(Math.random()>0.5?1:-1),y:p},{x:n,y:p+(Math.random()>0.5?1:-1)}];for(let g of t){if(g.x<0||g.x>=o||g.y<0||g.y>=a)continue;let d=`${g.x},${g.y}`;if(r.has(d))continue;r.add(d),yield g}}}for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=`${i},${l}`;if(r.has(c))continue;yield{x:i,y:l}}break}case"HUMAN_EDGE_HUGGER":{let r=[];for(let s=0;s<a;s++)for(let l=0;l<o;l++){let i=Math.min(l,s,o-1-l,a-1-s);r.push({point:{x:l,y:s},score:i*3.5+(Math.random()-0.5)*5.5})}r.sort((s,l)=>s.score-l.score);for(let s of r)yield s.point;break}case"HUMAN_BLOBS":{let r=new Set,s=o*a;while(r.size<s){let l=Math.floor(Math.random()*o),i=Math.floor(Math.random()*a),c=1+Math.floor(Math.random()*4);for(let p=i-c;p<=i+c;p++)for(let n=l-c;n<=l+c;n++){if(n<0||n>=o||p<0||p>=a)continue;let t=Math.atan2(p-i,n-l),g=c+Math.sin(t*3+Math.random())*0.8;if(Math.hypot(n-l,p-i)>g)continue;let d=`${n},${p}`;if(r.has(d))continue;r.add(d),yield{x:n,y:p}}}break}case"HUMAN_BACKTRACK":{let r=new Set,s=[];for(let l=0;l<a;l++)for(let i=0;i<o;i++)s.push({x:i,y:l});s.sort((l,i)=>l.y-i.y+(Math.random()-0.5)*2.2+(l.x-i.x)*0.04);for(let l=0;l<s.length;l++){let i=s[l],c=`${i.x},${i.y}`;if(r.has(c))continue;if(r.add(c),yield i,l>1&&Math.random()>0.74){let p=s[l-1],n=`${p.x},${p.y}`;if(!r.has(n))r.add(n),yield p}}for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=`${i},${l}`;if(r.has(c))continue;yield{x:i,y:l}}break}case"HUMAN_SHAKY_DIAGONAL":{let r=[];for(let s=0;s<a;s++)for(let l=0;l<o;l++){let i=Math.abs(l-s)*0.6,c=Math.sin(l*1.4+s*0.8)*1.8+Math.cos(s*1.1+l*0.5)*1.5;r.push({point:{x:l,y:s},score:i+c+(Math.random()-0.5)*3.2})}r.sort((s,l)=>s.score-l.score);for(let s of r)yield s.point;break}case"HUMAN_LATE_FIXES":{let r=[],s=[];for(let l=0;l<a;l++)for(let i=0;i<o;i++)if(Math.random()>0.9)s.push({x:i,y:l});else r.push({x:i,y:l});r.sort((l,i)=>l.y-i.y+(Math.random()-0.5)*1.5+(Math.random()>0.85?l.x-i.x:0)),s.sort((l,i)=>Math.hypot(i.x-o/2,i.y-a/2)-Math.hypot(l.x-o/2,l.y-a/2)),yield*r,yield*s;break}case"DIAGONAL_BRUSH":{for(let r=0;r<o+a-1;r++){let s=r%2===0,l=[],i=Math.max(0,r-o+1),c=Math.min(a-1,r);for(let p=i;p<=c;p++){let n=r-p;if(n>=0&&n<o)l.push({x:n,y:p})}if(Math.random()>0.55)l.reverse();if(s)for(let p=l.length-1;p>=0;p--)yield l[p];else yield*l}break}case"BRUSH_STROKES":{let r=Array.from({length:a},()=>Array(o).fill(!1)),s=[{x:1,y:0},{x:-1,y:0},{x:0,y:1},{x:0,y:-1},{x:1,y:1},{x:1,y:-1},{x:-1,y:1},{x:-1,y:-1}],l=(p,n)=>p>=0&&p<o&&n>=0&&n<a,i=0,c=o*a;for(let p=0;p<c*6&&i<c;p++){let n=Math.floor(Math.random()*o),t=Math.floor(Math.random()*a),g=s[Math.floor(Math.random()*s.length)],d=3+Math.floor(Math.random()*16);for(let e=0;e<d;e++){if(!l(n,t))break;if(!r[t][n])r[t][n]=!0,i++,yield{x:n,y:t};if(Math.random()>0.72)g=s[Math.floor(Math.random()*s.length)];n+=g.x,t+=g.y}}for(let p=0;p<a;p++)for(let n=0;n<o;n++)if(!r[p][n])yield{x:n,y:p};break}case"SPIRAL_FROM_CENTER":case"SPIRAL_TO_CENTER":{let r=new Set,s=o*a,l=Math.floor(o/2),i=Math.floor(a/2),c=[[1,0],[0,1],[-1,0],[0,-1]],p=0,n=1,t=(d,e)=>d>=0&&d<o&&e>=0&&e<a,g=function*(){let d=0;while(d<s){for(let e=0;e<2;e++){for(let u=0;u<n;u++){if(t(l,i)){let w=`${l},${i}`;if(!r.has(w)){if(r.add(w),yield{x:l,y:i},d++,d>=s)return}}l+=c[p][0],i+=c[p][1]}p=(p+1)%4}n++}};if(this.strategy==="SPIRAL_FROM_CENTER")yield*g();else{let d=[...g()];for(let e=d.length-1;e>=0;e--)yield d[e]}break}case"SCRIBBLE":{let r=new Set,s=o*a,l=Math.floor(o/2),i=Math.floor(a/2);for(let c=0;r.size<s&&c<s*24;c++){let p=`${l},${i}`;if(!r.has(p))r.add(p),yield{x:l,y:i};if(l+=Math.floor(Math.random()*3)-1,i+=Math.floor(Math.random()*3)-1,l<0||l>=o||i<0||i>=a)l=Math.floor(Math.random()*o),i=Math.floor(Math.random()*a)}for(let c=0;c<a;c++)for(let p=0;p<o;p++){let n=`${p},${c}`;if(r.has(n))continue;r.add(n),yield{x:p,y:c}}break}case"CROSSHATCH":{let r=[];for(let i=0;i<o+a-1;i++)for(let c=Math.max(0,i-o+1);c<=Math.min(a-1,i);c++){let p=i-c;r.push({x:p,y:c})}let s=[];for(let i=-a+1;i<o;i++)for(let c=0;c<a;c++){let p=c+i;if(p>=0&&p<o)s.push({x:p,y:c})}let l=new Set;for(let i of[...r,...s]){let c=`${i.x},${i.y}`;if(l.has(c))continue;l.add(c),yield i}break}case"WAVE_SWEEP":{let r=new Set;for(let s=0;s<o;s++){let i=(Math.sin(s/Math.max(1,o-1)*Math.PI*4)+1)*0.5*(a-1)|0;for(let c=0;c<a;c++){let p=i+c,n=i-c;for(let t of[p,n]){if(t<0||t>=a)continue;let g=`${s},${t}`;if(r.has(g))continue;r.add(g),yield{x:s,y:t}}}}break}case"SCATTERED_LINES":{let r=new Set,s=o*a;for(let l=0;r.size<s&&l<s*14;l++){let i=Math.floor(Math.random()*o),c=Math.floor(Math.random()*a),p=Math.random()*Math.PI*2,n=Math.round(Math.cos(p)),t=Math.round(Math.sin(p)),g=6+Math.floor(Math.random()*28);for(let d=0;d<g;d++){if(i<0||i>=o||c<0||c>=a)break;let e=`${i},${c}`;if(!r.has(e))r.add(e),yield{x:i,y:c};i+=n,c+=t}}for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=`${i},${l}`;if(r.has(c))continue;r.add(c),yield{x:i,y:l}}break}case"CONTOUR_JITTER":{let r=new Set;for(let s=0;s<Math.ceil(Math.min(o,a)/2);s++){let l=[],i=s,c=s,p=o-s-1,n=a-s-1;for(let t=i;t<=p;t++)l.push({x:t,y:c});for(let t=c+1;t<=n;t++)l.push({x:p,y:t});for(let t=p-1;t>=i;t--)l.push({x:t,y:n});for(let t=n-1;t>c;t--)l.push({x:i,y:t});for(let t=l.length-1;t>0;t--){let g=Math.floor(Math.random()*(t+1)),d=l[t];l[t]=l[g],l[g]=d}for(let t of l){let g=`${t.x},${t.y}`;if(r.has(g))continue;r.add(g),yield t}}break}case"SPIRAL_WOBBLE":{let r=new Set,s=o/2,l=a/2,i=Math.hypot(s,l);for(let c=0;r.size<o*a&&c<o*a*9;c++){let p=c/(o*a*9)*i,n=c*0.31+Math.sin(c*0.07)*0.7,t=Math.round(s+Math.cos(n)*p),g=Math.round(l+Math.sin(n)*p);if(t<0||t>=o||g<0||g>=a)continue;let d=`${t},${g}`;if(r.has(d))continue;r.add(d),yield{x:t,y:g}}for(let c=0;c<a;c++)for(let p=0;p<o;p++){let n=`${p},${c}`;if(r.has(n))continue;yield{x:p,y:c}}break}case"CLUSTER_BURSTS":{let r=new Set,s=o*a;for(let l=0;r.size<s&&l<s*12;l++){let i=Math.floor(Math.random()*o),c=Math.floor(Math.random()*a),p=2+Math.floor(Math.random()*10);for(let n=c-p;n<=c+p;n++)for(let t=i-p;t<=i+p;t++){if(t<0||t>=o||n<0||n>=a)continue;if(Math.hypot(t-i,n-c)>p)continue;let g=`${t},${n}`;if(r.has(g))continue;r.add(g),yield{x:t,y:n}}}for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=`${i},${l}`;if(r.has(c))continue;r.add(c),yield{x:i,y:l}}break}case"ORBITAL":{let r=new Set,s=(o-1)/2,l=(a-1)/2,i=Math.ceil(Math.max(s,l));for(let c=0;c<=i;c++){let p=Math.max(16,Math.ceil(2*Math.PI*Math.max(1,c)*2));for(let n=0;n<p;n++){let t=n/p*Math.PI*2+(c%2?0.3:-0.3),g=Math.round(s+Math.cos(t)*c),d=Math.round(l+Math.sin(t)*c);if(g<0||g>=o||d<0||d>=a)continue;let e=`${g},${d}`;if(r.has(e))continue;r.add(e),yield{x:g,y:d}}}for(let c=0;c<a;c++)for(let p=0;p<o;p++){let n=`${p},${c}`;if(r.has(n))continue;yield{x:p,y:c}}break}case"FLOW_FIELD":{let r=new Set,s=o*a;for(let l=0;r.size<s&&l<s*18;l++){let i=Math.floor(Math.random()*o),c=Math.floor(Math.random()*a);for(let p=0;p<120;p++){if(i<0||i>=o||c<0||c>=a)break;let n=`${i},${c}`;if(!r.has(n))r.add(n),yield{x:i,y:c};let t=Math.sin(i*0.09)*1.8+Math.cos(c*0.08)*1.6+Math.sin((i+c)*0.05);i+=Math.round(Math.cos(t)),c+=Math.round(Math.sin(t))}}for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=`${i},${l}`;if(r.has(c))continue;r.add(c),yield{x:i,y:l}}break}case"EDGE_IN":{let r=new Set,s=Math.ceil(Math.min(o,a)/2);for(let l=0;l<s;l++){let i=l,c=o-1-l,p=l,n=a-1-l;for(let t=i;t<=c;t++)for(let g of[p,n]){let d=`${t},${g}`;if(r.has(d))continue;r.add(d),yield{x:t,y:g}}for(let t=p+1;t<=n-1;t++)for(let g of[i,c]){let d=`${g},${t}`;if(r.has(d))continue;r.add(d),yield{x:g,y:t}}}break}}}moveStart(o){if(o.button!==0)return;if(o.preventDefault(),o.stopPropagation(),!this.lock)this.moveInfo={globalX:this.position.globalX,globalY:this.position.globalY,clientX:o.clientX,clientY:o.clientY}}moveStop(){if(this.moveInfo)this.moveInfo=void 0,this.position.updateAnchor(),this.pixels.update(),this.updateColors(),k(this.bot)}move(o){if(!this.moveInfo)return;let a=Math.round((o.clientX-this.moveInfo.clientX)/this.position.pixelSize),r=Math.round((o.clientY-this.moveInfo.clientY)/this.position.pixelSize);if(this.moveInfo.globalX!==void 0){if(this.position.globalX=a+this.moveInfo.globalX,this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,this.moveInfo.width-a)}else if(this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,a+this.moveInfo.width);if(this.moveInfo.globalY!==void 0){if(this.position.globalY=r+this.moveInfo.globalY,this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,this.moveInfo.height-r)}else if(this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,r+this.moveInfo.height);this.update(),k(this.bot)}resizeStart(o){if(this.lock||o.button!==0)return;o.preventDefault(),o.stopPropagation(),this.moveInfo={clientX:o.clientX,clientY:o.clientY};let a=o.target;if(a.classList.contains("n"))this.moveInfo.height=this.pixels.height,this.moveInfo.globalY=this.position.globalY;if(a.classList.contains("e"))this.moveInfo.width=this.pixels.width;if(a.classList.contains("s"))this.moveInfo.height=this.pixels.height;if(a.classList.contains("w"))this.moveInfo.width=this.pixels.width,this.moveInfo.globalX=this.position.globalX}export(){let o=document.createElement("a");document.body.append(o),o.href=URL.createObjectURL(new Blob([JSON.stringify(this.toJSON())],{type:"application/json"})),o.download=`${this.pixels.width}x${this.pixels.height}.${Y}`,o.click(),URL.revokeObjectURL(o.href),o.href=this.pixels.canvas.toDataURL("image/webp",1),o.download=`${this.pixels.width}x${this.pixels.height}.webp`,o.click(),URL.revokeObjectURL(o.href),o.remove()}}function fo(){let o=localStorage.getItem("kglacer-macro:shield-config");if(!o)return!1;try{return JSON.parse(o).enabled!==!1}catch{return!1}}function No(o){localStorage.setItem("kglacer-macro:shield-config",JSON.stringify({enabled:o}))}function ga(o){let a=`${o?.host??""} ${o?.username??""}`.toLowerCase(),r=/(mx|mex|mexico)/.test(a)?"MX":"AUTO";localStorage.setItem("__afm_proxy_hint",r)}function Do(o){if(!fo())return;if(document.getElementById("kgm-shield-full"))return;ga(o);let a=document.createElement("script");a.id="kgm-shield-full",a.textContent=`// ==UserScript==
