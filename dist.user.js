...
<span class="premium"></span>`,g.querySelector(".swatch").style.setProperty("--swatch-color",go(l.realColor)),g.addEventListener("click",()=>{if(s){s=!1;return}p(),this.updateTasks()}),g.addEventListener("dragstart",(u)=>{s=!0,g.classList.add("dragging"),u.dataTransfer?.setData("text/plain",String(r)),u.dataTransfer.effectAllowed="move"}),g.addEventListener("dragend",()=>{g.classList.remove("dragging")}),g.addEventListener("dragover",(u)=>{u.preventDefault(),g.classList.add("drag-target")}),g.addEventListener("dragleave",()=>{g.classList.remove("drag-target")}),g.addEventListener("drop",(u)=>{u.preventDefault(),g.classList.remove("drag-target");let w=Number.parseInt(u.dataTransfer?.getData("text/plain")??"-1",10);if(w<0||w===r||w>=this.colors.length)return;this.colors.splice(r,0,...this.colors.splice(w,1)),A(this.bot),this.updateColors()});let d=document.createElement("button");d.textContent=e("buy"),d.className="buy-chip",d.addEventListener("click",(u)=>{u.stopPropagation(),document.getElementById("color-"+l.realColor)?.click()}),g.append(d);let f=`${t} ${n.join(" ")} ${l.realColor} ${q[l.realColor]}`;if(!a||f.toLowerCase().includes(a))this.$colorsDialogList.append(g)}}*strategyPositionIterator(){let o=this.pixels.pixels[0].length,a=this.pixels.pixels.length;switch(this.strategy){case"DOWN":{for(let r=0;r<a;r++)for(let i=0;i<o;i++)yield{x:i,y:r};break}case"UP":{for(let r=a-1;r>=0;r--)for(let i=0;i<o;i++)yield{x:i,y:r};break}case"LEFT":{for(let r=0;r<o;r++)for(let i=0;i<a;i++)yield{x:r,y:i};break}case"RIGHT":{for(let r=o-1;r>=0;r--)for(let i=0;i<a;i++)yield{x:r,y:i};break}case"RANDOM":{let r=[];for(let i=0;i<a;i++)for(let l=0;l<o;l++)r.push({x:l,y:i});for(let i=r.length-1;i>=0;i--){let l=Math.floor(Math.random()*(i+1)),s=r[i];r[i]=r[l],r[l]=s}yield*r;break}case"ZIGZAG":{for(let r=0;r<a;r++)if(r%2===0)for(let i=0;i<o;i++)yield{x:i,y:r};else for(let i=o-1;i>=0;i--)yield{x:i,y:r};break}case"HUMANIZED":{let r=Math.max(4,Math.floor(Math.min(o,a)/10)),i=[];for(let l=0;l<a;l+=r)for(let s=0;s<o;s+=r)i.push({x:s,y:l});for(let l=i.length-1;l>=0;l--){let s=Math.floor(Math.random()*(l+1)),c=i[l];i[l]=i[s],i[s]=c}for(let l=0;l<i.length;l++){let s=i[l],c=Math.min(a,s.y+r),t=Math.min(o,s.x+r);for(let n=s.y;n<c;n++)if(Math.random()>0.35)for(let g=s.x;g<t;g++)yield{x:g,y:n};else for(let g=t-1;g>=s.x;g--)yield{x:g,y:n}}break}case"HUMAN_SOFT_DITHER":{let r=new Set;for(let i=0;i<a;i++){let l=Math.floor(Math.random()*3)-1;if((i+l)%2===0)for(let c=0;c<o;c+=2)r.add(`${c},${i}`),yield{x:c,y:i};else for(let c=1;c<o;c+=2)r.add(`${c},${i}`),yield{x:c,y:i}}for(let i=0;i<a;i++)for(let l=0;l<o;l++){let s=`${l},${i}`;if(r.has(s))continue;yield{x:l,y:i}}break}case"HUMAN_PATCHY":{let r=new Set,i=o*a;while(r.size<i){let l=Math.floor(Math.random()*o),s=Math.floor(Math.random()*a),c=1+Math.floor(Math.random()*5);for(let t=s-c;t<=s+c;t++)for(let n=l-c;n<=l+c;n++){if(n<0||n>=o||t<0||t>=a)continue;if(Math.hypot(n-l,t-s)>c+Math.random()*1.2)continue;let p=`${n},${t}`;if(r.has(p))continue;r.add(p),yield{x:n,y:t}}}break}case"HUMAN_SWEEP_ARCS":{let r=new Set,i=(o-1)/2,l=(a-1)/2,s=Math.hypot(i,l);for(let c=0;c<4;c++){let t=Math.random()*Math.PI*2;for(let n=0;n<=s;n+=0.35){let p=Math.PI/2+Math.random()*(Math.PI/1.5),g=Math.max(10,Math.floor(n*8));for(let d=0;d<g;d++){let f=t+p*d/g+Math.sin(n)*0.08,u=Math.round(i+Math.cos(f)*n),w=Math.round(l+Math.sin(f)*n);if(u<0||u>=o||w<0||w>=a)continue;let k=`${u},${w}`;if(r.has(k))continue;r.add(k),yield{x:u,y:w}}}}for(let c=0;c<a;c++)for(let t=0;t<o;t++){let n=`${t},${c}`;if(r.has(n))continue;yield{x:t,y:c}}break}case"HUMAN_MICRO_CORRECTIONS":{let r=new Set;for(let i=0;i<a;i++){let l=i%2===0?1:-1,s=l>0?0:o-1;for(let c=0;c<o;c++){let t=s+(Math.random()>0.82?l:0),n=i+(Math.random()>0.9?1:0);for(let p of[{x:s,y:i},{x:t,y:i},{x:s,y:n}]){if(p.x<0||p.x>=o||p.y<0||p.y>=a)continue;let g=`${p.x},${p.y}`;if(r.has(g))continue;r.add(g),yield p}s+=l}}for(let i=0;i<a;i++)for(let l=0;l<o;l++){let s=`${l},${i}`;if(r.has(s))continue;yield{x:l,y:i}}break}case"HUMAN_JITTER_FILL":{let r=[];for(let i=0;i<a;i++)for(let l=0;l<o;l++)r.push({x:l,y:i});r.sort((i,l)=>{let s=i.y+(Math.random()-0.5)*1.8,c=l.y+(Math.random()-0.5)*1.8;if(s!==c)return s-c;return i.x+(Math.random()-0.5)*2-(l.x+(Math.random()-0.5)*2)}),yield*r;break}case"HUMAN_CORNER_BIAS":{let r=[{x:0,y:0},{x:o-1,y:0},{x:0,y:a-1},{x:o-1,y:a-1}],i=r[Math.floor(Math.random()*r.length)],l=[];for(let s=0;s<a;s++)for(let c=0;c<o;c++){let n=Math.hypot(c-i.x,s-i.y)+Math.random()*3.5;l.push({point:{x:c,y:s},score:n})}l.sort((s,c)=>s.score-c.score);for(let s of l)yield s.point;break}case"HUMAN_LONG_STROKES":{let r=new Set,i=o*a;while(r.size<i){let l=Math.floor(Math.random()*o),s=Math.floor(Math.random()*a),c=Math.random()*Math.PI*2,t=Math.sign(Math.cos(c)),n=Math.sign(Math.sin(c)),p=10+Math.floor(Math.random()*40);for(let g=0;g<p;g++){if(l<0||l>=o||s<0||s>=a)break;let d=`${l},${s}`;if(!r.has(d))r.add(d),yield{x:l,y:s};if(Math.random()>0.78)l+=n,s+=t;else l+=t,s+=n}}break}case"HUMAN_TAP_CLUSTERS":{let r=new Set,i=o*a;while(r.size<i){let l=Math.floor(Math.random()*o),s=Math.floor(Math.random()*a),c=3+Math.floor(Math.random()*10);for(let t=0;t<c;t++){let n=Math.round(l+(Math.random()-0.5)*6),p=Math.round(s+(Math.random()-0.5)*6);if(n<0||n>=o||p<0||p>=a)continue;let g=`${n},${p}`;if(r.has(g))continue;r.add(g),yield{x:n,y:p}}}break}case"HUMAN_MESSY_SPIRAL":{let r=new Set,i=(o-1)/2,l=(a-1)/2,s=Math.hypot(i,l)+2;for(let c=0;r.size<o*a;c++){let t=c/3,n=Math.min(s,t*0.18),p=t*0.29+Math.sin(t*0.13)*0.8,g=Math.round(i+Math.cos(p)*n+Math.sin(t)*0.7),d=Math.round(l+Math.sin(p)*n+Math.cos(t)*0.7);if(g<0||g>=o||d<0||d>=a){if(c>o*a*18)break;continue}let f=`${g},${d}`;if(r.has(f)){if(Math.random()>0.9)continue}else r.add(f),yield{x:g,y:d};if(c>o*a*18)break}for(let c=0;c<a;c++)for(let t=0;t<o;t++){let n=`${t},${c}`;if(r.has(n))continue;yield{x:t,y:c}}break}case"HUMAN_DRUNK_WALK":{let r=new Set,i=Math.floor(Math.random()*o),l=Math.floor(Math.random()*a),s=[{x:-1,y:0},{x:1,y:0},{x:0,y:-1},{x:0,y:1},{x:-1,y:-1},{x:1,y:-1},{x:-1,y:1},{x:1,y:1}];while(r.size<o*a){let c=`${i},${l}`;if(!r.has(c))r.add(c),yield{x:i,y:l};let t=[];for(let g of s){let d=i+g.x,f=l+g.y;if(d<0||d>=o||f<0||f>=a)continue;t.push({x:d,y:f})}if(!t.length)break;let n=t.filter((g)=>{return!r.has(`${g.x},${g.y}`)});if(n.length&&Math.random()>0.2){let g=n[Math.floor(Math.random()*n.length)];i=g.x,l=g.y;continue}let p=t[Math.floor(Math.random()*t.length)];i=p.x,l=p.y}for(let c=0;c<a;c++)for(let t=0;t<o;t++){let n=`${t},${c}`;if(r.has(n))continue;yield{x:t,y:c}}break}case"HUMAN_NOISE_CLOUD":{let r=[];for(let i=0;i<a;i++)for(let l=0;l<o;l++){let s=Math.sin((l+1)*0.93+Math.random()*0.8)+Math.cos((i+1)*1.17+Math.random()*0.8),c=(Math.random()-0.5)*2.6,t=Math.hypot(l-o/2,i-a/2)*0.08;r.push({point:{x:l,y:i},score:s+c+t})}r.sort((i,l)=>i.score-l.score);for(let i of r)yield i.point;break}case"HUMAN_PATCH_JUMP":{let r=new Set,i=[];for(let l=0;l<Math.max(6,o*a/18);l++)i.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*a)});while(r.size<o*a){let l=i[Math.floor(Math.random()*i.length)],s=1+Math.floor(Math.random()*3),c=1+Math.floor(Math.random()*3);for(let t=l.y-c;t<=l.y+c;t++)for(let n=l.x-s;n<=l.x+s;n++){if(n<0||n>=o||t<0||t>=a)continue;if(Math.random()>0.86)continue;let p=`${n},${t}`;if(r.has(p))continue;r.add(p),yield{x:n,y:t}}if(Math.random()>0.72&&i.length<o*a/2)i.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*a)});if(r.size>o*a*0.92)break}for(let l=0;l<a;l++)for(let s=0;s<o;s++){let c=`${s},${l}`;if(r.has(c))continue;yield{x:s,y:l}}break}case"HUMAN_HESITANT_LINES":{let r=new Set;for(let i=0;i<a;i++){let l=i%2===0;for(let s=0;s<o;s++){let c=l?s:o-1-s,t=`${c},${i}`;if(!r.has(t))r.add(t),yield{x:c,y:i};if(Math.random()>0.7){let n=Math.max(0,Math.min(o-1,c+(Math.random()>0.5?1:-1))),p=Math.max(0,Math.min(a-1,i+(Math.random()>0.65?1:0))),g=`${n},${p}`;if(!r.has(g))r.add(g),yield{x:n,y:p}}}}for(let i=0;i<a;i++)for(let l=0;l<o;l++){let s=`${l},${i}`;if(r.has(s))continue;yield{x:l,y:i}}break}case"HUMAN_OVERLAP_SWEEPS":{let r=[],i=Math.random()*Math.PI*2;for(let l=0;l<a;l++)for(let s=0;s<o;s++){let c=Math.sin((s+l)*0.42+i)*2.2,t=Math.cos((s-l)*0.3+i)*1.4;r.push({point:{x:s,y:l},score:l+c+t+(Math.random()-0.5)*3.4})}r.sort((l,s)=>l.score-s.score);for(let l of r)yield l.point;break}case"HUMAN_WOBBLE_DRIFT":{let r=[],i=o/2,l=a/2;for(let s=0;s<a;s++)for(let c=0;c<o;c++){let t=Math.hypot(c-i,s-l)*0.25,n=Math.sin((c+1)*0.9)*1.8+Math.cos((s+1)*1.1)*1.8+Math.sin((c+s)*0.35)*1.4;r.push({point:{x:c,y:s},score:t+n+(Math.random()-0.5)*2.8})}r.sort((s,c)=>s.score-c.score);for(let s of r)yield s.point;break}case"HUMAN_GAP_RECOVERY":{let r=new Set,i=[];for(let l=0;l<a;l++)for(let s=0;s<o;s++){if(Math.random()>0.87){i.push({x:s,y:l});continue}r.add(`${s},${l}`),yield{x:s,y:l}}i.sort((l,s)=>Math.hypot(l.x-o/2,l.y-a/2)-Math.hypot(s.x-o/2,s.y-a/2));for(let l of i){let s=`${l.x},${l.y}`;if(r.has(s))continue;r.add(s),yield l}break}case"HUMAN_STAIRCASE":{let r=new Set,i=o+a-1;for(let l=0;l<i;l++){let s=Math.max(0,l-o+1),c=Math.min(a-1,l);for(let t=s;t<=c;t++){let n=l-t,p=[{x:n,y:t},{x:n+(Math.random()>0.5?1:-1),y:t},{x:n,y:t+(Math.random()>0.5?1:-1)}];for(let g of p){if(g.x<0||g.x>=o||g.y<0||g.y>=a)continue;let d=`${g.x},${g.y}`;if(r.has(d))continue;r.add(d),yield g}}}for(let l=0;l<a;l++)for(let s=0;s<o;s++){let c=`${s},${l}`;if(r.has(c))continue;yield{x:s,y:l}}break}case"HUMAN_EDGE_HUGGER":{let r=[];for(let i=0;i<a;i++)for(let l=0;l<o;l++){let s=Math.min(l,i,o-1-l,a-1-i);r.push({point:{x:l,y:i},score:s*3.5+(Math.random()-0.5)*5.5})}r.sort((i,l)=>i.score-l.score);for(let i of r)yield i.point;break}case"HUMAN_BLOBS":{let r=new Set,i=o*a;while(r.size<i){let l=Math.floor(Math.random()*o),s=Math.floor(Math.random()*a),c=1+Math.floor(Math.random()*4);for(let t=s-c;t<=s+c;t++)for(let n=l-c;n<=l+c;n++){if(n<0||n>=o||t<0||t>=a)continue;let p=Math.atan2(t-s,n-l),g=c+Math.sin(p*3+Math.random())*0.8;if(Math.hypot(n-l,t-s)>g)continue;let d=`${n},${t}`;if(r.has(d))continue;r.add(d),yield{x:n,y:t}}}break}case"HUMAN_BACKTRACK":{let r=new Set,i=[];for(let l=0;l<a;l++)for(let s=0;s<o;s++)i.push({x:s,y:l});i.sort((l,s)=>l.y-s.y+(Math.random()-0.5)*2.2+(l.x-s.x)*0.04);for(let l=0;l<i.length;l++){let s=i[l],c=`${s.x},${s.y}`;if(r.has(c))continue;if(r.add(c),yield s,l>1&&Math.random()>0.74){let t=i[l-1],n=`${t.x},${t.y}`;if(!r.has(n))r.add(n),yield t}}for(let l=0;l<a;l++)for(let s=0;s<o;s++){let c=`${s},${l}`;if(r.has(c))continue;yield{x:s,y:l}}break}case"HUMAN_SHAKY_DIAGONAL":{let r=[];for(let i=0;i<a;i++)for(let l=0;l<o;l++){let s=Math.abs(l-i)*0.6,c=Math.sin(l*1.4+i*0.8)*1.8+Math.cos(i*1.1+l*0.5)*1.5;r.push({point:{x:l,y:i},score:s+c+(Math.random()-0.5)*3.2})}r.sort((i,l)=>i.score-l.score);for(let i of r)yield i.point;break}case"HUMAN_LATE_FIXES":{let r=[],i=[];for(let l=0;l<a;l++)for(let s=0;s<o;s++)if(Math.random()>0.9)i.push({x:s,y:l});else r.push({x:s,y:l});r.sort((l,s)=>l.y-s.y+(Math.random()-0.5)*1.5+(Math.random()>0.85?l.x-s.x:0)),i.sort((l,s)=>Math.hypot(s.x-o/2,s.y-a/2)-Math.hypot(l.x-o/2,l.y-a/2)),yield*r,yield*i;break}case"DIAGONAL_BRUSH":{for(let r=0;r<o+a-1;r++){let i=r%2===0,l=[],s=Math.max(0,r-o+1),c=Math.min(a-1,r);for(let t=s;t<=c;t++){let n=r-t;if(n>=0&&n<o)l.push({x:n,y:t})}if(Math.random()>0.55)l.reverse();if(i)for(let t=l.length-1;t>=0;t--)yield l[t];else yield*l}break}case"BRUSH_STROKES":{let r=Array.from({length:a},()=>Array(o).fill(!1)),i=[{x:1,y:0},{x:-1,y:0},{x:0,y:1},{x:0,y:-1},{x:1,y:1},{x:1,y:-1},{x:-1,y:1},{x:-1,y:-1}],l=(t,n)=>t>=0&&t<o&&n>=0&&n<a,s=0,c=o*a;for(let t=0;t<c*6&&s<c;t++){let n=Math.floor(Math.random()*o),p=Math.floor(Math.random()*a),g=i[Math.floor(Math.random()*i.length)],d=3+Math.floor(Math.random()*16);for(let f=0;f<d;f++){if(!l(n,p))break;if(!r[p][n])r[p][n]=!0,s++,yield{x:n,y:p};if(Math.random()>0.72)g=i[Math.floor(Math.random()*i.length)];n+=g.x,p+=g.y}}for(let t=0;t<a;t++)for(let n=0;n<o;n++)if(!r[t][n])yield{x:n,y:t};break}case"SPIRAL_FROM_CENTER":case"SPIRAL_TO_CENTER":{let r=new Set,i=o*a,l=Math.floor(o/2),s=Math.floor(a/2),c=[[1,0],[0,1],[-1,0],[0,-1]],t=0,n=1,p=(d,f)=>d>=0&&d<o&&f>=0&&f<a,g=function*(){let d=0;while(d<i){for(let f=0;f<2;f++){for(let u=0;u<n;u++){if(p(l,s)){let w=`${l},${s}`;if(!r.has(w)){if(r.add(w),yield{x:l,y:s},d++,d>=i)return}}l+=c[t][0],s+=c[t][1]}t=(t+1)%4}n++}};if(this.strategy==="SPIRAL_FROM_CENTER")yield*g();else{let d=[...g()];for(let f=d.length-1;f>=0;f--)yield d[f]}break}case"SCRIBBLE":{let r=new Set,i=o*a,l=Math.floor(o/2),s=Math.floor(a/2);for(let c=0;r.size<i&&c<i*24;c++){let t=`${l},${s}`;if(!r.has(t))r.add(t),yield{x:l,y:s};if(l+=Math.floor(Math.random()*3)-1,s+=Math.floor(Math.random()*3)-1,l<0||l>=o||s<0||s>=a)l=Math.floor(Math.random()*o),s=Math.floor(Math.random()*a)}for(let c=0;c<a;c++)for(let t=0;t<o;t++){let n=`${t},${c}`;if(r.has(n))continue;r.add(n),yield{x:t,y:c}}break}case"CROSSHATCH":{let r=[];for(let s=0;s<o+a-1;s++)for(let c=Math.max(0,s-o+1);c<=Math.min(a-1,s);c++){let t=s-c;r.push({x:t,y:c})}let i=[];for(let s=-a+1;s<o;s++)for(let c=0;c<a;c++){let t=c+s;if(t>=0&&t<o)i.push({x:t,y:c})}let l=new Set;for(let s of[...r,...i]){let c=`${s.x},${s.y}`;if(l.has(c))continue;l.add(c),yield s}break}case"WAVE_SWEEP":{let r=new Set;for(let i=0;i<o;i++){let s=(Math.sin(i/Math.max(1,o-1)*Math.PI*4)+1)*0.5*(a-1)|0;for(let c=0;c<a;c++){let t=s+c,n=s-c;for(let p of[t,n]){if(p<0||p>=a)continue;let g=`${i},${p}`;if(r.has(g))continue;r.add(g),yield{x:i,y:p}}}}break}case"SCATTERED_LINES":{let r=new Set,i=o*a;for(let l=0;r.size<i&&l<i*14;l++){let s=Math.floor(Math.random()*o),c=Math.floor(Math.random()*a),t=Math.random()*Math.PI*2,n=Math.round(Math.cos(t)),p=Math.round(Math.sin(t)),g=6+Math.floor(Math.random()*28);for(let d=0;d<g;d++){if(s<0||s>=o||c<0||c>=a)break;let f=`${s},${c}`;if(!r.has(f))r.add(f),yield{x:s,y:c};s+=n,c+=p}}for(let l=0;l<a;l++)for(let s=0;s<o;s++){let c=`${s},${l}`;if(r.has(c))continue;r.add(c),yield{x:s,y:l}}break}case"CONTOUR_JITTER":{let r=new Set;for(let i=0;i<Math.ceil(Math.min(o,a)/2);i++){let l=[],s=i,c=i,t=o-i-1,n=a-i-1;for(let p=s;p<=t;p++)l.push({x:p,y:c});for(let p=c+1;p<=n;p++)l.push({x:t,y:p});for(let p=t-1;p>=s;p--)l.push({x:p,y:n});for(let p=n-1;p>c;p--)l.push({x:s,y:p});for(let p=l.length-1;p>0;p--){let g=Math.floor(Math.random()*(p+1)),d=l[p];l[p]=l[g],l[g]=d}for(let p of l){let g=`${p.x},${p.y}`;if(r.has(g))continue;r.add(g),yield p}}break}case"SPIRAL_WOBBLE":{let r=new Set,i=o/2,l=a/2,s=Math.hypot(i,l);for(let c=0;r.size<o*a&&c<o*a*9;c++){let t=c/(o*a*9)*s,n=c*0.31+Math.sin(c*0.07)*0.7,p=Math.round(i+Math.cos(n)*t),g=Math.round(l+Math.sin(n)*t);if(p<0||p>=o||g<0||g>=a)continue;let d=`${p},${g}`;if(r.has(d))continue;r.add(d),yield{x:p,y:g}}for(let c=0;c<a;c++)for(let t=0;t<o;t++){let n=`${t},${c}`;if(r.has(n))continue;yield{x:t,y:c}}break}case"CLUSTER_BURSTS":{let r=new Set,i=o*a;for(let l=0;r.size<i&&l<i*12;l++){let s=Math.floor(Math.random()*o),c=Math.floor(Math.random()*a),t=2+Math.floor(Math.random()*10);for(let n=c-t;n<=c+t;n++)for(let p=s-t;p<=s+t;p++){if(p<0||p>=o||n<0||n>=a)continue;if(Math.hypot(p-s,n-c)>t)continue;let g=`${p},${n}`;if(r.has(g))continue;r.add(g),yield{x:p,y:n}}}for(let l=0;l<a;l++)for(let s=0;s<o;s++){let c=`${s},${l}`;if(r.has(c))continue;r.add(c),yield{x:s,y:l}}break}case"ORBITAL":{let r=new Set,i=(o-1)/2,l=(a-1)/2,s=Math.ceil(Math.max(i,l));for(let c=0;c<=s;c++){let t=Math.max(16,Math.ceil(2*Math.PI*Math.max(1,c)*2));for(let n=0;n<t;n++){let p=n/t*Math.PI*2+(c%2?0.3:-0.3),g=Math.round(i+Math.cos(p)*c),d=Math.round(l+Math.sin(p)*c);if(g<0||g>=o||d<0||d>=a)continue;let f=`${g},${d}`;if(r.has(f))continue;r.add(f),yield{x:g,y:d}}}for(let c=0;c<a;c++)for(let t=0;t<o;t++){let n=`${t},${c}`;if(r.has(n))continue;yield{x:t,y:c}}break}case"FLOW_FIELD":{let r=new Set,i=o*a;for(let l=0;r.size<i&&l<i*18;l++){let s=Math.floor(Math.random()*o),c=Math.floor(Math.random()*a);for(let t=0;t<120;t++){if(s<0||s>=o||c<0||c>=a)break;let n=`${s},${c}`;if(!r.has(n))r.add(n),yield{x:s,y:c};let p=Math.sin(s*0.09)*1.8+Math.cos(c*0.08)*1.6+Math.sin((s+c)*0.05);s+=Math.round(Math.cos(p)),c+=Math.round(Math.sin(p))}}for(let l=0;l<a;l++)for(let s=0;s<o;s++){let c=`${s},${l}`;if(r.has(c))continue;r.add(c),yield{x:s,y:l}}break}case"EDGE_IN":{let r=new Set,i=Math.ceil(Math.min(o,a)/2);for(let l=0;l<i;l++){let s=l,c=o-1-l,t=l,n=a-1-l;for(let p=s;p<=c;p++)for(let g of[t,n]){let d=`${p},${g}`;if(r.has(d))continue;r.add(d),yield{x:p,y:g}}for(let p=t+1;p<=n-1;p++)for(let g of[s,c]){let d=`${g},${p}`;if(r.has(d))continue;r.add(d),yield{x:g,y:p}}}break}}}moveStart(o){if(o.button!==0)return;if(o.preventDefault(),o.stopPropagation(),!this.lock)this.moveInfo={globalX:this.position.globalX,globalY:this.position.globalY,clientX:o.clientX,clientY:o.clientY}}moveStop(){if(this.moveInfo)this.moveInfo=void 0,this.position.updateAnchor(),this.pixels.update(),this.updateColors(),A(this.bot)}move(o){if(!this.moveInfo)return;let a=Math.round((o.clientX-this.moveInfo.clientX)/this.position.pixelSize),r=Math.round((o.clientY-this.moveInfo.clientY)/this.position.pixelSize);if(this.moveInfo.globalX!==void 0){if(this.position.globalX=a+this.moveInfo.globalX,this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,this.moveInfo.width-a)}else if(this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,a+this.moveInfo.width);if(this.moveInfo.globalY!==void 0){if(this.position.globalY=r+this.moveInfo.globalY,this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,this.moveInfo.height-r)}else if(this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,r+this.moveInfo.height);this.update(),A(this.bot)}resizeStart(o){if(this.lock||o.button!==0)return;o.preventDefault(),o.stopPropagation(),this.moveInfo={clientX:o.clientX,clientY:o.clientY};let a=o.target;if(a.classList.contains("n"))this.moveInfo.height=this.pixels.height,this.moveInfo.globalY=this.position.globalY;if(a.classList.contains("e"))this.moveInfo.width=this.pixels.width;if(a.classList.contains("s"))this.moveInfo.height=this.pixels.height;if(a.classList.contains("w"))this.moveInfo.width=this.pixels.width,this.moveInfo.globalX=this.position.globalX}export(){let o=document.createElement("a");document.body.append(o),o.href=URL.createObjectURL(new Blob([JSON.stringify(this.toJSON())],{type:"application/json"})),o.download=`${this.pixels.width}x${this.pixels.height}.${X}`,o.click(),URL.revokeObjectURL(o.href),o.href=this.pixels.canvas.toDataURL("image/webp",1),o.download=`${this.pixels.width}x${this.pixels.height}.webp`,o.click(),URL.revokeObjectURL(o.href),o.remove()}}function eo(){let o=localStorage.getItem("kglacer-macro:shield-config");if(!o)return!1;try{return JSON.parse(o).enabled!==!1}catch{return!1}}function No(o){localStorage.setItem("kglacer-macro:shield-config",JSON.stringify({enabled:o}))}function ga(o){let a=`${o?.host??""} ${o?.username??""}`.toLowerCase(),r=/(mx|mex|mexico)/.test(a)?"MX":"AUTO";localStorage.setItem("__afm_proxy_hint",r)}function Do(o){if(!eo())return;if(document.getElementById("kgm-shield-full"))return;ga(o);let a=document.createElement("script");a.id="kgm-shield-full",a.textContent=`// ==UserScript==
