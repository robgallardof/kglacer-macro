...
`;class G{bot;image;width;brightness;exactColor;dithering;static async fromJSON(o,s){let a=new Image;return a.src=s.url.startsWith("http")?await fetch(s.url,{cache:"no-store"}).then((p)=>p.blob()).then((p)=>URL.createObjectURL(p)):s.url,await W(a,["load"],["error"]),new G(o,a,s.width,s.brightness,s.exactColor,s.dithering)}canvas=document.createElement("canvas");context=this.canvas.getContext("2d");pixels;colors=new Map;resolution;get height(){return this.width/this.resolution|0}set height(o){this.width=o*this.resolution|0}constructor(o,s,a=s.naturalWidth,p=0,l=!1,c=!1){this.bot=o;this.image=s;this.width=a;this.brightness=p;this.exactColor=l;this.dithering=c;if(l)this.resolution=1,this.width=1000;else this.resolution=this.image.naturalWidth/this.image.naturalHeight;this.update()}update(){this.canvas.width=this.width,this.canvas.height=this.height,this.colors.clear();let o=new Map;for(let a=1;a<64;a++)if(this.exactColor||!this.bot.unavailableColors.has(a))o.set(j[a],[a,a]);this.context.imageSmoothingEnabled=!1,this.context.imageSmoothingQuality="low",this.context.drawImage(this.image,0,0,this.canvas.width,this.canvas.height),this.pixels=Array.from({length:this.canvas.height},()=>new Array(this.canvas.width));let s=this.context.getImageData(0,0,this.canvas.width,this.canvas.height).data;for(let a=0;a<this.canvas.height;a++)for(let p=0;p<this.canvas.width;p++){let l=(a*this.canvas.width+p)*4,c=s[l],g=s[l+1],r=s[l+2],f=s[l+3],w=this.dithering?this.ditherOffset(p,a):0,u=this.clampByte(c+w),b=this.clampByte(g+w),n=this.clampByte(r+w),i=`${u},${b},${n}`;if(this.exactColor){this.pixels[a][p]=f<100?0:j.indexOf(i);continue}let m,z;if(f<100)m=z=0;else if(o.has(i))[m,z]=o.get(i);else{let A=1/0,h=1/0;for(let H=0;H<V.length;H++){let Q=V[H],J=zo(io(u,b,n),Q,this.brightness);if(!this.bot.unavailableColors.has(H)&&J<A)A=J,m=H;if(J<h)h=J,z=H}o.set(i,[m,z])}if(m!==0)this.context.fillStyle=`oklab(${V[m][0]*100}% ${V[m][1]} ${V[m][2]})`,this.context.fillRect(p,a,1,1);this.pixels[a][p]=m;let k=this.colors.get(z);if(k)k.amount++;else this.colors.set(z,{color:m,amount:1,realColor:z})}}toJSON(){let o=document.createElement("canvas");return o.width=this.image.naturalWidth,o.height=this.image.naturalHeight,o.getContext("2d").drawImage(this.image,0,0),{url:o.toDataURL("image/webp",1),width:this.width,brightness:this.brightness,exactColor:this.exactColor,dithering:this.dithering}}ditherOffset(o,s){return[[0,8,2,10],[12,4,14,6],[3,11,1,9],[15,7,13,5]][s%4][o%4]-8}clampByte(o){return Math.max(0,Math.min(255,o|0))}}var S="kglacer-macro-settings",Mo=["kglacermacro","wbot"],$="kgm";function Co(){let o=[S,...Mo];for(let s=0;s<o.length;s++){let a=o[s],p=localStorage.getItem(a);if(!p)continue;return{json:p,key:a}}return}function ho(){let o=Co();if(!o)return;let s;try{if(s=JSON.parse(o.json),typeof s!=="object")throw new Error("NOT VALID SAVE");if(s.version===1){let a=s;s.images=a.widget.images,s.strategy=a.widget.strategy,delete a.widget}if(o.key!==S)localStorage.setItem(S,o.json)}catch{localStorage.removeItem(o.key),s=void 0}return s}var Ao;function P(o,s=!1){if(clearTimeout(Ao),s)localStorage.setItem(S,JSON.stringify(o));else Ao=setTimeout(()=>{localStorage.setItem(S,JSON.stringify(o))},600)}var M=1000,Ro=2048,Y=M*Ro,B=[],T=[],Lo=Date.now();function I(o){B.push(o),T.push({id:Lo++,latitude:(2*Math.atan(Math.exp(-(o.y/Y*(2*Math.PI)-Math.PI)))-Math.PI/2)*180/Math.PI,longitude:(o.x/Y*(2*Math.PI)-Math.PI)*180/Math.PI,name:"KGLACER_MACRO_FAVORITE"})}I({x:Y/3|0,y:Y/3|0});I({x:Y/3*2|0,y:Y/3*2|0});function C(o){let[s,a]=o.style.transform.slice(32,-31).split(", ").map((p)=>Number.parseFloat(p));return{x:s,y:a}}class N{bot;static fromJSON(o,s){return new N(o,...s)}static fromScreenPosition(o,s){let{anchorScreenPosition:a,pixelSize:p,anchorWorldPosition:l}=o.findAnchorsForScreen(s);return new N(o,l.x+(s.x-a.x)/p|0,l.y+(s.y-a.y)/p|0)}globalX=0;globalY=0;get tileX(){return this.globalX/M|0}set tileX(o){this.globalX=o*M+this.x}get tileY(){return this.globalY/M|0}set tileY(o){this.globalY=o*M+this.y}get x(){return this.globalX%M}set x(o){this.globalX=this.tileX*M+o}get y(){return this.globalY%M}set y(o){this.globalY=this.tileY*M+o}anchor1Index;anchor2Index;get pixelSize(){return(C(this.bot.$stars[this.anchor2Index]).x-C(this.bot.$stars[this.anchor1Index]).x)/(B[this.anchor2Index].x-B[this.anchor1Index].x)}constructor(o,s,a,p,l){this.bot=o;if(p===void 0||l===void 0)this.globalX=s,this.globalY=a;else this.globalX=s*M+p,this.globalY=a*M+l;this.updateAnchor()}updateAnchor(){this.anchor1Index=0,this.anchor2Index=1;let o=1/0,s=1/0;for(let a=0;a<B.length;a++){let{x:p,y:l}=B[a];if(p<this.globalX&&l<this.globalY){let c=this.globalX-p+(this.globalY-l);if(c<o)o=c,this.anchor1Index=a}else if(p>this.globalX&&l>this.globalY){let c=p-this.globalX+(l-this.globalY);if(c<s)s=c,this.anchor2Index=a}}}toScreenPosition(){let o=B[this.anchor1Index],s=C(this.bot.$stars[this.anchor1Index]);return{x:(this.globalX-o.x)*this.pixelSize+s.x,y:(this.globalY-o.y)*this.pixelSize+s.y}}getMapColor(){return this.bot.mapsCache.get(this.tileX+"/"+this.tileY).pixels[this.y][this.x]}scrollScreenTo(){let{x:o,y:s}=this.toScreenPosition();this.bot.moveMap({x:o-window.innerWidth/3,y:s-window.innerHeight/3})}clone(){return new N(this.bot,this.tileX,this.tileY,this.x,this.y)}toJSON(){return[this.globalX,this.globalY]}}class R extends O{bot;position;pixels;strategy;opacity;drawTransparentPixels;drawColorsInOrder;colors;lock;static async fromJSON(o,s){return new R(o,N.fromJSON(o,s.position),await G.fromJSON(o,s.pixels),s.strategy,s.opacity,s.drawTransparentPixels,s.drawColorsInOrder,s.colors,s.lock)}element=document.createElement("div");tasks=[];moveInfo;$brightness;$canvas;$colorsDialog;$colorsDialogList;$colorSearch;$openColors;$openPreview;$closeColors;$closePreview;$delete;$dithering;$drawColorsInOrder;$drawTransparent;$export;$lock;$opacity;$progressLine;$progressText;$previewDialog;$previewDialogList;$resetSize;$resetSizeSpan;$settings;$strategy;$topbar;$wrapper;colorDialogDragState;suppressNextColorDialogBackdropClick=!1;previewCacheSignature;previewSequenceCache=new Map;previewAnimations=new WeakMap;previewAnimationHandles=new Set;constructor(o,s,a,p="SPIRAL_FROM_CENTER",l=50,c=!1,g=!1,r=[],f=!1){super();this.bot=o;this.position=s;this.pixels=a;this.strategy=p;this.opacity=l;this.drawTransparentPixels=c;this.drawColorsInOrder=g;this.colors=r;this.lock=f;this.element.innerHTML=Ho,this.element.classList.add("wimage"),q(this.element),document.body.append(this.element),this.populateElementsWithSelector(this.element,{$brightness:".brightness",$colorsDialog:".colors-dialog",$colorsDialogList:".colors-dialog-list",$colorSearch:".color-search",$openColors:".open-colors",$openPreview:".open-preview",$closeColors:".close-colors",$closePreview:".close-preview",$delete:".delete",$dithering:".dithering",$drawColorsInOrder:".draw-colors-in-order",$drawTransparent:".draw-transparent",$export:".export",$lock:".lock",$opacity:".opacity",$progressLine:".wprogress div",$progressText:".wprogress span",$previewDialog:".preview-dialog",$previewDialogList:".preview-dialog-list",$resetSize:".reset-size",$settings:".wform",$strategy:".strategy",$topbar:".wtopbar",$wrapper:".wrapper"}),this.$resetSizeSpan=this.$resetSize.querySelector("span"),this.$canvas=this.pixels.canvas,this.$wrapper.prepend(this.pixels.canvas),document.body.append(this.$colorsDialog,this.$previewDialog),this.registerEvent(this.$strategy,"change",()=>{if(this.strategy=this.$strategy.value,this.$previewDialog.open)this.renderStrategyPreviewSamples();P(this.bot)}),this.registerEvent(this.$opacity,"input",()=>{this.opacity=this.$opacity.valueAsNumber,this.$opacity.style.setProperty("--val",this.opacity+"%"),this.update(),P(this.bot)}),this.$opacity.style.setProperty("--val",this.opacity+"%");let w;this.registerEvent(this.$brightness,"input",()=>{clearTimeout(w),w=setTimeout(()=>{this.refreshImageAdjustments()},150)}),this.registerEvent(this.$dithering,"change",()=>{this.pixels.dithering=this.$dithering.checked,this.refreshImageAdjustments()}),this.registerEvent(this.$resetSize,"click",()=>{this.pixels.width=this.pixels.image.naturalWidth,this.pixels.update(),this.updateColors(),this.update(),P(this.bot)}),this.registerEvent(this.$drawTransparent,"click",()=>{this.drawTransparentPixels=this.$drawTransparent.checked,P(this.bot)}),this.registerEvent(this.$drawColorsInOrder,"click",()=>{this.drawColorsInOrder=this.$drawColorsInOrder.checked,P(this.bot)}),this.registerEvent(this.$lock,"click",()=>{this.lock=!this.lock,this.update(),P(this.bot)}),this.registerEvent(this.$delete,"click",this.destroy.bind(this)),this.registerEvent(this.$openColors,"click",()=>{this.openColorPanel()}),this.registerEvent(this.$openPreview,"click",()=>{this.openPreviewPanel()}),this.registerEvent(this.$closeColors,"click",()=>{this.closeDialog(this.$colorsDialog)}),this.registerEvent(this.$closePreview,"click",()=>{this.closeDialog(this.$previewDialog)}),this.registerEvent(this.$colorsDialog.querySelector(".colors-dialog-head"),"pointerdown",this.startColorDialogDrag.bind(this)),this.registerEvent(document,"pointermove",this.moveColorDialog.bind(this),{passive:!1}),this.registerEvent(document,"pointerup",this.stopColorDialogDrag.bind(this)),this.registerEvent(document,"pointercancel",this.stopColorDialogDrag.bind(this)),this.registerEvent(this.$colorsDialog,"click",(u)=>{if(this.suppressNextColorDialogBackdropClick){this.suppressNextColorDialogBackdropClick=!1;return}if(u.target===this.$colorsDialog)this.closeDialog(this.$colorsDialog)}),this.registerEvent(this.$previewDialog,"click",(u)=>{if(u.target===this.$previewDialog)this.closeDialog(this.$previewDialog)}),this.registerEvent(this.$colorSearch,"input",()=>{this.updateColors()}),this.registerEvent(this.$export,"click",this.export.bind(this)),this.registerEvent(this.$topbar,"mousedown",this.moveStart.bind(this)),this.registerEvent(this.$canvas,"mousedown",this.moveStart.bind(this)),this.registerEvent(document,"mouseup",this.moveStop.bind(this)),this.registerEvent(document,"mousemove",this.move.bind(this));for(let u of this.element.querySelectorAll(".resize"))this.registerEvent(u,"mousedown",this.resizeStart.bind(this));this.update(),this.updateColors()}toJSON(){return{pixels:this.pixels.toJSON(),position:this.position.toJSON(),strategy:this.strategy,opacity:this.opacity,drawTransparentPixels:this.drawTransparentPixels,drawColorsInOrder:this.drawColorsInOrder,colors:this.colors,lock:this.lock}}refreshImageAdjustments(){this.pixels.brightness=this.$brightness.valueAsNumber,this.pixels.update(),this.updateColors(),this.update(),P(this.bot)}updateTasks(){this.tasks.length=0;let o=this.position.clone(),s=new Set,a=new Map;for(let p=0;p<this.colors.length;p++){let l=this.colors[p];if(l.disabled)s.add(l.realColor);a.set(l.realColor,p)}for(let{x:p,y:l}of this.strategyPositionIterator()){let c=this.pixels.pixels[l][p];if(s.has(c))continue;o.globalX=this.position.globalX+p,o.globalY=this.position.globalY+l;let g=o.getMapColor();if(c!==g&&(this.drawTransparentPixels||c!==0))this.tasks.push({position:o.clone(),color:c})}if(this.drawColorsInOrder)this.tasks.sort((p,l)=>(a.get(p.color)??0)-(a.get(l.color)??0));this.update(),this.bot.widget.update()}update(){let{x:o,y:s}=this.position.toScreenPosition(),a=this.position.pixelSize*this.pixels.width,p=this.position.pixelSize*this.pixels.height;this.element.style.transform=`translate3d(${o.toFixed(3)}px, ${s.toFixed(3)}px, 0)`,this.element.style.width=`${a}px`,this.element.style.height=`${p}px`,this.$canvas.style.opacity=`${this.opacity}%`,this.element.classList.remove("hidden"),this.$resetSizeSpan.textContent=this.pixels.width.toString(),this.$brightness.valueAsNumber=this.pixels.brightness,this.$dithering.checked=this.pixels.dithering,this.$strategy.value=this.strategy,this.$opacity.valueAsNumber=this.opacity,this.$drawTransparent.checked=this.drawTransparentPixels,this.$drawColorsInOrder.checked=this.drawColorsInOrder;let l=this.pixels.pixels.length*this.pixels.pixels[0].length,c=Math.max(0,l-this.tasks.length),g=l>0?c/l*100|0:0;this.$progressText.textContent=`${c}/${l} ${g}% ETA: ${this.tasks.length/120|0}h`,this.$progressLine.style.transform=`scaleX(${g/100})`,this.$canvas.classList[this.lock?"add":"remove"]("no-pointer-events");for(let r of this.element.querySelectorAll(".resize"))r.classList[this.lock?"add":"remove"]("no-pointer-events");this.$lock.classList[this.lock?"add":"remove"]("locked")}exportImage(){this.export()}destroy(){super.destroy(),this.element.remove(),this.$colorsDialog.remove(),this.$previewDialog.remove(),so(this.bot.images,this),this.bot.widget.update(),P(this.bot)}openColorPanel(){if(this.$colorsDialog.open){this.$colorSearch.focus();return}this.$colorsDialog.style.position="fixed",this.$colorsDialog.style.left="",this.$colorsDialog.style.top="",this.$colorsDialog.style.margin="auto",this.$colorsDialog.showModal(),this.$colorSearch.focus()}openPreviewPanel(){if(this.$previewDialog.open){this.renderStrategyPreviewSamples();return}this.$previewDialog.style.position="fixed",this.$previewDialog.style.left="",this.$previewDialog.style.top="",this.$previewDialog.style.margin="auto",this.$previewDialog.showModal(),this.renderStrategyPreviewSamples()}closeDialog(o){if(!o.open)return;if(o===this.$previewDialog)this.stopPreviewAnimations();if(typeof o.requestClose==="function")o.requestClose();else o.close()}stopPreviewAnimations(){for(let o of this.previewAnimationHandles)cancelAnimationFrame(o);this.previewAnimationHandles.clear()}startColorDialogDrag(o){if(o.button!==0)return;if(o.target?.closest("button,input,select,textarea,a,label"))return;let a=this.$colorsDialog.getBoundingClientRect();this.colorDialogDragState={pointerId:o.pointerId,offsetX:o.clientX-a.left,offsetY:o.clientY-a.top,moved:!1},o.preventDefault()}moveColorDialog(o){if(!this.colorDialogDragState)return;if(o.pointerId!==this.colorDialogDragState.pointerId)return;let s=this.$colorsDialog.getBoundingClientRect(),a=Math.max(8,window.innerWidth-s.width-8),p=Math.max(8,window.innerHeight-s.height-8),l=Math.min(a,Math.max(8,o.clientX-this.colorDialogDragState.offsetX)),c=Math.min(p,Math.max(8,o.clientY-this.colorDialogDragState.offsetY));if(!this.colorDialogDragState.moved&&(Math.abs(o.movementX)>0||Math.abs(o.movementY)>0))this.colorDialogDragState.moved=!0;this.$colorsDialog.style.left=`${Math.round(l)}px`,this.$colorsDialog.style.top=`${Math.round(c)}px`,o.preventDefault()}stopColorDialogDrag(o){if(!this.colorDialogDragState)return;if(o.pointerId!==this.colorDialogDragState.pointerId)return;if(this.colorDialogDragState.moved)this.suppressNextColorDialogBackdropClick=!0;this.colorDialogDragState=void 0}renderStrategyPreviewSamples(){this.stopPreviewAnimations(),this.invalidatePreviewCacheIfNeeded();let o=this.$strategy.value;this.$previewDialogList.innerHTML="";let s=document.createDocumentFragment(),a=document.createElement("article");a.className="preview-card";let p=document.createElement("strong");p.textContent=this.getStrategyLabel(o);let l=document.createElement("canvas");l.className="preview-canvas",l.width=156,l.height=156,this.paintStrategyPreview(l,o),a.append(p,l),s.append(a),this.$previewDialogList.append(s)}invalidatePreviewCacheIfNeeded(){let o=`${this.pixels.width}x${this.pixels.height}:${this.pixels.image.src.length}`;if(this.previewCacheSignature===o)return;this.previewCacheSignature=o,this.previewSequenceCache.clear()}getStrategyLabel(o){switch(o){case"RANDOM":return d("random");case"HUMANIZED":return d("humanized");case"HUMAN_SOFT_DITHER":return d("humanSoftDither");case"HUMAN_PATCHY":return d("humanPatchy");case"HUMAN_SWEEP_ARCS":return d("humanSweepArcs");case"HUMAN_MICRO_CORRECTIONS":return d("humanMicroCorrections");case"HUMAN_JITTER_FILL":return d("humanJitterFill");case"HUMAN_CORNER_BIAS":return d("humanCornerBias");case"HUMAN_LONG_STROKES":return d("humanLongStrokes");case"HUMAN_TAP_CLUSTERS":return d("humanTapClusters");case"HUMAN_MESSY_SPIRAL":return d("humanMessySpiral");case"HUMAN_DRUNK_WALK":return d("humanDrunkWalk");case"HUMAN_NOISE_CLOUD":return d("humanNoiseCloud");case"HUMAN_PATCH_JUMP":return d("humanPatchJump");case"HUMAN_HESITANT_LINES":return d("humanHesitantLines");case"HUMAN_OVERLAP_SWEEPS":return d("humanOverlapSweeps");case"HUMAN_WOBBLE_DRIFT":return d("humanWobbleDrift");case"HUMAN_GAP_RECOVERY":return d("humanGapRecovery");case"HUMAN_STAIRCASE":return d("humanStaircase");case"HUMAN_EDGE_HUGGER":return d("humanEdgeHugger");case"HUMAN_BLOBS":return d("humanBlobs");case"HUMAN_BACKTRACK":return d("humanBacktrack");case"HUMAN_SHAKY_DIAGONAL":return d("humanShakyDiagonal");case"HUMAN_LATE_FIXES":return d("humanLateFixes");case"ZIGZAG":return d("zigzag");case"BRUSH_STROKES":return d("brushStrokes");case"DIAGONAL_BRUSH":return d("diagonalBrush");case"DOWN":return d("down");case"UP":return d("up");case"LEFT":return d("left");case"RIGHT":return d("right");case"SPIRAL_FROM_CENTER":return d("spiralOut");case"SPIRAL_TO_CENTER":return d("spiralIn");case"SCRIBBLE":return d("scribble");case"CROSSHATCH":return d("crosshatch");case"WAVE_SWEEP":return d("waveSweep");case"SCATTERED_LINES":return d("scatteredLines");case"CONTOUR_JITTER":return d("contourJitter");case"SPIRAL_WOBBLE":return d("spiralWobble");case"CLUSTER_BURSTS":return d("clusterBursts");case"ORBITAL":return d("orbital");case"FLOW_FIELD":return d("flowField");case"EDGE_IN":return d("edgeIn");default:return o}}paintStrategyPreview(o,s){let a=o.getContext("2d");if(!a)return;a.fillStyle="#0f1526",a.fillRect(0,0,o.width,o.height);let p=this.getImagePreviewMask(),l=this.getCachedPreviewSequence(s,p),c=Math.min(o.width/this.pixels.width,o.height/this.pixels.height),g=(o.width-this.pixels.width*c)/2,r=(o.height-this.pixels.height*c)/2,f=this.previewAnimations.get(o);if(f)cancelAnimationFrame(f),this.previewAnimationHandles.delete(f);let w=(k)=>{let A=requestAnimationFrame((h)=>{this.previewAnimationHandles.delete(A),k(h)});return this.previewAnimationHandles.add(A),A},u=(k)=>{a.fillStyle="#0f1526",a.fillRect(0,0,o.width,o.height);for(let A=0;A<Math.min(k,l.length);A++){let h=l[A],H=this.pixels.pixels[h.y]?.[h.x]??0;if(!H)continue;a.fillStyle=go(H),a.fillRect(g+h.x*c,r+h.y*c,Math.max(1,c),Math.max(1,c))}},b=Math.min(3400,Math.max(900,l.length*8)),i=b+220,m=(k,A)=>{if(!this.$previewDialog.open)return;let h=(A-k)%i,H=Math.min(1,h/b),Q=H*H*(3-2*H);u(Math.floor(l.length*Q));let J=w((Z)=>{m(k,Z)});this.previewAnimations.set(o,J)},z=performance.now();m(z,z)}getCachedPreviewSequence(o,s){let a=`${o}:${this.pixels.width}x${this.pixels.height}:${s.length}`,p=this.previewSequenceCache.get(a);if(p)return p;let l=this.strategy;this.strategy=o;let c=[...this.strategyPositionIterator()];this.strategy=l;let g=new Set(s.map(({x:f,y:w})=>`${f}:${w}`)),r=c.filter(({x:f,y:w})=>g.has(`${f}:${w}`));return this.previewSequenceCache.set(a,r),r}getImagePreviewMask(){let o=this.pixels.width,s=this.pixels.height,a=[];for(let p=0;p<s;p++)for(let l=0;l<o;l++)if((this.pixels.pixels[p]?.[l]??0)!==0)a.push({x:l,y:p});return a.length?a:this.fallbackPreviewMask()}fallbackPreviewMask(){let o=[],s=this.pixels.width/2,a=this.pixels.height/2,p=Math.max(4,Math.min(this.pixels.width,this.pixels.height)/2.5);for(let l=0;l<this.pixels.height;l++)for(let c=0;c<this.pixels.width;c++)if((c-s)**2+(l-a)**2<=p**2)o.push({x:c,y:l});return o}applyLocale(){if(q(this.element),this.updateColors(),this.$previewDialog.open)this.renderStrategyPreviewSamples()}colorHex(o){let s=j[o]??"0,0,0",[a=0,p=0,l=0]=s.split(",").map((c)=>Number.parseInt(c,10));return`#${[a,p,l].map((c)=>c.toString(16).padStart(2,"0")).join("")}`}colorKeywords(o){let s=j[o]??"0,0,0",[a=0,p=0,l=0]=s.split(",").map((f)=>Number.parseInt(f,10)),c=Math.max(a,p,l),g=Math.min(a,p,l);if(c-g<15)return["gray","grey","gris","neutral","neutro"];if(a>p+30&&a>l+30)return["red","rojo"];if(p>a+30&&p>l+30)return["green","verde"];if(l>a+30&&l>p+30)return["blue","azul"];if(a>170&&p>120&&l<130)return["orange","naranja"];if(a>170&&p>110&&l>140)return["pink","rosa"];if(a>120&&p<100&&l>120)return["purple","violet","morado"];if(a>130&&p>130&&l<90)return["yellow","amarillo"];return["brown","cafe","marron"]}updateColors(){this.$colorsDialogList.innerHTML="";let o=this.pixels.pixels.length*this.pixels.pixels[0].length;this.$colorsDialogList.setAttribute("aria-label",d("colorPanelResults"));let s=this.$colorSearch.value.trim().toLowerCase();if(this.colors.length!==this.pixels.colors.size||this.colors.some((a)=>!this.pixels.colors.has(a.realColor)))this.colors=this.pixels.colors.values().toArray().sort((a,p)=>p.amount-a.amount).map((a)=>({realColor:a.realColor,disabled:!1})),P(this.bot);for(let a=0;a<this.colors.length;a++){let p=this.colors[a],l=this.pixels.colors.get(p.realColor),c=!1,g=l.realColor!==l.color,r=l.amount/o*100,f=this.colorHex(l.realColor),w=this.colorKeywords(l.realColor),u=()=>{p.disabled=p.disabled?void 0:!0,b.classList.toggle("disabled",Boolean(p.disabled));let i=b.querySelector(".state");if(i)i.textContent=p.disabled?d("disabled"):d("enabled");P(this.bot)},b=document.createElement("button");if(b.className=`color-chip ${p.disabled?"disabled":""}`,b.draggable=!0,b.setAttribute("aria-label",`${d("overlayColors")} #${a+1}: ${f.toUpperCase()}`),b.innerHTML=`<span class="order-index">#${a+1}</span>
...
<span class="premium ${g?"on":""}">${g?d("premium"):""}</span>`,b.querySelector(".swatch").style.setProperty("--swatch-color",go(l.realColor)),b.addEventListener("click",()=>{if(c){c=!1;return}u()}),b.addEventListener("dragstart",(i)=>{c=!0,b.classList.add("dragging"),i.dataTransfer?.setData("text/plain",String(a)),i.dataTransfer.effectAllowed="move"}),b.addEventListener("dragend",()=>{b.classList.remove("dragging")}),b.addEventListener("dragover",(i)=>{i.preventDefault(),b.classList.add("drag-target")}),b.addEventListener("dragleave",()=>{b.classList.remove("drag-target")}),b.addEventListener("drop",(i)=>{i.preventDefault(),b.classList.remove("drag-target");let m=Number.parseInt(i.dataTransfer?.getData("text/plain")??"-1",10);if(m<0||m===a||m>=this.colors.length)return;this.colors.splice(a,0,...this.colors.splice(m,1)),P(this.bot),this.updateColors()}),g){let i=document.createElement("button");i.textContent=d("buy"),i.className="buy-chip",i.addEventListener("click",(m)=>{m.stopPropagation(),document.getElementById("color-"+l.realColor)?.click()}),b.append(i)}let n=`${f} ${w.join(" ")} ${l.realColor} ${j[l.realColor]}`;if(!s||n.toLowerCase().includes(s))this.$colorsDialogList.append(b)}}*strategyPositionIterator(){let o=this.pixels.pixels[0].length,s=this.pixels.pixels.length;switch(this.strategy){case"DOWN":{for(let a=0;a<s;a++)for(let p=0;p<o;p++)yield{x:p,y:a};break}case"UP":{for(let a=s-1;a>=0;a--)for(let p=0;p<o;p++)yield{x:p,y:a};break}case"LEFT":{for(let a=0;a<o;a++)for(let p=0;p<s;p++)yield{x:a,y:p};break}case"RIGHT":{for(let a=o-1;a>=0;a--)for(let p=0;p<s;p++)yield{x:a,y:p};break}case"RANDOM":{let a=[];for(let p=0;p<s;p++)for(let l=0;l<o;l++)a.push({x:l,y:p});for(let p=a.length-1;p>=0;p--){let l=Math.floor(Math.random()*(p+1)),c=a[p];a[p]=a[l],a[l]=c}yield*a;break}case"ZIGZAG":{for(let a=0;a<s;a++)if(a%2===0)for(let p=0;p<o;p++)yield{x:p,y:a};else for(let p=o-1;p>=0;p--)yield{x:p,y:a};break}case"HUMANIZED":{let a=Math.max(4,Math.floor(Math.min(o,s)/10)),p=[];for(let l=0;l<s;l+=a)for(let c=0;c<o;c+=a)p.push({x:c,y:l});for(let l=p.length-1;l>=0;l--){let c=Math.floor(Math.random()*(l+1)),g=p[l];p[l]=p[c],p[c]=g}for(let l=0;l<p.length;l++){let c=p[l],g=Math.min(s,c.y+a),r=Math.min(o,c.x+a);for(let f=c.y;f<g;f++)if(Math.random()>0.35)for(let u=c.x;u<r;u++)yield{x:u,y:f};else for(let u=r-1;u>=c.x;u--)yield{x:u,y:f}}break}case"HUMAN_SOFT_DITHER":{let a=new Set;for(let p=0;p<s;p++){let l=Math.floor(Math.random()*3)-1;if((p+l)%2===0)for(let g=0;g<o;g+=2)a.add(`${g},${p}`),yield{x:g,y:p};else for(let g=1;g<o;g+=2)a.add(`${g},${p}`),yield{x:g,y:p}}for(let p=0;p<s;p++)for(let l=0;l<o;l++){let c=`${l},${p}`;if(a.has(c))continue;yield{x:l,y:p}}break}case"HUMAN_PATCHY":{let a=new Set,p=o*s;while(a.size<p){let l=Math.floor(Math.random()*o),c=Math.floor(Math.random()*s),g=1+Math.floor(Math.random()*5);for(let r=c-g;r<=c+g;r++)for(let f=l-g;f<=l+g;f++){if(f<0||f>=o||r<0||r>=s)continue;if(Math.hypot(f-l,r-c)>g+Math.random()*1.2)continue;let w=`${f},${r}`;if(a.has(w))continue;a.add(w),yield{x:f,y:r}}}break}case"HUMAN_SWEEP_ARCS":{let a=new Set,p=(o-1)/2,l=(s-1)/2,c=Math.hypot(p,l);for(let g=0;g<4;g++){let r=Math.random()*Math.PI*2;for(let f=0;f<=c;f+=0.35){let w=Math.PI/2+Math.random()*(Math.PI/1.5),u=Math.max(10,Math.floor(f*8));for(let b=0;b<u;b++){let n=r+w*b/u+Math.sin(f)*0.08,i=Math.round(p+Math.cos(n)*f),m=Math.round(l+Math.sin(n)*f);if(i<0||i>=o||m<0||m>=s)continue;let z=`${i},${m}`;if(a.has(z))continue;a.add(z),yield{x:i,y:m}}}}for(let g=0;g<s;g++)for(let r=0;r<o;r++){let f=`${r},${g}`;if(a.has(f))continue;yield{x:r,y:g}}break}case"HUMAN_MICRO_CORRECTIONS":{let a=new Set;for(let p=0;p<s;p++){let l=p%2===0?1:-1,c=l>0?0:o-1;for(let g=0;g<o;g++){let r=c+(Math.random()>0.82?l:0),f=p+(Math.random()>0.9?1:0);for(let w of[{x:c,y:p},{x:r,y:p},{x:c,y:f}]){if(w.x<0||w.x>=o||w.y<0||w.y>=s)continue;let u=`${w.x},${w.y}`;if(a.has(u))continue;a.add(u),yield w}c+=l}}for(let p=0;p<s;p++)for(let l=0;l<o;l++){let c=`${l},${p}`;if(a.has(c))continue;yield{x:l,y:p}}break}case"HUMAN_JITTER_FILL":{let a=[];for(let p=0;p<s;p++)for(let l=0;l<o;l++)a.push({x:l,y:p});a.sort((p,l)=>{let c=p.y+(Math.random()-0.5)*1.8,g=l.y+(Math.random()-0.5)*1.8;if(c!==g)return c-g;return p.x+(Math.random()-0.5)*2-(l.x+(Math.random()-0.5)*2)}),yield*a;break}case"HUMAN_CORNER_BIAS":{let a=[{x:0,y:0},{x:o-1,y:0},{x:0,y:s-1},{x:o-1,y:s-1}],p=a[Math.floor(Math.random()*a.length)],l=[];for(let c=0;c<s;c++)for(let g=0;g<o;g++){let f=Math.hypot(g-p.x,c-p.y)+Math.random()*3.5;l.push({point:{x:g,y:c},score:f})}l.sort((c,g)=>c.score-g.score);for(let c of l)yield c.point;break}case"HUMAN_LONG_STROKES":{let a=new Set,p=o*s;while(a.size<p){let l=Math.floor(Math.random()*o),c=Math.floor(Math.random()*s),g=Math.random()*Math.PI*2,r=Math.sign(Math.cos(g)),f=Math.sign(Math.sin(g)),w=10+Math.floor(Math.random()*40);for(let u=0;u<w;u++){if(l<0||l>=o||c<0||c>=s)break;let b=`${l},${c}`;if(!a.has(b))a.add(b),yield{x:l,y:c};if(Math.random()>0.78)l+=f,c+=r;else l+=r,c+=f}}break}case"HUMAN_TAP_CLUSTERS":{let a=new Set,p=o*s;while(a.size<p){let l=Math.floor(Math.random()*o),c=Math.floor(Math.random()*s),g=3+Math.floor(Math.random()*10);for(let r=0;r<g;r++){let f=Math.round(l+(Math.random()-0.5)*6),w=Math.round(c+(Math.random()-0.5)*6);if(f<0||f>=o||w<0||w>=s)continue;let u=`${f},${w}`;if(a.has(u))continue;a.add(u),yield{x:f,y:w}}}break}case"HUMAN_MESSY_SPIRAL":{let a=new Set,p=(o-1)/2,l=(s-1)/2,c=Math.hypot(p,l)+2;for(let g=0;a.size<o*s;g++){let r=g/3,f=Math.min(c,r*0.18),w=r*0.29+Math.sin(r*0.13)*0.8,u=Math.round(p+Math.cos(w)*f+Math.sin(r)*0.7),b=Math.round(l+Math.sin(w)*f+Math.cos(r)*0.7);if(u<0||u>=o||b<0||b>=s){if(g>o*s*18)break;continue}let n=`${u},${b}`;if(a.has(n)){if(Math.random()>0.9)continue}else a.add(n),yield{x:u,y:b};if(g>o*s*18)break}for(let g=0;g<s;g++)for(let r=0;r<o;r++){let f=`${r},${g}`;if(a.has(f))continue;yield{x:r,y:g}}break}case"HUMAN_DRUNK_WALK":{let a=new Set,p=Math.floor(Math.random()*o),l=Math.floor(Math.random()*s),c=[{x:-1,y:0},{x:1,y:0},{x:0,y:-1},{x:0,y:1},{x:-1,y:-1},{x:1,y:-1},{x:-1,y:1},{x:1,y:1}];while(a.size<o*s){let g=`${p},${l}`;if(!a.has(g))a.add(g),yield{x:p,y:l};let r=[];for(let u of c){let b=p+u.x,n=l+u.y;if(b<0||b>=o||n<0||n>=s)continue;r.push({x:b,y:n})}if(!r.length)break;let f=r.filter((u)=>{return!a.has(`${u.x},${u.y}`)});if(f.length&&Math.random()>0.2){let u=f[Math.floor(Math.random()*f.length)];p=u.x,l=u.y;continue}let w=r[Math.floor(Math.random()*r.length)];p=w.x,l=w.y}for(let g=0;g<s;g++)for(let r=0;r<o;r++){let f=`${r},${g}`;if(a.has(f))continue;yield{x:r,y:g}}break}case"HUMAN_NOISE_CLOUD":{let a=[];for(let p=0;p<s;p++)for(let l=0;l<o;l++){let c=Math.sin((l+1)*0.93+Math.random()*0.8)+Math.cos((p+1)*1.17+Math.random()*0.8),g=(Math.random()-0.5)*2.6,r=Math.hypot(l-o/2,p-s/2)*0.08;a.push({point:{x:l,y:p},score:c+g+r})}a.sort((p,l)=>p.score-l.score);for(let p of a)yield p.point;break}case"HUMAN_PATCH_JUMP":{let a=new Set,p=[];for(let l=0;l<Math.max(6,o*s/18);l++)p.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*s)});while(a.size<o*s){let l=p[Math.floor(Math.random()*p.length)],c=1+Math.floor(Math.random()*3),g=1+Math.floor(Math.random()*3);for(let r=l.y-g;r<=l.y+g;r++)for(let f=l.x-c;f<=l.x+c;f++){if(f<0||f>=o||r<0||r>=s)continue;if(Math.random()>0.86)continue;let w=`${f},${r}`;if(a.has(w))continue;a.add(w),yield{x:f,y:r}}if(Math.random()>0.72&&p.length<o*s/2)p.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*s)});if(a.size>o*s*0.92)break}for(let l=0;l<s;l++)for(let c=0;c<o;c++){let g=`${c},${l}`;if(a.has(g))continue;yield{x:c,y:l}}break}case"HUMAN_HESITANT_LINES":{let a=new Set;for(let p=0;p<s;p++){let l=p%2===0;for(let c=0;c<o;c++){let g=l?c:o-1-c,r=`${g},${p}`;if(!a.has(r))a.add(r),yield{x:g,y:p};if(Math.random()>0.7){let f=Math.max(0,Math.min(o-1,g+(Math.random()>0.5?1:-1))),w=Math.max(0,Math.min(s-1,p+(Math.random()>0.65?1:0))),u=`${f},${w}`;if(!a.has(u))a.add(u),yield{x:f,y:w}}}}for(let p=0;p<s;p++)for(let l=0;l<o;l++){let c=`${l},${p}`;if(a.has(c))continue;yield{x:l,y:p}}break}case"HUMAN_OVERLAP_SWEEPS":{let a=[],p=Math.random()*Math.PI*2;for(let l=0;l<s;l++)for(let c=0;c<o;c++){let g=Math.sin((c+l)*0.42+p)*2.2,r=Math.cos((c-l)*0.3+p)*1.4;a.push({point:{x:c,y:l},score:l+g+r+(Math.random()-0.5)*3.4})}a.sort((l,c)=>l.score-c.score);for(let l of a)yield l.point;break}case"HUMAN_WOBBLE_DRIFT":{let a=[],p=o/2,l=s/2;for(let c=0;c<s;c++)for(let g=0;g<o;g++){let r=Math.hypot(g-p,c-l)*0.25,f=Math.sin((g+1)*0.9)*1.8+Math.cos((c+1)*1.1)*1.8+Math.sin((g+c)*0.35)*1.4;a.push({point:{x:g,y:c},score:r+f+(Math.random()-0.5)*2.8})}a.sort((c,g)=>c.score-g.score);for(let c of a)yield c.point;break}case"HUMAN_GAP_RECOVERY":{let a=new Set,p=[];for(let l=0;l<s;l++)for(let c=0;c<o;c++){if(Math.random()>0.87){p.push({x:c,y:l});continue}a.add(`${c},${l}`),yield{x:c,y:l}}p.sort((l,c)=>Math.hypot(l.x-o/2,l.y-s/2)-Math.hypot(c.x-o/2,c.y-s/2));for(let l of p){let c=`${l.x},${l.y}`;if(a.has(c))continue;a.add(c),yield l}break}case"HUMAN_STAIRCASE":{let a=new Set,p=o+s-1;for(let l=0;l<p;l++){let c=Math.max(0,l-o+1),g=Math.min(s-1,l);for(let r=c;r<=g;r++){let f=l-r,w=[{x:f,y:r},{x:f+(Math.random()>0.5?1:-1),y:r},{x:f,y:r+(Math.random()>0.5?1:-1)}];for(let u of w){if(u.x<0||u.x>=o||u.y<0||u.y>=s)continue;let b=`${u.x},${u.y}`;if(a.has(b))continue;a.add(b),yield u}}}for(let l=0;l<s;l++)for(let c=0;c<o;c++){let g=`${c},${l}`;if(a.has(g))continue;yield{x:c,y:l}}break}case"HUMAN_EDGE_HUGGER":{let a=[];for(let p=0;p<s;p++)for(let l=0;l<o;l++){let c=Math.min(l,p,o-1-l,s-1-p);a.push({point:{x:l,y:p},score:c*3.5+(Math.random()-0.5)*5.5})}a.sort((p,l)=>p.score-l.score);for(let p of a)yield p.point;break}case"HUMAN_BLOBS":{let a=new Set,p=o*s;while(a.size<p){let l=Math.floor(Math.random()*o),c=Math.floor(Math.random()*s),g=1+Math.floor(Math.random()*4);for(let r=c-g;r<=c+g;r++)for(let f=l-g;f<=l+g;f++){if(f<0||f>=o||r<0||r>=s)continue;let w=Math.atan2(r-c,f-l),u=g+Math.sin(w*3+Math.random())*0.8;if(Math.hypot(f-l,r-c)>u)continue;let b=`${f},${r}`;if(a.has(b))continue;a.add(b),yield{x:f,y:r}}}break}case"HUMAN_BACKTRACK":{let a=new Set,p=[];for(let l=0;l<s;l++)for(let c=0;c<o;c++)p.push({x:c,y:l});p.sort((l,c)=>l.y-c.y+(Math.random()-0.5)*2.2+(l.x-c.x)*0.04);for(let l=0;l<p.length;l++){let c=p[l],g=`${c.x},${c.y}`;if(a.has(g))continue;if(a.add(g),yield c,l>1&&Math.random()>0.74){let r=p[l-1],f=`${r.x},${r.y}`;if(!a.has(f))a.add(f),yield r}}for(let l=0;l<s;l++)for(let c=0;c<o;c++){let g=`${c},${l}`;if(a.has(g))continue;yield{x:c,y:l}}break}case"HUMAN_SHAKY_DIAGONAL":{let a=[];for(let p=0;p<s;p++)for(let l=0;l<o;l++){let c=Math.abs(l-p)*0.6,g=Math.sin(l*1.4+p*0.8)*1.8+Math.cos(p*1.1+l*0.5)*1.5;a.push({point:{x:l,y:p},score:c+g+(Math.random()-0.5)*3.2})}a.sort((p,l)=>p.score-l.score);for(let p of a)yield p.point;break}case"HUMAN_LATE_FIXES":{let a=[],p=[];for(let l=0;l<s;l++)for(let c=0;c<o;c++)if(Math.random()>0.9)p.push({x:c,y:l});else a.push({x:c,y:l});a.sort((l,c)=>l.y-c.y+(Math.random()-0.5)*1.5+(Math.random()>0.85?l.x-c.x:0)),p.sort((l,c)=>Math.hypot(c.x-o/2,c.y-s/2)-Math.hypot(l.x-o/2,l.y-s/2)),yield*a,yield*p;break}case"DIAGONAL_BRUSH":{for(let a=0;a<o+s-1;a++){let p=a%2===0,l=[],c=Math.max(0,a-o+1),g=Math.min(s-1,a);for(let r=c;r<=g;r++){let f=a-r;if(f>=0&&f<o)l.push({x:f,y:r})}if(Math.random()>0.55)l.reverse();if(p)for(let r=l.length-1;r>=0;r--)yield l[r];else yield*l}break}case"BRUSH_STROKES":{let a=Array.from({length:s},()=>Array(o).fill(!1)),p=[{x:1,y:0},{x:-1,y:0},{x:0,y:1},{x:0,y:-1},{x:1,y:1},{x:1,y:-1},{x:-1,y:1},{x:-1,y:-1}],l=(r,f)=>r>=0&&r<o&&f>=0&&f<s,c=0,g=o*s;for(let r=0;r<g*6&&c<g;r++){let f=Math.floor(Math.random()*o),w=Math.floor(Math.random()*s),u=p[Math.floor(Math.random()*p.length)],b=3+Math.floor(Math.random()*16);for(let n=0;n<b;n++){if(!l(f,w))break;if(!a[w][f])a[w][f]=!0,c++,yield{x:f,y:w};if(Math.random()>0.72)u=p[Math.floor(Math.random()*p.length)];f+=u.x,w+=u.y}}for(let r=0;r<s;r++)for(let f=0;f<o;f++)if(!a[r][f])yield{x:f,y:r};break}case"SPIRAL_FROM_CENTER":case"SPIRAL_TO_CENTER":{let a=new Set,p=o*s,l=Math.floor(o/2),c=Math.floor(s/2),g=[[1,0],[0,1],[-1,0],[0,-1]],r=0,f=1,w=(b,n)=>b>=0&&b<o&&n>=0&&n<s,u=function*(){let b=0;while(b<p){for(let n=0;n<2;n++){for(let i=0;i<f;i++){if(w(l,c)){let m=`${l},${c}`;if(!a.has(m)){if(a.add(m),yield{x:l,y:c},b++,b>=p)return}}l+=g[r][0],c+=g[r][1]}r=(r+1)%4}f++}};if(this.strategy==="SPIRAL_FROM_CENTER")yield*u();else{let b=[...u()];for(let n=b.length-1;n>=0;n--)yield b[n]}break}case"SCRIBBLE":{let a=new Set,p=o*s,l=Math.floor(o/2),c=Math.floor(s/2);for(let g=0;a.size<p&&g<p*24;g++){let r=`${l},${c}`;if(!a.has(r))a.add(r),yield{x:l,y:c};if(l+=Math.floor(Math.random()*3)-1,c+=Math.floor(Math.random()*3)-1,l<0||l>=o||c<0||c>=s)l=Math.floor(Math.random()*o),c=Math.floor(Math.random()*s)}for(let g=0;g<s;g++)for(let r=0;r<o;r++){let f=`${r},${g}`;if(a.has(f))continue;a.add(f),yield{x:r,y:g}}break}case"CROSSHATCH":{let a=[];for(let c=0;c<o+s-1;c++)for(let g=Math.max(0,c-o+1);g<=Math.min(s-1,c);g++){let r=c-g;a.push({x:r,y:g})}let p=[];for(let c=-s+1;c<o;c++)for(let g=0;g<s;g++){let r=g+c;if(r>=0&&r<o)p.push({x:r,y:g})}let l=new Set;for(let c of[...a,...p]){let g=`${c.x},${c.y}`;if(l.has(g))continue;l.add(g),yield c}break}case"WAVE_SWEEP":{let a=new Set;for(let p=0;p<o;p++){let c=(Math.sin(p/Math.max(1,o-1)*Math.PI*4)+1)*0.5*(s-1)|0;for(let g=0;g<s;g++){let r=c+g,f=c-g;for(let w of[r,f]){if(w<0||w>=s)continue;let u=`${p},${w}`;if(a.has(u))continue;a.add(u),yield{x:p,y:w}}}}break}case"SCATTERED_LINES":{let a=new Set,p=o*s;for(let l=0;a.size<p&&l<p*14;l++){let c=Math.floor(Math.random()*o),g=Math.floor(Math.random()*s),r=Math.random()*Math.PI*2,f=Math.round(Math.cos(r)),w=Math.round(Math.sin(r)),u=6+Math.floor(Math.random()*28);for(let b=0;b<u;b++){if(c<0||c>=o||g<0||g>=s)break;let n=`${c},${g}`;if(!a.has(n))a.add(n),yield{x:c,y:g};c+=f,g+=w}}for(let l=0;l<s;l++)for(let c=0;c<o;c++){let g=`${c},${l}`;if(a.has(g))continue;a.add(g),yield{x:c,y:l}}break}case"CONTOUR_JITTER":{let a=new Set;for(let p=0;p<Math.ceil(Math.min(o,s)/2);p++){let l=[],c=p,g=p,r=o-p-1,f=s-p-1;for(let w=c;w<=r;w++)l.push({x:w,y:g});for(let w=g+1;w<=f;w++)l.push({x:r,y:w});for(let w=r-1;w>=c;w--)l.push({x:w,y:f});for(let w=f-1;w>g;w--)l.push({x:c,y:w});for(let w=l.length-1;w>0;w--){let u=Math.floor(Math.random()*(w+1)),b=l[w];l[w]=l[u],l[u]=b}for(let w of l){let u=`${w.x},${w.y}`;if(a.has(u))continue;a.add(u),yield w}}break}case"SPIRAL_WOBBLE":{let a=new Set,p=o/2,l=s/2,c=Math.hypot(p,l);for(let g=0;a.size<o*s&&g<o*s*9;g++){let r=g/(o*s*9)*c,f=g*0.31+Math.sin(g*0.07)*0.7,w=Math.round(p+Math.cos(f)*r),u=Math.round(l+Math.sin(f)*r);if(w<0||w>=o||u<0||u>=s)continue;let b=`${w},${u}`;if(a.has(b))continue;a.add(b),yield{x:w,y:u}}for(let g=0;g<s;g++)for(let r=0;r<o;r++){let f=`${r},${g}`;if(a.has(f))continue;yield{x:r,y:g}}break}case"CLUSTER_BURSTS":{let a=new Set,p=o*s;for(let l=0;a.size<p&&l<p*12;l++){let c=Math.floor(Math.random()*o),g=Math.floor(Math.random()*s),r=2+Math.floor(Math.random()*10);for(let f=g-r;f<=g+r;f++)for(let w=c-r;w<=c+r;w++){if(w<0||w>=o||f<0||f>=s)continue;if(Math.hypot(w-c,f-g)>r)continue;let u=`${w},${f}`;if(a.has(u))continue;a.add(u),yield{x:w,y:f}}}for(let l=0;l<s;l++)for(let c=0;c<o;c++){let g=`${c},${l}`;if(a.has(g))continue;a.add(g),yield{x:c,y:l}}break}case"ORBITAL":{let a=new Set,p=(o-1)/2,l=(s-1)/2,c=Math.ceil(Math.max(p,l));for(let g=0;g<=c;g++){let r=Math.max(16,Math.ceil(2*Math.PI*Math.max(1,g)*2));for(let f=0;f<r;f++){let w=f/r*Math.PI*2+(g%2?0.3:-0.3),u=Math.round(p+Math.cos(w)*g),b=Math.round(l+Math.sin(w)*g);if(u<0||u>=o||b<0||b>=s)continue;let n=`${u},${b}`;if(a.has(n))continue;a.add(n),yield{x:u,y:b}}}for(let g=0;g<s;g++)for(let r=0;r<o;r++){let f=`${r},${g}`;if(a.has(f))continue;yield{x:r,y:g}}break}case"FLOW_FIELD":{let a=new Set,p=o*s;for(let l=0;a.size<p&&l<p*18;l++){let c=Math.floor(Math.random()*o),g=Math.floor(Math.random()*s);for(let r=0;r<120;r++){if(c<0||c>=o||g<0||g>=s)break;let f=`${c},${g}`;if(!a.has(f))a.add(f),yield{x:c,y:g};let w=Math.sin(c*0.09)*1.8+Math.cos(g*0.08)*1.6+Math.sin((c+g)*0.05);c+=Math.round(Math.cos(w)),g+=Math.round(Math.sin(w))}}for(let l=0;l<s;l++)for(let c=0;c<o;c++){let g=`${c},${l}`;if(a.has(g))continue;a.add(g),yield{x:c,y:l}}break}case"EDGE_IN":{let a=new Set,p=Math.ceil(Math.min(o,s)/2);for(let l=0;l<p;l++){let c=l,g=o-1-l,r=l,f=s-1-l;for(let w=c;w<=g;w++)for(let u of[r,f]){let b=`${w},${u}`;if(a.has(b))continue;a.add(b),yield{x:w,y:u}}for(let w=r+1;w<=f-1;w++)for(let u of[c,g]){let b=`${u},${w}`;if(a.has(b))continue;a.add(b),yield{x:u,y:w}}}break}}}moveStart(o){if(o.button!==0)return;if(o.preventDefault(),o.stopPropagation(),!this.lock)this.moveInfo={globalX:this.position.globalX,globalY:this.position.globalY,clientX:o.clientX,clientY:o.clientY}}moveStop(){if(this.moveInfo)this.moveInfo=void 0,this.position.updateAnchor(),this.pixels.update(),this.updateColors(),P(this.bot)}move(o){if(!this.moveInfo)return;let s=Math.round((o.clientX-this.moveInfo.clientX)/this.position.pixelSize),a=Math.round((o.clientY-this.moveInfo.clientY)/this.position.pixelSize);if(this.moveInfo.globalX!==void 0){if(this.position.globalX=s+this.moveInfo.globalX,this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,this.moveInfo.width-s)}else if(this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,s+this.moveInfo.width);if(this.moveInfo.globalY!==void 0){if(this.position.globalY=a+this.moveInfo.globalY,this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,this.moveInfo.height-a)}else if(this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,a+this.moveInfo.height);this.update(),P(this.bot)}resizeStart(o){if(this.lock||o.button!==0)return;o.preventDefault(),o.stopPropagation(),this.moveInfo={clientX:o.clientX,clientY:o.clientY};let s=o.target;if(s.classList.contains("n"))this.moveInfo.height=this.pixels.height,this.moveInfo.globalY=this.position.globalY;if(s.classList.contains("e"))this.moveInfo.width=this.pixels.width;if(s.classList.contains("s"))this.moveInfo.height=this.pixels.height;if(s.classList.contains("w"))this.moveInfo.width=this.pixels.width,this.moveInfo.globalX=this.position.globalX}export(){let o=document.createElement("a");document.body.append(o),o.href=URL.createObjectURL(new Blob([JSON.stringify(this.toJSON())],{type:"application/json"})),o.download=`${this.pixels.width}x${this.pixels.height}.${$}`,o.click(),URL.revokeObjectURL(o.href),o.href=this.pixels.canvas.toDataURL("image/webp",1),o.download=`${this.pixels.width}x${this.pixels.height}.webp`,o.click(),URL.revokeObjectURL(o.href),o.remove()}}var ko=`/* stylelint-disable declaration-no-important */
...
`;class ao extends Error{name="KGlacerMacroError";constructor(o,s){super(o);s.widget.status=o}}class ro extends ao{name="NoImageError";constructor(o){super("❌ No image is selected",o)}}var U={toggleWidget:{key:"b",shift:!0},minimizeWidget:{key:"m",shift:!0},showWidgetPanel:{key:"s",shift:!0},hideWidgetPanel:{key:"h",shift:!0},toggleOverlay:{key:"v",shift:!0},draw:{key:"enter",shift:!0},addImage:{key:"i",shift:!0},showShortcuts:{key:"/",shift:!0},focusNextImage:{key:"n",shift:!0},focusPreviousImage:{key:"p",shift:!0},openColorPanel:{key:"o",shift:!0},toggleImageLock:{key:"l",shift:!0},clickPaintWhenReady:{key:"r",shift:!0}};function K(o,s){let a=s.key.toLowerCase(),p=o.key.toLowerCase(),c=a==="/"&&(p==="/"||p==="?"||o.code==="Slash")||p===a,g=s.ctrl===!0?o.ctrlKey||o.metaKey:!o.ctrlKey,r=s.ctrl===!0?!0:s.meta===!0?o.metaKey:!o.metaKey;return c&&o.shiftKey===Boolean(s.shift)&&g&&r&&o.altKey===Boolean(s.alt)}function Po(o){if(typeof HTMLElement==="undefined")return!1;if(!(o instanceof HTMLElement))return!1;let s=o.tagName.toLowerCase();return s==="input"||s==="textarea"||o.isContentEditable||o.closest('[contenteditable="true"]')!==null}var Jo=`<button class="wopen-button" aria-label="Toggle widget">
...
`;var No="kglacer-macro:overlay-hidden",To="https://raw.githubusercontent.com/robgallardof/kglacer-macro/refs/heads/main/src/img/logo.svg";class fo extends O{bot;element=document.createElement("div");get status(){return this.$status.innerHTML}set status(o){this.$status.innerHTML=o}get open(){return this.element.classList.contains("wopen")}set open(o){if(o)this.element.classList.add("wopen");else this.element.classList.remove("wopen")}$settings;$status;$shortcuts;$locale;$topbar;$draw;$addImage;$captureTemplate;$toggleOverlay;$strategy;$progressLine;$progressText;$images;$wopenButton;$widgetLogo;activeImageIndex=-1;constructor(o){super();this.bot=o;this.element.classList.add("wwidget"),this.element.innerHTML=Jo,q(this.element),document.body.append(this.element),this.populateElementsWithSelector(this.element,{$wopenButton:".wopen-button",$widgetLogo:".widget-logo",$settings:".wform",$status:".wstatus",$shortcuts:".shortcuts",$locale:".locale",$topbar:".wtopbar",$draw:".draw",$addImage:".add-image",$captureTemplate:".capture-template",$toggleOverlay:".toggle-overlay",$strategy:".strategy",$progressLine:".wprogress div",$progressText:".wprogress span",$images:".images"}),this.$widgetLogo.src=To,this.$wopenButton.addEventListener("click",()=>this.open=!this.open),this.$draw.addEventListener("click",()=>this.bot.draw()),this.$addImage.addEventListener("click",()=>this.addImage()),this.$captureTemplate.addEventListener("click",()=>{this.captureTemplate()}),this.$toggleOverlay.addEventListener("click",()=>{this.toggleOverlay()}),this.$strategy.addEventListener("change",()=>{this.bot.strategy=this.$strategy.value}),this.$locale.value=X(),this.$locale.addEventListener("change",()=>{oo(this.$locale.value),q(this.element);for(let s=0;s<this.bot.images.length;s++)this.bot.images[s].applyLocale();this.refreshOverlayToggleText()}),this.registerEvent(document,"keydown",this.handleKeyboard.bind(this),{passive:!1}),this.update(),this.syncOverlayVisibilityFromStorage(),this.open=!0,console.log("[KGM][Widget] Widget mounted and opened")}addImage(){return console.log("[KGM][Widget] Add image flow started"),this.setDisabled("add-image",!0),this.run(d("taskAddingImage"),async()=>{await this.bot.updateColors();let o=document.createElement("input");o.type="file",o.accept=`image/*,.${$}`,o.click(),await W(o,["change"],["cancel","error"]);let s=o.files?.[0];if(!s)throw new ro(this.bot);console.log("[KGM][Widget] File selected",{name:s.name,size:s.size,type:s.type});let a;if(s.name.endsWith(`.${$}`))a=await R.fromJSON(this.bot,JSON.parse(await s.text()));else{let p=new FileReader;p.readAsDataURL(s),await W(p,["load"],["error"]);let l=await this.compressImageBeforeLoad(p.result),c=new Image;c.src=l,await W(c,["load"],["error"]),a=new R(this.bot,N.fromScreenPosition(this.bot,this.defaultImageScreenPosition()),new G(this.bot,c))}this.bot.images.push(a),console.log("[KGM][Widget] Image instance added",{images:this.bot.images.length}),await this.bot.readMap(),a.updateTasks(),P(this.bot,!0),this.bot.updateTasks(),this.update()},()=>{this.setDisabled("add-image",!1)})}captureTemplate(){return this.setDisabled("capture-template",!0),this.run(d("taskCapturingMapImage"),async()=>{let o=await this.resolveCaptureBounds(),{minGlobalX:s,minGlobalY:a,maxGlobalX:p,maxGlobalY:l}=o,c=document.createElement("canvas");c.width=Math.max(1,p-s+1),c.height=Math.max(1,l-a+1);let g=c.getContext("2d");if(!g)throw new Error("Capture context unavailable");g.imageSmoothingEnabled=!1;let r=Math.floor(s/M),f=Math.floor(a/M),w=Math.floor(p/M),u=Math.floor(l/M),b=(w-r+1)*(u-f+1),n=0;for(let m=r;m<=w;m++)for(let z=f;z<=u;z++){this.status=`⌛ ${d("taskReadingTiles")} [${++n}/${b}]`;let k=await this.loadTileImage(m,z),A=m*M,h=z*M,H=Math.max(s,A),Q=Math.min(p,A+M-1),J=Math.max(a,h),Z=Math.min(l,h+M-1),E=H-A,v=J-h,_=Q-H+1,D=Z-J+1,t=H-s,x=J-a;g.drawImage(k,E,v,_,D,t,x,_,D)}let i=Date.now();await this.downloadCapture(c,"png",i),await this.downloadCapture(c,"webp",i)},()=>{this.setDisabled("capture-template",!1)})}async downloadCapture(o,s,a){let p=s==="webp"?"image/webp":"image/png",l=await new Promise((r,f)=>{o.toBlob((w)=>{if(!w){f(new Error(`Failed to create ${s.toUpperCase()} capture file`));return}r(w)},p)}),c=URL.createObjectURL(l),g=document.createElement("a");g.href=c,g.download=`wplace-capture-${a}.${s}`,g.click(),URL.revokeObjectURL(c)}async loadTileImage(o,s){let a;for(let p=1;p<=3;p++)try{let l=await fetch(`https://backend.wplace.live/files/s0/tiles/${o}/${s}.png`,{cache:"no-store",credentials:"include"});if(!l.ok)throw new Error(`Tile fetch failed (${o}/${s})`);let c=await l.blob(),g=new Image,r=URL.createObjectURL(c);return g.src=r,await W(g,["load"],["error"]),URL.revokeObjectURL(r),g}catch(l){if(a=l,p<3)await new Promise((c)=>setTimeout(c,p*200))}throw a instanceof Error?a:new Error(`Tile fetch failed (${o}/${s})`)}async resolveCaptureBounds(){if(!confirm(`Capture mode:
OK = manual coordinates (tile/pixel)
Cancel = select area on map`))return this.selectCaptureBounds();return this.selectCaptureBoundsFromCoordinates()}selectCaptureBoundsFromCoordinates(){let o=prompt("Top-left corner (tileX,tileY,pixelX,pixelY). Example: 120,340,1,1");if(!o)throw new Error("Capture cancelled");let s=prompt("Bottom-right corner (tileX,tileY,pixelX,pixelY). Example: 121,341,1000,1000");if(!s)throw new Error("Capture cancelled");let a=this.parseTilePixelCoordinate(o),p=this.parseTilePixelCoordinate(s);if(p.tileX<a.tileX||p.tileY<a.tileY||p.tileX===a.tileX&&p.tileY===a.tileY&&(p.pixelX<a.pixelX||p.pixelY<a.pixelY))throw new Error("Invalid coordinate order (top-left -> bottom-right)");let l=a.tileX*M+a.pixelX,c=a.tileY*M+a.pixelY,g=p.tileX*M+p.pixelX,r=p.tileY*M+p.pixelY;return Promise.resolve({minGlobalX:l,minGlobalY:c,maxGlobalX:g,maxGlobalY:r})}parseTilePixelCoordinate(o){let[s,a,p,l]=o.split(",").map((r)=>Number.parseInt(r.trim(),10));if([s,a,p,l].some((r)=>!Number.isFinite(r)))throw new Error("Invalid coordinate format");let c=Math.max(0,Math.min(999,p-1)),g=Math.max(0,Math.min(999,l-1));return{tileX:s,tileY:a,pixelX:c,pixelY:g}}selectCaptureBounds(){return new Promise((o,s)=>{let a=document.createElement("div");a.className="kgm-capture-overlay",a.innerHTML=`<div class="kgm-capture-hint">${d("captureHintSelectArea")}: A → B</div><div class="kgm-capture-box"></div>`;let p=a.querySelector(".kgm-capture-box");document.body.append(a);let l,c,g=()=>{window.removeEventListener("keydown",b,!0),a.removeEventListener("pointermove",w),a.removeEventListener("pointerdown",u),a.remove()},r=(n)=>{let i=Math.min(l.x,n.x),m=Math.min(l.y,n.y),z=Math.abs(l.x-n.x)+1,k=Math.abs(l.y-n.y)+1;return{left:i,top:m,width:z,height:k}},f=(n)=>{let{left:i,top:m,width:z,height:k}=r(n);p.style.left=`${i}px`,p.style.top=`${m}px`,p.style.width=`${z}px`,p.style.height=`${k}px`},w=(n)=>{if(!l)return;f({x:n.clientX,y:n.clientY})},u=(n)=>{if(n.preventDefault(),!l){l={x:n.clientX,y:n.clientY};let H=N.fromScreenPosition(this.bot,l);c={x:H.globalX,y:H.globalY},f(l);return}let i={x:n.clientX,y:n.clientY},m=N.fromScreenPosition(this.bot,i);if(g(),!c){s(new Error("Capture anchor point unavailable"));return}let z=Math.min(c.x,m.globalX),k=Math.min(c.y,m.globalY),A=Math.max(c.x,m.globalX),h=Math.max(c.y,m.globalY);if(A-z<1||h-k<1){s(new Error("Capture area too small"));return}o({minGlobalX:z,minGlobalY:k,maxGlobalX:A,maxGlobalY:h})},b=(n)=>{if(n.key!=="Escape")return;g(),s(new Error("Capture cancelled"))};window.addEventListener("keydown",b,!0),a.addEventListener("pointermove",w),a.addEventListener("pointerdown",u)})}defaultImageScreenPosition(){let o=Math.round(this.element.getBoundingClientRect().width);return{x:Math.max(256,o),y:32}}async compressImageBeforeLoad(o){let s=new Image;if(s.src=o,await W(s,["load"],["error"]),!(s.naturalWidth*s.naturalHeight>3000000||o.length>3000000))return o;let p=document.createElement("canvas");p.width=s.naturalWidth,p.height=s.naturalHeight;let l=p.getContext("2d");if(!l)return o;return l.drawImage(s,0,0),p.toDataURL("image/png")}update(){this.$strategy.value=this.bot.strategy;let o=0,s=0;for(let c=0;c<this.bot.images.length;c++){let g=this.bot.images[c];o+=g.pixels.pixels.length*g.pixels.pixels[0].length,s+=g.tasks.length}let a=Math.max(0,o-s),p=o>0?a/o*100|0:0;this.$progressText.textContent=`${a}/${o} ${p}% ETA: ${s/120|0}h`,this.$progressLine.style.transform=`scaleX(${p/100})`,this.$images.innerHTML="";let l=document.createDocumentFragment();for(let c=0;c<this.bot.images.length;c++){let g=this.bot.images[c],r=document.createElement("div");l.append(r),r.className="image",r.innerHTML=`<button class="preview" title="View preview">
  ...
  </div>`,r.querySelector(".preview").addEventListener("click",()=>{this.activeImageIndex=c,g.openPreviewPanel()}),r.querySelector(".colors").addEventListener("click",()=>{this.activeImageIndex=c,g.openColorPanel()}),r.querySelector(".preview-strategy").addEventListener("click",()=>{this.activeImageIndex=c,g.openPreviewPanel()}),r.querySelector(".download").addEventListener("click",()=>{g.exportImage()}),r.querySelector(".delete").addEventListener("click",()=>{g.destroy()}),r.querySelector(".up").addEventListener("click",()=>{po(this.bot.images,c,c-1),this.update(),P(this.bot)}),r.querySelector(".down").addEventListener("click",()=>{po(this.bot.images,c,c+1),this.update(),P(this.bot)})}this.$images.append(l)}syncOverlayVisibilityFromStorage(){let o=localStorage.getItem(No)==="true";document.body.classList.toggle("overlay-hidden",o),this.refreshOverlayToggleText()}toggleOverlay(o){let s=o??!document.body.classList.contains("overlay-hidden");document.body.classList.toggle("overlay-hidden",s),localStorage.setItem(No,String(s)),this.refreshOverlayToggleText()}refreshOverlayToggleText(){this.$toggleOverlay.textContent=document.body.classList.contains("overlay-hidden")?`${d("toggleOverlay")} (${d("disabled")})`:`${d("toggleOverlay")} (${d("enabled")})`}setDisabled(o,s){this.element.querySelector("."+o).disabled=s}async run(o,s,a,p="..."){console.log("[KGM][Widget] Task started",{status:o});let l=this.status;this.status=`${p} ${o}`;try{let c=await s();return this.status=l,console.log("[KGM][Widget] Task completed",{status:o}),c}catch(c){if(!(c instanceof ao))console.error(c),this.status=`${d("taskErrorPrefix")}: ${o}`;throw console.error("[KGM][Widget] Task failed",{status:o,error:c}),c}finally{await a?.()}}handleKeyboard(o){if(Po(o.target))return;if(K(o,U.toggleWidget)){o.preventDefault(),this.open=!this.open;return}if(K(o,U.showShortcuts)){o.preventDefault(),this.open=!0,this.$shortcuts.open=!0,this.$shortcuts.scrollIntoView({behavior:"smooth",block:"center"}),this.$shortcuts.classList.remove("shortcut-pulse"),requestAnimationFrame(()=>{this.$shortcuts.classList.add("shortcut-pulse")});return}if(K(o,U.toggleOverlay)){o.preventDefault(),this.toggleOverlay();return}if(K(o,U.focusNextImage)){o.preventDefault(),this.focusImageByStep(1);return}if(K(o,U.focusPreviousImage)){o.preventDefault(),this.focusImageByStep(-1);return}if(K(o,U.openColorPanel)){o.preventDefault(),this.openColorPanelForActiveImage();return}if(K(o,U.toggleImageLock)){o.preventDefault(),this.toggleLockForActiveImage();return}if(K(o,U.clickPaintWhenReady)){o.preventDefault(),this.waitAndClickPaintButton();return}if(K(o,U.addImage)&&!this.$addImage.disabled){o.preventDefault(),this.addImage();return}if(K(o,U.draw)&&!this.$draw.disabled)o.preventDefault(),this.bot.draw()}focusImageByStep(o){if(!this.bot.images.length)return;if(this.activeImageIndex<0||this.activeImageIndex>=this.bot.images.length)this.activeImageIndex=o>0?0:this.bot.images.length-1;else this.activeImageIndex=(this.activeImageIndex+o+this.bot.images.length)%this.bot.images.length;this.bot.images[this.activeImageIndex].position.scrollScreenTo()}getActiveImage(){if(!this.bot.images.length)return;if(this.activeImageIndex<0||this.activeImageIndex>=this.bot.images.length)this.activeImageIndex=0;return this.bot.images[this.activeImageIndex]}openColorPanelForActiveImage(){let o=this.getActiveImage();if(!o)return;o.openColorPanel()}toggleLockForActiveImage(){let o=this.getActiveImage();if(!o)return;o.lock=!o.lock,o.update(),P(this.bot)}async waitAndClickPaintButton(){await this.run(d("taskWaitingPaintButton"),async()=>{for(;;){let o=this.findNativePaintButton();if(o&&!o.disabled&&o.ariaDisabled!=="true"){o.click();return}await new Promise((s)=>setTimeout(s,500))}})}findNativePaintButton(){return["button.btn.btn-primary.btn-lg.relative.z-30","button.btn.btn-primary.btn-lg.sm\\:btn-xl.relative.z-30"].flatMap((a)=>Array.from(document.querySelectorAll(a))).find((a)=>/pintar|paint/i.test(a.textContent??""))}}var Eo=2,Uo="[KGM]",Ko="kglacer-macro:access-ok",wo="S0dNLXlZUjhTMW81bEhVemVjS1RFMEhxRVB4OVFkcjgxaEVz",Qo="kgm-access-locked";class Do{unavailableColors=new Set;mapsCache=new Map;me;$stars=[];strategy="SEQUENTIAL";images=[];_widget;get widget(){if(!this._widget)throw new Error("Widget is not initialized yet");return this._widget}markerPixelPositionResolvers=[];lastColor;log(o,s){if(s===void 0)console.log(`${Uo} ${o}`);else console.log(`${Uo} ${o}`,s)}constructor(){this.log("Boot sequence started"),document.body.classList.add(Qo);let o=ho();if(this.log("Save loaded",{hasSave:Boolean(o),imageCount:o?.images.length??0,strategy:o?.strategy}),o){for(let a=0;a<o.images.length;a++){let p=o.images[a];I({x:p.position[0]-1000,y:p.position[1]-1000}),I({x:p.position[0]+1000,y:p.position[1]+1000})}this.strategy=o.strategy}this.registerFetchInterceptor(),this.log("Fetch interceptor registered");let s=document.createElement("style");s.textContent=ko.replace("FAKE_FAVORITE_LOCATIONS",T.length.toString()),document.head.append(s),this.log("Styles injected",{fakeFavoriteLocations:T.length}),(async()=>{this.log("Widget initialization flow started"),await this.ensureAccessKey(),document.body.classList.remove(Qo),this._widget=new fo(this),await this.widget.run(d("taskInitializing"),async()=>{await this.waitForElement("login",".avatar.center-absolute.absolute"),await this.waitForElement("pixel count",".btn.btn-primary.btn-lg.relative.z-30 canvas");let a=await this.waitForElement("canvas",".maplibregl-canvas-container");if(new MutationObserver((p)=>{for(let l=0;l<p.length;l++)if(p[l].removedNodes.length!==0){this.updateStars();break}this.updateImages()}).observe(a,{attributes:!0,childList:!0,subtree:!0}),this.updateStars(),this.log("Stars updated after boot",{stars:this.$stars.length}),await F(500),await this.updateColors(),o)for(let p=0;p<o.images.length;p++){let l=await R.fromJSON(this,o.images[p]);this.images.push(l),l.update()}this.log("Saved images restored",{images:this.images.length}),await this.readMap(),this.updateTasks(),this.widget.setDisabled("draw",!1),this.widget.setDisabled("add-image",!1),this.widget.setDisabled("capture-template",!1),this.log("Initialization completed; controls enabled")})})()}async ensureAccessKey(){if(localStorage.getItem(Ko)===wo)return;await new Promise((o)=>{let s=document.createElement("dialog");s.className="kgm-modal access-dialog",s.innerHTML=`<form method="dialog" class="access-form">
...
</form>`,document.body.append(s),q(s);let a=s.querySelector(".access-input"),p=s.querySelector(".access-error"),l=s.querySelector(".access-locale");l.innerHTML=mo().map((c)=>`<option value="${c}" ${c===X()?"selected":""}>${c.toUpperCase()}</option>`).join(""),l.addEventListener("change",()=>{oo(l.value),q(s)}),s.addEventListener("cancel",(c)=>{c.preventDefault()}),s.querySelector("form").addEventListener("submit",(c)=>{c.preventDefault();let g=atob(wo);if(a.value.trim()!==g){p.textContent=d("invalidAccessKey");return}localStorage.setItem(Ko,wo),s.close(),s.remove(),o()}),s.showModal(),a.focus()})}draw(){this.log("Draw requested",{strategy:this.strategy,images:this.images.length}),this.widget.setDisabled("draw",!0),this.widget.status="",this.mapsCache.clear();let o=document.querySelector(".maplibregl-canvas"),s=(a)=>{if(!a.shiftKey)a.stopPropagation()};return this.widget.run(d("taskDrawing"),async()=>{await this.widget.run(d("taskInitializingDraw"),()=>Promise.all([this.updateColors(),this.readMap()])),globalThis.addEventListener("mousemove",s,!0),o.addEventListener("wheel",s,!0),this.updateTasks();let a=await fetch("https://backend.wplace.live/me",{credentials:"include"}).then((c)=>c.json()),p=Math.floor(a.charges.count);this.log("Charges fetched",{charges:p});let l=0;for(let c=0;c<this.images.length;c++)l+=this.images[c].tasks.length;switch(this.log("Tasks prepared",{tasks:l}),this.strategy){case"ALL":{while(p>0){let c=!0;for(let g=0;g<this.images.length;g++){let r=this.images[g].tasks.shift();if(!r)continue;this.drawTask(r),p--,await F(1),c=!1}if(c)break}break}case"PERCENTAGE":{for(let c=0;c<l&&p>0;c++){let g=1,r;for(let f=0;f<this.images.length;f++){let w=this.images[f],u=1-w.tasks.length/(w.pixels.pixels.length*w.pixels.pixels[0].length);if(u<g)g=u,r=w}this.drawTask(r.tasks.shift()),p--,await F(1)}break}case"SEQUENTIAL":for(let c=0;c<this.images.length;c++){let g=this.images[c];for(let r=g.tasks.shift();r&&p>0;r=g.tasks.shift())this.drawTask(r),p--,await F(1)}}this.widget.update(),this.log("Draw flow finished",{remainingCharges:p})},()=>{globalThis.removeEventListener("mousemove",s,!0),o.removeEventListener("wheel",s,!0),this.widget.setDisabled("draw",!1)})}toJSON(){return{version:Eo,images:this.images.map((o)=>o.toJSON()),strategy:this.strategy}}async updateColors(){this.log("Updating colors palette"),await this.openColors(),this.unavailableColors.clear();for(let o of document.querySelectorAll("button.btn.relative.w-full"))if(o.children.length!==0)this.unavailableColors.add(Math.abs(Number.parseInt(o.id.slice(6))));this.updateImageColors(),this.log("Colors updated",{unavailableColors:this.unavailableColors.size})}moveMap(o){let s=document.querySelector(".maplibregl-canvas"),a=window.innerWidth/2,p=window.innerHeight/2,l=a-o.x,c=p-o.y;function g(r,f,w){s.dispatchEvent(new MouseEvent(r,{bubbles:!0,cancelable:!0,clientX:f,clientY:w,buttons:1}))}g("mousedown",a,p),g("mousemove",l,c),g("mouseup",l,c)}readMap(){this.mapsCache.clear();let o=new Set;for(let a=0;a<this.images.length;a++){let p=this.images[a],{tileX:l,tileY:c}=new N(this,p.position.globalX+p.pixels.pixels[0].length,p.position.globalY+p.pixels.pixels.length);for(let g=p.position.tileX;g<=l;g++)for(let r=p.position.tileY;r<=c;r++)o.add(`${g}/${r}`)}let s=0;return this.log("Reading map tiles",{tileCount:o.size}),this.widget.run(`${d("taskReadingMap")} [0/${o.size}]`,()=>Promise.all([...o].map(async(a)=>{this.mapsCache.set(a,await G.fromJSON(this,{url:`https://backend.wplace.live/files/s0/tiles/${a}.png`,exactColor:!0})),this.widget.status=`⌛ ${d("taskReadingMap")} [${++s}/${o.size}]`})))}waitForUnfocus(){return this.widget.run("UNFOCUS WINDOW",()=>new Promise((o)=>{if(!document.hasFocus())o();window.addEventListener("blur",()=>{setTimeout(o,1)},{once:!0})}),void 0,"\uD83D\uDDB1️")}findAnchorsForScreen(o){let s=0,a=1,p=1/0,l=1/0;for(let r=0;r<this.$stars.length;r++){let{x:f,y:w}=C(this.$stars[r]);if(f<o.x&&w<o.y){let u=o.x-f+(o.y-w);if(u<p)p=u,s=r}else if(f>o.x&&w>o.y){let u=f-o.x+(w-o.y);if(u<l)l=u,a=r}}let c=C(this.$stars[s]),g=B[s];return{anchorScreenPosition:c,anchorWorldPosition:g,pixelSize:(C(this.$stars[a]).x-c.x)/(B[a].x-g.x)}}async openColors(){this.lastColor=void 0,document.querySelector(".flex.gap-2.px-3 > .btn-circle")?.click(),await F(1),document.querySelector(".btn.btn-primary.btn-lg.relative.z-30")?.click(),await F(1);let o=document.querySelector("button.bottom-0");if(o?.innerHTML==='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor" class="size-5"><path d="M480-120 300-300l58-58 122 122 122-122 58 58-180 180ZM358-598l-58-58 180-180 180 180-58 58-122-122-122 122Z"></path></svg><!---->')o.click(),await F(1)}drawTask(o){if(this.lastColor!==o.color){let p=document.getElementById("color-"+o.color);if(!p){this.log("Skipped draw task: color button not found",{color:o.color,tileX:o.position.tileX,tileY:o.position.tileY,x:o.position.x,y:o.position.y});return}p.click(),this.lastColor=o.color,this.log("Color switched for draw task",{color:o.color})}let s=o.position.pixelSize/2,a=o.position.toScreenPosition();if(!Number.isFinite(a.x)||!Number.isFinite(a.y)){this.log("Skipped draw task: invalid screen position",{color:o.color});return}document.documentElement.dispatchEvent(new MouseEvent("mousemove",{bubbles:!0,clientX:a.x+s,clientY:a.y+s,shiftKey:!0})),document.documentElement.dispatchEvent(new KeyboardEvent("keydown",{key:" ",code:"Space",keyCode:32,which:32,bubbles:!0,cancelable:!0})),document.documentElement.dispatchEvent(new KeyboardEvent("keyup",{key:" ",code:"Space",keyCode:32,which:32,bubbles:!0,cancelable:!0}))}registerFetchInterceptor(){let o=globalThis.fetch,s=/https:\/\/backend.wplace.live\/s\d+\/pixel\/(-?\d+)\/(-?\d+)\?x=(-?\d+)&y=(-?\d+)/;globalThis.fetch=async(a,p)=>{let l=await o(a,p),c=l.clone(),g="";if(typeof a=="string")g=a;else if(a instanceof Request)g=a.url;else if(a instanceof URL)g=a.href;if(l.url==="https://backend.wplace.live/me")this.me=await c.json(),this.me.favoriteLocations.unshift(...T),this.me.maxFavoriteLocations=1/0,l.json=()=>Promise.resolve(this.me),this.log("Patched /me response with favorite locations",{totalFavorites:this.me.favoriteLocations.length});let r=s.exec(g);if(r){for(let f=0;f<this.markerPixelPositionResolvers.length;f++)this.markerPixelPositionResolvers[f](new N(this,+r[1],+r[2],+r[3],+r[4]));this.markerPixelPositionResolvers.length=0,this.log("Resolved marker pixel position from network event")}return l}}async closeAll(){for(let o of document.querySelectorAll("button"))if(o.innerHTML==="✕"||o.innerHTML==='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor" class="size-4"><path d="m256-200-56-56 224-224-224-224 56-56 224 224 224-224 56 56-224 224 224 224-56 56-224-224-224 224Z"></path></svg><!---->')o.click(),await F(1)}waitForElement(o,s){return this.log("Waiting for element",{name:o,selector:s}),this.widget.run(`${d("taskWaitingFor")} ${o}`,()=>{return new Promise((a)=>{let p=document.querySelector(s);if(p){a(p);return}let l=new MutationObserver(()=>{let c=document.querySelector(s);if(c)l.disconnect(),a(c)});l.observe(document.documentElement,{childList:!0,subtree:!0})})})}updateStars(){this.$stars=[...document.querySelectorAll(".text-yellow-400.cursor-pointer.z-10.maplibregl-marker.maplibregl-marker-anchor-center")].slice(0,T.length),this.log("Star cache updated",{stars:this.$stars.length})}updateImages(){for(let o=0;o<this.images.length;o++)this.images[o].position.updateAnchor(),this.images[o].update()}updateTasks(){for(let o=0;o<this.images.length;o++)this.images[o].updateTasks()}updateImageColors(){for(let o=0;o<this.images.length;o++)this.images[o].updateColors()}}globalThis.kglacerMacro=new Do;globalThis.kgm=globalThis.kglacerMacro;globalThis.wbot=globalThis.kglacerMacro;
