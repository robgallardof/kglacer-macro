...
`;class e{dialog;options;ignoreNextBackdropClick=!1;constructor(o,p={}){this.dialog=o;this.options=p;this.dialog.classList.add("modal-widget"),(this.options.closeSelector?this.dialog.querySelector(this.options.closeSelector):void 0)?.addEventListener("click",()=>{this.dialog.close()}),this.dialog.addEventListener("click",(f)=>{if(this.ignoreNextBackdropClick){this.ignoreNextBackdropClick=!1;return}if(f.target===this.dialog)this.dialog.close()})}open(){if(!this.dialog.open)this.dialog.style.position="fixed",this.dialog.style.left="",this.dialog.style.top="",this.dialog.style.margin="auto",this.dialog.showModal();if(this.options.focusSelector)this.dialog.querySelector(this.options.focusSelector)?.focus({preventScroll:!0})}markBackdropClickIgnored(){this.ignoreNextBackdropClick=!0}}class r{bot;image;width;brightness;exactColor;static async fromJSON(o,p){let g=new Image;return g.src=p.url.startsWith("http")?await fetch(p.url,{cache:"no-store"}).then((f)=>f.blob()).then((f)=>URL.createObjectURL(f)):p.url,await m(g,["load"],["error"]),new r(o,g,p.width,p.brightness,p.exactColor)}canvas=document.createElement("canvas");context=this.canvas.getContext("2d");pixels;colors=new Map;resolution;get height(){return this.width/this.resolution|0}set height(o){this.width=o*this.resolution|0}constructor(o,p,g=p.naturalWidth,f=0,w=!1){this.bot=o;this.image=p;this.width=g;this.brightness=f;this.exactColor=w;if(w)this.resolution=1,this.width=1000;else this.resolution=this.image.naturalWidth/this.image.naturalHeight;this.update()}update(){this.canvas.width=this.width,this.canvas.height=this.height,this.colors.clear();let o=new Map;for(let g=1;g<64;g++)if(this.exactColor||!this.bot.unavailableColors.has(g))o.set(C[g],[g,g]);this.context.imageSmoothingEnabled=!1,this.context.imageSmoothingQuality="low",this.context.drawImage(this.image,0,0,this.canvas.width,this.canvas.height),this.pixels=Array.from({length:this.canvas.height},()=>new Array(this.canvas.width));let p=this.context.getImageData(0,0,this.canvas.width,this.canvas.height).data;for(let g=0;g<this.canvas.height;g++)for(let f=0;f<this.canvas.width;f++){let w=(g*this.canvas.width+f)*4,l=p[w],z=p[w+1],b=p[w+2],c=p[w+3],M=`${l},${z},${b}`;if(this.exactColor){this.pixels[g][f]=c<100?0:C.indexOf(M);continue}let H,A;if(c<100)H=A=0;else if(o.has(M))[H,A]=o.get(M);else{let K=1/0,B=1/0;for(let a=0;a<V.length;a++){let U=V[a],X=J0(u0(l,z,b),U,this.brightness);if(!this.bot.unavailableColors.has(a)&&X<K)K=X,H=a;if(X<B)B=X,A=a}o.set(M,[H,A])}if(H!==0)this.context.fillStyle=`oklab(${V[H][0]*100}% ${V[H][1]} ${V[H][2]})`,this.context.fillRect(f,g,1,1);this.pixels[g][f]=H;let u=this.colors.get(A);if(u)u.amount++;else this.colors.set(A,{color:H,amount:1,realColor:A})}}toJSON(){let o=document.createElement("canvas");return o.width=this.image.naturalWidth,o.height=this.image.naturalHeight,o.getContext("2d").drawImage(this.image,0,0),{url:o.toDataURL("image/webp",1),width:this.width,brightness:this.brightness,exactColor:this.exactColor}}}var $="kglacer-macro-settings",U0=["kglacermacro","wbot"],n="kgm";function T0(){let o=[$,...U0];for(let p=0;p<o.length;p++){let g=o[p],f=localStorage.getItem(g);if(!f)continue;return{json:f,key:g}}return}function N0(){let o=T0();if(!o)return;let p;try{if(p=JSON.parse(o.json),typeof p!=="object")throw new Error("NOT VALID SAVE");if(p.version===1){let g=p;p.images=g.widget.images,p.strategy=g.widget.strategy,delete g.widget}if(o.key!==$)localStorage.setItem($,o.json)}catch{localStorage.removeItem(o.key),p=void 0}return p}var G0;function Y(o,p=!1){if(clearTimeout(G0),p)localStorage.setItem($,JSON.stringify(o));else G0=setTimeout(()=>{localStorage.setItem($,JSON.stringify(o))},600)}var j=1000,O0=2048,h=j*O0,P=[],d=[],_0=Date.now();function y(o){P.push(o),d.push({id:_0++,latitude:(2*Math.atan(Math.exp(-(o.y/h*(2*Math.PI)-Math.PI)))-Math.PI/2)*180/Math.PI,longitude:(o.x/h*(2*Math.PI)-Math.PI)*180/Math.PI,name:"KGLACER_MACRO_FAVORITE"})}y({x:h/3|0,y:h/3|0});y({x:h/3*2|0,y:h/3*2|0});function R(o){let[p,g]=o.style.transform.slice(32,-31).split(", ").map((f)=>Number.parseFloat(f));return{x:p,y:g}}class D{bot;static fromJSON(o,p){return new D(o,...p)}static fromScreenPosition(o,p){let{anchorScreenPosition:g,pixelSize:f,anchorWorldPosition:w}=o.findAnchorsForScreen(p);return new D(o,w.x+(p.x-g.x)/f|0,w.y+(p.y-g.y)/f|0)}globalX=0;globalY=0;get tileX(){return this.globalX/j|0}set tileX(o){this.globalX=o*j+this.x}get tileY(){return this.globalY/j|0}set tileY(o){this.globalY=o*j+this.y}get x(){return this.globalX%j}set x(o){this.globalX=this.tileX*j+o}get y(){return this.globalY%j}set y(o){this.globalY=this.tileY*j+o}anchor1Index;anchor2Index;get pixelSize(){return(R(this.bot.$stars[this.anchor2Index]).x-R(this.bot.$stars[this.anchor1Index]).x)/(P[this.anchor2Index].x-P[this.anchor1Index].x)}constructor(o,p,g,f,w){this.bot=o;if(f===void 0||w===void 0)this.globalX=p,this.globalY=g;else this.globalX=p*j+f,this.globalY=g*j+w;this.updateAnchor()}updateAnchor(){this.anchor1Index=0,this.anchor2Index=1;let o=1/0,p=1/0;for(let g=0;g<P.length;g++){let{x:f,y:w}=P[g];if(f<this.globalX&&w<this.globalY){let l=this.globalX-f+(this.globalY-w);if(l<o)o=l,this.anchor1Index=g}else if(f>this.globalX&&w>this.globalY){let l=f-this.globalX+(w-this.globalY);if(l<p)p=l,this.anchor2Index=g}}}toScreenPosition(){let o=P[this.anchor1Index],p=R(this.bot.$stars[this.anchor1Index]);return{x:(this.globalX-o.x)*this.pixelSize+p.x,y:(this.globalY-o.y)*this.pixelSize+p.y}}getMapColor(){return this.bot.mapsCache.get(this.tileX+"/"+this.tileY).pixels[this.y][this.x]}scrollScreenTo(){let{x:o,y:p}=this.toScreenPosition();this.bot.moveMap({x:o-window.innerWidth/3,y:p-window.innerHeight/3})}clone(){return new D(this.bot,this.tileX,this.tileY,this.x,this.y)}toJSON(){return[this.globalX,this.globalY]}}var S0="https://raw.githubusercontent.com/robgallardof/kglacer-macro/refs/heads/main/src/img/logo.svg",F=56,B0;((Q)=>{Q.RANDOM="RANDOM";Q.HUMANIZED="HUMANIZED";Q.HUMAN_SOFT_DITHER="HUMAN_SOFT_DITHER";Q.HUMAN_PATCHY="HUMAN_PATCHY";Q.HUMAN_SWEEP_ARCS="HUMAN_SWEEP_ARCS";Q.HUMAN_MICRO_CORRECTIONS="HUMAN_MICRO_CORRECTIONS";Q.HUMAN_JITTER_FILL="HUMAN_JITTER_FILL";Q.HUMAN_CORNER_BIAS="HUMAN_CORNER_BIAS";Q.HUMAN_LONG_STROKES="HUMAN_LONG_STROKES";Q.HUMAN_TAP_CLUSTERS="HUMAN_TAP_CLUSTERS";Q.HUMAN_MESSY_SPIRAL="HUMAN_MESSY_SPIRAL";Q.HUMAN_DRUNK_WALK="HUMAN_DRUNK_WALK";Q.HUMAN_NOISE_CLOUD="HUMAN_NOISE_CLOUD";Q.HUMAN_PATCH_JUMP="HUMAN_PATCH_JUMP";Q.ZIGZAG="ZIGZAG";Q.BRUSH_STROKES="BRUSH_STROKES";Q.DIAGONAL_BRUSH="DIAGONAL_BRUSH";Q.DOWN="DOWN";Q.UP="UP";Q.LEFT="LEFT";Q.RIGHT="RIGHT";Q.SPIRAL_FROM_CENTER="SPIRAL_FROM_CENTER";Q.SPIRAL_TO_CENTER="SPIRAL_TO_CENTER";Q.SCRIBBLE="SCRIBBLE";Q.CROSSHATCH="CROSSHATCH";Q.WAVE_SWEEP="WAVE_SWEEP";Q.SCATTERED_LINES="SCATTERED_LINES";Q.CONTOUR_JITTER="CONTOUR_JITTER";Q.SPIRAL_WOBBLE="SPIRAL_WOBBLE";Q.CLUSTER_BURSTS="CLUSTER_BURSTS";Q.ORBITAL="ORBITAL";Q.FLOW_FIELD="FLOW_FIELD";Q.EDGE_IN="EDGE_IN"})(B0||={});class T extends i{bot;position;pixels;strategy;opacity;drawTransparentPixels;drawColorsInOrder;colors;lock;static async fromJSON(o,p){return new T(o,D.fromJSON(o,p.position),await r.fromJSON(o,p.pixels),p.strategy,p.opacity,p.drawTransparentPixels,p.drawColorsInOrder,p.colors,p.lock)}element=document.createElement("div");tasks=[];moveInfo;$brightness;$canvas;$colors;$colorsDialog;$colorsDialogList;$colorSearch;$openColors;$openPreview;$delete;$drawColorsInOrder;$drawTransparent;$export;$lock;$opacity;$progressLine;$progressText;$previewDialog;$previewDialogList;$resetSize;$resetSizeSpan;$settings;$strategy;$topbar;$wrapper;colorDialogDragState;colorModal;previewModal;logoPreviewMask;logoPreviewImage;previewSequenceCache=new Map;previewAnimations=new WeakMap;settingsPinTimeout;constructor(o,p,g,f="SPIRAL_FROM_CENTER",w=50,l=!1,z=!1,b=[],c=!1){super();this.bot=o;this.position=p;this.pixels=g;this.strategy=f;this.opacity=w;this.drawTransparentPixels=l;this.drawColorsInOrder=z;this.colors=b;this.lock=c;this.element.innerHTML=Q0,this.element.classList.add("wimage"),s(this.element),document.body.append(this.element),this.populateElementsWithSelector(this.element,{$brightness:".brightness",$colors:".colors",$colorsDialog:".colors-dialog",$colorsDialogList:".colors-dialog-list",$colorSearch:".color-search",$openColors:".open-colors",$openPreview:".open-preview",$delete:".delete",$drawColorsInOrder:".draw-colors-in-order",$drawTransparent:".draw-transparent",$export:".export",$lock:".lock",$opacity:".opacity",$progressLine:".wprogress div",$progressText:".wprogress span",$previewDialog:".preview-dialog",$previewDialogList:".preview-dialog-list",$resetSize:".reset-size",$settings:".wform",$strategy:".strategy",$topbar:".wtopbar",$wrapper:".wrapper"}),this.$resetSizeSpan=this.$resetSize.querySelector("span"),this.$canvas=this.pixels.canvas,this.$wrapper.prepend(this.pixels.canvas),document.body.append(this.$colorsDialog,this.$previewDialog),this.colorModal=new e(this.$colorsDialog,{closeSelector:".close-colors",focusSelector:".color-search"}),this.previewModal=new e(this.$previewDialog,{closeSelector:".close-preview"}),this.registerEvent(this.$strategy,"change",()=>{this.strategy=this.$strategy.value,Y(this.bot)}),this.registerEvent(this.$opacity,"input",()=>{this.opacity=this.$opacity.valueAsNumber,this.$opacity.style.setProperty("--val",this.opacity+"%"),this.update(),Y(this.bot)}),this.$opacity.style.setProperty("--val",this.opacity+"%");let M;this.registerEvent(this.$brightness,"change",()=>{clearTimeout(M),M=setTimeout(()=>{this.pixels.brightness=this.$brightness.valueAsNumber,this.pixels.update(),this.updateColors(),this.update(),Y(this.bot)},1000)}),this.registerEvent(this.$resetSize,"click",()=>{this.pixels.width=this.pixels.image.naturalWidth,this.pixels.update(),this.updateColors(),this.update(),Y(this.bot)}),this.registerEvent(this.$drawTransparent,"click",()=>{this.drawTransparentPixels=this.$drawTransparent.checked,Y(this.bot)}),this.registerEvent(this.$drawColorsInOrder,"click",()=>{this.drawColorsInOrder=this.$drawColorsInOrder.checked,Y(this.bot)}),this.registerEvent(this.$lock,"click",()=>{this.lock=!this.lock,this.update(),Y(this.bot)}),this.registerEvent(this.$delete,"click",this.destroy.bind(this)),this.registerEvent(this.$openColors,"click",()=>{this.openColorPanel()}),this.registerEvent(this.$openPreview,"click",()=>{this.openPreviewPanel()}),this.registerEvent(this.$colorsDialog.querySelector(".colors-dialog-head"),"pointerdown",this.startColorDialogDrag.bind(this)),this.registerEvent(document,"pointermove",this.moveColorDialog.bind(this),{passive:!1}),this.registerEvent(document,"pointerup",this.stopColorDialogDrag.bind(this)),this.registerEvent(document,"pointercancel",this.stopColorDialogDrag.bind(this)),this.registerEvent(this.$colorSearch,"input",()=>{this.updateColors()}),this.registerEvent(this.$export,"click",this.export.bind(this)),this.registerEvent(this.$topbar,"mousedown",this.moveStart.bind(this)),this.registerEvent(this.$canvas,"mousedown",this.moveStart.bind(this)),this.registerEvent(document,"mouseup",this.moveStop.bind(this)),this.registerEvent(document,"mousemove",this.move.bind(this));for(let H of this.element.querySelectorAll(".resize"))this.registerEvent(H,"mousedown",this.resizeStart.bind(this));this.update(),this.updateColors()}toJSON(){return{pixels:this.pixels.toJSON(),position:this.position.toJSON(),strategy:this.strategy,opacity:this.opacity,drawTransparentPixels:this.drawTransparentPixels,drawColorsInOrder:this.drawColorsInOrder,colors:this.colors,lock:this.lock}}updateTasks(){this.tasks.length=0;let o=this.position.clone(),p=new Set,g=new Map;for(let f=0;f<this.colors.length;f++){let w=this.colors[f];if(w.disabled)p.add(w.realColor);g.set(w.realColor,f)}for(let{x:f,y:w}of this.strategyPositionIterator()){let l=this.pixels.pixels[w][f];if(p.has(l))continue;o.globalX=this.position.globalX+f,o.globalY=this.position.globalY+w;let z=o.getMapColor();if(l!==z&&(this.drawTransparentPixels||l!==0))this.tasks.push({position:o.clone(),color:l})}if(this.drawColorsInOrder)this.tasks.sort((f,w)=>(g.get(f.color)??0)-(g.get(w.color)??0));this.update(),this.bot.widget.update()}update(){let{x:o,y:p}=this.position.toScreenPosition(),g=Math.round(this.position.pixelSize*this.pixels.width),f=Math.round(this.position.pixelSize*this.pixels.height);this.element.style.transform=`translate3d(${Math.round(o)}px, ${Math.round(p)}px, 0)`,this.element.style.width=`${g}px`,this.element.style.height=`${f}px`,this.$canvas.style.opacity=`${this.opacity}%`,this.element.classList.remove("hidden"),this.$resetSizeSpan.textContent=this.pixels.width.toString(),this.$brightness.valueAsNumber=this.pixels.brightness,this.$strategy.value=this.strategy,this.$opacity.valueAsNumber=this.opacity,this.$drawTransparent.checked=this.drawTransparentPixels,this.$drawColorsInOrder.checked=this.drawColorsInOrder;let w=this.pixels.pixels.length*this.pixels.pixels[0].length,l=w-this.tasks.length,z=l/w*100|0;this.$progressText.textContent=`${l}/${w} ${z}% ETA: ${this.tasks.length/120|0}h`,this.$progressLine.style.transform=`scaleX(${z}%)`,this.$wrapper.classList[this.lock?"add":"remove"]("no-pointer-events"),this.$lock.classList[this.lock?"add":"remove"]("locked")}destroy(){super.destroy(),this.element.remove(),this.$colorsDialog.remove(),this.$previewDialog.remove(),f0(this.bot.images,this),this.bot.widget.update(),Y(this.bot)}openColorPanel(){this.colorModal.open()}openPreviewPanel(){this.previewModal.open(),this.renderStrategyPreviewSamples()}openSettingsPanel(){this.position.scrollScreenTo(),this.element.classList.add("settings-pinned"),this.$settings.scrollIntoView({behavior:"smooth",block:"nearest"}),clearTimeout(this.settingsPinTimeout),this.settingsPinTimeout=setTimeout(()=>{this.element.classList.remove("settings-pinned")},6000)}startColorDialogDrag(o){if(o.button!==0)return;if(o.target?.closest("button,input,select,textarea,a,label"))return;let g=this.$colorsDialog.getBoundingClientRect();this.colorDialogDragState={pointerId:o.pointerId,offsetX:o.clientX-g.left,offsetY:o.clientY-g.top,moved:!1},o.preventDefault()}moveColorDialog(o){if(!this.colorDialogDragState)return;if(o.pointerId!==this.colorDialogDragState.pointerId)return;let p=this.$colorsDialog.getBoundingClientRect(),g=Math.max(8,window.innerWidth-p.width-8),f=Math.max(8,window.innerHeight-p.height-8),w=Math.min(g,Math.max(8,o.clientX-this.colorDialogDragState.offsetX)),l=Math.min(f,Math.max(8,o.clientY-this.colorDialogDragState.offsetY));if(!this.colorDialogDragState.moved&&(Math.abs(o.movementX)>0||Math.abs(o.movementY)>0))this.colorDialogDragState.moved=!0;this.$colorsDialog.style.left=`${Math.round(w)}px`,this.$colorsDialog.style.top=`${Math.round(l)}px`,o.preventDefault()}stopColorDialogDrag(o){if(!this.colorDialogDragState)return;if(o.pointerId!==this.colorDialogDragState.pointerId)return;if(this.colorDialogDragState.moved)this.colorModal.markBackdropClickIgnored();this.colorDialogDragState=void 0}renderStrategyPreviewSamples(){let o=this.$strategy.value,p=Object.values(B0).sort((f,w)=>{if(f===o)return-1;if(w===o)return 1;return 0});this.$previewDialogList.innerHTML="";let g=document.createDocumentFragment();for(let f=0;f<p.length;f++){let w=p[f],l=document.createElement("article");l.className="preview-card";let z=document.createElement("strong");z.textContent=this.getStrategyLabel(w);let b=document.createElement("canvas");b.className="preview-canvas",b.width=156,b.height=156,this.paintStrategyPreview(b,w,w===o),l.classList.toggle("active",w===o),l.addEventListener("click",()=>{this.$strategy.value=w,this.strategy=w,Y(this.bot),this.renderStrategyPreviewSamples()}),l.append(z,b),g.append(l)}this.$previewDialogList.append(g)}getStrategyLabel(o){switch(o){case"RANDOM":return J("random");case"HUMANIZED":return J("humanized");case"HUMAN_SOFT_DITHER":return J("humanSoftDither");case"HUMAN_PATCHY":return J("humanPatchy");case"HUMAN_SWEEP_ARCS":return J("humanSweepArcs");case"HUMAN_MICRO_CORRECTIONS":return J("humanMicroCorrections");case"HUMAN_JITTER_FILL":return J("humanJitterFill");case"HUMAN_CORNER_BIAS":return J("humanCornerBias");case"HUMAN_LONG_STROKES":return J("humanLongStrokes");case"HUMAN_TAP_CLUSTERS":return J("humanTapClusters");case"HUMAN_MESSY_SPIRAL":return J("humanMessySpiral");case"HUMAN_DRUNK_WALK":return J("humanDrunkWalk");case"HUMAN_NOISE_CLOUD":return J("humanNoiseCloud");case"HUMAN_PATCH_JUMP":return J("humanPatchJump");case"ZIGZAG":return J("zigzag");case"BRUSH_STROKES":return J("brushStrokes");case"DIAGONAL_BRUSH":return J("diagonalBrush");case"DOWN":return J("down");case"UP":return J("up");case"LEFT":return J("left");case"RIGHT":return J("right");case"SPIRAL_FROM_CENTER":return J("spiralOut");case"SPIRAL_TO_CENTER":return J("spiralIn");case"SCRIBBLE":return J("scribble");case"CROSSHATCH":return J("crosshatch");case"WAVE_SWEEP":return J("waveSweep");case"SCATTERED_LINES":return J("scatteredLines");case"CONTOUR_JITTER":return J("contourJitter");case"SPIRAL_WOBBLE":return J("spiralWobble");case"CLUSTER_BURSTS":return J("clusterBursts");case"ORBITAL":return J("orbital");case"FLOW_FIELD":return J("flowField");case"EDGE_IN":return J("edgeIn");default:return o}}paintStrategyPreview(o,p,g=!1){let f=o.getContext("2d");if(!f)return;f.fillStyle="#0f1526",f.fillRect(0,0,o.width,o.height);let w=this.getLogoPreviewMask(),l=this.previewSequenceCache.get(p)??this.computePreviewSequence(p,w);if(!this.previewSequenceCache.has(p))this.previewSequenceCache.set(p,l);let z=o.width/F,b=this.previewAnimations.get(o);if(b)cancelAnimationFrame(b);let c=(u)=>{f.fillStyle="#0f1526",f.fillRect(0,0,o.width,o.height),this.paintLogoGhost(f,z,w);for(let K=0;K<Math.min(u,l.length);K++){let B=l[K],a=K/Math.max(1,l.length-1);f.fillStyle=`hsl(${220-a*110} 90% ${43+a*18}%)`,f.fillRect(B.x*z,B.y*z,Math.max(1,z),Math.max(1,z))}};if(!g){c(l.length);return}let M=performance.now(),H=Math.min(2400,Math.max(700,l.length*5)),A=(u)=>{let K=u-M,B=Math.min(1,K/H);c(Math.floor(l.length*B));let a=B>=1?requestAnimationFrame(()=>{c(l.length)}):requestAnimationFrame(A);this.previewAnimations.set(o,a)};A(M)}paintLogoGhost(o,p,g){if(this.logoPreviewImage){o.save(),o.globalAlpha=0.22,o.drawImage(this.logoPreviewImage,0,0,F*p,F*p),o.restore();return}o.fillStyle="rgb(115 132 190 / 28%)";for(let f=0;f<g.length;f++){let w=g[f];o.fillRect(w.x*p,w.y*p,Math.max(1,p),Math.max(1,p))}}getLogoPreviewMask(){if(this.logoPreviewMask)return this.logoPreviewMask;this.logoPreviewMask=this.fallbackPreviewMask();let o=new Image;return o.src=S0,o.decode().then(()=>{this.logoPreviewImage=o;let p=document.createElement("canvas");p.width=F,p.height=F;let g=p.getContext("2d");if(!g)return;if(g.clearRect(0,0,p.width,p.height),g.drawImage(o,0,0,p.width,p.height),this.logoPreviewMask=this.alphaMaskFromCanvas(g,p.width,p.height),this.$previewDialog.open)this.renderStrategyPreviewSamples()}).catch(()=>{return}),this.logoPreviewMask}alphaMaskFromCanvas(o,p,g){let f=o.getImageData(0,0,p,g).data,w=[];for(let l=0;l<g;l++)for(let z=0;z<p;z++)if((f[(l*p+z)*4+3]??0)>24)w.push({x:z,y:l});return w.length?w:this.fallbackPreviewMask()}fallbackPreviewMask(){let o=[],p=F/2,g=F/2,f=Math.max(4,F/2.5);for(let w=0;w<F;w++)for(let l=0;l<F;l++)if((l-p)**2+(w-g)**2<=f**2)o.push({x:l,y:w});return o}applyLocale(){if(s(this.element),this.updateColors(),this.previewSequenceCache.clear(),this.$previewDialog.open)this.renderStrategyPreviewSamples()}computePreviewSequence(o,p){let g=this.strategy;this.strategy=o;let f=[...this.strategyPositionIterator(F)];this.strategy=g;let w=new Set(p.map(({x:l,y:z})=>`${l}:${z}`));return f.filter(({x:l,y:z})=>{return w.has(`${l}:${z}`)})}colorHex(o){let p=C[o]??"0,0,0",[g=0,f=0,w=0]=p.split(",").map((l)=>Number.parseInt(l,10));return`#${[g,f,w].map((l)=>l.toString(16).padStart(2,"0")).join("")}`}colorKeywords(o){let p=C[o]??"0,0,0",[g=0,f=0,w=0]=p.split(",").map((c)=>Number.parseInt(c,10)),l=Math.max(g,f,w),z=Math.min(g,f,w);if(l-z<15)return["gray","grey","gris","neutral","neutro"];if(g>f+30&&g>w+30)return["red","rojo"];if(f>g+30&&f>w+30)return["green","verde"];if(w>g+30&&w>f+30)return["blue","azul"];if(g>170&&f>120&&w<130)return["orange","naranja"];if(g>170&&f>110&&w>140)return["pink","rosa"];if(g>120&&f<100&&w>120)return["purple","violet","morado"];if(g>130&&f>130&&w<90)return["yellow","amarillo"];return["brown","cafe","marron"]}updateColors(){this.$colors.innerHTML="",this.$colorsDialogList.innerHTML="";let o=this.pixels.pixels.length*this.pixels.pixels[0].length,p=100/this.pixels.colors.size,g=document.createElement("div");g.className="colors-track",g.setAttribute("aria-label",J("overlayColors")),this.$colorsDialogList.setAttribute("aria-label",J("colorPanelResults"));let f=this.$colorSearch.value.trim().toLowerCase();if(this.colors.length!==this.pixels.colors.size||this.colors.some((l)=>!this.pixels.colors.has(l.realColor)))this.colors=this.pixels.colors.values().toArray().sort((l,z)=>z.amount-l.amount).map((l)=>({realColor:l.realColor,disabled:!1})),Y(this.bot);let w=0;for(let l=0;l<this.colors.length;l++){let z=this.colors[l],b=this.pixels.colors.get(z.realColor),c=!1,M=!1,H=b.realColor!==b.color,A=b.amount/o*100,u=this.colorHex(b.realColor),K=this.colorKeywords(b.realColor),B=()=>{if(c)return;z.disabled=z.disabled?void 0:!0,a.classList.toggle("color-disabled"),U.classList.toggle("disabled",Boolean(z.disabled));let G=U.querySelector(".state");if(G)G.textContent=z.disabled?J("disabled"):J("enabled");Y(this.bot)},a=document.createElement("button");if(a.setAttribute("aria-label",`${J("overlayColors")} #${l+1}: ${u.toUpperCase()}`),a.title=`${A.toFixed(1)}%`,a.innerHTML=`<span class="order-index">#${l+1}</span>`,z.disabled)a.classList.add("color-disabled");if(!H)a.style.background=x(b.realColor);else{a.classList.add("substitution"),a.style.setProperty("--wreal-color",x(b.realColor)),a.style.setProperty("--wsubstitution-color",x(b.color));let G=document.createElement("button"),N=document.createElement("button");G.textContent=J("buy"),N.textContent="✓",G.classList.add("buy"),N.classList.add("disable"),G.addEventListener("click",()=>{document.getElementById("color-"+b.realColor)?.click()}),N.addEventListener("click",B),a.append(G),a.append(N)}a.style.left=w+"%",a.style.width=A+"%",w+=A,a.style.setProperty("--wleft",p*l+"%"),a.style.setProperty("--wwidth",p+"%"),g.append(a);let U=document.createElement("button");if(U.className=`color-chip ${z.disabled?"disabled":""}`,U.draggable=!0,U.innerHTML=`<span class="order-index">#${l+1}</span>
<span class="drag" title="${J("up")} / ${J("down")}">⋮⋮</span>
<span class="swatch"></span>
<span class="meta">
  <span class="coverage">${A.toFixed(1)}% · ${u.toUpperCase()}</span>
  <span class="state">${z.disabled?J("disabled"):J("enabled")}</span>
</span>
<span class="premium ${H?"on":""}">${H?J("premium"):""}</span>`,U.querySelector(".swatch").style.setProperty("--swatch-color",x(b.realColor)),U.addEventListener("click",()=>{if(M){M=!1;return}B()}),U.addEventListener("dragstart",(G)=>{M=!0,U.classList.add("dragging"),G.dataTransfer?.setData("text/plain",String(l)),G.dataTransfer.effectAllowed="move"}),U.addEventListener("dragend",()=>{U.classList.remove("dragging")}),U.addEventListener("dragover",(G)=>{G.preventDefault(),U.classList.add("drag-target")}),U.addEventListener("dragleave",()=>{U.classList.remove("drag-target")}),U.addEventListener("drop",(G)=>{G.preventDefault(),U.classList.remove("drag-target");let N=Number.parseInt(G.dataTransfer?.getData("text/plain")??"-1",10);if(N<0||N===l||N>=this.colors.length)return;this.colors.splice(l,0,...this.colors.splice(N,1)),Y(this.bot),this.updateColors()}),H){let G=document.createElement("button");G.textContent=J("buy"),G.className="buy-chip",G.addEventListener("click",(N)=>{N.stopPropagation(),document.getElementById("color-"+b.realColor)?.click()}),U.append(G)}let X=`${u} ${K.join(" ")} ${b.realColor} ${C[b.realColor]}`;if(!f||X.toLowerCase().includes(f))this.$colorsDialogList.append(U);let O=(G)=>{let N=l,_=a.getBoundingClientRect().width,S=(v)=>{if(N=Math.min(this.colors.length-1,Math.max(0,Math.round(l+(v.clientX-G.clientX)/_))),N!==l)c=!0;let W=0;for(let I of g.children){if(I===a)continue;if(W===N)W++;I.style.setProperty("--wleft",p*W+"%"),W++}a.style.setProperty("--wleft",p*N+"%")};document.addEventListener("mousemove",S),document.addEventListener("mouseup",()=>{if(document.removeEventListener("mousemove",S),N!==l)this.colors.splice(N,0,...this.colors.splice(l,1));Y(this.bot),a.removeEventListener("mousedown",O),setTimeout(()=>{this.updateColors()},200)},{once:!0})};if(a.addEventListener("mousedown",O),!H)a.addEventListener("click",B)}this.$colors.append(g)}*strategyPositionIterator(o){let p=o??this.pixels.pixels[0].length,g=o??this.pixels.pixels.length;switch(this.strategy){case"DOWN":{for(let f=0;f<g;f++)for(let w=0;w<p;w++)yield{x:w,y:f};break}case"UP":{for(let f=g-1;f>=0;f--)for(let w=0;w<p;w++)yield{x:w,y:f};break}case"LEFT":{for(let f=0;f<p;f++)for(let w=0;w<g;w++)yield{x:f,y:w};break}case"RIGHT":{for(let f=p-1;f>=0;f--)for(let w=0;w<g;w++)yield{x:f,y:w};break}case"RANDOM":{let f=[];for(let w=0;w<g;w++)for(let l=0;l<p;l++)f.push({x:l,y:w});for(let w=f.length-1;w>=0;w--){let l=Math.floor(Math.random()*(w+1)),z=f[w];f[w]=f[l],f[l]=z}yield*f;break}case"ZIGZAG":{for(let f=0;f<g;f++)if(f%2===0)for(let w=0;w<p;w++)yield{x:w,y:f};else for(let w=p-1;w>=0;w--)yield{x:w,y:f};break}case"HUMANIZED":{let f=Math.max(4,Math.floor(Math.min(p,g)/10)),w=[];for(let l=0;l<g;l+=f)for(let z=0;z<p;z+=f)w.push({x:z,y:l});for(let l=w.length-1;l>=0;l--){let z=Math.floor(Math.random()*(l+1)),b=w[l];w[l]=w[z],w[z]=b}for(let l=0;l<w.length;l++){let z=w[l],b=Math.min(g,z.y+f),c=Math.min(p,z.x+f);for(let M=z.y;M<b;M++)if(Math.random()>0.35)for(let A=z.x;A<c;A++)yield{x:A,y:M};else for(let A=c-1;A>=z.x;A--)yield{x:A,y:M}}break}case"HUMAN_SOFT_DITHER":{let f=new Set;for(let w=0;w<g;w++){let l=Math.floor(Math.random()*3)-1;if((w+l)%2===0)for(let b=0;b<p;b+=2)f.add(`${b},${w}`),yield{x:b,y:w};else for(let b=1;b<p;b+=2)f.add(`${b},${w}`),yield{x:b,y:w}}for(let w=0;w<g;w++)for(let l=0;l<p;l++){let z=`${l},${w}`;if(f.has(z))continue;yield{x:l,y:w}}break}case"HUMAN_PATCHY":{let f=new Set,w=p*g;while(f.size<w){let l=Math.floor(Math.random()*p),z=Math.floor(Math.random()*g),b=1+Math.floor(Math.random()*5);for(let c=z-b;c<=z+b;c++)for(let M=l-b;M<=l+b;M++){if(M<0||M>=p||c<0||c>=g)continue;if(Math.hypot(M-l,c-z)>b+Math.random()*1.2)continue;let H=`${M},${c}`;if(f.has(H))continue;f.add(H),yield{x:M,y:c}}}break}case"HUMAN_SWEEP_ARCS":{let f=new Set,w=(p-1)/2,l=(g-1)/2,z=Math.hypot(w,l);for(let b=0;b<4;b++){let c=Math.random()*Math.PI*2;for(let M=0;M<=z;M+=0.35){let H=Math.PI/2+Math.random()*(Math.PI/1.5),A=Math.max(10,Math.floor(M*8));for(let u=0;u<A;u++){let K=c+H*u/A+Math.sin(M)*0.08,B=Math.round(w+Math.cos(K)*M),a=Math.round(l+Math.sin(K)*M);if(B<0||B>=p||a<0||a>=g)continue;let U=`${B},${a}`;if(f.has(U))continue;f.add(U),yield{x:B,y:a}}}}for(let b=0;b<g;b++)for(let c=0;c<p;c++){let M=`${c},${b}`;if(f.has(M))continue;yield{x:c,y:b}}break}case"HUMAN_MICRO_CORRECTIONS":{let f=new Set;for(let w=0;w<g;w++){let l=w%2===0?1:-1,z=l>0?0:p-1;for(let b=0;b<p;b++){let c=z+(Math.random()>0.82?l:0),M=w+(Math.random()>0.9?1:0);for(let H of[{x:z,y:w},{x:c,y:w},{x:z,y:M}]){if(H.x<0||H.x>=p||H.y<0||H.y>=g)continue;let A=`${H.x},${H.y}`;if(f.has(A))continue;f.add(A),yield H}z+=l}}for(let w=0;w<g;w++)for(let l=0;l<p;l++){let z=`${l},${w}`;if(f.has(z))continue;yield{x:l,y:w}}break}case"HUMAN_JITTER_FILL":{let f=[];for(let w=0;w<g;w++)for(let l=0;l<p;l++)f.push({x:l,y:w});f.sort((w,l)=>{let z=w.y+(Math.random()-0.5)*1.8,b=l.y+(Math.random()-0.5)*1.8;if(z!==b)return z-b;return w.x+(Math.random()-0.5)*2-(l.x+(Math.random()-0.5)*2)}),yield*f;break}case"HUMAN_CORNER_BIAS":{let f=[{x:0,y:0},{x:p-1,y:0},{x:0,y:g-1},{x:p-1,y:g-1}],w=f[Math.floor(Math.random()*f.length)],l=[];for(let z=0;z<g;z++)for(let b=0;b<p;b++){let M=Math.hypot(b-w.x,z-w.y)+Math.random()*3.5;l.push({point:{x:b,y:z},score:M})}l.sort((z,b)=>z.score-b.score);for(let z of l)yield z.point;break}case"HUMAN_LONG_STROKES":{let f=new Set,w=p*g;while(f.size<w){let l=Math.floor(Math.random()*p),z=Math.floor(Math.random()*g),b=Math.random()*Math.PI*2,c=Math.sign(Math.cos(b)),M=Math.sign(Math.sin(b)),H=10+Math.floor(Math.random()*40);for(let A=0;A<H;A++){if(l<0||l>=p||z<0||z>=g)break;let u=`${l},${z}`;if(!f.has(u))f.add(u),yield{x:l,y:z};if(Math.random()>0.78)l+=M,z+=c;else l+=c,z+=M}}break}case"HUMAN_TAP_CLUSTERS":{let f=new Set,w=p*g;while(f.size<w){let l=Math.floor(Math.random()*p),z=Math.floor(Math.random()*g),b=3+Math.floor(Math.random()*10);for(let c=0;c<b;c++){let M=Math.round(l+(Math.random()-0.5)*6),H=Math.round(z+(Math.random()-0.5)*6);if(M<0||M>=p||H<0||H>=g)continue;let A=`${M},${H}`;if(f.has(A))continue;f.add(A),yield{x:M,y:H}}}break}case"HUMAN_MESSY_SPIRAL":{let f=new Set,w=(p-1)/2,l=(g-1)/2,z=Math.hypot(w,l)+2;for(let b=0;f.size<p*g;b++){let c=b/3,M=Math.min(z,c*0.18),H=c*0.29+Math.sin(c*0.13)*0.8,A=Math.round(w+Math.cos(H)*M+Math.sin(c)*0.7),u=Math.round(l+Math.sin(H)*M+Math.cos(c)*0.7);if(A<0||A>=p||u<0||u>=g){if(b>p*g*18)break;continue}let K=`${A},${u}`;if(f.has(K)){if(Math.random()>0.9)continue}else f.add(K),yield{x:A,y:u};if(b>p*g*18)break}for(let b=0;b<g;b++)for(let c=0;c<p;c++){let M=`${c},${b}`;if(f.has(M))continue;yield{x:c,y:b}}break}case"HUMAN_DRUNK_WALK":{let f=new Set,w=Math.floor(Math.random()*p),l=Math.floor(Math.random()*g),z=[{x:-1,y:0},{x:1,y:0},{x:0,y:-1},{x:0,y:1},{x:-1,y:-1},{x:1,y:-1},{x:-1,y:1},{x:1,y:1}];while(f.size<p*g){let b=`${w},${l}`;if(!f.has(b))f.add(b),yield{x:w,y:l};let c=[];for(let A of z){let u=w+A.x,K=l+A.y;if(u<0||u>=p||K<0||K>=g)continue;c.push({x:u,y:K})}if(!c.length)break;let M=c.filter((A)=>{return!f.has(`${A.x},${A.y}`)});if(M.length&&Math.random()>0.2){let A=M[Math.floor(Math.random()*M.length)];w=A.x,l=A.y;continue}let H=c[Math.floor(Math.random()*c.length)];w=H.x,l=H.y}for(let b=0;b<g;b++)for(let c=0;c<p;c++){let M=`${c},${b}`;if(f.has(M))continue;yield{x:c,y:b}}break}case"HUMAN_NOISE_CLOUD":{let f=[];for(let w=0;w<g;w++)for(let l=0;l<p;l++){let z=Math.sin((l+1)*0.93+Math.random()*0.8)+Math.cos((w+1)*1.17+Math.random()*0.8),b=(Math.random()-0.5)*2.6,c=Math.hypot(l-p/2,w-g/2)*0.08;f.push({point:{x:l,y:w},score:z+b+c})}f.sort((w,l)=>w.score-l.score);for(let w of f)yield w.point;break}case"HUMAN_PATCH_JUMP":{let f=new Set,w=[];for(let l=0;l<Math.max(6,p*g/18);l++)w.push({x:Math.floor(Math.random()*p),y:Math.floor(Math.random()*g)});while(f.size<p*g){let l=w[Math.floor(Math.random()*w.length)],z=1+Math.floor(Math.random()*3),b=1+Math.floor(Math.random()*3);for(let c=l.y-b;c<=l.y+b;c++)for(let M=l.x-z;M<=l.x+z;M++){if(M<0||M>=p||c<0||c>=g)continue;if(Math.random()>0.86)continue;let H=`${M},${c}`;if(f.has(H))continue;f.add(H),yield{x:M,y:c}}if(Math.random()>0.72&&w.length<p*g/2)w.push({x:Math.floor(Math.random()*p),y:Math.floor(Math.random()*g)});if(f.size>p*g*0.92)break}for(let l=0;l<g;l++)for(let z=0;z<p;z++){let b=`${z},${l}`;if(f.has(b))continue;yield{x:z,y:l}}break}case"DIAGONAL_BRUSH":{for(let f=0;f<p+g-1;f++){let w=f%2===0,l=[],z=Math.max(0,f-p+1),b=Math.min(g-1,f);for(let c=z;c<=b;c++){let M=f-c;if(M>=0&&M<p)l.push({x:M,y:c})}if(Math.random()>0.55)l.reverse();if(w)for(let c=l.length-1;c>=0;c--)yield l[c];else yield*l}break}case"BRUSH_STROKES":{let f=Array.from({length:g},()=>Array(p).fill(!1)),w=[{x:1,y:0},{x:-1,y:0},{x:0,y:1},{x:0,y:-1},{x:1,y:1},{x:1,y:-1},{x:-1,y:1},{x:-1,y:-1}],l=(c,M)=>c>=0&&c<p&&M>=0&&M<g,z=0,b=p*g;for(let c=0;c<b*6&&z<b;c++){let M=Math.floor(Math.random()*p),H=Math.floor(Math.random()*g),A=w[Math.floor(Math.random()*w.length)],u=3+Math.floor(Math.random()*16);for(let K=0;K<u;K++){if(!l(M,H))break;if(!f[H][M])f[H][M]=!0,z++,yield{x:M,y:H};if(Math.random()>0.72)A=w[Math.floor(Math.random()*w.length)];M+=A.x,H+=A.y}}for(let c=0;c<g;c++)for(let M=0;M<p;M++)if(!f[c][M])yield{x:M,y:c};break}case"SPIRAL_FROM_CENTER":case"SPIRAL_TO_CENTER":{let f=new Set,w=p*g,l=Math.floor(p/2),z=Math.floor(g/2),b=[[1,0],[0,1],[-1,0],[0,-1]],c=0,M=1,H=(u,K)=>u>=0&&u<p&&K>=0&&K<g,A=function*(){let u=0;while(u<w){for(let K=0;K<2;K++){for(let B=0;B<M;B++){if(H(l,z)){let a=`${l},${z}`;if(!f.has(a)){if(f.add(a),yield{x:l,y:z},u++,u>=w)return}}l+=b[c][0],z+=b[c][1]}c=(c+1)%4}M++}};if(this.strategy==="SPIRAL_FROM_CENTER")yield*A();else{let u=[...A()];for(let K=u.length-1;K>=0;K--)yield u[K]}break}case"SCRIBBLE":{let f=new Set,w=p*g,l=Math.floor(p/2),z=Math.floor(g/2);for(let b=0;f.size<w&&b<w*24;b++){let c=`${l},${z}`;if(!f.has(c))f.add(c),yield{x:l,y:z};if(l+=Math.floor(Math.random()*3)-1,z+=Math.floor(Math.random()*3)-1,l<0||l>=p||z<0||z>=g)l=Math.floor(Math.random()*p),z=Math.floor(Math.random()*g)}for(let b=0;b<g;b++)for(let c=0;c<p;c++){let M=`${c},${b}`;if(f.has(M))continue;f.add(M),yield{x:c,y:b}}break}case"CROSSHATCH":{let f=[];for(let z=0;z<p+g-1;z++)for(let b=Math.max(0,z-p+1);b<=Math.min(g-1,z);b++){let c=z-b;f.push({x:c,y:b})}let w=[];for(let z=-g+1;z<p;z++)for(let b=0;b<g;b++){let c=b+z;if(c>=0&&c<p)w.push({x:c,y:b})}let l=new Set;for(let z of[...f,...w]){let b=`${z.x},${z.y}`;if(l.has(b))continue;l.add(b),yield z}break}case"WAVE_SWEEP":{let f=new Set;for(let w=0;w<p;w++){let z=(Math.sin(w/Math.max(1,p-1)*Math.PI*4)+1)*0.5*(g-1)|0;for(let b=0;b<g;b++){let c=z+b,M=z-b;for(let H of[c,M]){if(H<0||H>=g)continue;let A=`${w},${H}`;if(f.has(A))continue;f.add(A),yield{x:w,y:H}}}}break}case"SCATTERED_LINES":{let f=new Set,w=p*g;for(let l=0;f.size<w&&l<w*14;l++){let z=Math.floor(Math.random()*p),b=Math.floor(Math.random()*g),c=Math.random()*Math.PI*2,M=Math.round(Math.cos(c)),H=Math.round(Math.sin(c)),A=6+Math.floor(Math.random()*28);for(let u=0;u<A;u++){if(z<0||z>=p||b<0||b>=g)break;let K=`${z},${b}`;if(!f.has(K))f.add(K),yield{x:z,y:b};z+=M,b+=H}}for(let l=0;l<g;l++)for(let z=0;z<p;z++){let b=`${z},${l}`;if(f.has(b))continue;f.add(b),yield{x:z,y:l}}break}case"CONTOUR_JITTER":{let f=new Set;for(let w=0;w<Math.ceil(Math.min(p,g)/2);w++){let l=[],z=w,b=w,c=p-w-1,M=g-w-1;for(let H=z;H<=c;H++)l.push({x:H,y:b});for(let H=b+1;H<=M;H++)l.push({x:c,y:H});for(let H=c-1;H>=z;H--)l.push({x:H,y:M});for(let H=M-1;H>b;H--)l.push({x:z,y:H});for(let H=l.length-1;H>0;H--){let A=Math.floor(Math.random()*(H+1)),u=l[H];l[H]=l[A],l[A]=u}for(let H of l){let A=`${H.x},${H.y}`;if(f.has(A))continue;f.add(A),yield H}}break}case"SPIRAL_WOBBLE":{let f=new Set,w=p/2,l=g/2,z=Math.hypot(w,l);for(let b=0;f.size<p*g&&b<p*g*9;b++){let c=b/(p*g*9)*z,M=b*0.31+Math.sin(b*0.07)*0.7,H=Math.round(w+Math.cos(M)*c),A=Math.round(l+Math.sin(M)*c);if(H<0||H>=p||A<0||A>=g)continue;let u=`${H},${A}`;if(f.has(u))continue;f.add(u),yield{x:H,y:A}}for(let b=0;b<g;b++)for(let c=0;c<p;c++){let M=`${c},${b}`;if(f.has(M))continue;yield{x:c,y:b}}break}case"CLUSTER_BURSTS":{let f=new Set,w=p*g;for(let l=0;f.size<w&&l<w*12;l++){let z=Math.floor(Math.random()*p),b=Math.floor(Math.random()*g),c=2+Math.floor(Math.random()*10);for(let M=b-c;M<=b+c;M++)for(let H=z-c;H<=z+c;H++){if(H<0||H>=p||M<0||M>=g)continue;if(Math.hypot(H-z,M-b)>c)continue;let A=`${H},${M}`;if(f.has(A))continue;f.add(A),yield{x:H,y:M}}}for(let l=0;l<g;l++)for(let z=0;z<p;z++){let b=`${z},${l}`;if(f.has(b))continue;f.add(b),yield{x:z,y:l}}break}case"ORBITAL":{let f=new Set,w=(p-1)/2,l=(g-1)/2,z=Math.ceil(Math.max(w,l));for(let b=0;b<=z;b++){let c=Math.max(16,Math.ceil(2*Math.PI*Math.max(1,b)*2));for(let M=0;M<c;M++){let H=M/c*Math.PI*2+(b%2?0.3:-0.3),A=Math.round(w+Math.cos(H)*b),u=Math.round(l+Math.sin(H)*b);if(A<0||A>=p||u<0||u>=g)continue;let K=`${A},${u}`;if(f.has(K))continue;f.add(K),yield{x:A,y:u}}}for(let b=0;b<g;b++)for(let c=0;c<p;c++){let M=`${c},${b}`;if(f.has(M))continue;yield{x:c,y:b}}break}case"FLOW_FIELD":{let f=new Set,w=p*g;for(let l=0;f.size<w&&l<w*18;l++){let z=Math.floor(Math.random()*p),b=Math.floor(Math.random()*g);for(let c=0;c<120;c++){if(z<0||z>=p||b<0||b>=g)break;let M=`${z},${b}`;if(!f.has(M))f.add(M),yield{x:z,y:b};let H=Math.sin(z*0.09)*1.8+Math.cos(b*0.08)*1.6+Math.sin((z+b)*0.05);z+=Math.round(Math.cos(H)),b+=Math.round(Math.sin(H))}}for(let l=0;l<g;l++)for(let z=0;z<p;z++){let b=`${z},${l}`;if(f.has(b))continue;f.add(b),yield{x:z,y:l}}break}case"EDGE_IN":{let f=new Set,w=Math.ceil(Math.min(p,g)/2);for(let l=0;l<w;l++){let z=l,b=p-1-l,c=l,M=g-1-l;for(let H=z;H<=b;H++)for(let A of[c,M]){let u=`${H},${A}`;if(f.has(u))continue;f.add(u),yield{x:H,y:A}}for(let H=c+1;H<=M-1;H++)for(let A of[z,b]){let u=`${A},${H}`;if(f.has(u))continue;f.add(u),yield{x:A,y:H}}}break}}}moveStart(o){if(o.button!==0)return;if(o.preventDefault(),o.stopPropagation(),!this.lock)this.moveInfo={globalX:this.position.globalX,globalY:this.position.globalY,clientX:o.clientX,clientY:o.clientY}}moveStop(){if(this.moveInfo)this.moveInfo=void 0,this.position.updateAnchor(),this.pixels.update(),this.updateColors(),Y(this.bot)}move(o){if(!this.moveInfo)return;let p=Math.round((o.clientX-this.moveInfo.clientX)/this.position.pixelSize),g=Math.round((o.clientY-this.moveInfo.clientY)/this.position.pixelSize);if(this.moveInfo.globalX!==void 0){if(this.position.globalX=p+this.moveInfo.globalX,this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,this.moveInfo.width-p)}else if(this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,p+this.moveInfo.width);if(this.moveInfo.globalY!==void 0){if(this.position.globalY=g+this.moveInfo.globalY,this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,this.moveInfo.height-g)}else if(this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,g+this.moveInfo.height);this.update(),Y(this.bot)}resizeStart(o){if(this.lock||o.button!==0)return;o.preventDefault(),o.stopPropagation(),this.moveInfo={clientX:o.clientX,clientY:o.clientY};let p=o.target;if(p.classList.contains("n"))this.moveInfo.height=this.pixels.height,this.moveInfo.globalY=this.position.globalY;if(p.classList.contains("e"))this.moveInfo.width=this.pixels.width;if(p.classList.contains("s"))this.moveInfo.height=this.pixels.height;if(p.classList.contains("w"))this.moveInfo.width=this.pixels.width,this.moveInfo.globalX=this.position.globalX}export(){let o=document.createElement("a");document.body.append(o),o.href=URL.createObjectURL(new Blob([JSON.stringify(this.toJSON())],{type:"application/json"})),o.download=`${this.pixels.width}x${this.pixels.height}.${n}`,o.click(),URL.revokeObjectURL(o.href),o.href=this.pixels.canvas.toDataURL("image/webp",1),o.download=`${this.pixels.width}x${this.pixels.height}.webp`,o.click(),URL.revokeObjectURL(o.href),o.remove()}}var Y0=`/* stylelint-disable declaration-no-important */
...
.kgm-auth-submit {
  background: linear-gradient(180deg, #384d7b, #2a3d66);
  font-weight: 600;
}
...
`;var F0="kglacer-macro:overlay-hidden",Z0="kglacer-macro:access-ok",s0="S0dNLXlZUjhTMW81bEhVemVjS1RFMEhxRVB4OVFkcjgxaEVz",$0="https://raw.githubusercontent.com/robgallardof/kglacer-macro/refs/heads/main/src/img/logo.svg";class c0 extends i{bot;element=document.createElement("div");get status(){return this.$status.innerHTML}set status(o){this.$status.innerHTML=o}get open(){return this.element.classList.contains("wopen")}set open(o){if(o)this.element.classList.add("wopen");else this.element.classList.remove("wopen")}$settings;$status;$shortcuts;$locale;$topbar;$draw;$addImage;$toggleOverlay;$strategy;$progressLine;$progressText;$images;$wopenButton;$widgetLogo;activeImageIndex=-1;isUnlocked=!1;unlockPromise;resolveUnlock;constructor(o){super();this.bot=o;this.unlockPromise=new Promise((p)=>{this.resolveUnlock=p}),this.element.classList.add("wwidget"),this.element.innerHTML=D0,s(this.element),document.body.append(this.element),this.populateElementsWithSelector(this.element,{$wopenButton:".wopen-button",$widgetLogo:".widget-logo",$settings:".wform",$status:".wstatus",$shortcuts:".shortcuts",$locale:".locale",$topbar:".wtopbar",$draw:".draw",$addImage:".add-image",$toggleOverlay:".toggle-overlay",$strategy:".strategy",$progressLine:".wprogress div",$progressText:".wprogress span",$images:".images"}),this.$widgetLogo.src=$0,this.$wopenButton.addEventListener("click",()=>this.open=!this.open),this.$draw.addEventListener("click",()=>this.bot.draw()),this.$addImage.addEventListener("click",()=>this.addImage()),this.$toggleOverlay.addEventListener("click",()=>{this.toggleOverlay()}),this.$strategy.addEventListener("change",()=>{this.bot.strategy=this.$strategy.value}),this.$locale.value=b0(),this.$locale.addEventListener("change",()=>{K0(this.$locale.value),s(this.element);for(let p=0;p<this.bot.images.length;p++)this.bot.images[p].applyLocale();this.refreshOverlayToggleText()}),this.registerEvent(document,"keydown",this.handleKeyboard.bind(this),{passive:!1}),this.update(),this.syncOverlayVisibilityFromStorage(),this.initializeAccessGate(),console.log("[KGM][Widget] Widget mounted and opened")}initializeAccessGate(){if(sessionStorage.getItem(Z0)==="true"){this.isUnlocked=!0,this.open=!0,this.resolveUnlock();return}this.element.style.display="none";let o=document.createElement("div");o.className="kgm-auth-overlay",o.innerHTML=`<div class="kgm-auth-modal">
  <h2>KGM Access</h2>
...
</div>`,document.body.append(o);let p=o.querySelector(".kgm-auth-input"),g=o.querySelector(".kgm-auth-submit"),f=o.querySelector(".kgm-auth-error"),w=()=>{if(btoa(p.value.trim())!==s0){f.textContent="Clave incorrecta";return}sessionStorage.setItem(Z0,"true"),this.isUnlocked=!0,this.element.style.display="",this.open=!0,o.remove(),this.resolveUnlock()};g.addEventListener("click",w),p.addEventListener("keydown",(l)=>{if(l.key==="Enter")w()}),p.focus()}addImage(){return console.log("[KGM][Widget] Add image flow started"),this.setDisabled("add-image",!0),this.run("Adding image",async()=>{await this.bot.updateColors();let o=document.createElement("input");o.type="file",o.accept=`image/*,.${n}`,o.click(),await m(o,["change"],["cancel","error"]);let p=o.files?.[0];if(!p)throw new z0(this.bot);console.log("[KGM][Widget] File selected",{name:p.name,size:p.size,type:p.type});let g;if(p.name.endsWith(`.${n}`))g=await T.fromJSON(this.bot,JSON.parse(await p.text()));else{let f=new FileReader;f.readAsDataURL(p),await m(f,["load"],["error"]);let w=await this.compressImageBeforeLoad(f.result),l=new Image;l.src=w,await m(l,["load"],["error"]),g=new T(this.bot,D.fromScreenPosition(this.bot,this.defaultImageScreenPosition()),new r(this.bot,l))}this.bot.images.push(g),console.log("[KGM][Widget] Image instance added",{images:this.bot.images.length}),await this.bot.readMap(),g.updateTasks(),Y(this.bot,!0),console.log("[KGM][Widget] Image persisted, reloading page"),document.location.reload()},()=>{this.setDisabled("add-image",!1)})}defaultImageScreenPosition(){let o=Math.round(this.element.getBoundingClientRect().width);return{x:Math.max(256,o),y:32}}async compressImageBeforeLoad(o){let p=new Image;if(p.src=o,await m(p,["load"],["error"]),!(p.naturalWidth*p.naturalHeight>3000000||o.length>3000000))return o;let f=document.createElement("canvas");f.width=p.naturalWidth,f.height=p.naturalHeight;let w=f.getContext("2d");if(!w)return o;return w.drawImage(p,0,0),f.toDataURL("image/png")}update(){this.$strategy.value=this.bot.strategy;let o=0,p=0;for(let l=0;l<this.bot.images.length;l++){let z=this.bot.images[l];o+=z.pixels.pixels.length*z.pixels.pixels[0].length,p+=z.tasks.length}let g=o-p,f=g/o*100|0;this.$progressText.textContent=`${g}/${o} ${f}% ETA: ${p/120|0}h`,this.$progressLine.style.transform=`scaleX(${f}%)`,this.$images.innerHTML="";let w=document.createDocumentFragment();for(let l=0;l<this.bot.images.length;l++){let z=this.bot.images[l],b=document.createElement("div");w.append(b),b.className="image",b.innerHTML=`<button class="preview" title="Focus image">
  ...
  </div>`,b.querySelector(".preview").addEventListener("click",()=>{this.activeImageIndex=l,z.position.scrollScreenTo()}),b.querySelector(".settings").addEventListener("click",()=>{this.activeImageIndex=l,z.openSettingsPanel()}),b.querySelector(".remove").addEventListener("click",()=>{if(this.activeImageIndex===l)this.activeImageIndex=-1;z.destroy()}),b.querySelector(".up").addEventListener("click",()=>{l0(this.bot.images,l,l-1),this.update(),Y(this.bot)}),b.querySelector(".down").addEventListener("click",()=>{l0(this.bot.images,l,l+1),this.update(),Y(this.bot)})}this.$images.append(w)}syncOverlayVisibilityFromStorage(){let o=localStorage.getItem(F0)==="true";document.body.classList.toggle("overlay-hidden",o),this.refreshOverlayToggleText()}toggleOverlay(o){let p=o??!document.body.classList.contains("overlay-hidden");document.body.classList.toggle("overlay-hidden",p),localStorage.setItem(F0,String(p)),this.refreshOverlayToggleText()}refreshOverlayToggleText(){this.$toggleOverlay.textContent=document.body.classList.contains("overlay-hidden")?`${J("toggleOverlay")} (${J("disabled")})`:`${J("toggleOverlay")} (${J("enabled")})`}setDisabled(o,p){this.element.querySelector("."+o).disabled=p}async run(o,p,g,f="..."){await this.unlockPromise,console.log("[KGM][Widget] Task started",{status:o});let w=this.status;this.status=`${f} ${o}`;try{let l=await p();return this.status=w,console.log("[KGM][Widget] Task completed",{status:o}),l}catch(l){if(!(l instanceof o0))console.error(l),this.status=`Error: ${o}`;throw console.error("[KGM][Widget] Task failed",{status:o,error:l}),l}finally{await g?.()}}handleKeyboard(o){if(!this.isUnlocked)return;if(j0(o.target))return;if(q(o,Z.toggleWidget)){o.preventDefault(),this.open=!this.open;return}if(q(o,Z.showShortcuts)){o.preventDefault(),this.$shortcuts.scrollIntoView({behavior:"smooth",block:"center"}),this.$shortcuts.classList.remove("shortcut-pulse"),requestAnimationFrame(()=>{this.$shortcuts.classList.add("shortcut-pulse")});return}if(q(o,Z.toggleOverlay)){o.preventDefault(),this.toggleOverlay();return}if(q(o,Z.focusNextImage)){o.preventDefault(),this.focusImageByStep(1);return}if(q(o,Z.focusPreviousImage)){o.preventDefault(),this.focusImageByStep(-1);return}if(q(o,Z.openColorPanel)){o.preventDefault(),this.openColorPanelForActiveImage();return}if(q(o,Z.toggleImageLock)){o.preventDefault(),this.toggleLockForActiveImage();return}if(q(o,Z.addImage)&&!this.$addImage.disabled){o.preventDefault(),this.addImage();return}if(q(o,Z.draw)&&!this.$draw.disabled)o.preventDefault(),this.bot.draw()}focusImageByStep(o){if(!this.bot.images.length)return;if(this.activeImageIndex<0||this.activeImageIndex>=this.bot.images.length)this.activeImageIndex=o>0?0:this.bot.images.length-1;else this.activeImageIndex=(this.activeImageIndex+o+this.bot.images.length)%this.bot.images.length;this.bot.images[this.activeImageIndex].position.scrollScreenTo()}getActiveImage(){if(!this.bot.images.length)return;if(this.activeImageIndex<0||this.activeImageIndex>=this.bot.images.length)this.activeImageIndex=0;return this.bot.images[this.activeImageIndex]}openColorPanelForActiveImage(){let o=this.getActiveImage();if(!o)return;o.openColorPanel()}toggleLockForActiveImage(){let o=this.getActiveImage();if(!o)return;o.lock=!o.lock,o.update(),Y(this.bot)}}var h0=2,q0="[KGM]";class X0{unavailableColors=new Set;mapsCache=new Map;me;$stars=[];strategy="SEQUENTIAL";images=[];widget=new c0(this);markerPixelPositionResolvers=[];lastColor;log(o,p){if(p===void 0)console.log(`${q0} ${o}`);else console.log(`${q0} ${o}`,p)}constructor(){this.log("Boot sequence started");let o=N0();if(this.log("Save loaded",{hasSave:Boolean(o),imageCount:o?.images.length??0,strategy:o?.strategy}),o){for(let g=0;g<o.images.length;g++){let f=o.images[g];y({x:f.position[0]-1000,y:f.position[1]-1000}),y({x:f.position[0]+1000,y:f.position[1]+1000})}this.strategy=o.strategy}this.registerFetchInterceptor(),this.log("Fetch interceptor registered");let p=document.createElement("style");p.textContent=Y0.replace("FAKE_FAVORITE_LOCATIONS",d.length.toString()),document.head.append(p),this.log("Styles injected",{fakeFavoriteLocations:d.length}),this.widget.run("Initializing",async()=>{this.log("Widget initialization flow started"),await this.waitForElement("login",".avatar.center-absolute.absolute"),await this.waitForElement("pixel count",".btn.btn-primary.btn-lg.relative.z-30 canvas");let g=await this.waitForElement("canvas",".maplibregl-canvas-container");if(new MutationObserver((f)=>{for(let w=0;w<f.length;w++)if(f[w].removedNodes.length!==0){this.updateStars();break}this.updateImages()}).observe(g,{attributes:!0,childList:!0,subtree:!0}),this.updateStars(),this.log("Stars updated after boot",{stars:this.$stars.length}),await L(500),await this.updateColors(),o)for(let f=0;f<o.images.length;f++){let w=await T.fromJSON(this,o.images[f]);this.images.push(w),w.update()}this.log("Saved images restored",{images:this.images.length}),await this.readMap(),this.updateTasks(),this.widget.setDisabled("draw",!1),this.widget.setDisabled("add-image",!1),this.log("Initialization completed; controls enabled")})}draw(){this.log("Draw requested",{strategy:this.strategy,images:this.images.length}),this.widget.setDisabled("draw",!0),this.widget.status="",this.mapsCache.clear();let o=document.querySelector(".maplibregl-canvas"),p=(g)=>{if(!g.shiftKey)g.stopPropagation()};return this.widget.run("Drawing",async()=>{await this.widget.run("Initializing draw",()=>Promise.all([this.updateColors(),this.readMap()])),globalThis.addEventListener("mousemove",p,!0),o.addEventListener("wheel",p,!0),this.updateTasks();let g=await fetch("https://backend.wplace.live/me",{credentials:"include"}).then((l)=>l.json()),f=Math.floor(g.charges.count);this.log("Charges fetched",{charges:f});let w=0;for(let l=0;l<this.images.length;l++)w+=this.images[l].tasks.length;switch(this.log("Tasks prepared",{tasks:w}),this.strategy){case"ALL":{while(f>0){let l=!0;for(let z=0;z<this.images.length;z++){let b=this.images[z].tasks.shift();if(!b)continue;this.drawTask(b),f--,await L(1),l=!1}if(l)break}break}case"PERCENTAGE":{for(let l=0;l<w&&f>0;l++){let z=1,b;for(let c=0;c<this.images.length;c++){let M=this.images[c],H=1-M.tasks.length/(M.pixels.pixels.length*M.pixels.pixels[0].length);if(H<z)z=H,b=M}this.drawTask(b.tasks.shift()),f--,await L(1)}break}case"SEQUENTIAL":for(let l=0;l<this.images.length;l++){let z=this.images[l];for(let b=z.tasks.shift();b&&f>0;b=z.tasks.shift())this.drawTask(b),f--,await L(1)}}this.widget.update(),this.log("Draw flow finished",{remainingCharges:f})},()=>{globalThis.removeEventListener("mousemove",p,!0),o.removeEventListener("wheel",p,!0),this.widget.setDisabled("draw",!1)})}toJSON(){return{version:h0,images:this.images.map((o)=>o.toJSON()),strategy:this.strategy}}async updateColors(){this.log("Updating colors palette"),await this.openColors(),this.unavailableColors.clear();for(let o of document.querySelectorAll("button.btn.relative.w-full"))if(o.children.length!==0)this.unavailableColors.add(Math.abs(Number.parseInt(o.id.slice(6))));this.updateImageColors(),this.log("Colors updated",{unavailableColors:this.unavailableColors.size})}moveMap(o){let p=document.querySelector(".maplibregl-canvas"),g=window.innerWidth/2,f=window.innerHeight/2,w=g-o.x,l=f-o.y;function z(b,c,M){p.dispatchEvent(new MouseEvent(b,{bubbles:!0,cancelable:!0,clientX:c,clientY:M,buttons:1}))}z("mousedown",g,f),z("mousemove",w,l),z("mouseup",w,l)}readMap(){this.mapsCache.clear();let o=new Set;for(let g=0;g<this.images.length;g++){let f=this.images[g],{tileX:w,tileY:l}=new D(this,f.position.globalX+f.pixels.pixels[0].length,f.position.globalY+f.pixels.pixels.length);for(let z=f.position.tileX;z<=w;z++)for(let b=f.position.tileY;b<=l;b++)o.add(`${z}/${b}`)}let p=0;return this.log("Reading map tiles",{tileCount:o.size}),this.widget.run(`Reading map [0/${o.size}]`,()=>Promise.all([...o].map(async(g)=>{this.mapsCache.set(g,await r.fromJSON(this,{url:`https://backend.wplace.live/files/s0/tiles/${g}.png`,exactColor:!0})),this.widget.status=`⌛ Reading map [${++p}/${o.size}]`})))}waitForUnfocus(){return this.widget.run("UNFOCUS WINDOW",()=>new Promise((o)=>{if(!document.hasFocus())o();window.addEventListener("blur",()=>{setTimeout(o,1)},{once:!0})}),void 0,"\uD83D\uDDB1️")}findAnchorsForScreen(o){let p=0,g=1,f=1/0,w=1/0;for(let b=0;b<this.$stars.length;b++){let{x:c,y:M}=R(this.$stars[b]);if(c<o.x&&M<o.y){let H=o.x-c+(o.y-M);if(H<f)f=H,p=b}else if(c>o.x&&M>o.y){let H=c-o.x+(M-o.y);if(H<w)w=H,g=b}}let l=R(this.$stars[p]),z=P[p];return{anchorScreenPosition:l,anchorWorldPosition:z,pixelSize:(R(this.$stars[g]).x-l.x)/(P[g].x-z.x)}}async openColors(){this.lastColor=void 0,document.querySelector(".flex.gap-2.px-3 > .btn-circle")?.click(),await L(1),document.querySelector(".btn.btn-primary.btn-lg.relative.z-30")?.click(),await L(1);let o=document.querySelector("button.bottom-0");if(o?.innerHTML==='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor" class="size-5"><path d="M480-120 300-300l58-58 122 122 122-122 58 58-180 180ZM358-598l-58-58 180-180 180 180-58 58-122-122-122 122Z"></path></svg><!---->')o.click(),await L(1)}drawTask(o){if(this.lastColor!==o.color)document.getElementById("color-"+o.color).click(),this.lastColor=o.color,this.log("Color switched for draw task",{color:o.color});let p=o.position.pixelSize/2,g=o.position.toScreenPosition();document.documentElement.dispatchEvent(new MouseEvent("mousemove",{bubbles:!0,clientX:g.x+p,clientY:g.y+p,shiftKey:!0})),document.documentElement.dispatchEvent(new KeyboardEvent("keydown",{key:" ",code:"Space",keyCode:32,which:32,bubbles:!0,cancelable:!0})),document.documentElement.dispatchEvent(new KeyboardEvent("keyup",{key:" ",code:"Space",keyCode:32,which:32,bubbles:!0,cancelable:!0}))}registerFetchInterceptor(){let o=globalThis.fetch,p=/https:\/\/backend.wplace.live\/s\d+\/pixel\/(-?\d+)\/(-?\d+)\?x=(-?\d+)&y=(-?\d+)/;globalThis.fetch=async(g,f)=>{let w=await o(g,f),l=w.clone(),z="";if(typeof g=="string")z=g;else if(g instanceof Request)z=g.url;else if(g instanceof URL)z=g.href;if(w.url==="https://backend.wplace.live/me")this.me=await l.json(),this.me.favoriteLocations.unshift(...d),this.me.maxFavoriteLocations=1/0,w.json=()=>Promise.resolve(this.me),this.log("Patched /me response with favorite locations",{totalFavorites:this.me.favoriteLocations.length});let b=p.exec(z);if(b){for(let c=0;c<this.markerPixelPositionResolvers.length;c++)this.markerPixelPositionResolvers[c](new D(this,+b[1],+b[2],+b[3],+b[4]));this.markerPixelPositionResolvers.length=0,this.log("Resolved marker pixel position from network event")}return w}}async closeAll(){for(let o of document.querySelectorAll("button"))if(o.innerHTML==="✕"||o.innerHTML==='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor" class="size-4"><path d="m256-200-56-56 224-224-224-224 56-56 224 224 224-224 56 56-224 224 224 224-56 56-224-224-224 224Z"></path></svg><!---->')o.click(),await L(1)}waitForElement(o,p){return this.log("Waiting for element",{name:o,selector:p}),this.widget.run(`Waiting for ${o}`,()=>{return new Promise((g)=>{let f=document.querySelector(p);if(f){g(f);return}let w=new MutationObserver(()=>{let l=document.querySelector(p);if(l)w.disconnect(),g(l)});w.observe(document.documentElement,{childList:!0,subtree:!0})})})}updateStars(){this.$stars=[...document.querySelectorAll(".text-yellow-400.cursor-pointer.z-10.maplibregl-marker.maplibregl-marker-anchor-center")].slice(0,d.length),this.log("Star cache updated",{stars:this.$stars.length})}updateImages(){for(let o=0;o<this.images.length;o++)this.images[o].position.updateAnchor(),this.images[o].update()}updateTasks(){for(let o=0;o<this.images.length;o++)this.images[o].updateTasks()}updateImageColors(){for(let o=0;o<this.images.length;o++)this.images[o].updateColors()}}globalThis.kglacerMacro=new X0;globalThis.kgm=globalThis.kglacerMacro;globalThis.wbot=globalThis.kglacerMacro;export{X0 as KGlacerMacro};
