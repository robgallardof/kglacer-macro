...
<span class="premium ${r?"on":""}">${r?d("premium"):""}</span>`,b.querySelector(".swatch").style.setProperty("--swatch-color",Ho(l.realColor)),b.addEventListener("click",()=>{if(c){c=!1;return}u()}),b.addEventListener("dragstart",(n)=>{c=!0,b.classList.add("dragging"),n.dataTransfer?.setData("text/plain",String(a)),n.dataTransfer.effectAllowed="move"}),b.addEventListener("dragend",()=>{b.classList.remove("dragging")}),b.addEventListener("dragover",(n)=>{n.preventDefault(),b.classList.add("drag-target")}),b.addEventListener("dragleave",()=>{b.classList.remove("drag-target")}),b.addEventListener("drop",(n)=>{n.preventDefault(),b.classList.remove("drag-target");let z=Number.parseInt(n.dataTransfer?.getData("text/plain")??"-1",10);if(z<0||z===a||z>=this.colors.length)return;this.colors.splice(a,0,...this.colors.splice(z,1)),A(this.bot),this.updateColors()}),r){let n=document.createElement("button");n.textContent=d("buy"),n.className="buy-chip",n.addEventListener("click",(z)=>{z.stopPropagation(),document.getElementById("color-"+l.realColor)?.click()}),b.append(n)}let m=`${f} ${w.join(" ")} ${l.realColor} ${K[l.realColor]}`;if(!p||m.toLowerCase().includes(p))this.$colorsDialogList.append(b)}}*strategyPositionIterator(){let o=this.pixels.pixels[0].length,p=this.pixels.pixels.length;switch(this.strategy){case"DOWN":{for(let a=0;a<p;a++)for(let s=0;s<o;s++)yield{x:s,y:a};break}case"UP":{for(let a=p-1;a>=0;a--)for(let s=0;s<o;s++)yield{x:s,y:a};break}case"LEFT":{for(let a=0;a<o;a++)for(let s=0;s<p;s++)yield{x:a,y:s};break}case"RIGHT":{for(let a=o-1;a>=0;a--)for(let s=0;s<p;s++)yield{x:a,y:s};break}case"RANDOM":{let a=[];for(let s=0;s<p;s++)for(let l=0;l<o;l++)a.push({x:l,y:s});for(let s=a.length-1;s>=0;s--){let l=Math.floor(Math.random()*(s+1)),c=a[s];a[s]=a[l],a[l]=c}yield*a;break}case"ZIGZAG":{for(let a=0;a<p;a++)if(a%2===0)for(let s=0;s<o;s++)yield{x:s,y:a};else for(let s=o-1;s>=0;s--)yield{x:s,y:a};break}case"HUMANIZED":{let a=Math.max(4,Math.floor(Math.min(o,p)/10)),s=[];for(let l=0;l<p;l+=a)for(let c=0;c<o;c+=a)s.push({x:c,y:l});for(let l=s.length-1;l>=0;l--){let c=Math.floor(Math.random()*(l+1)),r=s[l];s[l]=s[c],s[c]=r}for(let l=0;l<s.length;l++){let c=s[l],r=Math.min(p,c.y+a),g=Math.min(o,c.x+a);for(let f=c.y;f<r;f++)if(Math.random()>0.35)for(let u=c.x;u<g;u++)yield{x:u,y:f};else for(let u=g-1;u>=c.x;u--)yield{x:u,y:f}}break}case"HUMAN_SOFT_DITHER":{let a=new Set;for(let s=0;s<p;s++){let l=Math.floor(Math.random()*3)-1;if((s+l)%2===0)for(let r=0;r<o;r+=2)a.add(`${r},${s}`),yield{x:r,y:s};else for(let r=1;r<o;r+=2)a.add(`${r},${s}`),yield{x:r,y:s}}for(let s=0;s<p;s++)for(let l=0;l<o;l++){let c=`${l},${s}`;if(a.has(c))continue;yield{x:l,y:s}}break}case"HUMAN_PATCHY":{let a=new Set,s=o*p;while(a.size<s){let l=Math.floor(Math.random()*o),c=Math.floor(Math.random()*p),r=1+Math.floor(Math.random()*5);for(let g=c-r;g<=c+r;g++)for(let f=l-r;f<=l+r;f++){if(f<0||f>=o||g<0||g>=p)continue;if(Math.hypot(f-l,g-c)>r+Math.random()*1.2)continue;let w=`${f},${g}`;if(a.has(w))continue;a.add(w),yield{x:f,y:g}}}break}case"HUMAN_SWEEP_ARCS":{let a=new Set,s=(o-1)/2,l=(p-1)/2,c=Math.hypot(s,l);for(let r=0;r<4;r++){let g=Math.random()*Math.PI*2;for(let f=0;f<=c;f+=0.35){let w=Math.PI/2+Math.random()*(Math.PI/1.5),u=Math.max(10,Math.floor(f*8));for(let b=0;b<u;b++){let m=g+w*b/u+Math.sin(f)*0.08,n=Math.round(s+Math.cos(m)*f),z=Math.round(l+Math.sin(m)*f);if(n<0||n>=o||z<0||z>=p)continue;let H=`${n},${z}`;if(a.has(H))continue;a.add(H),yield{x:n,y:z}}}}for(let r=0;r<p;r++)for(let g=0;g<o;g++){let f=`${g},${r}`;if(a.has(f))continue;yield{x:g,y:r}}break}case"HUMAN_MICRO_CORRECTIONS":{let a=new Set;for(let s=0;s<p;s++){let l=s%2===0?1:-1,c=l>0?0:o-1;for(let r=0;r<o;r++){let g=c+(Math.random()>0.82?l:0),f=s+(Math.random()>0.9?1:0);for(let w of[{x:c,y:s},{x:g,y:s},{x:c,y:f}]){if(w.x<0||w.x>=o||w.y<0||w.y>=p)continue;let u=`${w.x},${w.y}`;if(a.has(u))continue;a.add(u),yield w}c+=l}}for(let s=0;s<p;s++)for(let l=0;l<o;l++){let c=`${l},${s}`;if(a.has(c))continue;yield{x:l,y:s}}break}case"HUMAN_JITTER_FILL":{let a=[];for(let s=0;s<p;s++)for(let l=0;l<o;l++)a.push({x:l,y:s});a.sort((s,l)=>{let c=s.y+(Math.random()-0.5)*1.8,r=l.y+(Math.random()-0.5)*1.8;if(c!==r)return c-r;return s.x+(Math.random()-0.5)*2-(l.x+(Math.random()-0.5)*2)}),yield*a;break}case"HUMAN_CORNER_BIAS":{let a=[{x:0,y:0},{x:o-1,y:0},{x:0,y:p-1},{x:o-1,y:p-1}],s=a[Math.floor(Math.random()*a.length)],l=[];for(let c=0;c<p;c++)for(let r=0;r<o;r++){let f=Math.hypot(r-s.x,c-s.y)+Math.random()*3.5;l.push({point:{x:r,y:c},score:f})}l.sort((c,r)=>c.score-r.score);for(let c of l)yield c.point;break}case"HUMAN_LONG_STROKES":{let a=new Set,s=o*p;while(a.size<s){let l=Math.floor(Math.random()*o),c=Math.floor(Math.random()*p),r=Math.random()*Math.PI*2,g=Math.sign(Math.cos(r)),f=Math.sign(Math.sin(r)),w=10+Math.floor(Math.random()*40);for(let u=0;u<w;u++){if(l<0||l>=o||c<0||c>=p)break;let b=`${l},${c}`;if(!a.has(b))a.add(b),yield{x:l,y:c};if(Math.random()>0.78)l+=f,c+=g;else l+=g,c+=f}}break}case"HUMAN_TAP_CLUSTERS":{let a=new Set,s=o*p;while(a.size<s){let l=Math.floor(Math.random()*o),c=Math.floor(Math.random()*p),r=3+Math.floor(Math.random()*10);for(let g=0;g<r;g++){let f=Math.round(l+(Math.random()-0.5)*6),w=Math.round(c+(Math.random()-0.5)*6);if(f<0||f>=o||w<0||w>=p)continue;let u=`${f},${w}`;if(a.has(u))continue;a.add(u),yield{x:f,y:w}}}break}case"HUMAN_MESSY_SPIRAL":{let a=new Set,s=(o-1)/2,l=(p-1)/2,c=Math.hypot(s,l)+2;for(let r=0;a.size<o*p;r++){let g=r/3,f=Math.min(c,g*0.18),w=g*0.29+Math.sin(g*0.13)*0.8,u=Math.round(s+Math.cos(w)*f+Math.sin(g)*0.7),b=Math.round(l+Math.sin(w)*f+Math.cos(g)*0.7);if(u<0||u>=o||b<0||b>=p){if(r>o*p*18)break;continue}let m=`${u},${b}`;if(a.has(m)){if(Math.random()>0.9)continue}else a.add(m),yield{x:u,y:b};if(r>o*p*18)break}for(let r=0;r<p;r++)for(let g=0;g<o;g++){let f=`${g},${r}`;if(a.has(f))continue;yield{x:g,y:r}}break}case"HUMAN_DRUNK_WALK":{let a=new Set,s=Math.floor(Math.random()*o),l=Math.floor(Math.random()*p),c=[{x:-1,y:0},{x:1,y:0},{x:0,y:-1},{x:0,y:1},{x:-1,y:-1},{x:1,y:-1},{x:-1,y:1},{x:1,y:1}];while(a.size<o*p){let r=`${s},${l}`;if(!a.has(r))a.add(r),yield{x:s,y:l};let g=[];for(let u of c){let b=s+u.x,m=l+u.y;if(b<0||b>=o||m<0||m>=p)continue;g.push({x:b,y:m})}if(!g.length)break;let f=g.filter((u)=>{return!a.has(`${u.x},${u.y}`)});if(f.length&&Math.random()>0.2){let u=f[Math.floor(Math.random()*f.length)];s=u.x,l=u.y;continue}let w=g[Math.floor(Math.random()*g.length)];s=w.x,l=w.y}for(let r=0;r<p;r++)for(let g=0;g<o;g++){let f=`${g},${r}`;if(a.has(f))continue;yield{x:g,y:r}}break}case"HUMAN_NOISE_CLOUD":{let a=[];for(let s=0;s<p;s++)for(let l=0;l<o;l++){let c=Math.sin((l+1)*0.93+Math.random()*0.8)+Math.cos((s+1)*1.17+Math.random()*0.8),r=(Math.random()-0.5)*2.6,g=Math.hypot(l-o/2,s-p/2)*0.08;a.push({point:{x:l,y:s},score:c+r+g})}a.sort((s,l)=>s.score-l.score);for(let s of a)yield s.point;break}case"HUMAN_PATCH_JUMP":{let a=new Set,s=[];for(let l=0;l<Math.max(6,o*p/18);l++)s.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*p)});while(a.size<o*p){let l=s[Math.floor(Math.random()*s.length)],c=1+Math.floor(Math.random()*3),r=1+Math.floor(Math.random()*3);for(let g=l.y-r;g<=l.y+r;g++)for(let f=l.x-c;f<=l.x+c;f++){if(f<0||f>=o||g<0||g>=p)continue;if(Math.random()>0.86)continue;let w=`${f},${g}`;if(a.has(w))continue;a.add(w),yield{x:f,y:g}}if(Math.random()>0.72&&s.length<o*p/2)s.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*p)});if(a.size>o*p*0.92)break}for(let l=0;l<p;l++)for(let c=0;c<o;c++){let r=`${c},${l}`;if(a.has(r))continue;yield{x:c,y:l}}break}case"HUMAN_HESITANT_LINES":{let a=new Set;for(let s=0;s<p;s++){let l=s%2===0;for(let c=0;c<o;c++){let r=l?c:o-1-c,g=`${r},${s}`;if(!a.has(g))a.add(g),yield{x:r,y:s};if(Math.random()>0.7){let f=Math.max(0,Math.min(o-1,r+(Math.random()>0.5?1:-1))),w=Math.max(0,Math.min(p-1,s+(Math.random()>0.65?1:0))),u=`${f},${w}`;if(!a.has(u))a.add(u),yield{x:f,y:w}}}}for(let s=0;s<p;s++)for(let l=0;l<o;l++){let c=`${l},${s}`;if(a.has(c))continue;yield{x:l,y:s}}break}case"HUMAN_OVERLAP_SWEEPS":{let a=[],s=Math.random()*Math.PI*2;for(let l=0;l<p;l++)for(let c=0;c<o;c++){let r=Math.sin((c+l)*0.42+s)*2.2,g=Math.cos((c-l)*0.3+s)*1.4;a.push({point:{x:c,y:l},score:l+r+g+(Math.random()-0.5)*3.4})}a.sort((l,c)=>l.score-c.score);for(let l of a)yield l.point;break}case"HUMAN_WOBBLE_DRIFT":{let a=[],s=o/2,l=p/2;for(let c=0;c<p;c++)for(let r=0;r<o;r++){let g=Math.hypot(r-s,c-l)*0.25,f=Math.sin((r+1)*0.9)*1.8+Math.cos((c+1)*1.1)*1.8+Math.sin((r+c)*0.35)*1.4;a.push({point:{x:r,y:c},score:g+f+(Math.random()-0.5)*2.8})}a.sort((c,r)=>c.score-r.score);for(let c of a)yield c.point;break}case"HUMAN_GAP_RECOVERY":{let a=new Set,s=[];for(let l=0;l<p;l++)for(let c=0;c<o;c++){if(Math.random()>0.87){s.push({x:c,y:l});continue}a.add(`${c},${l}`),yield{x:c,y:l}}s.sort((l,c)=>Math.hypot(l.x-o/2,l.y-p/2)-Math.hypot(c.x-o/2,c.y-p/2));for(let l of s){let c=`${l.x},${l.y}`;if(a.has(c))continue;a.add(c),yield l}break}case"HUMAN_STAIRCASE":{let a=new Set,s=o+p-1;for(let l=0;l<s;l++){let c=Math.max(0,l-o+1),r=Math.min(p-1,l);for(let g=c;g<=r;g++){let f=l-g,w=[{x:f,y:g},{x:f+(Math.random()>0.5?1:-1),y:g},{x:f,y:g+(Math.random()>0.5?1:-1)}];for(let u of w){if(u.x<0||u.x>=o||u.y<0||u.y>=p)continue;let b=`${u.x},${u.y}`;if(a.has(b))continue;a.add(b),yield u}}}for(let l=0;l<p;l++)for(let c=0;c<o;c++){let r=`${c},${l}`;if(a.has(r))continue;yield{x:c,y:l}}break}case"HUMAN_EDGE_HUGGER":{let a=[];for(let s=0;s<p;s++)for(let l=0;l<o;l++){let c=Math.min(l,s,o-1-l,p-1-s);a.push({point:{x:l,y:s},score:c*3.5+(Math.random()-0.5)*5.5})}a.sort((s,l)=>s.score-l.score);for(let s of a)yield s.point;break}case"HUMAN_BLOBS":{let a=new Set,s=o*p;while(a.size<s){let l=Math.floor(Math.random()*o),c=Math.floor(Math.random()*p),r=1+Math.floor(Math.random()*4);for(let g=c-r;g<=c+r;g++)for(let f=l-r;f<=l+r;f++){if(f<0||f>=o||g<0||g>=p)continue;let w=Math.atan2(g-c,f-l),u=r+Math.sin(w*3+Math.random())*0.8;if(Math.hypot(f-l,g-c)>u)continue;let b=`${f},${g}`;if(a.has(b))continue;a.add(b),yield{x:f,y:g}}}break}case"HUMAN_BACKTRACK":{let a=new Set,s=[];for(let l=0;l<p;l++)for(let c=0;c<o;c++)s.push({x:c,y:l});s.sort((l,c)=>l.y-c.y+(Math.random()-0.5)*2.2+(l.x-c.x)*0.04);for(let l=0;l<s.length;l++){let c=s[l],r=`${c.x},${c.y}`;if(a.has(r))continue;if(a.add(r),yield c,l>1&&Math.random()>0.74){let g=s[l-1],f=`${g.x},${g.y}`;if(!a.has(f))a.add(f),yield g}}for(let l=0;l<p;l++)for(let c=0;c<o;c++){let r=`${c},${l}`;if(a.has(r))continue;yield{x:c,y:l}}break}case"HUMAN_SHAKY_DIAGONAL":{let a=[];for(let s=0;s<p;s++)for(let l=0;l<o;l++){let c=Math.abs(l-s)*0.6,r=Math.sin(l*1.4+s*0.8)*1.8+Math.cos(s*1.1+l*0.5)*1.5;a.push({point:{x:l,y:s},score:c+r+(Math.random()-0.5)*3.2})}a.sort((s,l)=>s.score-l.score);for(let s of a)yield s.point;break}case"HUMAN_LATE_FIXES":{let a=[],s=[];for(let l=0;l<p;l++)for(let c=0;c<o;c++)if(Math.random()>0.9)s.push({x:c,y:l});else a.push({x:c,y:l});a.sort((l,c)=>l.y-c.y+(Math.random()-0.5)*1.5+(Math.random()>0.85?l.x-c.x:0)),s.sort((l,c)=>Math.hypot(c.x-o/2,c.y-p/2)-Math.hypot(l.x-o/2,l.y-p/2)),yield*a,yield*s;break}case"DIAGONAL_BRUSH":{for(let a=0;a<o+p-1;a++){let s=a%2===0,l=[],c=Math.max(0,a-o+1),r=Math.min(p-1,a);for(let g=c;g<=r;g++){let f=a-g;if(f>=0&&f<o)l.push({x:f,y:g})}if(Math.random()>0.55)l.reverse();if(s)for(let g=l.length-1;g>=0;g--)yield l[g];else yield*l}break}case"BRUSH_STROKES":{let a=Array.from({length:p},()=>Array(o).fill(!1)),s=[{x:1,y:0},{x:-1,y:0},{x:0,y:1},{x:0,y:-1},{x:1,y:1},{x:1,y:-1},{x:-1,y:1},{x:-1,y:-1}],l=(g,f)=>g>=0&&g<o&&f>=0&&f<p,c=0,r=o*p;for(let g=0;g<r*6&&c<r;g++){let f=Math.floor(Math.random()*o),w=Math.floor(Math.random()*p),u=s[Math.floor(Math.random()*s.length)],b=3+Math.floor(Math.random()*16);for(let m=0;m<b;m++){if(!l(f,w))break;if(!a[w][f])a[w][f]=!0,c++,yield{x:f,y:w};if(Math.random()>0.72)u=s[Math.floor(Math.random()*s.length)];f+=u.x,w+=u.y}}for(let g=0;g<p;g++)for(let f=0;f<o;f++)if(!a[g][f])yield{x:f,y:g};break}case"SPIRAL_FROM_CENTER":case"SPIRAL_TO_CENTER":{let a=new Set,s=o*p,l=Math.floor(o/2),c=Math.floor(p/2),r=[[1,0],[0,1],[-1,0],[0,-1]],g=0,f=1,w=(b,m)=>b>=0&&b<o&&m>=0&&m<p,u=function*(){let b=0;while(b<s){for(let m=0;m<2;m++){for(let n=0;n<f;n++){if(w(l,c)){let z=`${l},${c}`;if(!a.has(z)){if(a.add(z),yield{x:l,y:c},b++,b>=s)return}}l+=r[g][0],c+=r[g][1]}g=(g+1)%4}f++}};if(this.strategy==="SPIRAL_FROM_CENTER")yield*u();else{let b=[...u()];for(let m=b.length-1;m>=0;m--)yield b[m]}break}case"SCRIBBLE":{let a=new Set,s=o*p,l=Math.floor(o/2),c=Math.floor(p/2);for(let r=0;a.size<s&&r<s*24;r++){let g=`${l},${c}`;if(!a.has(g))a.add(g),yield{x:l,y:c};if(l+=Math.floor(Math.random()*3)-1,c+=Math.floor(Math.random()*3)-1,l<0||l>=o||c<0||c>=p)l=Math.floor(Math.random()*o),c=Math.floor(Math.random()*p)}for(let r=0;r<p;r++)for(let g=0;g<o;g++){let f=`${g},${r}`;if(a.has(f))continue;a.add(f),yield{x:g,y:r}}break}case"CROSSHATCH":{let a=[];for(let c=0;c<o+p-1;c++)for(let r=Math.max(0,c-o+1);r<=Math.min(p-1,c);r++){let g=c-r;a.push({x:g,y:r})}let s=[];for(let c=-p+1;c<o;c++)for(let r=0;r<p;r++){let g=r+c;if(g>=0&&g<o)s.push({x:g,y:r})}let l=new Set;for(let c of[...a,...s]){let r=`${c.x},${c.y}`;if(l.has(r))continue;l.add(r),yield c}break}case"WAVE_SWEEP":{let a=new Set;for(let s=0;s<o;s++){let c=(Math.sin(s/Math.max(1,o-1)*Math.PI*4)+1)*0.5*(p-1)|0;for(let r=0;r<p;r++){let g=c+r,f=c-r;for(let w of[g,f]){if(w<0||w>=p)continue;let u=`${s},${w}`;if(a.has(u))continue;a.add(u),yield{x:s,y:w}}}}break}case"SCATTERED_LINES":{let a=new Set,s=o*p;for(let l=0;a.size<s&&l<s*14;l++){let c=Math.floor(Math.random()*o),r=Math.floor(Math.random()*p),g=Math.random()*Math.PI*2,f=Math.round(Math.cos(g)),w=Math.round(Math.sin(g)),u=6+Math.floor(Math.random()*28);for(let b=0;b<u;b++){if(c<0||c>=o||r<0||r>=p)break;let m=`${c},${r}`;if(!a.has(m))a.add(m),yield{x:c,y:r};c+=f,r+=w}}for(let l=0;l<p;l++)for(let c=0;c<o;c++){let r=`${c},${l}`;if(a.has(r))continue;a.add(r),yield{x:c,y:l}}break}case"CONTOUR_JITTER":{let a=new Set;for(let s=0;s<Math.ceil(Math.min(o,p)/2);s++){let l=[],c=s,r=s,g=o-s-1,f=p-s-1;for(let w=c;w<=g;w++)l.push({x:w,y:r});for(let w=r+1;w<=f;w++)l.push({x:g,y:w});for(let w=g-1;w>=c;w--)l.push({x:w,y:f});for(let w=f-1;w>r;w--)l.push({x:c,y:w});for(let w=l.length-1;w>0;w--){let u=Math.floor(Math.random()*(w+1)),b=l[w];l[w]=l[u],l[u]=b}for(let w of l){let u=`${w.x},${w.y}`;if(a.has(u))continue;a.add(u),yield w}}break}case"SPIRAL_WOBBLE":{let a=new Set,s=o/2,l=p/2,c=Math.hypot(s,l);for(let r=0;a.size<o*p&&r<o*p*9;r++){let g=r/(o*p*9)*c,f=r*0.31+Math.sin(r*0.07)*0.7,w=Math.round(s+Math.cos(f)*g),u=Math.round(l+Math.sin(f)*g);if(w<0||w>=o||u<0||u>=p)continue;let b=`${w},${u}`;if(a.has(b))continue;a.add(b),yield{x:w,y:u}}for(let r=0;r<p;r++)for(let g=0;g<o;g++){let f=`${g},${r}`;if(a.has(f))continue;yield{x:g,y:r}}break}case"CLUSTER_BURSTS":{let a=new Set,s=o*p;for(let l=0;a.size<s&&l<s*12;l++){let c=Math.floor(Math.random()*o),r=Math.floor(Math.random()*p),g=2+Math.floor(Math.random()*10);for(let f=r-g;f<=r+g;f++)for(let w=c-g;w<=c+g;w++){if(w<0||w>=o||f<0||f>=p)continue;if(Math.hypot(w-c,f-r)>g)continue;let u=`${w},${f}`;if(a.has(u))continue;a.add(u),yield{x:w,y:f}}}for(let l=0;l<p;l++)for(let c=0;c<o;c++){let r=`${c},${l}`;if(a.has(r))continue;a.add(r),yield{x:c,y:l}}break}case"ORBITAL":{let a=new Set,s=(o-1)/2,l=(p-1)/2,c=Math.ceil(Math.max(s,l));for(let r=0;r<=c;r++){let g=Math.max(16,Math.ceil(2*Math.PI*Math.max(1,r)*2));for(let f=0;f<g;f++){let w=f/g*Math.PI*2+(r%2?0.3:-0.3),u=Math.round(s+Math.cos(w)*r),b=Math.round(l+Math.sin(w)*r);if(u<0||u>=o||b<0||b>=p)continue;let m=`${u},${b}`;if(a.has(m))continue;a.add(m),yield{x:u,y:b}}}for(let r=0;r<p;r++)for(let g=0;g<o;g++){let f=`${g},${r}`;if(a.has(f))continue;yield{x:g,y:r}}break}case"FLOW_FIELD":{let a=new Set,s=o*p;for(let l=0;a.size<s&&l<s*18;l++){let c=Math.floor(Math.random()*o),r=Math.floor(Math.random()*p);for(let g=0;g<120;g++){if(c<0||c>=o||r<0||r>=p)break;let f=`${c},${r}`;if(!a.has(f))a.add(f),yield{x:c,y:r};let w=Math.sin(c*0.09)*1.8+Math.cos(r*0.08)*1.6+Math.sin((c+r)*0.05);c+=Math.round(Math.cos(w)),r+=Math.round(Math.sin(w))}}for(let l=0;l<p;l++)for(let c=0;c<o;c++){let r=`${c},${l}`;if(a.has(r))continue;a.add(r),yield{x:c,y:l}}break}case"EDGE_IN":{let a=new Set,s=Math.ceil(Math.min(o,p)/2);for(let l=0;l<s;l++){let c=l,r=o-1-l,g=l,f=p-1-l;for(let w=c;w<=r;w++)for(let u of[g,f]){let b=`${w},${u}`;if(a.has(b))continue;a.add(b),yield{x:w,y:u}}for(let w=g+1;w<=f-1;w++)for(let u of[c,r]){let b=`${u},${w}`;if(a.has(b))continue;a.add(b),yield{x:u,y:w}}}break}}}moveStart(o){if(o.button!==0)return;if(o.preventDefault(),o.stopPropagation(),!this.lock)this.moveInfo={globalX:this.position.globalX,globalY:this.position.globalY,clientX:o.clientX,clientY:o.clientY}}moveStop(){if(this.moveInfo)this.moveInfo=void 0,this.position.updateAnchor(),this.pixels.update(),this.updateColors(),A(this.bot)}move(o){if(!this.moveInfo)return;let p=Math.round((o.clientX-this.moveInfo.clientX)/this.position.pixelSize),a=Math.round((o.clientY-this.moveInfo.clientY)/this.position.pixelSize);if(this.moveInfo.globalX!==void 0){if(this.position.globalX=p+this.moveInfo.globalX,this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,this.moveInfo.width-p)}else if(this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,p+this.moveInfo.width);if(this.moveInfo.globalY!==void 0){if(this.position.globalY=a+this.moveInfo.globalY,this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,this.moveInfo.height-a)}else if(this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,a+this.moveInfo.height);this.update(),A(this.bot)}resizeStart(o){if(this.lock||o.button!==0)return;o.preventDefault(),o.stopPropagation(),this.moveInfo={clientX:o.clientX,clientY:o.clientY};let p=o.target;if(p.classList.contains("n"))this.moveInfo.height=this.pixels.height,this.moveInfo.globalY=this.position.globalY;if(p.classList.contains("e"))this.moveInfo.width=this.pixels.width;if(p.classList.contains("s"))this.moveInfo.height=this.pixels.height;if(p.classList.contains("w"))this.moveInfo.width=this.pixels.width,this.moveInfo.globalX=this.position.globalX}export(){let o=document.createElement("a");document.body.append(o),o.href=URL.createObjectURL(new Blob([JSON.stringify(this.toJSON())],{type:"application/json"})),o.download=`${this.pixels.width}x${this.pixels.height}.${_}`,o.click(),URL.revokeObjectURL(o.href),o.href=this.pixels.canvas.toDataURL("image/webp",1),o.download=`${this.pixels.width}x${this.pixels.height}.webp`,o.click(),URL.revokeObjectURL(o.href),o.remove()}}var Jo=`/* stylelint-disable declaration-no-important */
...
`;var Po="kglacer-macro:overlay-hidden",_o="https://raw.githubusercontent.com/robgallardof/kglacer-macro/refs/heads/main/src/img/logo.svg";class po extends S{bot;element=document.createElement("div");get status(){return this.$status.innerHTML}set status(o){this.$status.innerHTML=o}get open(){return this.element.classList.contains("wopen")}set open(o){if(o)this.element.classList.add("wopen");else this.element.classList.remove("wopen")}$settings;$status;$shortcuts;$locale;$topbar;$draw;$addImage;$toggleOverlay;$strategy;$progressLine;$progressText;$images;$wopenButton;$widgetLogo;activeImageIndex=-1;constructor(o){super();this.bot=o;this.element.classList.add("wwidget"),this.element.innerHTML=Uo,h(this.element),document.body.append(this.element),this.populateElementsWithSelector(this.element,{$wopenButton:".wopen-button",$widgetLogo:".widget-logo",$settings:".wform",$status:".wstatus",$shortcuts:".shortcuts",$locale:".locale",$topbar:".wtopbar",$draw:".draw",$addImage:".add-image",$toggleOverlay:".toggle-overlay",$strategy:".strategy",$progressLine:".wprogress div",$progressText:".wprogress span",$images:".images"}),this.$widgetLogo.src=_o,this.$wopenButton.addEventListener("click",()=>this.open=!this.open),this.$draw.addEventListener("click",()=>this.bot.draw()),this.$addImage.addEventListener("click",()=>this.addImage()),this.$toggleOverlay.addEventListener("click",()=>{this.toggleOverlay()}),this.$strategy.addEventListener("change",()=>{this.bot.strategy=this.$strategy.value}),this.$locale.value=E(),this.$locale.addEventListener("change",()=>{v(this.$locale.value),h(this.element);for(let p=0;p<this.bot.images.length;p++)this.bot.images[p].applyLocale();this.refreshOverlayToggleText()}),this.registerEvent(document,"keydown",this.handleKeyboard.bind(this),{passive:!1}),this.update(),this.syncOverlayVisibilityFromStorage(),this.open=!0,console.log("[KGM][Widget] Widget mounted and opened")}addImage(){return console.log("[KGM][Widget] Add image flow started"),this.setDisabled("add-image",!0),this.run("Adding image",async()=>{await this.bot.updateColors();let o=document.createElement("input");o.type="file",o.accept=`image/*,.${_}`,o.click(),await Q(o,["change"],["cancel","error"]);let p=o.files?.[0];if(!p)throw new ao(this.bot);console.log("[KGM][Widget] File selected",{name:p.name,size:p.size,type:p.type});let a;if(p.name.endsWith(`.${_}`))a=await q.fromJSON(this.bot,JSON.parse(await p.text()));else{let s=new FileReader;s.readAsDataURL(p),await Q(s,["load"],["error"]);let l=await this.compressImageBeforeLoad(s.result),c=new Image;c.src=l,await Q(c,["load"],["error"]),a=new q(this.bot,J.fromScreenPosition(this.bot,this.defaultImageScreenPosition()),new B(this.bot,c))}this.bot.images.push(a),console.log("[KGM][Widget] Image instance added",{images:this.bot.images.length}),await this.bot.readMap(),a.updateTasks(),A(this.bot,!0),console.log("[KGM][Widget] Image persisted, reloading page"),document.location.reload()},()=>{this.setDisabled("add-image",!1)})}defaultImageScreenPosition(){let o=Math.round(this.element.getBoundingClientRect().width);return{x:Math.max(256,o),y:32}}async compressImageBeforeLoad(o){let p=new Image;if(p.src=o,await Q(p,["load"],["error"]),!(p.naturalWidth*p.naturalHeight>3000000||o.length>3000000))return o;let s=document.createElement("canvas");s.width=p.naturalWidth,s.height=p.naturalHeight;let l=s.getContext("2d");if(!l)return o;return l.drawImage(p,0,0),s.toDataURL("image/png")}update(){this.$strategy.value=this.bot.strategy;let o=0,p=0;for(let c=0;c<this.bot.images.length;c++){let r=this.bot.images[c];o+=r.pixels.pixels.length*r.pixels.pixels[0].length,p+=r.tasks.length}let a=Math.max(0,o-p),s=o>0?a/o*100|0:0;this.$progressText.textContent=`${a}/${o} ${s}% ETA: ${p/120|0}h`,this.$progressLine.style.transform=`scaleX(${s/100})`,this.$images.innerHTML="";let l=document.createDocumentFragment();for(let c=0;c<this.bot.images.length;c++){let r=this.bot.images[c],g=document.createElement("div");l.append(g),g.className="image",g.innerHTML=`<button class="preview" title="View preview">
  ...
  </div>`,g.querySelector(".preview").addEventListener("click",()=>{this.activeImageIndex=c,r.openPreviewPanel()}),g.querySelector(".colors").addEventListener("click",()=>{this.activeImageIndex=c,r.openColorPanel()}),g.querySelector(".preview-strategy").addEventListener("click",()=>{this.activeImageIndex=c,r.openPreviewPanel()}),g.querySelector(".download").addEventListener("click",()=>{r.exportImage()}),g.querySelector(".delete").addEventListener("click",()=>{r.destroy()}),g.querySelector(".up").addEventListener("click",()=>{y(this.bot.images,c,c-1),this.update(),A(this.bot)}),g.querySelector(".down").addEventListener("click",()=>{y(this.bot.images,c,c+1),this.update(),A(this.bot)})}this.$images.append(l)}syncOverlayVisibilityFromStorage(){let o=localStorage.getItem(Po)==="true";document.body.classList.toggle("overlay-hidden",o),this.refreshOverlayToggleText()}toggleOverlay(o){let p=o??!document.body.classList.contains("overlay-hidden");document.body.classList.toggle("overlay-hidden",p),localStorage.setItem(Po,String(p)),this.refreshOverlayToggleText()}refreshOverlayToggleText(){this.$toggleOverlay.textContent=document.body.classList.contains("overlay-hidden")?`${d("toggleOverlay")} (${d("disabled")})`:`${d("toggleOverlay")} (${d("enabled")})`}setDisabled(o,p){this.element.querySelector("."+o).disabled=p}async run(o,p,a,s="..."){console.log("[KGM][Widget] Task started",{status:o});let l=this.status;this.status=`${s} ${o}`;try{let c=await p();return this.status=l,console.log("[KGM][Widget] Task completed",{status:o}),c}catch(c){if(!(c instanceof I))console.error(c),this.status=`Error: ${o}`;throw console.error("[KGM][Widget] Task failed",{status:o,error:c}),c}finally{await a?.()}}handleKeyboard(o){if(No(o.target))return;if(U(o,N.toggleWidget)){o.preventDefault(),this.open=!this.open;return}if(U(o,N.showShortcuts)){o.preventDefault(),this.open=!0,this.$shortcuts.open=!0,this.$shortcuts.scrollIntoView({behavior:"smooth",block:"center"}),this.$shortcuts.classList.remove("shortcut-pulse"),requestAnimationFrame(()=>{this.$shortcuts.classList.add("shortcut-pulse")});return}if(U(o,N.toggleOverlay)){o.preventDefault(),this.toggleOverlay();return}if(U(o,N.focusNextImage)){o.preventDefault(),this.focusImageByStep(1);return}if(U(o,N.focusPreviousImage)){o.preventDefault(),this.focusImageByStep(-1);return}if(U(o,N.openColorPanel)){o.preventDefault(),this.openColorPanelForActiveImage();return}if(U(o,N.toggleImageLock)){o.preventDefault(),this.toggleLockForActiveImage();return}if(U(o,N.addImage)&&!this.$addImage.disabled){o.preventDefault(),this.addImage();return}if(U(o,N.draw)&&!this.$draw.disabled)o.preventDefault(),this.bot.draw()}focusImageByStep(o){if(!this.bot.images.length)return;if(this.activeImageIndex<0||this.activeImageIndex>=this.bot.images.length)this.activeImageIndex=o>0?0:this.bot.images.length-1;else this.activeImageIndex=(this.activeImageIndex+o+this.bot.images.length)%this.bot.images.length;this.bot.images[this.activeImageIndex].position.scrollScreenTo()}getActiveImage(){if(!this.bot.images.length)return;if(this.activeImageIndex<0||this.activeImageIndex>=this.bot.images.length)this.activeImageIndex=0;return this.bot.images[this.activeImageIndex]}openColorPanelForActiveImage(){let o=this.getActiveImage();if(!o)return;o.openColorPanel()}toggleLockForActiveImage(){let o=this.getActiveImage();if(!o)return;o.lock=!o.lock,o.update(),A(this.bot)}}var To=2,ho="[KGM]",Do="kglacer-macro:access-ok",so="S0dNLXlZUjhTMW81bEhVemVjS1RFMEhxRVB4OVFkcjgxaEVz",Ko="kgm-access-locked";class Bo{unavailableColors=new Set;mapsCache=new Map;me;$stars=[];strategy="SEQUENTIAL";images=[];_widget;get widget(){if(!this._widget)throw new Error("Widget is not initialized yet");return this._widget}markerPixelPositionResolvers=[];lastColor;log(o,p){if(p===void 0)console.log(`${ho} ${o}`);else console.log(`${ho} ${o}`,p)}constructor(){this.log("Boot sequence started"),document.body.classList.add(Ko);let o=Go();if(this.log("Save loaded",{hasSave:Boolean(o),imageCount:o?.images.length??0,strategy:o?.strategy}),o){for(let a=0;a<o.images.length;a++){let s=o.images[a];T({x:s.position[0]-1000,y:s.position[1]-1000}),T({x:s.position[0]+1000,y:s.position[1]+1000})}this.strategy=o.strategy}this.registerFetchInterceptor(),this.log("Fetch interceptor registered");let p=document.createElement("style");p.textContent=Jo.replace("FAKE_FAVORITE_LOCATIONS",R.length.toString()),document.head.append(p),this.log("Styles injected",{fakeFavoriteLocations:R.length}),(async()=>{this.log("Widget initialization flow started"),await this.ensureAccessKey(),document.body.classList.remove(Ko),this._widget=new po(this),await this.widget.run("Initializing",async()=>{await this.waitForElement("login",".avatar.center-absolute.absolute"),await this.waitForElement("pixel count",".btn.btn-primary.btn-lg.relative.z-30 canvas");let a=await this.waitForElement("canvas",".maplibregl-canvas-container");if(new MutationObserver((s)=>{for(let l=0;l<s.length;l++)if(s[l].removedNodes.length!==0){this.updateStars();break}this.updateImages()}).observe(a,{attributes:!0,childList:!0,subtree:!0}),this.updateStars(),this.log("Stars updated after boot",{stars:this.$stars.length}),await P(500),await this.updateColors(),o)for(let s=0;s<o.images.length;s++){let l=await q.fromJSON(this,o.images[s]);this.images.push(l),l.update()}this.log("Saved images restored",{images:this.images.length}),await this.readMap(),this.updateTasks(),this.widget.setDisabled("draw",!1),this.widget.setDisabled("add-image",!1),this.log("Initialization completed; controls enabled")})})()}async ensureAccessKey(){if(localStorage.getItem(Do)===so)return;await new Promise((o)=>{let p=document.createElement("dialog");p.className="kgm-modal access-dialog",p.innerHTML=`<form method="dialog" class="access-form">
...
</form>`,document.body.append(p),h(p);let a=p.querySelector(".access-input"),s=p.querySelector(".access-error"),l=p.querySelector(".access-locale");l.innerHTML=mo().map((c)=>`<option value="${c}" ${c===E()?"selected":""}>${c.toUpperCase()}</option>`).join(""),l.addEventListener("change",()=>{v(l.value),h(p)}),p.addEventListener("cancel",(c)=>{c.preventDefault()}),p.querySelector("form").addEventListener("submit",(c)=>{c.preventDefault();let r=atob(so);if(a.value.trim()!==r){s.textContent=d("invalidAccessKey");return}localStorage.setItem(Do,so),p.close(),p.remove(),o()}),p.showModal(),a.focus()})}draw(){this.log("Draw requested",{strategy:this.strategy,images:this.images.length}),this.widget.setDisabled("draw",!0),this.widget.status="",this.mapsCache.clear();let o=document.querySelector(".maplibregl-canvas"),p=(a)=>{if(!a.shiftKey)a.stopPropagation()};return this.widget.run("Drawing",async()=>{await this.widget.run("Initializing draw",()=>Promise.all([this.updateColors(),this.readMap()])),globalThis.addEventListener("mousemove",p,!0),o.addEventListener("wheel",p,!0),this.updateTasks();let a=await fetch("https://backend.wplace.live/me",{credentials:"include"}).then((c)=>c.json()),s=Math.floor(a.charges.count);this.log("Charges fetched",{charges:s});let l=0;for(let c=0;c<this.images.length;c++)l+=this.images[c].tasks.length;switch(this.log("Tasks prepared",{tasks:l}),this.strategy){case"ALL":{while(s>0){let c=!0;for(let r=0;r<this.images.length;r++){let g=this.images[r].tasks.shift();if(!g)continue;this.drawTask(g),s--,await P(1),c=!1}if(c)break}break}case"PERCENTAGE":{for(let c=0;c<l&&s>0;c++){let r=1,g;for(let f=0;f<this.images.length;f++){let w=this.images[f],u=1-w.tasks.length/(w.pixels.pixels.length*w.pixels.pixels[0].length);if(u<r)r=u,g=w}this.drawTask(g.tasks.shift()),s--,await P(1)}break}case"SEQUENTIAL":for(let c=0;c<this.images.length;c++){let r=this.images[c];for(let g=r.tasks.shift();g&&s>0;g=r.tasks.shift())this.drawTask(g),s--,await P(1)}}this.widget.update(),this.log("Draw flow finished",{remainingCharges:s})},()=>{globalThis.removeEventListener("mousemove",p,!0),o.removeEventListener("wheel",p,!0),this.widget.setDisabled("draw",!1)})}toJSON(){return{version:To,images:this.images.map((o)=>o.toJSON()),strategy:this.strategy}}async updateColors(){this.log("Updating colors palette"),await this.openColors(),this.unavailableColors.clear();for(let o of document.querySelectorAll("button.btn.relative.w-full"))if(o.children.length!==0)this.unavailableColors.add(Math.abs(Number.parseInt(o.id.slice(6))));this.updateImageColors(),this.log("Colors updated",{unavailableColors:this.unavailableColors.size})}moveMap(o){let p=document.querySelector(".maplibregl-canvas"),a=window.innerWidth/2,s=window.innerHeight/2,l=a-o.x,c=s-o.y;function r(g,f,w){p.dispatchEvent(new MouseEvent(g,{bubbles:!0,cancelable:!0,clientX:f,clientY:w,buttons:1}))}r("mousedown",a,s),r("mousemove",l,c),r("mouseup",l,c)}readMap(){this.mapsCache.clear();let o=new Set;for(let a=0;a<this.images.length;a++){let s=this.images[a],{tileX:l,tileY:c}=new J(this,s.position.globalX+s.pixels.pixels[0].length,s.position.globalY+s.pixels.pixels.length);for(let r=s.position.tileX;r<=l;r++)for(let g=s.position.tileY;g<=c;g++)o.add(`${r}/${g}`)}let p=0;return this.log("Reading map tiles",{tileCount:o.size}),this.widget.run(`Reading map [0/${o.size}]`,()=>Promise.all([...o].map(async(a)=>{this.mapsCache.set(a,await B.fromJSON(this,{url:`https://backend.wplace.live/files/s0/tiles/${a}.png`,exactColor:!0})),this.widget.status=`⌛ Reading map [${++p}/${o.size}]`})))}waitForUnfocus(){return this.widget.run("UNFOCUS WINDOW",()=>new Promise((o)=>{if(!document.hasFocus())o();window.addEventListener("blur",()=>{setTimeout(o,1)},{once:!0})}),void 0,"\uD83D\uDDB1️")}findAnchorsForScreen(o){let p=0,a=1,s=1/0,l=1/0;for(let g=0;g<this.$stars.length;g++){let{x:f,y:w}=Y(this.$stars[g]);if(f<o.x&&w<o.y){let u=o.x-f+(o.y-w);if(u<s)s=u,p=g}else if(f>o.x&&w>o.y){let u=f-o.x+(w-o.y);if(u<l)l=u,a=g}}let c=Y(this.$stars[p]),r=D[p];return{anchorScreenPosition:c,anchorWorldPosition:r,pixelSize:(Y(this.$stars[a]).x-c.x)/(D[a].x-r.x)}}async openColors(){this.lastColor=void 0,document.querySelector(".flex.gap-2.px-3 > .btn-circle")?.click(),await P(1),document.querySelector(".btn.btn-primary.btn-lg.relative.z-30")?.click(),await P(1);let o=document.querySelector("button.bottom-0");if(o?.innerHTML==='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor" class="size-5"><path d="M480-120 300-300l58-58 122 122 122-122 58 58-180 180ZM358-598l-58-58 180-180 180 180-58 58-122-122-122 122Z"></path></svg><!---->')o.click(),await P(1)}drawTask(o){if(this.lastColor!==o.color)document.getElementById("color-"+o.color).click(),this.lastColor=o.color,this.log("Color switched for draw task",{color:o.color});let p=o.position.pixelSize/2,a=o.position.toScreenPosition();document.documentElement.dispatchEvent(new MouseEvent("mousemove",{bubbles:!0,clientX:a.x+p,clientY:a.y+p,shiftKey:!0})),document.documentElement.dispatchEvent(new KeyboardEvent("keydown",{key:" ",code:"Space",keyCode:32,which:32,bubbles:!0,cancelable:!0})),document.documentElement.dispatchEvent(new KeyboardEvent("keyup",{key:" ",code:"Space",keyCode:32,which:32,bubbles:!0,cancelable:!0}))}registerFetchInterceptor(){let o=globalThis.fetch,p=/https:\/\/backend.wplace.live\/s\d+\/pixel\/(-?\d+)\/(-?\d+)\?x=(-?\d+)&y=(-?\d+)/;globalThis.fetch=async(a,s)=>{let l=await o(a,s),c=l.clone(),r="";if(typeof a=="string")r=a;else if(a instanceof Request)r=a.url;else if(a instanceof URL)r=a.href;if(l.url==="https://backend.wplace.live/me")this.me=await c.json(),this.me.favoriteLocations.unshift(...R),this.me.maxFavoriteLocations=1/0,l.json=()=>Promise.resolve(this.me),this.log("Patched /me response with favorite locations",{totalFavorites:this.me.favoriteLocations.length});let g=p.exec(r);if(g){for(let f=0;f<this.markerPixelPositionResolvers.length;f++)this.markerPixelPositionResolvers[f](new J(this,+g[1],+g[2],+g[3],+g[4]));this.markerPixelPositionResolvers.length=0,this.log("Resolved marker pixel position from network event")}return l}}async closeAll(){for(let o of document.querySelectorAll("button"))if(o.innerHTML==="✕"||o.innerHTML==='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor" class="size-4"><path d="m256-200-56-56 224-224-224-224 56-56 224 224 224-224 56 56-224 224 224 224-56 56-224-224-224 224Z"></path></svg><!---->')o.click(),await P(1)}waitForElement(o,p){return this.log("Waiting for element",{name:o,selector:p}),this.widget.run(`Waiting for ${o}`,()=>{return new Promise((a)=>{let s=document.querySelector(p);if(s){a(s);return}let l=new MutationObserver(()=>{let c=document.querySelector(p);if(c)l.disconnect(),a(c)});l.observe(document.documentElement,{childList:!0,subtree:!0})})})}updateStars(){this.$stars=[...document.querySelectorAll(".text-yellow-400.cursor-pointer.z-10.maplibregl-marker.maplibregl-marker-anchor-center")].slice(0,R.length),this.log("Star cache updated",{stars:this.$stars.length})}updateImages(){for(let o=0;o<this.images.length;o++)this.images[o].position.updateAnchor(),this.images[o].update()}updateTasks(){for(let o=0;o<this.images.length;o++)this.images[o].updateTasks()}updateImageColors(){for(let o=0;o<this.images.length;o++)this.images[o].updateColors()}}globalThis.kglacerMacro=new Bo;globalThis.kgm=globalThis.kglacerMacro;globalThis.wbot=globalThis.kglacerMacro;
