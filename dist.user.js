...
<span class="premium ${r?"on":""}">${r?d("premium"):""}</span>`,b.querySelector(".swatch").style.setProperty("--swatch-color",no(a.realColor)),b.addEventListener("click",()=>{if(c){c=!1;return}u()}),b.addEventListener("dragstart",(z)=>{c=!0,b.classList.add("dragging"),z.dataTransfer?.setData("text/plain",String(l)),z.dataTransfer.effectAllowed="move"}),b.addEventListener("dragend",()=>{b.classList.remove("dragging")}),b.addEventListener("dragover",(z)=>{z.preventDefault(),b.classList.add("drag-target")}),b.addEventListener("dragleave",()=>{b.classList.remove("drag-target")}),b.addEventListener("drop",(z)=>{z.preventDefault(),b.classList.remove("drag-target");let H=Number.parseInt(z.dataTransfer?.getData("text/plain")??"-1",10);if(H<0||H===l||H>=this.colors.length)return;this.colors.splice(l,0,...this.colors.splice(H,1)),J(this.bot),this.updateColors()}),r){let z=document.createElement("button");z.textContent=d("buy"),z.className="buy-chip",z.addEventListener("click",(H)=>{H.stopPropagation(),document.getElementById("color-"+a.realColor)?.click()}),b.append(z)}let m=`${f} ${w.join(" ")} ${a.realColor} ${Z[a.realColor]}`;if(!s||m.toLowerCase().includes(s))this.$colorsDialogList.append(b)}}*strategyPositionIterator(){let o=this.pixels.pixels[0].length,s=this.pixels.pixels.length;switch(this.strategy){case"DOWN":{for(let l=0;l<s;l++)for(let p=0;p<o;p++)yield{x:p,y:l};break}case"UP":{for(let l=s-1;l>=0;l--)for(let p=0;p<o;p++)yield{x:p,y:l};break}case"LEFT":{for(let l=0;l<o;l++)for(let p=0;p<s;p++)yield{x:l,y:p};break}case"RIGHT":{for(let l=o-1;l>=0;l--)for(let p=0;p<s;p++)yield{x:l,y:p};break}case"RANDOM":{let l=[];for(let p=0;p<s;p++)for(let a=0;a<o;a++)l.push({x:a,y:p});for(let p=l.length-1;p>=0;p--){let a=Math.floor(Math.random()*(p+1)),c=l[p];l[p]=l[a],l[a]=c}yield*l;break}case"ZIGZAG":{for(let l=0;l<s;l++)if(l%2===0)for(let p=0;p<o;p++)yield{x:p,y:l};else for(let p=o-1;p>=0;p--)yield{x:p,y:l};break}case"HUMANIZED":{let l=Math.max(4,Math.floor(Math.min(o,s)/10)),p=[];for(let a=0;a<s;a+=l)for(let c=0;c<o;c+=l)p.push({x:c,y:a});for(let a=p.length-1;a>=0;a--){let c=Math.floor(Math.random()*(a+1)),r=p[a];p[a]=p[c],p[c]=r}for(let a=0;a<p.length;a++){let c=p[a],r=Math.min(s,c.y+l),g=Math.min(o,c.x+l);for(let f=c.y;f<r;f++)if(Math.random()>0.35)for(let u=c.x;u<g;u++)yield{x:u,y:f};else for(let u=g-1;u>=c.x;u--)yield{x:u,y:f}}break}case"HUMAN_SOFT_DITHER":{let l=new Set;for(let p=0;p<s;p++){let a=Math.floor(Math.random()*3)-1;if((p+a)%2===0)for(let r=0;r<o;r+=2)l.add(`${r},${p}`),yield{x:r,y:p};else for(let r=1;r<o;r+=2)l.add(`${r},${p}`),yield{x:r,y:p}}for(let p=0;p<s;p++)for(let a=0;a<o;a++){let c=`${a},${p}`;if(l.has(c))continue;yield{x:a,y:p}}break}case"HUMAN_PATCHY":{let l=new Set,p=o*s;while(l.size<p){let a=Math.floor(Math.random()*o),c=Math.floor(Math.random()*s),r=1+Math.floor(Math.random()*5);for(let g=c-r;g<=c+r;g++)for(let f=a-r;f<=a+r;f++){if(f<0||f>=o||g<0||g>=s)continue;if(Math.hypot(f-a,g-c)>r+Math.random()*1.2)continue;let w=`${f},${g}`;if(l.has(w))continue;l.add(w),yield{x:f,y:g}}}break}case"HUMAN_SWEEP_ARCS":{let l=new Set,p=(o-1)/2,a=(s-1)/2,c=Math.hypot(p,a);for(let r=0;r<4;r++){let g=Math.random()*Math.PI*2;for(let f=0;f<=c;f+=0.35){let w=Math.PI/2+Math.random()*(Math.PI/1.5),u=Math.max(10,Math.floor(f*8));for(let b=0;b<u;b++){let m=g+w*b/u+Math.sin(f)*0.08,z=Math.round(p+Math.cos(m)*f),H=Math.round(a+Math.sin(m)*f);if(z<0||z>=o||H<0||H>=s)continue;let M=`${z},${H}`;if(l.has(M))continue;l.add(M),yield{x:z,y:H}}}}for(let r=0;r<s;r++)for(let g=0;g<o;g++){let f=`${g},${r}`;if(l.has(f))continue;yield{x:g,y:r}}break}case"HUMAN_MICRO_CORRECTIONS":{let l=new Set;for(let p=0;p<s;p++){let a=p%2===0?1:-1,c=a>0?0:o-1;for(let r=0;r<o;r++){let g=c+(Math.random()>0.82?a:0),f=p+(Math.random()>0.9?1:0);for(let w of[{x:c,y:p},{x:g,y:p},{x:c,y:f}]){if(w.x<0||w.x>=o||w.y<0||w.y>=s)continue;let u=`${w.x},${w.y}`;if(l.has(u))continue;l.add(u),yield w}c+=a}}for(let p=0;p<s;p++)for(let a=0;a<o;a++){let c=`${a},${p}`;if(l.has(c))continue;yield{x:a,y:p}}break}case"HUMAN_JITTER_FILL":{let l=[];for(let p=0;p<s;p++)for(let a=0;a<o;a++)l.push({x:a,y:p});l.sort((p,a)=>{let c=p.y+(Math.random()-0.5)*1.8,r=a.y+(Math.random()-0.5)*1.8;if(c!==r)return c-r;return p.x+(Math.random()-0.5)*2-(a.x+(Math.random()-0.5)*2)}),yield*l;break}case"HUMAN_CORNER_BIAS":{let l=[{x:0,y:0},{x:o-1,y:0},{x:0,y:s-1},{x:o-1,y:s-1}],p=l[Math.floor(Math.random()*l.length)],a=[];for(let c=0;c<s;c++)for(let r=0;r<o;r++){let f=Math.hypot(r-p.x,c-p.y)+Math.random()*3.5;a.push({point:{x:r,y:c},score:f})}a.sort((c,r)=>c.score-r.score);for(let c of a)yield c.point;break}case"HUMAN_LONG_STROKES":{let l=new Set,p=o*s;while(l.size<p){let a=Math.floor(Math.random()*o),c=Math.floor(Math.random()*s),r=Math.random()*Math.PI*2,g=Math.sign(Math.cos(r)),f=Math.sign(Math.sin(r)),w=10+Math.floor(Math.random()*40);for(let u=0;u<w;u++){if(a<0||a>=o||c<0||c>=s)break;let b=`${a},${c}`;if(!l.has(b))l.add(b),yield{x:a,y:c};if(Math.random()>0.78)a+=f,c+=g;else a+=g,c+=f}}break}case"HUMAN_TAP_CLUSTERS":{let l=new Set,p=o*s;while(l.size<p){let a=Math.floor(Math.random()*o),c=Math.floor(Math.random()*s),r=3+Math.floor(Math.random()*10);for(let g=0;g<r;g++){let f=Math.round(a+(Math.random()-0.5)*6),w=Math.round(c+(Math.random()-0.5)*6);if(f<0||f>=o||w<0||w>=s)continue;let u=`${f},${w}`;if(l.has(u))continue;l.add(u),yield{x:f,y:w}}}break}case"HUMAN_MESSY_SPIRAL":{let l=new Set,p=(o-1)/2,a=(s-1)/2,c=Math.hypot(p,a)+2;for(let r=0;l.size<o*s;r++){let g=r/3,f=Math.min(c,g*0.18),w=g*0.29+Math.sin(g*0.13)*0.8,u=Math.round(p+Math.cos(w)*f+Math.sin(g)*0.7),b=Math.round(a+Math.sin(w)*f+Math.cos(g)*0.7);if(u<0||u>=o||b<0||b>=s){if(r>o*s*18)break;continue}let m=`${u},${b}`;if(l.has(m)){if(Math.random()>0.9)continue}else l.add(m),yield{x:u,y:b};if(r>o*s*18)break}for(let r=0;r<s;r++)for(let g=0;g<o;g++){let f=`${g},${r}`;if(l.has(f))continue;yield{x:g,y:r}}break}case"HUMAN_DRUNK_WALK":{let l=new Set,p=Math.floor(Math.random()*o),a=Math.floor(Math.random()*s),c=[{x:-1,y:0},{x:1,y:0},{x:0,y:-1},{x:0,y:1},{x:-1,y:-1},{x:1,y:-1},{x:-1,y:1},{x:1,y:1}];while(l.size<o*s){let r=`${p},${a}`;if(!l.has(r))l.add(r),yield{x:p,y:a};let g=[];for(let u of c){let b=p+u.x,m=a+u.y;if(b<0||b>=o||m<0||m>=s)continue;g.push({x:b,y:m})}if(!g.length)break;let f=g.filter((u)=>{return!l.has(`${u.x},${u.y}`)});if(f.length&&Math.random()>0.2){let u=f[Math.floor(Math.random()*f.length)];p=u.x,a=u.y;continue}let w=g[Math.floor(Math.random()*g.length)];p=w.x,a=w.y}for(let r=0;r<s;r++)for(let g=0;g<o;g++){let f=`${g},${r}`;if(l.has(f))continue;yield{x:g,y:r}}break}case"HUMAN_NOISE_CLOUD":{let l=[];for(let p=0;p<s;p++)for(let a=0;a<o;a++){let c=Math.sin((a+1)*0.93+Math.random()*0.8)+Math.cos((p+1)*1.17+Math.random()*0.8),r=(Math.random()-0.5)*2.6,g=Math.hypot(a-o/2,p-s/2)*0.08;l.push({point:{x:a,y:p},score:c+r+g})}l.sort((p,a)=>p.score-a.score);for(let p of l)yield p.point;break}case"HUMAN_PATCH_JUMP":{let l=new Set,p=[];for(let a=0;a<Math.max(6,o*s/18);a++)p.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*s)});while(l.size<o*s){let a=p[Math.floor(Math.random()*p.length)],c=1+Math.floor(Math.random()*3),r=1+Math.floor(Math.random()*3);for(let g=a.y-r;g<=a.y+r;g++)for(let f=a.x-c;f<=a.x+c;f++){if(f<0||f>=o||g<0||g>=s)continue;if(Math.random()>0.86)continue;let w=`${f},${g}`;if(l.has(w))continue;l.add(w),yield{x:f,y:g}}if(Math.random()>0.72&&p.length<o*s/2)p.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*s)});if(l.size>o*s*0.92)break}for(let a=0;a<s;a++)for(let c=0;c<o;c++){let r=`${c},${a}`;if(l.has(r))continue;yield{x:c,y:a}}break}case"HUMAN_HESITANT_LINES":{let l=new Set;for(let p=0;p<s;p++){let a=p%2===0;for(let c=0;c<o;c++){let r=a?c:o-1-c,g=`${r},${p}`;if(!l.has(g))l.add(g),yield{x:r,y:p};if(Math.random()>0.7){let f=Math.max(0,Math.min(o-1,r+(Math.random()>0.5?1:-1))),w=Math.max(0,Math.min(s-1,p+(Math.random()>0.65?1:0))),u=`${f},${w}`;if(!l.has(u))l.add(u),yield{x:f,y:w}}}}for(let p=0;p<s;p++)for(let a=0;a<o;a++){let c=`${a},${p}`;if(l.has(c))continue;yield{x:a,y:p}}break}case"HUMAN_OVERLAP_SWEEPS":{let l=[],p=Math.random()*Math.PI*2;for(let a=0;a<s;a++)for(let c=0;c<o;c++){let r=Math.sin((c+a)*0.42+p)*2.2,g=Math.cos((c-a)*0.3+p)*1.4;l.push({point:{x:c,y:a},score:a+r+g+(Math.random()-0.5)*3.4})}l.sort((a,c)=>a.score-c.score);for(let a of l)yield a.point;break}case"HUMAN_WOBBLE_DRIFT":{let l=[],p=o/2,a=s/2;for(let c=0;c<s;c++)for(let r=0;r<o;r++){let g=Math.hypot(r-p,c-a)*0.25,f=Math.sin((r+1)*0.9)*1.8+Math.cos((c+1)*1.1)*1.8+Math.sin((r+c)*0.35)*1.4;l.push({point:{x:r,y:c},score:g+f+(Math.random()-0.5)*2.8})}l.sort((c,r)=>c.score-r.score);for(let c of l)yield c.point;break}case"HUMAN_GAP_RECOVERY":{let l=new Set,p=[];for(let a=0;a<s;a++)for(let c=0;c<o;c++){if(Math.random()>0.87){p.push({x:c,y:a});continue}l.add(`${c},${a}`),yield{x:c,y:a}}p.sort((a,c)=>Math.hypot(a.x-o/2,a.y-s/2)-Math.hypot(c.x-o/2,c.y-s/2));for(let a of p){let c=`${a.x},${a.y}`;if(l.has(c))continue;l.add(c),yield a}break}case"HUMAN_STAIRCASE":{let l=new Set,p=o+s-1;for(let a=0;a<p;a++){let c=Math.max(0,a-o+1),r=Math.min(s-1,a);for(let g=c;g<=r;g++){let f=a-g,w=[{x:f,y:g},{x:f+(Math.random()>0.5?1:-1),y:g},{x:f,y:g+(Math.random()>0.5?1:-1)}];for(let u of w){if(u.x<0||u.x>=o||u.y<0||u.y>=s)continue;let b=`${u.x},${u.y}`;if(l.has(b))continue;l.add(b),yield u}}}for(let a=0;a<s;a++)for(let c=0;c<o;c++){let r=`${c},${a}`;if(l.has(r))continue;yield{x:c,y:a}}break}case"HUMAN_EDGE_HUGGER":{let l=[];for(let p=0;p<s;p++)for(let a=0;a<o;a++){let c=Math.min(a,p,o-1-a,s-1-p);l.push({point:{x:a,y:p},score:c*3.5+(Math.random()-0.5)*5.5})}l.sort((p,a)=>p.score-a.score);for(let p of l)yield p.point;break}case"HUMAN_BLOBS":{let l=new Set,p=o*s;while(l.size<p){let a=Math.floor(Math.random()*o),c=Math.floor(Math.random()*s),r=1+Math.floor(Math.random()*4);for(let g=c-r;g<=c+r;g++)for(let f=a-r;f<=a+r;f++){if(f<0||f>=o||g<0||g>=s)continue;let w=Math.atan2(g-c,f-a),u=r+Math.sin(w*3+Math.random())*0.8;if(Math.hypot(f-a,g-c)>u)continue;let b=`${f},${g}`;if(l.has(b))continue;l.add(b),yield{x:f,y:g}}}break}case"HUMAN_BACKTRACK":{let l=new Set,p=[];for(let a=0;a<s;a++)for(let c=0;c<o;c++)p.push({x:c,y:a});p.sort((a,c)=>a.y-c.y+(Math.random()-0.5)*2.2+(a.x-c.x)*0.04);for(let a=0;a<p.length;a++){let c=p[a],r=`${c.x},${c.y}`;if(l.has(r))continue;if(l.add(r),yield c,a>1&&Math.random()>0.74){let g=p[a-1],f=`${g.x},${g.y}`;if(!l.has(f))l.add(f),yield g}}for(let a=0;a<s;a++)for(let c=0;c<o;c++){let r=`${c},${a}`;if(l.has(r))continue;yield{x:c,y:a}}break}case"HUMAN_SHAKY_DIAGONAL":{let l=[];for(let p=0;p<s;p++)for(let a=0;a<o;a++){let c=Math.abs(a-p)*0.6,r=Math.sin(a*1.4+p*0.8)*1.8+Math.cos(p*1.1+a*0.5)*1.5;l.push({point:{x:a,y:p},score:c+r+(Math.random()-0.5)*3.2})}l.sort((p,a)=>p.score-a.score);for(let p of l)yield p.point;break}case"HUMAN_LATE_FIXES":{let l=[],p=[];for(let a=0;a<s;a++)for(let c=0;c<o;c++)if(Math.random()>0.9)p.push({x:c,y:a});else l.push({x:c,y:a});l.sort((a,c)=>a.y-c.y+(Math.random()-0.5)*1.5+(Math.random()>0.85?a.x-c.x:0)),p.sort((a,c)=>Math.hypot(c.x-o/2,c.y-s/2)-Math.hypot(a.x-o/2,a.y-s/2)),yield*l,yield*p;break}case"DIAGONAL_BRUSH":{for(let l=0;l<o+s-1;l++){let p=l%2===0,a=[],c=Math.max(0,l-o+1),r=Math.min(s-1,l);for(let g=c;g<=r;g++){let f=l-g;if(f>=0&&f<o)a.push({x:f,y:g})}if(Math.random()>0.55)a.reverse();if(p)for(let g=a.length-1;g>=0;g--)yield a[g];else yield*a}break}case"BRUSH_STROKES":{let l=Array.from({length:s},()=>Array(o).fill(!1)),p=[{x:1,y:0},{x:-1,y:0},{x:0,y:1},{x:0,y:-1},{x:1,y:1},{x:1,y:-1},{x:-1,y:1},{x:-1,y:-1}],a=(g,f)=>g>=0&&g<o&&f>=0&&f<s,c=0,r=o*s;for(let g=0;g<r*6&&c<r;g++){let f=Math.floor(Math.random()*o),w=Math.floor(Math.random()*s),u=p[Math.floor(Math.random()*p.length)],b=3+Math.floor(Math.random()*16);for(let m=0;m<b;m++){if(!a(f,w))break;if(!l[w][f])l[w][f]=!0,c++,yield{x:f,y:w};if(Math.random()>0.72)u=p[Math.floor(Math.random()*p.length)];f+=u.x,w+=u.y}}for(let g=0;g<s;g++)for(let f=0;f<o;f++)if(!l[g][f])yield{x:f,y:g};break}case"SPIRAL_FROM_CENTER":case"SPIRAL_TO_CENTER":{let l=new Set,p=o*s,a=Math.floor(o/2),c=Math.floor(s/2),r=[[1,0],[0,1],[-1,0],[0,-1]],g=0,f=1,w=(b,m)=>b>=0&&b<o&&m>=0&&m<s,u=function*(){let b=0;while(b<p){for(let m=0;m<2;m++){for(let z=0;z<f;z++){if(w(a,c)){let H=`${a},${c}`;if(!l.has(H)){if(l.add(H),yield{x:a,y:c},b++,b>=p)return}}a+=r[g][0],c+=r[g][1]}g=(g+1)%4}f++}};if(this.strategy==="SPIRAL_FROM_CENTER")yield*u();else{let b=[...u()];for(let m=b.length-1;m>=0;m--)yield b[m]}break}case"SCRIBBLE":{let l=new Set,p=o*s,a=Math.floor(o/2),c=Math.floor(s/2);for(let r=0;l.size<p&&r<p*24;r++){let g=`${a},${c}`;if(!l.has(g))l.add(g),yield{x:a,y:c};if(a+=Math.floor(Math.random()*3)-1,c+=Math.floor(Math.random()*3)-1,a<0||a>=o||c<0||c>=s)a=Math.floor(Math.random()*o),c=Math.floor(Math.random()*s)}for(let r=0;r<s;r++)for(let g=0;g<o;g++){let f=`${g},${r}`;if(l.has(f))continue;l.add(f),yield{x:g,y:r}}break}case"CROSSHATCH":{let l=[];for(let c=0;c<o+s-1;c++)for(let r=Math.max(0,c-o+1);r<=Math.min(s-1,c);r++){let g=c-r;l.push({x:g,y:r})}let p=[];for(let c=-s+1;c<o;c++)for(let r=0;r<s;r++){let g=r+c;if(g>=0&&g<o)p.push({x:g,y:r})}let a=new Set;for(let c of[...l,...p]){let r=`${c.x},${c.y}`;if(a.has(r))continue;a.add(r),yield c}break}case"WAVE_SWEEP":{let l=new Set;for(let p=0;p<o;p++){let c=(Math.sin(p/Math.max(1,o-1)*Math.PI*4)+1)*0.5*(s-1)|0;for(let r=0;r<s;r++){let g=c+r,f=c-r;for(let w of[g,f]){if(w<0||w>=s)continue;let u=`${p},${w}`;if(l.has(u))continue;l.add(u),yield{x:p,y:w}}}}break}case"SCATTERED_LINES":{let l=new Set,p=o*s;for(let a=0;l.size<p&&a<p*14;a++){let c=Math.floor(Math.random()*o),r=Math.floor(Math.random()*s),g=Math.random()*Math.PI*2,f=Math.round(Math.cos(g)),w=Math.round(Math.sin(g)),u=6+Math.floor(Math.random()*28);for(let b=0;b<u;b++){if(c<0||c>=o||r<0||r>=s)break;let m=`${c},${r}`;if(!l.has(m))l.add(m),yield{x:c,y:r};c+=f,r+=w}}for(let a=0;a<s;a++)for(let c=0;c<o;c++){let r=`${c},${a}`;if(l.has(r))continue;l.add(r),yield{x:c,y:a}}break}case"CONTOUR_JITTER":{let l=new Set;for(let p=0;p<Math.ceil(Math.min(o,s)/2);p++){let a=[],c=p,r=p,g=o-p-1,f=s-p-1;for(let w=c;w<=g;w++)a.push({x:w,y:r});for(let w=r+1;w<=f;w++)a.push({x:g,y:w});for(let w=g-1;w>=c;w--)a.push({x:w,y:f});for(let w=f-1;w>r;w--)a.push({x:c,y:w});for(let w=a.length-1;w>0;w--){let u=Math.floor(Math.random()*(w+1)),b=a[w];a[w]=a[u],a[u]=b}for(let w of a){let u=`${w.x},${w.y}`;if(l.has(u))continue;l.add(u),yield w}}break}case"SPIRAL_WOBBLE":{let l=new Set,p=o/2,a=s/2,c=Math.hypot(p,a);for(let r=0;l.size<o*s&&r<o*s*9;r++){let g=r/(o*s*9)*c,f=r*0.31+Math.sin(r*0.07)*0.7,w=Math.round(p+Math.cos(f)*g),u=Math.round(a+Math.sin(f)*g);if(w<0||w>=o||u<0||u>=s)continue;let b=`${w},${u}`;if(l.has(b))continue;l.add(b),yield{x:w,y:u}}for(let r=0;r<s;r++)for(let g=0;g<o;g++){let f=`${g},${r}`;if(l.has(f))continue;yield{x:g,y:r}}break}case"CLUSTER_BURSTS":{let l=new Set,p=o*s;for(let a=0;l.size<p&&a<p*12;a++){let c=Math.floor(Math.random()*o),r=Math.floor(Math.random()*s),g=2+Math.floor(Math.random()*10);for(let f=r-g;f<=r+g;f++)for(let w=c-g;w<=c+g;w++){if(w<0||w>=o||f<0||f>=s)continue;if(Math.hypot(w-c,f-r)>g)continue;let u=`${w},${f}`;if(l.has(u))continue;l.add(u),yield{x:w,y:f}}}for(let a=0;a<s;a++)for(let c=0;c<o;c++){let r=`${c},${a}`;if(l.has(r))continue;l.add(r),yield{x:c,y:a}}break}case"ORBITAL":{let l=new Set,p=(o-1)/2,a=(s-1)/2,c=Math.ceil(Math.max(p,a));for(let r=0;r<=c;r++){let g=Math.max(16,Math.ceil(2*Math.PI*Math.max(1,r)*2));for(let f=0;f<g;f++){let w=f/g*Math.PI*2+(r%2?0.3:-0.3),u=Math.round(p+Math.cos(w)*r),b=Math.round(a+Math.sin(w)*r);if(u<0||u>=o||b<0||b>=s)continue;let m=`${u},${b}`;if(l.has(m))continue;l.add(m),yield{x:u,y:b}}}for(let r=0;r<s;r++)for(let g=0;g<o;g++){let f=`${g},${r}`;if(l.has(f))continue;yield{x:g,y:r}}break}case"FLOW_FIELD":{let l=new Set,p=o*s;for(let a=0;l.size<p&&a<p*18;a++){let c=Math.floor(Math.random()*o),r=Math.floor(Math.random()*s);for(let g=0;g<120;g++){if(c<0||c>=o||r<0||r>=s)break;let f=`${c},${r}`;if(!l.has(f))l.add(f),yield{x:c,y:r};let w=Math.sin(c*0.09)*1.8+Math.cos(r*0.08)*1.6+Math.sin((c+r)*0.05);c+=Math.round(Math.cos(w)),r+=Math.round(Math.sin(w))}}for(let a=0;a<s;a++)for(let c=0;c<o;c++){let r=`${c},${a}`;if(l.has(r))continue;l.add(r),yield{x:c,y:a}}break}case"EDGE_IN":{let l=new Set,p=Math.ceil(Math.min(o,s)/2);for(let a=0;a<p;a++){let c=a,r=o-1-a,g=a,f=s-1-a;for(let w=c;w<=r;w++)for(let u of[g,f]){let b=`${w},${u}`;if(l.has(b))continue;l.add(b),yield{x:w,y:u}}for(let w=g+1;w<=f-1;w++)for(let u of[c,r]){let b=`${u},${w}`;if(l.has(b))continue;l.add(b),yield{x:u,y:w}}}break}}}moveStart(o){if(o.button!==0)return;if(o.preventDefault(),o.stopPropagation(),!this.lock)this.moveInfo={globalX:this.position.globalX,globalY:this.position.globalY,clientX:o.clientX,clientY:o.clientY}}moveStop(){if(this.moveInfo)this.moveInfo=void 0,this.position.updateAnchor(),this.pixels.update(),this.updateColors(),J(this.bot)}move(o){if(!this.moveInfo)return;let s=Math.round((o.clientX-this.moveInfo.clientX)/this.position.pixelSize),l=Math.round((o.clientY-this.moveInfo.clientY)/this.position.pixelSize);if(this.moveInfo.globalX!==void 0){if(this.position.globalX=s+this.moveInfo.globalX,this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,this.moveInfo.width-s)}else if(this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,s+this.moveInfo.width);if(this.moveInfo.globalY!==void 0){if(this.position.globalY=l+this.moveInfo.globalY,this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,this.moveInfo.height-l)}else if(this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,l+this.moveInfo.height);this.update(),J(this.bot)}resizeStart(o){if(this.lock||o.button!==0)return;o.preventDefault(),o.stopPropagation(),this.moveInfo={clientX:o.clientX,clientY:o.clientY};let s=o.target;if(s.classList.contains("n"))this.moveInfo.height=this.pixels.height,this.moveInfo.globalY=this.position.globalY;if(s.classList.contains("e"))this.moveInfo.width=this.pixels.width;if(s.classList.contains("s"))this.moveInfo.height=this.pixels.height;if(s.classList.contains("w"))this.moveInfo.width=this.pixels.width,this.moveInfo.globalX=this.position.globalX}export(){let o=document.createElement("a");document.body.append(o),o.href=URL.createObjectURL(new Blob([JSON.stringify(this.toJSON())],{type:"application/json"})),o.download=`${this.pixels.width}x${this.pixels.height}.${t}`,o.click(),URL.revokeObjectURL(o.href),o.href=this.pixels.canvas.toDataURL("image/webp",1),o.download=`${this.pixels.width}x${this.pixels.height}.webp`,o.click(),URL.revokeObjectURL(o.href),o.remove()}}var Po=`/* stylelint-disable declaration-no-important */
...
`;class co extends Error{name="KGlacerMacroError";constructor(o,s){super(o);s.widget.status=o}}class uo extends co{name="NoImageError";constructor(o){super("❌ No image is selected",o)}}var k={toggleWidget:{key:"b",shift:!0},minimizeWidget:{key:"m",shift:!0},showWidgetPanel:{key:"s",shift:!0},hideWidgetPanel:{key:"h",shift:!0},toggleOverlay:{key:"v",shift:!0},draw:{key:"enter",shift:!0},addImage:{key:"i",shift:!0},showShortcuts:{key:"/",shift:!0},focusNextImage:{key:"n",shift:!0},focusPreviousImage:{key:"p",shift:!0},openColorPanel:{key:"o",shift:!0},toggleImageLock:{key:"l",shift:!0}};function D(o,s){let l=s.key.toLowerCase(),p=o.key.toLowerCase(),c=l==="/"&&(p==="/"||p==="?"||o.code==="Slash")||p===l,r=s.ctrl===!0?o.ctrlKey||o.metaKey:!o.ctrlKey,g=s.ctrl===!0?!0:s.meta===!0?o.metaKey:!o.metaKey;return c&&o.shiftKey===Boolean(s.shift)&&r&&g&&o.altKey===Boolean(s.alt)}function Go(o){if(typeof HTMLElement==="undefined")return!1;if(!(o instanceof HTMLElement))return!1;let s=o.tagName.toLowerCase();return s==="input"||s==="textarea"||o.isContentEditable||o.closest('[contenteditable="true"]')!==null}var Ko=`<button class="wopen-button" aria-label="Toggle widget">
...
`;var Qo="kglacer-macro:overlay-hidden",Eo="https://raw.githubusercontent.com/robgallardof/kglacer-macro/refs/heads/main/src/img/logo.svg";class bo extends v{bot;element=document.createElement("div");get status(){return this.$status.innerHTML}set status(o){this.$status.innerHTML=o}get open(){return this.element.classList.contains("wopen")}set open(o){if(o)this.element.classList.add("wopen");else this.element.classList.remove("wopen")}$settings;$status;$shortcuts;$locale;$topbar;$draw;$addImage;$captureTemplate;$toggleOverlay;$strategy;$progressLine;$progressText;$images;$wopenButton;$widgetLogo;activeImageIndex=-1;constructor(o){super();this.bot=o;this.element.classList.add("wwidget"),this.element.innerHTML=Ko,F(this.element),document.body.append(this.element),this.populateElementsWithSelector(this.element,{$wopenButton:".wopen-button",$widgetLogo:".widget-logo",$settings:".wform",$status:".wstatus",$shortcuts:".shortcuts",$locale:".locale",$topbar:".wtopbar",$draw:".draw",$addImage:".add-image",$captureTemplate:".capture-template",$toggleOverlay:".toggle-overlay",$strategy:".strategy",$progressLine:".wprogress div",$progressText:".wprogress span",$images:".images"}),this.$widgetLogo.src=Eo,this.$wopenButton.addEventListener("click",()=>this.open=!this.open),this.$draw.addEventListener("click",()=>this.bot.draw()),this.$addImage.addEventListener("click",()=>this.addImage()),this.$captureTemplate.addEventListener("click",()=>{this.captureTemplate()}),this.$toggleOverlay.addEventListener("click",()=>{this.toggleOverlay()}),this.$strategy.addEventListener("change",()=>{this.bot.strategy=this.$strategy.value}),this.$locale.value=I(),this.$locale.addEventListener("change",()=>{so(this.$locale.value),F(this.element);for(let s=0;s<this.bot.images.length;s++)this.bot.images[s].applyLocale();this.refreshOverlayToggleText()}),this.registerEvent(document,"keydown",this.handleKeyboard.bind(this),{passive:!1}),this.update(),this.syncOverlayVisibilityFromStorage(),this.open=!0,console.log("[KGM][Widget] Widget mounted and opened")}addImage(){return console.log("[KGM][Widget] Add image flow started"),this.setDisabled("add-image",!0),this.run("Adding image",async()=>{await this.bot.updateColors();let o=document.createElement("input");o.type="file",o.accept=`image/*,.${t}`,o.click(),await i(o,["change"],["cancel","error"]);let s=o.files?.[0];if(!s)throw new uo(this.bot);console.log("[KGM][Widget] File selected",{name:s.name,size:s.size,type:s.type});let l;if(s.name.endsWith(`.${t}`))l=await Y.fromJSON(this.bot,JSON.parse(await s.text()));else{let p=new FileReader;p.readAsDataURL(s),await i(p,["load"],["error"]);let a=await this.compressImageBeforeLoad(p.result),c=new Image;c.src=a,await i(c,["load"],["error"]),l=new Y(this.bot,K.fromScreenPosition(this.bot,this.defaultImageScreenPosition()),new V(this.bot,c))}this.bot.images.push(l),console.log("[KGM][Widget] Image instance added",{images:this.bot.images.length}),await this.bot.readMap(),l.updateTasks(),J(this.bot,!0),this.bot.updateTasks(),this.update()},()=>{this.setDisabled("add-image",!1)})}captureTemplate(){return this.setDisabled("capture-template",!0),this.run("Capturing map image",async()=>{let o=this.askCaptureFormat(),s=await this.selectCaptureBounds(),l=K.fromScreenPosition(this.bot,{x:s.left,y:s.top}),p=K.fromScreenPosition(this.bot,{x:s.left+s.width-1,y:s.top+s.height-1}),a=Math.min(l.globalX,p.globalX),c=Math.min(l.globalY,p.globalY),r=Math.max(l.globalX,p.globalX),g=Math.max(l.globalY,p.globalY),f=document.createElement("canvas");f.width=Math.max(1,r-a+1),f.height=Math.max(1,g-c+1);let w=f.getContext("2d");if(!w)throw new Error("Capture context unavailable");w.imageSmoothingEnabled=!1;let u=Math.floor(a/n),b=Math.floor(c/n),m=Math.floor(r/n),z=Math.floor(g/n),H=(m-u+1)*(z-b+1),M=0;for(let P=u;P<=m;P++)for(let G=b;G<=z;G++){this.status=`⌛ Reading tiles [${++M}/${H}]`;let Q=await this.loadTileImage(P,G),L=P*n,T=G*n,E=Math.max(a,L),B=Math.min(r,L+n-1),S=Math.max(c,T),e=Math.min(g,T+n-1),_=E-L,W=S-T,y=B-E+1,oo=e-S+1,ao=E-a,lo=S-c;w.drawImage(Q,_,W,y,oo,ao,lo,y,oo)}let U=o==="webp"?"image/webp":"image/png",A=await new Promise((P,G)=>{f.toBlob((Q)=>{if(!Q){G(new Error("Failed to create capture file"));return}P(Q)},U)}),h=URL.createObjectURL(A),N=document.createElement("a");N.href=h,N.download=`wplace-capture-${Date.now()}.${o}`,N.click(),URL.revokeObjectURL(h)},()=>{this.setDisabled("capture-template",!1)})}askCaptureFormat(){let o=window.prompt(d("captureFormatPrompt"),"png")?.trim().toLowerCase();if(!o)return"png";if(o==="png"||o==="webp")return o;return"png"}async loadTileImage(o,s){let l=await fetch(`https://backend.wplace.live/files/s0/tiles/${o}/${s}.png`,{credentials:"include"});if(!l.ok)throw new Error(`Tile fetch failed (${o}/${s})`);let p=await l.blob(),a=new Image,c=URL.createObjectURL(p);return a.src=c,await i(a,["load"],["error"]),URL.revokeObjectURL(c),a}selectCaptureBounds(){return new Promise((o,s)=>{let l=document.createElement("div");l.className="kgm-capture-overlay",l.innerHTML=`<div class="kgm-capture-hint">${d("captureTemplate")}: A → B</div><div class="kgm-capture-box"></div>`;let p=l.querySelector(".kgm-capture-box");document.body.append(l);let a,c=()=>{window.removeEventListener("keydown",u,!0),l.removeEventListener("pointermove",f),l.removeEventListener("pointerdown",w),l.remove()},r=(b)=>{let m=Math.min(a.x,b.x),z=Math.min(a.y,b.y),H=Math.abs(a.x-b.x),M=Math.abs(a.y-b.y);return{left:m,top:z,width:H,height:M}},g=(b)=>{let{left:m,top:z,width:H,height:M}=r(b);p.style.left=`${m}px`,p.style.top=`${z}px`,p.style.width=`${H}px`,p.style.height=`${M}px`},f=(b)=>{if(!a)return;g({x:b.clientX,y:b.clientY})},w=(b)=>{if(b.preventDefault(),!a){a={x:b.clientX,y:b.clientY},g(a);return}let m=r({x:b.clientX,y:b.clientY});if(c(),m.width<2||m.height<2){s(new Error("Capture area too small"));return}o(m)},u=(b)=>{if(b.key!=="Escape")return;c(),s(new Error("Capture cancelled"))};window.addEventListener("keydown",u,!0),l.addEventListener("pointermove",f),l.addEventListener("pointerdown",w)})}defaultImageScreenPosition(){let o=Math.round(this.element.getBoundingClientRect().width);return{x:Math.max(256,o),y:32}}async compressImageBeforeLoad(o){let s=new Image;if(s.src=o,await i(s,["load"],["error"]),!(s.naturalWidth*s.naturalHeight>3000000||o.length>3000000))return o;let p=document.createElement("canvas");p.width=s.naturalWidth,p.height=s.naturalHeight;let a=p.getContext("2d");if(!a)return o;return a.drawImage(s,0,0),p.toDataURL("image/png")}update(){this.$strategy.value=this.bot.strategy;let o=0,s=0;for(let c=0;c<this.bot.images.length;c++){let r=this.bot.images[c];o+=r.pixels.pixels.length*r.pixels.pixels[0].length,s+=r.tasks.length}let l=Math.max(0,o-s),p=o>0?l/o*100|0:0;this.$progressText.textContent=`${l}/${o} ${p}% ETA: ${s/120|0}h`,this.$progressLine.style.transform=`scaleX(${p/100})`,this.$images.innerHTML="";let a=document.createDocumentFragment();for(let c=0;c<this.bot.images.length;c++){let r=this.bot.images[c],g=document.createElement("div");a.append(g),g.className="image",g.innerHTML=`<button class="preview" title="View preview">
  ...
  </div>`,g.querySelector(".preview").addEventListener("click",()=>{this.activeImageIndex=c,r.openPreviewPanel()}),g.querySelector(".colors").addEventListener("click",()=>{this.activeImageIndex=c,r.openColorPanel()}),g.querySelector(".preview-strategy").addEventListener("click",()=>{this.activeImageIndex=c,r.openPreviewPanel()}),g.querySelector(".download").addEventListener("click",()=>{r.exportImage()}),g.querySelector(".delete").addEventListener("click",()=>{r.destroy()}),g.querySelector(".up").addEventListener("click",()=>{go(this.bot.images,c,c-1),this.update(),J(this.bot)}),g.querySelector(".down").addEventListener("click",()=>{go(this.bot.images,c,c+1),this.update(),J(this.bot)})}this.$images.append(a)}syncOverlayVisibilityFromStorage(){let o=localStorage.getItem(Qo)==="true";document.body.classList.toggle("overlay-hidden",o),this.refreshOverlayToggleText()}toggleOverlay(o){let s=o??!document.body.classList.contains("overlay-hidden");document.body.classList.toggle("overlay-hidden",s),localStorage.setItem(Qo,String(s)),this.refreshOverlayToggleText()}refreshOverlayToggleText(){this.$toggleOverlay.textContent=document.body.classList.contains("overlay-hidden")?`${d("toggleOverlay")} (${d("disabled")})`:`${d("toggleOverlay")} (${d("enabled")})`}setDisabled(o,s){this.element.querySelector("."+o).disabled=s}async run(o,s,l,p="..."){console.log("[KGM][Widget] Task started",{status:o});let a=this.status;this.status=`${p} ${o}`;try{let c=await s();return this.status=a,console.log("[KGM][Widget] Task completed",{status:o}),c}catch(c){if(!(c instanceof co))console.error(c),this.status=`Error: ${o}`;throw console.error("[KGM][Widget] Task failed",{status:o,error:c}),c}finally{await l?.()}}handleKeyboard(o){if(Go(o.target))return;if(D(o,k.toggleWidget)){o.preventDefault(),this.open=!this.open;return}if(D(o,k.showShortcuts)){o.preventDefault(),this.open=!0,this.$shortcuts.open=!0,this.$shortcuts.scrollIntoView({behavior:"smooth",block:"center"}),this.$shortcuts.classList.remove("shortcut-pulse"),requestAnimationFrame(()=>{this.$shortcuts.classList.add("shortcut-pulse")});return}if(D(o,k.toggleOverlay)){o.preventDefault(),this.toggleOverlay();return}if(D(o,k.focusNextImage)){o.preventDefault(),this.focusImageByStep(1);return}if(D(o,k.focusPreviousImage)){o.preventDefault(),this.focusImageByStep(-1);return}if(D(o,k.openColorPanel)){o.preventDefault(),this.openColorPanelForActiveImage();return}if(D(o,k.toggleImageLock)){o.preventDefault(),this.toggleLockForActiveImage();return}if(D(o,k.addImage)&&!this.$addImage.disabled){o.preventDefault(),this.addImage();return}if(D(o,k.draw)&&!this.$draw.disabled)o.preventDefault(),this.bot.draw()}focusImageByStep(o){if(!this.bot.images.length)return;if(this.activeImageIndex<0||this.activeImageIndex>=this.bot.images.length)this.activeImageIndex=o>0?0:this.bot.images.length-1;else this.activeImageIndex=(this.activeImageIndex+o+this.bot.images.length)%this.bot.images.length;this.bot.images[this.activeImageIndex].position.scrollScreenTo()}getActiveImage(){if(!this.bot.images.length)return;if(this.activeImageIndex<0||this.activeImageIndex>=this.bot.images.length)this.activeImageIndex=0;return this.bot.images[this.activeImageIndex]}openColorPanelForActiveImage(){let o=this.getActiveImage();if(!o)return;o.openColorPanel()}toggleLockForActiveImage(){let o=this.getActiveImage();if(!o)return;o.lock=!o.lock,o.update(),J(this.bot)}}var So=2,ko="[KGM]",Do="kglacer-macro:access-ok",mo="S0dNLXlZUjhTMW81bEhVemVjS1RFMEhxRVB4OVFkcjgxaEVz",qo="kgm-access-locked";class Fo{unavailableColors=new Set;mapsCache=new Map;me;$stars=[];strategy="SEQUENTIAL";images=[];_widget;get widget(){if(!this._widget)throw new Error("Widget is not initialized yet");return this._widget}markerPixelPositionResolvers=[];lastColor;log(o,s){if(s===void 0)console.log(`${ko} ${o}`);else console.log(`${ko} ${o}`,s)}constructor(){this.log("Boot sequence started"),document.body.classList.add(qo);let o=ho();if(this.log("Save loaded",{hasSave:Boolean(o),imageCount:o?.images.length??0,strategy:o?.strategy}),o){for(let l=0;l<o.images.length;l++){let p=o.images[l];x({x:p.position[0]-1000,y:p.position[1]-1000}),x({x:p.position[0]+1000,y:p.position[1]+1000})}this.strategy=o.strategy}this.registerFetchInterceptor(),this.log("Fetch interceptor registered");let s=document.createElement("style");s.textContent=Po.replace("FAKE_FAVORITE_LOCATIONS",$.length.toString()),document.head.append(s),this.log("Styles injected",{fakeFavoriteLocations:$.length}),(async()=>{this.log("Widget initialization flow started"),await this.ensureAccessKey(),document.body.classList.remove(qo),this._widget=new bo(this),await this.widget.run("Initializing",async()=>{await this.waitForElement("login",".avatar.center-absolute.absolute"),await this.waitForElement("pixel count",".btn.btn-primary.btn-lg.relative.z-30 canvas");let l=await this.waitForElement("canvas",".maplibregl-canvas-container");if(new MutationObserver((p)=>{for(let a=0;a<p.length;a++)if(p[a].removedNodes.length!==0){this.updateStars();break}this.updateImages()}).observe(l,{attributes:!0,childList:!0,subtree:!0}),this.updateStars(),this.log("Stars updated after boot",{stars:this.$stars.length}),await q(500),await this.updateColors(),o)for(let p=0;p<o.images.length;p++){let a=await Y.fromJSON(this,o.images[p]);this.images.push(a),a.update()}this.log("Saved images restored",{images:this.images.length}),await this.readMap(),this.updateTasks(),this.widget.setDisabled("draw",!1),this.widget.setDisabled("add-image",!1),this.widget.setDisabled("capture-template",!1),this.log("Initialization completed; controls enabled")})})()}async ensureAccessKey(){if(localStorage.getItem(Do)===mo)return;await new Promise((o)=>{let s=document.createElement("dialog");s.className="kgm-modal access-dialog",s.innerHTML=`<form method="dialog" class="access-form">
...
</form>`,document.body.append(s),F(s);let l=s.querySelector(".access-input"),p=s.querySelector(".access-error"),a=s.querySelector(".access-locale");a.innerHTML=Ho().map((c)=>`<option value="${c}" ${c===I()?"selected":""}>${c.toUpperCase()}</option>`).join(""),a.addEventListener("change",()=>{so(a.value),F(s)}),s.addEventListener("cancel",(c)=>{c.preventDefault()}),s.querySelector("form").addEventListener("submit",(c)=>{c.preventDefault();let r=atob(mo);if(l.value.trim()!==r){p.textContent=d("invalidAccessKey");return}localStorage.setItem(Do,mo),s.close(),s.remove(),o()}),s.showModal(),l.focus()})}draw(){this.log("Draw requested",{strategy:this.strategy,images:this.images.length}),this.widget.setDisabled("draw",!0),this.widget.status="",this.mapsCache.clear();let o=document.querySelector(".maplibregl-canvas"),s=(l)=>{if(!l.shiftKey)l.stopPropagation()};return this.widget.run("Drawing",async()=>{await this.widget.run("Initializing draw",()=>Promise.all([this.updateColors(),this.readMap()])),globalThis.addEventListener("mousemove",s,!0),o.addEventListener("wheel",s,!0),this.updateTasks();let l=await fetch("https://backend.wplace.live/me",{credentials:"include"}).then((c)=>c.json()),p=Math.floor(l.charges.count);this.log("Charges fetched",{charges:p});let a=0;for(let c=0;c<this.images.length;c++)a+=this.images[c].tasks.length;switch(this.log("Tasks prepared",{tasks:a}),this.strategy){case"ALL":{while(p>0){let c=!0;for(let r=0;r<this.images.length;r++){let g=this.images[r].tasks.shift();if(!g)continue;this.drawTask(g),p--,await q(1),c=!1}if(c)break}break}case"PERCENTAGE":{for(let c=0;c<a&&p>0;c++){let r=1,g;for(let f=0;f<this.images.length;f++){let w=this.images[f],u=1-w.tasks.length/(w.pixels.pixels.length*w.pixels.pixels[0].length);if(u<r)r=u,g=w}this.drawTask(g.tasks.shift()),p--,await q(1)}break}case"SEQUENTIAL":for(let c=0;c<this.images.length;c++){let r=this.images[c];for(let g=r.tasks.shift();g&&p>0;g=r.tasks.shift())this.drawTask(g),p--,await q(1)}}this.widget.update(),this.log("Draw flow finished",{remainingCharges:p})},()=>{globalThis.removeEventListener("mousemove",s,!0),o.removeEventListener("wheel",s,!0),this.widget.setDisabled("draw",!1)})}toJSON(){return{version:So,images:this.images.map((o)=>o.toJSON()),strategy:this.strategy}}async updateColors(){this.log("Updating colors palette"),await this.openColors(),this.unavailableColors.clear();for(let o of document.querySelectorAll("button.btn.relative.w-full"))if(o.children.length!==0)this.unavailableColors.add(Math.abs(Number.parseInt(o.id.slice(6))));this.updateImageColors(),this.log("Colors updated",{unavailableColors:this.unavailableColors.size})}moveMap(o){let s=document.querySelector(".maplibregl-canvas"),l=window.innerWidth/2,p=window.innerHeight/2,a=l-o.x,c=p-o.y;function r(g,f,w){s.dispatchEvent(new MouseEvent(g,{bubbles:!0,cancelable:!0,clientX:f,clientY:w,buttons:1}))}r("mousedown",l,p),r("mousemove",a,c),r("mouseup",a,c)}readMap(){this.mapsCache.clear();let o=new Set;for(let l=0;l<this.images.length;l++){let p=this.images[l],{tileX:a,tileY:c}=new K(this,p.position.globalX+p.pixels.pixels[0].length,p.position.globalY+p.pixels.pixels.length);for(let r=p.position.tileX;r<=a;r++)for(let g=p.position.tileY;g<=c;g++)o.add(`${r}/${g}`)}let s=0;return this.log("Reading map tiles",{tileCount:o.size}),this.widget.run(`Reading map [0/${o.size}]`,()=>Promise.all([...o].map(async(l)=>{this.mapsCache.set(l,await V.fromJSON(this,{url:`https://backend.wplace.live/files/s0/tiles/${l}.png`,exactColor:!0})),this.widget.status=`⌛ Reading map [${++s}/${o.size}]`})))}waitForUnfocus(){return this.widget.run("UNFOCUS WINDOW",()=>new Promise((o)=>{if(!document.hasFocus())o();window.addEventListener("blur",()=>{setTimeout(o,1)},{once:!0})}),void 0,"\uD83D\uDDB1️")}findAnchorsForScreen(o){let s=0,l=1,p=1/0,a=1/0;for(let g=0;g<this.$stars.length;g++){let{x:f,y:w}=C(this.$stars[g]);if(f<o.x&&w<o.y){let u=o.x-f+(o.y-w);if(u<p)p=u,s=g}else if(f>o.x&&w>o.y){let u=f-o.x+(w-o.y);if(u<a)a=u,l=g}}let c=C(this.$stars[s]),r=j[s];return{anchorScreenPosition:c,anchorWorldPosition:r,pixelSize:(C(this.$stars[l]).x-c.x)/(j[l].x-r.x)}}async openColors(){this.lastColor=void 0,document.querySelector(".flex.gap-2.px-3 > .btn-circle")?.click(),await q(1),document.querySelector(".btn.btn-primary.btn-lg.relative.z-30")?.click(),await q(1);let o=document.querySelector("button.bottom-0");if(o?.innerHTML==='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor" class="size-5"><path d="M480-120 300-300l58-58 122 122 122-122 58 58-180 180ZM358-598l-58-58 180-180 180 180-58 58-122-122-122 122Z"></path></svg><!---->')o.click(),await q(1)}drawTask(o){if(this.lastColor!==o.color)document.getElementById("color-"+o.color).click(),this.lastColor=o.color,this.log("Color switched for draw task",{color:o.color});let s=o.position.pixelSize/2,l=o.position.toScreenPosition();document.documentElement.dispatchEvent(new MouseEvent("mousemove",{bubbles:!0,clientX:l.x+s,clientY:l.y+s,shiftKey:!0})),document.documentElement.dispatchEvent(new KeyboardEvent("keydown",{key:" ",code:"Space",keyCode:32,which:32,bubbles:!0,cancelable:!0})),document.documentElement.dispatchEvent(new KeyboardEvent("keyup",{key:" ",code:"Space",keyCode:32,which:32,bubbles:!0,cancelable:!0}))}registerFetchInterceptor(){let o=globalThis.fetch,s=/https:\/\/backend.wplace.live\/s\d+\/pixel\/(-?\d+)\/(-?\d+)\?x=(-?\d+)&y=(-?\d+)/;globalThis.fetch=async(l,p)=>{let a=await o(l,p),c=a.clone(),r="";if(typeof l=="string")r=l;else if(l instanceof Request)r=l.url;else if(l instanceof URL)r=l.href;if(a.url==="https://backend.wplace.live/me")this.me=await c.json(),this.me.favoriteLocations.unshift(...$),this.me.maxFavoriteLocations=1/0,a.json=()=>Promise.resolve(this.me),this.log("Patched /me response with favorite locations",{totalFavorites:this.me.favoriteLocations.length});let g=s.exec(r);if(g){for(let f=0;f<this.markerPixelPositionResolvers.length;f++)this.markerPixelPositionResolvers[f](new K(this,+g[1],+g[2],+g[3],+g[4]));this.markerPixelPositionResolvers.length=0,this.log("Resolved marker pixel position from network event")}return a}}async closeAll(){for(let o of document.querySelectorAll("button"))if(o.innerHTML==="✕"||o.innerHTML==='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor" class="size-4"><path d="m256-200-56-56 224-224-224-224 56-56 224 224 224-224 56 56-224 224 224 224-56 56-224-224-224 224Z"></path></svg><!---->')o.click(),await q(1)}waitForElement(o,s){return this.log("Waiting for element",{name:o,selector:s}),this.widget.run(`Waiting for ${o}`,()=>{return new Promise((l)=>{let p=document.querySelector(s);if(p){l(p);return}let a=new MutationObserver(()=>{let c=document.querySelector(s);if(c)a.disconnect(),l(c)});a.observe(document.documentElement,{childList:!0,subtree:!0})})})}updateStars(){this.$stars=[...document.querySelectorAll(".text-yellow-400.cursor-pointer.z-10.maplibregl-marker.maplibregl-marker-anchor-center")].slice(0,$.length),this.log("Star cache updated",{stars:this.$stars.length})}updateImages(){for(let o=0;o<this.images.length;o++)this.images[o].position.updateAnchor(),this.images[o].update()}updateTasks(){for(let o=0;o<this.images.length;o++)this.images[o].updateTasks()}updateImageColors(){for(let o=0;o<this.images.length;o++)this.images[o].updateColors()}}globalThis.kglacerMacro=new Fo;globalThis.kgm=globalThis.kglacerMacro;globalThis.wbot=globalThis.kglacerMacro;
