...
`;var ko="kglacer-macro:overlay-hidden",Fo="kglacer-macro:auto-farm-config",Eo="https://raw.githubusercontent.com/robgallardof/kglacer-macro/refs/heads/main/src/img/logo.svg";class fo extends X{bot;element=document.createElement("div");get status(){return this.$status.innerHTML}set status(o){this.$status.innerHTML=o}get open(){return this.element.classList.contains("wopen")}set open(o){if(o)this.element.classList.add("wopen");else this.element.classList.remove("wopen")}$settings;$status;$shortcuts;$locale;$topbar;$draw;$drawAndPaint;$addImage;$captureTemplate;$toggleOverlay;$autofarmConfig;$autofarmStatus;$strategy;$progressLine;$progressText;$images;$wopenButton;$widgetLogo;activeImageIndex=-1;autoFarmIntervalId;autoFarmConfig;autoFarmTickRunning=!1;constructor(o){super();this.bot=o;this.element.classList.add("wwidget"),this.element.innerHTML=No,Q(this.element),document.body.append(this.element),this.populateElementsWithSelector(this.element,{$wopenButton:".wopen-button",$widgetLogo:".widget-logo",$settings:".wform",$status:".wstatus",$shortcuts:".shortcuts",$locale:".locale",$topbar:".wtopbar",$draw:".draw",$drawAndPaint:".draw-and-paint",$addImage:".add-image",$captureTemplate:".capture-template",$toggleOverlay:".toggle-overlay",$autofarmConfig:".autofarm-config",$autofarmStatus:".autofarm-status",$strategy:".strategy",$progressLine:".wprogress div",$progressText:".wprogress span",$images:".images"}),this.$widgetLogo.src=Eo,this.$wopenButton.addEventListener("click",()=>this.open=!this.open),this.$draw.addEventListener("click",()=>this.bot.draw()),this.$drawAndPaint.addEventListener("click",()=>{this.drawAndClickPaintWhenReady()}),this.$addImage.addEventListener("click",()=>this.addImage()),this.$captureTemplate.addEventListener("click",()=>{this.captureTemplate()}),this.$toggleOverlay.addEventListener("click",()=>{this.toggleOverlay()}),this.$autofarmConfig.addEventListener("click",()=>{this.openAutoFarmModal()}),this.$strategy.addEventListener("change",()=>{this.bot.strategy=this.$strategy.value}),this.$locale.value=_(),this.$locale.addEventListener("change",()=>{oo(this.$locale.value),Q(this.element);for(let p=0;p<this.bot.images.length;p++)this.bot.images[p].applyLocale();this.refreshOverlayToggleText(),this.refreshAutoFarmStatusText()}),this.registerEvent(document,"keydown",this.handleKeyboard.bind(this),{passive:!1}),this.update(),this.syncOverlayVisibilityFromStorage(),this.loadAutoFarmConfigFromStorage(),this.refreshAutoFarmStatusText(),this.open=!0,console.log("[KGM][Widget] Widget mounted and opened")}addImage(){return console.log("[KGM][Widget] Add image flow started"),this.setDisabled("add-image",!0),this.run(n("taskAddingImage"),async()=>{await this.bot.updateColors();let o=document.createElement("input");o.type="file",o.accept=`image/*,.${O}`,o.click(),await q(o,["change"],["cancel","error"]);let p=o.files?.[0];if(!p)throw new go(this.bot);console.log("[KGM][Widget] File selected",{name:p.name,size:p.size,type:p.type});let s;if(p.name.endsWith(`.${O}`))s=await j.fromJSON(this.bot,JSON.parse(await p.text()));else{let l=new FileReader;l.readAsDataURL(p),await q(l,["load"],["error"]);let a=await this.compressImageBeforeLoad(l.result),c=new Image;c.src=a,await q(c,["load"],["error"]),await this.waitForStableViewportProjection(),s=new j(this.bot,N.fromScreenPosition(this.bot,this.defaultImageScreenPosition()),new W(this.bot,c))}this.bot.images.push(s),console.log("[KGM][Widget] Image instance added",{images:this.bot.images.length}),await this.bot.readMap(),s.updateTasks(),h(this.bot,!0),this.bot.updateTasks(),this.update(),globalThis.location.reload()},()=>{this.setDisabled("add-image",!1)})}captureTemplate(){return this.setDisabled("capture-template",!0),this.run(n("taskCapturingMapImage"),async()=>{let o=await this.resolveCaptureBounds(),{minGlobalX:p,minGlobalY:s,maxGlobalX:l,maxGlobalY:a}=o,c=document.createElement("canvas");c.width=Math.max(1,l-p+1),c.height=Math.max(1,a-s+1);let r=c.getContext("2d");if(!r)throw new Error("Capture context unavailable");r.imageSmoothingEnabled=!1;let g=Math.floor(p/M),f=Math.floor(s/M),u=Math.floor(l/M),w=Math.floor(a/M),d=(u-g+1)*(w-f+1),b=0;for(let m=g;m<=u;m++)for(let H=f;H<=w;H++){this.status=`⌛ ${n("taskReadingTiles")} [${++b}/${d}]`;let z=await this.loadTileImage(m,H),t=m*M,A=H*M,P=Math.max(p,t),k=Math.min(l,t+M-1),D=Math.max(s,A),Z=Math.min(a,A+M-1),E=P-t,I=D-A,Y=k-P+1,K=Z-D+1,v=P-p,x=D-s;r.drawImage(z,E,I,Y,K,v,x,Y,K)}let i=Date.now();await this.downloadCapture(c,"png",i)},()=>{this.setDisabled("capture-template",!1)})}async downloadCapture(o,p,s){let l=p==="webp"?"image/webp":"image/png",a=await new Promise((g,f)=>{o.toBlob((u)=>{if(!u){f(new Error(`Failed to create ${p.toUpperCase()} capture file`));return}g(u)},l)}),c=URL.createObjectURL(a),r=document.createElement("a");r.href=c,r.download=`wplace-capture-${s}.${p}`,r.click(),URL.revokeObjectURL(c)}async loadTileImage(o,p){let s;for(let l=1;l<=3;l++)try{let a=new Image;return a.crossOrigin="anonymous",a.referrerPolicy="no-referrer",a.src=`https://backend.wplace.live/files/s0/tiles/${o}/${p}.png?ts=${Date.now()}-${l}`,await q(a,["load"],["error"]),a}catch(a){if(s=a,l<3)await new Promise((c)=>setTimeout(c,l*200))}throw s instanceof Error?s:new Error(`Tile fetch failed (${o}/${p})`)}async resolveCaptureBounds(){return this.selectCaptureBounds()}selectCaptureBounds(){return new Promise((o,p)=>{let s=document.createElement("div");s.className="kgm-capture-overlay",s.innerHTML=`<div class="kgm-capture-hint">${n("captureHintSelectArea")}: A → B</div><div class="kgm-capture-box"></div>`;let l=s.querySelector(".kgm-capture-box");document.body.append(s);let a,c,r=()=>{window.removeEventListener("keydown",d,!0),s.removeEventListener("pointermove",u),s.removeEventListener("pointerdown",w),s.remove()},g=(b)=>{let i=Math.min(a.x,b.x),m=Math.min(a.y,b.y),H=Math.abs(a.x-b.x)+1,z=Math.abs(a.y-b.y)+1;return{left:i,top:m,width:H,height:z}},f=(b)=>{let{left:i,top:m,width:H,height:z}=g(b);l.style.left=`${i}px`,l.style.top=`${m}px`,l.style.width=`${H}px`,l.style.height=`${z}px`},u=(b)=>{if(!a)return;f({x:b.clientX,y:b.clientY})},w=(b)=>{if(b.preventDefault(),!a){a={x:b.clientX,y:b.clientY};let P=N.fromScreenPosition(this.bot,a);c={x:P.globalX,y:P.globalY},f(a);return}let i={x:b.clientX,y:b.clientY},m=N.fromScreenPosition(this.bot,i);if(r(),!c){p(new Error("Capture anchor point unavailable"));return}let H=Math.min(c.x,m.globalX),z=Math.min(c.y,m.globalY),t=Math.max(c.x,m.globalX),A=Math.max(c.y,m.globalY);if(t-H<1||A-z<1){p(new Error("Capture area too small"));return}o({minGlobalX:H,minGlobalY:z,maxGlobalX:t,maxGlobalY:A})},d=(b)=>{if(b.key!=="Escape")return;r(),p(new Error("Capture cancelled"))};window.addEventListener("keydown",d,!0),s.addEventListener("pointermove",u),s.addEventListener("pointerdown",w)})}defaultImageScreenPosition(){let o=Math.round(this.element.getBoundingClientRect().width);return{x:Math.max(256,o),y:32}}async compressImageBeforeLoad(o){let p=new Image;if(p.src=o,await q(p,["load"],["error"]),!(p.naturalWidth*p.naturalHeight>3000000||o.length>3000000))return o;let l=document.createElement("canvas");l.width=p.naturalWidth,l.height=p.naturalHeight;let a=l.getContext("2d");if(!a)return o;return a.drawImage(p,0,0),l.toDataURL("image/png")}async waitForStableViewportProjection(){let o=this.defaultImageScreenPosition(),p=0,s;for(let l=0;l<45;l++){await new Promise((u)=>requestAnimationFrame(()=>{u()}));let{anchorScreenPosition:{x:a,y:c},pixelSize:r}=this.bot.findAnchorsForScreen(o);if(!Number.isFinite(r)||r<=0){p=0;continue}let g={anchorX:a,anchorY:c,pixelSize:r};if(!s){s=g,p=1;continue}if(Math.abs(g.anchorX-s.anchorX)+Math.abs(g.anchorY-s.anchorY)+Math.abs(g.pixelSize-s.pixelSize)<0.0012)p++;else p=0;if(s=g,p>=3)return}}update(){this.$strategy.value=this.bot.strategy;let o=0,p=0;for(let c=0;c<this.bot.images.length;c++){let r=this.bot.images[c];o+=r.pixels.pixels.length*r.pixels.pixels[0].length,p+=r.tasks.length}let s=Math.max(0,o-p),l=o>0?s/o*100|0:0;this.$progressText.textContent=`${s}/${o} ${l}% ETA: ${p/120|0}h`,this.$progressLine.style.transform=`scaleX(${l/100})`,this.$images.innerHTML="";let a=document.createDocumentFragment();for(let c=0;c<this.bot.images.length;c++){let r=this.bot.images[c],g=document.createElement("div");a.append(g),g.className="image",g.innerHTML=`<button class="preview" title="View preview">
  ...
  </div>`,g.querySelector(".preview").addEventListener("click",()=>{this.activeImageIndex=c,r.openPreviewPanel()}),g.querySelector(".colors").addEventListener("click",()=>{this.activeImageIndex=c,r.openColorPanel()}),g.querySelector(".preview-strategy").addEventListener("click",()=>{this.activeImageIndex=c,r.openPreviewPanel()}),g.querySelector(".download").addEventListener("click",()=>{r.exportImage()}),g.querySelector(".delete").addEventListener("click",()=>{r.destroy()}),g.querySelector(".up").addEventListener("click",()=>{lo(this.bot.images,c,c-1),this.update(),h(this.bot)}),g.querySelector(".down").addEventListener("click",()=>{lo(this.bot.images,c,c+1),this.update(),h(this.bot)})}this.$images.append(a)}syncOverlayVisibilityFromStorage(){let o=localStorage.getItem(ko)==="true";document.body.classList.toggle("overlay-hidden",o),this.refreshOverlayToggleText()}toggleOverlay(o){let p=o??!document.body.classList.contains("overlay-hidden");document.body.classList.toggle("overlay-hidden",p),localStorage.setItem(ko,String(p)),this.refreshOverlayToggleText()}refreshOverlayToggleText(){this.$toggleOverlay.textContent=document.body.classList.contains("overlay-hidden")?`${n("toggleOverlay")} (${n("disabled")})`:`${n("toggleOverlay")} (${n("enabled")})`}refreshAutoFarmStatusText(){if(!this.autoFarmConfig){this.$autofarmStatus.textContent=n("autoFarmNeedsConfig");return}this.$autofarmStatus.textContent=this.autoFarmIntervalId?`${n("autoFarmRunning")} (${this.formatAutoFarmDelay(this.autoFarmConfig.timerMs)})`:n("autoFarmStopped")}formatAutoFarmDelay(o){if(o%3600000===0)return`${o/3600000}h`;if(o%60000===0)return`${o/60000}m`;return`${o/1000}s`}stopAutoFarm(){if(!this.autoFarmIntervalId)return;clearInterval(this.autoFarmIntervalId),this.autoFarmIntervalId=void 0,this.refreshAutoFarmStatusText()}startAutoFarm(){if(!this.autoFarmConfig){this.status=`⚠️ ${n("autoFarmNeedsConfig")}`,this.refreshAutoFarmStatusText();return}this.stopAutoFarm(),this.autoFarmIntervalId=window.setInterval(()=>{this.runAutoFarmCycle()},this.autoFarmConfig.timerMs),this.runAutoFarmCycle(),this.refreshAutoFarmStatusText()}async runAutoFarmCycle(){if(!this.autoFarmConfig||this.autoFarmTickRunning)return;this.autoFarmTickRunning=!0;try{if(await this.bot.drawTransparentPixelsBatch(this.autoFarmConfig.pixels)<=0){this.status=`⚠️ ${n("autoFarmNoTransparentTasks")}`;return}await this.waitAndClickPaintButton()}finally{this.autoFarmTickRunning=!1}}saveAutoFarmConfig(o){this.autoFarmConfig=o,localStorage.setItem(Fo,JSON.stringify(o))}loadAutoFarmConfigFromStorage(){let o=localStorage.getItem(Fo);if(!o)return;try{let p=JSON.parse(o);if(typeof p.value!=="number"||!Number.isFinite(p.value)||p.value<1)return;let s=typeof p.pixels==="number"&&Number.isFinite(p.pixels)&&p.pixels>=1?Math.floor(p.pixels):60,l=p.unit==="hours"||p.unit==="minutes"||p.unit==="seconds"?p.unit:"minutes",a=typeof p.timerMs==="number"&&p.timerMs>0?p.timerMs:l==="hours"?p.value*3600000:l==="minutes"?p.value*60000:p.value*1000;this.autoFarmConfig={value:Math.max(1,Math.floor(p.value)),pixels:s,unit:l,timerMs:a}}catch{return}}openAutoFarmModal(){let o=document.createElement("dialog");o.className="kgm-modal autofarm-dialog";let p=this.autoFarmConfig?.unit??"minutes",s=this.autoFarmConfig?.value??1,l=this.autoFarmConfig?.pixels??60;o.innerHTML=`<form method="dialog" class="autofarm-form">
