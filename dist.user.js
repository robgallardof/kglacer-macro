...
<span class="premium"></span>`,d.querySelector(".swatch").style.setProperty("--swatch-color",go(l.realColor)),d.addEventListener("click",()=>{if(i){i=!1;return}f(),this.updateTasks()}),d.addEventListener("dragstart",(b)=>{i=!0,d.classList.add("dragging"),b.dataTransfer?.setData("text/plain",String(r)),b.dataTransfer.effectAllowed="move"}),d.addEventListener("dragend",()=>{d.classList.remove("dragging")}),d.addEventListener("dragover",(b)=>{b.preventDefault(),d.classList.add("drag-target")}),d.addEventListener("dragleave",()=>{d.classList.remove("drag-target")}),d.addEventListener("drop",(b)=>{b.preventDefault(),d.classList.remove("drag-target");let u=Number.parseInt(b.dataTransfer?.getData("text/plain")??"-1",10);if(u<0||u===r||u>=this.colors.length)return;this.colors.splice(r,0,...this.colors.splice(u,1)),A(this.bot),this.updateColors()});let w=document.createElement("button");w.textContent=e("buy"),w.className="buy-chip",w.addEventListener("click",(b)=>{b.stopPropagation(),document.getElementById("color-"+l.realColor)?.click()}),d.append(w);let m=`${p} ${t.join(" ")} ${l.realColor} ${q[l.realColor]}`;if(!a||m.toLowerCase().includes(a))this.$colorsDialogList.append(d)}}syncColorBulkToggle(){let o=this.colors.filter((r)=>!r.disabled).length,a=o===this.colors.length;this.$toggleAllColors.checked=a,this.$toggleAllColors.indeterminate=o>0&&!a}*strategyPositionIterator(){let o=this.pixels.pixels[0].length,a=this.pixels.pixels.length;switch(this.strategy){case"DOWN":{for(let r=0;r<a;r++)for(let s=0;s<o;s++)yield{x:s,y:r};break}case"UP":{for(let r=a-1;r>=0;r--)for(let s=0;s<o;s++)yield{x:s,y:r};break}case"LEFT":{for(let r=0;r<o;r++)for(let s=0;s<a;s++)yield{x:r,y:s};break}case"RIGHT":{for(let r=o-1;r>=0;r--)for(let s=0;s<a;s++)yield{x:r,y:s};break}case"RANDOM":{let r=[];for(let s=0;s<a;s++)for(let l=0;l<o;l++)r.push({x:l,y:s});for(let s=r.length-1;s>=0;s--){let l=Math.floor(Math.random()*(s+1)),i=r[s];r[s]=r[l],r[l]=i}yield*r;break}case"ZIGZAG":{for(let r=0;r<a;r++)if(r%2===0)for(let s=0;s<o;s++)yield{x:s,y:r};else for(let s=o-1;s>=0;s--)yield{x:s,y:r};break}case"HUMANIZED":{let r=Math.max(4,Math.floor(Math.min(o,a)/10)),s=[];for(let l=0;l<a;l+=r)for(let i=0;i<o;i+=r)s.push({x:i,y:l});for(let l=s.length-1;l>=0;l--){let i=Math.floor(Math.random()*(l+1)),c=s[l];s[l]=s[i],s[i]=c}for(let l=0;l<s.length;l++){let i=s[l],c=Math.min(a,i.y+r),p=Math.min(o,i.x+r);for(let t=i.y;t<c;t++)if(Math.random()>0.35)for(let g=i.x;g<p;g++)yield{x:g,y:t};else for(let g=p-1;g>=i.x;g--)yield{x:g,y:t}}break}case"HUMAN_SOFT_DITHER":{let r=new Set;for(let s=0;s<a;s++){let l=Math.floor(Math.random()*3)-1;if((s+l)%2===0)for(let c=0;c<o;c+=2)r.add(`${c},${s}`),yield{x:c,y:s};else for(let c=1;c<o;c+=2)r.add(`${c},${s}`),yield{x:c,y:s}}for(let s=0;s<a;s++)for(let l=0;l<o;l++){let i=`${l},${s}`;if(r.has(i))continue;yield{x:l,y:s}}break}case"HUMAN_PATCHY":{let r=new Set,s=o*a;while(r.size<s){let l=Math.floor(Math.random()*o),i=Math.floor(Math.random()*a),c=1+Math.floor(Math.random()*5);for(let p=i-c;p<=i+c;p++)for(let t=l-c;t<=l+c;t++){if(t<0||t>=o||p<0||p>=a)continue;if(Math.hypot(t-l,p-i)>c+Math.random()*1.2)continue;let n=`${t},${p}`;if(r.has(n))continue;r.add(n),yield{x:t,y:p}}}break}case"HUMAN_SWEEP_ARCS":{let r=new Set,s=(o-1)/2,l=(a-1)/2,i=Math.hypot(s,l);for(let c=0;c<4;c++){let p=Math.random()*Math.PI*2;for(let t=0;t<=i;t+=0.35){let n=Math.PI/2+Math.random()*(Math.PI/1.5),g=Math.max(10,Math.floor(t*8));for(let f=0;f<g;f++){let d=p+n*f/g+Math.sin(t)*0.08,w=Math.round(s+Math.cos(d)*t),m=Math.round(l+Math.sin(d)*t);if(w<0||w>=o||m<0||m>=a)continue;let b=`${w},${m}`;if(r.has(b))continue;r.add(b),yield{x:w,y:m}}}}for(let c=0;c<a;c++)for(let p=0;p<o;p++){let t=`${p},${c}`;if(r.has(t))continue;yield{x:p,y:c}}break}case"HUMAN_MICRO_CORRECTIONS":{let r=new Set;for(let s=0;s<a;s++){let l=s%2===0?1:-1,i=l>0?0:o-1;for(let c=0;c<o;c++){let p=i+(Math.random()>0.82?l:0),t=s+(Math.random()>0.9?1:0);for(let n of[{x:i,y:s},{x:p,y:s},{x:i,y:t}]){if(n.x<0||n.x>=o||n.y<0||n.y>=a)continue;let g=`${n.x},${n.y}`;if(r.has(g))continue;r.add(g),yield n}i+=l}}for(let s=0;s<a;s++)for(let l=0;l<o;l++){let i=`${l},${s}`;if(r.has(i))continue;yield{x:l,y:s}}break}case"HUMAN_JITTER_FILL":{let r=[];for(let s=0;s<a;s++)for(let l=0;l<o;l++)r.push({x:l,y:s});r.sort((s,l)=>{let i=s.y+(Math.random()-0.5)*1.8,c=l.y+(Math.random()-0.5)*1.8;if(i!==c)return i-c;return s.x+(Math.random()-0.5)*2-(l.x+(Math.random()-0.5)*2)}),yield*r;break}case"HUMAN_CORNER_BIAS":{let r=[{x:0,y:0},{x:o-1,y:0},{x:0,y:a-1},{x:o-1,y:a-1}],s=r[Math.floor(Math.random()*r.length)],l=[];for(let i=0;i<a;i++)for(let c=0;c<o;c++){let t=Math.hypot(c-s.x,i-s.y)+Math.random()*3.5;l.push({point:{x:c,y:i},score:t})}l.sort((i,c)=>i.score-c.score);for(let i of l)yield i.point;break}case"HUMAN_LONG_STROKES":{let r=new Set,s=o*a;while(r.size<s){let l=Math.floor(Math.random()*o),i=Math.floor(Math.random()*a),c=Math.random()*Math.PI*2,p=Math.sign(Math.cos(c)),t=Math.sign(Math.sin(c)),n=10+Math.floor(Math.random()*40);for(let g=0;g<n;g++){if(l<0||l>=o||i<0||i>=a)break;let f=`${l},${i}`;if(!r.has(f))r.add(f),yield{x:l,y:i};if(Math.random()>0.78)l+=t,i+=p;else l+=p,i+=t}}break}case"HUMAN_TAP_CLUSTERS":{let r=new Set,s=o*a;while(r.size<s){let l=Math.floor(Math.random()*o),i=Math.floor(Math.random()*a),c=3+Math.floor(Math.random()*10);for(let p=0;p<c;p++){let t=Math.round(l+(Math.random()-0.5)*6),n=Math.round(i+(Math.random()-0.5)*6);if(t<0||t>=o||n<0||n>=a)continue;let g=`${t},${n}`;if(r.has(g))continue;r.add(g),yield{x:t,y:n}}}break}case"HUMAN_MESSY_SPIRAL":{let r=new Set,s=(o-1)/2,l=(a-1)/2,i=Math.hypot(s,l)+2;for(let c=0;r.size<o*a;c++){let p=c/3,t=Math.min(i,p*0.18),n=p*0.29+Math.sin(p*0.13)*0.8,g=Math.round(s+Math.cos(n)*t+Math.sin(p)*0.7),f=Math.round(l+Math.sin(n)*t+Math.cos(p)*0.7);if(g<0||g>=o||f<0||f>=a){if(c>o*a*18)break;continue}let d=`${g},${f}`;if(r.has(d)){if(Math.random()>0.9)continue}else r.add(d),yield{x:g,y:f};if(c>o*a*18)break}for(let c=0;c<a;c++)for(let p=0;p<o;p++){let t=`${p},${c}`;if(r.has(t))continue;yield{x:p,y:c}}break}case"HUMAN_DRUNK_WALK":{let r=new Set,s=Math.floor(Math.random()*o),l=Math.floor(Math.random()*a),i=[{x:-1,y:0},{x:1,y:0},{x:0,y:-1},{x:0,y:1},{x:-1,y:-1},{x:1,y:-1},{x:-1,y:1},{x:1,y:1}];while(r.size<o*a){let c=`${s},${l}`;if(!r.has(c))r.add(c),yield{x:s,y:l};let p=[];for(let g of i){let f=s+g.x,d=l+g.y;if(f<0||f>=o||d<0||d>=a)continue;p.push({x:f,y:d})}if(!p.length)break;let t=p.filter((g)=>{return!r.has(`${g.x},${g.y}`)});if(t.length&&Math.random()>0.2){let g=t[Math.floor(Math.random()*t.length)];s=g.x,l=g.y;continue}let n=p[Math.floor(Math.random()*p.length)];s=n.x,l=n.y}for(let c=0;c<a;c++)for(let p=0;p<o;p++){let t=`${p},${c}`;if(r.has(t))continue;yield{x:p,y:c}}break}case"HUMAN_NOISE_CLOUD":{let r=[];for(let s=0;s<a;s++)for(let l=0;l<o;l++){let i=Math.sin((l+1)*0.93+Math.random()*0.8)+Math.cos((s+1)*1.17+Math.random()*0.8),c=(Math.random()-0.5)*2.6,p=Math.hypot(l-o/2,s-a/2)*0.08;r.push({point:{x:l,y:s},score:i+c+p})}r.sort((s,l)=>s.score-l.score);for(let s of r)yield s.point;break}case"HUMAN_PATCH_JUMP":{let r=new Set,s=[];for(let l=0;l<Math.max(6,o*a/18);l++)s.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*a)});while(r.size<o*a){let l=s[Math.floor(Math.random()*s.length)],i=1+Math.floor(Math.random()*3),c=1+Math.floor(Math.random()*3);for(let p=l.y-c;p<=l.y+c;p++)for(let t=l.x-i;t<=l.x+i;t++){if(t<0||t>=o||p<0||p>=a)continue;if(Math.random()>0.86)continue;let n=`${t},${p}`;if(r.has(n))continue;r.add(n),yield{x:t,y:p}}if(Math.random()>0.72&&s.length<o*a/2)s.push({x:Math.floor(Math.random()*o),y:Math.floor(Math.random()*a)});if(r.size>o*a*0.92)break}for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=`${i},${l}`;if(r.has(c))continue;yield{x:i,y:l}}break}case"HUMAN_HESITANT_LINES":{let r=new Set;for(let s=0;s<a;s++){let l=s%2===0;for(let i=0;i<o;i++){let c=l?i:o-1-i,p=`${c},${s}`;if(!r.has(p))r.add(p),yield{x:c,y:s};if(Math.random()>0.7){let t=Math.max(0,Math.min(o-1,c+(Math.random()>0.5?1:-1))),n=Math.max(0,Math.min(a-1,s+(Math.random()>0.65?1:0))),g=`${t},${n}`;if(!r.has(g))r.add(g),yield{x:t,y:n}}}}for(let s=0;s<a;s++)for(let l=0;l<o;l++){let i=`${l},${s}`;if(r.has(i))continue;yield{x:l,y:s}}break}case"HUMAN_OVERLAP_SWEEPS":{let r=[],s=Math.random()*Math.PI*2;for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=Math.sin((i+l)*0.42+s)*2.2,p=Math.cos((i-l)*0.3+s)*1.4;r.push({point:{x:i,y:l},score:l+c+p+(Math.random()-0.5)*3.4})}r.sort((l,i)=>l.score-i.score);for(let l of r)yield l.point;break}case"HUMAN_WOBBLE_DRIFT":{let r=[],s=o/2,l=a/2;for(let i=0;i<a;i++)for(let c=0;c<o;c++){let p=Math.hypot(c-s,i-l)*0.25,t=Math.sin((c+1)*0.9)*1.8+Math.cos((i+1)*1.1)*1.8+Math.sin((c+i)*0.35)*1.4;r.push({point:{x:c,y:i},score:p+t+(Math.random()-0.5)*2.8})}r.sort((i,c)=>i.score-c.score);for(let i of r)yield i.point;break}case"HUMAN_GAP_RECOVERY":{let r=new Set,s=[];for(let l=0;l<a;l++)for(let i=0;i<o;i++){if(Math.random()>0.87){s.push({x:i,y:l});continue}r.add(`${i},${l}`),yield{x:i,y:l}}s.sort((l,i)=>Math.hypot(l.x-o/2,l.y-a/2)-Math.hypot(i.x-o/2,i.y-a/2));for(let l of s){let i=`${l.x},${l.y}`;if(r.has(i))continue;r.add(i),yield l}break}case"HUMAN_STAIRCASE":{let r=new Set,s=o+a-1;for(let l=0;l<s;l++){let i=Math.max(0,l-o+1),c=Math.min(a-1,l);for(let p=i;p<=c;p++){let t=l-p,n=[{x:t,y:p},{x:t+(Math.random()>0.5?1:-1),y:p},{x:t,y:p+(Math.random()>0.5?1:-1)}];for(let g of n){if(g.x<0||g.x>=o||g.y<0||g.y>=a)continue;let f=`${g.x},${g.y}`;if(r.has(f))continue;r.add(f),yield g}}}for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=`${i},${l}`;if(r.has(c))continue;yield{x:i,y:l}}break}case"HUMAN_EDGE_HUGGER":{let r=[];for(let s=0;s<a;s++)for(let l=0;l<o;l++){let i=Math.min(l,s,o-1-l,a-1-s);r.push({point:{x:l,y:s},score:i*3.5+(Math.random()-0.5)*5.5})}r.sort((s,l)=>s.score-l.score);for(let s of r)yield s.point;break}case"HUMAN_BLOBS":{let r=new Set,s=o*a;while(r.size<s){let l=Math.floor(Math.random()*o),i=Math.floor(Math.random()*a),c=1+Math.floor(Math.random()*4);for(let p=i-c;p<=i+c;p++)for(let t=l-c;t<=l+c;t++){if(t<0||t>=o||p<0||p>=a)continue;let n=Math.atan2(p-i,t-l),g=c+Math.sin(n*3+Math.random())*0.8;if(Math.hypot(t-l,p-i)>g)continue;let f=`${t},${p}`;if(r.has(f))continue;r.add(f),yield{x:t,y:p}}}break}case"HUMAN_BACKTRACK":{let r=new Set,s=[];for(let l=0;l<a;l++)for(let i=0;i<o;i++)s.push({x:i,y:l});s.sort((l,i)=>l.y-i.y+(Math.random()-0.5)*2.2+(l.x-i.x)*0.04);for(let l=0;l<s.length;l++){let i=s[l],c=`${i.x},${i.y}`;if(r.has(c))continue;if(r.add(c),yield i,l>1&&Math.random()>0.74){let p=s[l-1],t=`${p.x},${p.y}`;if(!r.has(t))r.add(t),yield p}}for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=`${i},${l}`;if(r.has(c))continue;yield{x:i,y:l}}break}case"HUMAN_SHAKY_DIAGONAL":{let r=[];for(let s=0;s<a;s++)for(let l=0;l<o;l++){let i=Math.abs(l-s)*0.6,c=Math.sin(l*1.4+s*0.8)*1.8+Math.cos(s*1.1+l*0.5)*1.5;r.push({point:{x:l,y:s},score:i+c+(Math.random()-0.5)*3.2})}r.sort((s,l)=>s.score-l.score);for(let s of r)yield s.point;break}case"HUMAN_LATE_FIXES":{let r=[],s=[];for(let l=0;l<a;l++)for(let i=0;i<o;i++)if(Math.random()>0.9)s.push({x:i,y:l});else r.push({x:i,y:l});r.sort((l,i)=>l.y-i.y+(Math.random()-0.5)*1.5+(Math.random()>0.85?l.x-i.x:0)),s.sort((l,i)=>Math.hypot(i.x-o/2,i.y-a/2)-Math.hypot(l.x-o/2,l.y-a/2)),yield*r,yield*s;break}case"DIAGONAL_BRUSH":{for(let r=0;r<o+a-1;r++){let s=r%2===0,l=[],i=Math.max(0,r-o+1),c=Math.min(a-1,r);for(let p=i;p<=c;p++){let t=r-p;if(t>=0&&t<o)l.push({x:t,y:p})}if(Math.random()>0.55)l.reverse();if(s)for(let p=l.length-1;p>=0;p--)yield l[p];else yield*l}break}case"BRUSH_STROKES":{let r=Array.from({length:a},()=>Array(o).fill(!1)),s=[{x:1,y:0},{x:-1,y:0},{x:0,y:1},{x:0,y:-1},{x:1,y:1},{x:1,y:-1},{x:-1,y:1},{x:-1,y:-1}],l=(p,t)=>p>=0&&p<o&&t>=0&&t<a,i=0,c=o*a;for(let p=0;p<c*6&&i<c;p++){let t=Math.floor(Math.random()*o),n=Math.floor(Math.random()*a),g=s[Math.floor(Math.random()*s.length)],f=3+Math.floor(Math.random()*16);for(let d=0;d<f;d++){if(!l(t,n))break;if(!r[n][t])r[n][t]=!0,i++,yield{x:t,y:n};if(Math.random()>0.72)g=s[Math.floor(Math.random()*s.length)];t+=g.x,n+=g.y}}for(let p=0;p<a;p++)for(let t=0;t<o;t++)if(!r[p][t])yield{x:t,y:p};break}case"SPIRAL_FROM_CENTER":case"SPIRAL_TO_CENTER":{let r=new Set,s=o*a,l=Math.floor(o/2),i=Math.floor(a/2),c=[[1,0],[0,1],[-1,0],[0,-1]],p=0,t=1,n=(f,d)=>f>=0&&f<o&&d>=0&&d<a,g=function*(){let f=0;while(f<s){for(let d=0;d<2;d++){for(let w=0;w<t;w++){if(n(l,i)){let m=`${l},${i}`;if(!r.has(m)){if(r.add(m),yield{x:l,y:i},f++,f>=s)return}}l+=c[p][0],i+=c[p][1]}p=(p+1)%4}t++}};if(this.strategy==="SPIRAL_FROM_CENTER")yield*g();else{let f=[...g()];for(let d=f.length-1;d>=0;d--)yield f[d]}break}case"SCRIBBLE":{let r=new Set,s=o*a,l=Math.floor(o/2),i=Math.floor(a/2);for(let c=0;r.size<s&&c<s*24;c++){let p=`${l},${i}`;if(!r.has(p))r.add(p),yield{x:l,y:i};if(l+=Math.floor(Math.random()*3)-1,i+=Math.floor(Math.random()*3)-1,l<0||l>=o||i<0||i>=a)l=Math.floor(Math.random()*o),i=Math.floor(Math.random()*a)}for(let c=0;c<a;c++)for(let p=0;p<o;p++){let t=`${p},${c}`;if(r.has(t))continue;r.add(t),yield{x:p,y:c}}break}case"CROSSHATCH":{let r=[];for(let i=0;i<o+a-1;i++)for(let c=Math.max(0,i-o+1);c<=Math.min(a-1,i);c++){let p=i-c;r.push({x:p,y:c})}let s=[];for(let i=-a+1;i<o;i++)for(let c=0;c<a;c++){let p=c+i;if(p>=0&&p<o)s.push({x:p,y:c})}let l=new Set;for(let i of[...r,...s]){let c=`${i.x},${i.y}`;if(l.has(c))continue;l.add(c),yield i}break}case"WAVE_SWEEP":{let r=new Set;for(let s=0;s<o;s++){let i=(Math.sin(s/Math.max(1,o-1)*Math.PI*4)+1)*0.5*(a-1)|0;for(let c=0;c<a;c++){let p=i+c,t=i-c;for(let n of[p,t]){if(n<0||n>=a)continue;let g=`${s},${n}`;if(r.has(g))continue;r.add(g),yield{x:s,y:n}}}}break}case"SCATTERED_LINES":{let r=new Set,s=o*a;for(let l=0;r.size<s&&l<s*14;l++){let i=Math.floor(Math.random()*o),c=Math.floor(Math.random()*a),p=Math.random()*Math.PI*2,t=Math.round(Math.cos(p)),n=Math.round(Math.sin(p)),g=6+Math.floor(Math.random()*28);for(let f=0;f<g;f++){if(i<0||i>=o||c<0||c>=a)break;let d=`${i},${c}`;if(!r.has(d))r.add(d),yield{x:i,y:c};i+=t,c+=n}}for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=`${i},${l}`;if(r.has(c))continue;r.add(c),yield{x:i,y:l}}break}case"CONTOUR_JITTER":{let r=new Set;for(let s=0;s<Math.ceil(Math.min(o,a)/2);s++){let l=[],i=s,c=s,p=o-s-1,t=a-s-1;for(let n=i;n<=p;n++)l.push({x:n,y:c});for(let n=c+1;n<=t;n++)l.push({x:p,y:n});for(let n=p-1;n>=i;n--)l.push({x:n,y:t});for(let n=t-1;n>c;n--)l.push({x:i,y:n});for(let n=l.length-1;n>0;n--){let g=Math.floor(Math.random()*(n+1)),f=l[n];l[n]=l[g],l[g]=f}for(let n of l){let g=`${n.x},${n.y}`;if(r.has(g))continue;r.add(g),yield n}}break}case"SPIRAL_WOBBLE":{let r=new Set,s=o/2,l=a/2,i=Math.hypot(s,l);for(let c=0;r.size<o*a&&c<o*a*9;c++){let p=c/(o*a*9)*i,t=c*0.31+Math.sin(c*0.07)*0.7,n=Math.round(s+Math.cos(t)*p),g=Math.round(l+Math.sin(t)*p);if(n<0||n>=o||g<0||g>=a)continue;let f=`${n},${g}`;if(r.has(f))continue;r.add(f),yield{x:n,y:g}}for(let c=0;c<a;c++)for(let p=0;p<o;p++){let t=`${p},${c}`;if(r.has(t))continue;yield{x:p,y:c}}break}case"CLUSTER_BURSTS":{let r=new Set,s=o*a;for(let l=0;r.size<s&&l<s*12;l++){let i=Math.floor(Math.random()*o),c=Math.floor(Math.random()*a),p=2+Math.floor(Math.random()*10);for(let t=c-p;t<=c+p;t++)for(let n=i-p;n<=i+p;n++){if(n<0||n>=o||t<0||t>=a)continue;if(Math.hypot(n-i,t-c)>p)continue;let g=`${n},${t}`;if(r.has(g))continue;r.add(g),yield{x:n,y:t}}}for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=`${i},${l}`;if(r.has(c))continue;r.add(c),yield{x:i,y:l}}break}case"ORBITAL":{let r=new Set,s=(o-1)/2,l=(a-1)/2,i=Math.ceil(Math.max(s,l));for(let c=0;c<=i;c++){let p=Math.max(16,Math.ceil(2*Math.PI*Math.max(1,c)*2));for(let t=0;t<p;t++){let n=t/p*Math.PI*2+(c%2?0.3:-0.3),g=Math.round(s+Math.cos(n)*c),f=Math.round(l+Math.sin(n)*c);if(g<0||g>=o||f<0||f>=a)continue;let d=`${g},${f}`;if(r.has(d))continue;r.add(d),yield{x:g,y:f}}}for(let c=0;c<a;c++)for(let p=0;p<o;p++){let t=`${p},${c}`;if(r.has(t))continue;yield{x:p,y:c}}break}case"FLOW_FIELD":{let r=new Set,s=o*a;for(let l=0;r.size<s&&l<s*18;l++){let i=Math.floor(Math.random()*o),c=Math.floor(Math.random()*a);for(let p=0;p<120;p++){if(i<0||i>=o||c<0||c>=a)break;let t=`${i},${c}`;if(!r.has(t))r.add(t),yield{x:i,y:c};let n=Math.sin(i*0.09)*1.8+Math.cos(c*0.08)*1.6+Math.sin((i+c)*0.05);i+=Math.round(Math.cos(n)),c+=Math.round(Math.sin(n))}}for(let l=0;l<a;l++)for(let i=0;i<o;i++){let c=`${i},${l}`;if(r.has(c))continue;r.add(c),yield{x:i,y:l}}break}case"EDGE_IN":{let r=new Set,s=Math.ceil(Math.min(o,a)/2);for(let l=0;l<s;l++){let i=l,c=o-1-l,p=l,t=a-1-l;for(let n=i;n<=c;n++)for(let g of[p,t]){let f=`${n},${g}`;if(r.has(f))continue;r.add(f),yield{x:n,y:g}}for(let n=p+1;n<=t-1;n++)for(let g of[i,c]){let f=`${g},${n}`;if(r.has(f))continue;r.add(f),yield{x:g,y:n}}}break}}}moveStart(o){if(o.button!==0)return;if(o.preventDefault(),o.stopPropagation(),!this.lock)this.moveInfo={globalX:this.position.globalX,globalY:this.position.globalY,clientX:o.clientX,clientY:o.clientY}}moveStop(){if(this.moveInfo)this.moveInfo=void 0,this.position.updateAnchor(),this.pixels.update(),this.updateColors(),A(this.bot)}move(o){if(!this.moveInfo)return;let a=Math.round((o.clientX-this.moveInfo.clientX)/this.position.pixelSize),r=Math.round((o.clientY-this.moveInfo.clientY)/this.position.pixelSize);if(this.moveInfo.globalX!==void 0){if(this.position.globalX=a+this.moveInfo.globalX,this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,this.moveInfo.width-a)}else if(this.moveInfo.width!==void 0)this.pixels.width=Math.max(1,a+this.moveInfo.width);if(this.moveInfo.globalY!==void 0){if(this.position.globalY=r+this.moveInfo.globalY,this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,this.moveInfo.height-r)}else if(this.moveInfo.height!==void 0)this.pixels.height=Math.max(1,r+this.moveInfo.height);this.update(),A(this.bot)}resizeStart(o){if(this.lock||o.button!==0)return;o.preventDefault(),o.stopPropagation(),this.moveInfo={clientX:o.clientX,clientY:o.clientY};let a=o.target;if(a.classList.contains("n"))this.moveInfo.height=this.pixels.height,this.moveInfo.globalY=this.position.globalY;if(a.classList.contains("e"))this.moveInfo.width=this.pixels.width;if(a.classList.contains("s"))this.moveInfo.height=this.pixels.height;if(a.classList.contains("w"))this.moveInfo.width=this.pixels.width,this.moveInfo.globalX=this.position.globalX}export(){let o=document.createElement("a");document.body.append(o),o.href=URL.createObjectURL(new Blob([JSON.stringify(this.toJSON())],{type:"application/json"})),o.download=`${this.pixels.width}x${this.pixels.height}.${X}`,o.click(),URL.revokeObjectURL(o.href),o.href=this.pixels.canvas.toDataURL("image/webp",1),o.download=`${this.pixels.width}x${this.pixels.height}.webp`,o.click(),URL.revokeObjectURL(o.href),o.remove()}}function eo(){let o=localStorage.getItem("kglacer-macro:shield-config");if(!o)return!1;try{return JSON.parse(o).enabled!==!1}catch{return!1}}function No(o){localStorage.setItem("kglacer-macro:shield-config",JSON.stringify({enabled:o}))}function ga(o){let a=`${o?.host??""} ${o?.username??""}`.toLowerCase(),r=/(mx|mex|mexico)/.test(a)?"MX":"AUTO";localStorage.setItem("__afm_proxy_hint",r)}function Do(o){if(!eo())return;if(document.getElementById("kgm-shield-full"))return;ga(o);let a=document.createElement("script");a.id="kgm-shield-full",a.textContent=`// ==UserScript==
